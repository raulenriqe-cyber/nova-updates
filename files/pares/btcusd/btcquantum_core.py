...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for BTCUSD')
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        # 🎯 RELAXED THRESHOLDS FOR BTC - Allow more attacks
        ...
        # NOTE: LLM6 runs as TCP server on port 5004, we'll query it via socket
        self.llm6_port = 5004  # LLM6 NOVA Smart Money Oracle port (hardcoded)
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('BTCUSD')
                    if news_conf > 60:
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('BTCUSD')
    ...
    def calculate_confluence_score(self, 
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For BTCUSD at $2600, 0.5% = $13 which is reasonable for fib zone
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 VETO POR PROBABILIDAD MUY BAJA - ULTRA RELAXED FOR BTC
            # Si la probabilidad es <12%, NO atacar bajo ningún concepto
            # ⭐ RELAXED: De 22% a 12% para permitir más ataques en BTC
            # ═══════════════════════════════════════════════════════════════
            ...
            if len(closes) >= 5:
                ...
                pct_threshold = 0.03  # 0.03% = ~$0.78 para BTCUSD, ~$0.0003 para EURUSD
            ...
            # Esto hace el código compatible con BTCUSD, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            if len(closes) >= 3 and closes[-3] > 0:
                ...
                # Umbral: 0.08% = fuerte contra (BTCUSD: ~$2.10, EURUSD: ~$0.0009)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🧠 TIERS BASADOS EN CONCIENCIA - RELAXED FOR BTC SCALPING
            # Combina Score + Consciousness Probability + Calc Entry Score
            # ⭐ RELAXED: Thresholds bajados para permitir más ataques en BTC
            # ═══════════════════════════════════════════════════════════════
            ...
            # 🎯 TIER 5: Trinity Force - Relajado para BTC
            ...
            # Para BTC scalping, Trinity sabe lo que hace
            # ═══════════════════════════════════════════════════════════════
...
class NOVAMarketStateDetector:
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        elif spread > 0.5:  # For BTCUSD/EURUSD, abnormally wide
...
class Config:
    HOST = '127.0.0.1'
    QUANTUM_PORT = 8854      # Injector - receive genomes from Quimera
    EXECUTOR_PORT = 8856     # Executor - send commands to Quimera (MATCHES config.yaml)
    TRINITY_PORT = 8866  # Trinity server port (matches btcconfig.yaml)
    KRAKEN_PORT = 8852
    LLM1_PORT = 8855
    LLM2_PORT = 8857
    LLM3_PORT = 8858
    LLM4_PORT = 8859
    LLM5_PORT = 8861  # LLM5 SUPREME chart analyzer
    LLM6_PORT = 5004  # ⭐ LLM6 NOVA Smart Money Oracle
    LLM11_PORT = 8867        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 8868        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 8870  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # Trading - SNIPER SCALPING M1 (Crypto Predator Style)
    # Strategy: Wait 120s between trades, HIGH quality setups only, quick profits
    # 🔧 FIX: For M1 BTCUSD, 1 pip = $0.10, so 10 pips = $1.00 move
    ...
    MIN_CONFIDENCE = 55              # 🎯 RELAXED: 55%+ confidence for BTC scalping (was 78 - too restrictive)
    MIN_TRINITY_CONFIDENCE = 10      # MINIMAL: 10%+ required - Trinity/Kraken decide, Quimera executes
    MIN_ADX = 20                     # RELAXED: allow more trades in low-trend BTC (was 32 - too restrictive)
    ...
    # Network
    SOCKET_TIMEOUT = 3.5  # 🔧 FIX: Increased from 2.0 to 3.5 — MAGNETIC_BITCOIN Trinity latency ~2112ms was causing 100% timeouts → 0 decisions
    BUFFER_SIZE = 65536
    ...
    # ⚔️ STRATEGY SELECTOR (loaded from btcconfig.yaml)
    ACTIVE_STRATEGY = "MAGNETIC_BITCOIN"  # Default, will be overridden by yaml

    @classmethod
    def load_strategy_from_yaml(cls):
        """Load active strategy from btcconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'btcconfig.yaml')
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    config = yaml.safe_load(f)
                strategy = config.get('active_strategy', 'MAGNETIC_BITCOIN').upper()
                cls.ACTIVE_STRATEGY = strategy
                print(f"[CONFIG] ⚔️ BTCUSD strategy loaded: {strategy}")
                return strategy
        except Exception as e:
            print(f"[CONFIG] Error loading BTCUSD strategy: {e} - using default MAGNETIC_BITCOIN")
        return 'MAGNETIC_BITCOIN'
...
class NOVADashboard:
    ...
    def render(self):
        ...
        ║  by Polarice Labs © 2026 | Crypto Predator Precision                         ║
        ╚════════════════════════════════════════════════════════════════════════════╝
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'BTCUSD'))
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}NOVA{self.RESET} Sniper v18.00 {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
...
class AdaptiveTPSLEngine:
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
                                     support_resistance=None, spread=0.10, closes=None):
        """
        Calculate INTELLIGENT TP/SL for M1 BTCUSD using ALL available data:
        ...
        # Ensure minimum ATR for BTCUSD M1
        # 🔧 FIX: M1 ATR typically 0.3-1.5 (in price), clamp to realistic range
        atr = max(0.30, min(2.0, atr))  # Clamp ATR 0.30-2.0 for M1 safety
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 BTCUSD typical ATR: 0.3-1.5 (in price = 3-15 pips)
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 BTCUSD SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico BTCUSD: 0.5-2.0 (en dólares)
        # Multiplicadores: TP = ATR × 1.1, SL = ATR × 0.8
        # Esto da R:R de ~1.375:1 mínimo
        # ═══════════════════════════════════════════════════════════════
        
        # ATR viene en dólares para BTCUSD (ej: 1.5 = $1.50 de movimiento)
        # Clamp ATR a rangos razonables para M1
        atr_clamped = max(0.30, min(3.0, atr))  # Min $0.30, Max $3.00
        
        # ⭐ MULTIPLICADORES PARA M1 SCALPING (usuario especificó 1.1 / 0.8)
        TP_MULTIPLIER = 1.1  # TP = ATR × 1.1
        SL_MULTIPLIER = 0.8  # SL = ATR × 0.8
        ...
        if m5_projected is not None and m5_alignment > 0.6:
            ...
            m5_tp_extension = min(m5_tp_extension, 3.00)  # Cap at $3.00 extra
        ...
        # 🔧 FIX v4: Aumentados a 35 pips - broker STOPS_LEVEL de BTCUSD es alto
        # ═══════════════════════════════════════════════════════════════════════════
        MIN_TP = 3.50   # $3.50 = 35 pips minimum TP (broker STOPS_LEVEL safe)
        MIN_SL = 3.00   # $3.00 = 30 pips minimum SL (broker STOPS_LEVEL safe)
        SAFETY = 0.50   # $0.50 = 5 pips safety buffer for ASK/BID slippage
        
        tp_distance_price = max(MIN_TP, tp_distance_price)
        sl_distance_price = max(MIN_SL, sl_distance_price)
        
        # ⚠️ HARD CAP para scalping M1 - aumentados para coincidir con mínimos
        tp_distance_price = min(10.00, tp_distance_price)  # Máximo $10.00 TP (100 pips) - increased for M5 projection
        sl_distance_price = min(6.00, sl_distance_price)   # Máximo $6.00 SL (60 pips)
        ...
        # Convert to pips for return (1 pip = $0.10 for BTCUSD)
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        ...
        # FIX v4: Mínimos AUMENTADOS para evitar "Invalid stops"
        # Broker STOPS_LEVEL de BTCUSD puede ser 30+ pips, usar mínimos más altos
        MIN_TP_PIPS = 35  # Mínimo 35 pips TP ($3.50)
        MIN_SL_PIPS = 30  # Mínimo 30 pips SL ($3.00)
        SAFETY_PIPS = 5   # 5 pips de seguridad extra ($0.50)
        
        tp_distance = max(MIN_TP_PIPS, min(60.0, tp_distance))   # 20-60 PIPS TP
        sl_distance = max(MIN_SL_PIPS, min(50.0, sl_distance))   # 15-50 PIPS SL
        
        # Point value conversion: BTCUSD = 0.10 per pip ($0.10 = 1 pip)
        point_value = 0.10  # BTCUSD: 1 pip = $0.10
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 25.0) if atr > 0 else 0  # [NOVA AUDIT F-11] BTCUSD ATR M1 ~$100-500 → bins 4-20
...
class QuantumConsciousnessEngine:
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        if SHARED_BRAIN_AVAILABLE and self.genome_counter % 30 == 0:
            try:
                ...
                update_regime_state(
                    symbol=genome.get('metadata', {}).get('symbol', 'BTCUSD'),
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('BTCUSD')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM7 OCULUS - Data Quality Validator (Port 8862)
            ...
            # ✨ LLM8 CHRONOS - Timing Optimizer (Port 8863)
            ...
            # ✨ LLM9 PREDATOR - Execution Engine (Port 8864)
        ...
        try:
            ...
            buy_keywords = ['BUY', 'WHALE', 'EXECUTE', 'READY']
            sell_keywords = ['SELL', 'VETO']
            hold_keywords = ['HOLD', 'WAIT', 'CAUTION', 'NEUTRAL']
            ...
            # CRITICAL: Round to valid lot increments (0.01 for BTCUSD)
            ...
            # � RELAXED RULE: Only block if consensus is very low (< 35%) - was 50%, too restrictive
            ...
            # 🔵 RELAXED THRESHOLD: Trinity must be >= 50% to execute trades (was 65% - too restrictive for BTC)
            # BTC scalping requires faster entries - lower threshold allows more opportunities
            ...
            # 🔧 FIX v4: Validate Trinity values against broker STOPS_LEVEL
            MIN_TP_DIST = 3.50  # $3.50 = 35 pips minimum (broker STOPS_LEVEL safe)
            MIN_SL_DIST = 3.00  # $3.00 = 30 pips minimum (broker STOPS_LEVEL safe)
            SAFETY = 0.50       # $0.50 = 5 pips safety buffer
            ...
            try:
                ...
                nova_dashboard.update(
                    ...
                    market_state=getattr(market_analysis_result.market_state, 'value', market_analysis_result.market_state) if market_analysis_result else 'UNKNOWN',
                    trend_strength=getattr(market_analysis_result.trend_strength, 'value', market_analysis_result.trend_strength) if market_analysis_result else 1,
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 8870
        ...
        try:
            ...
            # Send to sound.py server on port 8870 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 8870))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:8870")
        except socket.timeout:
...
class AdvancedIPC:
    ...
    def _play_sound_alert(self, alert_type, symbol='BTCUSD'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # 🔧 FIX: Use Config.SOUND_PORT instead of hardcoded 8870
            sound_port = getattr(Config, 'SOUND_PORT', 8870)
                    ...
                    # CRITICAL: This enables ML learning from trade results!
                        ...
                        try:
                            ...
                            try:
                                ...
                                self.logger.debug(f"[SHARED BRAIN] Write error (non-critical): {_sb_err}")
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "BTCUSD", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "BTCUSD"}, ...}
        """
    ...
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for BTCUSD data"""
        try:
            genome_type = genome.get('type', '')
            
            # Accept all valid genome types from MT5 bridge
            valid_types = ['market_genome', 'tick_genome', 'genome', 'tick', 'portfolio_sync']
            ...
            if genome_type in valid_types:
                ...
                try:
                    ...
                    # Minimal sanity: bid < ask (allow equal for crypto)
                    if bid > ask:
                        self.logger.warning(f"[Validator] Inverted: bid={bid} > ask={ask}")
                        return False
                    
                    # Acceptable range for BTCUSD
                    # Some brokers use divided prices (e.g., $33.55 = $33,550)
                    # Accept: $10-$500 (divided by 1000) OR $10,000-$500,000 (normal)
                    mid_price = (bid + ask) / 2.0
                    
                    # Normal price range ($10,000 - $500,000)
                    if 10000 <= mid_price <= 500000:
                        return True
                    
                    # Divided price range ($10 - $500, meaning $10,000 - $500,000)
                    if 10 <= mid_price <= 500:
                        self.logger.debug(f"[Validator] BTC using divided pricing: ${mid_price:.2f} (= ${mid_price*1000:.0f})")
                        return True
                    
                    # Price still out of range
                    self.logger.warning(f"[Validator] BTC price out of range: ${mid_price:.2f}")
                    return False
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - Quimera not connected to port 8856")
            ...
            # 1. Validate price is within realistic bounds (BTCUSD: $10k-$500k or divided $10-$500)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR BTCUSD
            # ⭐ CRITICAL FIX: Support both normal ($10k-$500k) and divided pricing ($10-$500)
            # Some brokers send BTC prices divided by 1000 (e.g., $28.19 = $28,190)
            valid_normal_range = 10000 <= entry_price <= 500000  # Normal BTC pricing
            valid_divided_range = 10 <= entry_price <= 500       # Divided by 1000
            
            if not (valid_normal_range or valid_divided_range):
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price} out of realistic range for BTCUSD (need $10-$500 or $10k-$500k)")
                return False
            
            # Log if using divided pricing
            if valid_divided_range and not valid_normal_range:
                self.logger.debug(f"[EINSTEIN] Using divided pricing: ${entry_price:.2f} (= ${entry_price*1000:.0f})")
            ...
            # ⚠️ CRITICAL: Clamp ATR to M1 BTCUSD realistic range
            # M1 ATR típico: $0.30-$1.50 (3-15 pips)
            # Si viene más alto, el mercado está loco o hay error
            atr_val = max(0.30, min(1.50, atr_val))
            ...
            # 🔧 FIX v4: Aumentados a 35 pips - broker BTCUSD tiene STOPS_LEVEL alto
            # ═══════════════════════════════════════════════════════════════════════════
            MIN_TP_DISTANCE = 3.50  # $3.50 = 35 pips minimum TP (broker STOPS_LEVEL safe)
            MIN_SL_DISTANCE = 3.00  # $3.00 = 30 pips minimum SL (broker STOPS_LEVEL safe)
            SAFETY_MARGIN = 0.50    # $0.50 = 5 pips extra safety buffer for ASK/BID slippage
            ...
            tp_distance = min(tp_distance, 6.00)  # Max $6.00 TP (60 pips)
            sl_distance = min(sl_distance, 5.00)  # Max $5.00 SL (50 pips)
...
class QuantumCore:
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        if closes and current_price > 0:
            ...
            if gap > 0.01:  # More than 0.01 (1 pip for BTCUSD) difference
    ...
    def _print_banner(self):
        ...
        logger.info("")
        logger.info("=" * 110)
        logger.info("NOVA SNIPER SCALPING v18.00 - FRANCOTIRADOR CONSCIOUSNESS (120s cooldown, 75%+ win rate)")
        logger.info("=" * 110)
        logger.info("")
    ...
    def start(self):
        ...
        logger.info(f"[OK] SNIPER SCALPING v18.00 ONLINE - 🎯 Crypto Predator Mode ACTIVE")
    ...
    def _send_pings_to_quimera(self):
        ...
        The MT5 EA (btcquimera_bridge.mq5) expects to receive PING messages
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            if genome_type not in ['market_genome', 'tick_genome', 'genome', 'tick']:
            ...
            # FIX: BTC EA sends bid/ask at ROOT level, not in tick_data
            # Also handle 'timeframes' → 'timeframe_analysis' key mapping
            if not tick_data or ('bid' not in tick_data):
                # Build tick_data from root-level fields (BTC EA flat format)
                tick_data = {
                    'bid': genome.get('bid', 0),
                    'ask': genome.get('ask', 0),
                    'spread': genome.get('spread', 0),
                    'volume': genome.get('tick_volume', 0),
                    'last': genome.get('bid', 0),  # Use bid as last if not available
                }
                genome['tick_data'] = tick_data
            
            # FIX: Map EA's 'timeframes' key to 'timeframe_analysis' for Trinity consensus
            if 'timeframe_analysis' not in genome and 'timeframes' in genome:
                raw_tfs = genome.get('timeframes', [])
                tf_analysis = []
                for tf in raw_tfs:
                    if isinstance(tf, dict):
                        ...
                        ma_fast = tf.get('ma_fast', 0)
                        ma_slow = tf.get('ma_slow', 0)
                        adx_val = tf.get('adx', 0)
                        
                        # Determine signal from indicator combination
                        signal = 'HOLD'
                        strength = 50
                        reason = ''
                        ...
                        if ma_fast > ma_slow and ma_fast > 0:
                            bullish_points += 30
                        elif ma_slow > ma_fast and ma_slow > 0:
                            bearish_points += 30
                        ...
                        if bullish_points > bearish_points + 10:
                            signal = 'BUY'
                            strength = min(95, 50 + bullish_points)
                            reason = f"MA_UP RSI={rsi_val:.0f} ADX={adx_val:.0f}"
                        elif bearish_points > bullish_points + 10:
                            signal = 'SELL'
                            strength = min(95, 50 + bearish_points)
                            reason = f"MA_DOWN RSI={rsi_val:.0f} ADX={adx_val:.0f}"
                        else:
                            signal = 'HOLD'
                            strength = 40
                            reason = f"NEUTRAL RSI={rsi_val:.0f}"
                ...
                if tf_analysis:
                    ...
                    self.logger.info(f"[Transform] ✅ Mapped {len(tf_analysis)} timeframes → timeframe_analysis")
            
            # FIX: Build metadata from root-level if missing (BTC EA flat format)
            if not metadata:
                metadata = {
                    'symbol': genome.get('symbol', 'BTCUSD'),
                    'magic': genome.get('magic', 0),
                    'timestamp': genome.get('timestamp', 0),
                }
                genome['metadata'] = metadata
            
            symbol = metadata.get('symbol', genome.get('symbol', 'BTCUSD'))
            bid = float(tick_data.get('bid', 0))
            ask = float(tick_data.get('ask', 0))
            ...
            # CRITICAL: Validate price is in reasonable range for BTCUSD
            # BTC trades at $80,000-$200,000+ (2024-2025 range)
            # Only reject obviously corrupt data (negative, zero, or absurdly low/high)
            if symbol == 'BTCUSD' and close_price > 0:
                if close_price < 100 or close_price > 500000:
                    # Only reject EXTREME outliers (< $100 or > $500,000)
                    ...
                    return None
                # Normal BTC range - accept silently
                self.logger.debug(f"[Transform] ✅ BTC price {close_price:.2f} in valid range")
            
            # CRITICAL: Validate OHLC consistency if present
            # FIX: Build bar_data from EA's timeframes array if bar_data is missing (BTC EA flat format)
            if not bar_data and genome.get('timeframes'):
                raw_tfs = genome.get('timeframes', [])
                # Use M5 primary timeframe for bar data
                for tf in raw_tfs:
                    if isinstance(tf, dict) and tf.get('timeframe') in ['M5', 'M1']:
                        bar_data = {
                            'current': {
                                'open': tf.get('open', close_price),
                                'high': tf.get('high', close_price),
                                'low': tf.get('low', close_price),
                                'close': tf.get('close', close_price),
                                'volume': tf.get('volume', 0),
                            }
                        }
                        genome['bar_data'] = bar_data
                        self.logger.debug(f"[Transform] Built bar_data from {tf.get('timeframe')} timeframe")
                        break
            ...
            # ═══ FIX: OVERRIDE with REAL indicators from EA's timeframes array ═══
            # The EA (btcquimera_bridge.mq5) sends pre-calculated indicators from MT5
            # These are calculated from REAL historical bars (50+ bars), much more accurate
            # than our synthetic calculations from limited tick buffer
            raw_tfs = genome.get('timeframes', [])
            if raw_tfs:
                # Use M5 as primary (matching btcconfig.yaml primary_timeframe)
                # Fallback to M1, then M15
                primary_tf = None
                for tf in raw_tfs:
                    if isinstance(tf, dict) and tf.get('timeframe') == 'M5':
                        primary_tf = tf
                        break
                if not primary_tf:
                    for tf in raw_tfs:
                        if isinstance(tf, dict) and tf.get('timeframe') == 'M1':
                            primary_tf = tf
                            break
                if not primary_tf and raw_tfs and isinstance(raw_tfs[0], dict):
                    primary_tf = raw_tfs[0]
                
                if primary_tf:
                    ea_rsi = primary_tf.get('rsi', 0)
                    ea_adx = primary_tf.get('adx', 0)
                    ea_atr = primary_tf.get('atr', 0)
                    ea_ma_fast = primary_tf.get('ma_fast', 0)
                    ea_ma_slow = primary_tf.get('ma_slow', 0)
                    ea_boll_upper = primary_tf.get('boll_upper', 0)
                    ea_boll_middle = primary_tf.get('boll_middle', 0)
                    ea_boll_lower = primary_tf.get('boll_lower', 0)
                    ea_stoch_main = primary_tf.get('stoch_main', 0)
                    ea_stoch_signal = primary_tf.get('stoch_signal', 0)
                    
                    # Override with EA values if they're valid (non-zero)
                    ind = transformed['indicators']['current']
                    if ea_rsi > 0:
                        ind['rsi'] = ea_rsi
                    if ea_adx > 0:
                        ind['adx'] = ea_adx
                    if ea_atr > 0:
                        ...
                        ind['atr_pips'] = ea_atr
                    if ea_ma_fast > 0:
                        ind['ma_fast'] = ea_ma_fast
                    if ea_ma_slow > 0:
                        ind['ma_slow'] = ea_ma_slow
                    if ea_boll_upper > 0:
                        ind['bollinger_upper'] = ea_boll_upper
                        ind['bollinger_middle'] = ea_boll_middle
                        ind['bollinger_lower'] = ea_boll_lower
                        ind['bb_width'] = (ea_boll_upper - ea_boll_lower) / ea_boll_middle if ea_boll_middle > 0 else 0
                    if ea_stoch_main > 0:
                        ind['stoch_k'] = ea_stoch_main
                        ind['stoch_d'] = ea_stoch_signal
                    
                    self.logger.info(f"[Transform] ✅ OVERRIDE: Using EA indicators from {primary_tf.get('timeframe')}: RSI={ind['rsi']:.1f} ADX={ind['adx']:.1f} ATR={ind['atr']:.2f}")
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            symbol = genome.get('metadata', {}).get('symbol', 'BTCUSD')
            # FIX: Also get symbol from root level (BTC EA sends flat structure)
            if symbol == 'BTCUSD' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')
            
            # Handle Quimera format (market_genome, tick_genome, genome, tick)
            # FIX: BTC EA sends type="genome" or type="tick" - accept both
            if genome_type in ['market_genome', 'tick_genome', 'genome', 'tick']:
            ...
            nova_detector = NOVAMarketStateDetector(logger=logger)
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                # Calculate bid/ask from current price with typical BTCUSD spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 0.10  # Half spread for BTCUSD (typical spread = 0.20)
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    ...
                    self.engine.last_trade_time = time.time()  # 🔧 FIX: Also update engine cooldown
                    ...
                    # 🔧 FIX: Rollback position increment since trade didn't execute
                    with self.ipc.stats_lock:
    ...
    def run(self):
        ...
        last_position_reconcile = time.time()  # FIX RECONCILE: BTC no tenía esta variable
        start_time = time.time()
            ...
            while self.running:
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        with open('quantum_stats.json', 'w') as f:
                ...
                # FIX RECONCILE v2 — 3 señales para detectar posiciones fantasma (BTC no tenía reconcile)
