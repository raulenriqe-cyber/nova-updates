...
║  Port: 8864 (TCP Server) | Pure Python + Optional Ollama | <100ms latency   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM9:
    ...
    def __init__(self):
        ...
        log.info(f"[LLM9] Port: 8864 | Hybrid: Deterministic + Ollama Context")
...
¿Contexto? 1-palabra: (Fuerte/Normal/Débil/Caótico)"""
    ...
    def _calculate_stop_loss(self, data):
        ...
        try:
            ...
            # Clamp para broker requirements (M1 scalping - MÁS AJUSTADO)
            sl_distance = max(0.30, min(1.00, sl_distance))  # Tighter: $0.30-$1.00
            ...
            log.info(f"[LLM9 🎯 SL] {decision}: price={price:.2f} sl={stop_loss:.2f} dist=${sl_distance:.2f}")
            return stop_loss
    ...
    def analyze(self, data):
        ...
        try:
            ...
            if consensus_confidence >= 65 and consensus_decision in ['BUY', 'SELL']:
                ...
                if consensus_confidence < 65:
                    reason = f"Trinity confidence too low: {consensus_confidence:.0f}% < 65%"
                else:
