...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 ₿ BTCUSD CONFIG LOADER - CENTRALIZED CONFIGURATION           ║
║                                                                              ║
║  Loads BTCUSD ports and parameters from btcconfig.yaml                       ║
║  Single source of truth for all BTC system configuration                     ║
║                                                                              ║
...
class BTCConfig:
    """Centralized configuration manager for BTCUSD system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from btcconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"btcconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('sockets.trinity_port') -> 8866
        """
        config = cls.load()
    ...
    def get_port(cls, service: str) -> int:
        ...
        if service.lower() == 'trinity':
            ...
            if llm_num.isdigit():
                ...
                port = cls.get(f'llms.models.BTC_LLM{llm_num}.port')
                if port is not None:
                    return port
...
def get_trinity_port() -> int:
    """Get Trinity port for BTCUSD"""
    return BTCConfig.get_port('trinity')

def get_kraken_port() -> int:
    """Get Kraken port for BTCUSD"""
    return BTCConfig.get_port('kraken')

def get_llm_port(llm_num: int) -> int:
    """Get port for specific LLM (1-10)"""
    return BTCConfig.get_port(f'llm{llm_num}')

def get_ollama_port() -> int:
    """Get Ollama port (shared)"""
    return BTCConfig.get_port('ollama')

def get_sound_port() -> int:
    """Get Sound alert port"""
    return BTCConfig.get_port('sound')

def get_injector_port() -> int:
    """Get Injector port (MT5 -> Python)"""
    return BTCConfig.get_port('injector')

def get_executor_port() -> int:
    """Get Executor port (Python -> MT5)"""
    return BTCConfig.get_port('executor')
...
def get_all_llm_ports() -> Dict[str, int]:
    ...
    for i in range(1, 11):
        try:
            ports[f'llm{i}'] = BTCConfig.get_port(f'llm{i}')
        except:
            pass
    return ports
...
def get_symbol() -> str:
    """Get market symbol"""
    return BTCConfig.get('symbol', 'BTCUSD')

def get_timeframe() -> str:
    """Get primary timeframe"""
    return BTCConfig.get('primary_timeframe', 'M5')

def get_sl_range() -> tuple:
    """Get SL min/max USD"""
    return (
        BTCConfig.get('trading.scalp_sl', 150),
        BTCConfig.get('trading.swing_sl', 800)
    )

def get_tp_range() -> tuple:
    """Get TP min/max USD"""
    return (
        BTCConfig.get('trading.scalp_tp', 100),
        BTCConfig.get('trading.swing_tp', 600)
    )

def get_cooldown_seconds() -> int:
    """Get cooldown between trades (BTC needs longer cooldown)"""
    return 300  # 5 minutes for crypto volatility

def get_min_confidence() -> int:
    """Get minimum confidence threshold"""
    return BTCConfig.get('confidence.minimum_signal', 70)

def get_max_positions() -> int:
    """Get max positions"""
    return BTCConfig.get('trading.max_positions', 3)

def get_pip_value() -> float:
    """Get pip value for BTCUSD"""
    return BTCConfig.get('trading.pip_value', 1.0)

def get_volatility_thresholds() -> dict:
    """Get ATR thresholds for volatility regimes"""
    return {
        'low': BTCConfig.get('indicators.atr.low_volatility', 200),
        'normal': BTCConfig.get('indicators.atr.normal_volatility', 500),
        'high': BTCConfig.get('indicators.atr.high_volatility', 1200),
        'extreme': BTCConfig.get('indicators.atr.extreme_volatility', 2500)
    }
...
if __name__ == '__main__':
    print("═" * 80)
    print("₿ BTCUSD CONFIG LOADER - TEST")
    print("═" * 80)
