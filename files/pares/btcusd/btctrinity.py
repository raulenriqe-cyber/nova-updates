...
class TrinityConsensus:
    def __init__(self):
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD)
        self.core_llm_ports = {
            'BAYESIAN': 8855, 
            'TECHNICAL': 8857, 
            'CHART': 8858, 
            'RISK': 8859, 
            'SUPREME': 8861,
            'SMART_MONEY': 5004  # LLM6 - Smart Money Oracle (whale/sweep detection) ⭐ ADDED
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution)
        self.enhancement_llm_ports = {
            'OCULUS': 8862,    # LLM7 - Data Quality Validator
            'CHRONOS': 8863,   # LLM8 - Timing Optimizer  
            'PREDATOR': 8864,  # LLM9 - Execution Engine
            'NOVA': 8865,       # LLM10 - Decision Auditor & Emergency Halt
            'STRATEGIST': 8867,   # BTCUSD LLM11 - Strategist Guru
            'SENTINEL': 8868      # BTCUSD LLM12 - Quality Guardian
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 8862, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 8863, genome)
        if chronos_response:
        ...
        predator_response = self._query_enhancement_llm('PREDATOR', 8864, enhanced_genome)
        rr_ratio = 0.0
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] 🚫 Make sure all 6 LLMs are running on ports 8855, 8857, 8858, 8859, 8861, 5004")
        ...
        # FIX: If no timeframe data available, trust LLMs directly
        # Don't force HOLD just because timeframe analysis is missing
        tf_has_data = tf_alignment > 0 or tf_confidence > 0
        
        # Decision precedence:
        # 0. If NO timeframe data → Trust LLM decision directly (don't conflict-block)
        ...
        if not tf_has_data:
            # No timeframe data available - trust LLMs directly
            final_decision = llm_decision
            final_confidence = llm_confidence
            if llm_decision in ['BUY', 'SELL']:
                log.info(f"[TRINITY] 🎯 NO TF DATA - Trusting LLM: {llm_decision} @ {llm_confidence}%")
            else:
                log.info(f"[TRINITY] Using LLM decision (no TF data): {final_decision} @ {final_confidence}%")
            ...
            # FIX: Trade if LLM confidence >= 60% OR if TF has no data (tf_confidence==0 = TF abstained, not a real conflict)
            ...
            if llm_confidence >= 72 or tf_confidence == 0:  # 🔧 FIX: raised from 60/62→72
                final_decision = llm_decision
                penalty = 0.80 if tf_confidence == 0 else 0.75  # 🔧 FIX: stronger penalty
                ...
                if tf_confidence == 0:
                    ...
                    log.warning(f"[TRINITY] ⚠️ TRADING WITH CAUTION: LLM ({llm_confidence}%) overrides TF ({consensus_result['reason']})")
            else:
        ...
        # ⭐ RELAXED FOR BTC: Less aggressive penalties, more boosts
        # ═══════════════════════════════════════════════════════════════════
        ...
        # 🔧 RELAXED FOR BTC: Bajar thresholds para permitir más trades
        # BTC scalping tiene R:R más bajo naturalmente
        if rr_ratio > 0:
            if rr_ratio < 0.35:
                ...
                confidence = max(35, confidence - 8)
                log.warning(f"[TRINITY] ⚠️ Low R:R ({rr_ratio:.2f}) - confidence reduced to {confidence}%")
            elif rr_ratio >= 2.0:
    ...
    def _query_enhancement_llm(self, llm_name: str, port: int, genome: dict) -> dict:
        ...
        try:
            ...
            # Send request
            request_json = json.dumps(genome, default=str).encode('utf-8')
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for BTCUSD)
    ...
    def _aggregate_votes(self, llm_responses: dict) -> tuple:
        ...
        # Decision based on highest score - 🎯 RELAXED FOR BTC SCALPING
        if buy_score > sell_score and buy_score > hold_score:
            ...
            # 🎯 RELAXED: Mínimo 40% para BUY (was 65%, muy restrictivo)
            # quantum_core tiene su propio filtro, no duplicar aquí
            if confidence < 40:
                decision = 'HOLD'
                confidence = 35
            ...
            # 🎯 RELAXED: Mínimo 40% para SELL (was 65%)
            if confidence < 40:
                decision = 'HOLD'
                confidence = 35
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            # 2️⃣ VALIDACIÓN PRE-ENVÍO (Hueco #5, #10)
            ...
            # 3️⃣ CONECTAR CON LLM10 (CON TIMING)
            start_time = time.time()
            
            try:
                response = requests.post(
                    f'http://{self.host}:8865/audit',
                    json=audit_context,
                ...
                log.error(f"[NOVA CONNECTION] ❌ No se pudo conectar a puerto 8865")
                return self._fallback_nova_response()
...
# ⭐ TCP SERVER - Trinity accepts genomes from quantum_core on port 8866
class TrinityServer:
