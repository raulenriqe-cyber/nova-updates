...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 8852)
  - Audio announcements via sound.py (BTCUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='config.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 8852 (not 8856 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 8852)
            self.symbols = config.get('core_engine', {}).get('symbols', ['BTCUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 8852  # Default to 8852, NOT 8856
            self.symbols = ['BTCUSD']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 50.0    # [NOVA AUDIT F-12] $50 TP fallback for BTC (was $0.05)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 30.0    # [NOVA AUDIT F-12] $30 SL fallback for BTC
    ...
    def _execute_order(self, order):
        ...
        try:
            ...
            # Validate symbol
            valid_symbols = {'BTCUSD', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'GBPUSD', 'GBP',
