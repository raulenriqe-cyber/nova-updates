...
║  Ports: 5565 (XAUUSD) | 6565 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | ✨ LLM10_NOVA | %(message)s",
    datefmt="%H:%M:%S"
)
...
class NOVA:
    ...
    def __init__(self, port=5565, symbol='XAUUSD'):
        self.port = port
        self.symbol = symbol
    ...
    def _audit_logic_consistency(self, genome, trinity_decision, llm_responses):
        ...
        try:
            ...
            if decision in ['BUY', 'SELL']:
                ...
                # 🔧 FIX: Guard sma_fast/sma_slow — only compare if len(closes) >= 50 (where they're defined)
                is_buy_causal = (rsi < 50 or macd > 0) and (sma_fast > sma_slow if len(closes) >= 50 else True)
                is_sell_causal = (rsi > 50 or macd < 0) and (sma_fast < sma_slow if len(closes) >= 50 else True)
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # Calculate multiplier from quality score
            # 🔧 FIX: Old formula `quality_score / 100.0` was LINEAR 1:1 mapping,
            # meaning quality=62 gave mult=0.62 (38% penalty!) - mathematically wrong.
            # Quality 62/100 is "passing" not "catastrophic".
            # Fixed: sqrt() mapping for graduated penalty curve:
            #   quality=100 → mult=1.00 | quality=81 → mult=0.90
            #   quality=64 → mult=0.80 | quality=49 → mult=0.70
            #   quality=36 → mult=0.60 | quality=25 → mult=0.50
            import math
            # 🔧 FIX: Bias multiplier baseline so typical quality (50) → ~0.90 (near-neutral)
            # Old formula: sqrt(50/100)=0.71 → 29% penalty on EVERY trade
            # New formula: 0.70 + 0.35*sqrt(50/100) = 0.70 + 0.247 = 0.947 → only 5% penalty
            multiplier = max(0.50, 0.70 + 0.35 * math.sqrt(quality_score / 100.0))
            ...
            # Clamp to reasonable range (allow boost for high quality)
            multiplier = max(0.0, min(1.15, multiplier))
            
            # Build result
            result = {
                'decision': 'AUDIT',
                'quality_score': quality_score,
                'audit_score': quality_score,  # 🔧 FIX: Alias for Trinity compatibility
                'quality_breakdown': {
                ...
                'quality_multiplier': round(multiplier, 3),  # 🔧 FIX: Alias for Trinity compatibility
            ...
            return {
                'decision': 'AUDIT',
                'quality_score': 50,
                'audit_score': 50,  # 🔧 FIX: Alias for Trinity compatibility
                ...
                'categories': ['ERROR'],
                'multiplier': 0.5,
                'quality_multiplier': 0.5,  # 🔧 FIX: Alias for Trinity compatibility
                'timestamp': time.time()
            }
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para XAUUSD: correlación con DXY
            if symbol == 'XAUUSD':
    ...
    def run_server_tcp(self):
        ...
        server.listen(5)  # Allow up to 5 pending connections (Phase 1 + Phase 3 concurrent)
    ...
    def _handle_request_tcp(self, conn):
        ...
        try:
            ...
            try:
                ...
                log.warning(f"[NOVA] JSON parse error: {je} | Length={data_length} Got={len(data_bytes)} | First 50 bytes: {data_bytes[:50]!r}")
                return
            ...
            log.warning(f"TCP Request handling error: {e} | type={type(e).__name__}")
...
if __name__ == '__main__':
    ...
    # Support both ports
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 5565
    symbol = 'XAUUSD' if port == 5565 else 'EURUSD'
