...
class LLM6_SmartMoney:
    ...
    def _detect_sweeps(self, genome: Dict) -> Tuple[SweepType, Optional[str]]:
        ...
        """
        # ⭐ FIX: Use 20 candles so the 15-candle S/R lookup actually works
        closes = np.array(genome['closes'][-20:])
        opens = np.array(genome['opens'][-20:])
        lows = np.array(genome['lows'][-20:])
        highs = np.array(genome['highs'][-20:])
        volumes = np.array(genome['volumes'][-20:])
        ...
        # Recent lows/highs for sweep reference - FIX: use 15 candles instead of 3 for real S/R
        recent_low = min(lows[-16:-1]) if len(lows) > 15 else (min(lows[-4:-1]) if len(lows) > 3 else lows[-2])
        recent_high = max(highs[-16:-1]) if len(highs) > 15 else (max(highs[-4:-1]) if len(highs) > 3 else highs[-2])
    ...
    def _detect_false_breaks(self, genome: Dict) -> float:
        ...
        # Factor 1: Wick largo (rechazo) - FIX: only count the DOMINANT wick, not both
        body = abs(current_close - current_open)
        upper_wick = current_high - max(current_close, current_open)
        lower_wick = min(current_close, current_open) - current_low
        candle_range = current_high - current_low
        
        # FIX: Require minimum range ($0.50 for XAUUSD) to avoid noise on tiny candles
        if candle_range < 0.50:
            return 0.0  # Tiny candle = no false break
        
        if body > 0:
            # Only score the LARGER wick (the direction of rejection)
            if upper_wick > lower_wick and upper_wick > body * 1.5:
                score += 25  # Rejection from above
            elif lower_wick > upper_wick and lower_wick > body * 1.5:
                score += 25  # Rejection from below
    ...
    def _calculate_whale_confidence(self, 
        ...
        # Factor 2: OBV signal (0-25 puntos) - FIX: compare to baseline, not just existence
        # OBV is cumulative so it's NEVER zero - must check if it's anomalous
        if abs(imbalance) > 20 and obv != 0:
            # Only count OBV as whale activity if there's also order flow imbalance
            score += 15
        
        # Factor 3: A/D line (0-20 puntos) - FIX: same issue, A/D is never zero
        if abs(imbalance) > 20 and ad_line != 0:
            score += 10
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # SISTEMA 8: FAIR VALUE GAP (FVG) DETECTION — NEW INTELLIGENCE
    # ═════════════════════════════════════════════════════════════════════════
    
    def _detect_fvg(self, genome: Dict) -> list:
        """
        Detect Fair Value Gaps (FVGs) — institutional price inefficiencies.
        
        A Bullish FVG: candle[i] high < candle[i+2] low (gap up)
        A Bearish FVG: candle[i] low > candle[i+2] high (gap down)
        
        Price tends to return to fill FVGs before continuing — key for entries.
        """
        fvgs = []
        try:
            closes = [float(c) for c in genome.get('closes', genome.get('bar_data', {}).get('closes', []))]
            highs = [float(h) for h in genome.get('highs', genome.get('bar_data', {}).get('highs', []))]
            lows = [float(l) for l in genome.get('lows', genome.get('bar_data', {}).get('lows', []))]
            
            if len(highs) < 5 or len(lows) < 5:
                return fvgs
            
            # Scan last 20 candles for FVGs
            scan_range = min(20, len(highs) - 2)
            for i in range(len(highs) - scan_range, len(highs) - 2):
                if i < 0:
                    continue
            ...
            for fvg in fvgs:
                fvg['price_in_gap'] = fvg['bottom'] <= current_price <= fvg['top']
                fvg['distance'] = min(abs(current_price - fvg['top']), abs(current_price - fvg['bottom']))
            
        except Exception as e:
            log.debug(f"[LLM6] FVG detection error: {e}")
        
        return fvgs
    
    # ═════════════════════════════════════════════════════════════════════════
    # SISTEMA 9: ORDER BLOCK DETECTION — NEW INTELLIGENCE
    # ═════════════════════════════════════════════════════════════════════════
    
    def _detect_order_blocks(self, genome: Dict) -> list:
        """
        ...
        Institutional traders use OBs as re-entry zones.
        """
        order_blocks = []
        try:
            closes = [float(c) for c in genome.get('closes', genome.get('bar_data', {}).get('closes', []))]
            opens = [float(o) for o in genome.get('opens', genome.get('bar_data', {}).get('opens', []))]
            highs = [float(h) for h in genome.get('highs', genome.get('bar_data', {}).get('highs', []))]
            lows = [float(l) for l in genome.get('lows', genome.get('bar_data', {}).get('lows', []))]
            ...
            # Calculate average candle body for "impulse" threshold
            bodies = [abs(closes[i] - opens[i]) for i in range(len(closes))]
            avg_body = sum(bodies[-20:]) / min(20, len(bodies)) if bodies else 1.0
            impulse_threshold = avg_body * 2.5  # Impulse = 2.5x average body
            
            for i in range(1, min(20, len(closes) - 2)):
                idx = len(closes) - 1 - i
                if idx < 1:
                    break
                
                current_body = closes[idx] - opens[idx]
                prev_body = closes[idx - 1] - opens[idx - 1]
                
                # Bullish OB: Previous candle bearish + current candle strong bullish impulse
                if prev_body < 0 and current_body > impulse_threshold:
                    order_blocks.append({
                        'type': 'BULLISH_OB',
                        'top': opens[idx - 1],  # OB zone = body of the opposing candle
                        'bottom': closes[idx - 1],
                ...
                elif prev_body > 0 and current_body < -impulse_threshold:
                    order_blocks.append({
                        'type': 'BEARISH_OB',
                        'top': closes[idx - 1],
                        'bottom': opens[idx - 1],
                        'strength': abs(current_body) / avg_body,
                        'candle_index': idx - 1
                    })
            ...
            for ob in order_blocks:
                ...
                ob['distance'] = abs(current_price - ob_mid)
                ob['price_at_ob'] = ob['bottom'] <= current_price <= ob['top']
            
        except Exception as e:
            log.debug(f"[LLM6] Order Block detection error: {e}")
    ...
    def _calculate_weighted_votes(self, signal: SmartMoneySignal, genome: Dict = None) -> Dict[str, float]:
        ...
        # Voto 3: Whale Confidence (Peso: 1.6) - 🔧 FIX: Direction follows OBV+A/D consensus
        # Whale activity should amplify the dominant direction, not always vote BUY
        if signal.whale_confidence > 15:
            # Determine whale direction from OBV + A/D consensus
            whale_direction = 0.0
            if signal.obv > 0:
                whale_direction += 1.0
            elif signal.obv < 0:
            ...
            # If OBV and A/D agree, whale amplifies that direction
            # If they disagree (whale_direction=0), whale doesn't vote on direction
            if signal.whale_confidence > 50 and whale_direction != 0:
                whale_sign = 1.0 if whale_direction > 0 else -1.0
                votes['whale_strong'] = whale_sign * 1.0 * self.config['vote_weight_whale']
            elif signal.whale_confidence > 30 and whale_direction != 0:
                whale_sign = 1.0 if whale_direction > 0 else -1.0
                votes['whale_moderate'] = whale_sign * 0.5 * self.config['vote_weight_whale']
            elif signal.whale_confidence > 15 and whale_direction != 0:
                whale_sign = 1.0 if whale_direction > 0 else -1.0
                votes['whale_weak'] = whale_sign * 0.25 * self.config['vote_weight_whale']
        ...
        # ═══════════════════════════════════════════════════════════════
        # Voto 8: Fair Value Gap (Peso: 1.6) — NEW INTELLIGENCE
        # Price at/near FVG = high probability zone for reaction
        # ═══════════════════════════════════════════════════════════════
        fvgs = getattr(self, '_last_fvgs', [])
        if fvgs:
            # Find most recent FVG where price is inside the gap
            active_fvgs = [f for f in fvgs if f.get('price_in_gap', False)]
            if active_fvgs:
                # Price is INSIDE an FVG — strong signal
                fvg = active_fvgs[0]  # Most recent
                if fvg['type'] == 'BULLISH_FVG':
                    votes['fvg_bullish'] = 1.0 * 1.6
                    logger.info(f"📊 LLM6 FVG: Price IN Bullish FVG [{fvg['bottom']:.2f}-{fvg['top']:.2f}]")
                else:
                    votes['fvg_bearish'] = -1.0 * 1.6
                    logger.info(f"📊 LLM6 FVG: Price IN Bearish FVG [{fvg['bottom']:.2f}-{fvg['top']:.2f}]")
            else:
                # Price near FVG (within closest gap_size distance)
                nearest = min(fvgs, key=lambda f: f.get('distance', 999))
                if nearest.get('distance', 999) < nearest.get('gap_size', 1) * 2:
                    if nearest['type'] == 'BULLISH_FVG':
                        votes['fvg_near_bullish'] = 0.5 * 1.6
                    else:
                        votes['fvg_near_bearish'] = -0.5 * 1.6
        
        # ═══════════════════════════════════════════════════════════════
        # Voto 9: Order Block (Peso: 1.8) — NEW INTELLIGENCE
        # Price at Order Block = institutional re-entry zone
        # ═══════════════════════════════════════════════════════════════
        obs = getattr(self, '_last_order_blocks', [])
        if obs:
            active_obs = [ob for ob in obs if ob.get('price_at_ob', False)]
            if active_obs:
                # Price is AT an Order Block — very strong signal
                ob = max(active_obs, key=lambda x: x.get('strength', 0))  # Strongest OB
                if ob['type'] == 'BULLISH_OB':
                    votes['ob_bullish'] = 1.2 * 1.8
                    logger.info(f"🏦 LLM6 OB: Price AT Bullish OB [{ob['bottom']:.2f}-{ob['top']:.2f}] strength={ob['strength']:.1f}x")
                else:
                    votes['ob_bearish'] = -1.2 * 1.8
                    logger.info(f"🏦 LLM6 OB: Price AT Bearish OB [{ob['bottom']:.2f}-{ob['top']:.2f}] strength={ob['strength']:.1f}x")
            else:
                # Price near an OB
                nearest_ob = min(obs, key=lambda x: x.get('distance', 999))
                if nearest_ob.get('strength', 0) > 3.0:  # Only strong OBs when not at price
                    if nearest_ob['type'] == 'BULLISH_OB':
                        votes['ob_near_bullish'] = 0.4 * 1.8
                    else:
                        votes['ob_near_bearish'] = -0.4 * 1.8
    ...
    def analyze(self, genome: Dict) -> SmartMoneySignal:
        ...
        try:
            ...
            # Sistema 8: Fair Value Gap Detection (NEW)
            fvgs = self._detect_fvg(genome)
            
            # Sistema 9: Order Block Detection (NEW)
            order_blocks = self._detect_order_blocks(genome)
            
            # Store for voting system
            self._last_fvgs = fvgs
            self._last_order_blocks = order_blocks
    ...
    def _make_decision(self, signal: SmartMoneySignal) -> SmartMoneySignal:
        ...
        # BASE confidence - FIX: removed artificial +40 floor that made minimum confidence 77.5%
        # Now confidence properly reflects actual signal strength
        base_confidence = signal.whale_confidence * 0.3  # 30% of whale conf as base (was 50%)
        
        # Calcular puntuación ponderada
        if signal.votes:
            total_weight = signal.total_vote_weight
            
            if total_weight > 0.5:  # Voto COMPRA
                signal.recommendation = "BUY"
                signal.confidence = min(95, base_confidence + (total_weight * 15))
                signal.reasoning = f"COMPRA: Votos {total_weight:.1f} | Ballena {signal.whale_confidence:.0f}%"
            
            elif total_weight < -0.5:  # Voto VENTA
                signal.recommendation = "SELL"
                signal.confidence = min(95, base_confidence + (abs(total_weight) * 15))
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 2650))
                genome['closes'] = [price] * 50
                genome['opens'] = [price] * 50
                genome['highs'] = [price * 1.001] * 50
                genome['lows'] = [price * 0.999] * 50
                genome['volumes'] = [100] * 50
                logger.warning(f"[LLM6] Using synthetic data from current_price: {price}")
                return True
            
            logger.error(f"❌ Genome inválido: No se pudo extraer OHLCV")
            return False
        
        if any(len(genome[k]) < 3 for k in required_keys):
            logger.error(f"❌ Genome: Datos insuficientes")
            return False
            ...
            try:
                ...
                try:
                    ...
                    response = {
                        ...
                        'decision': signal.recommendation,  # 🔧 FIX: Trinity reads 'decision' not 'recommendation'
...
def run_tcp_server():
    ...
    try:
        ...
        port = config.get('llm_settings', {}).get('llm6_port', 5004)
    except Exception as e:
        port = 5004
