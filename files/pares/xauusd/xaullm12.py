...
MIN_RR_RATIO = 1.5     # 🔧 Aligned with SmartTPSL fix (was 2.0 → blocked R:R 1.5-2.0 range)
...
class IntradayHeatmap:
    ...
    def update(self, trade: dict, memory: dict):
        ...
        # 🔧 FIX: Was +4 which creates invalid keys like "20-00" for hour 20-23.
        # Use +3 for inclusive end: block 20-23, not 20-00.
        hour_block = f"{(ts.hour // 4) * 4:02d}-{((ts.hour // 4) * 4 + 3) % 24:02d}"
    ...
    def get_current_score(self, memory: dict) -> float:
        ...
        now = datetime.now(timezone.utc)
        # 🔧 FIX: Match the update() format — use +3 for inclusive end
        hour_block = f"{(now.hour // 4) * 4:02d}-{((now.hour // 4) * 4 + 3) % 24:02d}"
