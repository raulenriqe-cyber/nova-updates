...
class LLM5ChartAnalyzer:
    ...
    def analyze(self, symbol='XAUUSD', timeframe='M1') -> Dict[str, Any]:
        ...
        # ATR normalization: instrument-agnostic trend measurement
        atr_norm = self._atr(closes, highs, lows) if len(closes) >= 14 else float(np.mean([abs(highs[i] - lows[i]) for i in range(-min(5, len(highs)), 0)])) if len(highs) >= 2 else 1.0
        atr_norm = max(atr_norm, 0.001)  # prevent division by zero
        ...
        if len(closes) >= 20:
            ...
            # Determinar tendencia por MAs (normalized by ATR, not percentage)
            if short_ma > mid_ma > long_ma:
                trend_direction = 'UPTREND'
                trend_strength = min(1.0, (short_ma - long_ma) / atr_norm / 2.0)
            elif short_ma < mid_ma < long_ma:
                trend_direction = 'DOWNTREND'
                trend_strength = min(1.0, (long_ma - short_ma) / atr_norm / 2.0)
            
            # Confirmar con cambio de precio reciente (ATR-normalized)
            price_change_abs = abs(closes[-1] - closes[-10])
            price_change_pct = (closes[-1] - closes[-10]) / closes[-10] * 100
            if price_change_pct < -0.1:  # Cayendo más de 0.1%
                if trend_direction != 'DOWNTREND':
                    trend_direction = 'DOWNTREND'
                    trend_strength = min(1.0, price_change_abs / atr_norm / 2.0)
            elif price_change_pct > 0.1:  # Subiendo más de 0.1%
        ...
        # Calcular momentum del precio (ATR-normalized)
        price_mom = closes[-1] - closes[-5] if len(closes) >= 5 else 0
        mom_atr_ratio = price_mom / atr_norm if atr_norm > 0 else 0
        
        # RSI alto + precio subiendo = MOMENTUM ALCISTA → seguir comprando
        if rsi > 55 and mom_atr_ratio > 0.3:  # 30% ATR over 5 candles
            bullish_signals += 0.6
            patterns.insert(0, {'name': 'Momentum_Up', 'type': 'bullish', 'confidence': 0.75})
            logger.info(f"[LLM5] 🟢 MOMENTUM UP (RSI={rsi:.1f}, mom={price_mom:.2f}, atr_ratio={mom_atr_ratio:.2f})")
        # RSI bajo + precio bajando = MOMENTUM BAJISTA → seguir vendiendo
        elif rsi < 45 and mom_atr_ratio < -0.3:
            bearish_signals += 0.6
            patterns.insert(0, {'name': 'Momentum_Down', 'type': 'bearish', 'confidence': 0.75})
            logger.info(f"[LLM5] 🔴 MOMENTUM DOWN (RSI={rsi:.1f}, mom={price_mom:.2f}, atr_ratio={mom_atr_ratio:.2f})")
        # Momentum fuerte confirmado
        elif rsi > 60 and mom_atr_ratio > 0.8:  # 80% ATR = strong move
            bullish_signals += 0.8
            patterns.insert(0, {'name': 'Strong_Momentum_Up', 'type': 'bullish', 'confidence': 0.85})
        elif rsi < 40 and mom_atr_ratio < -0.8:
            bearish_signals += 0.8
        ...
        # ════════════════════════════════════════════════════════════════════
        # 🧠 DEEP MARKET INTELLIGENCE: Volume + Traps + Exhaustion + Candle Size
        # El LLM5 ahora SABE leyendo las velas como un trader profesional
        # ════════════════════════════════════════════════════════════════════
        intel_reasons = []
        
        if len(closes) >= 5 and len(opens) >= 5:
            # ═══ ATR estimation for candle size context ═══
            atr_est = float(np.mean([highs[i] - lows[i] for i in range(-min(14, len(highs)), 0)])) if len(highs) >= 2 else 1.0
            
            # ═══ 1. CANDLE MORPHOLOGY - Body/Wick Intelligence ═══
            for idx in [-2, -1]:  # Last 2 candles
                if abs(idx) > len(opens):
                    continue
                ...
                if idx == -1:  # Current candle
                    # MOMENTUM CANDLE: Body >75% = directional conviction
                    if body_pct > 0.75:
                        if is_bull:
                            bullish_signals += 0.4
                            intel_reasons.append(f"MOMENTUM_BULL({body_pct:.0%})")
                        else:
                            bearish_signals += 0.4
                            intel_reasons.append(f"MOMENTUM_BEAR({body_pct:.0%})")
                    
                    # INDECISION CANDLE: Body <20% = no conviction
                    elif body_pct < 0.2 and rng > atr_est * 0.5:
                        bullish_signals *= 0.85
                        bearish_signals *= 0.85
                        intel_reasons.append("INDECISION")
                    
                    # REJECTION CANDLE: Long wick = S/R rejection
                    if upper_wick_pct > 0.55 and body_pct < 0.35:
                        bearish_signals += 0.5
                        bullish_signals *= 0.7
                        intel_reasons.append(f"SELL_REJECTION({upper_wick_pct:.0%})")
                    elif lower_wick_pct > 0.55 and body_pct < 0.35:
                        bullish_signals += 0.5
                        bearish_signals *= 0.7
                        intel_reasons.append(f"BUY_REJECTION({lower_wick_pct:.0%})")
                    
                    # CANDLE SIZE vs ATR: Institutional or noise?
                    if atr_est > 0:
                        size_ratio = rng / atr_est
                        if size_ratio > 2.0 and body_pct > 0.6:
                            # INSTITUTIONAL MOVE: Big candle + big body
                            mult = 0.6 if is_bull else -0.6
                            if mult > 0:
                                bullish_signals += mult
                            else:
                                bearish_signals += abs(mult)
                            intel_reasons.append(f"INSTITUTIONAL({size_ratio:.1f}xATR)")
                        elif size_ratio < 0.2:
                            bullish_signals *= 0.9
                            bearish_signals *= 0.9
                            intel_reasons.append("MICRO_NOISE")
            
            # ═══ 2. ENGULFING PATTERN (precise) ═══
            if len(opens) >= 2:
                ...
                if pc < po and cc > co and cc > po and co < pc:
                    bullish_signals += 0.7
                    intel_reasons.append("ENGULFING_BULL★")
                elif pc > po and cc < co and cc < po and co > pc:
                    bearish_signals += 0.7
                    intel_reasons.append("ENGULFING_BEAR★")
            
            # ═══ 3. BULL/BEAR TRAP DETECTION ═══
            recent_high = float(np.max(highs[-6:-1])) if len(highs) >= 6 else float(highs[-2])
            recent_low = float(np.min(lows[-6:-1])) if len(lows) >= 6 else float(lows[-2])
            
            # Bull trap: breaks above then closes bearish
            if float(highs[-1]) > recent_high and float(closes[-1]) < float(opens[-1]):
                trap = (float(highs[-1]) - recent_high) / atr_est if atr_est > 0 else 0
                if trap > 0.1:
                    bearish_signals += 0.8
                    bullish_signals *= 0.5
                    intel_reasons.append(f"🪤BULL_TRAP({trap:.2f})")
            
            # Bear trap: breaks below then closes bullish
            if float(lows[-1]) < recent_low and float(closes[-1]) > float(opens[-1]):
                trap = (recent_low - float(lows[-1])) / atr_est if atr_est > 0 else 0
                if trap > 0.1:
                    bullish_signals += 0.8
                    bearish_signals *= 0.5
                    intel_reasons.append(f"🪤BEAR_TRAP({trap:.2f})")
            
            # ═══ 4. LIQUIDITY SWEEP (wick hunt stops then reverse) ═══
            last_rng = float(highs[-1]) - float(lows[-1])
            if last_rng > 0:
                upper_sweep = (float(highs[-1]) - max(float(opens[-1]), float(closes[-1]))) / last_rng
                lower_sweep = (min(float(opens[-1]), float(closes[-1])) - float(lows[-1])) / last_rng
                
                if upper_sweep > 0.65 and float(closes[-1]) < float(opens[-1]):
                    bearish_signals += 0.5
                    intel_reasons.append("SWEEP_UP→SELL")
                if lower_sweep > 0.65 and float(closes[-1]) > float(opens[-1]):
                    bullish_signals += 0.5
                    intel_reasons.append("SWEEP_DOWN→BUY")
            
            # ═══ 5. EXHAUSTION DETECTION ═══
            if len(opens) >= 4:
                # Consecutive diminishing bodies = momentum dying
                body_sizes = [abs(float(closes[i]) - float(opens[i])) for i in range(-4, 0)]
                if all(body_sizes[i] > body_sizes[i+1] for i in range(3)) and body_sizes[-1] < atr_est * 0.15:
                    bullish_signals *= 0.7
                    bearish_signals *= 0.7
                    intel_reasons.append("EXHAUSTION_PATTERN")
                
                # Long wicks in same direction = strong rejection zone
                upper_wicks = [(float(highs[i]) - max(float(opens[i]), float(closes[i]))) / max(float(highs[i]) - float(lows[i]), 0.001) for i in range(-3, 0)]
                if all(w > 0.4 for w in upper_wicks):
                    bearish_signals += 0.6
                    bullish_signals *= 0.6
                    intel_reasons.append("REPEATED_UPPER_REJECT")
                
                lower_wicks = [(min(float(opens[i]), float(closes[i])) - float(lows[i])) / max(float(highs[i]) - float(lows[i]), 0.001) for i in range(-3, 0)]
                if all(w > 0.4 for w in lower_wicks):
                    bullish_signals += 0.6
                    bearish_signals *= 0.6
                    intel_reasons.append("REPEATED_LOWER_REJECT")
            
            # ═══ 6. ADVANCED VOLUME-PRICE INTELLIGENCE ═══
            if len(volumes) >= 5 and float(np.sum(volumes[-5:])) > 0:
                ...
                if avg_vol > 0:
                    ...
                    if vol_ratio > 1.8:
                        if float(closes[-1]) > float(opens[-1]):
                            bullish_signals += 0.4
                            intel_reasons.append(f"VOL_BULL({vol_ratio:.1f}x)")
                        else:
                            bearish_signals += 0.4
                            intel_reasons.append(f"VOL_BEAR({vol_ratio:.1f}x)")
                    
                    # Volume-Price Divergence: price up but volume declining = weakness
                    if len(volumes) >= 5:
                        price_rising = float(closes[-1]) > float(closes[-5])
                        vol_falling = float(volumes[-1]) < float(volumes[-3]) < float(volumes[-5])
                        
                        if price_rising and vol_falling:
                            bullish_signals *= 0.75
                            intel_reasons.append("VOL_DIV_WEAK_UP")
                        
                        price_falling = float(closes[-1]) < float(closes[-5])
                        if price_falling and vol_falling:
                            bearish_signals *= 0.75
                            intel_reasons.append("VOL_DIV_WEAK_DN")
                    
                    # Climax volume = potential reversal
                    if vol_ratio > 3.5:
                        intel_reasons.append(f"CLIMAX_VOL({vol_ratio:.1f}x)⚠️")
            
            # ═══ 7. BODY TREND (consecutive same-direction candles) ═══
            bull_count = sum(1 for i in range(-5, 0) if float(closes[i]) > float(opens[i]))
            bear_count = 5 - bull_count
            if bull_count >= 4:
                bullish_signals += 0.3
                intel_reasons.append(f"BODY_UP({bull_count}/5)")
            elif bear_count >= 4:
                bearish_signals += 0.3
                intel_reasons.append(f"BODY_DN({bear_count}/5)")
            
            if intel_reasons:
                logger.info(f"[🧠 DEEP_INTEL] BULL={bullish_signals:.2f} BEAR={bearish_signals:.2f} | {', '.join(intel_reasons)}")
    ...
    def _candlestick_patterns(self, opens, closes, highs, lows):
        ...
        for i in range(start_idx, len(closes)):
            ...
            # HAMMER / HANGING MAN - Cuerpo pequeño arriba, mecha larga abajo
            # Context-dependent: reversal signals need prior trend
            if body_ratio < 0.35 and lower_wick > 2 * body and upper_wick < body:
                if i > 3 and closes[i] < closes[i-3]:  # After downtrend = bullish reversal
                    patterns.append({
                        'name': 'HAMMER_BULLISH',
                        'type': 'bullish',
                        'confidence': 0.78
                    })
                elif i > 3 and closes[i] > closes[i-3]:  # After uptrend = bearish warning
                    patterns.append({
                        'name': 'HANGING_MAN',
                        'type': 'bearish',
                        'confidence': 0.70
            ...
            # INVERTED HAMMER / SHOOTING STAR - Cuerpo pequeño abajo, mecha larga arriba
            # Mutually exclusive based on trend context
            if body_ratio < 0.35 and upper_wick > 2 * body and lower_wick < body:
                ...
                elif i > 3 and closes[i] > closes[i-3]:  # After uptrend = bearish reversal
                    patterns.append({
                        'name': 'SHOOTING_STAR',
                        'type': 'bearish',
                        'confidence': 0.78
...
class LLM5Client:
    ...
    @staticmethod
    def analyze(symbol='XAUUSD', timeframe='M1', candles=None):
