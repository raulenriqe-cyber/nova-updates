...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 5552)
  - Audio announcements via sound.py (XAUUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='config.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 5552 (not 5556 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 5552)
            self.symbols = config.get('core_engine', {}).get('symbols', ['XAUUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 5552  # Default to 5552, NOT 5556
            self.symbols = ['XAUUSD']
    ...
    def _handle_client(self, client_socket, addr):
        ...
        try:
            ...
            if order_data == b'PING':
                ack_header = struct.pack('>I', 3)
                client_socket.sendall(ack_header + b'ACK')
                log.debug(f"[PING] Responded with ACK to {addr}")
                client_socket.close()
                return
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + (50 * 0.01 if entry > 100 else 50)  # 🔧 AUDIT FIX: was 0.001 (5 pips), now 0.01 (50 pips for gold)
            warning = "TP was 0, set to entry + 50pips"
            
        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - (30 * 0.01 if entry > 100 else 30)  # 🔧 AUDIT FIX: was 0.001 (3 pips), now 0.01 (30 pips for gold)
