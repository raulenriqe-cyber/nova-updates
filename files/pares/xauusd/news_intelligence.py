...
# ═══════════════════════════════════════════════════════════════════════════════
# CONFIGURATION WITH SWISS PRECISION
# ═══════════════════════════════════════════════════════════════════════════════
...
# 🎯 Mathematical Integration: Timing and coherence controls
NEWS_UPDATE_INTERVAL = 600  # 10 minutes (600 segundos)
SENTIMENT_SMOOTHING_FACTOR = 0.7  # EMA for sentiment stability
COHERENCE_THRESHOLD = 0.75  # Minimum coherence for reliable signals
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> str:
        ...
        # Prompt MEJORADO - contexto específico para cada símbolo
        prompt = f"""Eres un analista de trading EXPERTO en forex y commodities.
...
XAUUSD (ORO):
  ...
  - NEUTRAL (N): cuando no hay impacto claro
...
EURUSD (EURO):
  ...
  - NEUTRAL (N): cuando no hay dirección clara
...
def main():
    ...
    print(f"   BTCUSD: {sentiment.get('BTCUSD', 'N')}")
...
# Auto-start background update cuando se importa (no interfiere)
_startup_thread = None
try:
    _news_engine = NewsIntelligenceEngine()
    import threading
    _startup_thread = threading.Thread(target=_news_engine.update_news_loop, daemon=True)
    _startup_thread.start()
except Exception:
    pass  # Silent fail - no interfiere con quantum_core
