...
║  Port: 5559 (TCP Server) - Dynamic Risk Management with AI Learning          ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class VolatilityCalculator:
    ...
    def atr(prices, period=14):
        ...
        for i in range(1, len(prices)):
            # ⭐ FIX: True Range from close-to-close (no H/L available from close-only data)
        ...
        try:
            ...
            if has_prior:
                ...
                try:
                    ...
                    # ═══ RSI: Momentum-following (aligned with LLM1/LLM2 philosophy) ═══
                    # ⭐ FIX: Was inverted (mean-reversion) contradicting LLM1/LLM2 momentum-following
                    if fallback_rsi > 55:
                        bullish_signals += 1
                        prior_reasons.append(f"RSI({fallback_rsi:.0f})>55=BULL")
                    elif fallback_rsi < 45:
                        bearish_signals += 1
                        prior_reasons.append(f"RSI({fallback_rsi:.0f})<45=BEAR")
            ...
            # ⭐ FIX: Default to HOLD not BUY (was systematic long bias)
            direction = str(data.get('direction', 'HOLD')).upper()
            if prior_direction_hint:
            ...
            # ================================================================
            # ⭐ FIX: Use Trinity's ATR first (local calc has broken formula)
            atr_value = float(data.get('atr', data.get('atr_14', 0)))
            if atr_value <= 0:
            ...
            # - Si confianza < 60%: requiere RR > 1.5 (FIX: was 1.8 but rr_target=1.6 at this tier = impossible)
            if confidence >= 75:
                min_rr = 1.2  # IMPROVE: was 1.5
            elif confidence >= 60:
                min_rr = 1.5
            else:
                min_rr = 1.5  # FIX: was 1.8 but rr_target=1.6 at this tier, causing permanent WAIT
...
def run_tcp_server():
    ...
    try:
        ...
        port = config.get('llm_settings', {}).get('llm4_port', 5559)
    except:
        port = 5559  # LLM4 is RISK on port 5559
