...
╠══════════════════════════════════════════════════════════════════════════════╣
║  Inteligencia Bayesiana + Market Regime Detection + Temporal Intelligence    ║
║  Predicción probabilística calibrada con feedback learning                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import socket, struct, json, logging, threading, time, re, pickle, os
import requests
...
try:
    ...
    log_err = f"⚠️ Pattern/Divergence detectors not available: {e}"
...
OLLAMA_PORT = 11434
OLLAMA_MODEL = "llama3:70b-instruct-q4_K_M"
...
class IndicatorConsensusAnalyzer:
    ...
    def analyze_consensus(self, indicators_dict):
        """
        🎯 ADVANCED BAYESIAN CONSENSUS with Cross-Correlation Analysis
        
        Uses:
        - Pearson correlation between price/RSI/ADX
        - Hidden divergence detection (price vs momentum)
        ...
        # ═══════════════════════════════════════════════════════════════
        # 🧠 ADVANCED: Hidden Divergence Detection
        # ═══════════════════════════════════════════════════════════════
        # Divergence = Price making new highs but RSI not confirming
        # Hidden Bullish Div: Price HL, RSI LH → continuation bullish
        # Hidden Bearish Div: Price LH, RSI HL → continuation bearish
        
        divergence_signal = 0.0
        
        # RSI overbought but ADX strong = hidden bearish continuation
        if rsi > 65 and adx > 30:
            divergence_signal = -0.35  # Bearish exhaustion signal
        # RSI oversold but ADX strong = hidden bullish continuation
        elif rsi < 35 and adx > 30:
            divergence_signal = 0.35  # Bullish reversal signal
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 TREND ANALYSIS (Bayesian Prior: 30%)
        # ═══════════════════════════════════════════════════════════════
        # ADX > 30 = Strong trend (high confidence)
        # ADX 25-30 = Developing trend
        # ADX < 25 = Ranging (penalize directional trades)
        
        if adx > 30:
            # Strong trend - use price position for direction
            sma_aligned = indicators_dict.get('price_above_sma50', False)
            if sma_aligned:
                consensus_scores['BUY'] += 0.50  # Strong uptrend
            else:
                consensus_scores['SELL'] += 0.50  # Strong downtrend
        elif adx > 25:
            # Developing trend
            sma_aligned = indicators_dict.get('price_above_sma50', False)
            if sma_aligned:
                consensus_scores['BUY'] += 0.30
            else:
                consensus_scores['SELL'] += 0.30
        elif adx > 20:
            # Weak trend - REDUCIDO
            sma_aligned = indicators_dict.get('price_above_sma50', False)
            if sma_aligned:
                consensus_scores['BUY'] += 0.08  # Was 0.15 - REDUCIDO
            else:
                consensus_scores['SELL'] += 0.08
        else:
            # Ranging market - STRONG HOLD bias
            consensus_scores['HOLD'] += 0.40  # Was 0.25 - MÁS FUERTE
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 MOMENTUM ANALYSIS with RSI-MACD Correlation (30%)
        # ═══════════════════════════════════════════════════════════════
        # Perfect bullish setup: RSI 30-45 + MACD histogram turning positive
        # Perfect bearish setup: RSI 55-70 + MACD histogram turning negative
        
        # 🧠 ADVANCED: Check MACD momentum acceleration
        macd_prev = indicators_dict.get('macd_histogram_prev', macd_h)
        macd_accel = macd_h - macd_prev  # Positive = momentum accelerating
        
        if 30 < rsi < 45 and macd_h > 0:
            # Bullish momentum building from oversold
            boost = 0.65 if macd_accel > 0 else 0.55  # Extra if accelerating
            consensus_scores['BUY'] += boost
        elif 55 < rsi < 70 and macd_h < 0:
            # Bearish momentum building from overbought
            boost = 0.65 if macd_accel < 0 else 0.55
            consensus_scores['SELL'] += boost
        elif rsi < 35 and macd_h < -0.002:
            # Deep oversold with momentum - strong reversal candidate
            consensus_scores['BUY'] += 0.48
        elif rsi > 65 and macd_h > 0.002:
            # Deep overbought with momentum - strong reversal candidate
            consensus_scores['SELL'] += 0.48
        elif 40 < rsi < 60:
            # Neutral zone - use MACD for direction
            if abs(macd_h) > 0.001:
                if macd_h > 0:
                    consensus_scores['BUY'] += 0.25 if macd_accel > 0 else 0.18
                else:
                    consensus_scores['SELL'] += 0.25 if macd_accel < 0 else 0.18
            else:
                consensus_scores['HOLD'] += 0.15
        else:
            # RSI extreme without MACD confirmation = caution
            consensus_scores['HOLD'] += 0.10
        
        # Apply divergence signal
        if divergence_signal > 0:
            consensus_scores['BUY'] += divergence_signal
        elif divergence_signal < 0:
            consensus_scores['SELL'] += abs(divergence_signal)
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 VOLATILITY ANALYSIS with Bollinger Band Position (20%)
        # ═══════════════════════════════════════════════════════════════
        # BB %B < 0.2 = Oversold, near lower band → BUY
        # BB %B > 0.8 = Overbought, near upper band → SELL
        # BB squeeze (low ATR) = prepare for breakout
        
        if bb_pct < 0.15:
            # Extreme lower band - strong mean reversion BUY
            consensus_scores['BUY'] += 0.30
        elif bb_pct < 0.25:
            # Lower band area
            consensus_scores['BUY'] += 0.20
        elif bb_pct > 0.85:
            # Extreme upper band - strong mean reversion SELL
            consensus_scores['SELL'] += 0.30
        elif bb_pct > 0.75:
            # Upper band area
            ...
            # Mid-range - neutral
            consensus_scores['HOLD'] += 0.08
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 VOLUME CONFIRMATION (10%)
        # ═══════════════════════════════════════════════════════════════
        if obv_trend == 'UP':
            consensus_scores['BUY'] += 0.15
        elif obv_trend == 'DOWN':
            consensus_scores['SELL'] += 0.15
        else:
            consensus_scores['HOLD'] += 0.05
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 PATTERN CONFIRMATION (10%)
        # ═══════════════════════════════════════════════════════════════
        if pattern in ['BULLISH', 'ENGULFING_UP', 'HAMMER', 'MORNING_STAR']:
            consensus_scores['BUY'] += 0.18
        elif pattern in ['BEARISH', 'ENGULFING_DOWN', 'SHOOTING_STAR', 'EVENING_STAR']:
            consensus_scores['SELL'] += 0.18
        else:
            consensus_scores['HOLD'] += 0.05
        
        # ═══════════════════════════════════════════════════════════════
        # 🎯 NORMALIZE SCORES (Bayesian Posterior)
        # ═══════════════════════════════════════════════════════════════
...
class LLM1Experto:
    ...
    # ============================================================
    # 🔧 MEJORA: ANÁLISIS DE CONTEXTO DEL GRÁFICO
    # Agrega análisis de estructura HH/HL/LL/LH similar a LLM3
    # ============================================================
    
    def _analyze_price_structure(self, closes, highs, lows):
        """
        Análisis de estructura de mercado (HH/HL/LL/LH)
        Similar a LLM3 pero con enfoque bayesiano
        """
        if len(closes) < 10:
            return {'structure': 'UNKNOWN', 'confidence': 0.0, 'reason': 'Insufficient data'}
        ...
        mid_high = max(recent_highs[3:7])
        latest_high = max(recent_highs[-3:])
        mid_low = min(recent_lows[3:7])
        latest_low = min(recent_lows[-3:])
        
        # BULLISH: HH + HL
        if latest_high > mid_high and latest_low > mid_low:
            # Calcular confianza bayesiana basada en magnitud del movimiento
            hh_magnitude = (latest_high - mid_high) / mid_high
            hl_magnitude = (latest_low - mid_low) / mid_low
            confidence = min(0.95, 0.75 + (hh_magnitude + hl_magnitude) * 10)
            return {
                'structure': 'BULLISH_HH_HL', 
                'confidence': confidence,
                'reason': f'HH+HL confirmed (HH:{hh_magnitude*100:.2f}%, HL:{hl_magnitude*100:.2f}%)'
            }
        
        # BEARISH: LH + LL
        elif latest_high < mid_high and latest_low < mid_low:
            lh_magnitude = (mid_high - latest_high) / mid_high
            ll_magnitude = (mid_low - latest_low) / mid_low
            confidence = min(0.95, 0.75 + (lh_magnitude + ll_magnitude) * 10)
            return {
                'structure': 'BEARISH_LH_LL',
                'confidence': confidence,
                'reason': f'LH+LL confirmed (LH:{lh_magnitude*100:.2f}%, LL:{ll_magnitude*100:.2f}%)'
            }
        ...
        if range_pct < 0.005:  # Menos de 0.5% de rango
            return {
                'structure': 'CONSOLIDATION',
                'confidence': 0.70,
                'reason': f'Tight range ({range_pct*100:.3f}%)'
            }
        
        return {
            'structure': 'NEUTRAL',
            'confidence': 0.5,
            'reason': 'Mixed structure, no clear pattern'
        }
    
    def _analyze_candle_narrative(self, closes, opens, highs, lows):
        """
        Analizar las últimas 10-20 velas para entender la narrativa
        ¿Está en acumulación? ¿Distribución? ¿Breakout?
        """
        if len(closes) < 20:
            return 'UNKNOWN'
        ...
        last_volatility = np.std(last_10)
        prev_volatility = np.std(prev_10)
        
        last_momentum = last_10[-1] - last_10[0]
        prev_momentum = prev_10[-1] - prev_10[0]
        
        # Breakout: volatilidad aumenta + momentum fuerte
        if last_volatility > prev_volatility * 1.3 and abs(last_momentum) > abs(prev_momentum) * 1.5:
            return 'BREAKOUT_PHASE'
        
        # Acumulación: volatilidad baja + rango estrecho
        elif last_volatility < prev_volatility * 0.7:
            return 'ACCUMULATION_PHASE'
        
        # Distribución: momentum se debilita
        elif abs(last_momentum) < abs(prev_momentum) * 0.5:
            return 'DISTRIBUTION_PHASE'
        
        return 'TRENDING_PHASE'
    
    def _analyze_multi_candle_context(self, closes, highs, lows):
        """
        Análisis sistemático de últimas 20 velas
        Para entender la NARRATIVA completa del mercado
        """
        if len(closes) < 20:
            return {'context': 'INSUFFICIENT_DATA', 'confidence': 0, 'sections': []}
        
        # Últimas 20 velas en grupos de 5
        section_1 = closes[-20:-15]  # Más antigua
        section_2 = closes[-15:-10]
        section_3 = closes[-10:-5]
        section_4 = closes[-5:]       # Más reciente
        
        # Calcular momentum de cada sección
        m1 = section_1[-1] - section_1[0]
        m2 = section_2[-1] - section_2[0]
        m3 = section_3[-1] - section_3[0]
        m4 = section_4[-1] - section_4[0]
        
        sections_data = [
            {'section': 1, 'momentum': m1, 'direction': 'UP' if m1 > 0 else 'DOWN'},
            {'section': 2, 'momentum': m2, 'direction': 'UP' if m2 > 0 else 'DOWN'},
            {'section': 3, 'momentum': m3, 'direction': 'UP' if m3 > 0 else 'DOWN'},
            {'section': 4, 'momentum': m4, 'direction': 'UP' if m4 > 0 else 'DOWN'}
        ]
        
        # Detectar patrones
        if m1 > 0 and m2 > 0 and m3 > 0 and m4 > 0:
            return {
                'context': 'SUSTAINED_UPTREND',
                'confidence': 95,
                'sections': sections_data,
                'reason': 'All 4 sections positive momentum'
            }
        
        elif m1 < 0 and m2 < 0 and m3 < 0 and m4 < 0:
            return {
                'context': 'SUSTAINED_DOWNTREND',
                'confidence': 95,
                'sections': sections_data,
                'reason': 'All 4 sections negative momentum'
            }
            ...
            return {
                ...
                'confidence': 85,
                'sections': sections_data,
                'reason': 'Clear reversal from down to up'
            }
        
        elif m1 > 0 and m2 > 0 and m3 < 0 and m4 < 0:
            return {
                'context': 'REVERSAL_DOWN',
                'confidence': 85,
                'sections': sections_data,
                'reason': 'Clear reversal from up to down'
            }
        
        elif abs(m1) < 0.50 and abs(m2) < 0.50 and abs(m3) < 0.50:
            # FIX: was 0.0001 which is impossible for XAUUSD ($2900) - $0.50 is minimal M1 movement
            return {
                'context': 'CONSOLIDATION',
                'confidence': 80,
                'sections': sections_data,
                'reason': 'Very low momentum across sections'
            }
        ...
        return {
            ...
            'confidence': 50,
            'sections': sections_data,
            'reason': 'No clear directional pattern'
        }
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # 🧠 DEEP CANDLE INTELLIGENCE - El cerebro que SABE, no adivina
    # Analiza: morfología, body/wick ratios, volumen, tamaño vs ATR, trampas,
    # patrones de agotamiento, momentum real vs falso, velas institucionales
    # ════════════════════════════════════════════════════════════════════════════
    def _deep_candle_intelligence(self, opens, closes, highs, lows, volumes, atr, price, decision):
        """
        🧠 ANÁLISIS PROFUNDO DE VELAS - Cada vela cuenta una historia
        
        Componentes:
        1. MORFOLOGÍA: Body %, upper wick %, lower wick % → tipo de vela
        2. MOMENTUM REAL: Tamaño de body vs ATR → ¿movimiento real o ruido?
        3. VOLUMEN-PRECIO: ¿El volumen confirma el movimiento?
        4. TRAMPAS: Bull/Bear traps, liquidity sweeps, false breakouts
        5. AGOTAMIENTO: Wicks largos + body pequeño = fin de movimiento
        6. INSTITUCIONAL: Velas grandes con volumen = dinero inteligente
        """
        result = {'boost': 0, 'reasons': [], 'analysis': 'NO_DATA'}
        ...
        try:
            ...
            # Ultimas 10 velas para análisis
            O = [float(x) for x in opens[-10:]]
            C = [float(x) for x in closes[-10:]]
            H = [float(x) for x in highs[-10:]]
            L = [float(x) for x in lows[-10:]]
            V = [float(x) for x in volumes[-10:]] if len(volumes) >= 5 else [1.0] * len(O)
            
            # Asegurar misma longitud
            min_len = min(len(O), len(C), len(H), len(L), len(V))
            O, C, H, L, V = O[-min_len:], C[-min_len:], H[-min_len:], L[-min_len:], V[-min_len:]
            ...
            # ═══════════════════════════════════════════════════════════
            # 1. MORFOLOGÍA DE ÚLTIMAS 3 VELAS (body, wick ratios)
            # ═══════════════════════════════════════════════════════════
            last3_morphology = []
            for i in range(-3, 0):
                if abs(i) > len(O):
                ...
                if candle_range <= 0:
                    continue
                body = abs(c - o)
                body_pct = body / candle_range  # 0-1
            ...
            # ═══════════════════════════════════════════════════════════
            # 2. TAMAÑO DE VELA vs ATR (¿movimiento real o ruido?)
            # ═══════════════════════════════════════════════════════════
            if atr > 0 and last3_morphology:
                last_candle = last3_morphology[-1]
                candle_vs_atr = last_candle['range'] / atr
                
                if candle_vs_atr > 1.5:
                    # Vela EXPANSIVA: >150% del ATR = movimiento institucional
                    if last_candle['body_pct'] > 0.6:
                        # Body grande + vela grande = MOMENTUM REAL
                        if (last_candle['is_bullish'] and decision == 'BUY') or \
                           (not last_candle['is_bullish'] and decision == 'SELL'):
                            boost += 10
                            reasons.append(f"MOMENTUM_CANDLE({candle_vs_atr:.1f}xATR)")
                            analysis_parts.append("EXPANSION")
                        elif (last_candle['is_bullish'] and decision == 'SELL') or \
                             (not last_candle['is_bullish'] and decision == 'BUY'):
                            boost -= 8
                            reasons.append(f"CONTRA_MOMENTUM(-8)")
                            analysis_parts.append("CONTRA_EXPANSION")
                    else:
                        # Body pequeño + vela grande = INDECISIÓN/RECHAZO
                        boost -= 5
                        reasons.append(f"REJECTION_CANDLE({candle_vs_atr:.1f}xATR)")
                        analysis_parts.append("REJECTION")
                        
                elif candle_vs_atr < 0.3:
                    # Vela MICRO: <30% del ATR = sin movimiento real
                    boost -= 3
                    reasons.append("MICRO_CANDLE")
                    analysis_parts.append("NOISE")
            
            # ═══════════════════════════════════════════════════════════
            # 3. DETECCIÓN DE AGOTAMIENTO (Exhaustion)
            # ═══════════════════════════════════════════════════════════
            if len(last3_morphology) >= 3:
                # Patrón: Bodies decrecientes = momentum muriendo
                bodies = [m['body_pct'] for m in last3_morphology]
                if bodies[0] > bodies[1] > bodies[2] and bodies[2] < 0.3:
                    # Bodies cada vez más pequeños, último <30% body = AGOTAMIENTO
                    if decision in ['BUY', 'SELL']:
                        boost -= 7
                        reasons.append("EXHAUSTION_PATTERN")
                        analysis_parts.append("EXHAUSTED")
                
                # Patrón: Wicks largos consecutivos = zona de rechazo
                wicks_up = [m['upper_wick_pct'] for m in last3_morphology]
                wicks_down = [m['lower_wick_pct'] for m in last3_morphology]
                
                avg_upper_wick = sum(wicks_up) / len(wicks_up)
                avg_lower_wick = sum(wicks_down) / len(wicks_down)
                
                if avg_upper_wick > 0.45:
                    # Rechazo consistente arriba = RESISTENCIA FUERTE
                    if decision == 'BUY':
                        boost -= 8
                        reasons.append(f"UPPER_REJECTION_ZONE({avg_upper_wick:.0%})")
                        analysis_parts.append("RESIST_REJECT")
                    elif decision == 'SELL':
                        boost += 5
                        reasons.append(f"SELL_CONFIRM_REJECTION({avg_upper_wick:.0%})")
                        
                if avg_lower_wick > 0.45:
                    # Rechazo consistente abajo = SOPORTE FUERTE
                    if decision == 'SELL':
                        boost -= 8
                        reasons.append(f"LOWER_REJECTION_ZONE({avg_lower_wick:.0%})")
                        analysis_parts.append("SUPPORT_REJECT")
                    elif decision == 'BUY':
                        boost += 5
                        reasons.append(f"BUY_CONFIRM_SUPPORT({avg_lower_wick:.0%})")
            
            # ═══════════════════════════════════════════════════════════
            # 4. DETECCIÓN DE TRAMPAS (Bull/Bear Traps)
            # ═══════════════════════════════════════════════════════════
            if len(O) >= 5:
                # BULL TRAP: Precio rompe alto previo pero cierra abajo
                recent_high = max(H[-5:-1])  # Alto de las últimas 4 (no la actual)
                if H[-1] > recent_high and C[-1] < O[-1]:
                    # Rompió el alto pero cerró bajista = BULL TRAP
                    trap_strength = (H[-1] - recent_high) / atr if atr > 0 else 0
                    if trap_strength > 0.1:
                        if decision == 'BUY':
                            boost -= 12
                            reasons.append(f"🪤BULL_TRAP({trap_strength:.2f})")
                            analysis_parts.append("BULL_TRAP!")
                        elif decision == 'SELL':
                            boost += 8
                            reasons.append(f"TRAP→SELL_CONFIRM({trap_strength:.2f})")
                
                # BEAR TRAP: Precio rompe bajo previo pero cierra arriba
                recent_low = min(L[-5:-1])
                if L[-1] < recent_low and C[-1] > O[-1]:
                    trap_strength = (recent_low - L[-1]) / atr if atr > 0 else 0
                    if trap_strength > 0.1:
                        if decision == 'SELL':
                            boost -= 12
                            reasons.append(f"🪤BEAR_TRAP({trap_strength:.2f})")
                            analysis_parts.append("BEAR_TRAP!")
                        elif decision == 'BUY':
                            boost += 8
                            reasons.append(f"TRAP→BUY_CONFIRM({trap_strength:.2f})")
                
                # LIQUIDITY SWEEP: Wick largo que barre stops y regresa
                last_h, last_l, last_c, last_o = H[-1], L[-1], C[-1], O[-1]
                body_center = (last_c + last_o) / 2
                candle_range = last_h - last_l
                if candle_range > 0:
                    upper_sweep = (last_h - max(last_c, last_o)) / candle_range
                    lower_sweep = (min(last_c, last_o) - last_l) / candle_range
                    
                    if upper_sweep > 0.6 and last_c < last_o:
                        # Sweep arriba + cierre bajista = SELL signal
                        if decision == 'SELL':
                            boost += 6
                            reasons.append("LIQUIDITY_SWEEP_UP→SELL")
                        elif decision == 'BUY':
                            boost -= 6
                            reasons.append("LIQUIDITY_SWEEP_UP⚠️")
                    
                    if lower_sweep > 0.6 and last_c > last_o:
                        # Sweep abajo + cierre alcista = BUY signal
                        if decision == 'BUY':
                            boost += 6
                            reasons.append("LIQUIDITY_SWEEP_DOWN→BUY")
                        elif decision == 'SELL':
                            boost -= 6
                            reasons.append("LIQUIDITY_SWEEP_DOWN⚠️")
            
            # ═══════════════════════════════════════════════════════════
            # 5. VOLUMEN-PRECIO INTELIGENTE
            # ═══════════════════════════════════════════════════════════
            if len(V) >= 5 and any(v > 0 for v in V):
                avg_vol = sum(V[-5:]) / 5
                last_vol = V[-1]
                ...
                if avg_vol > 0:
                    ...
                    # Volumen confirma movimiento (>1.5x avg + body grande)
                    if vol_ratio > 1.5 and last3_morphology:
                        last_m = last3_morphology[-1]
                        if last_m['body_pct'] > 0.5:
                            if (last_m['is_bullish'] and decision == 'BUY') or \
                               (not last_m['is_bullish'] and decision == 'SELL'):
                                boost += 7
                                reasons.append(f"VOL_CONFIRMS({vol_ratio:.1f}x)")
                                analysis_parts.append("VOL_CONFIRM")
                    
                    # Divergencia: precio sube pero volumen baja = debilidad
                    if len(C) >= 5:
                        price_up = C[-1] > C[-5]
                        vol_declining = V[-1] < V[-3] < V[-5] if len(V) >= 5 else False
                        
                        if price_up and vol_declining and decision == 'BUY':
                            boost -= 6
                            reasons.append("VOL_DIVERGENCE_WEAK_BUY")
                            analysis_parts.append("VOL_DIV")
                        
                        price_down = C[-1] < C[-5]
                        if price_down and vol_declining and decision == 'SELL':
                            boost -= 6
                            reasons.append("VOL_DIVERGENCE_WEAK_SELL")
                    
                    # CLIMAX VOLUME: >3x avg = posible reversión
                    if vol_ratio > 3.0:
                        boost -= 4
                        reasons.append(f"CLIMAX_VOL({vol_ratio:.1f}x)⚠️")
                        analysis_parts.append("VOL_CLIMAX")
            
            # ═══════════════════════════════════════════════════════════
            # 6. TENDENCIA DE CUERPO DE VELAS (Candle Body Trend)
            # ═══════════════════════════════════════════════════════════
            if len(O) >= 5:
                bullish_bodies = sum(1 for i in range(-5, 0) if C[i] > O[i])
                bearish_bodies = sum(1 for i in range(-5, 0) if C[i] < O[i])
                
                # 4/5 o 5/5 velas en misma dirección = tendencia clara
                if bullish_bodies >= 4:
                    if decision == 'BUY':
                        boost += 5
                        reasons.append(f"BODY_TREND_UP({bullish_bodies}/5)")
                    elif decision == 'SELL':
                        boost -= 5
                        reasons.append(f"BODY_TREND_UP_CONTRA(-5)")
                
                if bearish_bodies >= 4:
                    if decision == 'SELL':
                        boost += 5
                        reasons.append(f"BODY_TREND_DN({bearish_bodies}/5)")
                    elif decision == 'BUY':
                        boost -= 5
                        reasons.append(f"BODY_TREND_DN_CONTRA(-5)")
            
            # Cap boost
            boost = max(-20, min(20, boost))
            
            analysis = '|'.join(analysis_parts) if analysis_parts else 'NEUTRAL'
            result = {'boost': boost, 'reasons': reasons, 'analysis': analysis}
            
        except Exception as e:
            log.debug(f"[CANDLE_INTEL] Error: {e}")
    ...
    def analyze(self, data):
        ...
        try:
            ...
            atr = float(data.get('atr', data.get('atr_14', 10)))
            ...
            # FIX CRITICAL: Trinity sends 'closes' not 'prices' - use closes as fallback
            prices = data.get('prices', data.get('closes', [price]))
            ...
            # ⭐ FIX: Try bar_data first (Trinity sends OHLC inside bar_data)
            bar_data = data.get('bar_data', {})
            opens = data.get('opens', bar_data.get('opens', prices))
            closes = data.get('closes', bar_data.get('closes', prices))
            highs = data.get('highs', bar_data.get('highs', prices))
            lows = data.get('lows', bar_data.get('lows', prices))
            ...
            opens = [float(o) for o in opens if o and o > 0][-20:]  # FIX: was [:20] taking FIRST 20 (oldest), now takes LAST 20 (newest)
            ...
            # ============================================================
            # 🔧 MEJORA: ANÁLISIS DE ESTRUCTURA DE MERCADO (NUEVO)
            # Análisis de contexto del gráfico - HH/HL/LL/LH
            # ============================================================
            price_structure = self._analyze_price_structure(closes, highs, lows)
            candle_narrative = self._analyze_candle_narrative(closes, opens, highs, lows)
            multi_candle_context = self._analyze_multi_candle_context(closes, highs, lows)
            
            # Log análisis de contexto
            log.info(f"[CONTEXT] Structure: {price_structure['structure']} @ {price_structure['confidence']*100:.0f}% | "
                    f"Narrative: {candle_narrative} | Multi-Candle: {multi_candle_context['context']}")
            ...
            # FIX: BB signal was INVERTED - buying at upper band (tops), selling at lower band (bottoms)
            # Upper BB = overbought = SELL signal, Lower BB = oversold = BUY signal (mean-reversion for M1)
            ...
            # 🎯 CALIBRATED: Lower thresholds for M1 scalping responsiveness
            ...
            # 🎯 PRECISION ENTRY: ADX filter for clean trends + net signal + momentum
            ...
            # ⭐ ANÁLISIS DE PATRONES ARMÓNICOS (GARTLEY, BUTTERFLY, CRAB) - NUEVA INTELIGENCIA
                ...
                try:
                    ...
                    if harmonic_patterns and len(harmonic_patterns) > 0:
                        ...
                        if (is_bullish and decision == 'BUY') or (is_bearish and decision == 'SELL'):
                            ...
                            log.info(f"[HARMONIC] ⭐ {pattern_type} (reliability={reliability:.2f}) | Decision={decision} | Boost:+{harmonic_boost}")
                        else:
                            # Si hay patrón pero contradice la decision, podría debilitar confianza
                            if is_bullish and decision == 'SELL':
                                log.warning(f"[HARMONIC] ⚠️ BULLISH {pattern_type} but decision is SELL - contradiction")
                            elif is_bearish and decision == 'BUY':
                                log.warning(f"[HARMONIC] ⚠️ BEARISH {pattern_type} but decision is BUY - contradiction")
            ...
            # ============================================================
            # 🔧 MEJORA: BOOST POR ANÁLISIS DE CONTEXTO
            # Aplicar bonos basados en estructura y narrativa del precio
            # ============================================================
            context_boost = 0
            context_reasons = []
            
            # ============================================================
            # 🧠 DEEP CANDLE INTELLIGENCE - Análisis REAL de velas
            # No adivina: SABE basándose en morfología, volumen, trampas
            # ============================================================
            candle_intel = self._deep_candle_intelligence(opens, closes, highs, lows, volumes, atr, price, decision)
            if candle_intel['boost'] != 0:
                context_boost += candle_intel['boost']
                context_reasons.extend(candle_intel['reasons'])
                log.info(f"[🧠 CANDLE_INTEL] {candle_intel['analysis']} | Boost: {candle_intel['boost']:+d}")
            
            # Boost por estructura HH/HL o LH/LL
            if price_structure['structure'] == 'BULLISH_HH_HL' and decision == 'BUY':
                structure_bonus = int(price_structure['confidence'] * 15)  # Max +15
                context_boost += structure_bonus
                context_reasons.append(f"HH+HL({structure_bonus:+d})")
            elif price_structure['structure'] == 'BEARISH_LH_LL' and decision == 'SELL':
                structure_bonus = int(price_structure['confidence'] * 15)  # Max +15
                context_boost += structure_bonus
                context_reasons.append(f"LH+LL({structure_bonus:+d})")
            
            # Boost por narrativa de velas
            if candle_narrative == 'BREAKOUT_PHASE' and decision in ['BUY', 'SELL']:
                context_boost += 8
                context_reasons.append("BREAKOUT")
            elif candle_narrative == 'TRENDING_PHASE' and decision in ['BUY', 'SELL']:
                context_boost += 5
                context_reasons.append("TRENDING")
            elif candle_narrative == 'CONSOLIDATION':
                # Penalizar trades en consolidación
                context_boost -= 10
                context_reasons.append("CONSOLIDATION(-10)")
            
            # Boost por contexto multi-vela
            if multi_candle_context['context'] == 'SUSTAINED_UPTREND' and decision == 'BUY':
                context_boost += int(multi_candle_context['confidence'] * 0.12)  # Max +11
                context_reasons.append("SUSTAINED_UP")
            elif multi_candle_context['context'] == 'SUSTAINED_DOWNTREND' and decision == 'SELL':
                context_boost += int(multi_candle_context['confidence'] * 0.12)  # Max +11
                context_reasons.append("SUSTAINED_DOWN")
            elif multi_candle_context['context'] == 'REVERSAL_UP' and decision == 'BUY':
                context_boost += 7
                context_reasons.append("REVERSAL_UP")
            elif multi_candle_context['context'] == 'REVERSAL_DOWN' and decision == 'SELL':
                context_boost += 7
                context_reasons.append("REVERSAL_DOWN")
            
            # Aplicar boost total de contexto
            if context_boost != 0:
                confidence = max(15, min(95, confidence + context_boost))
                log.info(f"[CONTEXT_BOOST] {'; '.join(context_reasons)} | Total: {context_boost:+d} → confidence={confidence}%")
            ...
            if price_structure['structure'] != 'NEUTRAL':
                reason_parts.append(f"Struct={price_structure['structure']}")
            if candle_narrative != 'UNKNOWN':
                reason_parts.append(f"Narrative={candle_narrative}")
            if multi_candle_context['context'] != 'MIXED':
                reason_parts.append(f"MultiCandle={multi_candle_context['context']}")
            ...
            # Añadir context boost si existe
            if context_boost != 0:
                reason_parts.append(f"Context({context_boost:+d})")
            
            reason = " | ".join(reason_parts)
            
            # ============================================================
            # FIX CRITICAL: Safety filters that were DEAD CODE after return
            # These MUST execute BEFORE returning the result
            # ============================================================
            
            # ADX check - don't trade in ranging markets
            # 🧠 FIX 2026-02-20: max(45,...) floor made LLM1 ALWAYS output 45% 
            # because ADX<22 fires most of the time and 55*0.7=38.5→clamp→45
            # NEW: max(25,...) lets the actual uncertainty show through
            if decision != 'HOLD' and adx < 22:
                log.warning(f"[LLM1] {decision} BLOCKED: ADX={adx:.1f} too low (ranging market)")
                decision = 'HOLD'
                confidence = max(25, int(confidence * 0.7))
            
            # RSI EXTREME check - don't BUY when overbought or SELL when oversold
            if decision == 'BUY' and rsi > 75:
                log.warning(f"[LLM1] BUY BLOCKED: RSI={rsi:.1f} OVERBOUGHT (>75)")
                decision = 'HOLD'
                confidence = max(35, int(confidence * 0.5))
            elif decision == 'SELL' and rsi < 25:
                log.warning(f"[LLM1] SELL BLOCKED: RSI={rsi:.1f} OVERSOLD (<25)")
                decision = 'HOLD'
                confidence = max(35, int(confidence * 0.5))
            ...
            result = {
                ...
                # ⭐ PATRONES ARMÓNICOS - NUEVA INTELIGENCIA
            ...
            # FIX CRITICAL: Error should return HOLD, not BUY - was causing random buy trades on any exception
            ...
            # 🧠 CRITICAL: MTF validation before final decision
            # Don't trade if multiple timeframes don't confirm
            if trinity_decision != 'HOLD' and mtf_validation.get('confirms', 0) < 2:
                log.warning(f"[LLM1] ❌ {trinity_decision} REJECTED: Only {mtf_validation['confirms']}/3 timeframes confirm")
                trinity_decision = 'HOLD'
                final_confidence = max(25, final_confidence * 0.6)
            
            # 🧠 CRITICAL: ADX check - don't trade in ranging markets
            if trinity_decision != 'HOLD' and adx < 22:
                log.warning(f"[LLM1] ❌ {trinity_decision} REJECTED: ADX={adx:.1f} too low (ranging market)")
                trinity_decision = 'HOLD'
                final_confidence = max(25, final_confidence * 0.7)
            
            # 🚨 CRITICAL: RSI EXTREME check - don't BUY when overbought or SELL when oversold
            if trinity_decision == 'BUY' and rsi > 75:
                log.warning(f"[LLM1] ❌ BUY REJECTED: RSI={rsi:.1f} OVERBOUGHT (>75)")
                trinity_decision = 'HOLD'
                final_confidence = max(35, final_confidence * 0.5)
            elif trinity_decision == 'SELL' and rsi < 25:
                log.warning(f"[LLM1] ❌ SELL REJECTED: RSI={rsi:.1f} OVERSOLD (<25)")
                trinity_decision = 'HOLD'
                final_confidence = max(35, final_confidence * 0.5)
            ...
            if DIAGNOSTICS_ENABLED and TRACER:
                TRACER.log_llm_decision(
                    ...
                    decision=trinity_decision,
                    confidence=final_confidence,
                    reasoning=f"Regime:{regime}({regime_prob:.1%}) RSI:{rsi} ADX:{adx} MACD:{macd_signal} Direction:{direction_score:.2f} MTF:{mtf_validation.get('confirms', 0)}/3"
                )
                if DASHBOARD:
...
{Y}|{R}                                                              {Y}|{R}
{Y}|{R}  {Y}███╗   ██╗{W} ██████╗ ██╗   ██╗ █████╗ {R}                       {Y}|{R}
{Y}|{R}  {Y}████╗  ██║{W}██╔═══██╗██║   ██║██╔══██╗{R}   {C}LLM1 EXPERT{R}         {Y}|{R}
{Y}|{R}  {Y}██╔██╗ ██║{W}██║   ██║██║   ██║███████║{R}   {D}Pattern Analysis{R}    {Y}|{R}
{Y}|{R}  {Y}██║╚██╗██║{W}██║   ██║╚██╗ ██╔╝██╔══██║{R}                       {Y}|{R}
{Y}|{R}  {Y}██║ ╚████║{W}╚██████╔╝ ╚████╔╝ ██║  ██║{R}                       {Y}|{R}
{Y}|{R}  {Y}╚═╝  ╚═══╝{W} ╚═════╝   ╚═══╝  ╚═╝  ╚═╝{R}                       {Y}|{R}
{Y}|{R}                                                              {Y}|{R}
...
{Y}|{R}  {G}● PORT: {port}{R}                  {D}Bayesian Market Model{R}        {Y}|{R}
{Y}+{'='*62}+{R}
""")
...
def run_tcp_server():
    ...
    try:
        ...
        port = config.get('llm_settings', {}).get('llm1_port', 5555)
    except:
        port = 5555  # Fallback
