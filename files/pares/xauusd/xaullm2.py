...
║  Port: 5557 (TCP Server) - NEVER FAILS, ALWAYS RESPONDS                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ============================================================
# LOGGING SETUP (UTF-8 for Windows compatibility with emojis)
# ============================================================
import sys
import io
...
class MonteCarloSimulator:
    ...
    @staticmethod
    def simulate_price_paths(current_price, atr, periods=10, simulations=500, drift=0):
        """Simula 500 escenarios de precio en próximas N barras con drift basado en momentum"""
        np.random.seed()
        ...
        for t in range(1, periods):
            returns = np.random.normal(drift, daily_volatility, simulations)
...
class LLM2TechnicalExpert:
    ...
    def __init__(self):
        ...
        self.memory = {'XAUUSD': deque(maxlen=300)}
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # ⭐ FIX: Extract MACD, SMA, BB indicators (Trinity sends these but LLM2 was ignoring them)
            macd_val = float(genome.get('macd', float(indicators.get('macd', 0))))
            macd_hist = float(genome.get('macd_histogram', float(indicators.get('macd_histogram', 0))))
            sma_50 = float(genome.get('sma_50', float(indicators.get('sma_50', 0))))
            sma_200 = float(genome.get('sma_200', float(indicators.get('sma_200', 0))))
            bb_upper = float(genome.get('bb_upper', float(indicators.get('bb_upper', 0))))
            bb_lower = float(genome.get('bb_lower', float(indicators.get('bb_lower', 0))))
            ...
            # ============================================================
            # MONTE CARLO SIMULATION (with momentum-based drift)
            # ============================================================
            # ⭐ FIX: Calculate drift from recent price action so MC isn't always ~50%
            if len(prices) >= 10:
                mc_drift = (float(prices[-1]) - float(prices[-10])) / float(prices[-10]) / 10
            elif len(prices) >= 5:
                mc_drift = (float(prices[-1]) - float(prices[-5])) / float(prices[-5]) / 5
            else:
                mc_drift = 0
            # Clamp drift to prevent extreme bias
            mc_drift = max(-0.005, min(0.005, mc_drift))
            mc_sim = MonteCarloSimulator.simulate_price_paths(price, atr, periods=10, simulations=500, drift=mc_drift)
            ...
            # ⭐ FIX: Initialize before conditional block to prevent NameError
            price_momentum_5 = 0
            momentum_accel = 0
            ...
            # Monte Carlo - MÁS EXIGENTE (now with drift, actually reaches thresholds)
            if bull_prob > 0.70:  # Was 0.65 - más estricto
                bull_signals += 0.8  # Was 1.0 - REDUCIDO
            elif bull_prob < 0.30:  # Was 0.35 - más estricto
                bear_signals += 0.8
            
            # ⭐ FIX: MACD indicator analysis (was completely ignored)
            if macd_val != 0 or macd_hist != 0:
                if macd_hist > 0 and macd_val > 0:
                    bull_signals += 0.6  # MACD histogram positive + line positive
                elif macd_hist < 0 and macd_val < 0:
                    bear_signals += 0.6  # MACD histogram negative + line negative
                elif macd_hist > 0 and macd_val < 0:
                    bull_signals += 0.3  # Histogram turning positive (early reversal)
                elif macd_hist < 0 and macd_val > 0:
                    bear_signals += 0.3  # Histogram turning negative (early reversal)
            
            # ⭐ FIX: SMA trend structure (was completely ignored)
            if sma_50 > 0 and sma_200 > 0:
                if price > sma_50 > sma_200:
                    bull_signals += 0.5  # Price above both MAs, golden cross structure
                elif price < sma_50 < sma_200:
                    bear_signals += 0.5  # Price below both MAs, death cross structure
                elif price > sma_50 and sma_50 < sma_200:
                    bull_signals += 0.2  # Price recovering but MAs still bearish
                elif price < sma_50 and sma_50 > sma_200:
                    bear_signals += 0.2  # Price weakening but MAs still bullish
            
            # ⭐ FIX: Bollinger Band analysis (was completely ignored)
            if bb_upper > 0 and bb_lower > 0 and bb_upper > bb_lower:
                bb_range = bb_upper - bb_lower
                bb_position = (price - bb_lower) / bb_range if bb_range > 0 else 0.5
                if bb_position > 0.95:
                    bear_signals += 0.4  # At upper band = overbought
                elif bb_position < 0.05:
                    bull_signals += 0.4  # At lower band = oversold
            
            # ⭐ FIX: fib_score used in decisions (was calculated but never used)
            if fib_score >= 30:
                # Near a key Fibonacci level - boosts signal in the appropriate direction
                if price <= support * 1.002 if support else False:
                    bull_signals += 0.5  # Near Fib support = bullish bounce potential
                elif price >= resistance * 0.998 if resistance else False:
                    bear_signals += 0.5  # Near Fib resistance = bearish rejection potential
            ...
            if bull_signals >= 2.2 and technical_score > 0.32:  # Adjusted for reduced scoring (was 2.5/0.30)
                ...
                # 🔥 FIX 2026-02-19: Lowered from 3/4 to 2/4 confirmations
                # On M1 Gold, ADX is often 15-25 and S/R proximity is unreliable,
                # so requiring 3/4 meant TECHNICAL almost NEVER voted directionally.
                if confirmations >= 2:
                    decision = 'BUY'
                    base_conf = 55 + (bull_signals * 5.0)
            ...
            # ═══════════════════════════════════════════════════════════════
                ...
                decision = 'HOLD'
                # 🔥 FIX 2026-02-19: Higher base confidence so HOLD from LLM2
                # doesn't dominate at 45-47% (was 40 + signal*20 = ~46)
                confidence = 35.0 + (signal_strength * 15.0)  # Now 35-50
                confidence = max(25.0, confidence)
            ...
            # RELAXED: Solo vetar en extremos muy fuertes (overbought>78, oversold<18)
            if decision == 'BUY' and rsi > 78:
                log.warning(f"[LLM2] ❌ BUY VETOED: RSI={rsi:.1f} OVERBOUGHT (>78)")
                decision = 'HOLD'
                confidence = 42
                reason = f'BUY vetoed: RSI={rsi:.1f} too high (overbought)'
            elif decision == 'SELL' and rsi < 18:
                log.warning(f"[LLM2] ❌ SELL VETOED: RSI={rsi:.1f} OVERSOLD (<18)")
                decision = 'HOLD'
                confidence = 42
...
def start_server():
    """Inicia servidor TCP en puerto 5557"""
    try:
        ...
        port = config.get('llm_settings', {}).get('llm2_port', 5557)
