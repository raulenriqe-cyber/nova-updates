...
SESSIONS = {
    'TOKYO':    (0, 9),     # 🔧 USDJPY peak activity during Tokyo session
    'LONDON':   (8, 12),
    'OVERLAP':  (12, 17),
    'NEW_YORK': (17, 21),
    'OFF':      (21, 24),
...
class NarrativeEngine:
    ...
    def generate_narrative(self, data: dict, macro: dict, cross: dict) -> dict:
        ...
        rsi = data.get('rsi', 50)
        adx = data.get('adx', 20)
        atr = data.get('atr', 1.0)  # 🔧 Calibrated for USDJPY
        ...
        # DXY cross-asset for USDJPY
        dxy = cross.get('dxy_proxy', 104)
        if dxy > 0:
            # DXY up = USDJPY up (positive correlation). Strong USD lifts USDJPY.
            if dxy > 105: buy_signals += 1
            elif dxy < 103: sell_signals += 1
...
class PhaseDetector:
    ...
    def detect_phase(self, data: dict) -> dict:
        ...
        if adx < 20 and bb_width < 0.005 and volume_ratio > 0.8:  # 🔧 Calibrated for USDJPY
            phase = 'ACCUMULATION'
        elif adx > 25 and price > sma_50 and macd_hist > 0:
            phase = 'MARKUP'
        elif adx < 25 and price > sma_200 and bb_width > 0.007 and volume_ratio < 0.9:  # 🔧 Calibrated for USDJPY
            phase = 'DISTRIBUTION'
        elif adx > 25 and price < sma_50 and macd_hist < 0:
            phase = 'MARKDOWN'
        else:
...
class ScenarioProjector:
    ...
    def generate_scenarios(self, data: dict, narrative: dict, phase: dict) -> dict:
        ...
        price = data.get('price', data.get('bid', 0))
        atr = data.get('atr', data.get('atr_14', 1.0))  # 🔧 Calibrated for USDJPY
        ...
        return {
            'bullish': {
                ...
                'target': round(price + atr * 2, 3),
                'description': f"Price rises to {price + atr * 2:.3f} (+{atr * 2:.3f})"
            },
            'neutral': {
                'probability': round(p_neutral, 2),
                'range': [round(price - atr * 0.5, 3), round(price + atr * 0.5, 3)],
                'description': f"Consolidation between {price - atr * 0.5:.3f} - {price + atr * 0.5:.3f}"
            },
            'bearish': {
                'probability': round(p_bear, 2),
                'target': round(price - atr * 2, 3),
                'description': f"Price drops to {price - atr * 2:.3f} (-{atr * 2:.3f})"
            }
        }
...
class StrategySelector:
    ...
    def select_strategy(self, dimensions: dict) -> dict:
        ...
        if session == 'LONDON':
            ...
            scores['TREND_CONTINUATION'] += 1.0
        elif session in ('ASIA', 'TOKYO'):
            # TOKYO: USDJPY highest-liquidity session — breakouts are real here
            if SYMBOL == 'USDJPY':
                scores['BREAKOUT_MOMENTUM'] += 1.5
                scores['TREND_CONTINUATION'] += 1.0
...
class SmartTPSL:
    ...
    def calculate(self, data: dict, strategy: str, conviction: dict,
        ...
        if price <= 0 or atr <= 0:
            return {'tp1': 0, 'tp2': 0, 'tp3': 0, 'sl': 0, 'rr_ratio': 0, 'rr_acceptable': False}  # 🔧 FIX CAT-76
        ...
        break_even_pips = exec_quality.get('break_even_pips', 0.15)  # 🔧 Calibrated for USDJPY
        # Ensure TP1 covers break-even cost (at minimum)
        min_tp1 = break_even_pips * 3  # 3x break-even minimum

        # ── Calculate levels ──
        if action == 'BUY':
            sl = round(price - atr * cfg['sl_atr'], 3)
            tp1 = round(price + max(atr * cfg['tp1_atr'], min_tp1), 3)
            tp2 = round(price + atr * cfg['tp2_atr'], 3)
            tp3 = round(price + atr * cfg['tp3_atr'], 3)
        else:  # SELL
            sl = round(price + atr * cfg['sl_atr'], 3)
            tp1 = round(price - max(atr * cfg['tp1_atr'], min_tp1), 3)
            tp2 = round(price - atr * cfg['tp2_atr'], 3)
        ...
        rr_ratio = reward1 / max(risk, atr * 0.001)  # 🔧 pair-safe floor (was 0.01)
        ...
        return {
            ...
            'tp2': tp2,
            'tp3': tp3,
            'risk_pips': round(risk, 3),
            'reward_pips': round(reward1, 3),
