...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                    🎯 NOVA TRADING AI - QUANTUM CORE v17.03                  ║
║                    USDJPY M15 - Samurai Predator Edition                      ║
║                       by Polarice Labs © 2026                                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Enterprise-Grade Algorithmic Trading Engine                                 ║
║  Advanced ML Consciousness with Trinity Integration                          ║
║  5-LLM Consensus + Kraken Execution + ConfluenceGate                         ║
╚══════════════════════════════════════════════════════════════════════════════╝

PROFESSIONAL ARCHITECTURE - 9 LLMs + News + 7 ML Systems:

🧠 9 LLMs (Trinity Consensus):
1. BAYESIAN - Análisis probabilístico
2. TECHNICAL - Análisis técnico puro
3. CHART - Pattern recognition
4. RISK - Risk management
5. SUPREME - Chart patterns (1000+)
6. NOVA - Smart Money Oracle
7. OCULUS - Data Quality Validator
8. CHRONOS - Timing Optimizer
9. PREDATOR - Execution Engine

📰 News Intelligence:
1. DuckDuckGo API + Ollama sentiment

⚙️ 7 ML Validation Systems:
1. Deep Neural Network Predictor (3-layer ensemble)
2. Attention-Based Market State Analyzer
3. Transformer-based Pattern Recognition
4. Adaptive Kalman Filtering for volatility
5. Genetic Algorithm Optimization
6. Reinforcement Learning Q-Network
7. Ensemble Stacking with meta-learner

Additional Systems:
- NOVA-MSDA Quality Scoring (0-100%)
- Kraken Sound Amplification (9 levels)
- Trinity Oracle (Consensus combiner)
- ConfluenceGate - Intelligent Attack Decision

INTEGRATION: PING/ACK heartbeat protocol (TCP keepalive)
"""
...
import json
import time
# FIX: Removed redundant 'import datetime' - using 'from datetime import datetime' at line 105
import logging
import socket
import threading
import struct
...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for USDJPY')
...
try:
    ...
    print("[WARNING] News Intelligence not available for USDJPY - news sentiment disabled")
...
try:
    ...
    try:
        ...
        from jpyllm5 import LLM5Client  # Pair-local fallback for USDJPY

# ⭐ LLM9 PREDATOR - EXECUTION ENGINE (Activating for JPY)
try:
    from jpyllm9 import PREDATOR as JpyLLM9Predator
    LLM9_PREDATOR_AVAILABLE = True
except ImportError:
    LLM9_PREDATOR_AVAILABLE = False
    print("[WARNING] LLM9 PREDATOR not available for USDJPY - execution engine disabled")
...
try:
    from jpyllm6 import (  # 🔧 FIX CAT-75: archivo es jpyllm6.py (no llm6.py)
        LLM6_SmartMoney,
        SmartMoneySignal,
        MarketIntention,
        SweepType
...
# 2. LLM COLLECTIVE WISDOM (0-1.5) - Sabiduría colectiva de los 5 LLMs
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        # 🎯 CALIBRATING THRESHOLDS - Finding attack point
        self.min_perfect_score = 5.0   # PERFECT - attack on 5.0+ 
        self.min_good_score = 4.5      # GOOD - quality setups
        self.min_wait_score = 3.5      # WAIT - minimum quality for attack
        self.min_sweep_score = 3.5     # SWEEP - trust smart money signals
        self.lock = Lock()
        ...
        # 🔧 FIX: Was hardcoded 5004 (XAUUSD port). USDJPY LLM6 = 7862
        self.llm6_port = 7862  # LLM6 NOVA Smart Money Oracle port (USDJPY)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        ...
        try:
            ...
            # Fallback: Extract from price_data.history if root-level not available
            ...
            request = {
                ...
                'symbol': genome.get('symbol', 'USDJPY'),
            }
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        # ═══════════════════════════════════════════════════════════════
        # 🧠 -1. ML LEARNING CONTEXT - Use Past Trades for Smarter Decisions
        # Query historical similar trades to adjust base probability
        # ═══════════════════════════════════════════════════════════════
        ml_context = {
            'direction': direction,
        ...
        ml_prob, ml_matches, ml_details = self.get_context_probability_from_learning(ml_context)
        if ml_matches >= 3:
            # Blend current base with ML learned probability (30% weight)
            base_prob = 0.70 * base_prob + 0.30 * ml_prob
            evidence.append(f"🧠 ML LEARNING: {ml_details} → base={base_prob:.2f}")
        ...
        if trinity_llms:
            ...
            if llm_count >= 3:
                ...
                # CONSENSO FUERTE: 70%+ LLMs en la misma dirección
                if consensus_ratio >= 0.70 and avg_conf >= 60:
                    llm_boost = 1.25 + (consensus_ratio - 0.70) * 0.5
                ...
                # CONSENSO MODERADO: 50-70% LLMs de acuerdo
                elif consensus_ratio >= 0.50:
                    llm_boost = 1.10 + (consensus_ratio - 0.50) * 0.25
        ...
        # ═══════════════════════════════════════════════════════════════
        # 5. SMART MONEY EVIDENCE (Sweeps, LLM6)
        # ═══════════════════════════════════════════════════════════════
        ...
        if llm6_sweep and llm6_sweep != 'NONE':
            ...
            if direction == 'BUY' and llm6_sweep in buy_sweeps:
                likelihood_modifiers.append(1.40)  # Smart money = fuerte señal
                evidence.append(f"💰 LLM6 Smart Money: {llm6_sweep} (+40%)")
                ...
                warnings.append(f"🚫 LLM6 {llm6_sweep} CONTRA BUY (-45%)")
            elif direction == 'SELL' and llm6_sweep in buy_sweeps:
                likelihood_modifiers.append(0.55)
                warnings.append(f"🚫 LLM6 {llm6_sweep} CONTRA SELL (-45%)")
        
        # False break probability
        if llm6_false > 60:
            likelihood_modifiers.append(0.70)
            warnings.append(f"⚠️ False break {llm6_false:.0f}% (-30%)")
        elif llm6_false < 20 and llm6_whale > 70:
            likelihood_modifiers.append(1.15)
            evidence.append(f"Whale conf {llm6_whale:.0f}%, low false break (+15%)")
        ...
        # ═══════════════════════════════════════════════════════════════
        # 7. RSI DIVERGENCE DETECTION - Predictor MUY potente
        # Divergencia = precio hace nuevo extremo pero RSI no confirma
        # ═══════════════════════════════════════════════════════════════
        
        if len(closes) >= 10 and len(highs) >= 10 and len(lows) >= 10:
            # Calcular RSI histórico simplificado para las últimas 10 velas
        ...
        # Using datetime imported at module level (line 105)
        now = datetime.utcnow()
        hour_utc = now.hour
        ...
        # ═══════════════════════════════════════════════════════════════
        # 📰 NEWS INTELLIGENCE BIAS - Sentimiento de noticias USDJPY
        # Las noticias afectan la dirección pero NO son determinantes
        # ═══════════════════════════════════════════════════════════════
        
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('USDJPY')
    ...
    def calculate_confluence_score(self, 
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For USDJPY at 1.08, 0.5% = 54 pips which is reasonable for fib zone
        ...
        for pattern in patterns:
            ...
            # Check if pattern aligns with decision
            if trinity_decision == 'BUY':
                for bp in bullish_patterns:
                    if bp in pattern_name_norm:
                        pattern_score += 0.6 * pattern_strength
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 📰 16. NEWS SENTIMENT INTELLIGENCE (0-1.0) - Sentimiento USDJPY
        ...
        ml_prediction_genome = genome.get('ml_prediction', 'NEUTRAL')
        dnn_signal = genome.get('dnn_signal', 0)
        
        # ML prediction CONFIRMA la dirección = bonus
        if ml_prediction_genome in ['BUY', 'SELL']:
            if (ml_prediction_genome == 'BUY' and trinity_decision == 'BUY') or \
               (ml_prediction_genome == 'SELL' and trinity_decision == 'SELL'):
                # DNN signal strength influences score (0-100)
                ml_boost = min(1.0, 0.3 + (dnn_signal / 150))  # Base 0.3 + up to 0.67
                ml_score = ml_boost
                intelligence_notes.append(f"🤖 ML/DNN: {ml_prediction_genome} confirma {trinity_decision} (+{ml_score:.2f})")
            
            # ML CONTRADICE la dirección = penalty
            elif (ml_prediction_genome == 'BUY' and trinity_decision == 'SELL') or \
                 (ml_prediction_genome == 'SELL' and trinity_decision == 'BUY'):
                ml_penalty = min(0.5, 0.2 + (dnn_signal / 200))  # Base 0.2 + up to 0.5
                ml_score = -ml_penalty
                intelligence_notes.append(f"🤖 ML/DNN: {ml_prediction_genome} CONTRA {trinity_decision} ({ml_score:.2f})")
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🧠 ML PREDICTION CONFLICT VETO (NEW - 2026-02-04)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 📈 SMART TREND VETO (INTELLIGENT - 2026-02-04)
            # Bloquea trades contra tendencia EXCEPTO si:
            # 1. Confianza altísima (>78%) - posible reversión anticipada
            # 2. MAs están convergiendo (cambio de tendencia inminente)
            # 3. Hora de alta liquidez (sesión asiática para USDJPY)
            # ═══════════════════════════════════════════════════════════════
            ma5 = indicators.get('ma5', indicators.get('ma_fast', 0))
            ma20 = indicators.get('ma20', indicators.get('ma_slow', 0))
            ...
            # Detectar hora actual (UTC) para sesiones de alta liquidez
            # Using datetime imported at module level (line 105)
            current_hour_utc = datetime.utcnow().hour
            # Sesión ASIÁTICA (mejor para USDJPY): 00:00-08:00 UTC (Tokyo/Sydney)
            # Sesión LONDON/NY overlap: 12:00-16:00 UTC
            high_liquidity_session = (0 <= current_hour_utc <= 8) or (12 <= current_hour_utc <= 16)
            
            if ma5 and ma20 and current_price:
                # Calcular convergencia de MAs (cambio de tendencia inminente)
                ma_diff_pct = ((ma5 - ma20) / ma20) * 100 if ma20 else 0
                ma_converging = abs(ma_diff_pct) < 0.05  # MAs muy cerca = posible cruce
                ...
                if ma50:
                    ...
                    # Sin MA50, usar solo MA5 vs MA20
                ...
                # ═══════════════════════════════════════════════════════════
                # 🧠 CONDICIONES PARA PERMITIR CONTRA-TENDENCIA
                # ═══════════════════════════════════════════════════════════
                if trinity_confidence >= 78:
                    # Confianza ALTÍSIMA = puede anticipar reversión
                    allow_counter_trend = True
                    notes.append(f"✅ Alta confianza {trinity_confidence:.0f}% permite contra-tendencia")
                
                if ma_converging:
                    # MAs convergiendo = cambio de tendencia inminente
                    allow_counter_trend = True
                    notes.append(f"✅ MAs convergiendo ({ma_diff_pct:.3f}%) - cambio inminente")
                
                # RSI extremo también puede indicar reversión
                rsi = indicators.get('rsi', 50)
                ...
                # ═══════════════════════════════════════════════════════════
                # 🚫 APLICAR VETO SOLO SI NO HAY EXCEPCIÓN
                # ═══════════════════════════════════════════════════════════
                if not allow_counter_trend:
                    ...
                    # También vetar si MAs muy desalineadas
                ...
                # Log info de tendencia para debugging
                trend_dir = "ALCISTA" if ma_diff_pct > 0 else "BAJISTA"
                logger.debug(f"📈 TREND: {trend_dir} | MA_diff={ma_diff_pct:.3f}% | Converging={ma_converging} | Allow_CT={allow_counter_trend} | Session={'ASIA/OVERLAP' if high_liquidity_session else 'LOW'}")
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 VETO POR PROBABILIDAD MUY BAJA - Relajado
            # Si la probabilidad es <28%, NO atacar bajo ningún concepto
            # ⭐ RELAJADO: Ahora que LLMs contribuyen, bajar threshold
            # ═══════════════════════════════════════════════════════════════
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🧠 CRITICAL: IMMEDIATE PRICE MOMENTUM FILTER
            # Verifica que las últimas velas se muevan en la dirección correcta
            # Evita comprar mientras el precio CAE o vender mientras SUBE
            # ═══════════════════════════════════════════════════════════════
            price_data = genome.get('price_data', {})
            closes = price_data.get('close', [])[-10:]  # Últimas 10 velas
            
            # Calcular momentum inmediato (últimas 3-5 velas)
            if len(closes) >= 5:
                # Momentum de las últimas 5 velas
                momentum_5 = closes[-1] - closes[-5]  # Precio actual vs hace 5 velas
                momentum_3 = closes[-1] - closes[-3]  # Precio actual vs hace 3 velas
                momentum_1 = closes[-1] - closes[-2]  # Último movimiento
                
                # Contar velas verdes vs rojas (últimas 5)
                green_candles = sum(1 for i in range(-5, 0) if len(closes) > abs(i) and 
                                   closes[i] > closes[i-1])
                red_candles = 5 - green_candles
                
                # ═══════════════════════════════════════════════════════════
                # REGLA DE ORO: No ir contra el momentum inmediato
                # ═══════════════════════════════════════════════════════════
                if trinity_decision == 'BUY':
                    # HARD BLOCK solo en casos EXTREMOS (todas las velas rojas + caída fuerte)
                    # XAUUSD: $2.00+ = movimiento muy fuerte en 5 velas M1
                    is_falling_extreme = momentum_5 < -2.0 and momentum_3 < -1.0 and red_candles >= 5
                    
                    if is_falling_extreme:
                        # BLOQUEAR BUY solo si precio cayendo MUY fuerte (crash)
                        rsi = indicators.get('rsi', 50)
                        if rsi > 25:  # Solo bloquear si RSI no está en oversold extremo
                            self._clear_pending(trinity_decision)
                            return False, 'HOLD', 0, \
                                   f"🚫 BUY BLOCKED: Crash detectado ({red_candles}/5 rojas, mom={momentum_5:.2f})"
                        else:
                            notes_str = f"⚠️ BUY contra-tendencia (RSI oversold {rsi:.0f})"
                    
                elif trinity_decision == 'SELL':
                    # HARD BLOCK solo en casos EXTREMOS (todas las velas verdes + subida fuerte)
                    is_rising_extreme = momentum_5 > 2.0 and momentum_3 > 1.0 and green_candles >= 5
                    
                    if is_rising_extreme:
                        # BLOQUEAR SELL solo si precio subiendo MUY fuerte (rally)
                        rsi = indicators.get('rsi', 50)
                        if rsi < 75:  # Solo bloquear si RSI no está en overbought extremo
                            self._clear_pending(trinity_decision)
                            return False, 'HOLD', 0, \
                                   f"🚫 SELL BLOCKED: Rally detectado ({green_candles}/5 verdes, mom={momentum_5:.2f})"
                        else:
                            notes_str = f"⚠️ SELL contra-tendencia (RSI overbought {rsi:.0f})"
            ...
            # Verificar momentum inmediato antes de override
            # PERMISIVO: Solo bloquea si movimiento MUY fuerte contra dirección
            momentum_ok = True
            if len(closes) >= 3:
                last_momentum = closes[-1] - closes[-3]
                # XAUUSD moves ~$0.50-1.00 per candle, $1.50+ = movimiento fuerte
                if trinity_decision == 'BUY' and last_momentum < -1.5:
                    momentum_ok = False  # BUY pero últimas 3 velas bajando MUY fuerte
                elif trinity_decision == 'SELL' and last_momentum > 1.5:
                    momentum_ok = False  # SELL pero últimas 3 velas subiendo MUY fuerte
            ...
            # Combina Score + Consciousness Probability para decisión INTELIGENTE
            # ═══════════════════════════════════════════════════════════════
            
            # 🎯 TIER 0: CONSCIOUSNESS GOLD - Probabilidad >70% = SABE que va a ganar
            if consciousness_prob >= 0.70 and momentum_ok:
                self._clear_pending(trinity_decision)
            ...
            if score >= self.min_perfect_score and momentum_ok and consciousness_prob >= 0.55:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence, \
                       f"⭐⭐⭐⭐⭐ SNIPER (Score={score:.1f}, Prob={consciousness_prob:.0%})"
            
            # 🎯 TIER 2: ELITE - Score bueno + Prob decente
            if score >= self.min_good_score and momentum_ok and consciousness_prob >= 0.50:
                self._clear_pending(trinity_decision)
            ...
            if score >= self.min_wait_score and momentum_ok and consciousness_prob >= 0.48:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 5: Trinity Force - Solo si Prob es decente (>=45%)
            if trinity_confidence >= 60 and momentum_ok and score >= 2.5 and consciousness_prob >= 0.45:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.85, \
                       f"⭐ TRINITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Conf={trinity_confidence}%)"
            
            # 🎯 TIER 6: High Confidence Override - Requiere Prob mínima
            if trinity_confidence >= 75 and score >= 2.0 and consciousness_prob >= 0.42:
                self._clear_pending(trinity_decision)
            ...
            reason = f"🧠 NO EDGE: Prob={consciousness_prob:.0%}, Score={score:.1f} | {block_str[:50]}"
    ...
    def _validate_direction_ultra_exigent(
        ...
        # ═══════════════════════════════════════════════════════════════
        # 6. CANDLE MOMENTUM VALIDATION (últimas 3-5 velas)
        # ═══════════════════════════════════════════════════════════════
        price_data = genome.get('price_data', {})
        ...
        if len(closes) >= 3:
            ...
            if direction == 'BUY':
                if momentum_3 < -2.0:  # Cayendo fuerte $2+
                    validations_failed += 1
                    failures.append(f"Momentum 3v={momentum_3:.2f} contra BUY")
                elif momentum_3 > 0.5:  # Subiendo
                    validations_passed += 1
                    confirmations.append("Momentum alcista confirma BUY")
            else:  # SELL
                if momentum_3 > 2.0:  # Subiendo fuerte $2+
                    validations_failed += 1
                    failures.append(f"Momentum 3v={momentum_3:.2f} contra SELL")
                elif momentum_3 < -0.5:  # Cayendo
                    validations_passed += 1
    ...
    # ═══════════════════════════════════════════════════════════════
    # 🧠 ML LEARNING INTEGRATION - Use Past Trades to Make Smarter Decisions
    # ═══════════════════════════════════════════════════════════════
    
    def set_ml_feedback(self, feedback_system):
        """Connect ML learning feedback system to ConfluenceGate"""
    ...
    def get_pattern_boost_from_learning(self, patterns: list, direction: str) -> float:
        """
        🧠 INTELLIGENT PATTERN BOOST from ML Learning
        
        Uses historical pattern winrates to boost/penalize confidence.
        If a pattern has 80% winrate historically, it gets a boost.
        If a pattern has 30% winrate historically, it gets a penalty.
        
        Returns: Boost multiplier (0.5 to 1.5)
        """
        if not hasattr(self, 'ml_feedback') or not self.ml_feedback:
            return 1.0  # No boost if no ML system
        
        try:
            memory_bank = getattr(self.ml_feedback, 'memory_bank', None)
            if not memory_bank:
                return 1.0
            
            # Get pattern statistics from memory bank
            pattern_stats = getattr(memory_bank, 'pattern_stats', {})
            ...
            for pattern in patterns:
                pattern_key = f"{pattern}_{direction}"
                if pattern_key in pattern_stats:
                    stats = pattern_stats[pattern_key]
                    wins = stats.get('wins', 0)
                    total = stats.get('total', 0)
                    
                    if total >= 5:  # Need minimum 5 trades for statistical significance
                        winrate = wins / total
                        # Winrate 80% → 1.3 boost, 50% → 1.0, 20% → 0.7
                        pattern_boost = 0.7 + (winrate * 0.6)
                        total_boost += pattern_boost
                        pattern_count += 1
            
            if pattern_count > 0:
                avg_boost = total_boost / pattern_count
                # Clamp between 0.5 and 1.5
                return max(0.5, min(1.5, avg_boost))
    ...
    def get_context_probability_from_learning(self, context: dict) -> tuple:
        """
        🧠 INTELLIGENT CONTEXT MATCHING from ML Learning
        
        Finds similar historical trading episodes and calculates
        the probability of success based on how similar trades performed.
        
        Returns: (probability: float 0-1, num_matches: int, details: str)
        """
        if not hasattr(self, 'ml_feedback') or not self.ml_feedback:
            return 0.5, 0, "No ML system connected"
        
        try:
            memory_bank = getattr(self.ml_feedback, 'memory_bank', None)
            if not memory_bank:
                return 0.5, 0, "No memory bank available"
            
            # Find similar episodes
            # 🔧 FIX CAT-77: find_similar_episodes may not exist in LLMMemoryBank
            if not hasattr(memory_bank, 'find_similar_episodes'):
                return 0.5, 0, "find_similar_episodes not implemented"
            similar_episodes = memory_bank.find_similar_episodes(context, max_results=10)
            
            if not similar_episodes:
                return 0.5, 0, "No similar episodes found"
            
            # Calculate win probability from similar episodes
            wins = sum(1 for ep in similar_episodes if ep.get('result') == 'win')
            total = len(similar_episodes)
            
            if total > 0:
                probability = wins / total
                # Weight by recency - more recent episodes are more relevant
                avg_similarity = sum(ep.get('similarity', 0.5) for ep in similar_episodes) / total
                
                details = f"Found {total} similar trades: {wins}W/{total-wins}L ({probability*100:.0f}% WR)"
                
                # Blend base 50% with learned probability (30% weight)
                blended_prob = 0.7 * 0.5 + 0.3 * probability
                
                return blended_prob, total, details
            
            return 0.5, 0, "No episodes to analyze"
            
        except Exception as e:
            return 0.5, 0, f"ML Error: {e}"
...
class NOVAMarketStateDetector:
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Frozen market detection (market closed or no data) - RELAXED PENALTY
        # 🔧 2025-02: Reducido -40 a -20 para permitir más agresividad en demo
        if bid == ask and spread == 0:
            self.alerts.append("🚫 FROZEN_MARKET: bid==ask (market closed or no data)")
            liquidity_score -= 20  # 🔧 Era -40, ahora -20
        
        # Spread too wide (illiquidity)
        elif spread > 0.0005:  # For USDJPY, abnormally wide
            self.alerts.append(f"⚠️ WIDE_SPREAD: {spread:.6f} pips (liquidity crisis)")
            liquidity_score -= 25
...
class Config:
    HOST = '127.0.0.1'
    # ═══════════════════════════════════════════════════════════════════════════
    # 🎯 USDJPY PORT LAYOUT (78xx range) - Different from EURUSD (68xx range)
    # ═══════════════════════════════════════════════════════════════════════════
    QUANTUM_PORT = 7854      # Injector - receive genomes from Quimera
    EXECUTOR_PORT = 7856     # Executor - send commands to Quimera
    TRINITY_PORT = 7866      # Trinity server port (USDJPY)
    KRAKEN_PORT = 7852       # Kraken order executor
    LLM1_PORT = 7855         # LLM1 Neural Predictor
    LLM2_PORT = 7857         # LLM2 Feature Extractor
    LLM3_PORT = 7858         # LLM3 Support/Resistance
    LLM4_PORT = 7859         # LLM4 Risk Calculator
    LLM5_PORT = 7861         # LLM5 SUPREME chart analyzer (AEGIS)
    LLM6_PORT = 7862         # ⭐ LLM6 NOVA Smart Money Oracle
    LLM11_PORT = 7867        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 7868        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 7870        # 🔊 USDJPY Sound Engine
    
    # Trading - USDJPY M5/M1 PRECISION SCALPING (Maximum Data Intelligence)
    # Strategy: Use M1 data for precise entries, M5 for confirmation
    # ═══════════════════════════════════════════════════════════════════════════
    # 🎯 USDJPY M5/M1: ATR M1=2-5 pips, M5=4-10 pips
    # TP óptimo: 1.5-2x ATR, SL óptimo: 0.8-1x ATR → R:R = 1.5:1 a 2:1
    # M1 provee data valiosa para timing perfecto de entrada
    # ═══════════════════════════════════════════════════════════════════════════
    TIMEFRAME = 'M5'         # Timeframe principal (M5 para decisiones)
    EXECUTION_TF = 'M1'      # Timeframe de ejecución (M1 para timing preciso)
    
    TP_MIN_PIPS = 10       # 8 pips mínimo (realista para M5 scalping)
    TP_MAX_PIPS = 40      # 25 pips máximo (capturar movimientos buenos)
    SL_MIN_PIPS = 8       # 5 pips mínimo SL (tight scalping)
    SL_MAX_PIPS = 20      # 12 pips máximo SL (control estricto)
    ATR_FLOOR = 3.0       # USDJPY M5: ATR mínimo 3 pips para considerar trade
    CONSOLIDATION_THRESHOLD = 5.0    # Umbral consolidación para M5
    
    # USDJPY específico - Point/Pip conversion
    POINT_VALUE = 0.001    # USDJPY: 1 point = 0.00001
    PIP_VALUE = 0.01       # USDJPY: 1 pip = 10 points = 0.0001
    DIGITS = 3               # USDJPY tiene 5 decimales
    
    # Risk - SNIPER PRECISION (Not aggressive, but surgical)
    # 🔧 FIX: SAMURAI — TOKYO OPEN EDITION v1.1
    MAX_CONCURRENT_TRADES = 3        # Up to 3 concurrent trades (3 katanas)
    MAX_DAILY_LOSS_PERCENT = 2.0     # Strict loss control - protect capital
    MAX_CONSECUTIVE_LOSSES = 3       # FIX A-JPY-02: halt after 3 consecutive losses
    LOSS_HALT_DURATION_SECONDS = 300 # FIX A-JPY-02: 5-minute cooldown after streak
    MIN_CONFIDENCE = 58              # 🔧 SAMURAI v1.1 - 58%+ (Was 60)
    MIN_TRINITY_CONFIDENCE = 53      # 🔧 SAMURAI v1.1 - Consensus Baseline
    MIN_ADX = 18                     # 🔧 SAMURAI v1.1 - Breakout Entry (Was 22)
    ...
    # 🔧 SAMURAI v1.1 - Optimized Ranges
    MIN_RSI_FOR_BUY = 30             # 🔧 SAMURAI v1.1
    MAX_RSI_FOR_BUY = 70             # 🔧 SAMURAI v1.1
    MIN_RSI_FOR_SELL = 30            # 🔧 SAMURAI v1.1 (Symmetric)
    MAX_RSI_FOR_SELL = 70            # 🔧 SAMURAI v1.1
    ...
    # SNIPER COOLDOWN - M5/M1 Precision Scalping
    SNIPER_COOLDOWN_SECONDS = 90    # 🔧 AUMENTADO: 90s (era 45s) - menos trades
    WARMUP_SECONDS = 30             # 🔧 AUMENTADO: 30s warmup (era 15s)
    
    # Session-Aware Trading (UTC hours)
    LONDON_OPEN_UTC = 7       # London opens 07:00 UTC
    LONDON_CLOSE_UTC = 16     # London closes 16:00 UTC  
    NY_OPEN_UTC = 12          # NY opens 12:00 UTC
    NY_CLOSE_UTC = 21         # NY closes 21:00 UTC
    BEST_HOURS_START = 8      # Mejor hora: 08:00-11:00 UTC (London morning)
    BEST_HOURS_END = 15       # y 12:00-15:00 UTC (London/NY overlap)
    ...
    # ⚔️ STRATEGY SELECTOR (loaded from jpyconfig.yaml)
    ACTIVE_STRATEGY = "SAMURAI"  # Default, will be overridden by yaml

    @classmethod
    def load_strategy_from_yaml(cls):
        """Load active strategy from jpyconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'jpyconfig.yaml')
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    config = yaml.safe_load(f)
                strategy = config.get('active_strategy', 'SAMURAI').upper()
                cls.ACTIVE_STRATEGY = strategy
                print(f"[CONFIG] ⚔️ USDJPY strategy loaded: {strategy}")
                return strategy
        except Exception as e:
            print(f"[CONFIG] Error loading USDJPY strategy: {e} - using default SAMURAI")
        return 'SAMURAI'
...
class NOVADashboard:
    ...
    def __init__(self):
        ...
        self.data = {
            ...
            'ask': 0.0,
            'spread': 0.0,
            'last_price': 150.00,  # USDJPY típico ~150
            'price_change': 0.0,
            'price_change_pct': 0.0,
            
            # Indicators (calibrados para USDJPY M15)
            'rsi': 0.0,  # FIXED: No default 50, must be real-time
            'macd': 0.0,
            'macd_signal': 0.0,
            'macd_hist': 0.0,
            'atr': 30.0,  # USDJPY M15 típico = 30 pips
            'adx': 0.0,
            'bb_upper': 0.0,
            'bb_middle': 0.0,
            'bb_lower': 0.0,
            ...
            'llm9_status': 'ONLINE',  # ⭐ LLM9 EXECUTION ENGINE (ACTIVATED)
            ...
            'llm10_status': 'IDLE',  # ⭐ LLM10 MSDA - Market State Detection Agent (Auditor)
            'llm10_vote': 'HOLD',  # MSDA audits the consensus decision
            'llm10_conf': 0,  # MSDA confidence = consensus confidence
    ...
    def render(self):
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'USDJPY'))
        ...
        lines.append("")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}███╗   ██╗{self.RESET}{SOFT_WHITE}{self.BOLD} ██████╗ ██╗   ██╗ █████╗ {self.RESET}    {self.CYAN}{self.BOLD}MARKET{self.RESET} {self.WHITE}{self.BOLD}USDJPY{self.RESET}  •  {self.GREEN}●{self.RESET} {now.strftime('%H:%M:%S')}  •  Up {self.CYAN}{uptime_str}{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}NOVA Trading AI{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}██╔██╗ ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██║   ██║██║   ██║███████║{self.RESET}    {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        lines.append(f"  {self.CYAN}{self.BOLD}MARKET{self.RESET}  {self.WHITE}{self.BOLD}{d['symbol']:<8}{self.RESET} {self.CYAN}(M15 Samurai Predator Edition){self.RESET}  {price_color}{self.BOLD}{self._format_price(d['last_price']):>10}{self.RESET}  {self._format_change(d['price_change'])}  {self._format_change(d['price_change_pct'], pct=True)}    {self.CYAN}{sparkline}{self.RESET}  {trend_arrow}{self.YELLOW}{d['trend']:^9}{self.RESET}")
        ...
        # ══════════════════════════════════════════════════════════════════════════════════════
        # 🧠 9-LLM NEURAL BRAIN - CLEAN LAYOUT
        # ══════════════════════════════════════════════════════════════════════════════════════
        ...
        lines.append(f"  Adv   {l6}  {l7}  {l8}  {l9}  {l10}    Quality {d.get('llm7_quality_score',0):2.0f}%  Timing {d.get('llm8_timing_multiplier',1.0):.2f}x  R/R {d.get('llm9_rr_ratio',0.0):.1f}:1")
...
class LLMParallelConsensus:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-79: 5 workers para llm1-5 en paralelo real
        self.logger = logger
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome).encode('utf-8')
...
# ==================== ADAPTIVE TP/SL ENGINE (USDJPY M15 DAY TRADING) ====================
class AdaptiveTPSLEngine:
    """
    🎯 USDJPY M15 Day Trading TP/SL Engine
    Dynamically adjust TP/SL based on:
    - ATR (volatility) - primary driver
    - ADX (trend strength) - confidence booster
    - RSI (momentum exhaustion) - timing
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 🎯 USDJPY M5/M1 CONSTANTS - PRECISION SCALPING CALIBRATED
    # ═══════════════════════════════════════════════════════════════════════════
    # USDJPY M5 typical ATR: 4-10 pips, M1 ATR: 2-5 pips
    # Using M1 for precise entries, M5 for confirmation
    # Optimal TP: 1.5-2x ATR, Optimal SL: 0.8-1x ATR → R:R = 1.5:1 to 2:1
    # ═══════════════════════════════════════════════════════════════════════════
    
    POINT_VALUE = 0.001    # USDJPY: 1 point = 0.00001
    PIP_VALUE = 0.01       # USDJPY: 1 pip = 10 points
    
    # ATR to Pips conversion: ATR ya viene en pips de Quimera
    ATR_MIN_PIPS = 2         # Mínimo ATR para considerar trade (M1 puede ser bajo)
    ATR_MAX_PIPS = 15        # Máximo ATR razonable M5
    
    # TP/SL Multipliers (basados en ATR) - M5/M1 SCALPING
    BASE_TP_MULTIPLIER = 2.0  # TP = ATR × 2.0 (≈10-16 pips si ATR=5-8)
    BASE_SL_MULTIPLIER = 0.9  # SL = ATR × 0.9 (≈4-7 pips tight stops)
    
    # Hard limits en PIPS - M5/M1 SCALPING
    TP_MIN_PIPS = 6           # Mínimo 6 pips TP (scalping agresivo)
    TP_MAX_PIPS = 40          # Máximo 25 pips TP (no sobreextender)
    SL_MIN_PIPS = 4           # Mínimo 4 pips SL (muy tight)
    SL_MAX_PIPS = 20          # Máximo 12 pips SL (control estricto)
    
    def __init__(self):
        self.atr_values = deque(maxlen=100)
        self.volatility_regimes = deque(maxlen=50)
        self.success_history = deque(maxlen=100)
        self.session_multiplier = 1.0
    
    def _get_session_multiplier(self):
        """
        🕐 Session-aware trading multiplier
        London/NY overlap (12:00-16:00 UTC) = best liquidity = tighter spreads
        Asian session = lower volatility = reduce expectations
        """
        # FIX: Use datetime class directly (already imported at top level)
        now_utc = datetime.utcnow()
        hour = now_utc.hour
        
        # London/NY Overlap: 12:00-16:00 UTC - PRIME TIME
        if 12 <= hour < 16:
            return 1.15  # +15% TP extension (más movimiento)
        # London Session: 07:00-12:00 UTC - Good volatility
        elif 7 <= hour < 12:
            return 1.05  # +5% TP extension
        # NY Afternoon: 16:00-20:00 UTC - Still decent
        elif 16 <= hour < 20:
            return 1.0   # Normal
        # Asian/Off-hours: 20:00-07:00 UTC - Low volatility
        else:
            return 0.85  # -15% TP (conservative)
    
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
                                     support_resistance=None, spread=0.00010):
        """
        🎯 Calculate INTELLIGENT TP/SL for USDJPY M5/M1 Precision Scalping
        
        Uses ALL available intelligence from M1 high-frequency data:
        - ATR (primary volatility measure - M1 granularity)
        ...
        """
        # ═══════════════════════════════════════════════════════════════
        # EXTRACT INDICATORS (M1 data for precision)
        # ═══════════════════════════════════════════════════════════════
        # ATR viene en PIPS de Quimera (ya convertido)
        atr_pips = float(indicators.get('atr', 6))  # Default 6 pips M5
        rsi = float(indicators.get('rsi', 50))
        ...
        if direction == 'BUY':
            if rsi > 72:
                # Overbought BUY - reduce TP (reversal likely)
                tp_multiplier *= 0.70
            elif rsi < 28:
                # Oversold BUY - EXCELLENT entry, extend TP
                tp_multiplier *= 1.25
            elif rsi < 40:
                # Good BUY zone
                tp_multiplier *= 1.10
            ...
            if rsi < 28:
                # Oversold SELL - reduce TP
                tp_multiplier *= 0.70
            elif rsi > 72:
                # Overbought SELL - EXCELLENT entry
                tp_multiplier *= 1.25
            elif rsi > 60:
                # Good SELL zone
                tp_multiplier *= 1.10
        ...
        if bb_width > 0:
            ...
            if direction == 'BUY':
                ...
                if bb_position < 0.15:
                    ...
                    # Near upper band - excellent SELL
                    tp_multiplier *= 1.20
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 6: SESSION TIMING (Liquidity Awareness)
        # ═══════════════════════════════════════════════════════════════
        ...
        if support_resistance:
            ...
            if direction == 'BUY':
                ...
                if dist_to_resistance > 0 and dist_to_resistance < atr_pips * tp_multiplier:
                    # Reduce TP to 90% of resistance distance
        ...
        # Calculate final prices (5 decimals for USDJPY)
        if direction == 'BUY':
        ...
        logger.debug(f"[TP/SL M15 EU] ATR={atr_pips:.1f}p | ADX={adx:.0f} | RSI={rsi:.0f} | "
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        """
        USDJPY M5/M1 Precision Scalping: Calculate TP/SL based on volatility regime
        
        Args:
            current_price: Current USDJPY price (e.g., 1.0850)
            atr: ATR in PIPS (e.g., 6 pips for M5)
            direction: 'BUY' or 'SELL'
            win_rate: Historical win rate 0-1
            volatility_regime: 'HIGH', 'NORMAL', or 'LOW'
        """
        # ═══════════════════════════════════════════════════════════════
        # USDJPY M5/M1 PRECISION SCALPING MULTIPLIERS
        # M5 ATR típico: 4-10 pips, M1 ATR: 2-5 pips
        # TP óptimo: 1.8-2.2x ATR, SL óptimo: 0.7-0.9x ATR (tight stops)
        # ═══════════════════════════════════════════════════════════════
        if volatility_regime == 'HIGH':
            # High volatility: extend TP, slightly wider SL
            atr_multiplier_tp = 2.2   # TP = 2.2x ATR (~13 pips si ATR=6)
            atr_multiplier_sl = 0.9   # SL = 0.9x ATR (~5 pips)
        elif volatility_regime == 'LOW':
            # Low volatility: quick scalps
            atr_multiplier_tp = 1.6   # TP = 1.6x ATR (~10 pips)
            atr_multiplier_sl = 0.75  # SL = 0.75x ATR (~4.5 pips)
        else:  # NORMAL
            # Normal regime: balanced scalping
            atr_multiplier_tp = 2.0   # TP = 2x ATR (~12 pips)
            atr_multiplier_sl = 0.85  # SL = 0.85x ATR (~5 pips)
        
        # Win rate adjustment (más agresivo en scalping)
        if win_rate > 0.60:
            # Winning streak: más agresivo
            atr_multiplier_tp *= 1.15  # +15% TP
            atr_multiplier_sl *= 0.85  # -15% SL (más tight)
        elif win_rate < 0.40:
            # Losing streak: conservador
            atr_multiplier_tp *= 0.85  # -15% TP
            atr_multiplier_sl *= 1.15  # +15% SL (protección)
        
        # Calculate raw distances in pips
        tp_pips = atr * atr_multiplier_tp
        sl_pips = atr * atr_multiplier_sl
        ...
        # USDJPY M15 HARD LIMITS (in pips)
        tp_pips = max(self.TP_MIN_PIPS, min(self.TP_MAX_PIPS, tp_pips))  # 12-40 pips
        sl_pips = max(self.SL_MIN_PIPS, min(self.SL_MAX_PIPS, sl_pips))  # 7-20 pips
        
        # Convert pips to price distance (USDJPY: 1 pip = 0.0001)
        tp_distance = tp_pips * self.PIP_VALUE
        sl_distance = sl_pips * self.PIP_VALUE
        ...
        if direction == 'BUY':
            if tp <= current_price or sl >= current_price:
                # Force valid values (15 pips TP, 10 pips SL)
                tp = round(current_price + (15 * self.PIP_VALUE), 5)
                sl = round(current_price - (10 * self.PIP_VALUE), 5)
        else:  # SELL
            if tp >= current_price or sl <= current_price:
                tp = round(current_price - (15 * self.PIP_VALUE), 5)
                sl = round(current_price + (10 * self.PIP_VALUE), 5)
        
        return tp, sl, int(tp_pips), int(sl_pips)
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.01) if atr > 0 else 0  # [NOVA AUDIT F-11] USDJPY pip=0.01, ATR M1 ~2-10 pips
...
class MarketMicrostructureAnalyzer:
    ...
    def analyze_tick(self, bid, ask, volume, direction):
        ...
        spread = ask - bid
        spread_pips = spread * 10000  # For USDJPY
...
class DeepNeuralNetwork:
    ...
    def predict(self, features):
        ...
        try:
            # BUGFIX: Convert features to dict if it's a list BEFORE any .get() calls
...
class MarketCycleAnalyzer:
    ...
    def detect_sweep(self, prices, support_level, resistance_level, tolerance_pips=0.5):
        """
        SWEEP DETECTION: Detecta si el precio tocó soporte/resistencia RECIENTEMENTE
        
        Sweep = precio tocó S/R y REBOTÓ en dirección opuesta
        Esto valida que el nivel es REAL y hay REVERSIÓN
        
        FIX: Lógica correcta de rebote
        - SUPPORT SWEEP: Precio baja, toca soporte (closes[-2] <= support), luego sube (close[-1] > closes[-2])
        - RESISTANCE SWEEP: Precio sube, toca resistencia (closes[-2] >= resistance), luego baja (close[-1] < closes[-2])
        """
        ...
        # BUG FIX #35: Protect against accessing prices[-1], [-2], [-3] without validation
        if len(prices) < 3:
            return {'sweep_detected': False, 'type': 'NONE', 'entry_quality': 0}
        
        try:
            current_price = float(prices[-1]) if isinstance(prices[-1], (int, float)) else float(prices[-1].get('close', 0))
            prev_price = float(prices[-2]) if isinstance(prices[-2], (int, float)) else float(prices[-2].get('close', 0))
            two_back_price = float(prices[-3]) if isinstance(prices[-3], (int, float)) else float(prices[-3].get('close', 0))
        ...
        # ============ SUPPORT SWEEP UP ============
        # Pattern: Precio toca soporte y rebota UP
        # Trigger: prev_price <= support (tocó soporte)
        #         current_price > prev_price (rebotó hacia arriba)
        if prev_price <= (support_level + tolerance) and prev_price >= (support_level - tolerance):
            # Vela anterior tocó soporte (dentro de tolerancia)
            if current_price > prev_price:
                # Precio rebotó hacia ARRIBA - SWEEP confirmado
                return {
                    'sweep_detected': True,
                    ...
                    'entry_quality': 85,
                    'recommendation': 'BUY',
                    'description': f'🟢 SWEEP CONFIRMADO: Precio tocó soporte ${support_level:.2f} y rebotó UP',
                    'pattern_detected': True
                }
        
        # ============ RESISTANCE SWEEP DOWN ============
        # Pattern: Precio toca resistencia y rebota DOWN
        # Trigger: prev_price >= resistance (tocó resistencia)
        #         current_price < prev_price (rebotó hacia abajo)
        if prev_price <= (resistance_level + tolerance) and prev_price >= (resistance_level - tolerance):
            # Vela anterior tocó resistencia (dentro de tolerancia)
            if current_price < prev_price:
                # Precio rebotó hacia ABAJO - SWEEP confirmado
                return {
                    'sweep_detected': True,
                    ...
                    'entry_quality': 85,
                    'recommendation': 'SELL',
                    'description': f'🔴 SWEEP CONFIRMADO: Precio tocó resistencia ${resistance_level:.2f} y rebotó DOWN',
                    'pattern_detected': True
                }
        
        # ============ EXTENDED SUPPORT SWEEP UP ============
        # Si no hay sweep en la última vela, busca en las últimas 2 velas (M1 puede ser lento)
        if two_back_price <= (support_level + tolerance) and two_back_price >= (support_level - tolerance):
            # Hace 2 velas tocó soporte
            if current_price > two_back_price:
                # Precio rebotó hacia ARRIBA desde hace 2 velas
                return {
                    'sweep_detected': True,
                    'type': 'SUPPORT_SWEEP_UP',
                    'level': support_level,
                    'entry_quality': 75,  # Calidad ligeramente menor (más antiguo)
                    'recommendation': 'BUY (soporte validado hace 2 velas)',
                    'pattern_detected': True
                }
        
        # ============ EXTENDED RESISTANCE SWEEP DOWN ============
        if two_back_price <= (resistance_level + tolerance) and two_back_price >= (resistance_level - tolerance):
            # Hace 2 velas tocó resistencia
            if current_price < two_back_price:
                # Precio rebotó hacia ABAJO desde hace 2 velas
                return {
                    'sweep_detected': True,
                    'type': 'RESISTANCE_SWEEP_DOWN',
                    'level': resistance_level,
                    'entry_quality': 75,  # Calidad ligeramente menor (más antiguo)
                    'recommendation': 'SELL (resistencia validada hace 2 velas)',
                    'pattern_detected': True
                }
        
        # No hay sweep reciente
        return {'sweep_detected': False, 'type': 'NONE', 'entry_quality': 0}
...
class QuantumConsciousnessEngine:
    """Master engine integrating all 12 systems + advanced feedback loops"""
    
    def __init__(self):
        self.logger = logger
        self.core = None  # Reference to QuantumCore (set after creation)
        ...
        # 🎯 NEW: Advanced Trap & Exhaustion Detection
        ...
        self.cooldown_passed_logged = False  # Track if we already logged cooldown expiration
        ...
        self.rl_engine = RealQLearningAgent(learning_rate=0.15, discount_factor=0.95, epsilon_start=0.3, strategy_tag=Config.ACTIVE_STRATEGY, pip_factor=100)  # JPY: pip=0.01 → factor=100
        ...
        if CALIBRATION_ADVISOR_AVAILABLE:
            self.calibration_advisor = CalibrationAdvisor(
                ...
                pair=getattr(Config, 'SYMBOL', 'JPY'),
        ...
        # ⭐ LLM9 PREDATOR EXECUTION ENGINE (for trade calculations)
        if LLM9_PREDATOR_AVAILABLE:
            self.llm9_predator = JpyLLM9Predator()
            self.llm9_enabled = True
            self.logger.info("[✅ LLM9] PREDATOR Execution Engine initialized for USDJPY")
        else:
            self.llm9_enabled = False
            self.llm9_predator = None
            self.logger.warning("[⚠️ LLM9] PREDATOR Execution Engine not available - using fallback")
        ...
        # 🧠 CONNECT ML LEARNING TO CONFLUENCE GATE for intelligent decision making
        confluence_gate.set_ml_feedback(self.llm_feedback)
        self.logger.info("[✅ ML Integration] ConfluenceGate connected to LLM Learning - using past trades for smarter decisions")
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None):
        """Full consciousness processing with all 12 systems integrated
        ...
        # ⭐ NOTE: genome_counter is now incremented in run() loop when genome arrives
        # Removed duplicate increment here to prevent double-counting
        
        # ⭐⭐⭐ CRITICAL FIX: CALCULATE & ADD INDICATORS TO GENOME IMMEDIATELY
        # The genome from Quimera does NOT have indicators yet
        # Must transform it first to extract RSI, MACD, ADX, ATR for the dashboard
        try:
            # FIX: Use core's _transform_quimera_to_trinity method (not self - wrong class)
            if self.core:
                transformed = self.core._transform_quimera_to_trinity(genome)
            else:
                self.logger.warning(f"[Consciousness] Core reference not set - cannot transform genome")
                transformed = None
            if transformed and 'indicators' in transformed:
                # Copy indicators back to original genome for dashboard display
                genome['indicators'] = transformed['indicators']
                self.logger.info(f"[Consciousness] ✅ Genome indicators calculated: RSI={genome['indicators']['current'].get('rsi', 0):.1f} ADX={genome['indicators']['current'].get('adx', 0):.1f}")
            else:
                self.logger.warning(f"[Consciousness] ⚠️ Transform failed or no indicators - dashboard may show 0%")
        except Exception as e:
            self.logger.warning(f"[Consciousness] Transform error: {e} - continuing without indicators")
        ...
        # ⭐ SAMURAI FORMAT: bid/ask at root level, NOT in tick_data
        # CRITICAL: Try multiple price keys since different data sources use different names
        bid = float(genome.get('bid') or tick_data.get('bid', 0) or tick_data.get('Bid', 0))
        ask = float(genome.get('ask') or tick_data.get('ask', 0) or tick_data.get('Ask', 0))
        
        # ⭐ SAMURAI FORMAT: price in timeframes[0].close
        timeframes = genome.get('timeframes', [])
        primary_tf = timeframes[0] if timeframes else {}
        
        # Try multiple keys for price: last, close, price, current, etc.
        current_price = (
            float(primary_tf.get('close', 0)) or
        ...
        spread = float(genome.get('spread') or 0) or ((ask - bid) * 100 if bid > 0 and ask > 0 else 0)  # In pips
        
        # Extract indicators from genome
        # ⭐ SAMURAI FORMAT: Use _samurai_indicators or primary_tf
        samurai_ind = genome.get('_samurai_indicators', {})
        indicators = genome.get('indicators', {}).get('current', {})
        if not indicators:
            indicators = genome.get('indicators', {})
        
        # Priority: SAMURAI -> primary_tf -> indicators -> defaults
        rsi_val = float(samurai_ind.get('rsi') or primary_tf.get('rsi') or indicators.get('rsi', 50.0))
        ...
        adx_val = float(samurai_ind.get('adx') or primary_tf.get('adx') or indicators.get('adx', 10.0))
        atr_val = float(samurai_ind.get('atr') or primary_tf.get('atr') or indicators.get('atr', 0.0))
        
        # ⭐ DEBUG: Log genome structure to identify indicator issue
        self.logger.debug(f"[GENOME-DEBUG] Indicators structure: samurai={bool(samurai_ind)} primary_tf={bool(primary_tf)}")
        self.logger.debug(f"[GENOME-DEBUG] Values - RSI:{rsi_val:.1f} MACD:{macd_val:.4f} ADX:{adx_val:.1f} ATR:{atr_val:.4f}")
        ...
        # ═══════════════════════════════════════════════════════════════════════════════════════
        # 🔥 CALCULATE ML SYSTEMS, PATTERNS & ORDER FLOW BEFORE DASHBOARD UPDATE
        # This ensures dashboard ALWAYS gets complete data in ONE update call
        # ═══════════════════════════════════════════════════════════════════════════════════════
        
        # Extract prices for ML calculations
        prices_list = price_data.get('history', [])
        closes = np.array([p.get('close', 0) for p in prices_list]) if prices_list else np.array([current_price])
        highs = np.array([p.get('high', 0) for p in prices_list]) if prices_list else np.array([current_price])
        lows = np.array([p.get('low', 0) for p in prices_list]) if prices_list else np.array([current_price])
        
        # Calculate ML predictions (DNN + LSTM)
        ml_prediction = {'direction': 'NEUTRAL', 'confidence': 0}
        ...
        try:
            if len(closes) >= 20:
                # DNN prediction
                features = self.dnn.extract_advanced_features(prices_list)
                ...
                if hasattr(self, 'bayesian'):
                    indicators_bayes = {
                        'rsi': rsi_val,
                        'macd_histogram': macd_hist
                    }
                    bayesian_bull, bayesian_bear = self.bayesian.update_beliefs(indicators_bayes)
        except Exception as e:
            self.logger.debug(f"[ML-Calc] Error: {e}")
        
        # Calculate Bayesian from LLM votes
        bayesian_bull_llm = self._calculate_bayesian_from_llms(trinity_llms, 'BUY')
        bayesian_bear_llm = self._calculate_bayesian_from_llms(trinity_llms, 'SELL')
        ...
        try:
            if len(closes) >= 5 and hasattr(self, 'pattern_engine_advanced'):
                patterns = self.pattern_engine_advanced.analyze_patterns(closes, highs, lows) or []
                patterns_detected = self._extract_pattern_names(patterns)
                pattern_count = len(patterns)
                bullish_patterns = sum(1 for p in patterns if self._is_bullish_pattern(p))
                bearish_patterns = sum(1 for p in patterns if self._is_bearish_pattern(p))
        except Exception as e:
            self.logger.debug(f"[Patterns-Calc] Error: {e}")
        ...
        try:
            ...
            if hasattr(self, 'microstructure'):
                order_imbalance = self.microstructure.order_imbalance
        except Exception as e:
            self.logger.debug(f"[OrderFlow-Calc] Error: {e}")
        ...
        # ═══════════════════════════════════════════════════════════════════════════════════════
        # 🎯 SINGLE COMPLETE DASHBOARD UPDATE - All data included
        # ═══════════════════════════════════════════════════════════════════════════════════════
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM10 MSDA - Market State Detection Agent (Auditor)
            llm10_status='ONLINE' if trinity_llms else 'IDLE',  # Always active if trinity is running
            llm10_vote=trinity_decision,  # MSDA audits the consensus decision
            llm10_conf=int(trinity_confidence),  # MSDA confidence = consensus confidence
            ...
            # 🎯 ML SYSTEMS (NOW CALCULATED BEFORE UPDATE)
            ml_prediction=ml_prediction.get('direction', 'NEUTRAL'),
            ml_confidence=max(0, min(1.0, consensus_score)),
            dnn_signal=ml_prediction.get('confidence', 0),
            lstm_momentum=lstm_forecast.get('momentum', 0),
            rl_action='HOLD',  # Placeholder - calculated in full consciousness if needed
            bayesian_bull=bayesian_bull_llm,  # Use LLM-based calculation
            bayesian_bear=bayesian_bear_llm,  # Use LLM-based calculation
            anomaly_score=0.0,  # Placeholder
            
            # 🎯 PATTERNS (NOW CALCULATED BEFORE UPDATE)
            patterns_detected=patterns_detected,
            ...
            pattern_visual_notes=pattern_intel.get('visual_notes', [])[:3],
            
            # 🎯 ORDER FLOW (NOW CALCULATED BEFORE UPDATE)
            ...
            # 🎯 ATTACK STATUS (NOW CALCULATED BEFORE UPDATE)
        ...
        )  # ✅ SINGLE COMPLETE DASHBOARD UPDATE - All ML/Patterns/OrderFlow calculated before this
        ...
        if self.last_trade_time > 0 and time_since_trade < self.cooldown_seconds:
            ...
            # Reset cooldown_passed flag when in cooldown
            self.cooldown_passed_logged = False
        ...
        # Cooldown passed! Log it ONLY ONCE
        if self.last_trade_time > 0 and not self.cooldown_passed_logged:
            self.logger.warning(f"[COOLDOWN_PASSED] ✅ Cooldown expired ({time_since_trade:.1f}s >= {self.cooldown_seconds}s) - ready to attack")
            self.cooldown_passed_logged = True
        ...
        try:
            ...
            # llm_consensus = self.llm_parallel.query_all_llms(genome, timeout=1.5)
            llm_consensus = {'decision': 'HOLD', 'confidence': 0, 'num_llms_responding': 0, 'individual_votes': {}}
            ...
            if trinity_confidence > 60:
                ...
                weights = {
                    ...
                    'llm_parallel': 0.12,      # Decrease
                    'ml_ensemble': 0.10,       # Decrease
                    'lstm_momentum': 0.08,     # Decrease
                    'rl_agent': 0.04,          # Decrease
                    'bayesian': 0.04,          # Decrease
                    'microstructure': 0.02,    # Decrease
                    'llm7_quality': 0.06,      # ⭐ Mantener LLM7
                    'llm8_timing': 0.07,       # ⭐ Mantener LLM8
                    'llm9_execution': 0.07,    # ⭐ Mantener LLM9
                }
            ...
            # CRITICAL FIX: If Trinity is >= 67% confident, consensus_score should be AT LEAST that high
            # This ensures the dashboard attack_probability reflects Trinity's confidence
            if trinity_confidence >= 67:
                consensus_score = max(consensus_score, trinity_confidence / 100.0)
                self.logger.warning(f"[Consensus] Boosting to Trinity confidence: {trinity_confidence}% (score was {(consensus_score - trinity_confidence/100.0) * 100:.0f}%)")
            ...
            # 🔧 2025-02: Relajado 65%→63% - Skip anomaly check if Trinity confident
            if is_anomaly and anomaly_score < -0.5 and trinity_confidence < 63:
                self.logger.warning(f"[ANOMALY_VETO] 🚫 Blocking: is_anomaly={is_anomaly}, score={anomaly_score:.2f} < -0.5, trinity_conf={trinity_confidence}% < 63%")
                return {'action': 'HOLD', 'reason': 'anomalous_market', 'anomaly_score': anomaly_score}
            
            # ============ SPOOFING DETECTION VETO ============
            # 🔧 2025-02: Relajado 60%→58% - Skip spoofing check if Trinity confident
            if spoofing_detected and trinity_confidence < 58:
                self.logger.warning(f"[SPOOFING_VETO] 🚫 Blocking: spoofing detected + Trinity {trinity_confidence}% < 58%")
            ...
            # 🔧 2025-02: Relajado -2% para más agresividad
            if trend_type == 'RANGING' and trinity_decision in ['BUY', 'SELL']:
                ...
                # CRITICAL: Skip if Trinity is highly confident (>58%) - Trinity override
                if consensus_score < 0.53 and trinity_confidence < 58:
                    self.logger.warning(f"[TREND_VETO] 🚫 Blocking: RANGING market, consensus={consensus_score*100:.0f}% < 53%, trinity={trinity_confidence}% < 58%")
                    return {'action': 'HOLD', 'reason': 'range_bound_market', 'trend': trend_type, 'score': consensus_score}
                elif trinity_confidence >= 58:
            ...
            # 🔧 2025-02: Relajado 50%→48% para más agresividad
            if consensus_score < 0.48 and trinity_decision in ['BUY', 'SELL']:
                consensus_reason = "insufficient_consensus"  # FIX: Define missing variable
                self.logger.warning(f"[CONSENSUS_VETO] 🚫 BLOCKING: Consensus {consensus_score*100:.0f}% < 48% (reason: {consensus_reason}) - Too risky even with Trinity {trinity_confidence}%")
                return {
                    'action': 'HOLD',
            ...
            # � RELAXED THRESHOLD FOR USDJPY: Trinity must be >= 73% to execute
            # 🔧 2025-02: Relajado 75%→73% para más agresividad
            if trinity_confidence >= 73.0:
                self.logger.info(f"[CONFIDENCE_CHECK] ✅ Trinity {trinity_confidence}% >= 73% + Consensus {consensus_score*100:.0f}% - proceeding with {trinity_decision}")
                pass  # Continue to decision execution below
            elif trinity_confidence < Config.MIN_TRINITY_CONFIDENCE:
                self.logger.warning(f"[CONFIDENCE_VETO] 🚫 Blocking: Trinity {trinity_confidence}% < {Config.MIN_TRINITY_CONFIDENCE}% minimum")
                return {
                    'action': 'HOLD',
                    'reason': 'low_trinity_confidence',
                    'trinity_confidence': trinity_confidence,
                    'threshold': Config.MIN_TRINITY_CONFIDENCE
                }
            
            # ============ CONSOLIDATION TRAP DETECTION (RELAXED FOR M5 USDJPY) ============
            # For M5 USDJPY scalping, we can trade even in tight ranges
            ...
            # 🔧 FIX: USDJPY ATR is in 5-digit format (0.0003-0.0020)
            # Only block if ATR is extremely low (< 3 pips = 0.0003)
            if atr_value < 0.0003:  # 🔧 USDJPY: 3 pips minimum
                self.logger.warning(f"[CONSOLIDATION_VETO] 🚫 Blocking: ATR={atr_value:.5f} < 0.0003 (too tight)")
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                # ============ WHALE TRAP LOGIC ============
                # IF TRAP DETECTED:
                #  - WARN but ALLOW BUY (don't block, use tight stops)
                #  - ALLOW SELL (take profits from the fake breakout)
                if is_trap and trap_confidence > 50:
                    if trinity_decision == 'BUY':
                        self.logger.warning(f"[WHALE TRAP WARNING] Proceeding with BUY despite fake breakout detected ({trap_confidence}% confidence) - TIGHT STOPS REQUIRED")
                    elif trinity_decision == 'SELL':
                        self.logger.warning(f"[WHALE TRAP] ALLOWED SELL - profiting from fake breakout ({trap_confidence}%)")
            ...
            self.logger.warning(f"[DECISION_FINAL] {decision} (from trinity_{trinity_decision}) | Conf={trinity_confidence}% | Consensus={consensus_score*100:.0f}% | All vetos passed ✅")
            
            # ========== CRITICAL FIX: USE TRINITY'S TP/SL (NOT quantum_core recalculation) ==========
            # Trinity/LLM already calculated intelligent TP/SL based on:
            # - ATR volatility
            # - Consensus confidence
            # - ADX trend strength
            # - 4 LLM votes + ML intelligence
            # ═══════════════════════════════════════════════════════════════════════════
            # 🔧 FIX: ALWAYS calculate TP/SL locally - Trinity values are unreliable
            # Trinity sometimes returns garbage values (e.g., 4587 when price is 2688)
            # ═══════════════════════════════════════════════════════════════════════════
            
            # ALWAYS calculate locally - ignore Trinity TP/SL
            tp_price = None
            sl_price = None
            tp_dist = 0
            sl_dist = 0
            ...
            self.logger.info(f"[TP/SL] Calculating INTELLIGENT TP/SL locally for {decision}")
            ...
            if decision == 'BUY':
                if tp_price <= current_price:
                    self.logger.error(f"[BUY] Invalid TP: entry={current_price}, tp={tp_price} (TP must be > entry)")
                    return {'action': 'HOLD', 'reason': 'invalid_buy_tp'}
                if sl_price >= current_price:
                    self.logger.error(f"[BUY] Invalid SL: entry={current_price}, sl={sl_price} (SL must be < entry)")
                    return {'action': 'HOLD', 'reason': 'invalid_buy_sl'}
            elif decision == 'SELL':
                if tp_price >= current_price:
                    self.logger.error(f"[SELL] Invalid TP: entry={current_price}, tp={tp_price} (TP must be < entry)")
                    return {'action': 'HOLD', 'reason': 'invalid_sell_tp'}
                if sl_price <= current_price:
                    self.logger.error(f"[SELL] Invalid SL: entry={current_price}, sl={sl_price} (SL must be > entry)")
            ...
            try:
                ...
                # ═══════════════════════════════════════════════════════════════════════════════════════
                # ❌ REMOVED: Second dashboard update (REDUNDANT)
                # All ML/patterns/orderflow data now calculated and sent in FIRST update (line 7211)
                # This eliminates flicker and ensures dashboard always has complete data
                # ═══════════════════════════════════════════════════════════════════════════════════════
                ...
                nova_dashboard.update(
                    ...
                    volatility_regime=vol_regime,
                    cycle_phase=cycle_phase,
                    market_state=trend_type if trend_type else 'RANGING',
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via jpysound.py server on port 7870 (USDJPY)
        ...
        try:
            ...
            # Send to jpysound.py server on port 7870 (USDJPY) with 4-byte length header
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', Config.SOUND_PORT))  # 7870 for USDJPY
            ...
            self.logger.info(f"[AUDIO] 🔊 Sent {decision} alert to jpysound.py (port {Config.SOUND_PORT})")
            
        except ConnectionRefusedError:
            self.logger.warning(f"[AUDIO] jpysound.py not running on 127.0.0.1:{Config.SOUND_PORT}")
        except socket.timeout:
            self.logger.debug(f"[AUDIO] jpysound.py timeout")
        except Exception as e:
    ...
    def _calculate_attack_probability(self, trinity_llms, trinity_decision, trinity_confidence):
        ...
        try:
            ...
            vote_ratio = total_action_votes / 5.0  # 5 LLMs total
...
class AdvancedIPC:
    ...
    def _play_sound_alert(self, alert_type, symbol='USDJPY'):
        """Play TP/SL audio alert via jpysound.py server on port 7870 (USDJPY)"""
        try:
            ...
            self.logger.warning(f"[AUDIO] 🔊 {alert_type} alert sent to jpysound.py (port {Config.SOUND_PORT})")
        except ConnectionRefusedError:
            self.logger.error(f"[AUDIO] ❌ jpysound.py NOT RUNNING! Start it with: python jpysound.py")
        except socket.timeout:
        ...
        try:
            ...
            if data:
                ...
                if request.get('type') == 'handshake':
                    ...
                    # ⭐ FIX: Spawn dedicated ping sender thread IMMEDIATELY
                    ping_thread = threading.Thread(
                        target=self._send_executor_pings,
                        args=(client, addr),
                        daemon=True
                    )
                    ping_thread.start()
            ...
            while True:
                try:
                    ...
                    if hasattr(e, 'winerror') and e.winerror == 10038:
                        ...
                        self.logger.warning(f"[Executor-RX] Socket error from {addr}: {e}")
                    break
                
                except Exception as e:
                    self.logger.warning(f"[Executor-RX] Error from {addr}: {type(e).__name__}: {e}")
                    break
        
        except Exception as e:
            self.logger.warning(f"[Executor-RX] Thread error for {addr}: {e}")
    ...
    def _handle_genome(self, client, addr):
        ...
        try:
            client.settimeout(120.0)  # ⭐ 120 sec timeout - USDJPY M15 sends genome every 15 min
            
            # Keep connection open and receive multiple genomes
            while True:
                try:
                    # Receive first 4 bytes - could be length header or start of JSON
                    first_bytes = client.recv(4)
                    if len(first_bytes) == 0:
                        # Connection closed gracefully by client
                        self.logger.info(f"[Injector] Connection closed gracefully by {addr}")
                        break
                    if len(first_bytes) < 4:
                        self.logger.warning(f"[Injector] Incomplete header from {addr} - received {len(first_bytes)} bytes")
                        break
                    
                    # Auto-detect protocol: check if first bytes look like JSON
                    if first_bytes[0:1] == b'{':
                        # Raw JSON mode (no length header) - read until newline or timeout
                        self.logger.debug(f"[Injector] Detected raw JSON mode from {addr}")
                        data = first_bytes
                        while True:
                            try:
                                client.settimeout(0.5)  # Short timeout for streaming
                                ...
                                break
                        client.settimeout(120.0)  # Restore timeout
                        # Strip trailing newline if present
                        if data.endswith(b'\n'):
                            data = data[:-1]
                    else:
                        # Length-prefixed mode (4-byte header)
                        msg_len = struct.unpack('<I', first_bytes)[0]  # Little-endian from MQL5
                    ...
                    try:
                        ...
                        self.logger.warning(f"[Injector] 🔍 GENOME RECEIVED: type={genome.get('type')} | size={len(data)} bytes")
                    except (json.JSONDecodeError, UnicodeDecodeError) as e:
                        self.logger.error(f"[Injector] Invalid JSON from {addr}: {e}")
                        break
                    
                    # Respond to handshakes on injector port
                    ...
                    self.logger.warning(f"[Injector] 🔍 VALIDATING genome...")
                    is_valid = self._validate_tick(genome)
                    self.logger.warning(f"[Injector] 🔍 VALIDATION RESULT: {is_valid}")
    ...
    def _send_executor_pings(self, client, addr):
        """⭐ DEDICATED PING SENDER - Sends pings every 5 seconds to keep EA handshake alive"""
        ...
        try:
            while True:
                try:
                    ...
                    with self.quimera_lock:
                        if ('executor', addr) not in self.quimera_connections:
                            self.logger.info(f"[PingSender] Connection {addr} deregistered, stopping pings")
                            break
                    ...
                    self.logger.debug(f"[PingSender] ✓ Ping sent to {addr}")
                    consecutive_errors = 0
                    ...
                    if consecutive_errors >= max_errors:
                        self.logger.warning(f"[PingSender] Too many errors ({consecutive_errors}) for {addr}, stopping")
                        break
                    time.sleep(1)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.logger.warning(f"[QuimeraConn] Error on {addr}: {e}")
                    break
        
        except Exception as e:
            self.logger.warning(f"[QuimeraConn] Handler error for {addr}: {e}")
    ...
    def _validate_tick(self, genome):
        ...
        try:
            ...
            # Accept market_genome or tick_genome (including Multi-TF variants from euquimera_bridge.mq5)
            # ⭐ Multi-TF EA sends: tick_genome_multi_tf, market_genome_multi_tf
            # ⭐ SAMURAI EA (jpyquimera_bridge.mq5) sends: genome, tick, portfolio_sync
            valid_types = [
                'market_genome', 'tick_genome',
                'market_genome_multi_tf', 'tick_genome_multi_tf',
                'genome', 'tick', 'portfolio_sync', 'trade_event'  # ⭐ SAMURAI format
            ]
            if genome_type in valid_types:
                ...
                if genome_type in ['genome', 'tick']:
                    ...
                    if bid is None or ask is None:
                        self.logger.warning(f"[Validator] SAMURAI: Missing bid/ask")
                        return False
                    ...
                    try:
                        ...
                        if bid >= ask:
                            self.logger.warning(f"[Validator] SAMURAI: Inverted bid={bid} >= ask={ask}")
                            return False
                        
                        # USDJPY range: 100-200
                        mid_price = (bid + ask) / 2.0
                        if mid_price < 100.0 or mid_price > 200.0:
                            self.logger.warning(f"[Validator] SAMURAI: Price out of range: {mid_price:.5f}")
                            return False
                        
                        return True
                    except (ValueError, TypeError):
                        self.logger.warning(f"[Validator] SAMURAI: Bad bid/ask values")
                        return False
                ...
                # OLD FORMAT: bid/ask inside tick_data
                ...
                try:
                    ...
                    # 🔧 FIX: Acceptable range for USDJPY (100-200) - Correct for JPY pair!
                    mid_price = (bid + ask) / 2.0
                    if mid_price < 100.0 or mid_price > 200.0:
                        self.logger.warning(f"[Validator] Price out of range: {mid_price:.5f}")
                        return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-74: default=str serializa numpy/Enum/datetime
            ...
            try:
                ...
                if len(resp_header) == 4:
                    ...
                    try:
                        ...
                        is_success = b'ACK' in resp_data or b'FILLED' in resp_data or b'success' in resp_data
                    ...
                    if is_success:
                        ...
                        self.logger.info(f"[Kraken] Order {order_id} ACK'd ({latency:.2f}ms)")
                        sock.close()
                        with self.stats_lock:
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            entry_price = decision.get('entry_price', 0)
            if entry_price <= 0:
                # FIX: Try to recover using current tick data
                tick_data = decision.get('tick_data', {})
                bid = tick_data.get('bid', 0)
                ask = tick_data.get('ask', 0)
                
                if decision.get('action') == 'BUY' and ask > 0:
                    entry_price = ask
                    self.logger.warning(f"[Quimera TX] RECOVERED: entry_price={entry_price} from tick ask")
                elif decision.get('action') == 'SELL' and bid > 0:
                    entry_price = bid  
                    self.logger.warning(f"[Quimera TX] RECOVERED: entry_price={entry_price} from tick bid")
                else:
            ...
            with self.quimera_lock:
                ...
                for key, conn in self.quimera_connections.items():
                    if isinstance(key, tuple) and key[0] == 'executor':
                        executor_conn = conn
                        self.logger.debug(f"[Quimera TX] Found executor connection: {key}")
                        break
            
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - EA not connected to port {Config.EXECUTOR_PORT}")
                self.logger.error(f"[Quimera TX] ❌ Make sure jpyquimera_bridge EA is running in MT5 and connected")
            ...
            # 1. Validate price is within realistic bounds (USDJPY: 0.5-2.0 range)
            # 2. Validate stops are within reasonable distance from entry (5-50 pips)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR USDJPY
            # USDJPY price range: 100-200 (typically around 140-160)
            if not (100.0 <= entry_price <= 200.0):
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price} out of realistic range [100-200] for USDJPY")
                return False
            ...
            # USDJPY ATR: 1 pip = 0.01 (3-digit), not 0.0001 like EURUSD
            atr_val = decision.get('atr', 0.05)  # Default 5 pips for USDJPY (0.05 = 5 pips)
            
            # ⚠️ CRITICAL: Clamp ATR to USDJPY M15 realistic range
            # USDJPY M15 ATR típico: 0.03-0.15 (3-15 pips in 3-digit)
            # Si viene más alto, el mercado está loco o hay error
            atr_val = max(0.02, min(0.20, atr_val))
            self.logger.debug(f"[EINSTEIN] ATR clamped: {atr_val:.3f}")
            
            # ⭐ SCALPING TP/SL: ATR × 1.5 (TP) and ATR × 1.0 (SL) for USDJPY M15
            tp_distance = atr_val * 1.5
            sl_distance = atr_val * 1.0
            
            # ═══════════════════════════════════════════════════════════════════════════
            # MT5 STOPS_LEVEL REQUIREMENT: Broker requires minimum distance from ASK/BID
            # USDJPY uses 3 decimals (155.xxx), 1 pip = 0.01
            # STOPS_LEVEL típico: 10-30 points (1-3 pips)
            # ═══════════════════════════════════════════════════════════════════════════
            PIP_SIZE = 0.01  # USDJPY: 1 pip = 0.01 (3-digit broker uses 0.001)
            MIN_TP_DISTANCE = 0.25  # 25 pips minimum TP (0.25 = 25 pips for USDJPY) (broker STOPS_LEVEL safe)
            MIN_SL_DISTANCE = 0.20  # 20 pips minimum SL (0.20 = 20 pips for USDJPY) (broker STOPS_LEVEL safe)
            
            tp_distance = max(MIN_TP_DISTANCE, tp_distance)
            sl_distance = max(MIN_SL_DISTANCE, sl_distance)
            
            # ⚠️ HARD CAP: Máximo absoluto para scalping M15 USDJPY
            tp_distance = min(tp_distance, 0.25)  # Max 25 pips TP
            sl_distance = min(sl_distance, 0.12)  # Max 12 pips SL
            
            self.logger.info(f"[EINSTEIN] ✅ ATR-Based: TP={tp_distance:.3f} ({tp_distance/PIP_SIZE:.0f}p) SL={sl_distance:.3f} ({sl_distance/PIP_SIZE:.0f}p)")
            
            # Calculate final prices (3 decimals for USDJPY standard)
            if action == 'BUY':
                tp_price = round(entry_price + tp_distance, 3)
                sl_price = round(entry_price - sl_distance, 3)
            else:  # SELL
                tp_price = round(entry_price - tp_distance, 3)
                sl_price = round(entry_price + sl_distance, 3)
            
            tp_pips = tp_distance / PIP_SIZE  # Convert to pips for USDJPY
            sl_pips = sl_distance / PIP_SIZE  # Convert to pips for USDJPY
            ...
            if action == 'BUY':
                if not (tp_price > entry_price > sl_price):
                    self.logger.error(f"[EINSTEIN] ❌ BUY order invalid: TP({tp_price}) > Entry({entry_price}) > SL({sl_price}) FAILED")
                    tp_price = round(entry_price + tp_distance, 3)
                    sl_price = round(entry_price - sl_distance, 3)
            elif action == 'SELL':
                if not (sl_price > entry_price > tp_price):
                    self.logger.error(f"[EINSTEIN] ❌ SELL order invalid: SL({sl_price}) > Entry({entry_price}) > TP({tp_price}) FAILED")
                    tp_price = round(entry_price - tp_distance, 3)
                    sl_price = round(entry_price + sl_distance, 3)
            ...
            tp_pips_final = tp_distance_final / PIP_SIZE  # USDJPY: 1 pip = 0.01
            sl_pips_final = sl_distance_final / PIP_SIZE  # USDJPY: 1 pip = 0.01
            self.logger.info(f"[EINSTEIN] ✅ VALIDATED: {action} Entry={entry_price:.3f} TP={tp_price:.3f}({tp_pips_final:.0f}p) SL={sl_price:.3f}({sl_pips_final:.0f}p)")
            
            # 🔧 FIX: EA expects 'volume' field, not 'lot'
            lot_size = decision.get('lot', 0.01)
            
            response = {
                'type': 'trade_order',
                'action': action,
                'volume': lot_size,  # EA uses 'volume' field
                'lot': lot_size,     # Keep 'lot' for backwards compatibility
                'sl': sl_price,
                'tp': tp_price,
                'entry_price': entry_price,  # For EA reference
                'timeframe': decision.get('timeframe', 'M15'),  # EA needs this for cooldown
                'comment': f"NOVA_SAMURAI_{decision.get('timeframe', 'M15')}"
            }
...
class QuantumCore:
    ...
    def __init__(self):
        ...
        self.engine.core = self  # FIX: Inject reference so engine can access _transform_quimera_to_trinity
        self.running = True
        ...
        def _on_trade_closed_performance(entry, exit, result_pips, symbol, reason):
            ...
            try:
                _reward = max(float(np.tanh(result_pips / 10.0)), -0.05)  # floor: avoid runaway negative Q-values
        ...
        # FIX M-JPY-01: singleton NOVAMarketStateDetector — instantiate once, reuse every call
    ...
    def _monitor_heartbeat(self):
        ...
        connection_states = {'trinity': None, 'kraken': None}
        ...
        while self.running:
            try:
                ...
                status_msg = f"[Network] Trinity {trinity_emoji} | Kraken {kraken_emoji} | Uptime:{uptime_secs}s"
                ...
                # Positive status if both alive
                if trinity_alive and kraken_alive:
                    status_msg += " | ✓ Both OPERATIONAL"
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            # ⭐ Accept both standard and Multi-TF genome types + SAMURAI format
            valid_types = [
                'market_genome', 'tick_genome',
                'market_genome_multi_tf', 'tick_genome_multi_tf',
                'genome', 'tick'  # ⭐ SAMURAI format from jpyquimera_bridge.mq5
            ]
            ...
            # ⭐ SAMURAI FORMAT: data at root level, not in metadata/tick_data
            metadata = genome.get('metadata', {})
            tick_data = genome.get('tick_data', {})
            bar_data = genome.get('bar_data', {})
            
            # ⭐ SAMURAI: symbol at root level
            symbol = genome.get('symbol') or metadata.get('symbol', 'USDJPY')
            
            # ⭐ SAMURAI: bid/ask at root level
            ...
            # CRITICAL: Validate price is in reasonable range for USDJPY
            # USDJPY trades around 140-160 (JPY pairs have different price ranges!)
            if 'JPY' in symbol and close_price > 0:
                if close_price < 100.0 or close_price > 200.0:
                    # Only reject EXTREME outliers for JPY pairs
                    ...
                    return None
                # USDJPY normal range is 140-160, log unusual but accept
                if close_price < 130 or close_price > 170:
                    self.logger.warning(f"[Transform] ⚠️ Price {close_price:.5f} is unusual but accepting (USDJPY may be in volatile market)")
            
            # CRITICAL: Validate OHLC consistency if present
            # ⭐ SAMURAI FORMAT: OHLC in timeframes[0] (first/primary timeframe)
            timeframes = genome.get('timeframes', [])
            if timeframes and len(timeframes) > 0:
                # Use first timeframe (M5 or M1) for OHLC
                primary_tf = timeframes[0]
                high = float(primary_tf.get('high', close_price))
                low = float(primary_tf.get('low', close_price))
                open_price = float(primary_tf.get('open', close_price))
                close_price = float(primary_tf.get('close', close_price))
                volume = float(primary_tf.get('volume', 0))
                
                # Extract indicators from SAMURAI format
                bar_data = {
                    'current': {
                        'high': high,
                        'low': low,
                    ...
                    'history': []  # SAMURAI doesn't send history, we'll build from buffer
                }
                
                # Also extract pre-calculated indicators from SAMURAI
                # These are more accurate than recalculating from price buffer
                genome['_samurai_indicators'] = {
                    'rsi': primary_tf.get('rsi', 50),
                    'adx': primary_tf.get('adx', 20),
                    'atr': primary_tf.get('atr', 0),
                    'atr_pips': primary_tf.get('atr_pips', 0),
                    'ma_fast': primary_tf.get('ma_fast', close_price),
                    'ma_slow': primary_tf.get('ma_slow', close_price),
                    'boll_upper': primary_tf.get('boll_upper', close_price),
                    'boll_middle': primary_tf.get('boll_middle', close_price),
                    'boll_lower': primary_tf.get('boll_lower', close_price),
                    'stoch_main': primary_tf.get('stoch_main', 50),
                    'stoch_signal': primary_tf.get('stoch_signal', 50),
                    'trend': primary_tf.get('trend', 0)
                }
                self.logger.info(f"[Transform] SAMURAI indicators: RSI={genome['_samurai_indicators']['rsi']:.1f} ADX={genome['_samurai_indicators']['adx']:.1f} ATR={genome['_samurai_indicators']['atr']:.4f}")
            ...
            # ⭐ SAMURAI: Use pre-calculated indicators if available (more accurate!)
            samurai_ind = genome.get('_samurai_indicators', {})
            if samurai_ind:
                rsi_calculated = samurai_ind.get('rsi', 50.0)
                atr_calculated = samurai_ind.get('atr', 0.0)
                adx_calculated = samurai_ind.get('adx', 25.0)
                self.logger.info(f"[Transform] Using SAMURAI indicators: RSI={rsi_calculated:.1f} ADX={adx_calculated:.1f} ATR={atr_calculated:.4f}")
            else:
                # Calculate real indicators from price history
            ...
            # ⭐ SAMURAI: Also use pre-calculated MAs and Bollinger if available
            samurai_ind = genome.get('_samurai_indicators', {})
            ...
            # ⭐ SAMURAI: Use pre-calculated MAs and Bollinger if available
            if samurai_ind:
                ma_fast_calc = samurai_ind.get('ma_fast', close_price)
                ma_slow_calc = samurai_ind.get('ma_slow', close_price)
                bb_upper_calc = samurai_ind.get('boll_upper', close_price * 1.005)
                bb_middle_calc = samurai_ind.get('boll_middle', close_price)
                bb_lower_calc = samurai_ind.get('boll_lower', close_price * 0.995)
            else:
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            # ─── GUARD A-JPY-02: Consecutive-loss halt ───────────────────────────────
            ...
            # ═══════════════════════════════════════════════════════════════════════════
            # 🛡️ FROZEN MARKET DETECTION: Warn but DON'T BLOCK during demo/weekends
            # 🔧 2025-02: Relajado - solo warning, permite trading en demo/fin de semana
            # Detects simulation mode or market closure
            # ═══════════════════════════════════════════════════════════════════════════
            # 🔥 FIX: Read bid/ask from tick_data (NOT from root level)
            tick_data = genome.get('tick_data', {})
            bid = tick_data.get('bid', 0) if tick_data else genome.get('bid', 0)
            ask = tick_data.get('ask', 0) if tick_data else genome.get('ask', 0)
            atr = genome.get('indicators', {}).get('atr', 0)
            
            # Check if market is frozen (bid == ask and ATR near 0) - WARNING ONLY, NO BLOCK
            if bid == ask and atr < 0.0001:
                if not hasattr(self, '_frozen_market_warning'):
                    self.logger.warning(f"⚠️ [FROZEN MARKET] WARNING: Bid={bid}, Ask={ask}, ATR={atr} | Market may be CLOSED - continuing anyway")
                    self._frozen_market_warning = True
                # 🔧 RELAJADO: No bloquear, solo advertir - permite trading en demo/weekends
            
            # ═══════════════════════════════════════════════════════════════════════════
            # 🛡️ MASTER SAFETY CHECK: Require genome reception before trading
            # 🔧 2025-02: RELAJADO - habilita trading al recibir genomas, no requiere Trinity
            # This allows testing during weekends when Trinity is not running
            # ═══════════════════════════════════════════════════════════════════════════
            if not hasattr(self, '_trading_enabled') or not self._trading_enabled:
                # Check if we have received genomas (indicates MT5 is running)
                trinity_responses = self.ipc.stats.get('trinity_responses', 0)
                kraken_acks = self.ipc.stats.get('kraken_acks', 0)
                genome_count = getattr(self, 'genome_counter', 0)
                
                # 🔧 RELAJADO: Habilitar si hay genomas O respuestas de Trinity
                if genome_count >= 5 or trinity_responses > 0 or kraken_acks > 0:
                    # System is ready - received enough genomas or Trinity is responding
                    self._trading_enabled = True
                    self.logger.info(f"[MASTER SAFETY] ✅ Trading ENABLED - Genomes={genome_count} Trinity={trinity_responses} Kraken={kraken_acks}")
                else:
                    # Still warming up - log warning and continue without trading
                    if not hasattr(self, '_safety_warning_logged'):
                        self.logger.warning(f"[MASTER SAFETY] ⚠️ Warming up - waiting for 5 genomes or Trinity response")
                        self._safety_warning_logged = True
            
            # ⭐ SAMURAI FORMAT FIX: symbol at root level, not in metadata
            symbol = genome.get('symbol') or genome.get('metadata', {}).get('symbol', 'USDJPY')
            genome_type = genome.get('type', '')
            
            # Handle Quimera format (market_genome or tick_genome) + Multi-TF variants
            # ⭐ Multi-TF EA sends: tick_genome_multi_tf, market_genome_multi_tf
            # ⭐ SAMURAI EA (jpyquimera_bridge.mq5) sends: genome, tick
            valid_genome_types = [
                'market_genome', 'tick_genome',
                'market_genome_multi_tf', 'tick_genome_multi_tf',
                'genome', 'tick'  # ⭐ SAMURAI format from jpyquimera_bridge.mq5
            ...
            if genome_type in valid_genome_types:
                ...
                # ⭐ SAMURAI FORMAT: bid/ask at root level OR in tick_data
                tick_data = genome.get('tick_data', {})
                bid = genome.get('bid') or tick_data.get('bid', 0)
                ask = genome.get('ask') or tick_data.get('ask', 0)
                if bid > 0 and ask > 0:
                    mid_price = (bid + ask) / 2.0
                ...
                # DEBUG: Log indicators being sent to Trinity
            ...
            # FIX M-JPY-01: use singleton instantiated in __init__
            ...
            # Trinity's NOVA checks genome['bid'] and genome['ask'], not tick_data
            tick_data = genome.get('tick_data', {})
            if tick_data and 'bid' in tick_data and 'ask' in tick_data:
                genome['bid'] = tick_data['bid']
                genome['ask'] = tick_data['ask']
                logger.debug(f"[Trinity-Prep] Copied bid={genome['bid']:.5f} ask={genome['ask']:.5f} to root level")
            ...
            if trinity_resp and not trinity_resp.get('_is_fallback', False):
                ...
                # ⭐ CRITICAL FIX: Ensure trinity_resp ALWAYS has llm_responses filled
                # If jpytrinity.py returned response without llm_responses, fill it now
                if 'llm_responses' not in trinity_resp or not trinity_resp.get('llm_responses'):
                    trinity_resp['llm_responses'] = {
                        'BAYESIAN': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0)},
                        'TECHNICAL': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0)},
                        'CHART': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0)},
                        'RISK': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0)},
                        'SUPREME': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0)},
                        'OCULUS': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0), 'quality_score': trinity_resp.get('quality_score', 0)},
                        'CHRONOS': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0), 'timing_multiplier': trinity_resp.get('timing_multiplier', 1.0)},
                        'PREDATOR': {'decision': trinity_resp.get('decision', 'HOLD'), 'confidence': trinity_resp.get('confidence', 0), 'rr_ratio': trinity_resp.get('rr_ratio', 0.0)}
                    }
                    self.logger.warning(f"[Process] ⚠️ jpytrinity response missing llm_responses - filled automatically")
            ...
            # ─── GUARD C-JPY-01: ATR=0 circuit breaker ───────────────────────────────
            if atr_val == 0.0:
                with self.ipc.stats_lock:
                ...
                if _atr_zero_count_val >= 3:
                    self.logger.debug(f"[GUARD] 🛑 ATR=0 x{_atr_zero_count_val} consecutive — corrupted genome, skipping")
                    return None
            else:
                with self.ipc.stats_lock:
            ...
            try:
                ...
                if hasattr(self.engine, 'dnn') and len(list(self.price_buffer)) >= 10:
                    # Get quick ML prediction from indicators
                    feature_vector = [rsi_val, adx_val, macd_val, atr_val]
                    try:
                        ml_pred = self.engine.dnn.predict(feature_vector)
                        ml_pred_action = ml_pred.get('direction', 'NEUTRAL')
                        ml_pred_confidence = ml_pred.get('confidence', 0) / 100.0
                        dnn_signal = ml_pred.get('confidence', 0) / 50.0 - 1.0  # -1.0 to +1.0
                    except Exception as e:
                        self.logger.debug(f"[ML] DNN prediction error: {e}")
                        ml_pred_action = 'NEUTRAL'
                
                # Get LSTM momentum if available
                if hasattr(self.engine, 'lstm'):
                    try:
                        lstm_pred = self.engine.lstm.get_next_candle_prediction()
                        lstm_momentum = lstm_pred.get('momentum', 0)
                    except Exception as e:
                        self.logger.debug(f"[ML] LSTM error: {e}")
                        lstm_momentum = 0.0
                ...
                if hasattr(self.engine, 'bayesian'):
                    try:
                        # Quick Bayesian update based on RSI
                        if rsi_val > 60:
                            bull_prob = 70
                            bear_prob = 30
                        elif rsi_val < 40:
            ...
            # SEND TO QUIMERA FIRST (immediately, before any local validation)
            # Quimera will do its own position validation with real-time MT5 data
            if decision:
                self.ipc.send_decision_to_quimera(decision, tick_id)
            else:
                # Send empty HOLD if no decision
                self.ipc.send_decision_to_quimera({'action': 'HOLD', 'confidence': 0, 'entry_price': 0, 'tp_distance': 0, 'sl_distance': 0, 'lot': 0}, tick_id)
            ...
            _did_increment = False  # FIX C-JPY-02: track if THIS call incremented open_positions
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-JPY-05: direction_poison SL-protection check (stacking)
                            ...
                            if _remaining_poison > 0:
                                ...
                                logger.warning(f"[POISON] 🛑 {decision['action']} poisoned for {_remaining_poison}s - HOLD")
                            else:
                                ...
                                _did_increment = True  # FIX C-JPY-02
                        ...
                        # FIX A-JPY-05: direction_poison SL-protection check (first trade)
                        _poison_expiry = self.ipc.stats.get(f'dir_poison_until_{decision["action"]}', 0)
                        _remaining_poison = int(_poison_expiry - time.time())
                        if _remaining_poison > 0:
                            decision_for_kraken = decision.copy()
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    ...
                    # FIX A-JPY-01: atomic timestamp update under stats_lock
                    _now = time.time()
                    with self.ipc.stats_lock:
                        self.last_trade_time = _now
                        self.engine.last_trade_time = _now
                    self.engine.cooldown_passed_logged = False  # Reset flag for next cooldown cycle
                    ...
                    try:
                        ...
                        self.logger.info(f"[ML] ✓ Trade registered for learning: {trade_id} | {decision_for_kraken['action']} @ {decision_for_kraken['entry_price']:.5f}")
                    ...
                    # FIX C-JPY-02: Rollback ONLY if this call was the one that incremented
                    if _did_increment:
                        with self.ipc.stats_lock:
    ...
    def run(self):
        ...
        last_position_reconcile = time.time()  # FIX A-JPY-04
        start_time = time.time()
        
        # Start live dashboard - ULTRA FAST refresh for real-time tick tracking
        ...
        try:
            while self.running:
                ...
                # 🔧 FIX CAT-78: last_trinity_response==0 (uninitialized) → time_since=Unix epoch (~1.77Bs) → false alarm
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)
                ...
                for genome in genomes:
                    # ⭐ INCREMENT GENOME COUNTER when genome ARRIVES (not when processed with consciousness)
                    # This ensures dashboard shows "Genomes X/650" even if Trinity hasn't responded yet
                    ...
                    if SHARED_BRAIN_AVAILABLE and self.engine.genome_counter % 30 == 0:
                        try:
                    ...
                    if self.engine.genome_counter % 500 == 0:
                        try:
                            self.engine.rl_engine.save_checkpoint()
                            logger.info(f"[RL] Periodic Q-table save at genome {self.engine.genome_counter}")
                        except Exception as _rl_save_e:
                            logger.warning(f"[RL] Periodic save error: {_rl_save_e}")
                ...
                # Update dashboard stats every 100ms for real-time responsiveness
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        os.replace(_tmp_path, 'quantum_stats.json')  # FIX M-JPY-02: atomic write
                        
                        last_stats_write = time.time()
                    except Exception as e:
                        logger.debug(f"Error writing stats: {e}")
                
                # FIX A-JPY-04: Position reconcile every 60 s (stale position guard)
    ...
    def _update_dashboard_from_genome(self, genome):
        ...
        try:
            ...
            # ⭐ SAMURAI FORMAT: bid/ask at root level, NOT in tick_data
            tick_data = genome.get('tick_data', {})
            bid = float(genome.get('bid') or tick_data.get('bid', 0.0))
            ask = float(genome.get('ask') or tick_data.get('ask', 0.0))
            
            # ⭐ SAMURAI FORMAT: Get price from timeframes[0] if available
            timeframes = genome.get('timeframes', [])
            primary_tf = timeframes[0] if timeframes else {}
            
            # CRITICAL: Use REAL last price from Quimera, NOT mid-point
            last_price = float(tick_data.get('last', 0.0)) or float(primary_tf.get('close', 0.0))
            ...
            # ⭐ SAMURAI: spread at root level (in points)
            spread = float(genome.get('spread') or tick_data.get('spread', 0.0))
            if spread == 0 and bid > 0 and ask > 0:
                spread = (ask - bid) * 100  # Convert to pips for JPY pairs
            ...
            # ⭐ SAMURAI FORMAT: Use _samurai_indicators if available (pre-calculated by EA)
            samurai_ind = genome.get('_samurai_indicators', {})
            
            # ═══ EXTRACT INDICATORS - Use SAMURAI first, then indicators, then primary_tf ═══
            rsi_val = float(samurai_ind.get('rsi') or indicators.get('rsi') or primary_tf.get('rsi', 50.0))
            atr_val = float(samurai_ind.get('atr') or indicators.get('atr') or primary_tf.get('atr', 0.0))
            adx_val = float(samurai_ind.get('adx') or indicators.get('adx') or primary_tf.get('adx', 0.0))
            ma_fast = float(samurai_ind.get('ma_fast') or indicators.get('ma_fast') or primary_tf.get('ma_fast', 0.0))
            ma_slow = float(samurai_ind.get('ma_slow') or indicators.get('ma_slow') or primary_tf.get('ma_slow', 0.0))
            bb_upper = float(samurai_ind.get('boll_upper') or indicators.get('bollinger_upper') or primary_tf.get('boll_upper', 0.0))
            bb_middle = float(samurai_ind.get('boll_middle') or indicators.get('bollinger_middle') or primary_tf.get('boll_middle', 0.0))
            bb_lower = float(samurai_ind.get('boll_lower') or indicators.get('bollinger_lower') or primary_tf.get('boll_lower', 0.0))
            macd_val = float(indicators.get('macd', 0.0))
            macd_signal_val = float(indicators.get('macd_signal', 0.0))
            macd_hist_val = float(indicators.get('macd_histogram', 0.0))
            
            # ═══ STOCHASTIC - SAMURAI format ═══
            stoch_k_val = float(samurai_ind.get('stoch_main') or indicators.get('stoch_k') or primary_tf.get('stoch_main', 50.0))
            stoch_d_val = float(samurai_ind.get('stoch_signal') or indicators.get('stoch_d') or primary_tf.get('stoch_signal', 50.0))
            ...
            # ═══ TREND DETERMINATION ═══
            # ⭐ SAMURAI: Use trend from primary_tf first (integer: 1=bull, -1=bear, 0=neutral)
            samurai_trend = primary_tf.get('trend', 0)
            prior_analysis = genome.get('prior_analysis', {})
            llm1_regime = prior_analysis.get('llm1_regime', None)
            
            if samurai_trend == 1:
                trend = 'BULLISH'
            elif samurai_trend == -1:
                trend = 'BEARISH'
            elif llm1_regime and llm1_regime in ['TRENDING', 'RANGING', 'VOLATILE']:
            ...
            # Session from Quimera OR SAMURAI OR detect locally
            # ⭐ SAMURAI FORMAT: session at root level
            session = genome.get('session') or quimera.get('session', None)
...
# 
# QA AUDIT BLOCK  jpyquantum_core.py (USDJPY)
# Audited & fixed: 2026-02-27
#
# ID          SEV   Issue                                                  Status
#    
# C-JPY-01    CRIT  ATR=0 circuit breaker (_atr_zero_count under lock)    FIXED
# C-JPY-02    CRIT  _did_increment flag + guarded rollback                 FIXED
# A-JPY-01    HIGH  lastrade_time atomic under stats_lock                  FIXED
# A-JPY-02    HIGH  Consecutive-loss halt guard (MAX_CONSECUTIVE_LOSSES)   FIXED
# A-JPY-03    HIGH  Daily loss cap guard (MAX_DAILY_LOSS_PERCENT)          FIXED
# A-JPY-04    HIGH  Position reconcile timer in run() (stale pos reset)    FIXED
# A-JPY-05    HIGH  Direction-poison SL-protection check                   FIXED
# M-JPY-01    MED   NOVAMarketStateDetector singleton in __init__          FIXED
# M-JPY-02    MED   Atomic quantum_stats.json write (os.replace)           FIXED
#
# SCORE: 9/9 fixes applied  SYNTAX OK (py_compile)
# 
