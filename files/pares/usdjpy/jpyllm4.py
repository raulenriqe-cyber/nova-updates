...
║  Port: 7859 (TCP Server) - USDJPY M5                                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import sys, os
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | LLM4_RISK | %(message)s",
    datefmt="%H:%M:%S"
)
...
class DynamicRiskCalculator:
    ...
    # ═══════════════════════════════════════════════════════════════
    # USDJPY M5 - Parámetros para movimientos rápidos direccionales
    # ═══════════════════════════════════════════════════════════════
    USDJPY_PIP_VALUE = 0.01  # 1 pip = 0.0001 para USDJPY
    
    # Límites de SL en pips (M5 = SL más ajustados que M15)
    SL_MIN_PIPS = 25.0   # SL mínimo: 25 pips (broker STOPS_LEVEL safe)
    SL_MAX_PIPS = 20.0  # SL máximo: 12 pips
    ...
    # Multiplicadores ATR por régimen (ajustados para USDJPY M5)
    REGIME_MULTIPLIERS = {
    ...
    def calculate_dynamic_targets(self, atr, current_price, regime='UNKNOWN', direction='BUY'):
        """Calcula TP y SL dinámicos OPTIMIZADOS para USDJPY M5"""
        
        # Fallback conservador si no hay ATR válido
        if not atr or atr <= 0:
            sl_pips = 7.0   # Default SL: 7 pips
            tp_pips = 12.0  # Default TP: 12 pips (R:R 1.7:1)
            sl_distance = sl_pips * self.USDJPY_PIP_VALUE
            tp_distance = tp_pips * self.USDJPY_PIP_VALUE
        else:
            ...
            sl_pips = sl_distance / self.USDJPY_PIP_VALUE
            ...
            tp_pips = tp_distance / self.USDJPY_PIP_VALUE
...
def run_tcp_server():
    ...
    # USDJPY M15 - Port +1000 offset
    try:
        with open('config.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm4_port', 7859)
    except:
        port = 7859  # USDJPY LLM4 RISK port (FIXED: was 7858)
