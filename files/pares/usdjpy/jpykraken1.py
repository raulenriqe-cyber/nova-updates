...
NOVA Trading AI By Polarice Labs © 2026 - USDJPY M5 Edition
...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 7852 USDJPY)
  - Audio announcements via jpysound.py
"""
import sys, os
...
# ═══════════════════════════════════════════════════════════════════════════
try:
    from jpy_config_loader import get_kraken_port
...
# ⭐ AUDIO ANNOUNCEMENTS - Connection to jpysound.py (USDJPY)
try:
    from jpysound import announce
    AUDIO_AVAILABLE = True
except ImportError:
    AUDIO_AVAILABLE = False
    def announce(action, symbol=""): pass  # Dummy if jpysound.py not available
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='config.yaml'):
        ...
        # ⭐ Load configuration - Try jpyconfig.yaml first (centralized), fallback to config.yaml
        self.port = 7852  # Default USDJPY
        self.symbols = ['USDJPY']  # USDJPY M15 default
        
        # Try jpyconfig.yaml (centralized USDJPY config)
        try:
            if EU_CONFIG_LOADED:
                self.port = get_kraken_port()
                log.info(f"✅ Loaded Kraken port {self.port} from jpyconfig.yaml")
            else:
                raise ImportError("jpy_config_loader not available")
        except Exception as e:
            log.warning(f"jpyconfig.yaml load failed ({e}), trying config.yaml")
            # Fallback: Try legacy config.yaml
            try:
                import yaml
                with open(config_path, 'r') as f:
                    config = yaml.safe_load(f) or {}
                # USDJPY KRAKEN: Port 7852 (not 5552 which is XAUUSD)
                self.port = config.get('kraken_executor_port', 7852)
                self.symbols = config.get('core_engine', {}).get('symbols', ['USDJPY'])  # USDJPY M15
                log.info(f"✅ Loaded Kraken port {self.port} from config.yaml")
            except Exception as e2:
                log.warning(f"Config load failed: {e2}, using hardcoded defaults")
                self.port = 7852  # USDJPY default port
                self.symbols = ['USDJPY']  # USDJPY M15 default
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        """
        Validate TP/SL prices - USDJPY M15 INTELLIGENT VALIDATION
        
        Returns: (is_valid, tp_corrected, sl_corrected, warning_msg)
        
        USDJPY M15 Parameters:
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # 🎯 USDJPY M15 DEFAULT VALUES (realistic for slower market)
        # ═══════════════════════════════════════════════════════════════════════════
        DEFAULT_TP_PIPS = 15  # 15 pips default TP (achievable in 1-2 M15 candles)
        DEFAULT_SL_PIPS = 10  # 10 pips default SL (tight but safe)
        PIP_VALUE = 0.01    # USDJPY pip value
        
        # Check for zero/missing values
        if tp == 0:
            log.warning(f"[VALIDATION] TP is 0, setting to entry + {DEFAULT_TP_PIPS} pips (USDJPY M15)")
            if action == "BUY":
        ...
        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - {DEFAULT_SL_PIPS} pips (USDJPY M15)")
            if action == "BUY":
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'USDJPY')
            ...
            # Validate symbol
            valid_symbols = {'XAUUSD', 'GOLD', 'XAU', 'USDJPY', 'EUR', 'GBPUSD', 'GBP',
            ...
            # ═══════════════════════════════════════════════════════════════════════════
            # 🎯 USDJPY M15 INTELLIGENT SCALING
            # ═══════════════════════════════════════════════════════════════════════════
            # USDJPY moves slower than gold, so we need different scaling:
