...
║  Port: 7857 (TCP Server) - USDJPY M15                                        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import sys, os
...
class LLM2TechnicalExpert:
    ...
    def __init__(self):
        ...
        self.memory = {'USDJPY': deque(maxlen=300)}
...
def start_server():
    """Inicia servidor TCP en puerto 7856 - USDJPY M15"""
    try:
        config_file = 'jpyconfig.yaml'
        try:
        ...
        port = config.get('llm_settings', {}).get('llm2_port', 7857)
        if not port or port < 1024:
            port = 7857  # FIXED: Was 7856 (EXECUTOR PORT!), now 7857

    except Exception as e:
        log.warning(f"Could not read config: {e}")
        port = 7857  # FIXED: Was 7856
