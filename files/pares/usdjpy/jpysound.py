...
╠══════════════════════════════════════════════════════════════════════════════╣
║  Premium Male Voice: GuyNeural (Ultra-Natural Deep Voice)                    ║
...
# ═══════════════════════════════════════════════════════════════════════════
try:
    from jpy_config_loader import get_sound_port
...
# ============ CONFIGURATION USDJPY M15 ============
# 🔧 FIXED: Was 7860 - MUST match jpyquantum_core.py Config.SOUND_PORT
if EU_CONFIG_LOADED:
    try:
        PORT = get_sound_port()
    except:
    ...
    PORT = 7870  # Default USDJPY sound port
...
# Voice - Premium Male Voice (GuyNeural - Deep, Professional, Commanding)
...
# ============ PLAY ALERT ============
def play_alert(action, symbol='USDJPY'):
    """Play audio alert"""
    action = action.upper()
...
def handle_client(sock, addr):
    try:
        ...
        symbol = data.get('symbol', 'USDJPY').upper()
