...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 🎯 USDJPY CONFIG LOADER - CENTRALIZED CONFIGURATION          ║
║                                                                              ║
║  Loads USDJPY M15 ports and parameters from jpyconfig.yaml                   ║
...
class EUConfig:
    """Centralized configuration manager for USDJPY M15 system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from jpyconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"jpyconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('ports.trinity.port') -> 7866
        """
        config = cls.load()
    ...
    def get_port(cls, service: str) -> int:
        ...
        if service.lower() == 'trinity':
            ...
            return cls.get('ports.shared.ollama.port', 11434)  # Default ollama port
            ...
            if llm_num.isdigit():
                ...
                port = cls.get(f'llms.models.JPY_LLM{llm_num}.port')
                if port is not None:
                    return port
...
def get_symbol() -> str:
    """Get market symbol"""
    return EUConfig.get('market.symbol', 'USDJPY')
...
if __name__ == '__main__':
    print("═" * 80)
    print("🎯 USDJPY CONFIG LOADER - TEST")
    print("═" * 80)
