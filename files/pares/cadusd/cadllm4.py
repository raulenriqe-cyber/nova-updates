...
║  Port: 9159 (TCP Server) - Dynamic Risk Management with AI Learning          ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM4RiskManager:
    ...
    def analyze(self, data):
        ...
        try:
            ...
            # PHASE 10: RR ratio validation - relaxed for CAD
            # - Si confianza > 75%: permite RR > 1.0 (was 1.2)
            # - Si confianza 60-75%: requiere RR > 1.2 (was 1.5)
            # - Si confianza < 60%: requiere RR > 1.4 (was 1.8)
            if confidence >= 75:
                min_rr = 1.0  # PHASE 10: was 1.2
            elif confidence >= 60:
                min_rr = 1.2  # PHASE 10: was 1.5
            else:
                min_rr = 1.4  # PHASE 10: was 1.8
            
            if rr_ratio < min_rr:
                decision = 'WAIT'
                reason_parts.append(f"RR_LOW:{rr_ratio:.2f}<{min_rr:.1f}(conf:{confidence:.0f}%)")
            
            # PHASE 10: Confianza mínima - relaxed for CAD
            if confidence < 25:  # PHASE 10: was 35%
                decision = 'WAIT'
...
def run_tcp_server():
    ...
    try:
        ...
        port = config.get('llm_settings', {}).get('llm4_port', 9159)
    except:
        port = 9159  # LLM4 is RISK on port 9159
