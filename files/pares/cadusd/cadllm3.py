...
║  Port: 9158 (TCP Server) - MATHEMATICAL PRECISION ENHANCED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'USDCAD').upper().strip()
            ...
            # Detecta estructura de mercado con precisión matemática
            # ════════════════════════════════════════════════════════════════
            if len(closes) >= 10:
                # Get recent swing points (últimas 10 velas)
                recent_highs = highs[-10:]
                recent_lows = lows[-10:]
                recent_closes = closes[-10:]
                
                # Check for Higher Highs (HH) and Higher Lows (HL) - BULLISH structure
                mid_high = max(recent_highs[3:7])  # Middle swing high
                latest_high = max(recent_highs[-3:])  # Latest swing high
                
                mid_low = min(recent_lows[3:7])  # Middle swing low
                latest_low = min(recent_lows[-3:])  # Latest swing low
            ...
            # Initialize pattern with default value (prevents UnboundLocalError)
            pattern = 'NEUTRAL'
            
            # 🧠 CRITICAL VALIDATION: Context score must support direction
            # PHASE 10: Relaxed thresholds for CAD to allow more directional votes
            # BUY requires: context_score >= +1.2 (was +2.0)
            # SELL requires: context_score <= -1.2 (was -2.0)
            # Weak directional: ±0.3 to ±1.2
            # HOLD if: -0.3 < context_score < +0.3 (truly neutral)
            
            if context_score >= 1.2:  # PHASE 10: was 2.0
                # Strong bullish context - allow BUY if RSI not extreme
                if rsi < 78 and adx > 15:  # PHASE 10: RSI 75→78, ADX 22→15
                    decision = 'BUY'
            ...
            elif context_score <= -1.2:  # PHASE 10: was -2.0
                # Strong bearish context - allow SELL if RSI not extreme
                if rsi > 22 and adx > 15:  # PHASE 10: RSI 25→22, ADX 22→15
                    decision = 'SELL'
            ...
            # RSI > 60 + momentum > 0.0005 + context > 0.8 + ADX > 25 = BUY (momentum override)
            elif rsi > 60 and price_mom_5 > 0.0005 and context_score >= 0.8 and adx > 25:
                decision = 'BUY'
                confidence = min(88.0, 60.0 + (rsi - 50) * 0.4)
                pattern = 'MOMENTUM_UP'
                ...
                log.info(f"[LLM3] MOMENTUM UP (RSI={rsi:.1f}, mom={price_mom_5:.6f}, ctx={context_score:.1f}, ADX={adx:.0f}) -> BUY")
            elif context_score >= 0.8:  # PHASE 10: was 1.5 - moderate bullish
                decision = 'BUY'
            ...
            elif context_score <= -0.8:  # PHASE 10: was -1.5 - moderate bearish
                decision = 'SELL'
            ...
            elif context_score >= 0.3:  # PHASE 10: weak bullish - still vote directionally
                decision = 'BUY'
                confidence = min(55.0, 35.0 + context_score * 15.0)
                pattern = 'WEAK_BULLISH'
                reason = " | ".join(reason_parts) if reason_parts else f"Weak bullish context (score={context_score:.1f})"
                log.info(f"[LLM3] 🟡 WEAK BULLISH (ctx={context_score:.1f}) → BUY at {confidence:.0f}%")
            elif context_score <= -0.3:  # PHASE 10: weak bearish - still vote directionally
                decision = 'SELL'
                confidence = min(55.0, 35.0 + abs(context_score) * 15.0)
                pattern = 'WEAK_BEARISH'
                reason = " | ".join(reason_parts) if reason_parts else f"Weak bearish context (score={context_score:.1f})"
                log.info(f"[LLM3] 🟡 WEAK BEARISH (ctx={context_score:.1f}) → SELL at {confidence:.0f}%")
            else:
                decision = 'HOLD'
                # Only truly neutral context_score (-0.3 to +0.3) gets HOLD
                hold_confidence = 30.0 + abs(context_score) * 23.0
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port=9158):  # FIXED: LLM3 is 9158
        self.host = host
        self.port = port
...
def main():
    ...
    try:
        # Cargar config
        port = 9158  # LLM3 is on 9158
        try:
            with open('cadconfig.yaml', 'r') as f:
                config = yaml.safe_load(f) or {}
            port = config.get('llm_settings', {}).get('llm3_port', 9158)
        except:
            pass
