...
class TrinityConsensus:
    def __init__(self):
        log.info('[TRINITY] BANK-GRADE CONSENSUS v3.0 Initialized - USDCAD')
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD) - USDCAD PORTS (75xx)
        self.core_llm_ports = {
            'BAYESIAN': 9155,     # USDCAD LLM1
            'TECHNICAL': 9157,    # USDCAD LLM2
            'CHART': 9158,        # USDCAD LLM3
            'RISK': 9159,         # USDCAD LLM4
            'SUPREME': 9161,      # USDCAD LLM5
            'SMART_MONEY': 9162   # LLM6 - Smart Money Oracle (USDCAD)
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution) - USDCAD PORTS
        self.enhancement_llm_ports = {
            'OCULUS': 9163,    # USDCAD LLM7 - Data Quality Validator
            'CHRONOS': 9164,   # USDCAD LLM8 - Timing Optimizer  
            'PREDATOR': 9165,  # USDCAD LLM9 - Execution Engine
            'NOVA': 9160,   # USDCAD LLM10 - Guardian Protector
            'STRATEGIST': 9167,   # CADUSD LLM11 - Strategist Guru
            'SENTINEL': 9168      # CADUSD LLM12 - Quality Guardian
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 9163, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 9164, genome)
        if chronos_response:
        ...
        predator_response = self._query_enhancement_llm('PREDATOR', 9165, enhanced_genome)
        rr_ratio = 0.0  # 🔧 FIX CAT-134: inicializar antes del condicional para evitar UnboundLocalError
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] ?? Make sure all 6 LLMs are running on ports 9155, 9157, 9158, 9159, 9161, 9162")
    ...
    def _query_enhancement_llm(self, llm_name: str, port: int, genome: dict) -> dict:
        ...
        try:
            ...
            # Send request
            request_json = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-131: default=str para numpy types
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for USDCAD)
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            try:
                ...
                log.error(f"[NOVA CONNECTION] ? No se pudo conectar a puerto 9165")
                return self._fallback_nova_response()
...
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                self.dashboard = QuantumDashboard(port=7567)  # USDCAD dashboard port
    ...
    def run(self, port=9166):
        """Start TCP server on specified port (USDCAD Trinity on 9166)"""
...
def run_server(port=9166):
    """Run Trinity consensus server with optional dashboard - USDCAD"""
...
if __name__ == '__main__':
    try:
        log.info('[TRINITY] -----------------------------------------')
        log.info('[TRINITY] TRINITY CONSENSUS ENGINE v1.0 - USDCAD')
        log.info('[TRINITY] Consensus Orchestrator - 10 LLM Integration')
        log.info('[TRINITY] -----------------------------------------')
        run_server(port=9166)  # USDCAD Trinity port
