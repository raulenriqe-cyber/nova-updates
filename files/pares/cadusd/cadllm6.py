...
class LLM6_SmartMoney:
    ...
    def _make_decision(self, signal: SmartMoneySignal) -> SmartMoneySignal:
        ...
        if signal.votes:
            ...
            if total_weight > 0.3:  # PHASE 10: was 0.5 - relaxed for CAD
            ...
            elif total_weight < -0.3:  # PHASE 10: was -0.5 - relaxed for CAD
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 0.6500))  # 🔧 USDCAD default
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (USDCAD 75xx range)"""
        return {
            'trinity_port': 9166,
            'kraken_port': 9152,
            'executor_port': 9156,
...
def run_tcp_server():
    ...
    # Load port from cadconfig.yaml
    try:
        with open('cadconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 9162)
    except Exception as e:
        port = 9162  # USDCAD LLM6 Smart Money Oracle
