"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - USDCAD Market Sentiment                         ║
...
║  Optimizado para USDCAD - US Dollar / Canadian Dollar                      ║
║  🍁 Oil Correlated Currency | BoC + US Data + Crude Oil + Energy Focus       ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-CAD")
...
# Símbolos a monitorear - USDCAD FOCUSED + correlated pairs
SYMBOLS = ["USDCAD", "USOIL", "DXY"]

# Fuentes confiables para forex + Canadian/North American sources
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "forexlive", "fxstreet", "dailyfx",
    "investing.com", "tradingview", "marketwatch", "cnbc",
    "financialjuice", "forexfactory", "ft.com",
    "globeandmail.com", "bnn.ca", "financialpost.com",
    "bankofcanada.ca", "statcan.gc.ca", "cbc.ca",
    "oilprice.com", "rigzone.com", "eia.gov",
    "kitco.com"
]

# Keywords por símbolo - USDCAD OPTIMIZED for Oil/Energy Currency
KEYWORDS = {
    "USDCAD": [
        "USDCAD", "canadian dollar", "USD/CAD", "loonie news",
        "BoC decision", "bank of canada", "loonie forecast",
        "canadian dollar", "fed loonie", "loonie strength",
        "loonie trading", "loonie weakness", "canadian economy",
        "canadian inflation", "canadian employment", "canadian gdp",
        "canadian retail sales", "canadian CPI", "canadian trade balance",
        "BoC rate decision", "BoC Macklem", "bank of canada rate",
        "crude oil price", "WTI oil", "brent crude", "oil futures",
        "oil inventories", "OPEC", "oil production canada",
        "EIA crude oil", "API crude", "energy sector canada",
        "US NFP", "US CPI", "FOMC decision", "fed rate",
        "US dollar index", "DXY", "greenback",
        "USDCAD rally", "USDCAD crash", "loonie forex", "oil currency"
    ],
    "USOIL": [
        "crude oil", "WTI price", "brent crude", "oil news today",
        "oil forecast", "oil futures", "OPEC production",
        "oil demand", "oil supply", "EIA inventory",
        "oil rally", "oil crash", "energy crisis"
    ],
    "DXY": [
        "dollar index", "DXY", "US dollar", "greenback",
        "dollar strength", "dollar weakness", "fed dollar",
        "US treasury yield", "dollar forecast"
    ]
}
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        Context-aware: USDCAD is an oil-correlated currency.
        - Oil prices rising → M (CAD strengthens, USDCAD drops)
        - Strong Canadian data → M
        - BoC hawkish → M (CAD stronger)
        - USD strength / Fed hawkish → B (USDCAD rises)
        - Oil crash → B (CAD weakens, USDCAD rises)
        - BoC dovish → B
        - Risk-off / USD strength → B
        """
        title = news_item.get('title', '')
        symbol = news_item.get('symbol', 'USDCAD')
...
Context: USDCAD is a commodity/carry currency. Canadian Dollar strengthens with:
- Rising oil prices, strong energy sector
- Rising iron ore, copper, gold, commodity prices
- BoC hawkish stance or rate hikes
- Risk-on sentiment in global markets
- Strong Canadian economic data (employment, GDP, CPI)

Canadian Dollar weakens with:
- Falling oil prices, weak energy sector
- Falling commodity prices
- BoC dovish stance or rate cuts
...
B = Bullish (CAD strengthens = USDCAD likely to go DOWN)
M = Bearish (CAD weakens = USDCAD likely to go UP)  
...
class NewsStorage:
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        USDCAD focused with cross-asset correlation (USOIL, DXY).
        """
        data = self.load_today()
        ...
        # USDCAD-specific confidence calculation
        aud_score = abs(sentiment.get('USDCAD', {}).get('score', 0))
        aud_total = sentiment.get('USDCAD', {}).get('total', 0)
        aud_conf = min(1.0, aud_score * (1 + min(aud_total, 10) * 0.1))
        
        # Boost confidence if correlated assets (Oil, DXY) agree with CAD direction
        aud_sent = sentiment.get('USDCAD', {}).get('sentiment', 'N')
        gold_sent = sentiment.get('DXY', {}).get('sentiment', 'N')
        nzd_sent = sentiment.get('USOIL', {}).get('sentiment', 'N')
        
        correlation_boost = 0
        if aud_sent == gold_sent and aud_sent != 'N':
            correlation_boost += 0.05  # Oil confirms CAD direction
        if aud_sent == nzd_sent and aud_sent != 'N':
            correlation_boost += 0.05  # DXY (dollar index) confirms
        
        aud_conf = min(1.0, aud_conf + correlation_boost)
        
        return {
            'USDCAD': sentiment.get('USDCAD', {}).get('sentiment', 'N'),
            'USOIL': sentiment.get('USOIL', {}).get('sentiment', 'N'),
            'DXY': sentiment.get('DXY', {}).get('sentiment', 'N'),
            'summary': ' | '.join(summaries),
            'confidence': aud_conf,
            'confidence_USDCAD': aud_conf,
            'raw': sentiment
        }
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para USDCAD.
    Orquesta: Fetch → Evaluate → Store → Provide to LLM
    🦘 Oil Currency focus: BoC, Crude Oil, US Data, Energy Sector
    """
    ...
    def fetch_and_evaluate(self) -> Dict:
        """
        Flujo completo:
        1. Busca noticias en DuckDuckGo (USDCAD, USOIL, DXY)
        ...
        """
        logger.info("=" * 60)
        logger.info("🦘 USDCAD NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        ...
        USDCAD-aware: uses cross-asset correlation for higher confidence.
        """
        sentiment = self.storage.get_sentiment_for_llm()
        s = sentiment.get(symbol, 'N')
        
        if symbol == 'USDCAD':
            conf = sentiment.get('confidence_USDCAD', sentiment.get('confidence', 0.5))
        else:
...
def main():
    ...
    print("=" * 70)
    print("🦘 NEWS INTELLIGENCE ENGINE - USDCAD")
    print("   Oil & Energy Currency | BoC + Crude Oil + US Data + Energy")
    ...
    print("=" * 70)
    print(f"   🦘 USDCAD: {sentiment.get('USDCAD', 'N')}")
    print(f"   🥝 USOIL: {sentiment.get('USOIL', 'N')}")
    print(f"   🥇 DXY: {sentiment.get('DXY', 'N')}")
    print(f"   Summary: {sentiment.get('summary', 'N/A')}")
    print(f"   Confidence: {sentiment.get('confidence_USDCAD', 0):.2f}")
    print("=" * 70)
...
def get_news_sentiment() -> Dict:
    ...
    Usage:
        ...
        if sentiment['USDCAD'] == 'B':
            # Bias bullish para CAD - oil strong
    """
    global _news_engine
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('USDCAD')
