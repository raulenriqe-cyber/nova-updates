...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║     ⚔️ NOVA EXCALIBUR KNIGHT - QUANTUM CORE v18.00 USDCAD ELITE EDITION      ║
...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for CADUSD')
...
# 🛢️ OILFANG MOMENTUM ENGINE (WTI Correlation Strategy)
try:
    from oilfang_momentum_strategy import OilfangMomentumStrategy, OilfangConfig, get_oilfang_strategy
    OILFANG_AVAILABLE = True
except ImportError:
    OILFANG_AVAILABLE = False
    OilfangMomentumStrategy = None
    OilfangConfig = None
    get_oilfang_strategy = None
    print("[WARNING] Oilfang Momentum Strategy not available")
...
try:
    from cadllm6 import (  # 🔧 FIX CAT-132: nombre correcto del módulo
        LLM6_SmartMoney,
        SmartMoneySignal,
        MarketIntention,
        SweepType
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        # 🎯 PHASE 10 RELAXED - USDCAD was WAY too strict vs other pairs
        self.min_perfect_score = 2.0   # PERFECT - was 3.5 (now match SEK)
        self.min_good_score = 1.5      # GOOD - was 2.8 (now match SEK)
        self.min_wait_score = 0.8      # WAIT - was 1.8 (now match SEK)
        self.min_sweep_score = 1.5     # SWEEP - was 2.8 (now match SEK)
        self.lock = Lock()
        ...
        # NOTE: LLM6 runs as TCP server on port 9162 (USDCAD 75xx range)
        self.llm6_port = 9162  # LLM6 NOVA Smart Money Oracle port (USDCAD)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        """Query LLM6 NOVA Smart Money Oracle via TCP (port 9162)"""
        try:
            ...
            current_price = float(genome.get('tick_data', {}).get('last', genome.get('price', 0.6500)))  # 🔧 FOREX: USDCAD ~0.65 (was 2650 Gold)
            if len(closes) < 3:
            ...
            request = {
                ...
                'symbol': genome.get('symbol', 'USDCAD'),
            }
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        # PHASE 10: Added 3 tiers like SEK (was only 1 tier at 75%)
        if trinity_confidence >= 80:
            ...
            base_prob = max(base_prob, 0.48)
            evidence.append(f"📊 Trinity DECENT conf={trinity_confidence:.0f}% (base=0.48)")
        ...
        atr = indicators.get('atr', 0.0010)  # 10 pips default for USDCAD forex
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('USDCAD')
                    if news_conf > 60:
        ...
        # ═══════════════════════════════════════════════════════════════
        # 10. SESSION TIMING - Horas de mejor probabilidad para USDCAD
        ...
        # Sydney/Tokyo session (22:00-07:00 UTC) = MEJOR momento para USDCAD
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('USDCAD')
    ...
    def calculate_confluence_score(self, 
        ...
        rsi = indicators.get('rsi', 50)
        adx = indicators.get('adx', 25)
        atr = indicators.get('atr', 0.0010)  # Default 10 pips for USDCAD forex
        macd = indicators.get('macd', 0)
        macd_signal = indicators.get('macd_signal', 0)
        ...
        if trinity_confidence >= 85:
            ...
            # 🔥 PHASE 10: CONTEXT BOOST SUPERIOR (copied from SEK intelligence)
            ...
            # 2. Validación de Momentum: MACD histograma como proxy
            if (trinity_decision == 'BUY' and macd_histogram > 0) or \
               (trinity_decision == 'SELL' and macd_histogram < 0):
                context_quality += 1
                
            # 3. Validación de Volatilidad: ATR óptimo para USDCAD
            if 0.0005 <= atr <= 0.0025:
                context_quality += 1
                
            # APLICAR BOOST BASADO EN CALIDAD DE CONTEXTO
            if context_quality >= 2:
                llm_score += 0.6  # Boost significativo con buen contexto técnico
            ...
            llm_score += 0.4  # PHASE 10: New tier for 40-50% (was no bonus below 50%)
        # Below 40% = NO SCORE BONUS
        ...
        # ATR Goldilocks zone: 0.0008-0.0025 (8-25 pips) para USDCAD scalping
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For USDCAD at ~1.25, 0.5% = 0.00625 which is reasonable for fib zone
        ...
        if 22 <= hour_utc or hour_utc <= 6:
            timing_score += 0.5
            intelligence_notes.append("⏰ SYDNEY/TOKYO - Máxima liquidez USDCAD")
        elif 13 <= hour_utc <= 16:
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # Mejor hora para USDCAD: sesión ASIÁTICA (00:00-08:00 UTC)
            # ═══════════════════════════════════════════════════════════════
            ma5 = indicators.get('ma5', indicators.get('ma_fast', 0))
            ma20 = indicators.get('ma20', indicators.get('ma_slow', 0))
            ...
            # Sesión ASIÁTICA (mejor para USDCAD): 00:00-08:00 UTC (Tokyo/Sydney)
            ...
            # 🔧 FIX: Was 28%→18%→12% — CAD is low-vol pair, consciousness naturally low
            # At 18% was still blocking all entries (Prob=10% typical)
            # ═══════════════════════════════════════════════════════════════
            ...
            if len(closes) >= 5:
                ...
                # USDCAD (~1.25) vs EURUSD (~1.08) - percentage based is symbol-agnostic
            ...
            # Esto hace el código compatible con USDCAD, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            if len(closes) >= 3 and closes[-3] > 0:
                ...
                # Umbral: 0.15% = fuerte contra (USDCAD: PHASE 10 widened from 0.12%)
                # 🔧 FIX: Was 0.08% which blocked too many CAD entries
                strong_against_pct = 0.15
                
                if trinity_decision == 'BUY':
                    if last_momentum_pct < -strong_against_pct:
                        # Momentum fuerte CONTRA - pero solo bloquear si RSI alto
                        # PHASE 10: RSI gate 45→40
            ...
            if momentum_ok and consciousness_prob >= 0.30 and calc_entry_score >= 2.0:  # 🛢️ OILFANG: relajado de 2.5→2.0
                trend = indicators.get('trend', 'NEUTRAL')
                if (trinity_decision == 'BUY' and trend == 'UPTREND') or \
                   (trinity_decision == 'SELL' and trend == 'DOWNTREND'):
                    calc_entry_score += 1.5  # PHASE 10: Increased bonus (was 1.0)
            ...
            if self.llm6_enabled:
                try:
                    ...
                    if llm6_response:
                        ...
                        self.last_llm6_decision = llm6_response.get('recommendation', 'HOLD')
            ...
            # ⭐ USDCAD: NEW Intelligence adds +9.5 potential score (24 dimensions total)
            ...
            for llm_name, llm_data in trinity_llms_data.items():
                if isinstance(llm_data, dict):
                    vote = llm_data.get('decision', llm_data.get('vote', llm_data.get('action', 'HOLD')))
                    conf = llm_data.get('confidence', 0)
                    llm_w = Config.LLM_WEIGHTS.get(llm_name, 1.0)
                    if conf > 0:
                        llm_votes_total += 1
            ...
            # 🎯 TIER 0: SUPER INTELLIGENCE GOLD - PHASE 10: Lowered all thresholds
            ...
            if super_intelligence >= 2.0 and consciousness_prob >= 0.42 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 0.5: CONSCIOUSNESS GOLD - PHASE 10: prob 55→42, score 5.5→3.5, calc 3.0→2.0
            if consciousness_prob >= 0.42 and momentum_ok and score >= 3.5 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 1: Multi-TF ALIGNMENT - PHASE 10: align 65→50, conf 55→40, calc 2.5→1.5
            if alignment_score >= 50 and trinity_confidence >= 40 and calc_entry_score >= 1.5:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 2: ULTRA-PREMIUM - PHASE 10: score 5.0→3.0, prob 45→30, calc 3.5→2.0
            if score >= 3.0 and momentum_ok and consciousness_prob >= 0.30 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 3: ELITE - PHASE 10: score 4.0→2.0, prob 40→28, calc 3.0→1.5
            if score >= 2.0 and momentum_ok and consciousness_prob >= 0.28 and calc_entry_score >= 1.5:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 4: QUALITY - FIX: prob 22→30, calc 1.0→2.5
            if score >= self.min_perfect_score and momentum_ok and consciousness_prob >= 0.30 and calc_entry_score >= 2.5:
                self._clear_pending(trinity_decision)
                adjusted_conf = trinity_confidence * 0.9
                return True, trinity_decision, adjusted_conf, \
                       f"⭐⭐⭐ QUALITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 5: SMART MONEY - FIX: prob 22→30, calc 1.0→2.5
            if score >= self.min_sweep_score and momentum_ok and consciousness_prob >= 0.30 and calc_entry_score >= 2.5:
            ...
            # 🎯 TIER 5b: TRINITY FORCE - FIX: conf 40→52, prob 15→28, calc 0.5→2.0, score 1.0→2.0
            if trinity_confidence >= 52 and momentum_ok and score >= 2.0 and consciousness_prob >= 0.28 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.85, \
                       f"⭐ TRINITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 6: HIGH CONFIDENCE OVERRIDE - FIX: conf 45→60, prob 15→26, calc 0.3→1.5
            if trinity_confidence >= 60 and score >= 0.8 and consciousness_prob >= 0.26 and calc_entry_score >= 1.5:
                self._clear_pending(trinity_decision)
            ...
            # REMOVED: TIER 8 FALLBACK — was accepting trinity 35% + consciousness 10% = guaranteed losses
    ...
    def _validate_direction_ultra_exigent(
        ...
        # 🧠 CONTEXT-AWARE: patrones débiles en tendencia fuerte = ruido
        # ═══════════════════════════════════════════════════════════════
        patterns = genome.get('patterns_detected', [])
        if not patterns:
            patterns = genome.get('chart_patterns', [])
        ...
        # Patrones de 1 vela = menor fiabilidad en tendencias fuertes
        minor_bullish_single = ['hammer', 'inverted_hammer', 'doji_bull']
        minor_bearish_single = ['hanging_man', 'doji_bear', 'shooting_star']
        
        # Contexto de tendencia para ponderar patrones
        _adx_ctx = indicators.get('adx', 25)
        _ma5_ctx = indicators.get('ma5', indicators.get('ma_fast', 0))
        _ma20_ctx = indicators.get('ma20', indicators.get('ma_slow', 0))
        _strong_downtrend = _adx_ctx > 35 and _ma5_ctx > 0 and _ma20_ctx > 0 and _ma5_ctx <= _ma20_ctx
        _strong_uptrend   = _adx_ctx > 35 and _ma5_ctx > 0 and _ma20_ctx > 0 and _ma5_ctx >= _ma20_ctx
        ...
        for pattern in patterns:
            ...
            if direction == 'BUY':
                if any(bp in p_name for bp in bullish_reversal):
                    ...
                    # 🧠 Patrón bajista débil en tendencia alcista fuerte = ruido
                    if _strong_uptrend and any(mp in p_name for mp in minor_bearish_single):
                        confirmations.append(f"⚠️ {p_name} ignorado (ruido en uptrend ADX={_adx_ctx:.0f})")
                    else:
                ...
                if any(bp in p_name for bp in bearish_reversal):
                    ...
                    # 🧠 Patrón alcista débil (1 vela) en downtrend fuerte = ruido estadístico
                    # En ADX>35 + MA5<=MA20, hammer tiene prob reversión <20% → ignorar
                    if _strong_downtrend and any(mp in p_name for mp in minor_bullish_single):
                        confirmations.append(f"⚠️ {p_name} ignorado (ruido en downtrend ADX={_adx_ctx:.0f})")
                    else:
                        validations_failed += 1
        ...
        if direction == 'BUY':
            if rsi > 80:  # Overbought extremo
                # 🛢️ OILFANG MOMENTUM EXCEPTION: En tendencia fuerte (ADX>50),
                # RSI alto = SEÑAL de momentum, no overbought. 1 falla en vez de 2.
                _adx_rsi = indicators.get('adx', 0)
                if _adx_rsi > 50:
                    validations_failed += 1
                    failures.append(f"RSI={rsi:.0f} alto pero momentum ADX={_adx_rsi:.0f} (1 falla)")
                else:
                    validations_failed += 2
        ...
        # ═══════════════════════════════════════════════════════════════
        adx = indicators.get('adx', 25)
        if adx < 15:  # 🔧 Bajado de 20→15 (consistente con XAU/GBP)
            validations_failed += 1
        ...
        if total_checks < 4 and validations_passed < validations_failed:
            # 🧠 EXCEPCIÓN INTELIGENTE: Tendencia fuerte + 1 sola falla menor de patrón
            # Si ADX>30 + conf>60% + solo 1 falla = probablemente ruido de patrón
            adx_rule5 = indicators.get('adx', 0)
            if adx_rule5 > 30 and confidence >= 60 and validations_failed == 1:
                confirmations.append(f"⚡ Rule5-bypass: ADX={adx_rule5:.0f} tendencia fuerte, 1 falla menor")
            else:
...
class NOVAMarketStateDetector:
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Spread too wide (illiquidity) - USDCAD spread in price units (0.0001 = 1 pip)
        elif spread > 0.0010:  # For USDCAD, > 10 pips is abnormally wide
...
class Config:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ⚔️ STRATEGY SELECTOR (loaded from cadconfig.yaml)
    # ═══════════════════════════════════════════════════════════════════════
    ACTIVE_STRATEGY = "SCALPER"  # Default, will be overridden by yaml
    EXCALIBUR_CONFIG = None      # ExcaliburKnightConfig instance if active
    OILFANG_CONFIG = None        # OilfangMomentumStrategy instance if active

    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 STRATEGY IDENTITY — SSOT (loaded from cadconfig.yaml → strategy_identity)
    # ═══════════════════════════════════════════════════════════════════════
    STRATEGY_DISPLAY_NAME = "SCALPER"                              # display_name
    STRATEGY_EMOJI        = "🧠"                                    # emoji
    STRATEGY_VERSION      = "18.00"                                # version
    STRATEGY_TAGLINE      = "M1 Super Intelligence | 10-LLM Brain" # tagline
    STRATEGY_EDITION      = "SCALPER"                              # edition
    
    # ═══════════════════════════════════════════════════════════════════════
    # NETWORK PORTS
    # ═══════════════════════════════════════════════════════════════════════
    HOST = '127.0.0.1'
    QUANTUM_PORT = 9154      # Injector - receive genomes from Quimera (USDCAD)
    EXECUTOR_PORT = 9156     # Executor - send commands to Quimera (USDCAD)
    TRINITY_PORT = 9166      # Trinity server port (USDCAD 75xx range)
    KRAKEN_PORT = 9152       # Kraken executor (USDCAD)
    LLM1_PORT = 9155         # BAYESIAN
    LLM2_PORT = 9157         # TECHNICAL
    LLM3_PORT = 9158         # CHART
    LLM4_PORT = 9159         # RISK
    LLM5_PORT = 9161         # LLM5 SUPREME chart analyzer
    LLM6_PORT = 9162         # ⭐ LLM6 NOVA Smart Money Oracle (USDCAD)
    LLM7_PORT = 9163         # LLM7 OCULUS Data Quality
    LLM8_PORT = 9164         # LLM8 CHRONOS Timing
    LLM9_PORT = 9165         # LLM9 PREDATOR Execution
    LLM10_PORT = 9160        # LLM10 SENTINEL Guardian
    LLM11_PORT = 9167        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 9168        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 9170  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 M1 SUPER INTELIGENCIA (10-LLM Brain + Full Quantum Core)
    # ═══════════════════════════════════════════════════════════════════════
    # Trading - M1 SUPER INTELLIGENCE SCALPING
    # 🔧 USDCAD M1: Tight targets, surgical precision with full 10-LLM brain
    TP_MIN_PIPS = 8       # 8 pips MINIMUM - M1 precision
    TP_MAX_PIPS = 20      # 20 pips MAX - M1 realistic targets
    SL_MIN_PIPS = 5       # 5 pip stops - tight M1 control
    SL_MAX_PIPS = 12      # Max 12 pip stop - protect capital
    ATR_FLOOR = 0.0005    # 5 pip min ATR - lower floor for M1 forex
    CONSOLIDATION_THRESHOLD = 5.0    # Reduced: Allow more trades in M1
    
    # Risk - SUPER INTELLIGENCE PRECISION for USDCAD M1
    MAX_CONCURRENT_TRADES = 3        # 🔧 FIX: Was 1 (too restrictive) → 3 positions
    MAX_DAILY_LOSS_PERCENT = 1.5     # USDCAD: 1.5% max daily loss
    MAX_CONSECUTIVE_LOSSES = 3       # FIX A-CAD-02: halt after 3 consecutive losses
    LOSS_HALT_DURATION_SECONDS = 300 # FIX A-CAD-02: 5-minute cooling-off period
    MIN_CONFIDENCE = 52              # PHASE 10: 52%+ (was 60 - too strict)
    MIN_TRINITY_CONFIDENCE = 50      # FIX: was 20 — way too low, letting 35% trades through
    MIN_ADX = 20                     # PHASE 10: 20 (was 22 - allow more entries)
    
    # ═══════════════════════════════════════════════════════════════
    # 🧠 PAIR-SPECIFIC LLM WEIGHTS - USDCAD (Commodity-driven pair)
    # AUD responds to: commodities, China data, RBA, risk sentiment
    # CHART patterns matter most, SMART_MONEY less (institutional ≠ commodity)
    # ═══════════════════════════════════════════════════════════════
    LLM_WEIGHTS = {
        'BAYESIAN': 1.2,       # Good for ranging commodity pairs
        'TECHNICAL': 1.0,      # Standard technical analysis
        'CHART': 1.5,          # Chart patterns very relevant for AUD
        'RISK': 0.5,           # ⚠️ REDUCED: Always votes HOLD @ 55% - too conservative
        'SUPREME': 1.3,        # Actually analyzes patterns, good for AUD
        'SMART_MONEY': 0.3,    # ⚠️ CRITICAL: SM defaults to HOLD@91% for AUD - minimal weight
        'OCULUS': 0.8,         # Quality validator - moderate
        'CHRONOS': 1.1,        # Timing (Asian session matters for AUD)
        'PREDATOR': 1.0,       # Execution engine - neutral
        'NOVA': 0.5,           # Offline/fallback - reduced
    }
    ...
    # Minimum directional LLMs needed (lowered from 3 for AUD where 6/9 default HOLD)
    ...
    SNIPER_COOLDOWN_SECONDS = 90    # 🔧 JPY-STYLE: 90s between attacks
    WARMUP_SECONDS = 0              # PHASE 10: 0s warmup (was 30 - unnecessary delay)
    ...
    EXCALIBUR_MAX_POSITIONS = 3      # PHASE 10: 3 positions (was 1 - too restrictive)
    ...
    # 🏦 LANCE FORGER SOVEREIGN STRATEGY (Carry Reversal Micro-Trading)
    # ═══════════════════════════════════════════════════════════════════════
    # 🔧 USDCAD CALIBRATED: Menos trades, más calidad, proteger capital
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 🛢️ OILFANG MOMENTUM ENGINE (WTI Correlation)
    # ═══════════════════════════════════════════════════════════════════════
    OILFANG_TP_MIN_PIPS = 10
    OILFANG_TP_MAX_PIPS = 24
    OILFANG_SL_MIN_PIPS = 7
    OILFANG_SL_MAX_PIPS = 13
    OILFANG_MAX_POSITIONS = 2
    OILFANG_MIN_CONFIDENCE = 65
    OILFANG_MIN_TRINITY_CONFIDENCE = 60
    OILFANG_MIN_ADX = 20
    OILFANG_ATR_FLOOR = 0.0003
    OILFANG_ATR_CEILING = 0.0011
    OILFANG_COOLDOWN_SECONDS = 0
    OILFANG_WTI_MIN_MOVEMENT = 0.25
    OILFANG_WTI_CONFIRMATION_BARS = 2
    OILFANG_WTI_CORRELATION_MIN = 0.65
    OILFANG_MOMENTUM_SCORE_MIN = 2.6
    OILFANG_VOLUME_MIN_RATIO = 1.20
    OILFANG_VOLUME_BARS = 3
    OILFANG_BREAKOUT_MIN_PIPS = 4
    OILFANG_BREAKOUT_MAX_PIPS = 10

    # ═══════════════════════════════════════════════════════════════════════
    # NETWORK / TIMEOUTS / RESILIENCE
    # ═══════════════════════════════════════════════════════════════════════
    SOCKET_TIMEOUT = 3.5  # Increased from 2.0 — CAD Trinity was hitting 2s timeout (decisions_made=0 with 5835 genomes)
    BUFFER_SIZE = 65536
    ...
    def load_strategy_from_yaml(cls):
        """Load active strategy from cadconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'cadconfig.yaml')
            if os.path.exists(config_path):
                ...
                # ── SSOT: load strategy identity from yaml ──
                identity = config.get('strategy_identity', {})
                cls.STRATEGY_DISPLAY_NAME = identity.get('display_name', cls.ACTIVE_STRATEGY)
                cls.STRATEGY_EMOJI        = identity.get('emoji', '🧠')
                cls.STRATEGY_VERSION      = identity.get('version', '18.00')
                cls.STRATEGY_TAGLINE      = identity.get('tagline', 'USDCAD Trading System')
                cls.STRATEGY_EDITION      = identity.get('edition', cls.ACTIVE_STRATEGY)
                ...
                elif strategy == 'OILFANG_MOMENTUM':
                    cls._apply_oilfang_config(config.get('oilfang_momentum', {}))
                    print(f"[CONFIG] 🛢️ OILFANG MOMENTUM ENGINE loaded - WTI Correlation Macro Momentum")
                else:
    ...
    @classmethod
    def is_oilfang_active(cls) -> bool:
        """Check if OilFang Momentum strategy is active"""
        return cls.ACTIVE_STRATEGY == 'OILFANG_MOMENTUM'
    ...
    @classmethod
    def _apply_oilfang_config(cls, oilfang_yaml: dict):
        """Apply OilFang Momentum configuration from yaml"""
        cls.TP_MIN_PIPS = oilfang_yaml.get('tp_min_pips', cls.OILFANG_TP_MIN_PIPS)
        cls.TP_MAX_PIPS = oilfang_yaml.get('tp_max_pips', cls.OILFANG_TP_MAX_PIPS)
        cls.SL_MIN_PIPS = oilfang_yaml.get('sl_min_pips', cls.OILFANG_SL_MIN_PIPS)
        cls.SL_MAX_PIPS = oilfang_yaml.get('sl_max_pips', cls.OILFANG_SL_MAX_PIPS)
        cls.MAX_CONCURRENT_TRADES = oilfang_yaml.get('max_concurrent_trades', cls.OILFANG_MAX_POSITIONS)
        cls.MIN_CONFIDENCE = int(oilfang_yaml.get('min_confidence', cls.OILFANG_MIN_CONFIDENCE))
        cls.MIN_TRINITY_CONFIDENCE = int(oilfang_yaml.get('min_trinity_confidence', cls.OILFANG_MIN_TRINITY_CONFIDENCE))
        cls.MIN_ADX = oilfang_yaml.get('min_adx', cls.OILFANG_MIN_ADX)
        cls.ATR_FLOOR = oilfang_yaml.get('atr_floor', cls.OILFANG_ATR_FLOOR)
        cls.SNIPER_COOLDOWN_SECONDS = 0  # No cooldown -- WTI bias is the filter
        cls.MIN_RSI_FOR_BUY = oilfang_yaml.get('min_rsi_for_buy', 30)
        cls.MAX_RSI_FOR_BUY = oilfang_yaml.get('max_rsi_for_buy', 75)
        cls.MIN_RSI_FOR_SELL = oilfang_yaml.get('min_rsi_for_sell', 25)
        cls.MAX_RSI_FOR_SELL = oilfang_yaml.get('max_rsi_for_sell', 70)
        cls.OILFANG_WTI_MIN_MOVEMENT = oilfang_yaml.get('wti_min_movement', cls.OILFANG_WTI_MIN_MOVEMENT)
        cls.OILFANG_WTI_CORRELATION_MIN = oilfang_yaml.get('wti_cad_correlation_min', cls.OILFANG_WTI_CORRELATION_MIN)
        cls.OILFANG_MOMENTUM_SCORE_MIN = oilfang_yaml.get('momentum_score_min', cls.OILFANG_MOMENTUM_SCORE_MIN)
        cls.OILFANG_VOLUME_MIN_RATIO = oilfang_yaml.get('volume_min_ratio', cls.OILFANG_VOLUME_MIN_RATIO)
        cls.OILFANG_BREAKOUT_MIN_PIPS = oilfang_yaml.get('breakout_min_pips', cls.OILFANG_BREAKOUT_MIN_PIPS)
        cls.OILFANG_BREAKOUT_MAX_PIPS = oilfang_yaml.get('breakout_max_pips', cls.OILFANG_BREAKOUT_MAX_PIPS)
        if OILFANG_AVAILABLE and get_oilfang_strategy:
            cls.OILFANG_CONFIG = get_oilfang_strategy(oilfang_yaml)
            print(f"[🛢️ OILFANG] Strategy instance created: {cls.OILFANG_CONFIG}")
    ...
    def get_strategy_info(cls) -> dict:
        ...
        elif cls.is_oilfang_active():
            return {
                'name': 'OILFANG_MOMENTUM',
                'description': 'WTI Correlation Macro Momentum Engine',
                'timeframe': 'M15 → M5/M1',
                'tp_range': f'{cls.OILFANG_TP_MIN_PIPS}-{cls.OILFANG_TP_MAX_PIPS} pips',
                'sl': f'{cls.OILFANG_SL_MIN_PIPS}-{cls.OILFANG_SL_MAX_PIPS} pips',
                'risk_reward': '1.6-2.2:1',
                'trades_per_session': '5-9',
                'style': '🛢️ OilFang WTI Momentum'
            }
        else:
            return {
                'name': 'SCALPER',
...
class NOVADashboard:
    """
    NOVA EXCALIBUR KNIGHT - USDCAD Elite Momentum Dashboard
    ...
    def render(self):
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'USDCAD'))
        ...
        lines.append("")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}███╗   ██╗{self.RESET}{SOFT_WHITE}{self.BOLD} ██████╗ ██╗   ██╗ █████╗ {self.RESET}    {self.CYAN}{self.BOLD}{Config.STRATEGY_EMOJI} {Config.STRATEGY_DISPLAY_NAME}{self.RESET}  •  {self.GREEN}●{self.RESET} {now.strftime('%H:%M:%S')}  •  Up {self.CYAN}{uptime_str}{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}USDCAD Elite Momentum by{self.RESET} {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}{Config.STRATEGY_EMOJI} {Config.STRATEGY_DISPLAY_NAME}{self.RESET} v{Config.STRATEGY_VERSION} USDCAD {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
    ...
    def _send_llm_decision_to_dashboard(self, llm_name, decision, confidence, reason=""):
        """Send LLM decision message to trinity_dashboard_v3 TCP listener on port 7567 (USDCAD)"""
        try:
            import socket
            ...
            # Serialize and send to localhost:7567 (USDCAD dashboard port)
...
class LLMParallelConsensus:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-136: 5 workers para 5 LLMs
        self.logger = logger
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-129: default=str para numpy types
...
class AdaptiveTPSLEngine:
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
        ...
        """
        Calculate INTELLIGENT TP/SL for M1 USDCAD using ALL available data:
        ...
        """
        # Extract indicators
        atr = float(indicators.get('atr', 0.0010))  # Default 10 pips for USDCAD
        rsi = float(indicators.get('rsi', 50))
        ...
        # Ensure minimum ATR for USDCAD M1
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 USDCAD typical ATR: 0.0003-0.0015 (= 3-15 pips)
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 USDCAD SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico USDCAD: 0.0005-0.0025 (5-25 pips)
        # Multiplicadores: TP = ATR × 1.1, SL = ATR × 0.8
        # Esto da R:R de ~1.375:1 mínimo
        # ═══════════════════════════════════════════════════════════════
        
        # ATR viene en precio para USDCAD forex (ej: 0.0010 = 10 pips)
        ...
        # 🔧 FIX v5: USDCAD M1 SCALPING - reduced distances for tight entries
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        # Convert to pips for return (1 pip = 0.0001 for USDCAD forex)
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        ...
        # FIX v5: Mínimos REDUCIDOS para M1 USDCAD scalping
        ...
        # Point value conversion: USDCAD = 0.0001 per pip (forex standard)
        point_value = 0.0001  # USDCAD: 1 pip = 0.0001 (e.g., 35 pips = 0.0035)
    ...
    def calculate_smart_plan(self, current_price, direction, genome, indicators, sweep_info=None):
        ...
        try:
            ...
            POINT = 0.0001  # USDCAD forex: 1 pip = 0.0001
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.0001) if atr > 0 else 0  # [NOVA AUDIT F-11] USDCAD pip=0.0001, ATR M1 ~3-8 pips
...
class QuantumConsciousnessEngine:
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        if SHARED_BRAIN_AVAILABLE and self.genome_counter % 30 == 0:
            try:
                ...
                pass  # Non-critical
        # 💾 RL PERIODIC SAVE: persist Q-table every 500 genomes
        ...
        atr_val = indicators.get('atr', 0.0010)  # Default 10 pips for USDCAD
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('USDCAD')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM7 OCULUS - Data Quality Validator (Port 9162)
            ...
            # ✨ LLM8 CHRONOS - Timing Optimizer (Port 9163)
            ...
            # ✨ LLM9 PREDATOR - Execution Engine (Port 9164)
            ...
            if closes and len(closes) >= 5:
                ...
                try:
                    ...
                    llm5_analysis = LLM5Client.analyze(
                        symbol=symbol,
                        timeframe='M1',
                        candles={  # 🔧 FIX CAT-128: parámetro correcto es 'candles'
                            'closes': closes,
                            'opens': opens,
                            'highs': highs,
                            'lows': lows,
        ...
        try:
            ...
            # CRITICAL: Round to valid lot increments (0.01 for USDCAD)
            ...
            if trend_type == 'RANGING' and trinity_decision in ['BUY', 'SELL']:
                # IMPROVED: Reduced thresholds — CAD is often RANGING, too strict = never attacks
                # 🔧 FIX: trinity 50→45, consensus 0.45→0.35 — was blocking ALL CAD entries
                if consensus_score < 0.35 and trinity_confidence < 45:
                    self.logger.warning(f"[TREND_VETO] 🚫 Blocking: RANGING market, consensus={consensus_score*100:.0f}% < 35%, trinity={trinity_confidence}% < 45%")
                    return {'action': 'HOLD', 'reason': 'range_bound_market', 'trend': trend_type, 'score': consensus_score}
                elif trinity_confidence >= 45:
                    self.logger.info(f"[TREND_ALLOWED] ✅ Allowing {trinity_decision} in RANGING market due to Trinity confidence ({trinity_confidence}%)")
            ...
            # USDCAD: Block if consensus < 30% (with 10-LLM rescue)
            # 🔧 FIX: Was 40% with NO LLM fallback → killed all trades in low-consensus environments
            consensus_reason = _consensus_data.get('reason', 'unknown') if '_consensus_data' in dir() else 'N/A'
            if consensus_score < 0.30 and trinity_decision in ['BUY', 'SELL']:
                ...
                if llm_consensus_decision == trinity_decision and llm_consensus_confidence >= 50:
                    ...
                    self.logger.info(f"[CONSENSUS_SMART] 🧠 Using 10-LLM conviction {consensus_score*100:.0f}% (TF unavailable: {consensus_reason}) — continuing")
                else:
                    self.logger.warning(f"[CONSENSUS_VETO] 🚫 BLOCKING: Consensus {consensus_score*100:.0f}% < 30% (reason: {consensus_reason}) - No 10-LLM conviction backup")
                    # 🔧 FIX: return was outside else — was blocking even when 10-LLM backup succeeded
                    return {
                        'action': 'HOLD',
            ...
            # 🔵 RELAXED THRESHOLD: Trinity must be >= 38% to execute trades
            # 🔧 FIX 2026-02-19: Was 45% → 38% — exotic pairs have lower consensus naturally
            if trinity_confidence >= 38.0:
                self.logger.info(f"[CONFIDENCE_CHECK] ✅ Trinity {trinity_confidence}% >= 38% + Consensus {consensus_score*100:.0f}% - proceeding with {trinity_decision}")
                pass  # Continue to decision execution below
            else:
                # 🚫 BLOCK: Any confidence below 38% is rejected
                self.logger.warning(f"[CONFIDENCE_VETO] 🚫 Blocking {trinity_decision}: Trinity {trinity_confidence}% < 38% minimum (consensus {consensus_score*100:.0f}%)")
                return {
                    'action': 'HOLD',
                    'reason': 'insufficient_trinity_confidence',
                    'trinity_confidence': trinity_confidence,
                    'threshold_required': 38.0,
                    'consensus_score': consensus_score
                }
            
            # ============ CONSOLIDATION TRAP DETECTION (RELAXED FOR M1) ============
            # For M1 forex scalping, we can trade even in tight ranges - adjusted for USDCAD
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                if trinity_decision == 'BUY':
                    ...
                    if not trend_aligned:
                        self.logger.warning(f"[TREND CHECK] ⚠️ BUY against trend: MA5={ma5:.2f} MA20={ma20:.2f} RSI={rsi_val:.0f} ADX={adx_val:.0f}")
                        trinity_confidence *= 0.92  # 🔧 FIX v2: Was 0.80 (20%) → 0.92 (8%) — SCALPER in RANGING needs room to breathe
                elif trinity_decision == 'SELL':
                    ...
                    if not trend_aligned:
                        self.logger.warning(f"[TREND CHECK] ⚠️ SELL against trend: MA5={ma5:.2f} MA20={ma20:.2f} RSI={rsi_val:.0f} ADX={adx_val:.0f}")
                        trinity_confidence *= 0.92  # 🔧 FIX v2: Was 0.80 (20%) → 0.92 (8%) — prevent cascade death with CHRONOS
                
                # WHALE TRAP: Reduce confidence if detected (don't block, just penalize)
                if is_trap and trap_confidence > 60:
                    trinity_confidence *= 0.88  # 🔧 FIX v2: Was 0.85 (15%) → 0.88 (12%) — consistent with GBP fix
                ...
                if Config.is_excalibur_active() and hasattr(self, 'excalibur_analyzer') and self.excalibur_analyzer:
                    ...
                    if NEWS_INTELLIGENCE_AVAILABLE:
                        try:
                            news_bias = get_trading_bias('USDCAD')
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 9170
        ...
        try:
            ...
            # Send to sound.py server on port 9170 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 9170))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:9170")
        except socket.timeout:
...
class AdvancedIPC:
    ...
    def __init__(self):
        ...
        self.message_queue = deque(maxlen=50)  # Ring buffer: keep last 50 ticks (reduced from 1000 to prevent queue flooding)
        self.lock = RLock()
    ...
    def _play_sound_alert(self, alert_type, symbol='USDCAD'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # 🔧 FIX: Use Config.SOUND_PORT instead of hardcoded 9170
            sound_port = getattr(Config, 'SOUND_PORT', 9170)
                ...
                if request.get('type') == 'handshake':
                    ...
                    with self.quimera_lock:
                        self.quimera_connections[('executor', addr)] = {
                            ...
                            'type': 'executor',
                            'last_ping': time.time(),
                            'symbol': request.get('symbol', 'USDCAD')
                        }
                ...
                try:
                    ...
                    # CRITICAL: This enables ML learning from trade results!
                        ...
                        try:
                            ...
                            # 💾 SAVE STATS SNAPSHOT for dashboard persistence
                            try:
                                if hasattr(self, '_save_stats_snapshot'):
                                    self._save_stats_snapshot()
                            except Exception:
                                pass
                            # 🧠 SHARED BRAIN: Record trade outcome for cross-pair ML learning (executor path)
                            try:
    ...
    def _handle_genome(self, client, addr):
        ...
        try:
            client.settimeout(120.0)  # 120 sec timeout for persistent connections (increased for stability)
            ...
            while True:
                try:
                    ...
                    # Handle handshake on injector port - SEND ACK BACK
                    if genome.get('type') == 'handshake':
                        with self.stats_lock:
                            self.stats['quimera_handshakes'] += 1
                        try:
                            ack = json.dumps({'type': 'handshake_ack', 'status': 'connected', 'symbol': 'USDCAD', 'port': Config.QUANTUM_PORT}).encode('utf-8')
                            ack_msg = struct.pack('<I', len(ack)) + ack
                            client.sendall(ack_msg)
                            self.logger.info(f"[Injector] Handshake ACK sent to {addr}")
                        except Exception as e:
                            self.logger.warning(f"[Injector] Failed to send handshake ACK: {e}")
                        continue
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "USDCAD", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "USDCAD"}, ...}
        """
    ...
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for USDCAD data"""
        try:
            genome_type = genome.get('type', '')
            
            # ⭐ Accept both standard and Multi-TF genome types from audquimera
            valid_types = [
            ...
            if genome_type in valid_types:
                ...
                try:
                    ...
                    # Acceptable range for USDCAD
                    # USDCAD is a forex pair, typically 1.10 - 1.50
                    # Accept range: 0.5 - 3.0 to be safe for historical/future variations
                    mid_price = (bid + ask) / 2.0
                    
                    # USDCAD forex price range (0.5 - 3.0)
                    if 0.5 <= mid_price <= 3.0:
                        return True
                    
                    # Price out of realistic range for USDCAD
                    self.logger.warning(f"[Validator] USDCAD price out of range: {mid_price:.5f} (expected 0.5-3.0)")
                    return False
    ...
    def get_pending_genomes(self):
        """Get pending genomes with smart drain - process only latest when backlogged"""
        with self.lock:
            ...
            if queue_size > 2:
                # Backlogged: extract all, but only return the latest genome
                # Skip portfolio_sync messages, keep latest market genome
                all_genomes = list(self.message_queue)
                self.message_queue.clear()
                
                # Separate portfolio_sync from market genomes
                market_genomes = [g for g in all_genomes if g.get('type') != 'portfolio_sync']
                sync_genomes = [g for g in all_genomes if g.get('type') == 'portfolio_sync']
                
                result = []
                if sync_genomes:
                    result.append(sync_genomes[-1])  # Latest sync only
                if market_genomes:
                    result.append(market_genomes[-1])  # Latest market genome only
                
                self.logger.debug(f"[Queue] Smart drain: {queue_size} queued → {len(result)} processed (dropped {queue_size - len(result)} stale)")
                return result
            else:
                # Normal: process all
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-130: default=str para numpy types
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - Quimera not connected to port 9156")
            ...
            # 1. Validate price is within realistic bounds (USDCAD: 0.5-3.0 forex range)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR USDCAD
            # ⭐ USDCAD is forex pair, typically 1.10 - 1.50, accept 0.5 - 3.0 range
            valid_forex_range = 0.5 <= entry_price <= 3.0  # Normal USDCAD forex pricing
            
            if not valid_forex_range:
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price:.5f} out of realistic range for USDCAD (expected 0.5-3.0)")
                return False
            ...
            # ⚠️ CRITICAL: Clamp ATR to M1 USDCAD realistic range
            ...
            # 🔧 FIX v4: USDCAD forex - distances in price units (0.0001 = 1 pip)
            # ═══════════════════════════════════════════════════════════════════════════
            ...
            # 🚨 HARD CAP: Calibrated for USDCAD M1 SCALPER
...
class QuantumCore:
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        if closes and current_price > 0:
            ...
            if gap > 0.01:  # More than 0.01 (1 pip for USDCAD) difference
    ...
    def __init__(self):
        ...
        # 🛢️ OILFANG MOMENTUM Strategy Integration
        self.oilfang_strategy = None
        if Config.is_oilfang_active() and OILFANG_AVAILABLE and Config.OILFANG_CONFIG:
            self.oilfang_strategy = Config.OILFANG_CONFIG
            self.logger.info('[🛢️ OILFANG] Momentum Engine initialized - WTI Correlation 24/5 mode')
        ...
        # FIX M-CAD-01: NOVAMarketStateDetector singleton — instantiate once, not per genome
        self.nova_detector = NOVAMarketStateDetector(logger=self.logger)

        # FIX A-CAD-02: consecutive-loss halt timer
        self.loss_halt_until = 0.0

        # FIX A-CAD-04: position reconcile timer
    ...
    def _print_banner(self):
        ...
        logger.info("")
        logger.info("=" * 110)
        logger.info(f"{Config.STRATEGY_EMOJI}  {Config.STRATEGY_DISPLAY_NAME} v{Config.STRATEGY_VERSION} - USDCAD {Config.STRATEGY_TAGLINE}")
        logger.info("=" * 110)
        logger.info("")
        ...
        if is_excalibur:
            logger.info(f"{Config.STRATEGY_EMOJI} {Config.STRATEGY_DISPLAY_NAME} FEATURES:")
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            symbol = genome.get('symbol') or metadata.get('symbol', 'USDCAD')
            ...
            # CRITICAL: Validate price is in reasonable range for USDCAD FOREX
            # USDCAD typically trades 1.10-1.50, historical range 0.5-3.0
            # This is FOREX, NOT gold - prices are around 1.XX not thousands
            if symbol == 'USDCAD' and close_price > 0:
            ...
            if ea_rsi is not None and ea_atr is not None:
                ...
                ATR_FLOOR_USDCAD = 0.0003
                atr_at_floor = False
                if atr_calculated < ATR_FLOOR_USDCAD:
                    self.logger.warning(f"[Transform] ⚠️ ATR floor: {atr_calculated:.6f} → {ATR_FLOOR_USDCAD} (3 pips minimum)")
                    atr_calculated = ATR_FLOOR_USDCAD
                    atr_at_floor = True
            ...
            transformed = {
                ...
                'bar_data': {
                    ...
                    'opens': [float(h.get('open', open_price)) for h in history[-20:]] if history else [open_price] * 5,
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            if symbol == 'USDCAD' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')

            # ═══ FIX A-CAD-02: CONSECUTIVE LOSSES CIRCUIT BREAKER ═══
            with self.ipc.stats_lock:
                consecutive_losses = self.ipc.stats.get('consecutive_losses', 0)
            loss_halt_until = getattr(self, 'loss_halt_until', 0)
            if loss_halt_until > time.time():
                remaining = loss_halt_until - time.time()
                self.logger.warning(f"[A-CAD-02] Loss halt active — {remaining:.0f}s remaining. Skipping.")
                return None
            if consecutive_losses >= Config.MAX_CONSECUTIVE_LOSSES:
                self.loss_halt_until = time.time() + Config.LOSS_HALT_DURATION_SECONDS
                self.logger.error(f"[A-CAD-02] {consecutive_losses} consecutive losses → halt {Config.LOSS_HALT_DURATION_SECONDS}s")
                return None

            # ═══ FIX A-CAD-03: DAILY LOSS CIRCUIT BREAKER ═══
            with self.ipc.stats_lock:
            ...
            if balance > 0 and today_pnl / balance * 100 <= -Config.MAX_DAILY_LOSS_PERCENT:
                self.logger.error(f"[A-CAD-03] Daily loss limit hit: {today_pnl:.2f}/{balance:.2f} = {today_pnl/balance*100:.2f}%")
                return None
            ...
            nova_detector = self.nova_detector  # FIX M-CAD-01: use singleton, not per-call
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                # Calculate bid/ask from current price with typical USDCAD spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 0.00005  # Half spread for USDCAD (typical spread = 0.0001 = 1 pip)
            ...
            # ═══ FIX C-CAD-01: ATR=0 CIRCUIT BREAKER ═══
            if atr_val == 0.0:
                with self.ipc.stats_lock:
                    self.ipc.stats['_atr_zero_count'] = self.ipc.stats.get('_atr_zero_count', 0) + 1
                    _atr_zeros = self.ipc.stats['_atr_zero_count']
                if _atr_zeros >= 5:
                    self.logger.warning(f"[C-CAD-01] ATR=0 for {_atr_zeros} consecutive ticks — skipping to avoid divide-by-zero")
                    return None
            else:
                with self.ipc.stats_lock:
            ...
            _did_increment = False  # FIX C-CAD-02: track position increment for safe rollback
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-CAD-05: check direction-poison timer before stacking
                            _poison_key = f'dir_poison_until_{decision["action"]}'
                            _poison_until = getattr(self, _poison_key, 0)
                            if _poison_until > time.time():
                                logger.warning(f"[A-CAD-05] Dir-poison active for {decision['action']} — {_poison_until - time.time():.0f}s remaining. Blocking stack.")
                                decision_for_kraken = decision.copy()
                                decision_for_kraken['action'] = 'HOLD'
                                ...
                                _did_increment = True  # FIX C-CAD-02
                        ...
                        # FIX A-CAD-05: check direction-poison timer before first entry
                        _poison_key_first = f'dir_poison_until_{decision["action"]}'
                        _poison_until_first = getattr(self, _poison_key_first, 0)
                        if _poison_until_first > time.time():
                            logger.warning(f"[A-CAD-05] Dir-poison active for {decision['action']} — {_poison_until_first - time.time():.0f}s remaining. Blocking first entry.")
                            decision_for_kraken = decision.copy()
                            decision_for_kraken['action'] = 'HOLD'
                            decision_for_kraken['reason'] = f'dir_poison_{decision["action"]}'
                        else:
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    ...
                    _now = time.time()  # FIX A-CAD-01: atomic timestamp
                    with self.ipc.stats_lock:
                        self.last_trade_time = _now
                        self.engine.last_trade_time = _now

                    # 🔧 FIX: Increment daily trade counter  # FIX C-CAD-03
                    with self.ipc.stats_lock:
                    ...
                    try:
                        ...
                        state_key = self.engine.rl_agent.discretize_state(
                            ...
                            atr=genome.get('atr', 0.0010),  # 10 pips default for USDCAD
                            volatility=genome.get('volatility', 0.0010),  # 10 pips default
                    ...
                    # 🔧 FIX: Rollback position increment since trade didn't execute  # FIX C-CAD-02
                    if _did_increment:
                        with self.ipc.stats_lock:
    ...
    def run(self):
        ...
        start_time = time.time()
        last_position_reconcile = time.time()  # FIX A-CAD-04
        ...
        try:
            while self.running:
                ...
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)  # 🔧 FIX CAT-135: guard contra startup falso alarma
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        _tmp_path = 'quantum_stats.json.tmp'  # FIX M-CAD-02: atomic write
                ...
                # FIX A-CAD-04: POSITION RECONCILE every 60s — reset stale open_positions
    ...
    def _update_dashboard_from_genome(self, genome):
        ...
        try:
            # Extract metadata
            metadata = genome.get('metadata', {})
            symbol = metadata.get('symbol', 'USDCAD')
...
def _acquire_cad_singleton():
    """Prevent multiple instances using PID file"""
    import os as _os2, atexit as _atexit2
    _pid_file = _os2.path.join(_os2.path.dirname(__file__), 'cadquantum_core.pid')
    if _os2.path.exists(_pid_file):
        try:
            ...
            if _ps2.pid_exists(_old_pid) and _ps2.Process(_old_pid).is_running():
                print(f'[SINGLETON] CAD instance already running (PID={_old_pid}). Exiting.')
                return False
        except Exception:
            pass
...
# ═══════════════════════════════════════════════════════════════════════════════════
# 🏆 QA AUDIT CERTIFICATE — cadquantum_core.py (USDCAD)
...
#
# M-CAD-01 │ NOVAMarketStateDetector singleton in __init__; use self.nova_detector
#           │ Eliminates per-genome instantiation overhead (was L15062)
#
# A-CAD-02  │ Consecutive-losses circuit breaker at top of process_genome
#           │ Halts trading for LOSS_HALT_DURATION_SECONDS after MAX_CONSECUTIVE_LOSSES
#
# A-CAD-03  │ Daily-loss circuit breaker at top of process_genome
#           │ Returns None early when today_pnl/balance <= -MAX_DAILY_LOSS_PERCENT
#
# C-CAD-01  │ ATR=0 circuit breaker; _atr_zero_count under stats_lock
#           │ Returns None after 5 consecutive ATR=0 ticks to avoid divide-by-zero
#
# C-CAD-02  │ _did_increment = False before position lock; True only on APPROVE
#           │ Guarded rollback: if _did_increment → rollback on Kraken failure
#
# A-CAD-05  │ dir_poison_until_{BUY|SELL} check in BOTH stacking-approve
#           │ and first-trade-approve branches inside stats_lock
#
# A-CAD-01  │ Atomic last_trade_time: _now = time.time(); stats_lock block
#           │ sets both self.last_trade_time and self.engine.last_trade_time atomically
#
# C-CAD-03  │ daily_trade_count += 1 wrapped in stats_lock (race condition fix)
#           │ Same race condition as AUD — ThreadPoolExecutor concurrent writes
#
# A-CAD-04  │ last_position_reconcile timer in run() — resets stale open_positions
#           │ (>600s without activity) every 60s
#
# M-CAD-02  │ Atomic quantum_stats.json write via .tmp + os.replace()
#           │ Prevents dashboard reading partial JSON during concurrent writes
#
# CAD-SPECIFIC NOTES:
#   - OILFANG MOMENTUM strategy (4th strategy, unique to USDCAD) preserved intact
#   - MAX_CONCURRENT_TRADES = 3 (not 1) — oil correlation stacking — preserved
#   - dir_poison fix is ESPECIALLY CRITICAL here (3 concurrent positions = 3x rollback risk)
#   - LLM_WEIGHTS comments still reference "AUD" (copy-paste artifact — cosmetic only)
#
# SYNTAX VALIDATION: python -m py_compile cadquantum_core.py → SYNTAX OK ✅
# ═══════════════════════════════════════════════════════════════════════════════════
