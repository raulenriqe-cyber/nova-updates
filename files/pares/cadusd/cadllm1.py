...
class LLM1Experto:
    ...
    def analyze(self, data):
        ...
        try:
            ...
            closes = data.get('closes', data.get('prices', data.get('bar_data', {}).get('closes', prices)))
            ...
            elif rsi > 60 and price_momentum > 0.0003:
                # RSI muy fuerte + buen momentum = BUY fuerte (0.0003 scaled for USDCAD ~1.38)
                rsi_signal = 1
                rsi_extrema_confidence = 0.6
            elif rsi < 40 and price_momentum < -0.0003:
                # RSI muy débil + momentum negativo = SELL fuerte (scaled for USDCAD)
                rsi_signal = -1
                rsi_extrema_confidence = 0.6
            else:
            ...
            if len(prices) >= 3:
                ...
                if macd_histogram > 0 and price_momentum_3 > 0:
                    ...
                    macd_signal = -1
                    macd_momentum_boost = -0.3
                elif macd_histogram > 0 and price_momentum_3 < -0.0002:
                    # MACD positivo pero precio cayendo = NO COMPRAR (divergencia, evitar) (scaled for USDCAD)
                    macd_signal = 0
                    macd_momentum_boost = 0
                elif macd_histogram < 0 and price_momentum_3 > 0.0002:
                    # MACD negativo pero precio subiendo = NO VENDER (divergencia, evitar) (scaled for USDCAD)
                    macd_signal = 0
                    macd_momentum_boost = 0
            ...
            # VALIDATION: Señal clara requerida - PHASE 10 RELAXED
            # net_signal range: -5.0 to +5.0 | Threshold: 0.10 (2%) - was 0.2 (4%)
            has_signals = net_signal > 0.10 or net_signal < -0.10  # PHASE 10: 2% threshold
            has_momentum = price_momentum > 0.00015 or price_momentum < -0.00015  # FIX: scaled for USDCAD (was 0.15 from XAUUSD)
            
            valid_setup = has_signals or has_momentum  # REQUIRE ONE (more responsive)
            
            # 🎯 PRECISION ENTRY: ADX filter - PHASE 10 RELAXED
            adx_trending = adx > 18  # PHASE 10: was 25
            ...
            if net_signal > 0.10 and valid_setup and rsi > 35:  # PHASE 10: RSI floor 35 (was 40)
                ...
                if adx_strong and price_momentum > 0.0003 and rsi > 50:
                    confidence = min(94, int(confidence * 1.20))  # Strong boost (scaled for USDCAD)
                elif adx_trending and price_momentum > 0.0002:
                    confidence = min(90, int(confidence * 1.10))  # Moderate boost (scaled for USDCAD)
                elif not adx_trending:  # ADX < 18 = ranging
                    confidence = max(45, int(confidence * 0.85))  # PHASE 10: Less penalty
            ...
            # SAFE: On error, return HOLD with 0% confidence
            ...
            # PHASE 10: Lowered from 2→1 for faster entries
            ...
            # 🧠 CRITICAL: ADX check - PHASE 10: Lowered from 22→15
...
{Y}|{R}                                                              {Y}|{R}
{Y}+{'-'*62}+{R}
{Y}|{R}  {W}USDCAD Scalping Edition{R}      {D}by Polarice Labs © 2026{R}       {Y}|{R}
...
def run_tcp_server():
    ...
    # Cargar puerto desde cadconfig.yaml
    try:
        with open('cadconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm1_port', 9155)
    except:
        port = 9155  # Fallback
