...
║  Port: 9157 (TCP Server) - NEVER FAILS, ALWAYS RESPONDS                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class MonteCarloSimulator:
    ...
    @staticmethod
    def simulate_price_paths(current_price, atr, periods=10, simulations=500, recent_prices=None):
        ...
        # Derive drift from recent returns (momentum bias)
        drift = 0.0
        if recent_prices is not None and len(recent_prices) >= 5:
            recent = np.array(recent_prices[-20:], dtype=float)
            recent_returns = np.diff(recent) / recent[:-1]
            drift = float(np.mean(recent_returns))
            # Clamp drift to avoid extreme bias
            drift = max(-daily_volatility, min(daily_volatility, drift))
...
class LLM2TechnicalExpert:
    ...
    def __init__(self):
        ...
        self.memory = {'USDCAD': deque(maxlen=300)}
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # ============================================================
            mc_sim = MonteCarloSimulator.simulate_price_paths(price, atr, periods=10, simulations=500, recent_prices=prices)
            ...
            # Consolidation check - M1 USDCAD: ATR very low = reduced confidence
            # CHANGE: Don't return early, just reduce confidence
            # FOREX: ATR in price units (0.0005 = 5 pips, 0.0010 = 10 pips)
            consolidation_penalty = 0
            if atr < 0.0005:
                consolidation_penalty = 5  # Reduce confidence by 5 if extreme consolidation (<5 pips)
            elif atr < 0.0010:
                consolidation_penalty = 3  # Reduce by 3 if tight consolidation (<10 pips)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🎯 PHASE 10 RELAXED: BEARISH ENTRY SETUP
            # ═══════════════════════════════════════════════════════════════
            elif bear_signals >= 1.5 and technical_score < -0.20:  # PHASE 10: was 2.2/-0.32
                # Check for momentum confirmation
                momentum_confirmed = (rsi < 58 and rsi > 32 and price_momentum_5 < -0.05)  # PHASE 10: Relaxed
                ...
                adx_confirmed = adx > 20  # PHASE 10: was 25
                ...
                # 🎯 PHASE 10: Require 2/4 confirmations (was 3/4)
                if confirmations >= 2:
                    decision = 'SELL'
                    ...
                    decision = 'HOLD'
                    confidence = 50.0
                    reason = f'Bearish signals ({bear_signals:.1f}) but weak confirmation [{confirmations}/4]'
            
            # ═════════════════════════════════════════════════════════════
            # 🚫 PHASE 10: Produce weak directional votes instead of HOLD
            # ═════════════════════════════════════════════════════════════
            else:
                if bull_signals > bear_signals and bull_signals >= 0.8:
                    decision = 'BUY'
                    confidence = 35.0 + (bull_signals * 8.0)
                    confidence = min(55.0, confidence)
                    reason = f'Weak bullish: Bull={bull_signals:.1f} Bear={bear_signals:.1f} Score={technical_score:.2f}'
                elif bear_signals > bull_signals and bear_signals >= 0.8:
                    decision = 'SELL'
                    confidence = 35.0 + (bear_signals * 8.0)
                    confidence = min(55.0, confidence)
                    reason = f'Weak bearish: Bull={bull_signals:.1f} Bear={bear_signals:.1f} Score={technical_score:.2f}'
                else:
                    decision = 'HOLD'
                    confidence = 40.0 + (signal_strength * 20.0)
                    confidence = max(30.0, confidence)
                    reason = f'No signal: Bull={bull_signals:.1f} Bear={bear_signals:.1f} Score={technical_score:.2f}'
...
def start_server():
    """Inicia servidor TCP en puerto 9157"""
    try:
        config_file = 'cadconfig.yaml'
        try:
        ...
        port = config.get('llm_settings', {}).get('llm2_port', 9157)
