...
class LLM5ChartAnalyzer:
    ...
    def analyze(self, symbol='USDCAD', timeframe='M1') -> Dict[str, Any]:
        ...
        if signal_diff >= 0.15:  # PHASE 10: was 0.3 - relaxed for CAD
        ...
        if trend_direction == 'DOWNTREND' and trend_strength > 0.6:  # PHASE 10: was 0.4
            if decision == 'BUY':
                logger.warning(f"[LLM5] ⚠️ BLOCKED BUY in strong DOWNTREND! Forcing HOLD")
                decision = 'HOLD'
                pattern_confidence = 35.0
        elif trend_direction == 'UPTREND' and trend_strength > 0.6:  # PHASE 10: was 0.4
            if decision == 'SELL':
        ...
        if decision in ('BUY', 'SELL'):
            if adx_strong:
                ...
                # Ranging market: reduce confidence less aggressively
                pattern_confidence = pattern_confidence * 0.90  # PHASE 10: was 0.82
...
class LLM5Client:
    ...
    @staticmethod
    def analyze(symbol='USDCAD', timeframe='M1', candles=None):
