...
║  Ports: 9160 (USDCAD) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=9160, symbol='USDCAD'):
        self.port = port  # USDCAD LLM10 SENTINEL
        self.symbol = symbol
    ...
    def _audit_logic_consistency(self, genome, trinity_decision, llm_responses):
        ...
        score = 10  # FIX: Start at neutral midpoint (same as XAUUSD)
        alerts = []
    ...
    def _audit_context_awareness(self, genome):
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # USDCAD tends to fall on high VIX (flight to USD)
            else:
                score += 0
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # meaning quality=22 gave mult=0.22 (78% penalty!) - mathematically wrong.
            # Fixed: sqrt() mapping with floor at 0.50 (same as XAUUSD):
            ...
            import math
            multiplier = max(0.50, math.sqrt(quality_score / 100.0))
            ...
            result = {
                ...
                'audit_score': quality_score,  # Alias for Trinity compatibility
                'quality_breakdown': {
                ...
                'quality_multiplier': round(multiplier, 3),  # Alias for Trinity compatibility
            ...
            return {
                'decision': 'AUDIT',
                'quality_score': 50,
                'audit_score': 50,  # Alias for Trinity compatibility
                ...
                'categories': ['ERROR'],
                'multiplier': 0.5,
                'quality_multiplier': 0.5,  # Alias for Trinity compatibility
                'timestamp': time.time()
            }
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para USDCAD: correlación con DXY
            if symbol == 'USDCAD':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para USDCAD
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for USDCAD - caution on long trades'
                        })
    ...
    def run_server(self):
        ...
        server.listen(1)
        log.info(f"[NOVA] Listening on port {self.port}")
...
if __name__ == '__main__':
    ...
    # USDCAD LLM10 SENTINEL uses port 9160
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 9160
    symbol = 'USDCAD'
