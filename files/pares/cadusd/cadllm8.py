...
║  Port: 9164 (TCP Server) | Pure Python | Low Latency                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class CHRONOS:
    ...
    def __init__(self):
        self.port = 9164  # USDCAD LLM8 CHRONOS
        self.history = deque(maxlen=100)
        log.info("[CHRONOS] \u2728 Timing Optimizer initialized (Port 9164 USDCAD)")
    ...
    def _calculate_timing_score(self, data):
        ...
        try:
            ...
            adx = float(ind.get('adx', 20))
            macd = float(ind.get('macd_histogram', ind.get('macd', 0)))
            prices = data.get('prices', data.get('bar_data', {}).get('closes', []))
            closes = prices
