"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - CNHUSD Market Sentiment                         ║
...
║  Optimizado para CNHUSD - Chinese Yuan / US Dollar                      ║
║  🐉 PBOC-Controlled Exotic | PBOC + Capital Flows + DXY Focus               ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-CNH")
...
# Símbolos a monitorear - CNHUSD FOCUSED + correlated pairs
SYMBOLS = ["CNHUSD", "USDCNY", "XAUUSD"]

# Fuentes confiables para forex + China/Asia sources
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "forexlive", "fxstreet", "dailyfx",
    "investing.com", "tradingview", "marketwatch", "cnbc",
    "financialjuice", "forexfactory", "ft.com",
    "scmp.com", "caixin.com", "globaltimes.cn", "chinadaily.com.cn",
    "pbc.gov.cn", "safe.gov.cn", "nikkei.com", "asia.nikkei.com",
    "mining.com", "kitco.com", "goldprice.org"
]

# Keywords por símbolo - CNHUSD OPTIMIZED for PBOC-controlled exotic
KEYWORDS = {
    "CNHUSD": [
        "CNHUSD", "yuan dollar", "CNH/USD", "chinese yuan news",
        "PBOC decision", "peoples bank china", "yuan forecast",
        "chinese yuan", "fed yuan", "yuan strength",
        "yuan trading", "yuan weakness", "china economy",
        "china inflation", "china employment", "china gdp",
        "china retail sales", "china CPI", "china trade balance",
        "PBOC rate decision", "PBOC fix rate", "yuan fixing",
        "PBOC intervention", "yuan devaluation", "capital outflows china",
        "china PMI", "china manufacturing", "china stimulus",
        "china property crisis", "beijing economic policy",
        "USD CNH", "USDCNH", "offshore yuan", "onshore yuan",
        "yuan band", "yuan depreciation", "yuan appreciation",
        "china bond yield", "asian session forex",
        "CNHUSD rally", "CNHUSD crash", "yuan forex", "DXY dollar index"
    ],
    "USDCNY": [
        "USDCNY", "onshore yuan", "USD/CNY", "china onshore rate",
        "PBOC daily fix", "yuan fixing rate", "china forex reserves",
        "yuan midpoint", "yuan reference rate", "china capital controls"
    ],
    "XAUUSD": [
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        Context-aware: CNHUSD is a commodity/carry currency.
        ...
        symbol = news_item.get('symbol', 'CNHUSD')
...
Context: CNHUSD is a commodity/carry currency. Chinese Yuan strengthens with:
...
class NewsStorage:
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        CNHUSD focused with cross-asset correlation (NZDUSD, XAUUSD).
        """
        data = self.load_today()
        ...
        # CNHUSD-specific confidence calculation
        cnh_score = abs(sentiment.get('CNHUSD', {}).get('score', 0))
        cnh_total = sentiment.get('CNHUSD', {}).get('total', 0)
        cnh_conf = min(1.0, cnh_score * (1 + min(cnh_total, 10) * 0.1))
        
        # Boost confidence if correlated assets (Gold, onshore CNY) agree with CNH direction
        cnh_sent = sentiment.get('CNHUSD', {}).get('sentiment', 'N')
        gold_sent = sentiment.get('XAUUSD', {}).get('sentiment', 'N')
        cny_sent = sentiment.get('USDCNY', {}).get('sentiment', 'N')
        
        correlation_boost = 0
        if cnh_sent == gold_sent and cnh_sent != 'N':
            correlation_boost += 0.05  # Gold confirms CNH safe-haven flow
        if cnh_sent == cny_sent and cnh_sent != 'N':
            correlation_boost += 0.08  # Onshore CNY confirms offshore CNH direction
        
        cnh_conf = min(1.0, cnh_conf + correlation_boost)
        
        return {
            'CNHUSD': sentiment.get('CNHUSD', {}).get('sentiment', 'N'),
            'USDCNY': sentiment.get('USDCNY', {}).get('sentiment', 'N'),
            'XAUUSD': sentiment.get('XAUUSD', {}).get('sentiment', 'N'),
            'summary': ' | '.join(summaries),
            'confidence': cnh_conf,
            'confidence_CNHUSD': cnh_conf,
            'raw': sentiment
        }
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para CNHUSD.
    ...
    def fetch_and_evaluate(self) -> Dict:
        """
        Flujo completo:
        1. Busca noticias en DuckDuckGo (CNHUSD, NZDUSD, XAUUSD)
        ...
        """
        logger.info("=" * 60)
        logger.info("🦘 CNHUSD NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        ...
        CNHUSD-aware: uses cross-asset correlation for higher confidence.
        """
        sentiment = self.storage.get_sentiment_for_llm()
        s = sentiment.get(symbol, 'N')
        
        if symbol == 'CNHUSD':
            conf = sentiment.get('confidence_CNHUSD', sentiment.get('confidence', 0.5))
        else:
...
def main():
    ...
    print("=" * 70)
    print("🐉 NEWS INTELLIGENCE ENGINE - CNHUSD")
    print("   PBOC-Controlled Exotic | PBOC + Capital Flows + DXY")
    ...
    print("=" * 70)
    print(f"   🐉 CNHUSD: {sentiment.get('CNHUSD', 'N')}")
    print(f"   🏦 USDCNY: {sentiment.get('USDCNY', 'N')}")
    print(f"   🥇 XAUUSD: {sentiment.get('XAUUSD', 'N')}")
    print(f"   Summary: {sentiment.get('summary', 'N/A')}")
    print(f"   Confidence: {sentiment.get('confidence_CNHUSD', 0):.2f}")
    print("=" * 70)
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('CNHUSD')
