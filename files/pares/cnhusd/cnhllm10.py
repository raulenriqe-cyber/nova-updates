...
║  Ports: 8760 (CNHUSD) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=8760, symbol='CNHUSD'):
        self.port = port  # CNHUSD LLM10 SENTINEL
        self.symbol = symbol
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # CNHUSD tends to fall on high VIX (flight to USD)
            else:
                score += 0
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para CNHUSD: correlación con DXY
            if symbol == 'CNHUSD':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para CNHUSD
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for CNHUSD - caution on long trades'
                        })
...
if __name__ == '__main__':
    ...
    # CNHUSD LLM10 SENTINEL uses port 8760
