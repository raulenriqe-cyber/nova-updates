...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║     🐉 NOVA RED DRAGON - QUANTUM CORE v1.1 CNHUSD MEAN REVERSION             ║
...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for CNHUSD')
...
# ⚔️ EXCALIBUR KNIGHT STRATEGY (News-Driven Breakout Elite)
try:
...
# 🏦 LANCE FORGER SOVEREIGN STRATEGY (Carry Reversal Micro-Trading)
try:
    from lance_forger_strategy import LanceForgerSovereign, get_lance_forger
    LANCE_FORGER_AVAILABLE = True
except ImportError:
    LANCE_FORGER_AVAILABLE = False
    LanceForgerSovereign = None
    get_lance_forger = None
    print("[WARNING] Lance Forger Sovereign Strategy not available")
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        # 🎯 CNHUSD CALIBRATED - Par calmado requiere setups de ALTA calidad
        # CNHUSD tiene ~40-60% menos volatilidad que GBP → cada pip cuenta más
        self.min_perfect_score = 3.5   # PERFECT - solo setups premium (GBP=2.5)
        self.min_good_score = 2.8      # GOOD - calidad probada (GBP=2.0)
        self.min_wait_score = 1.8      # WAIT - mínimo viable (GBP=1.0)
        self.min_sweep_score = 2.8     # SWEEP - smart money confirmado (GBP=2.0)
        self.lock = Lock()
        ...
        # NOTE: LLM6 runs as TCP server on port 8762 (CNHUSD 87xx range)
        self.llm6_port = 8762  # LLM6 NOVA Smart Money Oracle port (CNHUSD)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        """Query LLM6 NOVA Smart Money Oracle via TCP (port 8762)"""
        try:
            ...
            current_price = float(genome.get('tick_data', {}).get('last', genome.get('price', 7.2500)))  # 🔧 FOREX: USDCNH ~7.25
            if len(closes) < 3:
            ...
            request = {
                ...
                'symbol': genome.get('symbol', 'CNHUSD'),
            }
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        atr = indicators.get('atr', 0.0100)  # 100 pips default for USDCNH forex
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('CNHUSD')
                    if news_conf > 60:
        ...
        # ═══════════════════════════════════════════════════════════════
        # 10. SESSION TIMING - Horas de mejor probabilidad para CNHUSD
        # Sydney/Tokyo = máxima liquidez para AUD pairs
        # London/NY overlap = buena liquidez pero no óptima para AUD
        # ═══════════════════════════════════════════════════════════════
        ...
        # Sydney/Tokyo session (22:00-07:00 UTC) = MEJOR momento para CNHUSD
        if 22 <= hour_utc or hour_utc <= 6:
            likelihood_modifiers.append(1.15)
            evidence.append("⏰ SYDNEY/TOKYO SESSION - Máxima liquidez AUD (+15%)")
        # London/NY overlap (13:00-16:00 UTC) = Buena liquidez general
        elif 13 <= hour_utc <= 16:
            likelihood_modifiers.append(1.08)
            evidence.append("⏰ LONDON/NY OVERLAP - Buena liquidez (+8%)")
        # London early (08:00-12:00 UTC) = AUD decreciendo
        elif 8 <= hour_utc <= 12:
            likelihood_modifiers.append(0.95)
            warnings.append("⏰ London solo - AUD liquidez decreciente (-5%)")
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('CNHUSD')
    ...
    def calculate_confluence_score(self, 
        ...
        rsi = indicators.get('rsi', 50)
        adx = indicators.get('adx', 25)
        atr = indicators.get('atr', 0.0100)  # Default 100 pips for USDCNH forex
        macd = indicators.get('macd', 0)
        macd_signal = indicators.get('macd_signal', 0)
        ...
        # ATR Goldilocks zone: 0.0050-0.0200 (50-200 pips) para USDCNH scalping
        if 0.0050 <= atr <= 0.0200:
            volatility_score += 0.5
            intelligence_notes.append(f"✅ ATR óptimo para scalping: {atr:.5f}")
        elif 0.0030 <= atr < 0.0050:
            volatility_score += 0.3  # Baja vol pero tradeable
        elif 0.0200 < atr <= 0.0350:
            volatility_score += 0.3  # Alta vol pero tradeable
        elif atr > 0.0350:
            volatility_score += 0.1  # Muy volátil - arriesgado
            intelligence_notes.append(f"⚠️ ATR muy alto ({atr:.5f}) - usar stops amplios")
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For CNHUSD at ~1.25, 0.5% = 0.00625 which is reasonable for fib zone
        ...
        if 22 <= hour_utc or hour_utc <= 6:
            timing_score += 0.5
            intelligence_notes.append("⏰ SYDNEY/TOKYO - Máxima liquidez CNHUSD")
        elif 13 <= hour_utc <= 16:
            timing_score += 0.35  # London/NY overlap - good but not prime for AUD
        elif 7 <= hour_utc <= 12:
            timing_score += 0.25  # London solo - moderate for AUD
        elif 17 <= hour_utc <= 21:
            timing_score += 0.15  # NY solo - low AUD activity
        ...
        # Estimar R:R basado en ATR, distancia a S/R, y confluencia factores
        # FIXED: Use dynamic R:R based on actual TP/SL from strategy, not constant ATR ratio
        strategy_tp_mult = genome.get('tp_multiplier', 2.0)
        strategy_sl_mult = genome.get('sl_multiplier', 1.0)  
        potential_reward = atr * strategy_tp_mult
        potential_risk = atr * strategy_sl_mult
        
        # Adjust R:R based on S/R proximity (closer to S/R = better R:R for reversals)
        if sweep_info and sweep_info.get('level', 0) and current_price:
            sr_level = sweep_info['level']
            sr_distance = abs(current_price - sr_level)
            if sr_distance < atr * 0.5:  # Very close to S/R
                if trinity_decision == 'BUY' and current_price > sr_level:
                    potential_reward *= 1.3  # Bouncing off support
                elif trinity_decision == 'SELL' and current_price < sr_level:
                    potential_reward *= 1.3  # Bouncing off resistance
        
        if potential_risk > 0:
            rr_ratio = potential_reward / potential_risk
            if rr_ratio >= 2.5:
                rr_score += 0.25
                intelligence_notes.append(f"💰 R:R excelente: {rr_ratio:.1f}:1")
            elif rr_ratio >= 2.0:
                rr_score += 0.20
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 🚀 NEW SUPER INTELLIGENCE: CONSUME EA DATA UPGRADES
        # ═══════════════════════════════════════════════════════════════════
        
        # ═══════════════════════════════════════════════════════════════════
        # 18. MTF ALIGNMENT INTELLIGENCE (0-2.0) - Multi-Timeframe Confluence
        # ═══════════════════════════════════════════════════════════════════
        mtf_score = 0.0
        mtf_data = genome.get('mtf_analysis', {})
        if mtf_data:
            try:
                alignment_score = mtf_data.get('alignment_score', 0)  # 0-100%
                bias = mtf_data.get('bias', 'MIXED')
                h1_data = mtf_data.get('h1', {})
                h4_data = mtf_data.get('h4', {})
                
                # HIGH alignment bonus (3+ timeframes agree)
                if alignment_score >= 75:
                    mtf_score += 1.5
                    intelligence_notes.append(f"🎯 MTF SUPER ALINEADO ({alignment_score}%) - triple confirmación")
                elif alignment_score >= 60:
                    mtf_score += 1.0
                    intelligence_notes.append(f"🎯 MTF alineado ({alignment_score}%)")
                elif alignment_score >= 40:
                    mtf_score += 0.5
                
                # Bias confirmation
                if (trinity_decision == 'BUY' and bias == 'BULLISH') or \
                   (trinity_decision == 'SELL' and bias == 'BEARISH'):
                    mtf_score += 0.5
                    intelligence_notes.append(f"🎯 MTF bias {bias} confirma {trinity_decision}")
                elif bias == 'MIXED':
                    mtf_score -= 0.2  # Mixed = uncertainty penalty
                
            except Exception as e:
                pass  # MTF data optional
        
        mtf_score = max(0, min(2.0, mtf_score))
        breakdown['18_mtf_alignment'] = round(mtf_score, 2)
        total_score += mtf_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 19. GOLD CORRELATION INTELLIGENCE (0-1.5) - XAU/AUD Relationship
        # ═══════════════════════════════════════════════════════════════════
        gold_score = 0.0
        gold_data = genome.get('gold_correlation', {})
        if gold_data:
            try:
                gold_trend = gold_data.get('trend', 'NEUTRAL')
                supports_aud_buy = gold_data.get('supports_aud_buy', False)
                supports_aud_sell = gold_data.get('supports_aud_sell', False)
                aud_gold_aligned = gold_data.get('aud_gold_aligned', False)
                
                # Perfect correlation = massive bonus
                if aud_gold_aligned:
                    gold_score += 1.0
                    intelligence_notes.append("🌟 AUD/Gold PERFECTA correlación")
                
                # Directional support
                if (trinity_decision == 'BUY' and supports_aud_buy) or \
                   (trinity_decision == 'SELL' and supports_aud_sell):
                    gold_score += 0.4
                    intelligence_notes.append(f"🥇 Gold soporta {trinity_decision}")
                elif (trinity_decision == 'BUY' and supports_aud_sell) or \
                     (trinity_decision == 'SELL' and supports_aud_buy):
                    gold_score -= 0.3  # Gold CONTRADICE = penalty
                    intelligence_notes.append(f"🥇 Gold CONTRA {trinity_decision}")
                
            except Exception as e:
                pass
        
        gold_score = max(-0.5, min(1.5, gold_score))
        breakdown['19_gold_correlation'] = round(gold_score, 2)
        total_score += gold_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 20. ADVANCED INDICATORS (0-1.5) - Ichimoku + MACD + CCI + Williams
        # ═══════════════════════════════════════════════════════════════════
        advanced_score = 0.0
        advanced_data = genome.get('advanced_indicators', {})
        if advanced_data:
            try:
                # Ichimoku Cloud Intelligence
                ichimoku = advanced_data.get('ichimoku', {})
                if ichimoku:
                    cloud_signal = ichimoku.get('cloud_signal', 'NEUTRAL')
                    bull_strength = ichimoku.get('bull_strength', 0)  # 0-5
                    bear_strength = ichimoku.get('bear_strength', 0)  # 0-5
                    
                    if (trinity_decision == 'BUY' and cloud_signal == 'BULLISH' and bull_strength >= 3) or \
                       (trinity_decision == 'SELL' and cloud_signal == 'BEARISH' and bear_strength >= 3):
                        advanced_score += 0.6
                        intelligence_notes.append(f"☁️ Ichimoku {cloud_signal} confirma {trinity_decision}")
                
                # MACD Histogram Power
                macd_data = advanced_data.get('macd', {})
                if macd_data:
                    histogram = macd_data.get('histogram', 0)
                    if (trinity_decision == 'BUY' and histogram > 0) or \
                       (trinity_decision == 'SELL' and histogram < 0):
                        advanced_score += 0.3
                
                # CCI Extremes
                cci = advanced_data.get('cci', {}).get('value', 0)
                if (trinity_decision == 'BUY' and cci < -100) or \
                   (trinity_decision == 'SELL' and cci > 100):
                    advanced_score += 0.3
                    intelligence_notes.append(f"📊 CCI extremo confirma {trinity_decision}")
                
                # Williams %R
                williams = advanced_data.get('williams_r', 0)
                if (trinity_decision == 'BUY' and williams < -80) or \
                   (trinity_decision == 'SELL' and williams > -20):
                    advanced_score += 0.3
                
            except Exception as e:
                pass
        
        advanced_score = max(0, min(1.5, advanced_score))
        breakdown['20_advanced_indicators'] = round(advanced_score, 2)
        total_score += advanced_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 21. VOLATILITY REGIME INTELLIGENCE (0-1.0) - Enhanced Volatility
        # ═══════════════════════════════════════════════════════════════════
        vol_regime_score = 0.0
        vol_regime_data = genome.get('volatility_regime', {})
        if isinstance(vol_regime_data, dict):  # New detailed format
            try:
                regime = vol_regime_data.get('regime', 'NORMAL')
                atr_zscore = vol_regime_data.get('atr_zscore', 0.0)
                bb_squeeze = vol_regime_data.get('bollinger_squeeze_pct', 50.0)
                
                # Volatility context scoring
                if regime == 'SQUEEZE' and bb_squeeze < 20:
                    vol_regime_score += 0.8  # Compression = big move coming
                    intelligence_notes.append("🌪️ SQUEEZE extremo - explosión inminente")
                elif regime == 'EXPANSION' and trinity_confidence >= 75:
                    vol_regime_score += 0.6  # High confidence + expansion = good
                    intelligence_notes.append("🌪️ EXPANSION + alta confianza")
                elif regime == 'ELEVATED' and trinity_confidence >= 80:
                    vol_regime_score += 0.4  # Need very high confidence in volatility
                elif regime == 'NORMAL':
                    vol_regime_score += 0.2  # Normal conditions = slight bonus
                
            except Exception as e:
                pass
        elif isinstance(vol_regime_data, str):  # Old simple format
            regime = vol_regime_data
            if regime == 'LOW':
                vol_regime_score += 0.3
            elif regime == 'NORMAL':
                vol_regime_score += 0.2
        
        vol_regime_score = max(0, min(1.0, vol_regime_score))
        breakdown['21_volatility_regime'] = round(vol_regime_score, 2)
        total_score += vol_regime_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 22. CANDLE PATTERNS (0-1.0) - Multi-Pattern Recognition
        # ═══════════════════════════════════════════════════════════════════
        pattern_score = 0.0
        pattern_data = genome.get('candle_patterns', {})
        if pattern_data:
            try:
                pattern_signal = pattern_data.get('pattern_signal', 0)  # -100 to +100
                patterns_detected = []
                
                # Check individual patterns
                for pattern_name in ['three_white_soldiers', 'three_black_crows', 
                                   'morning_star', 'evening_star', 'inside_bar', 'outside_bar']:
                    if pattern_data.get(pattern_name, False):
                        patterns_detected.append(pattern_name)
                
                # Strong bearish patterns for SELL
                if trinity_decision == 'SELL':
                    if pattern_data.get('three_black_crows', False) or \
                       pattern_data.get('evening_star', False):
                        pattern_score += 0.6
                        intelligence_notes.append(f"🕯️ Patrón bajista confirma SELL")
                    elif pattern_signal < -30:  # Strong bearish signal
                        pattern_score += 0.4
                
                # Strong bullish patterns for BUY
                elif trinity_decision == 'BUY':
                    if pattern_data.get('three_white_soldiers', False) or \
                       pattern_data.get('morning_star', False):
                        pattern_score += 0.6
                        intelligence_notes.append(f"🕯️ Patrón alcista confirma BUY")
                    elif pattern_signal > 30:  # Strong bullish signal
                        pattern_score += 0.4
                
                # Inside/Outside bar context
                if pattern_data.get('inside_bar', False):
                    pattern_score += 0.2  # Compression = likely breakout
                if pattern_data.get('outside_bar', False):
                    pattern_score += 0.3  # Expansion = volatility
                
            except Exception as e:
                pass
        
        pattern_score = max(0, min(1.0, pattern_score))
        breakdown['22_candle_patterns'] = round(pattern_score, 2)
        total_score += pattern_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 23. ORDER FLOW ANALYSIS (0-1.5) - Enhanced Smart Money Detection
        # ═══════════════════════════════════════════════════════════════════
        flow_score = 0.0
        flow_data = genome.get('order_flow_analysis', {})
        if flow_data:
            try:
                buy_volume = flow_data.get('buy_volume_total', 0)
                sell_volume = flow_data.get('sell_volume_total', 0)
                imbalance_pct = flow_data.get('imbalance_pct', 0)
                whale_detected = flow_data.get('whale_detected', False)
                delta_divergence = flow_data.get('delta_divergence', False)
                
                # Volume imbalance confirmation
                if trinity_decision == 'BUY' and imbalance_pct > 20:  # Buy pressure
                    flow_score += 0.5
                    intelligence_notes.append(f"💹 BUY flow dominante ({imbalance_pct:.0f}%)")
                elif trinity_decision == 'SELL' and imbalance_pct < -20:  # Sell pressure  
                    flow_score += 0.5
                    intelligence_notes.append(f"💹 SELL flow dominante ({imbalance_pct:.0f}%)")
                
                # Whale activity
                if whale_detected:
                    flow_score += 0.4
                    intelligence_notes.append("🐋 WHALE activity detected")
                
                # Delta divergence = strong signal
                if delta_divergence:
                    flow_score += 0.6
                    intelligence_notes.append("💹 Delta divergence - momentum shift")
                
            except Exception as e:
                pass
        
        flow_score = max(0, min(1.5, flow_score))
        breakdown['23_order_flow'] = round(flow_score, 2)
        total_score += flow_score
        
        # ═══════════════════════════════════════════════════════════════════
        # 24. SUPPORT/RESISTANCE INTELLIGENCE (0-1.0) - Enhanced S/R Analysis  
        # ═══════════════════════════════════════════════════════════════════
        sr_score = 0.0
        sr_data = genome.get('support_resistance', {})
        if sr_data:
            try:
                distance_to_support = sr_data.get('distance_to_support', 999)
                distance_to_resistance = sr_data.get('distance_to_resistance', 999)
                sr_position = sr_data.get('sr_position_pct', 50)  # 0-100% in range
                
                # Perfect S/R alignment
                if trinity_decision == 'BUY' and distance_to_support <= 5:  # Within 5 pips of support
                    sr_score += 0.6
                    intelligence_notes.append(f"🎯 BUY at strong support ({distance_to_support:.1f} pips)")
                elif trinity_decision == 'SELL' and distance_to_resistance <= 5:  # Within 5 pips of resistance  
                    sr_score += 0.6
                    intelligence_notes.append(f"🎯 SELL at strong resistance ({distance_to_resistance:.1f} pips)")
                
                # Position in S/R range context
                if trinity_decision == 'BUY' and sr_position < 25:  # Bottom of range
                    sr_score += 0.3
                elif trinity_decision == 'SELL' and sr_position > 75:  # Top of range
                    sr_score += 0.3
                
            except Exception as e:
                pass
        
        sr_score = max(0, min(1.0, sr_score))
        breakdown['24_support_resistance'] = round(sr_score, 2)
        total_score += sr_score
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # 🔧 REBALANCED: 90% + calc>=4.0 + llm>=1.5 (was 93/5.0/1.8 = too strict)
            # ═══════════════════════════════════════════════════════════════
            if trinity_confidence >= 90 and calc_entry_score >= 4.0 and not sweep_trap_detected:
                llm_wisdom = breakdown.get('2_llm_wisdom', 0)
                if llm_wisdom >= 1.5:  # 🔧 REBALANCED: Restored from 1.8 to 1.5
            ...
            # Si ML/DNN predice la DIRECCIÓN OPUESTA, NO atacar
            # Esto previene BUY cuando el modelo predice baja y viceversa
            # ═══════════════════════════════════════════════════════════════
            ml_prediction = genome.get('ml_prediction', 'NEUTRAL')
            ...
            # ═══════════════════════════════════════════════════════════════
            # 📈 SMART TREND VETO (INTELLIGENT - JPY-CALIBRATED)
            # Bloquea trades contra tendencia EXCEPTO si:
            # 1. Confianza altísima (>78%)
            # 2. MAs convergiendo
            # 3. RSI extremo sugiere reversión
            # Mejor hora para CNHUSD: sesión ASIÁTICA (00:00-08:00 UTC)
            # ═══════════════════════════════════════════════════════════════
            ma5 = indicators.get('ma5', indicators.get('ma_fast', 0))
            ma20 = indicators.get('ma20', indicators.get('ma_slow', 0))
            ma50 = indicators.get('ma50', 0)
            current_price = genome.get('current_price', indicators.get('close', 0))
            ...
            # Sesión ASIÁTICA (mejor para CNHUSD): 00:00-08:00 UTC (Tokyo/Sydney)
            # Sesión LONDON/NY overlap: 12:00-16:00 UTC
            high_liquidity_session = (0 <= current_hour_utc <= 8) or (12 <= current_hour_utc <= 16)
            
            if ma5 and ma20 and current_price:
                ma_diff_pct = ((ma5 - ma20) / ma20) * 100 if ma20 else 0
                ma_converging = abs(ma_diff_pct) < 0.05
                
                strong_uptrend = False
                strong_downtrend = False
                
                if ma50:
                    strong_uptrend = current_price > ma5 > ma20 > ma50
                    strong_downtrend = current_price < ma5 < ma20 < ma50
                else:
                    strong_uptrend = current_price > ma5 and ma5 > ma20 and ma_diff_pct > 0.08
                    strong_downtrend = current_price < ma5 and ma5 < ma20 and ma_diff_pct < -0.08
                
                if trinity_confidence >= 78:
                    allow_counter_trend = True
                    notes.append(f"✅ Alta confianza {trinity_confidence:.0f}% permite contra-tendencia")
                
                if ma_converging:
                    allow_counter_trend = True
                    notes.append(f"✅ MAs convergiendo ({ma_diff_pct:.3f}%) - cambio inminente")
                
                rsi = indicators.get('rsi', 50)
                if (trinity_decision == 'SELL' and rsi > 75) or (trinity_decision == 'BUY' and rsi < 25):
                    allow_counter_trend = True
                    notes.append(f"✅ RSI extremo ({rsi:.0f}) sugiere reversión")
                
                if not allow_counter_trend:
                    if trinity_decision == 'SELL' and strong_uptrend:
                        trend_veto = True
                        trend_reason = f"SELL contra tendencia ALCISTA sin justificación (conf={trinity_confidence:.0f}%)"
                    elif trinity_decision == 'BUY' and strong_downtrend:
                        trend_veto = True
                        trend_reason = f"BUY contra tendencia BAJISTA sin justificación (conf={trinity_confidence:.0f}%)"
                    elif trinity_decision == 'SELL' and ma_diff_pct > 0.12:
                        trend_veto = True
                        trend_reason = f"SELL contra MAs alcistas (MA5 +{ma_diff_pct:.2f}% sobre MA20)"
                    elif trinity_decision == 'BUY' and ma_diff_pct < -0.12:
                        trend_veto = True
                        trend_reason = f"BUY contra MAs bajistas (MA5 -{abs(ma_diff_pct):.2f}% bajo MA20)"
                
                logger.debug(f"📈 TREND: {'ALCISTA' if ma_diff_pct > 0 else 'BAJISTA'} | MA_diff={ma_diff_pct:.3f}% | Allow_CT={allow_counter_trend} | Session={'ASIA' if high_liquidity_session else 'LOW'}")
            
            if trend_veto:
                self._clear_pending(trinity_decision)
                logger.warning(f"📈 SMART TREND VETO: {trend_reason}")
                return False, 'HOLD', trinity_confidence * 0.3, \
                       f"📈 TREND VETO: {trend_reason}"
            ...
            # 🌐 SHARED BRAIN OPTION A: Cross-pair prior adjustment
            # Activo cuando hay datos (source_trades >= 3). Pasivo con DB vacío.
            if SHARED_BRAIN_AVAILABLE:
                try:
                    _cp_symbol = genome.get('metadata', {}).get('symbol', 'UNKNOWN')
                    _cp_prior = get_cross_pair_prior(_cp_symbol, trinity_decision)
                    if _cp_prior.get('source_trades', 0) >= 3:
                        _cp_adj = _cp_prior.get('prior_adjustment', 0.0)
                        _cp_boost = _cp_prior.get('confidence_boost', 0.0)
                        if _cp_adj != 0.0 or _cp_boost > 0.0:
                            _old_prob = consciousness_prob
                            consciousness_prob = min(0.99, max(0.01, consciousness_prob + _cp_adj + _cp_boost))
                            if _cp_prior.get('warning'):
                                warnings.append(f"🌐 {_cp_prior['warning']}")
                            _cw = _cp_prior.get('correlated_wins', 0)
                            _cl = _cp_prior.get('correlated_losses', 0)
                            _align = _cp_prior.get('regime_alignment', 0.0)
                            evidence.append(f"🌐 CROSS-PAIR: {_cw}W/{_cl}L | Adj={_cp_adj:+.2f} | Align={_align:.0%}")
                            logger.info(f"[SHARED BRAIN] 🌐 {_old_prob:.2%}→{consciousness_prob:.2%} adj={_cp_adj:+.2f} boost={_cp_boost:+.2f} trades={_cp_prior['source_trades']}")
                except Exception as _cp_err:
                    logger.debug(f"[SHARED BRAIN] Cross-pair prior error (non-critical): {_cp_err}")

            # 🌡️ SHARED BRAIN IDEA #2: Auto-detect market regime
            # 30s cache — zero DB overhead at tick frequency. Active when ≥2 pairs have fresh regime data.
            if SHARED_BRAIN_AVAILABLE:
                try:
                    _mr = get_market_regime(_cp_symbol, trinity_decision)
                    if _mr.get('confidence', 0) >= 0.3 and _mr.get('pairs_analyzed', 0) >= 2:
                        _mr_adj = _mr.get('consciousness_adjustment', 0.0)
                        if _mr_adj != 0.0:
                            _old_prob = consciousness_prob
                            consciousness_prob = min(0.99, max(0.01, consciousness_prob + _mr_adj))
                            logger.info(f"[SHARED BRAIN] 🌡️ {_old_prob:.2%}→{consciousness_prob:.2%} regime={_mr['dominant_regime']} trend={_mr['trend_direction']} adj={_mr_adj:+.2f}")
                        evidence.append(f"🌡️ REGIME: {_mr.get('dominant_regime','?')} | {_mr.get('trend_direction','?')} | {_mr.get('pairs_analyzed',0)}P | {_mr.get('confidence',0):.0%}")
                        if _mr.get('regime_warning'):
                            warnings.append(f"🌡️ {_mr['regime_warning']}")
                        if _mr.get('regime_changed', False):
                            logger.warning(f"[SHARED BRAIN] 🔄 Regime change detected: {_mr.get('regime_warning','')}")
                except Exception as _mr_err:
                    logger.debug(f"[SHARED BRAIN] Regime detection error (non-critical): {_mr_err}")
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 VETO POR PROBABILIDAD MUY BAJA - JPY-CALIBRATED
            # 🔧 CALIBRATED: 28% aligned with JPY (was 18% - too loose)
            # ═══════════════════════════════════════════════════════════════
            if consciousness_prob < 0.28:
                self._clear_pending(trinity_decision)
                return False, 'HOLD', trinity_confidence * 0.2, \
                       f"🧠 CONSCIOUSNESS VETO: Prob={consciousness_prob:.0%} < 28% | {warnings_str}"
            ...
            if len(closes) >= 5:
                ...
                # CNHUSD (~1.25) vs EURUSD (~1.08) - percentage based is symbol-agnostic
                current_close = closes[-1]
                pct_threshold = 0.03  # 0.03% movement threshold (works for all forex)
            ...
            # Esto hace el código compatible con CNHUSD, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            if len(closes) >= 3 and closes[-3] > 0:
                ...
                # Umbral: 0.08% = fuerte contra (percentage-based, works for all forex)
            ...
            if momentum_ok and consciousness_prob >= 0.22 and calc_entry_score >= 2.0:
                ...
                if (trinity_decision == 'BUY' and trend == 'UPTREND') or \
                   (trinity_decision == 'SELL' and trend == 'DOWNTREND'):
                    calc_entry_score += 1.0  # Moderate bonus (was 1.5 = too easy)
            ...
            if not llm6_response_received:
                ...
                # ─────────────────────────────────────────────────────────────
                # Using datetime imported at module level (line 178)
                ...
                for sup in support_levels[:3]:  # Check nearest 3 supports
                    ...
                    if sup_price > 0:
                        ...
                        if low < sup_price and close > sup_price:
                            ...
                            sweep_direction_fallback = 'BUY'
                            whale_signals += 2
                            whale_confidence_factors.append(f"SUPPORT_SWEEP_{sup_price:.5f}")
                            if trinity_decision == 'SELL':
                                is_trap_fallback = True
                                false_break_signals += 2
                            break
                ...
                for res in resistance_levels[:3]:  # Check nearest 3 resistances
                    ...
                    if res_price > 0:
                        ...
                        if high > res_price and close < res_price:
                            ...
                            sweep_direction_fallback = 'SELL'
                            whale_signals += 2
                            whale_confidence_factors.append(f"RESISTANCE_SWEEP_{res_price:.5f}")
                            if trinity_decision == 'BUY':
                                is_trap_fallback = True
                                false_break_signals += 2
                            break
            ...
            if false_break_prob > 50:
                ...
                llm6_reason = f"⚠️ Moderate false break risk ({false_break_prob:.0f}%)"
                logger.warning(f"⚠️ SOFT VETO: FalseBreak={false_break_prob:.0f}% - reducing confidence")
                ...
                if (trinity_decision == 'BUY' and sweep_direction == 'BUY') or \
                   (trinity_decision == 'SELL' and sweep_direction == 'SELL'):
                    ...
                    if is_trap:
                        ...
                        llm6_reason = f"⚠️ Sweep {sweep_direction} conflicts with {trinity_decision}"
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🧠 TIERS BASADOS EN CONCIENCIA - RECALIBRATED FOR NEW INTELLIGENCE
            # Combina Score + Consciousness Probability + Calc Entry Score
            # ⭐ CNHUSD: NEW Intelligence adds +9.5 potential score (24 dimensions total)
            # ⭐ OLD max ~10 → NEW max ~19.5. Thresholds adjusted accordingly.
            # ═══════════════════════════════════════════════════════════════
            ...
            # 🎯 TIER 0: SUPER INTELLIGENCE GOLD - NEW signals with multiple confirmations
            # NEW TIER: Use MTF + Gold + Advanced indicators for PREMIUM entries
            mtf_score = breakdown.get('18_mtf_alignment', 0)
            gold_score = breakdown.get('19_gold_correlation', 0)
            advanced_score = breakdown.get('20_advanced_indicators', 0)
            super_intelligence = mtf_score + gold_score + advanced_score
            
            if super_intelligence >= 3.0 and consciousness_prob >= 0.45 and calc_entry_score >= 2.5:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, min(95, trinity_confidence * 1.2), \
                       f"🚀💎 SUPER INTELLIGENCE ({super_intelligence:.1f} SI, {consciousness_prob:.0%} Prob, {score:.1f} Total)"
            
            # 🎯 TIER 0.5: CONSCIOUSNESS GOLD - High probability + decent score
            # Increased score requirement due to new intelligence potential
            if consciousness_prob >= 0.45 and momentum_ok and score >= 4.5 and calc_entry_score >= 2.5:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, min(95, trinity_confidence * 1.1), \
                       f"🧠💎 CONSCIOUSNESS GOLD (Prob={consciousness_prob:.0%}, Score={score:.1f}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 1: Multi-TF ALIGNMENT - 3+ TFs de acuerdo con fuerza alta
            ...
            # 🎯 TIER 2: ULTRA-PREMIUM - High score + prob (adjusted for new intelligence)
            if score >= 4.5 and momentum_ok and consciousness_prob >= 0.38 and calc_entry_score >= 2.8:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 3: ELITE - Good score + decent prob (adjusted thresholds)
            if score >= 3.5 and momentum_ok and consciousness_prob >= 0.32 and calc_entry_score >= 2.3:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 4: QUALITY - Decent score + minimum prob (legacy thresholds work)
            if score >= self.min_perfect_score and momentum_ok and consciousness_prob >= 0.28 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
                adjusted_conf = trinity_confidence * 0.9
                return True, trinity_decision, adjusted_conf, \
                       f"⭐⭐⭐ QUALITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 5: SMART MONEY - Sweep + prob confirma (legacy thresholds adjusted)
            if score >= self.min_sweep_score and momentum_ok and consciousness_prob >= 0.25 and calc_entry_score >= 1.5:
            ...
            # 🎯 TIER 6: TRINITY FORCE - Medium score with good confidence
            if trinity_confidence >= 50 and momentum_ok and score >= 2.5 and consciousness_prob >= 0.20 and calc_entry_score >= 1.2:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.85, \
                       f"⭐ TRINITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 7: HIGH CONFIDENCE OVERRIDE - Very high confidence with legacy score
            if trinity_confidence >= 58 and score >= self.min_good_score and consciousness_prob >= 0.18 and calc_entry_score >= 0.8:
                self._clear_pending(trinity_decision)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🎯 TIER 8: FALLBACK - Solo con Trinity MUY segura + score decente + probabilidad
            # CNHUSD es más lento, no atacar a ciegas como GBP scalping
            # Raised score requirement due to NEW intelligence potential
            # ═══════════════════════════════════════════════════════════════
            if trinity_confidence >= 58 and trinity_decision in ['BUY', 'SELL'] and score >= 2.3 and consciousness_prob >= 0.18:
                self._clear_pending(trinity_decision)
            ...
            # Log detallado de por qué no ataca
            block_reasons = []
...
class NOVAMarketStateDetector:
    ...
    - Macro correlations (DXY, GBP, VIX trending affecting this pair)
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Spread too wide (illiquidity) - USDCNH spread in price units (0.0001 = 1 pip)
        elif spread > 0.0080:  # For USDCNH, > 80 pips is abnormally wide
            self.alerts.append(f"⚠️ WIDE_SPREAD: {spread*10000:.1f} pips (liquidity crisis)")
            liquidity_score -= 25
        ...
        # We cannot access DXY/GBP/VIX from local data
...
class Config:
    """
    Configuration class for USDCNH trading.
    
    Active Strategy:
    - RED DRAGON ENGINE v1.1: Mean Reversion USD/CNH (Imperial Flow - Balanced Mode)
      H1 equilibrium context, M15 execution, M5 confirmation
      Win Rate: 70-76% | R:R 1:1.8-2.2 | 16-32 trades/month
    """
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 RED DRAGON ENGINE v1.1 - IMPERIAL FLOW (BALANCED)
    # ═══════════════════════════════════════════════════════════════════════
    ACTIVE_STRATEGY = "RED_DRAGON"
    
    # ═══════════════════════════════════════════════════════════════════════
    # NETWORK PORTS
    # ═══════════════════════════════════════════════════════════════════════
    HOST = '127.0.0.1'
    QUANTUM_PORT = 8754      # Injector - receive genomes from Quimera (CNHUSD)
    EXECUTOR_PORT = 8756     # Executor - send commands to Quimera (CNHUSD)
    TRINITY_PORT = 8766      # Trinity server port (CNHUSD 87xx range)
    KRAKEN_PORT = 8752       # Kraken executor (CNHUSD)
    LLM1_PORT = 8755         # BAYESIAN
    LLM2_PORT = 8757         # TECHNICAL
    LLM3_PORT = 8758         # CHART
    LLM4_PORT = 8759         # RISK
    LLM5_PORT = 8761         # LLM5 SUPREME chart analyzer
    LLM6_PORT = 8762         # ⭐ LLM6 NOVA Smart Money Oracle (CNHUSD)
    LLM7_PORT = 8763         # LLM7 OCULUS Data Quality
    LLM8_PORT = 8764         # LLM8 CHRONOS Timing
    LLM9_PORT = 8765         # LLM9 PREDATOR Execution
    LLM10_PORT = 8760        # LLM10 SENTINEL Guardian
    LLM11_PORT = 8767        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 8768        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 8770  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 RED DRAGON - CORE THRESHOLDS (Imperial Flow - Balanced)
    # ═══════════════════════════════════════════════════════════════════════
    MIN_CONFIDENCE = 75              # LLM confidence threshold (75% balanced)
    MIN_TRINITY_CONFIDENCE = 60      # Trinity consensus (60% ideal for USD/CNH)
    CONSENSUS_MIN_SCORE = 52         # Consensus minimum score (52% majority)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 MEAN REVERSION PARAMETERS - Equilibrium Dynamics
    # ═══════════════════════════════════════════════════════════════════════
    EQUILIBRIUM_EMA = 50             # EMA50 H1 = equilibrium point
    EQUILIBRIUM_ATR_MULTIPLIER = 1.5 # Bands = EMA50 ± 1.5×ATR
    DEVIATION_THRESHOLD_PERCENT = 0.0025  # 0.25% minimum deviation to trade
    
    # Triple TP System (50% / 30% / 20%)
    TP1_MULTIPLIER = 1.0             # TP1: Return to equilibrium (EMA50)
    TP2_MULTIPLIER = 1.5             # TP2: 1.5× initial deviation
    RUNNER_MULTIPLIER = 2.0          # Runner: Up to 2× deviation or trailing
    
    # Stop Loss & Trailing
    SL_ATR_MULTIPLIER = 1.5          # SL = 1.5×ATR(M15)
    TRAILING_ACTIVATION = 0.5        # Activate trailing after TP1
    TRAILING_STEP = 0.5              # Trail at 0.5×ATR distance
    RUNNER_TRAILING = 0.3            # Runner trailing (0.3×ATR aggressive)
    
    # Trading - Red Dragon (H1 context, M15 execution)
    TP_MIN_PIPS = 30.0               # Minimum TP (mean reversion to equilibrium)
    TP_MAX_PIPS = 150.0              # Maximum TP (full reversion + overshoot)
    SL_MIN_PIPS = 20.0               # Minimum SL (1.5×ATR floor)
    SL_MAX_PIPS = 80.0               # Maximum SL (exotic pair buffer)
    ATR_FLOOR = 0.0050               # 50 pip minimum ATR for USDCNH
    CONSOLIDATION_THRESHOLD = 5.0    # Consolidation detection
    
    # Risk Management - Red Dragon Balanced
    MAX_CONCURRENT_TRADES = 1        # 1 position max (mean reversion focus)
    MAX_DAILY_LOSS_PERCENT = 1.5     # 1.5% circuit breaker (conservative)
    MAX_CONSECUTIVE_LOSSES = 3       # FIX A-CNH-02: halt after 3 consecutive losses
    LOSS_HALT_DURATION_SECONDS = 300 # FIX A-CNH-02: 5-minute cooling-off period
    MIN_ADX = 26                     # Max ADX 26 (no trades if ADX > 26)
    MAX_ADX_FOR_REVERSION = 26       # Same as MIN_ADX (reversion only in ranges)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 RSI EXHAUSTION ZONES (M15 execution)
    # ═══════════════════════════════════════════════════════════════════════
    # Red Dragon looks for RSI extremes to confirm mean reversion
    RSI_OVERSOLD_MIN = 30.0          # Oversold zone minimum
    RSI_OVERSOLD_MAX = 35.0          # Oversold zone maximum
    RSI_OVERBOUGHT_MIN = 65.0        # Overbought zone minimum
    RSI_OVERBOUGHT_MAX = 70.0        # Overbought zone maximum
    
    REQUIRE_RSI_DIVERGENCE = False   # Balanced mode: no divergence required
    RSI_DIVERGENCE_LOOKBACK = 10     # Lookback period for divergence (if enabled)
    WICK_THRESHOLD_PERCENT = 0.40    # 40% wick for exhaustion candle
    
    MIN_RSI_FOR_BUY = 30             # Wide range for mean reversion
    MAX_RSI_FOR_BUY = 70             # Allow trades in full range
    MIN_RSI_FOR_SELL = 30            # Symmetric bounds
    MAX_RSI_FOR_SELL = 70            # Natural reversion zones
    
    # ADX SMOOTHING - Reduce noise from volatile markets  
    ADX_SMOOTH_PERIOD = 5            # 5-period MA of ADX to filter noise
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 MACRO FILTERS - PBOC & Market Conditions
    # ═══════════════════════════════════════════════════════════════════════
    MIN_ATR_PERCENT_OF_MEAN = 0.65   # ATR >= 65% of ATR mean (volatility floor)
    PBOC_FIXING_BLOCK_START = "01:15"  # PBOC fixing window start (UTC)
    PBOC_FIXING_BLOCK_END = "02:00"    # PBOC fixing window end (UTC)
    
    # News & Spread Filters
    AVOID_NEWS_BUFFER_MINUTES = 20   # Pause 20min before/after news
    MIN_SPREAD_RATIO = 2.0           # Spread <= 2× normal spread
    
    # Session Times (UTC)
    ASIAN_SESSION_START = 0          # 00:00 UTC
    ASIAN_SESSION_END = 8            # 08:00 UTC
    
    # MACD VALIDATION - Confirm momentum  
    REQUIRE_MACD_CONFIRMATION = True # Validate MACD for BUY/SELL
    MACD_DIVERGENCE_PENALTY = 0.50   # Reduce score 50% if divergence
    
    # COOLDOWN - Red Dragon Balanced (H1 context = fewer trades)
    SNIPER_COOLDOWN_SECONDS = 180    # 3 minutes between trades (mean reversion pace)
    WARMUP_SECONDS = 60              # 60s warmup for USDCNH data quality
    ...
    # ═══════════════════════════════════════════════════════════════
    # 🧠 PAIR-SPECIFIC LLM WEIGHTS - CNHUSD (Red Dragon optimized)
    # CNH responds to: PBOC fix rate, capital flows, Asian session, USD DXY
    # CHRONOS+RISK matter most (PBOC timing), CHART important (fix-rate patterns)
    # ═══════════════════════════════════════════════════════════════
    LLM_WEIGHTS = {
        'BAYESIAN': 1.2,       # 🐉 CNH: Good for yuan range-bound behavior
        'TECHNICAL': 1.1,      # 🐉 CNH: Technical levels respected in yuan
        'CHART': 1.4,          # 🐉 CNH HIGH: Chart patterns important (PBOC fix creates patterns)
        'RISK': 1.3,           # 🐉 CNH HIGH: PBOC intervention risk is REAL and critical
        'SUPREME': 1.0,        # 🐉 CNH: Neutral - less data for exotic
        'SMART_MONEY': 0.8,    # 🐉 CNH: Some institutional flow (less than majors)
        'OCULUS': 0.9,         # 🐉 CNH: Data quality important for exotic
        'CHRONOS': 1.4,        # 🐉 CNH HIGH: Asian session timing + PBOC fix schedule
        'PREDATOR': 1.0,       # 🐉 CNH: Execution neutral
        'NOVA': 0.4,           # 🐉 CNH: Fallback minimal for exotic
    }
    
    # HOLD votes count at reduced weight (HOLD = abstention, not conviction)
    HOLD_VOTE_WEIGHT = 0.3     # HOLD votes count 30% of directional votes
    
    # Minimum directional LLMs needed
    MIN_LLM_AGREEMENT = 2      # Need 2+ LLMs voting same direction
    
    # Network Configuration
    SOCKET_TIMEOUT = 3.5  # 🔧 FIX: Increased from 2.0 to 3.5 — RED_DRAGON Trinity latency ~2053ms was causing 100% timeouts → 0 decisions
    BUFFER_SIZE = 65536
    PING_INTERVAL = 3.0
    ...
    # LLM Query Timeouts
    OLLAMA_TIMEOUT = 90.0
    LLM_QUERY_TIMEOUT = 2.5
    
    # Resilience
    MAX_MESSAGE_SIZE = 1_000_000
    CIRCUIT_BREAKER_THRESHOLD = 3
    CIRCUIT_BREAKER_TIMEOUT = 30
    MAX_RSI_FOR_BUY = 72             # 🔧 JPY-STYLE: 72 (was 70)
    MIN_RSI_FOR_SELL = 28            # 🔧 JPY-STYLE: 28 (was 30)
    MAX_RSI_FOR_SELL = 70            # 🔧 JPY-STYLE: 70 (was 68)
    
    # MACD VALIDATION - Confirm momentum matches price
    REQUIRE_MACD_CONFIRMATION = True # Validate MACD for BUY/SELL
    MACD_DIVERGENCE_PENALTY = 0.5    # Reduce score 50% if divergence (JPY-style)
    
    # SNIPER COOLDOWN - Prevent overtrading
    SNIPER_COOLDOWN_SECONDS = 150    # 🐉 JPY+60s: Exotic needs longer cooldown    # 🔧 JPY-STYLE: 90s between attacks (was 120 - fewer trades)
    WARMUP_SECONDS = 45             # 🐉 JPY+15s: More warmup for exotic data quality             # 🔧 30s warmup (JPY-calibrated, was 90)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # 🐉 RED DRAGON STRATEGY - ACTIVE CONFIGURATION (See lines 5237-5400)
    # ═══════════════════════════════════════════════════════════════════════
    # The sections below (EXCALIBUR KNIGHT / LANCE FORGER) are LEGACY code
    # kept for reference. Current active strategy is RED_DRAGON v1.1
    # configured in ACTIVE_STRATEGY and core parameter overrides above.
    
    # ═══════════════════════════════════════════════════════════════════════
    # ⚔️ EXCALIBUR KNIGHT STRATEGY OVERRIDES (LEGACY - NOT IN USE)
    # ═══════════════════════════════════════════════════════════════════════
    # These values are used when ACTIVE_STRATEGY = "EXCALIBUR_KNIGHT"
    EXCALIBUR_TP_MIN_PIPS = 18       # 🐉 Wider for exotic spread       # TP1 minimum (M5 target)
    EXCALIBUR_TP_MAX_PIPS = 45       # 🐉 Conservative for exotic       # TP2 maximum (M15 swing)
    EXCALIBUR_TP_TYPICAL_PIPS = 25   # Typical post-breakout
    EXCALIBUR_SL_PIPS = 14           # 🐉 Slightly wider for exotic           # Tight SL for M1 entry
    EXCALIBUR_MAX_POSITIONS = 1      # 🔧 FIX: Max 1 position (was 3 - too many)
    EXCALIBUR_MIN_CONFIDENCE = 55    # 🔧 REDUCED: 55%+ (consolidation breakout)
    EXCALIBUR_MIN_TRINITY_CONFIDENCE = 42  # 🔧 REDUCED: 42%+ (allow weak consensus)
    EXCALIBUR_MIN_ADX = 12           # 🔧 REDUCED: 12 (consolidation breakout mode)
    EXCALIBUR_ATR_FLOOR = 0.0050     # 🐉 50 pip floor for USDCNH
    EXCALIBUR_ATR_MAX = 0.0300       # 🐉 300 pip max for USDCNH volatility
    EXCALIBUR_COOLDOWN_SECONDS = 300  # 🐉 5 min cooldown for exotic  # 🔧 FIX v3: 4 min cooldown (equilibrio)
    EXCALIBUR_MIN_RSI_FOR_BUY = 55   # 🔧 CALIBRATED: 55 (was 60 - too high for breakout)
    EXCALIBUR_MAX_RSI_FOR_BUY = 75   # 🔧 CALIBRATED: 75 (was 80 - too extreme)
    EXCALIBUR_MIN_RSI_FOR_SELL = 25  # 🔧 CALIBRATED: 25 (was 20 - too extreme)
    EXCALIBUR_MAX_RSI_FOR_SELL = 45  # 🔧 CALIBRATED: 45 (was 40 - too low for breakout)
    EXCALIBUR_BREAKOUT_PIPS = 8      # Min breakout distance (M1)
    EXCALIBUR_VOLUME_MULTIPLIER = 1.3  # Volume > 1.3x required
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🏦 LANCE FORGER SOVEREIGN STRATEGY (Carry Reversal Micro-Trading)
    # ═══════════════════════════════════════════════════════════════════════
    # 🔧 CNHUSD CALIBRATED: Menos trades, más calidad, proteger capital
    LANCE_FORGER_CONFIG = None         # LanceForgerSovereign instance if active
    LANCE_FORGER_TP_MIN_PIPS = 15      # 🐉 Wider TP for exotic      # Minimum TP (was 10 - too tight for M15)
    LANCE_FORGER_TP_MAX_PIPS = 40      # 🐉 Allow more room for exotic runs      # Maximum TP (was 30 - allow winners to run)
    LANCE_FORGER_SL_PIPS = 20          # 🐉 Wider SL for exotic spread          # Base SL 18 pips (was 15 - stop hunts en 15)
    LANCE_FORGER_MAX_POSITIONS = 1     # 🔧 FIX: Max 1 position ONLY (was 2 - caused overexposure)
    LANCE_FORGER_MIN_CONFIDENCE = 65   # Signal threshold (was 72 - REDUCED for consolidation)
    LANCE_FORGER_COOLDOWN_SECONDS = 300 # 🐉 5 min cooldown for exotic # 🔧 FIX v3: 4 min cooldown (equilibrio)
    LANCE_FORGER_MIN_RSI_BUY = 42      # RSI < 42 for BUY (was 45 - más selectivo)
    LANCE_FORGER_MAX_RSI_SELL = 58     # RSI > 58 for SELL (was 55 - más selectivo)
    LANCE_FORGER_ATR_MIN = 0.0080      # 🐉 80 pip min ATR for USDCNH
    LANCE_FORGER_ATR_MAX = 0.0400      # 🐉 400 pip max ATR for USDCNH
    LANCE_FORGER_DAILY_MAX_DD = 0.004  # 0.4% daily DD max (was 0.6% - proteger)
    LANCE_FORGER_MAX_DAILY_TRADES = 4  # 🐉 Fewer trades for exotic pair  # 🔧 FIX v3: 5 trades/day (4=bloqueado, 6=demasiados)
    ...
    def load_strategy_from_yaml(cls):
        """Load active strategy from cnhconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'cnhconfig.yaml')
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    config = yaml.safe_load(f)
                    
                strategy = config.get('active_strategy', 'RED_DRAGON').upper()
                cls.ACTIVE_STRATEGY = strategy
                
                if strategy == 'RED_DRAGON':
                    print(f"[CONFIG] 🐉 RED DRAGON v1.1 strategy loaded - H1 Mean Reversion USD/CNH")
                elif strategy == 'EXCALIBUR_KNIGHT':
                    cls._apply_excalibur_config(config.get('excalibur_knight', {}))
                    print(f"[CONFIG] ⚔️ EXCALIBUR KNIGHT strategy loaded - M1/M5 Elite Momentum Breakout")
                elif strategy == 'LANCE_FORGER':
                    cls._apply_lance_forger_config(config.get('lance_forger', {}))
                    print(f"[CONFIG] 🏦 LANCE FORGER SOVEREIGN loaded - Carry Reversal Micro-Trading")
                else:
                    print(f"[CONFIG] 🐉 RED DRAGON v1.1 active (default) - H1 Mean Reversion")
                    
                return strategy
        except ImportError:
            print("[CONFIG] PyYAML not installed - using default RED_DRAGON strategy")
        except Exception as e:
            print(f"[CONFIG] Error loading strategy: {e} - using default RED_DRAGON")
    ...
    @classmethod
    def is_lance_forger_active(cls) -> bool:
        """Check if Lance Forger Sovereign strategy is active"""
        return cls.ACTIVE_STRATEGY == 'LANCE_FORGER'
    
    @classmethod
    def _apply_lance_forger_config(cls, lance_forger_yaml: dict):
        """Apply Lance Forger Sovereign configuration from yaml"""
        if not lance_forger_yaml:
            return
            
        # Override main trading parameters for Lance Forger
        cls.TP_MIN_PIPS = lance_forger_yaml.get('tp_atr_multiplier', 1.8) * 10  # Approx based on ATR
        cls.TP_MAX_PIPS = cls.LANCE_FORGER_TP_MAX_PIPS
        cls.SL_MIN_PIPS = lance_forger_yaml.get('sl_fixed_pips', cls.LANCE_FORGER_SL_PIPS)
        cls.SL_MAX_PIPS = lance_forger_yaml.get('sl_fixed_pips', cls.LANCE_FORGER_SL_PIPS)
        cls.MAX_CONCURRENT_TRADES = lance_forger_yaml.get('max_concurrent_positions', cls.LANCE_FORGER_MAX_POSITIONS)
        cls.MIN_CONFIDENCE = int(lance_forger_yaml.get('min_signal_strength', 0.65) * 100)
        cls.SNIPER_COOLDOWN_SECONDS = cls.LANCE_FORGER_COOLDOWN_SECONDS
        
        # RSI bounds for Lance Forger (reversed logic - buy on low RSI, sell on high)
        cls.MIN_RSI_FOR_BUY = 20  # Allow buys when RSI is low
        cls.MAX_RSI_FOR_BUY = lance_forger_yaml.get('rsi_oversold_buy', cls.LANCE_FORGER_MIN_RSI_BUY)
        cls.MIN_RSI_FOR_SELL = lance_forger_yaml.get('rsi_overbought_sell', cls.LANCE_FORGER_MAX_RSI_SELL)
        cls.MAX_RSI_FOR_SELL = 80  # Allow sells when RSI is high
        
        # ATR bounds
        cls.ATR_FLOOR = lance_forger_yaml.get('atr_min_pips', 12) * 0.0001  # Convert pips to price
        
        # Create Lance Forger instance if module available
        if LANCE_FORGER_AVAILABLE and get_lance_forger:
            cls.LANCE_FORGER_CONFIG = get_lance_forger(lance_forger_yaml)
    ...
    def get_strategy_info(cls) -> dict:
        ...
        elif cls.is_lance_forger_active():
            return {
                'name': 'LANCE_FORGER',
                'description': 'Carry Reversal Micro-Trading Sovereign',
                'timeframe': 'M15',
                'tp_range': f'{cls.LANCE_FORGER_TP_MIN_PIPS}-{cls.LANCE_FORGER_TP_MAX_PIPS} pips',
                'sl': f'{cls.LANCE_FORGER_SL_PIPS} pips',
                'risk_reward': '1:1.8',
                'trades_per_day': '8-15',
                'style': 'Sovereign Carry Forger',
                'features': ['Bollinger Reversal', 'XAU Hedge', 'Volume Exhaustion', 'Carry Differential']
            }
        else:
            return {
                'name': 'RED_DRAGON',
                'description': 'Red Dragon v1.1 - H1 Mean Reversion Imperial Flow',
                'timeframe': 'H1',
                'tp_range': f'{cls.TP_MIN_PIPS}-{cls.TP_MAX_PIPS} pips',
                'sl': f'{cls.SL_MIN_PIPS}-{cls.SL_MAX_PIPS} pips',
                'risk_reward': '1:1.8-2.2',
                'trades_per_month': '16-32',
                'style': 'Dragon Equilibrium',
                'features': ['Triple TP', 'EMA50 Equilibrium', 'PBOC Filters', 'Divergence Detection']
            }

# Load strategy on module import
Config.load_strategy_from_yaml()

# ==================== NOVA RED DRAGON DASHBOARD ====================
class NOVADashboard:
    """
    NOVA RED DRAGON - CNHUSD Mean Reversion Dashboard
    by Polarice Labs 2026 | H1 Equilibrium AI Edition
    ...
    - Mean reversion probability meter
    - Order flow analysis
    """
    ...
    def _format_price(self, price, decimals=5):
        """Format price with proper decimals for USDCNH (5 decimal pair)"""
        if price == 0:
            return "-.-----"
    ...
    def render(self):
        """
        ╔════════════════════════════════════════════════════════════════════════════╗
        ║  NOVA TRADING AI - RED DRAGON MEAN REVERSION v1.1                          ║
        ║  by Polarice Labs © 2026 | H1 Equilibrium Precision                        ║
        ╚════════════════════════════════════════════════════════════════════════════╝
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'CNHUSD'))
        ...
        lines.append("")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}███╗   ██╗{self.RESET}{SOFT_WHITE}{self.BOLD} ██████╗ ██╗   ██╗ █████╗ {self.RESET}    {self.RED}{self.BOLD}🐉 RED DRAGON{self.RESET}  •  {self.GREEN}●{self.RESET} {now.strftime('%H:%M:%S')}  •  Up {self.CYAN}{uptime_str}{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}CNHUSD Mean Reversion by{self.RESET} {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        # ══════════════════════════════════════════════════════════════════════════════════════
        # 🔧 CALIBRATION ADVISOR — sugerencias de calibración visibles en dashboard
        # ══════════════════════════════════════════════════════════════════════════════════════
        _cal = d.get('calibration_lines', [])
        if _cal:
            for _cl in _cal:
                lines.append(f"  {_cl}")
            lines.append("")
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}🐉 RED DRAGON{self.RESET} v1.1 CNHUSD {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
    ...
    def _send_llm_decision_to_dashboard(self, llm_name, decision, confidence, reason=""):
        """Send LLM decision message to trinity_dashboard_v3 TCP listener on port 8767 (CNHUSD)"""
        try:
            import socket
            ...
            # Serialize and send to localhost:8767 (CNHUSD dashboard port)
...
class LLMParallelConsensus:
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome, default=str).encode('utf-8')
...
class AdaptiveTPSLEngine:
    ...
    def _calculate_m5_projection(self, closes, direction, atr):
        ...
        logger.debug(f"[M5 PROJECTION] slope={slope:.6f}, projected={projected_price:.5f}, "
                    f"distance={projection_distance*10000:.1f}pips, alignment={alignment:.2f}")
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
                                     support_resistance=None, spread=0.0001, closes=None):
        """
        Calculate INTELLIGENT TP/SL for M1 CNHUSD using ALL available data:
        ...
        """
        # Extract indicators
        atr = float(indicators.get('atr', 0.0100))  # Default 100 pips for USDCNH
        rsi = float(indicators.get('rsi', 50))
        adx = float(indicators.get('adx', 25))
        bb_upper = float(indicators.get('bollinger_upper', current_price + 0.0200))
        bb_lower = float(indicators.get('bollinger_lower', current_price - 0.0200))
        ma_fast = float(indicators.get('ma_fast', current_price))
        ma_slow = float(indicators.get('ma_slow', current_price))
        
        # Ensure minimum ATR for USDCNH M1
        # 🔧 FIX: M1 ATR typically 0.0030-0.0200 for USDCNH, clamp to realistic range
        atr = max(0.0030, min(0.0200, atr))  # Clamp ATR 30-200 pips for USDCNH M1
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 USDCNH typical ATR: 0.0030-0.0200 (= 30-200 pips exotic)
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 USDCNH SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico USDCNH: 0.0030-0.0200 (30-200 pips exotic)
        # Multiplicadores: TP = ATR × 1.1, SL = ATR × 0.8
        # Esto da R:R de ~1.375:1 mínimo
        # ═══════════════════════════════════════════════════════════════
        
        # ATR viene en precio para USDCNH forex (ej: 0.0100 = 100 pips)
        # Clamp ATR a rangos razonables para M1 USDCNH exotic
        atr_clamped = max(0.0030, min(0.0200, atr))  # Min 30 pips, Max 200 pips
        
        # ⭐ MULTIPLICADORES PARA M1 SCALPING - AHORA USA Steps 1-6 dinámicos
        ...
        if m5_projected is not None and m5_alignment > 0.6:
            ...
            m5_tp_extension = min(m5_tp_extension, 0.0050)  # Cap at 50 pips extra
            
            if m5_alignment > 0.8:
                # Very high alignment - use more of the projection
                logger.info(f"[M5 PROJECTION] ✅ STRONG alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension*10000:.1f}pips")
                tp_distance_price += m5_tp_extension
            elif m5_alignment > 0.6:
                # Moderate alignment - use half
                logger.info(f"[M5 PROJECTION] ↗ Moderate alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension*5000:.1f}pips")
        ...
        # 🔧 FIX v5: USDCNH M1 SCALPING - wider distances for exotic spread
        # ═══════════════════════════════════════════════════════════════════════════
        MIN_TP = 0.0020   # 20 pips minimum TP (USDCNH exotic spread ~30-50p)
        MIN_SL = 0.0015   # 15 pips minimum SL (USDCNH exotic)
        SAFETY = 0.0005   # 5 pips safety buffer for USDCNH spread
        
        tp_distance_price = max(MIN_TP, tp_distance_price)
        sl_distance_price = max(MIN_SL, sl_distance_price)
        
        # ⚠️ HARD CAP para USDCNH M1 scalping
        tp_distance_price = min(0.0100, tp_distance_price)  # Máximo 100 pips TP
        sl_distance_price = min(0.0060, sl_distance_price)   # Máximo 60 pips SL
        
        # Asegurar ratio R:R mínimo de 1.3:1
        min_rr = 1.3
        if tp_distance_price / max(sl_distance_price, 0.0001) < min_rr:
        ...
        if direction == 'BUY':
            ...
            tp_price = round(current_price + tp_distance_price + SAFETY, 5)
            sl_price = round(current_price - sl_distance_price - SAFETY, 5)
            ...
            if sl_price >= current_price:
                ...
                sl_price = round(current_price - MIN_SL - SAFETY, 5)
            if tp_price <= current_price:
                logger.error(f"[TP/SL] ❌ BUY TP TOO CLOSE! entry={current_price}, tp={tp_price}")
                tp_price = round(current_price + MIN_TP + SAFETY, 5)
        else:  # SELL
        ...
        if tp_dist_final < MIN_TP:
            logger.error(f"[TP/SL] ❌ TP distance too small: {tp_dist_final:.5f} < {MIN_TP:.5f}")
            tp_price = round(current_price + (MIN_TP + SAFETY if direction == 'BUY' else -(MIN_TP + SAFETY)), 5)
        
        if sl_dist_final < MIN_SL:
            logger.error(f"[TP/SL] ❌ SL distance too small: {sl_dist_final:.5f} < {MIN_SL:.5f}")
            sl_price = round(current_price - (MIN_SL + SAFETY if direction == 'BUY' else -(MIN_SL + SAFETY)), 5)
        
        # Convert to pips for return (1 pip = 0.0001 for CNHUSD forex)
        tp_pips = int(tp_distance_price / 0.0001)
        sl_pips = int(sl_distance_price / 0.0001)
        
        logger.info(f"[TP/SL FINAL] {direction} @ {current_price:.5f} → TP={tp_price:.5f} ({tp_pips}p), SL={sl_price:.5f} ({sl_pips}p) | VALID")
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        """LEGACY: Calculate TP/SL with multiple factors - ADAPTED FOR H1 MEAN REVERSION"""
        
        # H1 MEAN REVERSION: Wider TP/SL for equilibrium targets (30-150 pips TP, 20-80 pips SL)
        # Uses Triple TP system: 50% @ equilibrium, 30% @ 1.5x deviation, 20% runner
        if volatility_regime == 'HIGH':
        ...
        min_rr_ratio = 1.3
        if tp_distance / max(sl_distance, 0.0001) < min_rr_ratio:
            tp_distance = sl_distance * min_rr_ratio
        
        # FIX v5: Mínimos REDUCIDOS para M1 CNHUSD scalping
        # M1 requiere stops ajustados, 8-25 pips típico
        MIN_TP_PIPS = 10  # Mínimo 10 pips TP (M1 scalping)
        MIN_SL_PIPS = 8   # Mínimo 8 pips SL (M1 scalping)
        ...
        # Point value conversion: CNHUSD = 0.0001 per pip (forex standard)
        point_value = 0.0001  # CNHUSD: 1 pip = 0.0001 (e.g., 35 pips = 0.0035)
        safety_margin = SAFETY_PIPS * point_value  # 0.0005 extra
    ...
    def calculate_smart_plan(self, current_price, direction, genome, indicators, sweep_info=None):
        ...
        try:
            ...
            if direction == 'BUY':
                ...
                tp1_raw = min([t for t in potential_tp1s if t > current_price + 0.0020], default=current_price + 0.0030)
                ...
                potential_sls = [
                    support - atr * 0.3,
                    min(lows[-5:]) - atr * 0.2 if len(lows) >= 5 else support - 0.0040,
                    current_price - atr * 1.5 * atr_mult
                ]
                sl_raw = max([s for s in potential_sls if s < current_price - 0.0020], default=current_price - 0.0020)
                ...
                tp1_raw = max([t for t in potential_tp1s if t < current_price - 0.0020], default=current_price - 0.0030)
                ...
                potential_sls = [
                    resistance + atr * 0.3,
                    max(highs[-5:]) + atr * 0.2 if len(highs) >= 5 else resistance + 0.0040,
                    current_price + atr * 1.5 * atr_mult
                ]
                sl_raw = min([s for s in potential_sls if s > current_price + 0.0020], default=current_price + 0.0020)
            ...
            MIN_TP_PIPS = 20  # USDCNH exotic
            MIN_SL_PIPS = 15   # USDCNH exotic
            POINT = 0.0001  # USDCNH forex: 1 pip = 0.0001
            ...
            # Redondear (5 decimales para forex)
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.0001) if atr > 0 else 0  # [NOVA AUDIT F-11] USDCNH pip=0.0001, ATR M1 ~3-10 pips
...
class MarketCycleAnalyzer:
    ...
    def identify_cycle_phase(self, prices):
        ...
        if trend_strength < 0.001 and volume_trend > 1.2:
            return 'ACCUMULATION'  # Trending down or flat with high volume
        elif trend_strength > 0.005 and volume_trend > 1.1:
            return 'MARKUP'  # Trending up with high volume
        elif trend_strength > 0.003 and volume_trend < 0.9:
    ...
    def detect_sweep(self, prices, support_level, resistance_level, tolerance_pips=0.5, volumes=None):
        ...
        # For M1 forex scalping: typical sweep tolerance is 5-15 pips
        base_tolerance = float(tolerance_pips) if tolerance_pips else 0.0020  # 20 pips default USDCNH
        atr_value = getattr(self, 'last_atr', 0.0100)  # Get last ATR (100 pips default USDCNH)
        
        # Scale tolerance dynamically: higher ATR = higher tolerance needed
        dynamic_tolerance = max(0.0020, min(0.0080, atr_value * 1.5))  # 20-80 pips USDCNH
...
class QuantumConsciousnessEngine:
    ...
    def __init__(self):
        ...
        # 🔧 FIX: Daily trade counter - ENFORCE MAX_DAILY_TRADES
        self.daily_trade_count = 0
        self.daily_trade_date = datetime.now().date()
        
        # 🔧 REBALANCED: Direction change cooldown - prevent whiplash but don't block too long
        self.last_direction_change_time = 0
        self.direction_change_cooldown = 120  # 2 min extra cooldown on direction change (was 300 = too long)
        ...
        self.rl_engine = RealQLearningAgent(learning_rate=0.15, discount_factor=0.95, epsilon_start=0.3, strategy_tag=Config.ACTIVE_STRATEGY)

        # 📊 CALIBRATION ADVISOR (Mejora #6) - Diagnóstico inteligente de parámetros
        if CALIBRATION_ADVISOR_AVAILABLE:
            self.calibration_advisor = CalibrationAdvisor(
                strategy=Config.ACTIVE_STRATEGY,
                pair=getattr(Config, 'SYMBOL', 'CNH'),
                tp_pips=getattr(Config, 'TP_MIN_PIPS', None),
                sl_pips=getattr(Config, 'SL_MIN_PIPS', None),
            )
            self.logger.info("[✅ CalibrationAdvisor] Diagnóstico inteligente activo")
        else:
            self.calibration_advisor = None
            self.logger.warning("[CalibrationAdvisor] Módulo no disponible — importación fallida")
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        if SHARED_BRAIN_AVAILABLE and self.genome_counter % 30 == 0:
            try:
                ...
                update_regime_state(
                    symbol=genome.get('metadata', {}).get('symbol', 'CNHUSD'),
        ...
        spread = (ask - bid) * 10000 if bid > 0 and ask > 0 else 0  # In pips (USDCNH: 1pip=0.0001)
        ...
        atr_val = indicators.get('atr', 0.0100)  # Default 100 pips for USDCNH
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('CNHUSD')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM7 OCULUS - Data Quality Validator (Port 8762)
            ...
            # ✨ LLM8 CHRONOS - Timing Optimizer (Port 8763)
            ...
            # ✨ LLM9 PREDATOR - Execution Engine (Port 8764)
        ...
        if today != self.daily_trade_date:
            ...
            self.logger.warning(f"[DAILY RESET] 🔄 New trading day - counter reset to 0")
        
        max_daily = Config.LANCE_FORGER_MAX_DAILY_TRADES if Config.is_lance_forger_active() else 10
        if self.daily_trade_count >= max_daily:
            self.logger.warning(f"[MAX DAILY] 🚫 Daily limit reached ({self.daily_trade_count}/{max_daily}) - NO MORE TRADES TODAY")
            return {'action': 'HOLD', 'reason': f'max_daily_trades_{self.daily_trade_count}_of_{max_daily}'}
        
        # ============ 🔧 FIX: DIRECTION CHANGE COOLDOWN ============
        # After a direction change, wait extra time to prevent BUY→SELL whiplash
        if self.last_direction_change_time > 0:
            time_since_dir_change = current_time - self.last_direction_change_time
            if time_since_dir_change < self.direction_change_cooldown:
                remaining_dir = self.direction_change_cooldown - time_since_dir_change
                self.logger.warning(f"[DIR COOLDOWN] 🔄 Direction changed recently, extra cooldown: {remaining_dir:.1f}s remaining")
                return {'action': 'HOLD', 'reason': f'direction_change_cooldown_{remaining_dir:.0f}s'}
        ...
        try:
            ...
            if volumes is not None:
                ...
                bid_volume = volumes[-1] * 0.48  # Approximate bid volume (slight sell bias in forex)
            ...
            bid = tick_data.get('bid', current_price - 0.0010)
            ask = tick_data.get('ask', current_price + 0.0010)
            ...
            # CRITICAL: Round to valid lot increments (0.01 for CNHUSD)
            ...
            # CNHUSD: Block if consensus < 40% (stricter for calmer pair)
            consensus_reason = _consensus_data.get('reason', 'unknown') if '_consensus_data' in dir() else 'N/A'
            if consensus_score < 0.40 and trinity_decision in ['BUY', 'SELL']:
                self.logger.warning(f"[CONSENSUS_VETO] 🚫 BLOCKING: Consensus {consensus_score*100:.0f}% < 40% (reason: {consensus_reason}) - Too risky even with Trinity {trinity_confidence}%")
                return {
                    'action': 'HOLD',
            ...
            # 🔵 RELAXED THRESHOLD: Trinity must be >= 50% to execute trades (was 65% - too restrictive for GBP)
            # GBP scalping requires faster entries - lower threshold allows more opportunities
            ...
            # For M1 forex scalping, we can trade even in tight ranges - adjusted for CNHUSD
            atr_value = calculate_atr(highs, lows, closes, period=14) if len(highs) >= 14 else 0.0050
            # 🛡️ ATR FLOOR: When buffer has identical prices, ATR calculates as 0
            # Apply same floor as transform to prevent false consolidation veto
            if atr_value < 0.0030:
                atr_value = 0.0030  # 30 pips floor - USDCNH minimum
            ...
            if trinity_decision in ['BUY', 'SELL']:
                # INTELLIGENT ALLOWANCE: Trust LLM consensus for H1 mean reversion
                # Mean reversion requires 75%+ confidence, equilibrium-based entry
                ...
                # ════════════════════════════════════════════════════════════════════
                # ⚔️ EXCALIBUR KNIGHT STRATEGY VALIDATION
                ...
                if Config.is_excalibur_active() and hasattr(self, 'excalibur_analyzer') and self.excalibur_analyzer:
                    ...
                    excalibur_indicators = {
                        ...
                        'atr_pips': atr_value * 10000 if atr_value else 0,  # Convert to pips
                    ...
                    if NEWS_INTELLIGENCE_AVAILABLE:
                        try:
                            news_bias = get_trading_bias('CNHUSD')
                    ...
                    if excalibur_valid:
                        # ⚔️ EXCALIBUR APPROVED - Override with Excalibur parameters
                        trinity_decision = excalibur_result['signal']
                        trinity_confidence = excalibur_result['confidence']
                        
                        # Log Excalibur signal
                        self.logger.warning(f"[⚔️ EXCALIBUR] ✅ KNIGHT APPROVED: {trinity_decision} @ {excalibur_result['entry_price']:.5f}")
                        self.logger.info(f"[⚔️ EXCALIBUR] TP1={excalibur_result['tp1']:.5f} TP2={excalibur_result['tp2']:.5f} SL={excalibur_result['sl']:.5f}")
                        self.logger.info(f"[⚔️ EXCALIBUR] R:R=1:{excalibur_result['risk_reward']:.1f} Confidence={trinity_confidence:.0f}%")
                        for reason in excalibur_result['reasons']:
                            self.logger.info(f"[⚔️ EXCALIBUR] {reason}")
                        ...
                        if trinity_decision in ['BUY', 'SELL']:
                            self.logger.info(f"[⚔️ EXCALIBUR] ⏳ No valid breakout - waiting for knight's moment")
                            if excalibur_result.get('warnings'):
                                for warning in excalibur_result['warnings'][:3]:  # Limit to 3 warnings
                                    self.logger.debug(f"[⚔️ EXCALIBUR] {warning}")
                            trinity_decision = 'HOLD'
                            trinity_confidence = 0
            ...
            # 🔧 FIX v4: Validate Trinity values against broker STOPS_LEVEL (USDCNH exotic)
            MIN_TP_DIST = 0.0020  # 20 pips minimum (USDCNH exotic spread)
            MIN_SL_DIST = 0.0015  # 15 pips minimum (USDCNH exotic)
            SAFETY = 0.0010       # 10 pips safety buffer (USDCNH wide spread)
            ...
            if trinity_response and isinstance(trinity_response, dict) and not trinity_response.get('_is_fallback', False):
                ...
                if trinity_tp > 0 and trinity_sl > 0:
                    ...
                    if actual_tp_dist >= MIN_TP_DIST and actual_sl_dist >= MIN_SL_DIST:
                        ...
                        self.logger.info(f"[TP/SL] ✓ Using Trinity: TP={tp_price:.5f} ({actual_tp_dist*10000:.1f}pips), SL={sl_price:.5f} ({actual_sl_dist*10000:.1f}pips)")
                    else:
            ...
            if tp_price is None or sl_price is None:
                ...
                # Ensure ATR is in indicators (default 10 pips for forex)
                indicators['atr'] = atr if atr > 0 else 0.0100
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 8770
        ...
        try:
            ...
            # Send to sound.py server on port 8770 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 8770))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:8770")
        except socket.timeout:
...
class AdvancedIPC:
    ...
    def __init__(self):
        ...
        # 💾 PERSISTENT STATS: Load from previous sessions so dashboard never resets to 0
        self.stats_snapshot_file = os.path.join(os.path.dirname(__file__), 'logs', 'stats_snapshot.json')
        self._load_stats_snapshot()

    def _load_stats_snapshot(self):
        """Load persistent performance stats from previous sessions"""
        try:
            if os.path.exists(self.stats_snapshot_file):
                with open(self.stats_snapshot_file, 'r', encoding='utf-8') as f:
                    snap = json.load(f)
                for key in ('wins', 'losses', 'winrate', 'pnl_today', 'tp_hits', 'sl_hits', 'manual_closes'):
                    if key in snap:
                        self.stats[key] = snap[key]
        except Exception:
            pass  # Non-critical: start fresh on error

    def _save_stats_snapshot(self):
        """Persist performance stats to disk so they survive restarts"""
        try:
            snap = {k: self.stats.get(k, 0) for k in ('wins', 'losses', 'winrate', 'pnl_today', 'tp_hits', 'sl_hits', 'manual_closes')}
            snap['saved_at'] = datetime.now().isoformat()
            with open(self.stats_snapshot_file, 'w', encoding='utf-8') as f:
    ...
    def _play_sound_alert(self, alert_type, symbol='CNHUSD'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # 🔧 FIX: Use Config.SOUND_PORT instead of hardcoded 8770
            sound_port = getattr(Config, 'SOUND_PORT', 8770)
                ...
                if request.get('type') == 'handshake':
                    ...
                    with self.quimera_lock:
                        self.quimera_connections[('executor', addr)] = {
                            ...
                            'type': 'executor',
                            'last_ping': time.time(),
                            'symbol': request.get('symbol', 'CNHUSD')
                        }
                ...
                try:
                    ...
                    # CRITICAL: This enables ML learning from trade results!
                        ...
                        try:
                            ...
                            # 💾 PERSIST: Save to disk so stats survive restarts
                            self._save_stats_snapshot()

                            # 🧠 SHARED BRAIN: Record trade for cross-pair learning
                            try:
                                if SHARED_BRAIN_AVAILABLE:
                                    _dash = nova_dashboard.data if 'nova_dashboard' in globals() else {}
                                    ...
                                    try:
                                        ...
                                        if os.path.exists(config_path):
                                            with open(config_path, 'r', encoding='utf-8') as f:
                                                ...
                                                strategy_name = cfg.get('strategy', cfg.get('strategy_name', 'UNKNOWN'))
                                    except Exception:
                                        pass
                                    if strategy_name == 'UNKNOWN':
                                        strategy_name = _dash.get('strategy', _dash.get('strategy_name', 'UNKNOWN'))
                                    if strategy_name == 'UNKNOWN' and isinstance(_snap, dict):
                                        strategy_name = _snap.get('strategy', _snap.get('strategy_name', 'UNKNOWN'))
                                ...
                                self.logger.warning(f"[SHARED BRAIN] Write error (non-critical): {_sb_err}")
                            ...
                            # 📊 CALIBRATION ADVISOR: Registrar trade para diagnóstico
                            if self.calibration_advisor is not None:
                                try:
                                    _snap2 = self.stats.get('last_calibration_snapshot') or {}
                                    _market2 = _snap2.get('market', {}) if isinstance(_snap2, dict) else {}
                                    _session2 = (_market2.get('session') or self.stats.get('session', 'UNKNOWN'))
                                    _regime2  = (_market2.get('volatility') or self.stats.get('volatility_regime', 'UNKNOWN'))
                                    cal_report = self.calibration_advisor.record_trade(
                                        pnl_pips=pnl, pnl_usd=pnl_usd, reason=reason, action=action,
                                        session=_session2, regime=_regime2,
                                    )
                                    if cal_report:
                                        try:
                                            if 'nova_dashboard' in globals():
                                                nova_dashboard.data['calibration_lines'] = cal_report.to_dashboard_lines()
                                        except Exception:
                                            pass
                                        if cal_report.has_issues:
                                            for _line in cal_report.to_dashboard_lines():
                                                self.logger.warning(f"[CAL_ADVISOR] {_line}")
                                except Exception as _ca_err:
                                    self.logger.debug(f"[CAL_ADVISOR] Error (non-critical): {_ca_err}")
    ...
    def _handle_genome(self, client, addr):
        ...
        try:
            client.settimeout(120.0)  # 120 sec timeout for persistent connections (match EURUSD)
            ...
            while True:
                try:
                    ...
                    # Respond to handshakes on injector port (MUST send ACK like EURUSD)
                    ...
                    # Log genome receipt (DEBUG to avoid flooding)
                    self.logger.debug(f"[Injector] GENOME RECEIVED: type={genome.get('type')}")
                    
                    # Normalize genome from MQL5 flat structure to nested format
                    genome = self._normalize_genome(genome)
                    
                    # Validate and queue tick
                    is_valid = self._validate_tick(genome)
                    if is_valid:
                        with self.lock:
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "CNHUSD", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "CNHUSD"}, ...}
        """
        ...
        # Handle timeframes array -> indicators + timeframe_analysis for Trinity
        if 'timeframes' in genome and isinstance(genome['timeframes'], list):
            ...
            # 🎯 BUILD timeframe_analysis for Trinity consensus engine
            timeframe_analysis = []
            for tf_data in genome['timeframes']:
                if not isinstance(tf_data, dict):
                    continue
                
                tf_name = tf_data.get('timeframe', 'UNK')
                
                # Extract indicators for this TF
                rsi = tf_data.get('rsi', 50)
                adx = tf_data.get('adx', 20)
                ma_cross = tf_data.get('ma_cross', 0)  # 1=bullish, -1=bearish
                macd_main = tf_data.get('macd_main', 0)
                trend = tf_data.get('trend', 0)  # 1=up, -1=down, 0=neutral
                boll_position = tf_data.get('boll_position', 0.5)  # 0-1
                deviation = tf_data.get('deviation', 0)  # from EMA50
                
                # 🧠 INTELLIGENT SIGNAL CALCULATION
                signal = "HOLD"
                strength = 50
                reasons = []
                
                # RSI signal (30/70 thresholds)
                if rsi < 30:
                    ...
                    reasons.append(f"RSI({rsi:.0f})<40=BULLISH")
                    strength += 5
                    if signal == "HOLD": signal = "BUY"
                elif rsi > 60:
                    reasons.append(f"RSI({rsi:.0f})>60=BEARISH")
                    strength += 5
                    if signal == "HOLD": signal = "SELL"
                
                # ADX strength (confirm trend)
                if adx > 25:
                    reasons.append(f"ADX({adx:.0f})>25=STRONG")
                    strength += 10
                elif adx > 20:
                    reasons.append(f"ADX({adx:.0f})>20=MEDIUM")
                    strength += 5
                
                # MA Cross signal
                if ma_cross > 0:
                    reasons.append("MA_CROSS=BULL")
                    strength += 8
                    if signal == "HOLD": signal = "BUY"
                elif ma_cross < 0:
                    reasons.append("MA_CROSS=BEAR")
                    strength += 8
                    if signal == "HOLD": signal = "SELL"
                
                # MACD histogram
                if macd_main > 0:
                    reasons.append(f"MACD({macd_main:.5f})>0=BULL")
                    strength += 5
                    if signal == "HOLD": signal = "BUY"
                elif macd_main < 0:
                    reasons.append(f"MACD({macd_main:.5f})<0=BEAR")
                    strength += 5
                    if signal == "HOLD": signal = "SELL"
                
                # Bollinger position (extremes)
                if boll_position < 0.2:
                    reasons.append(f"BOLL({boll_position:.2f})<0.2=OVERSOLD")
                    strength += 12
                    signal = "BUY"
                elif boll_position > 0.8:
                    reasons.append(f"BOLL({boll_position:.2f})>0.8=OVERBOUGHT")
                    strength += 12
                    signal = "SELL"
                
                # Trend confirmation (from Bridge)
                if trend == 1:
                ...
                # Mean reversion signal (large deviation)
                if deviation > 0.003:  # 30 pips from equilibrium
                    price = tf_data.get('close', 0)
                    ema50 = tf_data.get('ema50', price)
                    if price > ema50:
                        reasons.append(f"MEAN_REVERSION: {deviation*10000:.0f}pips ABOVE EMA50")
                        strength += 15
                        signal = "SELL"
                    else:
                        reasons.append(f"MEAN_REVERSION: {deviation*10000:.0f}pips BELOW EMA50")
                        strength += 15
                        signal = "BUY"
                ...
                timeframe_analysis.append(tf_vote)
            
            # Add to genome for Trinity
            genome['timeframe_analysis'] = timeframe_analysis

        
        return genome
    
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for CNHUSD data"""
        try:
            genome_type = genome.get('type', '')
            
            # ⭐ Accept both standard and Multi-TF genome types from cnhquimera
            valid_types = [
            ...
            if genome_type in valid_types:
                ...
                try:
                    ...
                    # Minimal sanity: bid < ask (allow equal for FOREX)
                    if bid > ask:
                        self.logger.warning(f"[Validator] Inverted: bid={bid} > ask={ask}")
                        return False
                    
                    # Acceptable range for USDCNH
                    # USDCNH = USD per Chinese Yuan, typically ~7.25
                    # Accept range: 5.0 - 9.0 to be safe for historical/future variations
                    mid_price = (bid + ask) / 2.0
                    
                    # USDCNH forex price range (5.0 - 9.0)
                    if 5.0 <= mid_price <= 9.0:
                        return True
                    
                    # Price out of realistic range for USDCNH
                    self.logger.warning(f"[Validator] USDCNH price out of range: {mid_price:.5f} (expected 5.0-9.0)")
                    return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                sock.settimeout(5.0)  # Trinity needs ~2-3s to query LLMs in parallel
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - Quimera not connected to port 8756")
            ...
            # 1. Validate price is within realistic bounds (CNHUSD: 0.05-0.50 exotic range)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR USDCNH
            # ⭐ USDCNH = USD/Yuan, typically ~7.25
            valid_forex_range = 5.0 <= entry_price <= 9.0  # Normal USDCNH pricing
            
            if not valid_forex_range:
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price:.5f} out of realistic range for USDCNH (expected 5.0-9.0)")
                return False
            ...
            if abs(entry_price - original_entry) > 0.0050:  # > 50 pips change
                self.logger.warning(f"[EINSTEIN] ⚠️ Entry price changed significantly: original={original_entry:.5f}, current={entry_price:.5f} (diff={abs(entry_price-original_entry)*10000:.1f}pips)")
            
            # Use ATR-based approach (1.5x for TP, 1.0x for SL) - USDCNH calibrated
            atr_val = decision.get('atr', 0.0100)  # Default 100 pips for USDCNH
            atr_original = atr_val
            
            # ⚠️ CRITICAL: Clamp ATR to M1 USDCNH realistic range
            # M1 ATR típico: 0.0030-0.0200 (30-200 pips)
            atr_val = max(0.0030, min(0.0200, atr_val))
            if atr_val != atr_original:
                self.logger.debug(f"[EINSTEIN] ATR clamped: {atr_original:.5f} → {atr_val:.5f}")
            
            # ⭐ EINSTEIN v2.0: First check if Trinity TP/SL are valid
            # If Trinity/LLMs calculated good TP/SL, RESPECT them instead of overriding
            if tp_price > 0 and sl_price > 0:
                trinity_tp_dist = abs(tp_price - entry_price)
                trinity_sl_dist = abs(sl_price - entry_price)
                
                # Check proper ordering
                ordering_ok = False
                if action == 'BUY' and tp_price > entry_price > sl_price:
                    ordering_ok = True
                elif action == 'SELL' and sl_price > entry_price > tp_price:
                    ordering_ok = True
                
                # If Trinity TP/SL are valid and have good distance, use them
                if ordering_ok and trinity_tp_dist >= 0.0020 and trinity_sl_dist >= 0.0015:
                    self.logger.info(f"[EINSTEIN] ✅ Trinity TP/SL VALID - Respecting: TP={trinity_tp_dist*10000:.0f}p, SL={trinity_sl_dist*10000:.0f}p")
                    # Only apply safety caps
                    if trinity_tp_dist > 0.0100:  # Cap at 100 pips
                        if action == 'BUY':
                            tp_price = round(entry_price + 0.0100, 5)
                        else:
                            tp_price = round(entry_price - 0.0100, 5)
                    if trinity_sl_dist > 0.0060:  # Cap at 60 pips
                        if action == 'BUY':
                            sl_price = round(entry_price - 0.0060, 5)
                        else:
                            sl_price = round(entry_price + 0.0060, 5)
                    # Skip recalculation, go to final validation
                    tp_distance = abs(tp_price - entry_price)
                    sl_distance = abs(sl_price - entry_price)
                else:
                    self.logger.warning(f"[EINSTEIN] ⚠️ Trinity TP/SL invalid, recalculating with ATR")
                    tp_distance = atr_val * 1.5
                    sl_distance = atr_val * 1.0
            else:
                # No Trinity TP/SL - calculate from ATR
                tp_distance = atr_val * 1.5
                sl_distance = atr_val * 1.0
            
            # ═══════════════════════════════════════════════════════════════════════════
            # MT5 STOPS_LEVEL REQUIREMENT: Broker requires minimum distance from ASK/BID
            # 🔧 FIX v5: USDCNH exotic - distances in price units (0.0001 = 1 pip)
            # ═══════════════════════════════════════════════════════════════════════════
            MIN_TP_DISTANCE = 0.0020  # 20 pips minimum TP (USDCNH exotic spread)
            MIN_SL_DISTANCE = 0.0015  # 15 pips minimum SL
            SAFETY_MARGIN = 0.0005    # 5 pips buffer for USDCNH wide spread
            
            tp_distance = max(MIN_TP_DISTANCE, tp_distance)
            sl_distance = max(MIN_SL_DISTANCE, sl_distance)
            
            # 🚨 HARD CAP: Calibrated for USDCNH (TP 20-100p, SL 15-60p)
            tp_distance = min(tp_distance, 0.0100)  # 100 pips MAX TP
            sl_distance = min(sl_distance, 0.0060)  # 60 pips MAX SL
            
            self.logger.info(f"[EINSTEIN] ✅ ATR-Based: TP={tp_distance*10000:.0f}pips SL={sl_distance*10000:.0f}pips")
            ...
            if actual_tp_distance < MIN_TP_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ⚠️ TP distance ({actual_tp_distance*10000:.1f}pips) < MIN ({MIN_TP_DISTANCE*10000:.0f}pips), adjusting...")
                if action == 'BUY':
            ...
            if actual_sl_distance < MIN_SL_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ⚠️ SL distance ({actual_sl_distance*10000:.1f}pips) < MIN ({MIN_SL_DISTANCE*10000:.0f}pips), adjusting...")
                if action == 'BUY':
            ...
            tp_pips = actual_tp_distance / 0.0001  # Convert to pips (forex)
            sl_pips = actual_sl_distance / 0.0001
            ...
            if action == 'BUY':
                if not (tp_price > entry_price > sl_price):
                    self.logger.error(f"[EINSTEIN] ❌ BUY invalid: TP({tp_price:.5f}) > Entry({entry_price:.5f}) > SL({sl_price:.5f}) FAILED")
                    # Force recalculation with GUARANTEED SAFE values
                    tp_price = round(entry_price + MIN_TP_DISTANCE + SAFETY_MARGIN, 5)
                    sl_price = round(entry_price - MIN_SL_DISTANCE - SAFETY_MARGIN, 5)
                    self.logger.info(f"[EINSTEIN] 🔧 BUY corrected: TP={tp_price:.5f}, SL={sl_price:.5f}")
            elif action == 'SELL':
                if not (sl_price > entry_price > tp_price):
                    self.logger.error(f"[EINSTEIN] ❌ SELL invalid: SL({sl_price:.5f}) > Entry({entry_price:.5f}) > TP({tp_price:.5f}) FAILED")
                    # Force recalculation with GUARANTEED SAFE values
                    tp_price = round(entry_price - MIN_TP_DISTANCE - SAFETY_MARGIN, 5)
                    sl_price = round(entry_price + MIN_SL_DISTANCE + SAFETY_MARGIN, 5)
                    self.logger.info(f"[EINSTEIN] 🔧 SELL corrected: TP={tp_price:.5f}, SL={sl_price:.5f}")
            ...
            if tp_distance_final < MIN_TP_DISTANCE or sl_distance_final < MIN_SL_DISTANCE:
                self.logger.error(f"[EINSTEIN] 🚨 CRITICAL: Final distances BELOW minimum! TP={tp_distance_final*10000:.1f}pips (min={MIN_TP_DISTANCE*10000:.0f}), SL={sl_distance_final*10000:.1f}pips (min={MIN_SL_DISTANCE*10000:.0f})")
                return False
            
            self.logger.info(f"[EINSTEIN] ✅ VALIDATED: {action} Entry={entry_price:.5f} TP={tp_price:.5f}({tp_pips_final:.0f}p, dist={tp_distance_final*10000:.1f}pips) SL={sl_price:.5f}({sl_pips_final:.0f}p, dist={sl_distance_final*10000:.1f}pips)")
...
# ==================== MAIN ENGINE ====================
class QuantumCore:
    """NOVA RED DRAGON v1.1 - MEAN REVERSION EDITION"""
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        if closes and current_price > 0:
            ...
            if gap > 0.01:  # More than 0.01 (1 pip for CNHUSD) difference
    ...
    def __init__(self):
        ...
        if Config.is_excalibur_active() and EXCALIBUR_AVAILABLE:
            ...
            self.logger.info("[⚔️ EXCALIBUR] Knight analyzer initialized - M1/M5 Elite Momentum mode")
        ...
        # FIX M-CNH-01: NOVAMarketStateDetector singleton
        self.nova_detector = NOVAMarketStateDetector(logger=self.logger)

        # FIX A-CNH-02: consecutive-loss halt timer
        self.loss_halt_until = 0.0

        # FIX A-CNH-04: position reconcile timer
    ...
    def _print_banner(self):
        ...
        logger.info("=" * 110)
        if is_excalibur:
            logger.info("🐉  RED DRAGON v1.1 - CNHUSD MEAN REVERSION IMPERIAL FLOW")
            logger.info("🏆  \"The dragon awakens when yuan deviates from equilibrium\"")
        else:
            logger.info("🐉 NOVA RED DRAGON v1.1 - CNHUSD MEAN REVERSION CONSCIOUSNESS (180s cooldown, 70-76% win rate)")
        logger.info("=" * 110)
        logger.info("")
        ...
        trades_freq = strategy_info.get('trades_per_week', strategy_info.get('trades_per_day', 'N/A'))
        trades_label = 'Trades/Day' if 'trades_per_day' in strategy_info else 'Trades/Week'
        logger.info(f"  • {trades_label}: {trades_freq}")
        logger.info(f"  • Style: {strategy_info['style']}")
        logger.info("")
        
        if is_excalibur:
            logger.info("🦘 IMPERIAL DRAGON FEATURES:")
            logger.info("  + M15 Bollinger Reversal Detection")
            logger.info("  + XAU Gold Hedge Correlation")
            logger.info("  + Volume Exhaustion Analysis (>1.3x)")
            logger.info("  + Carry Differential Catalyst")
            logger.info("  + Sydney/Tokyo Session Optimization")
    ...
    def start(self):
        ...
        logger.info(f"[OK] RED DRAGON v1.1 ONLINE - 🐉 Mean Reversion Mode ACTIVE")
    ...
    def _send_pings_to_quimera(self):
        ...
        The MT5 EA (audquimera_bridge.mq5) expects to receive PING messages
        ...
        """
        logger.info("[PING] 🏓 Ping thread started - robust dead-socket detection (5s interval)")
        ping_count = 0

        while self.running:
            try:
                time.sleep(5.0)  # Send ping every 5s (EA bunker timeout is 15s)

                # Build message once, outside lock (don't hold lock during network I/O)
                ping_msg = json.dumps({'type': 'ping', 'timestamp': int(time.time() * 1000)}).encode('utf-8')
                full_msg = struct.pack('<I', len(ping_msg)) + ping_msg

                # Snapshot of active executor sockets — release lock before sendall
                with self.ipc.quimera_lock:
                    executor_snapshot = [
                        (k, v.get('socket'))
                        for k, v in self.ipc.quimera_connections.items()
                        if isinstance(k, tuple) and k[0] == 'executor'
                    ]

                dead_keys = []
                for key, sock in executor_snapshot:
                    if not sock:
                        continue
                    try:
                        sock.sendall(full_msg)
                        ping_count += 1
                        with self.ipc.quimera_lock:
                            if key in self.ipc.quimera_connections:
                                self.ipc.quimera_connections[key]['last_ping'] = time.time()
                        if ping_count % 20 == 0:
                            logger.debug(f"[PING] 🏓 Sent ping #{ping_count} to {key}")
                    except (BrokenPipeError, ConnectionResetError, ConnectionAbortedError):
                        logger.warning(f"[PING] ⚠️ Connection lost to {key} — removing dead socket")
                        dead_keys.append((key, sock))
                    except OSError as e:
                        err_code = getattr(e, 'winerror', None) or getattr(e, 'errno', 0)
                        if err_code in (10038, 10053, 10054, 10057, 32, 104):
                            logger.warning(f"[PING] ⚠️ Socket error {err_code} on {key} — removing dead socket")
                            dead_keys.append((key, sock))
                        else:
                            logger.error(f"[PING] ❌ Unexpected OS error on {key}: {e}")
                    except Exception as e:
                        logger.warning(f"[PING] ⚠️ Failed to ping {key}: {e}")

                # Remove dead sockets so the EA can reconnect cleanly
                for key, sock in dead_keys:
                    try:
                        with self.ipc.quimera_lock:
                            self.ipc.quimera_connections.pop(key, None)
                        sock.close()
                        logger.info(f"[PING] 🗑️ Dead executor removed: {key} — EA can reconnect")
                    except Exception:
                        pass
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        - genome / tick: bid/ask at root level OR in tick_data (bridge format)
        ...
        try:
            ...
            # ⭐ Accept ALL genome types including bridge format
            valid_types = [
                'market_genome', 'tick_genome',
                'market_genome_multi_tf', 'tick_genome_multi_tf',
                'genome', 'tick'  # ⭐ Bridge format from audquimera_bridge.mq5
            ]
            if genome_type not in valid_types:
                # Already in old format or unknown - return as-is
                return genome
            
            # Extract data from Quimera format
            # ⭐ BRIDGE FORMAT: data may be at root level, not in metadata/tick_data
            metadata = genome.get('metadata', {})
            tick_data = genome.get('tick_data', {})
            bar_data = genome.get('bar_data', {})
            
            # ⭐ BRIDGE: symbol at root level or in metadata
            symbol = genome.get('symbol') or metadata.get('symbol', 'CNHUSD')
            
            # ⭐ BRIDGE: bid/ask at root level OR in tick_data
            bid = float(genome.get('bid') or tick_data.get('bid', 0))
            ask = float(genome.get('ask') or tick_data.get('ask', 0))
            ...
            self.logger.debug(f"[Transform] Price from Quimera: bid={bid:.5f}, ask={ask:.5f}, last={close_price:.5f}")
            
            # CRITICAL: Validate price is in reasonable range for USDCNH FOREX
            # USDCNH = USD/Yuan, typically ~7.25
            # This is an exotic pair - prices are around 6.X-8.X
            if close_price > 0:
                if close_price < 5.0 or close_price > 9.0:
                    # Only reject EXTREME outliers (outside realistic range)
                    self.logger.error(f"[Transform] ❌ REJECT: Price {close_price:.5f} is extreme outlier for {symbol}")
                ...
                if close_price < 6.0 or close_price > 8.0:
                    self.logger.warning(f"[Transform] ⚠️ Price {close_price:.5f} is unusual but accepting (volatile market)")
            ...
            if history and len(history) > 0:
                ...
                self.logger.debug(f"[Transform] ✅ Using REAL history from Quimera: {len(history)} bars")
                ...
                for i, close_p in enumerate(prices):
                    ...
                    # 🚨 FIX: Add MICRO-VARIATION to closes for RSI calculation
                    # When all buffer prices are identical (eg 0.70819), RSI gets
                    # zero gain/loss → RSI=50 forever. Real M1 candles have close
                    # variation within the H/L range. Create realistic close pattern
                    # that oscillates within the bar's range for non-degenerate RSI.
                    bar_range = bar_high - bar_low
                    # Alternate close position: odd bars close higher, even bars lower
                    # This creates a realistic up/down close pattern for RSI
                    if i % 3 == 0:
                        bar_close = close_p + bar_range * 0.15   # Close near upper third
                    elif i % 3 == 1:
                        bar_close = close_p - bar_range * 0.10   # Close near lower third
                    else:
                        bar_close = close_p + bar_range * 0.05   # Close near middle
                    # Ensure close stays within H/L bounds
            ...
            # 🚨 BUG FIX: When EA sends same bid/ask repeatedly, price_buffer
            # has 100+ identical ticks. HYBRID mode mixes 50 zero-variation ticks
            # with 19 synthetic bars → dilutes ATR from ~14 pips to ~0.
            # FIX: Check tick variation FIRST. If < 5 pips, use BAR HISTORY ONLY
            # which preserves synthetic bar H/L variation for realistic ATR.
            # ============================================================
            tick_variation = 0.0
            ...
            if tick_variation >= 0.0005 and hasattr(self, 'price_buffer') and len(self.price_buffer) >= 14:
                # Ticks have REAL variation (5+ pips) — safe to use for USDCNH indicators
                if len(history) >= 20:
                ...
                self.logger.info(f"[Transform] BAR HISTORY ONLY: {len(closes_array)} bars (tick_var={tick_variation*10000:.1f}p < 5p)")
            else:
            ...
            # The EA (audquimera_bridge.mq5) sends REAL RSI, ATR, ADX, MAs, etc.
            # computed by MT5 from actual M1 candle history via CopyBuffer().
            # These are in genome['timeframes'][0] or genome['indicators'].
            # Previously we were IGNORING them and recalculating from synthetic bars
            # (price_buffer with identical prices) → RSI=0, ATR=0, ADX=80 → always HOLD.
            ...
            if isinstance(timeframes_list, list) and len(timeframes_list) > 0:
                ...
                # Already normalized by _normalize_genome
            ...
            if ea_tf and isinstance(ea_tf, dict):
                ...
                # Validate EA indicators are real (not zero/default)
                if _ea_rsi is not None and _ea_atr is not None and _ea_adx is not None:
                    try:
                        ...
                        # EA indicators are valid if ATR > 0 (market has price movement)
                        if _ea_atr > 0:
                            ea_rsi = _ea_rsi
                            ea_atr = _ea_atr
                            ea_adx = _ea_adx
            ...
            if ea_rsi is not None and ea_atr is not None:
                ...
                # FALLBACK: Calculate from synthetic bars (less accurate)
                ...
                # ADX calculation from synthetic bars
                adx_calculated = 25.0
                period = 14
                ...
                ATR_FLOOR_CNHUSD = 0.0005
                atr_at_floor = False
                if atr_calculated < ATR_FLOOR_CNHUSD:
                    self.logger.warning(f"[Transform] ⚠️ ATR floor: {atr_calculated:.6f} → {ATR_FLOOR_CNHUSD} (5 pips minimum)")
                    atr_calculated = ATR_FLOOR_CNHUSD
                    atr_at_floor = True
                
                if atr_at_floor and (rsi_calculated <= 1.0 or rsi_calculated >= 99.0):
                    self.logger.warning(f"[Transform] ⚠️ RSI reset: {rsi_calculated:.1f} → 50.0 (no real variation, ATR at floor)")
                    rsi_calculated = 50.0
                
                if atr_at_floor and adx_calculated >= 70.0:
                    self.logger.warning(f"[Transform] ⚠️ ADX reset: {adx_calculated:.1f} → 25.0 (no real variation, ATR at floor)")
                    adx_calculated = 25.0
            
            self.logger.debug(f"[Transform] Final indicators: RSI={rsi_calculated:.1f} ATR={atr_calculated:.4f} ADX={adx_calculated:.1f} (source={'EA' if ea_rsi is not None else 'SYNTHETIC'})")
            ...
            # MACD Calculation - always from closes array (EA doesn't send MACD)
            ...
            self.logger.debug(f"[Transform] ✅ Indicators: RSI={rsi_calculated:.1f} ADX={adx_calculated:.1f} ATR={atr_calculated:.4f} MA_F={ma_fast_calc:.2f} MA_S={ma_slow_calc:.2f} BB_U={bb_upper_calc:.2f}")
    ...
    def process_genome(self, genome):
        ...
        try:
            # 🏷️ MEJORA #2: Stamp active strategy on every genome before processing
            genome['active_strategy'] = Config.ACTIVE_STRATEGY
            genome['strategy_timeframe'] = getattr(Config, 'PRIMARY_TIMEFRAME', 'M1')
            ...
            symbol = genome.get('metadata', {}).get('symbol', 'CNHUSD')
            # FIX: Also get symbol from root level (GBP EA sends flat structure)
            if symbol == 'CNHUSD' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')

            # ═══ FIX BUG-CNH-CRIT-01: PBOC FIXING WINDOW BLOCK ═══
            import datetime as _dt
            _now_utc = _dt.datetime.utcnow().time()
            _pboc_start = _dt.time(1, 15)  # 01:15 UTC
            _pboc_end   = _dt.time(2, 0)   # 02:00 UTC
            if _pboc_start <= _now_utc <= _pboc_end:
                self.logger.warning(f"[PBOC BLOCK] 🖼️ Trading blocked during PBOC daily fixing window (01:15-02:00 UTC). Mean reversion invalid.")
                return None

            # ═══ FIX A-CNH-02: CONSECUTIVE LOSSES CIRCUIT BREAKER ═══
            ...
            if getattr(self, 'loss_halt_until', 0) > time.time():
                ...
                self.logger.warning(f"[A-CNH-02] Loss halt active — {remaining:.0f}s remaining. Skipping.")
                return None
            if consecutive_losses >= Config.MAX_CONSECUTIVE_LOSSES:
                self.loss_halt_until = time.time() + Config.LOSS_HALT_DURATION_SECONDS
                self.logger.error(f"[A-CNH-02] {consecutive_losses} consecutive losses → halt {Config.LOSS_HALT_DURATION_SECONDS}s")
                return None

            # ═══ FIX A-CNH-03: DAILY LOSS CIRCUIT BREAKER ═══
            with self.ipc.stats_lock:
            ...
            if balance > 0 and today_pnl / balance * 100 <= -Config.MAX_DAILY_LOSS_PERCENT:
                self.logger.error(f"[A-CNH-03] Daily loss limit: {today_pnl:.2f}/{balance:.2f} = {today_pnl/balance*100:.2f}%")
                return None
            ...
            if genome_type in valid_genome_types:
                ...
                # FIX: GBP EA sends bid/ask at root level, not in tick_data
                tick_data = genome.get('tick_data', {})
            ...
            nova_detector = self.nova_detector  # FIX M-CNH-01: use singleton
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                # Calculate bid/ask from current price with typical CNHUSD spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 0.0010  # Half spread for USDCNH (typical spread = 0.0020 = 20 pips)
                    genome['bid'] = current_price - spread
                    genome['ask'] = current_price + spread
                    self.logger.debug(f"[Trinity-Prep] ✅ Calculated bid={genome['bid']:.5f} ask={genome['ask']:.5f} from price={current_price:.5f}")
                else:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # 🎯 BUILD timeframe_analysis from REAL indicators for Trinity consensus
            # Trinity needs this array to produce BUY/SELL votes (not just HOLD)
            # ═══════════════════════════════════════════════════════════════════
            if not genome.get('timeframe_analysis'):
                ind = genome.get('indicators', {}).get('current', {})
                rsi = float(ind.get('rsi', 50))
                adx = float(ind.get('adx', 20))
                atr = float(ind.get('atr', 0))
                ma_fast = float(ind.get('ma_fast', 0))
                ma_slow = float(ind.get('ma_slow', 0))
                macd = float(ind.get('macd', 0))
                # FIX: keys are 'bollinger_upper/lower/middle' NOT 'bb_upper/bb_lower/bb_middle'
                bb_upper = float(ind.get('bollinger_upper', ind.get('bb_upper', 0)))
                bb_lower = float(ind.get('bollinger_lower', ind.get('bb_lower', 0)))
                bb_middle = float(ind.get('bollinger_middle', ind.get('bb_middle', 0)))
                
                current_price = float(genome.get('bid', 0)) or float(genome.get('ask', 0))
                
                # Calculate Bollinger position (0=lower band, 1=upper band)
                boll_range = bb_upper - bb_lower if bb_upper > bb_lower else 1
                boll_position = (current_price - bb_lower) / boll_range if boll_range > 0 else 0.5
                boll_position = max(0, min(1, boll_position))
                
                # Determine MA cross direction
                ma_cross = 1 if ma_fast > ma_slow else (-1 if ma_fast < ma_slow else 0)
                
                # Mean reversion deviation from middle BB (proxy for EMA)
                deviation = abs(current_price - bb_middle) if bb_middle > 0 else 0
                
                # Build signal for H1 timeframe (primary TF for Red Dragon CNH)
                tf_analysis = []
                for tf_name in ['H1', 'M30', 'M15']:
                    ...
                    if rsi < 30:
                        ...
                        reasons.append(f"RSI({rsi:.0f})<42=BULLISH_ZONE")
                        buy_points += 7
                    elif rsi > 58:
                        reasons.append(f"RSI({rsi:.0f})>58=BEARISH_ZONE")
                        sell_points += 7
                    
                    # 2. ADX trend strength
                    if adx > 25:
                        reasons.append(f"ADX({adx:.0f})>25=STRONG_TREND")
                        strength += 10
                    elif adx > 20:
                        reasons.append(f"ADX({adx:.0f})>20=MODERATE")
                        strength += 5
                    else:
                        reasons.append(f"ADX({adx:.0f})<20=WEAK")
                    
                    # 3. MA Cross
                    if ma_cross > 0:
                        reasons.append("MA_FAST>SLOW=BULLISH")
                        buy_points += 10
                    elif ma_cross < 0:
                        reasons.append("MA_FAST<SLOW=BEARISH")
                        sell_points += 10
                    
                    # 4. MACD histogram
                    if macd > 0.0001:
                        reasons.append(f"MACD({macd:.5f})=BULL")
                        buy_points += 6
                    elif macd < -0.0001:
                        reasons.append(f"MACD({macd:.5f})=BEAR")
                        sell_points += 6
                    
                    # 5. Bollinger position (mean reversion for Red Dragon)
                    if boll_position < 0.15:
                        reasons.append(f"BOLL({boll_position:.2f})=OVERSOLD_EXTREME")
                        buy_points += 15
                    elif boll_position < 0.30:
                        reasons.append(f"BOLL({boll_position:.2f})=LOWER_ZONE")
                        buy_points += 8
                    elif boll_position > 0.85:
                        reasons.append(f"BOLL({boll_position:.2f})=OVERBOUGHT_EXTREME")
                        sell_points += 15
                    elif boll_position > 0.70:
                        reasons.append(f"BOLL({boll_position:.2f})=UPPER_ZONE")
                        sell_points += 8
                    
                    # 6. Mean reversion from BB middle (Red Dragon specialty)
                    if deviation > 0.003:  # >30 pips from middle
                        if current_price > bb_middle:
                            reasons.append(f"MEAN_REV: {deviation*10000:.0f}p ABOVE_MID")
                            sell_points += 12
                        else:
                            reasons.append(f"MEAN_REV: {deviation*10000:.0f}p BELOW_MID")
                            buy_points += 12
                    
                    # Determine final signal
                    if buy_points >= sell_points + 5:
                        signal = "BUY"
                        strength = min(95, 50 + buy_points)
                    elif sell_points >= buy_points + 5:
                        signal = "SELL"
                        strength = min(95, 50 + sell_points)
                    else:
                        signal = "HOLD"
                        strength = max(30, 50 - abs(buy_points - sell_points))
                    ...
                    # Add slight variation for secondary TFs (M30, M15 more sensitive)
                    if tf_name == 'H1':
                        rsi = rsi + (rsi - 50) * 0.1  # M30 amplifies RSI slightly
                    elif tf_name == 'M30':
                        rsi = rsi + (rsi - 50) * 0.15  # M15 amplifies more
                
                genome['timeframe_analysis'] = tf_analysis
                self.logger.info(f"[TF-Analysis] 🎯 Built {len(tf_analysis)} TF votes: " + 
                    " | ".join(f"{t['timeframe']}={t['signal']}@{t['strength']}%" for t in tf_analysis))
            ...
            # ═══ FIX C-CNH-01: ATR=0 CIRCUIT BREAKER ═══
            if atr_val == 0.0:
                with self.ipc.stats_lock:
                    self.ipc.stats['_atr_zero_count'] = self.ipc.stats.get('_atr_zero_count', 0) + 1
                    _atr_zeros = self.ipc.stats['_atr_zero_count']
                if _atr_zeros >= 5:
                    self.logger.warning(f"[C-CNH-01] ATR=0 x{_atr_zeros} consecutive — skipping to avoid divide-by-zero")
                    return None
            else:
                with self.ipc.stats_lock:
            ...
            # ═══════════════════════════════════════════════════════════════════════════════
            # 🏦 LANCE FORGER SOVEREIGN STRATEGY - Carry Reversal Analysis
            # When active, Lance Forger can ENHANCE or REFINE Trinity's decision
            # Focus: Bollinger reversals, Volume exhaustion, Carry differential, XAU hedge
            # ═══════════════════════════════════════════════════════════════════════════════
            lance_forger_active = False
            lance_forger_signal = None
            
            if Config.is_lance_forger_active() and LANCE_FORGER_AVAILABLE:
                try:
                    # Get or create Lance Forger instance
                    if Config.LANCE_FORGER_CONFIG is None:
                        import yaml
                        config_path = os.path.join(os.path.dirname(__file__), 'cnhconfig.yaml')
                        with open(config_path, 'r', encoding='utf-8') as f:
                            yaml_config = yaml.safe_load(f)
                        Config.LANCE_FORGER_CONFIG = get_lance_forger(yaml_config.get('lance_forger', {}))
                    
                    lance_forger = Config.LANCE_FORGER_CONFIG
                    
                    if lance_forger:
                        # Prepare market data for Lance Forger
                        market_data = {
                            'current_price': current_price,
                            'bid': genome.get('bid', current_price),
                            'ask': genome.get('ask', current_price),
                            'bollinger_upper': bb_upper_val,
                            'bollinger_middle': bb_middle_val,
                            'bollinger_lower': bb_lower_val,
                            'rsi': rsi_val,
                            'atr': atr_val,
                            'volume': genome.get('tick_data', {}).get('volume', 100),
                            'xau_price': genome.get('correlations', {}).get('xau_price', 0),
                            'xau_delta': genome.get('correlations', {}).get('xau_delta', 0)
                        }
                        
                        # Analyze with Lance Forger
                        lance_forger_signal = lance_forger.analyze(genome, market_data)  # 🔧 FIX: needs genome + indicators
                        lance_forger_active = True
                        
                        if lance_forger_signal:
                            # 🔧 FIX: ForgerSignal uses .direction (ForgeDirection enum), .confidence, .reasons
                            _lf_dir = lance_forger_signal.direction.value  # FORGE_UP/FORGE_DOWN/AWAIT
                            lf_action = 'BUY' if _lf_dir == 'FORGE_UP' else ('SELL' if _lf_dir == 'FORGE_DOWN' else 'HOLD')
                            lf_strength = lance_forger_signal.confidence
                            lf_reason = ', '.join(lance_forger_signal.reasons) if lance_forger_signal.reasons else 'No reasons'
                            
                            self.logger.info(f"🏦 [LANCE_FORGER] Signal: {lf_action} @ {lf_strength:.0%} | {lf_reason}")
                            
                            # If Lance Forger has a STRONG signal (>70%), consider it
                            if lf_strength >= 0.70:
                                if lf_action in ['BUY', 'SELL']:
                                    # Lance Forger agrees with Trinity - BOOST confidence
                                    if lf_action == trinity_decision:
                                        trinity_conf = min(95, trinity_conf + int(lf_strength * 15))
                                        self.logger.info(f"🏦 [LANCE_FORGER] CONFLUENCE: Boosted confidence to {trinity_conf}%")
                                    
                                    # 🔧 REBALANCED: Lance Forger can override HOLD→BUY/SELL
                                    # but ONLY with very strong signals (>=80%) and Trinity undecided
                                    elif trinity_decision == 'HOLD' or trinity_conf < 60:
                                        if lf_strength >= 0.80:
                                            # Strong LF signal + Trinity undecided = allow override
                                            trinity_decision = lf_action
                                            trinity_conf = max(trinity_conf, int(lf_strength * 70))
                                            self.logger.warning(f"🏦 [LANCE_FORGER] OVERRIDE: {lf_action}@{int(lf_strength*100)}% overrides HOLD (Trinity undecided)")
                                        else:
                                            self.logger.info(f"🏦 [LANCE_FORGER] INFO: LF suggests {lf_action}@{int(lf_strength*100)}% but strength < 80% - NOT overriding")
                                
                                # Lance Forger says PAUSE - always respect risk management
                                elif lf_action == 'PAUSE':
                                    if trinity_decision in ['BUY', 'SELL']:
                                        self.logger.warning(f"🏦 [LANCE_FORGER] RISK PAUSE: {lf_reason}")
                                        trinity_decision = 'HOLD'
                                        trinity_conf = 0
                            
                            # Inject Lance Forger TP/SL if it has a valid signal
                            if lance_forger_signal.take_profit and lance_forger_signal.stop_loss:
                                genome['lance_forger_tp'] = lance_forger_signal.take_profit
                                genome['lance_forger_sl'] = lance_forger_signal.stop_loss
                                genome['lance_forger_hedge'] = lance_forger_signal.hedge
                        else:
                            self.logger.debug(f"🏦 [LANCE_FORGER] No signal generated (conditions not met)")
                            
                except Exception as lf_err:
                    self.logger.error(f"🏦 [LANCE_FORGER] Error: {lf_err}")
                    lance_forger_active = False
            
            # Inject Lance Forger state into genome for dashboard
            genome['lance_forger_active'] = lance_forger_active
            genome['lance_forger_signal'] = ('BUY' if lance_forger_signal.direction.value == 'FORGE_UP' else ('SELL' if lance_forger_signal.direction.value == 'FORGE_DOWN' else 'HOLD')) if lance_forger_signal else 'N/A'
            ...
            _did_increment = False  # FIX C-CNH-02: track position increment for safe rollback
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-CNH-05: check direction-poison before stacking
                            _poison_key = f'dir_poison_until_{decision["action"]}'
                            _poison_until = self.ipc.stats.get(_poison_key, 0)
                            if _poison_until > time.time():
                                logger.warning(f"[A-CNH-05] Dir-poison active for {decision['action']} — {_poison_until - time.time():.0f}s remaining. Blocking stack.")
                                decision_for_kraken = decision.copy()
                                decision_for_kraken['action'] = 'HOLD'
                                ...
                                _did_increment = True  # FIX C-CNH-02
                        ...
                        # FIX A-CNH-05: check direction-poison before first entry
                        ...
                        if _poison_until_first > time.time():
                            logger.warning(f"[A-CNH-05] Dir-poison first-entry blocked for {decision['action']} — {_poison_until_first - time.time():.0f}s remaining.")
                            decision_for_kraken = decision.copy()
                            decision_for_kraken['action'] = 'HOLD'
                            decision_for_kraken['reason'] = f'dir_poison_{decision["action"]}'
                        else:
                            # 🔧 FIX: Check direction change and apply extra cooldown
                            if self.engine.last_attack_direction and decision['action'] != self.engine.last_attack_direction:
                                self.engine.last_direction_change_time = time.time()
                                logger.warning(f"[DIR CHANGE] 🔄 Direction changed: {self.engine.last_attack_direction} → {decision['action']} | Extra cooldown activated")
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    ...
                    _now = time.time()  # FIX A-CNH-01: atomic timestamp
                    with self.ipc.stats_lock:
                        self.last_trade_time = _now
                        self.engine.last_trade_time = _now

                    # FIX C-CNH-03: daily_trade_count under stats_lock
                    with self.ipc.stats_lock:
                        self.engine.daily_trade_count += 1
                    # 🔧 FIX: Track last attack direction for direction change detection
                    self.engine.last_attack_direction = decision_for_kraken['action']
                    
                    self.logger.warning(f"[COOLDOWN_RESET] ⚡ {decision_for_kraken['action']} CONFIRMED BY KRAKEN! Cooldown timer restarted (next attack in {self.engine.cooldown_seconds}s) | Daily trades: {self.engine.daily_trade_count}/{Config.LANCE_FORGER_MAX_DAILY_TRADES}")
                    ...
                    try:
                        ...
                        state_key = self.engine.rl_agent.discretize_state(
                            ...
                            atr=genome.get('atr', 0.0100),  # 100 pips default for USDCNH
                            volatility=genome.get('volatility', 0.0100),  # 100 pips default
                    ...
                    # FIX C-CNH-02: guarded rollback — only rollback if THIS call incremented
                    if _did_increment:
                        with self.ipc.stats_lock:
    ...
    def run(self):
        ...
        start_time = time.time()
        last_position_reconcile = time.time()  # FIX A-CNH-04
        ...
        try:
            while self.running:
                ...
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)  # No response in 5 seconds
                ...
                # EA sends ~4 genomes/sec but processing takes longer.
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        _tmp_path = 'quantum_stats.json.tmp'  # FIX M-CNH-02: atomic write
                ...
                # FIX A-CNH-04: POSITION RECONCILE every 60s
    ...
    def _update_dashboard_from_genome(self, genome):
        ...
        try:
            # Extract metadata
            metadata = genome.get('metadata', {})
            symbol = metadata.get('symbol', 'CNHUSD')
...
_CNH_PID_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cnhquantum_core.pid')

def _acquire_cnh_singleton() -> bool:
    """Return True if this process should run; False if another cnhquantum_core.py
    is already alive. Never terminates sibling processes — prevents death-spiral."""
    try:
        import psutil as _psutil
        if os.path.exists(_CNH_PID_FILE):
            try:
                with open(_CNH_PID_FILE, 'r') as _f:
                    _old_pid = int(_f.read().strip())
                _old_proc = _psutil.Process(_old_pid)
                _cmdline  = ' '.join(_old_proc.cmdline())
                if 'cnhquantum_core' in _cmdline and _old_proc.is_running():
                    print(f"[SINGLETON] cnhquantum_core.py already running as PID={_old_pid}. "
                          f"This duplicate (PID={os.getpid()}) will exit cleanly.")
                    return False
            except Exception:
                pass
        with open(_CNH_PID_FILE, 'w') as _f:
                ...
                if os.path.exists(_CNH_PID_FILE):
                    with open(_CNH_PID_FILE, 'r') as _f:
                        if _f.read().strip() == str(os.getpid()):
                            os.remove(_CNH_PID_FILE)
            except Exception:
                pass
        atexit.register(_release_pid)
        return True
    except Exception as _e:
        print(f"[SINGLETON] Warning: PID lock unavailable ({_e}). Proceeding.")
        return True
# ─────────────────────────────────────────────────────────────────────────────

def main():
    """Main entry"""
    if not _acquire_cnh_singleton():
        sys.exit(0)
...
# ═══════════════════════════════════════════════════════════════════════════════════
# 🏆 QA AUDIT CERTIFICATE — cnhquantum_core.py (USDCNH / Red Dragon)
...
#
# BUG-CNH-CRIT-01 │ PBOC fixing window enforcement (01:15-02:00 UTC)
#                 │ Config had PBOC_FIXING_BLOCK_START/END defined but NEVER checked.
#                 │ Mean reversion trading during PBOC intervention = guaranteed stops.
#                 │ Now: process_genome returns None during fixing window.
#
# M-CNH-01        │ NOVAMarketStateDetector singleton in __init__; use self.nova_detector
#                 │ Eliminates per-genome instantiation (was L15215)
#
# A-CNH-02        │ Consecutive-losses circuit breaker at top of process_genome
#                 │ Halts trading for LOSS_HALT_DURATION_SECONDS after MAX_CONSECUTIVE_LOSSES
#
# A-CNH-03        │ Daily-loss circuit breaker at top of process_genome
#                 │ Returns None when today_pnl/balance <= -MAX_DAILY_LOSS_PERCENT (1.5%)
#
# C-CNH-01        │ ATR=0 circuit breaker; _atr_zero_count under stats_lock
#                 │ Returns None after 5 consecutive ATR=0 ticks
#                 │ CNH off-hours gaps increase ATR=0 risk vs other pairs
#
# C-CNH-02        │ _did_increment = False before position lock; True only on APPROVE
#                 │ Guarded rollback: if _did_increment → rollback on Kraken failure
#
# A-CNH-05        │ dir_poison_until_{BUY|SELL} check (via ipc.stats) in BOTH
#                 │ stacking-approve and first-trade-approve branches
#
# A-CNH-01        │ Atomic last_trade_time: _now = time.time(); stats_lock block
#                 │ sets both self.last_trade_time and self.engine.last_trade_time atomically
#
# C-CNH-03        │ daily_trade_count += 1 wrapped in stats_lock
#                 │ Fixes race condition from concurrent portfolio_sync + genome threads
#
# A-CNH-04        │ last_position_reconcile timer in run() — resets stale open_positions
#                 │ (>600s without activity) every 60s
#                 │ CRITICAL: MAX_CONCURRENT_TRADES=1 means a stuck open_positions=1
#                 │ permanently blocks ALL new entries.
#
# M-CNH-02        │ Atomic quantum_stats.json write via .tmp + os.replace()
#                 │ Prevents dashboard reading truncated JSON
#
# CNH-SPECIFIC NOTES:
#   - MAX_CONCURRENT_TRADES = 1 (mean reversion, only 1 position at a time)
#   - Strategy is RED DRAGON — pure mean reversion, RSI extremes + BB bands
#   - PBOC fix is the #1 tail risk for USDCNH — now properly blocked
#   - spread fallback 0.0010 is correct (≈1 pip for a ~7.2-priced pair)
#   - Multi-TF genome types were already in valid_genome_types ✅
#   - LLM_WEIGHTS correctly documented as "CNH (PBOC-driven pair)"
#
# SYNTAX VALIDATION: python -m py_compile cnhquantum_core.py → SYNTAX OK ✅
# ═══════════════════════════════════════════════════════════════════════════════════
