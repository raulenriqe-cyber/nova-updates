...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 💷 CNHUSD CONFIG LOADER - CENTRALIZED CONFIGURATION           ║
║                                                                              ║
║  Loads CNHUSD ports and parameters from cnhconfig.yaml                       ║
║  Single source of truth for all CNHUSD system configuration                  ║
║                                                                              ║
...
class cnhconfig:
    """Centralized configuration manager for CNHUSD system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from cnhconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"cnhconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('sockets.trinity_port') -> 8766
        """
        config = cls.load()
...
def get_trinity_port() -> int:
    """Get Trinity port for CNHUSD"""
    return cnhconfig.get_port('trinity')

def get_kraken_port() -> int:
    """Get Kraken port for CNHUSD"""
    return cnhconfig.get_port('kraken')

def get_llm_port(llm_num: int) -> int:
    """Get port for specific LLM (1-10)"""
    return cnhconfig.get_port(f'llm{llm_num}')

def get_ollama_port() -> int:
    """Get Ollama port (shared)"""
    return cnhconfig.get_port('ollama')

def get_sound_port() -> int:
    """Get Sound alert port"""
    return cnhconfig.get_port('sound')

def get_injector_port() -> int:
    """Get Injector port (MT5 -> Python)"""
    return cnhconfig.get_port('injector')

def get_executor_port() -> int:
    """Get Executor port (Python -> MT5)"""
    return cnhconfig.get_port('executor')
...
def get_all_llm_ports() -> Dict[str, int]:
    ...
    for i in range(1, 11):
        try:
            ports[f'llm{i}'] = cnhconfig.get_port(f'llm{i}')
        except:
            pass
    return ports
...
def get_symbol() -> str:
    """Get market symbol"""
    return cnhconfig.get('symbol', 'CNHUSD')

def get_timeframe() -> str:
    """Get primary timeframe"""
    return cnhconfig.get('primary_timeframe', 'M5')

def get_sl_range() -> tuple:
    """Get SL min/max USD"""
    return (
        cnhconfig.get('trading.scalp_sl', 150),
        cnhconfig.get('trading.swing_sl', 800)
    )

def get_tp_range() -> tuple:
    """Get TP min/max USD"""
    return (
        cnhconfig.get('trading.scalp_tp', 100),
        cnhconfig.get('trading.swing_tp', 600)
    )
...
def get_min_confidence() -> int:
    ...
    return cnhconfig.get('confidence.minimum_signal', 70)

def get_max_positions() -> int:
    """Get max positions"""
    return cnhconfig.get('trading.max_positions', 3)

def get_pip_value() -> float:
    """Get pip value for CNHUSD"""
    return cnhconfig.get('trading.pip_value', 1.0)

def get_volatility_thresholds() -> dict:
    """Get ATR thresholds for volatility regimes"""
    return {
        'low': cnhconfig.get('indicators.atr.low_volatility', 200),
        'normal': cnhconfig.get('indicators.atr.normal_volatility', 500),
        'high': cnhconfig.get('indicators.atr.high_volatility', 1200),
        'extreme': cnhconfig.get('indicators.atr.extreme_volatility', 2500)
    }
...
if __name__ == '__main__':
    print("═" * 80)
    print("₿ CNHUSD CONFIG LOADER - TEST")
    print("═" * 80)
