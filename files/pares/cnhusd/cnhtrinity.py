...
# Add parent directory to path for shared modules (trinity_dashboard_v3, etc.)
...
class TrinityConsensus:
    def __init__(self):
        log.info('[TRINITY] BANK-GRADE CONSENSUS v3.0 Initialized - CNHUSD')
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD) - CNHUSD PORTS (87xx)
        self.core_llm_ports = {
            'BAYESIAN': 8755,     # CNHUSD LLM1
            'TECHNICAL': 8757,    # CNHUSD LLM2
            'CHART': 8758,        # CNHUSD LLM3
            'RISK': 8759,         # CNHUSD LLM4
            'SUPREME': 8761,      # CNHUSD LLM5
            'SMART_MONEY': 8762   # LLM6 - Smart Money Oracle (CNHUSD)
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution) - CNHUSD PORTS
        self.enhancement_llm_ports = {
            'OCULUS': 8763,    # CNHUSD LLM7 - Data Quality Validator
            'CHRONOS': 8764,   # CNHUSD LLM8 - Timing Optimizer  
            'PREDATOR': 8765,  # CNHUSD LLM9 - Execution Engine
            'NOVA': 8760,   # CNHUSD LLM10 - Guardian Protector
            'STRATEGIST': 8767,   # CNHUSD LLM11 - Strategist Guru
            'SENTINEL': 8768      # CNHUSD LLM12 - Quality Guardian
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 8763, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 8764, genome)
        if chronos_response:
        ...
        rr_ratio = 0.0
        predator_response = self._query_enhancement_llm('PREDATOR', 8765, enhanced_genome)
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] ?? Make sure all 6 LLMs are running on ports 8755, 8757, 8758, 8759, 8761, 8762")
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for CNHUSD)
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            try:
                ...
                log.error(f"[NOVA CONNECTION] ? No se pudo conectar a puerto 8765")
                return self._fallback_nova_response()
...
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                self.dashboard = QuantumDashboard(port=8767)  # CNHUSD dashboard port
    ...
    def run(self, port=8766):
        """Start TCP server on specified port (CNHUSD Trinity on 8766)"""
...
def run_server(port=8766):
    """Run Trinity consensus server with optional dashboard - CNHUSD"""
...
if __name__ == '__main__':
    try:
        log.info('[TRINITY] -----------------------------------------')
        log.info('[TRINITY] TRINITY CONSENSUS ENGINE v1.0 - CNHUSD')
        log.info('[TRINITY] Consensus Orchestrator - 10 LLM Integration')
        log.info('[TRINITY] -----------------------------------------')
        run_server(port=8766)  # CNHUSD Trinity port
