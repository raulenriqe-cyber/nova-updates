...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 8752)
  - Audio announcements via sound.py (CNHUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='cnhconfig.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 8752 (not 8756 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 8752)
            self.symbols = config.get('core_engine', {}).get('symbols', ['CNHUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 8752  # Default to 8752, NOT 8756
            self.symbols = ['CNHUSD']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0050  # [NOVA AUDIT F-12] 50-pip fallback USDCNH (pip=0.0001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0030  # [NOVA AUDIT F-12] 30-pip fallback USDCNH
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'CNHUSD')
            ...
            # Validate symbol
            valid_symbols = {'CNHUSD', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'CNHUSD', 'GBP',
                           'USDJPY', 'JPY', 'CNHUSD', 'GBP', 'ETHUSD', 'ETH'}
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('cnhconfig.yaml')
        kraken.run()
