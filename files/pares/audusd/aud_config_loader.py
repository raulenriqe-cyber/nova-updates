...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 💷 AUDUSD CONFIG LOADER - CENTRALIZED CONFIGURATION           ║
║                                                                              ║
║  Loads AUDUSD ports and parameters from audconfig.yaml                       ║
...
class audconfig:
    """Centralized configuration manager for AUDUSD system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from audconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"audconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('sockets.trinity_port') -> 8566
        """
        config = cls.load()
...
def get_trinity_port() -> int:
    """Get Trinity port for AUDUSD"""
    return audconfig.get_port('trinity')

def get_kraken_port() -> int:
    """Get Kraken port for AUDUSD"""
    return audconfig.get_port('kraken')

def get_llm_port(llm_num: int) -> int:
    """Get port for specific LLM (1-10)"""
    return audconfig.get_port(f'llm{llm_num}')

def get_ollama_port() -> int:
    """Get Ollama port (shared)"""
    return audconfig.get_port('ollama')

def get_sound_port() -> int:
    """Get Sound alert port"""
    return audconfig.get_port('sound')

def get_injector_port() -> int:
    """Get Injector port (MT5 -> Python)"""
    return audconfig.get_port('injector')

def get_executor_port() -> int:
    """Get Executor port (Python -> MT5)"""
    return audconfig.get_port('executor')
...
def get_all_llm_ports() -> Dict[str, int]:
    ...
    for i in range(1, 11):
        try:
            ports[f'llm{i}'] = audconfig.get_port(f'llm{i}')
        except:
            pass
    return ports
...
def get_symbol() -> str:
    """Get market symbol"""
    return audconfig.get('symbol', 'AUDUSD')

def get_timeframe() -> str:
    """Get primary timeframe"""
    return audconfig.get('primary_timeframe', 'M5')

def get_sl_range() -> tuple:
    """Get SL min/max USD"""
    return (
        audconfig.get('trading.scalp_sl', 150),
        audconfig.get('trading.swing_sl', 800)
    )

def get_tp_range() -> tuple:
    """Get TP min/max USD"""
    return (
        audconfig.get('trading.scalp_tp', 100),
        audconfig.get('trading.swing_tp', 600)
    )
...
def get_min_confidence() -> int:
    ...
    return audconfig.get('confidence.minimum_signal', 70)

def get_max_positions() -> int:
    """Get max positions"""
    return audconfig.get('trading.max_positions', 3)

def get_pip_value() -> float:
    """Get pip value for AUDUSD"""
    return audconfig.get('trading.pip_value', 1.0)

def get_volatility_thresholds() -> dict:
    """Get ATR thresholds for volatility regimes"""
    return {
        'low': audconfig.get('indicators.atr.low_volatility', 200),
        'normal': audconfig.get('indicators.atr.normal_volatility', 500),
        'high': audconfig.get('indicators.atr.high_volatility', 1200),
        'extreme': audconfig.get('indicators.atr.extreme_volatility', 2500)
    }
...
if __name__ == '__main__':
    print("═" * 80)
    print("₿ AUDUSD CONFIG LOADER - TEST")
    print("═" * 80)
