...
class LLM6_SmartMoney:
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 0.6500))  # 🔧 AUDUSD default
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (AUDUSD 75xx range)"""
        return {
            'trinity_port': 8566,
            'kraken_port': 8552,
            'executor_port': 8556,
...
def run_tcp_server():
    ...
    # Load port from audconfig.yaml
    try:
        with open('audconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 8562)
    except Exception as e:
        port = 8562  # AUDUSD LLM6 Smart Money Oracle
