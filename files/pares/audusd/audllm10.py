...
║  Ports: 8560 (AUDUSD) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=8560, symbol='AUDUSD'):
        self.port = port  # AUDUSD LLM10 SENTINEL
        self.symbol = symbol
    ...
    def _audit_data_completeness(self, genome, trinity_data):
        ...
        try:
            ...
            if decision == 'BUY' and usd_direction == 'DOWN':
                score += 2  # AUDUSD sube cuando USD baja
            elif decision == 'SELL' and usd_direction == 'UP':
                score += 2  # AUDUSD baja cuando USD sube
            elif decision == 'BUY' and usd_direction == 'UP':
                score -= 2
                alerts.append(("DATA_CONTRADICTION", "MEDIUM",
                             "Buying AUDUSD while USD rising (negative correlation)",
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # AUDUSD tends to fall on high VIX (flight to USD)
            else:
                score += 0
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para AUDUSD: correlación con DXY
            if symbol == 'AUDUSD':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para AUDUSD
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for AUDUSD - caution on long trades'
                        })
...
if __name__ == '__main__':
    ...
    # AUDUSD LLM10 SENTINEL uses port 8560
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 8560
    symbol = 'AUDUSD'
