...
║  Port: 8559 (TCP Server) - Dynamic Risk Management with AI Learning          ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
            ...
            if has_prior:
                ...
                try:
                    ...
                    if fallback_macd_hist > 0:
                        bullish_signals += 1
                        prior_reasons.append(f"MACD_H({fallback_macd_hist:.4f})>0=BULL")
                    elif fallback_macd_hist < 0:
                        bearish_signals += 1
                        prior_reasons.append(f"MACD_H({fallback_macd_hist:.4f})<0=BEAR")
                    ...
                    log.info(f"[LLM4 FALLBACK] NO LLMs ready! RSI={fallback_rsi:.1f} MACD={fallback_macd:.4f} HIST={fallback_macd_hist:.4f} → {prior_direction_hint} (Bull:{bullish_signals} Bear:{bearish_signals})")
...
def run_tcp_server():
    ...
    try:
        ...
        port = config.get('llm_settings', {}).get('llm4_port', 8559)
    except:
        port = 8559  # LLM4 is RISK on port 8559
