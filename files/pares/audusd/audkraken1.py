...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 8552)
  - Audio announcements via sound.py (AUDUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='audconfig.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 8552 (not 8556 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 8552)
            self.symbols = config.get('core_engine', {}).get('symbols', ['AUDUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 8552  # Default to 8552, NOT 8556
            self.symbols = ['AUDUSD']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0050  # [NOVA AUDIT F-12] 50-pip fallback AUDUSD (pip=0.0001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0030  # [NOVA AUDIT F-12] 30-pip fallback AUDUSD
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'AUDUSD')
            ...
            # Validate symbol
            valid_symbols = {'AUDUSD', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'AUDUSD', 'GBP',
                           'USDJPY', 'JPY', 'AUDUSD', 'GBP', 'ETHUSD', 'ETH'}
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('audconfig.yaml')
        kraken.run()
