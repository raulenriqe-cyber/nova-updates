...
class TrinityConsensus:
    def __init__(self):
        log.info('[TRINITY] BANK-GRADE CONSENSUS v3.0 Initialized - NZDUSD')
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD) - NZDUSD PORTS (75xx)
        self.core_llm_ports = {
            'BAYESIAN': 9255,     # NZDUSD LLM1
            'TECHNICAL': 9257,    # NZDUSD LLM2
            'CHART': 9258,        # NZDUSD LLM3
            'RISK': 9259,         # NZDUSD LLM4
            'SUPREME': 9261,      # NZDUSD LLM5
            'SMART_MONEY': 9262   # LLM6 - Smart Money Oracle (NZDUSD)
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution) - NZDUSD PORTS
        self.enhancement_llm_ports = {
            'OCULUS': 9263,    # NZDUSD LLM7 - Data Quality Validator
            'CHRONOS': 9264,   # NZDUSD LLM8 - Timing Optimizer  
            'PREDATOR': 9265,  # NZDUSD LLM9 - Execution Engine
            'NOVA': 9260,   # NZDUSD LLM10 - Guardian Protector
            'STRATEGIST': 9267,   # NZDUSD LLM11 - Strategist Guru
            'SENTINEL': 9268      # NZDUSD LLM12 - Quality Guardian
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 9263, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 9264, genome)
        if chronos_response:
        ...
        predator_response = self._query_enhancement_llm('PREDATOR', 9265, enhanced_genome)
        rr_ratio = 0.0  # 🔧 FIX CAT-125: inicializar antes del condicional para evitar UnboundLocalError
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] ?? Make sure all 6 LLMs are running on ports 9255, 9257, 9258, 9259, 9261, 9262")
        ...
        nova_multiplier = nova_response.get('quality_multiplier', 1.0)  # ⭐ FIX: default was 0.7, now 1.0 (neutral)
    ...
    def _query_enhancement_llm(self, llm_name: str, port: int, genome: dict) -> dict:
        ...
        try:
            ...
            # Send request
            request_json = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-122: default=str para numpy types
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for NZDUSD)
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            try:
                ...
                log.error(f"[NOVA CONNECTION] ? No se pudo conectar a puerto 9265")
                return self._fallback_nova_response()
    ...
    def _fallback_nova_response(self) -> dict:
        ...
        return {
            'is_valid': True,
            'quality_multiplier': 1.0,  # ⭐ FIX: was 0.7 (penalizaba trades), ahora neutral 1.0
            'audit_score': 70,
...
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                self.dashboard = QuantumDashboard(port=7567)  # NZDUSD dashboard port
    ...
    def run(self, port=9266):
        """Start TCP server on specified port (NZDUSD Trinity on 9266)"""
...
def run_server(port=9266):
    """Run Trinity consensus server with optional dashboard - NZDUSD"""
...
if __name__ == '__main__':
    try:
        log.info('[TRINITY] -----------------------------------------')
        log.info('[TRINITY] TRINITY CONSENSUS ENGINE v1.0 - NZDUSD')
        log.info('[TRINITY] Consensus Orchestrator - 10 LLM Integration')
        log.info('[TRINITY] -----------------------------------------')
        run_server(port=9266)  # NZDUSD Trinity port
