"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - NZDUSD Market Sentiment                         ║
...
║  Optimizado para NZDUSD - New Zealand Dollar / US Dollar                      ║
║  � Dairy + Commodity Currency | RBNZ + China + Dairy + Agriculture Focus     ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-NZD")
...
# Símbolos a monitorear - NZDUSD FOCUSED + correlated pairs
SYMBOLS = ["NZDUSD", "NZDUSD", "AUDUSD"]

# Fuentes confiables para forex + New Zealand/Asia-Pacific sources
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "forexlive", "fxstreet", "dailyfx",
    "investing.com", "tradingview", "marketwatch", "cnbc",
    "financialjuice", "forexfactory", "ft.com",
    "rnz.co.nz", "nzherald.co.nz", "stuff.co.nz", "interest.co.nz",
    "rbnz.govt.nz", "stats.govt.nz", "scmp.com", "nikkei.com",
    "globaldairytrade.info", "fonterra.com", "dairynz.co.nz"
]

# Keywords por símbolo - NZDUSD OPTIMIZED for Commodity/Carry Currency
KEYWORDS = {
    "NZDUSD": [
        "NZDUSD", "kiwi dollar", "NZD/USD", "new zealand dollar news",
        "RBNZ decision", "reserve bank new zealand", "kiwi forecast",
        "new zealand dollar", "fed kiwi", "kiwi strength",
        "kiwi trading", "kiwi weakness", "new zealand economy",
        "new zealand inflation", "new zealand employment", "new zealand gdp",
        "new zealand retail sales", "new zealand CPI", "new zealand trade balance",
        "RBNZ rate decision", "RBNZ Orr", "reserve bank new zealand rate",
        "dairy prices", "Fonterra GDT auction", "whole milk powder price",
        "dairy commodity prices", "commodity prices today", "dairy price NZD",
        "China PMI", "China GDP", "China manufacturing", "China stimulus",
        "China trade data", "China property", "Beijing economic policy",
        "carry trade NZD", "risk sentiment forex", "NZX 50",
        "new zealand bond yield", "wellington session forex",
        "NZDUSD rally", "NZDUSD crash", "kiwi forex", "commodity currency"
    ],
    "NZDUSD": [
        "NZDUSD", "kiwi dollar", "NZD/USD", "new zealand dollar news",
        "RBNZ decision", "reserve bank new zealand", "kiwi forecast",
    ...
    "AUDUSD": [
        ...
        "aussie forecast", "australian economy", "RBA decision",
        "AUD carry trade", "aussie strength", "australian dollar forecast",
        "AUDUSD rally", "AUDUSD crash", "aussie forex trading"
    ]
}
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        Context-aware: NZDUSD is a commodity/carry currency.
        - China growth positive → B (NZD strengthens)
        - Dairy prices/agriculture UP → B
        - RBNZ hawkish → B
        - Risk-on sentiment → B
        - China slowdown → M
        - Commodity crash → M
        ...
        symbol = news_item.get('symbol', 'NZDUSD')
...
Context: NZDUSD is a commodity/carry currency. New Zealand Dollar strengthens with:
- China growth, stimulus, or strong PMI data
- Rising dairy, agriculture, whole milk powder prices
- RBNZ hawkish stance or rate hikes
- Risk-on sentiment in global markets
- Strong New Zealand economic data (employment, GDP, CPI)

New Zealand Dollar weakens with:
- China slowdown, property crisis, weak PMI
- Falling commodity prices
- RBNZ dovish stance or rate cuts
...
B = Bullish (NZD price likely to go UP)
M = Bearish (NZD price likely to go DOWN)  
...
class NewsStorage:
    ...
    def _get_today_file(self) -> Path:
        ...
        return self.news_dir / f"news_nzd_{today}.json"
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        NZDUSD focused with cross-asset correlation (NZDUSD, XAUUSD).
        """
        data = self.load_today()
        ...
        # NZDUSD-specific confidence calculation
        nzd_score = abs(sentiment.get('NZDUSD', {}).get('score', 0))
        nzd_total = sentiment.get('NZDUSD', {}).get('total', 0)
        nzd_conf = min(1.0, nzd_score * (1 + min(nzd_total, 10) * 0.1))
        
        # Boost confidence if correlated asset (AUD sister pair) agrees with NZD direction
        nzd_sent = sentiment.get('NZDUSD', {}).get('sentiment', 'N')
        aud_sent = sentiment.get('XAUUSD', {}).get('sentiment', 'N')
        nzd_sent = sentiment.get('NZDUSD', {}).get('sentiment', 'N')
        
        correlation_boost = 0
        if nzd_sent == aud_sent and nzd_sent != 'N':
            correlation_boost += 0.05  # AUD sister pair confirms NZD direction
        if nzd_sent == nzd_sent and nzd_sent != 'N':
            correlation_boost += 0.05  # NZD (sister currency) confirms
        
        nzd_conf = min(1.0, nzd_conf + correlation_boost)
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para NZDUSD.
    Orquesta: Fetch → Evaluate → Store → Provide to LLM
    🦘 Dairy/Commodity currency focus: RBNZ, China, Dairy, Agriculture, Risk Sentiment
    """
    ...
    def fetch_and_evaluate(self) -> Dict:
        """
        Flujo completo:
        1. Busca noticias en DuckDuckGo (NZDUSD, NZDUSD, XAUUSD)
        ...
        """
        logger.info("=" * 60)
        logger.info("🦘 NZDUSD NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        ...
        NZDUSD-aware: uses cross-asset correlation for higher confidence.
        """
        sentiment = self.storage.get_sentiment_for_llm()
        s = sentiment.get(symbol, 'N')
        
        if symbol == 'NZDUSD':
            conf = sentiment.get('confidence_NZDUSD', sentiment.get('confidence', 0.5))
        else:
...
def main():
    ...
    print("=" * 70)
    print("🦘 NEWS INTELLIGENCE ENGINE - NZDUSD")
    print("   Dairy & Commodity Currency | RBNZ + China + Dairy + Agriculture")
    ...
    print(f"   Confidence: {sentiment.get('confidence_NZDUSD', 0):.2f}")
    print("=" * 70)
...
def get_news_sentiment() -> Dict:
    ...
    Usage:
        ...
        if sentiment['NZDUSD'] == 'B':
            # Bias bullish para NZD - commodities strong
    """
    global _news_engine
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('NZDUSD')
