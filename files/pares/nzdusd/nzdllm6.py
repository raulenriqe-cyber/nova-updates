...
class LLM6_SmartMoney:
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 0.6500))  # 🔧 NZDUSD default
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (NZDUSD 75xx range)"""
        return {
            'trinity_port': 9266,
            'kraken_port': 9252,
            'executor_port': 9256,
...
def run_tcp_server():
    ...
    # Load port from nzdconfig.yaml
    try:
        with open('nzdconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 9262)
    except Exception as e:
        port = 9262  # NZDUSD LLM6 Smart Money Oracle
