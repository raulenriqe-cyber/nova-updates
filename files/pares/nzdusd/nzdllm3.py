...
║  Port: 9258 (TCP Server) - MATHEMATICAL PRECISION ENHANCED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'NZDUSD').upper().strip()
            ...
            # RSI > 60 + momentum > 0.5 + context > 0.8 + ADX > 25 = BUY (momentum override)
                ...
                confidence = min(88.0, 60.0 + (50 - rsi) * 0.4)
                pattern = 'MOMENTUM_DOWN'
                reason = f'RSI débil ({rsi:.0f}) + Momentum DOWN + Context {context_score:.1f} + ADX {adx:.0f} | {" | ".join(reason_parts)}' if reason_parts else f'Momentum DOWN'
                log.info(f"[LLM3] 🔴 MOMENTUM DOWN (RSI={rsi:.1f}, mom={price_mom_5:.2f}, ctx={context_score:.1f}, ADX={adx:.0f}) → SELL")
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port=9258):  # FIXED: LLM3 is 9258
        self.host = host
        self.port = port
...
def main():
    ...
    try:
        # Cargar config
        port = 9258  # LLM3 is on 9258
        try:
            with open('nzdconfig.yaml', 'r') as f:
                config = yaml.safe_load(f) or {}
            port = config.get('llm_settings', {}).get('llm3_port', 9258)
        except:
            pass
