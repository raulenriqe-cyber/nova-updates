...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 9252)
  - Audio announcements via sound.py (NZDUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='nzdconfig.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 9252 (not 9256 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 9252)
            self.symbols = config.get('core_engine', {}).get('symbols', ['NZDUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 9252  # Default to 9252, NOT 9256
            self.symbols = ['NZDUSD']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0050  # [NOVA AUDIT F-12] 50-pip fallback NZDUSD (pip=0.0001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0030  # [NOVA AUDIT F-12] 30-pip fallback NZDUSD
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'NZDUSD')
            ...
            # Validate symbol
            valid_symbols = {'NZDUSD', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'NZDUSD', 'GBP',
                           'USDJPY', 'JPY', 'NZDUSD', 'GBP', 'ETHUSD', 'ETH'}
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('nzdconfig.yaml')
        kraken.run()
