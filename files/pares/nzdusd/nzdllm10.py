...
║  Ports: 9260 (NZDUSD) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=9260, symbol='NZDUSD'):
        self.port = port  # NZDUSD LLM10 SENTINEL
        self.symbol = symbol
    ...
    def _audit_data_completeness(self, genome, trinity_data):
        ...
        try:
            ...
            if decision == 'BUY' and usd_direction == 'DOWN':
                score += 2  # NZDUSD sube cuando USD baja
            elif decision == 'SELL' and usd_direction == 'UP':
                score += 2  # NZDUSD baja cuando USD sube
            elif decision == 'BUY' and usd_direction == 'UP':
                score -= 2
                alerts.append(("DATA_CONTRADICTION", "MEDIUM",
                             "Buying NZDUSD while USD rising (negative correlation)",
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # NZDUSD tends to fall on high VIX (flight to USD)
            else:
                score += 0
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # Calculate multiplier (0-1, quality_score/100)
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para NZDUSD: correlación con DXY
            if symbol == 'NZDUSD':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para NZDUSD
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for NZDUSD - caution on long trades'
                        })
    ...
    def run_server(self):
        """HTTP Server on port 9260 — Trinity connects via requests.post"""
        nova_instance = self
        ...
        class NovaHTTPHandler(BaseHTTPRequestHandler):
            def do_POST(self):
                try:
                    ...
                    result = nova_instance.analyze(request_data)
                    ...
                    log.warning(f'[NOVA-HTTP] Error: {e}')
                    self.send_response(500)
                    self.end_headers()
            def log_message(self, fmt, *args):
                pass  # Suppress default HTTP log spam

        server = HTTPServer(('127.0.0.1', self.port), NovaHTTPHandler)
        log.info(f'[NOVA] HTTP server listening on port {self.port} (Trinity HTTP POST /audit)')
        try:
...
if __name__ == '__main__':
    ...
    # NZDUSD LLM10 SENTINEL uses port 9260
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 9260
    symbol = 'NZDUSD'
