...
_log_file = f'logs/llm1_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
_file_handler = logging.FileHandler(_log_file, encoding='utf-8')
_stream_handler = logging.StreamHandler()
_formatter = logging.Formatter("%(asctime)s | LLM1_EXPERTO | %(message)s", datefmt='%H:%M:%S')
...
class LLM1Experto:
    ...
    # ============================================================
    # 🧠 NUEVA INTELIGENCIA DE CONTEXTO - Mayor Highs/Lows
    # Similar a XAUUSD LLM1 para mejor análisis de tendencias
    # ============================================================
    def _analyze_price_structure(self, closes, highs, lows):
        """
        Analiza estructura HH/HL/LL/LH para confirmar tendencias
        Returns: {'structure': str, 'confidence': float, 'reason': str}
        """
        if len(closes) < 15 or len(highs) < 15 or len(lows) < 15:
            return {'structure': 'INSUFFICIENT_DATA', 'confidence': 0, 'reason': 'Need 15+ candles'}
        
        # Analizar últimas 15 velas
        recent_closes = np.array(closes[-15:])
        recent_highs = np.array(highs[-15:])
        recent_lows = np.array(lows[-15:])
        
        # Dividir en 3 segmentos de 5 velas
        seg1_high = max(recent_highs[0:5])
        seg2_high = max(recent_highs[5:10])
        seg3_high = max(recent_highs[10:15])
        
        seg1_low = min(recent_lows[0:5])
        seg2_low = min(recent_lows[5:10])
        seg3_low = min(recent_lows[10:15])
        
        # Detectar HH (Higher Highs) y HL (Higher Lows)
        hh_detected = (seg2_high > seg1_high) and (seg3_high > seg2_high)
        hl_detected = (seg2_low > seg1_low) and (seg3_low > seg2_low)
        
        # Detectar LL (Lower Lows) y LH (Lower Highs)
        ll_detected = (seg2_low < seg1_low) and (seg3_low < seg2_low)
        lh_detected = (seg2_high < seg1_high) and (seg3_high < seg2_high)
        
        # Calcular magnitud del movimiento
        hh_magnitude = (seg3_high - seg1_high) / seg1_high if hh_detected else 0
        hl_magnitude = (seg3_low - seg1_low) / seg1_low if hl_detected else 0
        ll_magnitude = (seg1_low - seg3_low) / seg1_low if ll_detected else 0
        lh_magnitude = (seg1_high - seg3_high) / seg1_high if lh_detected else 0
    ...
    def _analyze_candle_narrative(self, closes, opens, highs, lows):
        """
        Analiza narrativa de últimas 10-20 velas
        Returns: 'BREAKOUT_PHASE' | 'ACCUMULATION_PHASE' | 'DISTRIBUTION_PHASE' | 'TRENDING_PHASE'
        """
        if len(closes) < 20:
            return 'UNKNOWN'
    ...
    def _analyze_multi_candle_context(self, closes, highs, lows):
        """
        Análisis de últimas 20 velas en 4 secciones
        Returns: {'context': str, 'confidence': int, 'sections': list, 'reason': str}
        """
        if len(closes) < 20:
            return {'context': 'INSUFFICIENT_DATA', 'confidence': 0, 'sections': []}
        
        # Dividir en 4 secciones de 5 velas
        section_1 = closes[-20:-15]
        section_2 = closes[-15:-10]
        section_3 = closes[-10:-5]
        section_4 = closes[-5:]
        
        # Momentum de cada sección
        ...
        if m1 > 0 and m2 > 0 and m3 > 0 and m4 > 0:
            return {
                ...
                'reason': 'All 4 sections positive'
            }
            ...
            return {
                ...
                'confidence': 95,
                'sections': sections_data,
                'reason': 'All 4 sections negative'
            }
            ...
            return {
                ...
                'confidence': 50,
                'sections': sections_data,
                'reason': 'No clear multi-candle pattern'
            }
    ...
    def analyze(self, data):
        ...
        try:
            ...
            # 🔧 FIX: Trinity sends OHLC in bar_data, not at root level
            bar_data = data.get('bar_data', {})
            log.info(f"[DEBUG] bar_data keys: {list(bar_data.keys()) if bar_data else 'EMPTY'} | has_closes: {bool(bar_data.get('closes'))}")
            opens = bar_data.get('opens', data.get('opens', prices))
            closes = bar_data.get('closes', data.get('closes', prices))
            highs = bar_data.get('highs', data.get('highs', prices))
            lows = bar_data.get('lows', data.get('lows', prices))
            log.info(f"[DEBUG] OHLC lengths: opens={len(opens)}, closes={len(closes)}, highs={len(highs)}, lows={len(lows)}")
            ...
            elif candle_narrative == 'ACCUMULATION_PHASE':
                # Penalizar trades en acumulación
                context_boost -= 5
                context_reasons.append("ACCUMULATION(-5)")
            ...
            if context_boost != 0:
                ...
                log.info(f"[CONTEXT_BOOST] {context_boost:+d} | Reasons: {', '.join(context_reasons)}")
            ...
            if trinity_decision != 'HOLD' and mtf_validation.get('confirms', 0) < 2:
                ...
                final_confidence = max(45, final_confidence * 0.6)
            ...
            if trinity_decision != 'HOLD' and adx < 22:
                ...
                trinity_decision = 'HOLD'
                final_confidence = max(45, final_confidence * 0.7)
...
{Y}|{R}                                                              {Y}|{R}
{Y}+{'-'*62}+{R}
{Y}|{R}  {W}NZDUSD Scalping Edition{R}      {D}by Polarice Labs © 2026{R}       {Y}|{R}
...
def run_tcp_server():
    ...
    # Cargar puerto desde nzdconfig.yaml
    try:
        with open('nzdconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm1_port', 9255)
    except:
        port = 9255  # Fallback
