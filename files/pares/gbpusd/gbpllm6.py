...
class LLM6_SmartMoney:
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (GBPUSD 75xx range)"""
        return {
            'trinity_port': 7566,
            'kraken_port': 7552,
            'executor_port': 7556,
...
def run_tcp_server():
    ...
    # Load port from gbpconfig.yaml
    try:
        with open('gbpconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 7562)
    except Exception as e:
        port = 7562  # GBPUSD LLM6 Smart Money Oracle
