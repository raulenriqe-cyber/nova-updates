...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║     ⚔️ NOVA EXCALIBUR KNIGHT - QUANTUM CORE v18.00 GBPUSD ELITE EDITION      ║
...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for GBPUSD')
...
try:
    from gbpllm6 import (  # 🔧 FIX CAT-83
        LLM6_SmartMoney,
        SmartMoneySignal,
        MarketIntention,
        SweepType
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        # 🎯 RELAXED THRESHOLDS FOR GBP - Allow more attacks
        ...
        # NOTE: LLM6 runs as TCP server on port 7562 (GBPUSD 75xx range)
        self.llm6_port = 7562  # LLM6 NOVA Smart Money Oracle port (GBPUSD)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        """Query LLM6 NOVA Smart Money Oracle via TCP (port 7562)"""
        try:
            ...
            request = {
                ...
                'symbol': genome.get('symbol', 'GBPUSD'),
            }
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        atr = indicators.get('atr', 0.0010)  # 10 pips default for GBPUSD forex
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('GBPUSD')
                    if news_conf > 60:
        ...
        # Asia session (0:00-7:00 UTC) = Menor liquidez para GBPUSD
        elif 0 <= hour_utc <= 6:
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('GBPUSD')
    ...
    def calculate_confluence_score(self, 
        ...
        rsi = indicators.get('rsi', 50)
        adx = indicators.get('adx', 25)
        atr = indicators.get('atr', 0.0010)  # Default 10 pips for GBPUSD forex
        macd = indicators.get('macd', 0)
        macd_signal = indicators.get('macd_signal', 0)
        ...
        # ATR Goldilocks zone: 0.0008-0.0025 (8-25 pips) para GBPUSD scalping
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For GBPUSD at ~1.25, 0.5% = 0.00625 which is reasonable for fib zone
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            if len(closes) >= 5:
                ...
                # GBPUSD (~1.25) vs EURUSD (~1.08) - percentage based is symbol-agnostic
            ...
            # Esto hace el código compatible con GBPUSD, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            ...
            # ⭐ RELAXED: Thresholds bajados para permitir más ataques en GBP
            # ═══════════════════════════════════════════════════════════════
            ...
            # 🎯 TIER 5: Trinity Force - Relajado para GBP
            ...
            # 🎯 TIER 6: High Confidence Override - Último recurso con confianza OK
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🎯 TIER 7: FALLBACK - Si Trinity quiere atacar, dejarlo pasar con checks mínimos
            # Para GBP scalping, Trinity sabe lo que hace
            # ═══════════════════════════════════════════════════════════════
...
class NOVAMarketStateDetector:
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Spread too wide (illiquidity) - GBPUSD spread in price units (0.0001 = 1 pip)
        elif spread > 0.0010:  # For GBPUSD, > 10 pips is abnormally wide
...
class Config:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ⚔️ STRATEGY SELECTOR (loaded from gbpconfig.yaml)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    HOST = '127.0.0.1'
    QUANTUM_PORT = 7554      # Injector - receive genomes from Quimera (GBPUSD)
    EXECUTOR_PORT = 7556     # Executor - send commands to Quimera (GBPUSD)
    TRINITY_PORT = 7566      # Trinity server port (GBPUSD 75xx range)
    KRAKEN_PORT = 7552       # Kraken executor (GBPUSD)
    LLM1_PORT = 7555         # BAYESIAN
    LLM2_PORT = 7557         # TECHNICAL
    LLM3_PORT = 7558         # CHART
    LLM4_PORT = 7559         # RISK
    LLM5_PORT = 7561         # LLM5 SUPREME chart analyzer
    LLM6_PORT = 7562         # ⭐ LLM6 NOVA Smart Money Oracle (GBPUSD)
    LLM7_PORT = 7563         # LLM7 OCULUS Data Quality
    LLM8_PORT = 7564         # LLM8 CHRONOS Timing
    LLM9_PORT = 7565         # LLM9 PREDATOR Execution
    LLM10_PORT = 7560        # LLM10 SENTINEL Guardian
    LLM11_PORT = 7567        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 7568        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 7570  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 SCALPER STRATEGY DEFAULTS (M1/M5 Forex Scalping)
    # ═══════════════════════════════════════════════════════════════════════
    # Trading - SNIPER SCALPING M1 (Forex Scalper Style)
    # Strategy: Wait 120s between trades, HIGH quality setups only, quick profits
    # 🔧 FIX: For M1 GBPUSD, 1 pip = 0.0001 (forex), 10 pips = 0.0010 move
    # Realistic M1 scalping: TP 8-15 pips, SL 5-10 pips
    TP_MIN_PIPS = 8       # 8 pips MINIMUM (0.0008) - realistic M1 scalping
    TP_MAX_PIPS = 20      # 20 pips MAX (0.0020) - don't chase unrealistic targets
    SL_MIN_PIPS = 5       # 5 pip stops (0.0005) - tight risk control
    SL_MAX_PIPS = 12      # Max 12 pip stop (0.0012) - protect capital
    ...
    MAX_CONCURRENT_TRADES = 3        # ⚠️ REDUCED: 3 concurrent trades in SAME direction (was 4 - too many)
    MAX_DAILY_LOSS_PERCENT = 2.0     # Strict loss control - protect capital
    MAX_CONSECUTIVE_LOSSES = 3       # FIX A-GBP-02: halt after N consecutive losses
    LOSS_HALT_DURATION_SECONDS = 300 # FIX A-GBP-02: pause 5 min after consecutive-loss trigger
    MIN_CONFIDENCE = 57              # 🎯 +2%: 57%+ confidence for GBP scalping (was 55 - caused bidirectional attacks)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    SOCKET_TIMEOUT = 3.5  # Increased from 2.0 — GBP Trinity was hitting 2s timeout on every genome (decisions_made=0)
    BUFFER_SIZE = 65536
    ...
    def load_strategy_from_yaml(cls):
        """Load active strategy from gbpconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'gbpconfig.yaml')
...
class NOVADashboard:
    """
    NOVA EXCALIBUR KNIGHT - GBPUSD Elite Momentum Dashboard
    ...
    def render(self):
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'GBPUSD'))
        ...
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}GBPUSD Elite Momentum by{self.RESET} {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}⚔️ NOVA Excalibur Knight{self.RESET} v18.00 GBPUSD {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
    ...
    def _send_llm_decision_to_dashboard(self, llm_name, decision, confidence, reason=""):
        """Send LLM decision message to trinity_dashboard_v3 TCP listener on port 7567 (GBPUSD)"""
        try:
            import socket
            ...
            # Serialize and send to localhost:7567 (GBPUSD dashboard port)
...
class LLMParallelConsensus:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-86
        self.logger = logger
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-82
...
class AdaptiveTPSLEngine:
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
        ...
        """
        Calculate INTELLIGENT TP/SL for M1 GBPUSD using ALL available data:
        ...
        """
        # Extract indicators
        atr = float(indicators.get('atr', 0.0010))  # Default 10 pips for GBPUSD
        rsi = float(indicators.get('rsi', 50))
        ...
        # Ensure minimum ATR for GBPUSD M1
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 GBPUSD typical ATR: 0.0003-0.0015 (= 3-15 pips)
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 GBPUSD SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico GBPUSD: 0.0005-0.0025 (5-25 pips)
        # Multiplicadores: TP = ATR × 1.1, SL = ATR × 0.8
        # Esto da R:R de ~1.375:1 mínimo
        # ═══════════════════════════════════════════════════════════════
        
        # ATR viene en precio para GBPUSD forex (ej: 0.0010 = 10 pips)
        ...
        # 🔧 FIX v5: GBPUSD M1 SCALPING - reduced distances for tight entries
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        # Convert to pips for return (1 pip = 0.0001 for GBPUSD forex)
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        ...
        # FIX v5: Mínimos REDUCIDOS para M1 GBPUSD scalping
        ...
        # Point value conversion: GBPUSD = 0.0001 per pip (forex standard)
        point_value = 0.0001  # GBPUSD: 1 pip = 0.0001 (e.g., 35 pips = 0.0035)
    ...
    def calculate_smart_plan(self, current_price, direction, genome, indicators, sweep_info=None):
        ...
        try:
            ...
            POINT = 0.0001  # GBPUSD forex: 1 pip = 0.0001
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.0001) if atr > 0 else 0  # [NOVA AUDIT F-11] GBPUSD pip=0.0001, ATR M1 ~2-6 pips
...
class QuantumConsciousnessEngine:
    ...
    def __init__(self):
        ...
        self.rl_engine = RealQLearningAgent(learning_rate=0.15, discount_factor=0.95, epsilon_start=0.3, strategy_tag=Config.ACTIVE_STRATEGY, pip_factor=10000)  # GBP: pip=0.0001 → factor=10000        
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        atr_val = indicators.get('atr', 0.0010)  # Default 10 pips for GBPUSD
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('GBPUSD')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM7 OCULUS - Data Quality Validator (Port 7562)
            ...
            # ✨ LLM8 CHRONOS - Timing Optimizer (Port 7563)
            ...
            # ✨ LLM9 PREDATOR - Execution Engine (Port 7564)
        ...
        try:
            ...
            # CRITICAL: Round to valid lot increments (0.01 for GBPUSD)
            ...
            if trend_type == 'RANGING' and trinity_decision in ['BUY', 'SELL']:
                ...
                # 🔧 FIX: threshold 60→57 — EXCALIBUR is counter-trend by design, 57% matches MIN_CONFIDENCE
                # After TREND_CHECK penalty (×0.92), raw 65% becomes 59.8% — must be above 57 to bypass
                if consensus_score < 0.55 and trinity_confidence < 57:
            ...
            # For M1 forex scalping, we can trade even in tight ranges - adjusted for GBPUSD
            ...
            if atr_value < 0.0003:  # < 3 pips = dead market
                self.logger.warning(f"[CONSOLIDATION_VETO] 🚫 Blocking: ATR={atr_value*10000:.1f}pips < 3 (broken market)")
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                if trinity_decision == 'BUY':
                    ...
                    if not trend_aligned:
                        ...
                        trinity_confidence *= 0.92  # 🔧 FIX: Was 0.65 (35%!) → 0.92 (8%) — EXCALIBUR_KNIGHT is counter-trend by design
                elif trinity_decision == 'SELL':
                    ...
                    if not trend_aligned:
                        ...
                        trinity_confidence *= 0.92  # 🔧 FIX: Was 0.65 (35%!) → 0.92 (8%) — gentle warning, not a block
                
                # WHALE TRAP: Reduce confidence if detected (don't block, just penalize)
                if is_trap and trap_confidence > 60:
                    trinity_confidence *= 0.88  # 🔧 FIX: Was 0.70 (30%!) → 0.88 (12%) — prevent cascading kills with TREND_CHECK
                ...
                if Config.is_excalibur_active() and hasattr(self, 'excalibur_analyzer') and self.excalibur_analyzer:
                    ...
                    if NEWS_INTELLIGENCE_AVAILABLE:
                        try:
                            news_bias = get_trading_bias('GBPUSD')
            ...
            try:
                ...
                nova_dashboard.update(
                    ...
                    market_state=market_analysis_result.market_state.value if market_analysis_result else 'UNKNOWN',
                    trend_strength=market_analysis_result.trend_strength.value if market_analysis_result else 1,
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 7570
        ...
        try:
            ...
            # Send to sound.py server on port 7570 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 7570))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:7570")
        except socket.timeout:
...
class AdvancedIPC:
    ...
    def _play_sound_alert(self, alert_type, symbol='GBPUSD'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # 🔧 FIX: Use Config.SOUND_PORT instead of hardcoded 7570
            sound_port = getattr(Config, 'SOUND_PORT', 7570)
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "GBPUSD", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "GBPUSD"}, ...}
        """
    ...
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for GBPUSD data"""
        try:
            ...
            if genome_type in valid_types:
                ...
                try:
                    ...
                    # Acceptable range for GBPUSD
                    # GBPUSD is a forex pair, typically 1.10 - 1.50
                    # Accept range: 0.5 - 3.0 to be safe for historical/future variations
                    mid_price = (bid + ask) / 2.0
                    
                    # GBPUSD forex price range (0.5 - 3.0)
                    if 0.5 <= mid_price <= 3.0:
                        return True
                    
                    # Price out of realistic range for GBPUSD
                    self.logger.warning(f"[Validator] GBPUSD price out of range: {mid_price:.5f} (expected 0.5-3.0)")
                    return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-81
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - Quimera not connected to port 7556")
            ...
            # 1. Validate price is within realistic bounds (GBPUSD: 0.5-3.0 forex range)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR GBPUSD
            # ⭐ GBPUSD is forex pair, typically 1.10 - 1.50, accept 0.5 - 3.0 range
            valid_forex_range = 0.5 <= entry_price <= 3.0  # Normal GBPUSD forex pricing
            
            if not valid_forex_range:
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price:.5f} out of realistic range for GBPUSD (expected 0.5-3.0)")
                return False
            ...
            # ⚠️ CRITICAL: Clamp ATR to M1 GBPUSD realistic range
            ...
            # 🔧 FIX v4: GBPUSD forex - distances in price units (0.0001 = 1 pip)
            # ═══════════════════════════════════════════════════════════════════════════
            MIN_TP_DISTANCE = 0.0010  # 10 pips minimum TP (M1 scalping)
            MIN_SL_DISTANCE = 0.0008  # 8 pips minimum SL (M1 scalping)
            SAFETY_MARGIN = 0.0005    # 5 pips extra safety buffer for ASK/BID slippage
            
            tp_distance = max(MIN_TP_DISTANCE, tp_distance)
            sl_distance = max(MIN_SL_DISTANCE, sl_distance)
            
            # ⚠️ HARD CAP: Máximo absoluto para scalping M1 forex
            tp_distance = min(tp_distance, 0.0060)  # Max 60 pips TP
            sl_distance = min(sl_distance, 0.0050)  # Max 50 pips SL
...
class QuantumCore:
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        if closes and current_price > 0:
            ...
            if gap > 0.01:  # More than 0.01 (1 pip for GBPUSD) difference
    ...
    def __init__(self):
        ...
        # FIX M-GBP-01: NOVAMarketStateDetector singleton — avoid re-instantiation on every tick
    ...
    def _print_banner(self):
        ...
        logger.info("=" * 110)
        if is_excalibur:
            logger.info("⚔️  NOVA EXCALIBUR KNIGHT v18.00 - GBPUSD ELITE MOMENTUM SNIPER")
            logger.info("🛡️  \"En el fragor eterno de la batalla, aguardo el susurro del destino para asestar el golpe inmortal\"")
        else:
            logger.info("🎯 NOVA SNIPER SCALPING v18.00 - GBPUSD FRANCOTIRADOR CONSCIOUSNESS (120s cooldown, 75%+ win rate)")
        logger.info("=" * 110)
        logger.info("")
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            # CRITICAL: Validate price is in reasonable range for GBPUSD FOREX
            # GBPUSD typically trades 1.10-1.50, historical range 0.5-3.0
            # This is FOREX, NOT gold - prices are around 1.XX not thousands
            if symbol == 'GBPUSD' and close_price > 0:
            ...
            if len(highs_array) >= stoch_period and len(lows_array) >= stoch_period and len(closes_array) >= stoch_period:
                try:
                    ...
                    if stoch_range > 0:
                        ...
                        # %D = SMA(3) of %K (simplified: use single value for now)
                        stoch_d_calc = stoch_k_calc  # Will improve with history tracking
                except Exception as e:
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            if _consec >= Config.MAX_CONSECUTIVE_LOSSES:
                ...
                logger.error(f"[A-GBP-02] ⛔ {_consec} consecutive losses — halting {Config.LOSS_HALT_DURATION_SECONDS}s")
                return None

            # ═══ GUARD A-GBP-03: Daily loss cap ═══
            ...
            if _balance > 0 and (_today_pnl / _balance * 100) <= -Config.MAX_DAILY_LOSS_PERCENT:
                logger.error(f"[A-GBP-03] ⛔ Daily loss cap reached ({_today_pnl:.2f} / {_balance:.2f}) — no new trades today")
                return None
            ...
            if symbol == 'GBPUSD' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')
            ...
            if genome_type in valid_genome_types:
                # DEBUG: Log incoming genome keys to debug missing data
                self.logger.info(f"[Process] DEBUG: Received {genome_type} for {symbol}. Title keys: {list(genome.keys())}")
            ...
            # FIX M-GBP-01: use singleton — no re-instantiation on every tick
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                # Calculate bid/ask from current price with typical GBPUSD spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 0.00005  # Half spread for GBPUSD (typical spread = 0.0001 = 1 pip)
            ...
            # ═══ FIX C-GBP-01: ATR=0 circuit breaker ═══
            if atr_val == 0.0:
                with self.ipc.stats_lock:
                    _cnt = self.ipc.stats.get('_atr_zero_count', 0) + 1
                    self.ipc.stats['_atr_zero_count'] = _cnt
                if _cnt >= 5:
                    logger.error(f"[C-GBP-01] ⛔ ATR=0 for {_cnt} consecutive ticks — skipping trade (stale feed?)")
                    return None
                logger.warning(f"[C-GBP-01] ⚠️ ATR=0 tick #{_cnt} — skipping")
                return None
            else:
                with self.ipc.stats_lock:
            ...
            _did_increment = False  # FIX C-GBP-02: track if THIS call incremented open_positions
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-GBP-05: direction-poison check (SL-hit protection) — stacking
                            ...
                            if time.time() < _poison_time:
                                ...
                                _did_increment = True  # FIX C-GBP-02
                        ...
                        # FIX A-GBP-05: direction-poison check (SL-hit protection) — first trade
                        _poison_key = f'dir_poison_until_{decision["action"]}'
                        _poison_time = self.ipc.stats.get(_poison_key, 0)
                        if time.time() < _poison_time:
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    ...
                    # FIX A-GBP-01: atomic update — single timestamp, under stats_lock
                    _now = time.time()
                    with self.ipc.stats_lock:
                    ...
                    # FIX C-GBP-02: only rollback if THIS call incremented open_positions
    ...
    def run(self):
        ...
        last_position_reconcile = time.time()  # FIX A-GBP-04
        start_time = time.time()
        ...
        try:
            while self.running:
                ...
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)  # 🔧 FIX CAT-85
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        # FIX M-GBP-02: atomic write — prevents partial reads by dashboard
                ...
                # FIX A-GBP-04: position reconcile every 60s — reset stale open_positions
...
def _acquire_gbp_singleton():
    """Prevent multiple instances using PID file"""
    import os as _os2, atexit as _atexit2
    _pid_file = _os2.path.join(_os2.path.dirname(__file__), 'gbpquantum_core.pid')
    if _os2.path.exists(_pid_file):
        try:
            ...
            if _ps2.pid_exists(_old_pid) and _ps2.Process(_old_pid).is_running():
                print(f'[SINGLETON] GBP instance already running (PID={_old_pid}). Exiting.')
                return False
        except Exception:
            pass
...
# ═══════════════════════════════════════════════════════════════════════════════════
# 📋 QA AUDIT CERTIFICATE — gbpusd/gbpquantum_core.py
# ═══════════════════════════════════════════════════════════════════════════════════
# Auditor  : GitHub Copilot (Claude Sonnet 4.6)
# Date     : 2025-01-09
# File     : gbpusd/gbpquantum_core.py  (15 706 lines — EXCALIBUR KNIGHT v18.00)
# Strategy : NOVA EXCALIBUR KNIGHT v18.00 — GBPUSD Elite Momentum Sniper
#            Unique: ExcaliburSignalAnalyzer + ExcaliburNewsManager, Quimera sent
#            AFTER Kraken confirmation (reversed vs EUR/JPY), MAX_CONCURRENT_TRADES=3
#
# FIXES APPLIED (9 canonical + 2 config constants):
#
#   C-GBP-01 [CRITICAL] ATR=0 circuit breaker
#            _atr_zero_count under stats_lock; skip after 5 consecutive zero-ATR ticks
#            Location: process_genome() — after atr_val extraction in trading indicators block
#
#   C-GBP-02 [CRITICAL] _did_increment flag + guarded rollback
#            _did_increment=False before position lock; set True only in APPROVE branches;
#            rollback in Kraken-fail path gated on `if _did_increment`
#            Location: position control block + Kraken-fail else branch
#
#   A-GBP-01 [HIGH] Atomic last_trade_time update
#            Single _now = time.time(); both self.last_trade_time and
#            self.engine.last_trade_time written inside stats_lock in one block
#            Location: after kraken_success confirmation
#
#   A-GBP-02 [HIGH] Consecutive-loss halt guard
#            Checks ipc.stats['consecutive_losses'] >= Config.MAX_CONSECUTIVE_LOSSES (3);
#            sets loss_halt_until + resets counter; early-return None during halt
#            Location: top of process_genome() after genome stamp
#            Config added: MAX_CONSECUTIVE_LOSSES=3, LOSS_HALT_DURATION_SECONDS=300
#
#   A-GBP-03 [HIGH] Daily loss cap guard
#            today_pnl / balance * 100 <= -MAX_DAILY_LOSS_PERCENT → early-return None
#            (MAX_DAILY_LOSS_PERCENT=2.0 was defined but never enforced — now active)
#            Location: top of process_genome() after A-GBP-02 guard
#
#   A-GBP-04 [HIGH] Position reconcile timer in run()
#            last_position_reconcile timer every 60s; resets open_positions to 0 if
#            last_trade_opened_at > 600s ago (stale phantom positions)
#            Location: run() main loop before status log
#
#   A-GBP-05 [HIGH] Direction-poison SL-protection
#            dir_poison_until_{BUY|SELL} check in both stacking-approve branch and
#            first-trade-approve branch; converts to HOLD if poison active
#            Location: inside position control stats_lock block, both approve paths
#
#   M-GBP-01 [MEDIUM] NOVAMarketStateDetector singleton
#            self.nova_detector created once in __init__; per-call instantiation
#            `nova_detector = NOVAMarketStateDetector(logger=logger)` removed
#            Location: __init__ (after engine.cache_ttl) + process_genome() call site
#
#   M-GBP-02 [MEDIUM] Atomic quantum_stats.json write
#            write → quantum_stats.json.tmp → os.replace() to prevent torn reads
#            Location: run() stats write block
#
# SYNTAX CHECK: python -m py_compile gbpquantum_core.py → OK ✅
# GBP-SPECIFIC NOTES:
#   • EXCALIBUR integration (ExcaliburSignalAnalyzer/ExcaliburNewsManager) UNCHANGED
#   • Quimera-after-Kraken architecture UNCHANGED (GBP-specific design)
#   • Supreme Consciousness ML callbacks UNCHANGED
#   • Master Safety check (trinity_responses OR kraken_acks) UNCHANGED
# ═══════════════════════════════════════════════════════════════════════════════════
