"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - GBPUSD Market Sentiment                         ║
...
║  Optimizado para GBPUSD - British Pound / US Dollar                          ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-GBP")
...
# Símbolos a monitorear - GBPUSD FOCUSED
SYMBOLS = ["GBPUSD", "EURUSD", "USDJPY"]

# Fuentes confiables para forex
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "forexlive", "fxstreet", "dailyfx",
    "investing.com", "tradingview", "marketwatch", "cnbc",
    "financialjuice", "forexfactory", "ft.com", "bbc.com",
    "sky.com", "theguardian.com", "telegraph.co.uk", "poundsterlinglive"
]

# Keywords por símbolo - GBPUSD OPTIMIZED
KEYWORDS = {
    "GBPUSD": [
        "GBPUSD", "pound dollar", "GBP/USD", "sterling news",
        "BOE decision", "bank of england", "pound forecast",
        "british pound", "fed pound", "sterling strength",
        "pound trading", "pound weakness", "uk economy",
        "uk inflation", "uk employment", "uk gdp", "uk retail sales",
        "brexit", "british economy", "uk interest rate",
        "boe rate decision", "bailey boe", "bank of england rate",
        "gilt yields", "uk bond", "ftse 100", "london session",
        "pound rally", "pound crash", "cable forex", "cable trading"
    ],
    "EURUSD": [
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        symbol = news_item.get('symbol', 'GBPUSD')
...
class NewsStorage:
    ...
    def _get_today_file(self) -> Path:
        ...
        return self.news_dir / f"news_gbp_{today}.json"
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        gbp_score = abs(sentiment.get('GBPUSD', {}).get('score', 0))
        gbp_total = sentiment.get('GBPUSD', {}).get('total', 0)
        gbp_conf = min(1.0, gbp_score * (1 + min(gbp_total, 10) * 0.1))
        
        return {
            'GBPUSD': sentiment.get('GBPUSD', {}).get('sentiment', 'N'),
            'EURUSD': sentiment.get('EURUSD', {}).get('sentiment', 'N'),
            'USDJPY': sentiment.get('USDJPY', {}).get('sentiment', 'N'),
            'summary': ' | '.join(summaries),
            'confidence': gbp_conf,
            'confidence_gbpusd': gbp_conf,
            'raw': sentiment
        }
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para GBPUSD.
    ...
    def fetch_and_evaluate(self) -> Dict:
        ...
        """
        logger.info("=" * 60)
        logger.info("📰 GBPUSD NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        ...
        if symbol == 'GBPUSD':
            conf = sentiment.get('confidence_gbpusd', sentiment.get('confidence', 0.5))
        else:
...
def main():
    ...
    print("=" * 70)
    print("📰 NEWS INTELLIGENCE ENGINE - GBPUSD")
    ...
    print("=" * 70)
    print(f"   GBPUSD: {sentiment.get('GBPUSD', 'N')}")
    print(f"   EURUSD: {sentiment.get('EURUSD', 'N')}")
    print(f"   USDJPY: {sentiment.get('USDJPY', 'N')}")
...
def get_news_sentiment() -> Dict:
    ...
    Usage:
        ...
        if sentiment['GBPUSD'] == 'B':
            # Bias bullish para GBP
    """
    global _news_engine
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('GBPUSD')
