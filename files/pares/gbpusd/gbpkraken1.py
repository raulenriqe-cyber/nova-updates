...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 7552)
  - Audio announcements via sound.py (GBPUSD only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='gbpconfig.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 7552 (not 7556 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 7552)
            self.symbols = config.get('core_engine', {}).get('symbols', ['GBPUSD'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 7552  # Default to 7552, NOT 7556
            self.symbols = ['GBPUSD']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0050  # [NOVA AUDIT F-12] 50-pip fallback GBPUSD (pip=0.0001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0030  # [NOVA AUDIT F-12] 30-pip fallback GBPUSD
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'GBPUSD')
            ...
            # Validate symbol
            valid_symbols = {'GBPUSD', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'GBPUSD', 'GBP',
                           'USDJPY', 'JPY', 'GBPUSD', 'GBP', 'ETHUSD', 'ETH'}
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('gbpconfig.yaml')
        kraken.run()
