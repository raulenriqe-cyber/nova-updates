...
║  Port: 7558 (TCP Server) - MATHEMATICAL PRECISION ENHANCED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'GBPUSD').upper().strip()
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port=7558):  # FIXED: LLM3 is 7558
        self.host = host
        self.port = port
...
def main():
    ...
    try:
        # Cargar config
        port = 7558  # LLM3 is on 7558
        try:
            with open('gbpconfig.yaml', 'r') as f:
                config = yaml.safe_load(f) or {}
            port = config.get('llm_settings', {}).get('llm3_port', 7558)
        except:
            pass
