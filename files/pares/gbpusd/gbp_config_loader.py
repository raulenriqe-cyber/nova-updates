...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 💷 GBPUSD CONFIG LOADER - CENTRALIZED CONFIGURATION           ║
║                                                                              ║
║  Loads GBPUSD ports and parameters from gbpconfig.yaml                       ║
...
class GBPConfig:
    """Centralized configuration manager for GBPUSD system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from gbpconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"gbpconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('sockets.trinity_port') -> 7566
        """
        config = cls.load()
...
def get_trinity_port() -> int:
    """Get Trinity port for GBPUSD"""
    return GBPConfig.get_port('trinity')

def get_kraken_port() -> int:
    """Get Kraken port for GBPUSD"""
    return GBPConfig.get_port('kraken')

def get_llm_port(llm_num: int) -> int:
    """Get port for specific LLM (1-10)"""
    return GBPConfig.get_port(f'llm{llm_num}')

def get_ollama_port() -> int:
    """Get Ollama port (shared)"""
    return GBPConfig.get_port('ollama')

def get_sound_port() -> int:
    """Get Sound alert port"""
    return GBPConfig.get_port('sound')

def get_injector_port() -> int:
    """Get Injector port (MT5 -> Python)"""
    return GBPConfig.get_port('injector')

def get_executor_port() -> int:
    """Get Executor port (Python -> MT5)"""
    return GBPConfig.get_port('executor')
...
def get_all_llm_ports() -> Dict[str, int]:
    ...
    for i in range(1, 11):
        try:
            ports[f'llm{i}'] = GBPConfig.get_port(f'llm{i}')
        except:
            pass
    return ports
...
def get_symbol() -> str:
    """Get market symbol"""
    return GBPConfig.get('symbol', 'GBPUSD')

def get_timeframe() -> str:
    """Get primary timeframe"""
    return GBPConfig.get('primary_timeframe', 'M5')

def get_sl_range() -> tuple:
    """Get SL min/max USD"""
    return (
        GBPConfig.get('trading.scalp_sl', 150),
        GBPConfig.get('trading.swing_sl', 800)
    )

def get_tp_range() -> tuple:
    """Get TP min/max USD"""
    return (
        GBPConfig.get('trading.scalp_tp', 100),
        GBPConfig.get('trading.swing_tp', 600)
    )
...
def get_min_confidence() -> int:
    ...
    return GBPConfig.get('confidence.minimum_signal', 70)

def get_max_positions() -> int:
    """Get max positions"""
    return GBPConfig.get('trading.max_positions', 3)

def get_pip_value() -> float:
    """Get pip value for GBPUSD"""
    return GBPConfig.get('trading.pip_value', 1.0)

def get_volatility_thresholds() -> dict:
    """Get ATR thresholds for volatility regimes"""
    return {
        'low': GBPConfig.get('indicators.atr.low_volatility', 200),
        'normal': GBPConfig.get('indicators.atr.normal_volatility', 500),
        'high': GBPConfig.get('indicators.atr.high_volatility', 1200),
        'extreme': GBPConfig.get('indicators.atr.extreme_volatility', 2500)
    }
...
if __name__ == '__main__':
    print("═" * 80)
    print("₿ GBPUSD CONFIG LOADER - TEST")
    print("═" * 80)
