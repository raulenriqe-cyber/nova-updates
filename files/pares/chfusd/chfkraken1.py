...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 9901)
  - Audio announcements via sound.py (USDCHF only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='config.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 9901 (not 8656 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 9901)
            self.symbols = config.get('core_engine', {}).get('symbols', ['USDCHF'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 9901  # Default to 9901, NOT 8656
            self.symbols = ['USDCHF']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0050  # [NOVA AUDIT F-12] 50-pip fallback USDCHF (pip=0.0001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0030  # [NOVA AUDIT F-12] 30-pip fallback USDCHF
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'USDCHF')
            ...
            # Validate symbol
            valid_symbols = {'USDCHF', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'GBPUSD', 'GBP',
