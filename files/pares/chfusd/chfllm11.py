...
class NarrativeEngine:
    ...
    def generate_narrative(self, data: dict, macro: dict, cross: dict) -> dict:
        ...
        rsi = data.get('rsi', 50)
        adx = data.get('adx', 20)
        atr = data.get('atr', 0.0008)  # 🔧 Calibrated for CHFUSD
        ...
        # DXY cross-asset for CHFUSD
        dxy = cross.get('dxy_proxy', 104)
        if dxy > 0:
            # CHF is safe-haven like gold. DXY down = CHFUSD up (inverse).
...
class PhaseDetector:
    ...
    def detect_phase(self, data: dict) -> dict:
        ...
        if adx < 20 and bb_width < 0.004 and volume_ratio > 0.8:  # 🔧 Calibrated for CHFUSD
            phase = 'ACCUMULATION'
        elif adx > 25 and price > sma_50 and macd_hist > 0:
            phase = 'MARKUP'
        elif adx < 25 and price > sma_200 and bb_width > 0.006 and volume_ratio < 0.9:  # 🔧 Calibrated for CHFUSD
            phase = 'DISTRIBUTION'
        elif adx > 25 and price < sma_50 and macd_hist < 0:
            phase = 'MARKDOWN'
        else:
...
class ScenarioProjector:
    ...
    def generate_scenarios(self, data: dict, narrative: dict, phase: dict) -> dict:
        ...
        price = data.get('price', data.get('bid', 0))
        atr = data.get('atr', data.get('atr_14', 0.0008))  # 🔧 Calibrated for CHFUSD
        ...
        return {
            'bullish': {
                ...
                'target': round(price + atr * 2, 5),
                'description': f"Price rises to {price + atr * 2:.5f} (+{atr * 2:.5f})"
            },
            'neutral': {
                'probability': round(p_neutral, 2),
                'range': [round(price - atr * 0.5, 5), round(price + atr * 0.5, 5)],
                'description': f"Consolidation between {price - atr * 0.5:.5f} - {price + atr * 0.5:.5f}"
            },
            'bearish': {
                'probability': round(p_bear, 2),
                'target': round(price - atr * 2, 5),
                'description': f"Price drops to {price - atr * 2:.5f} (-{atr * 2:.5f})"
            }
        }
...
class SmartTPSL:
    ...
    def calculate(self, data: dict, strategy: str, conviction: dict,
        ...
        if price <= 0 or atr <= 0:
            return {'tp1': 0, 'tp2': 0, 'tp3': 0, 'sl': 0, 'rr_ratio': 0, 'rr_acceptable': False}  # 🔧 FIX CAT-142
        ...
        break_even_pips = exec_quality.get('break_even_pips', 0.00015)  # 🔧 Calibrated for CHFUSD
        # Ensure TP1 covers break-even cost (at minimum)
        min_tp1 = break_even_pips * 3  # 3x break-even minimum

        # ── Calculate levels ──
        if action == 'BUY':
            sl = round(price - atr * cfg['sl_atr'], 5)
            tp1 = round(price + max(atr * cfg['tp1_atr'], min_tp1), 5)
            tp2 = round(price + atr * cfg['tp2_atr'], 5)
            tp3 = round(price + atr * cfg['tp3_atr'], 5)
        else:  # SELL
            sl = round(price + atr * cfg['sl_atr'], 5)
            tp1 = round(price - max(atr * cfg['tp1_atr'], min_tp1), 5)
            tp2 = round(price - atr * cfg['tp2_atr'], 5)
        ...
        rr_ratio = reward1 / max(risk, atr * 0.001)  # 🔧 pair-safe floor (was 0.01 → broke CHFUSD)
