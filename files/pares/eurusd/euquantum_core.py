...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        self.min_perfect_score = 5.0   # 🔧 ARES WAR v1.2: 5.0 (PERFECT tier sin cambio)
        self.min_good_score = 4.5      # 🔧 ARES WAR v1.2: 4.5 (GOOD tier sin cambio)
        self.min_wait_score = 3.0      # 🔧 ARES WAR v1.2: 3.0 (was 3.5 — CONFLUENCE_GATE_MIN=3.0)
        self.min_sweep_score = 3.0     # 🔧 ARES WAR v1.2: 3.0 (was 3.5 — alta frecuencia)
        self.lock = Lock()
        ...
        self.high_prob_buy_setups = [
            ...
            ('OVERSOLD_REVERSAL', {'rsi_max': 34, 'hammer': True, 'support': True}, 0.78),  # ARES WAR v1.2: 34 (was 30)
        ...
        self.high_prob_sell_setups = [
            ('OVERBOUGHT_REVERSAL', {'rsi_min': 66, 'shooting_star': True, 'resistance': True}, 0.78),  # ARES WAR v1.2: 66 (was 70)
        ...
        self.LLM6_PORT     = 6562  # LLM6 NOVA Smart Money Oracle port (hardcoded)
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        is_oversold = rsi < 34   # ARES WAR v1.2: 34 (was 30) — más setups de compra
        is_overbought = rsi > 66  # ARES WAR v1.2: 66 (was 70) — más setups de venta
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('EURUSD')
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            for llm_name, llm_data in trinity_llms_data.items():
                if isinstance(llm_data, dict):
                    # FIX: Trinity returns 'decision' key, not 'vote' or 'action'
    ...
    def _validate_direction_ultra_exigent(
        ...
        bullish_reversal = ['hammer', 'bullish_engulf', 'morning_star', 'piercing',
                           ...
                           'dragonfly_doji', 'double_bottom', 'potential_double_bottom',
                           'uptrend', 'bullish', 'breakout_bull', 'higher_lows']
        bearish_reversal = ['shooting_star', 'bearish_engulf', 'evening_star',
                           'dark_cloud', 'doji_bear', 'hanging_man', 'three_black',
                           'gravestone_doji', 'breakout_bearish', 'lower_lows',
                           'downtrend', 'bearish', 'momentum_down', 'double_top',
                           'v_top', 'distribution']
    ...
    def get_context_probability_from_learning(self, context: dict) -> float:
        ...
        try:
            # 🔧 FIX CAT-69: find_similar_episodes may not exist in LLMMemoryBank
            if not hasattr(self.ml_feedback.memory_bank, 'find_similar_episodes'):
                return 0.5
...
class Config:
    ...
    QUANTUM_PORT  = 6554      # Injector - receive genomes from Quimera (CHF)
    EXECUTOR_PORT = 6556     # Executor - send commands to Quimera (CHF)
    TRINITY_PORT  = 7666  # Trinity server port
    KRAKEN_PORT   = 6552
    LLM1_PORT     = 6555
    LLM2_PORT     = 6557
    LLM3_PORT     = 6558
    LLM4_PORT     = 6559
    LLM5_PORT     = 6561  # LLM5 SUPREME chart analyzer
    LLM6_PORT     = 6562  # ⭐ LLM6 NOVA Smart Money Oracle
    LLM11_PORT    = 6566        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT    = 6567        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT    = 6570  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # Trading - ARES WAR v1.2 REVERSAL (Alta frecuencia 8-13 trades/sesión)
    # Strategy: RSI reversal moderated + MACD divergence + micro-trend alignment
    # ARES WAR v1.2: TP 6-14 pips / SL 6-12 pips / R:R 1.5-2.0:1
    # Trailing stop: activation +8p, step 3p
    TP_MIN_PIPS = 6       # ARES WAR v1.2: 6 pips min (was 8 - entrada más temprana)
    TP_MAX_PIPS = 14      # ARES WAR v1.2: 14 pips max (was 20 - no perseguir)
    SL_MIN_PIPS = 6       # ARES WAR v1.2: 6 pips min (was 5 - más protección)
    SL_MAX_PIPS = 12      # ARES WAR v1.2: 12 pips max (sin cambio)
    ...
    # �🇺 STRATEGY SELECTOR (loaded from euconfig.yaml)
    ACTIVE_STRATEGY = "ARES_WAR_1_2"  # Default, will be overridden by yaml

    @classmethod
    def load_strategy_from_yaml(cls):
        """Load active strategy from euconfig.yaml"""
        try:
            import yaml
            ...
            if os.path.exists(config_path):
                ...
                strategy = config.get('active_strategy', 'ARES_WAR_1_2').upper().replace('.', '_')
                cls.ACTIVE_STRATEGY = strategy
                print(f"[CONFIG] 🇪🇺 EURUSD strategy loaded: {strategy}")
                return strategy
        except Exception as e:
            print(f"[CONFIG] Error loading EURUSD strategy: {e} - using default ARES_WAR_1_2")
        return 'ARES_WAR_1_2'
...
class NOVADashboard:
    ...
    def render(self):
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'EURUSD'))
...
class LLMParallelConsensus:
    """Query all 5 LLMs in parallel for consensus - DEEP PYTHON RESILIENCE"""
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-71: 5 workers para llm1-5 en paralelo real
        self.logger = logger
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        if SHARED_BRAIN_AVAILABLE and self.genome_counter % 30 == 0:
            try:
                ...
                update_regime_state(
                    symbol=genome.get('metadata', {}).get('symbol', 'EURUSD'),
        ...
        try:
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                gate_indicators = {
                    ...
                    # 🔧 FIX: Use ma_fast/ma_slow as fallback (system stores as ma_fast/ma_slow, not ma5/ma20)
                    'ma5': indicators.get('ma5', indicators.get('ma_fast', 0)),
                    'ma20': indicators.get('ma20', indicators.get('ma_slow', 0)),
                }
            ...
            try:
                ...
                nova_dashboard.update(
                    ...
                    quantum_consciousness_level=getattr(self.consciousness_level, 'value', self.consciousness_level) if hasattr(self, 'consciousness_level') else 1,  # 🔧 FIX CAT-70
                    ...
                    market_state=getattr(market_analysis_result.market_state, 'value', market_analysis_result.market_state) if market_analysis_result else 'UNKNOWN',  # 🔧 FIX CAT-70
                    trend_strength=getattr(market_analysis_result.trend_strength, 'value', market_analysis_result.trend_strength) if market_analysis_result else 1,  # 🔧 FIX CAT-70
...
class AdvancedIPC:
    ...
    def _play_sound_alert(self, alert_type, symbol='EURUSD'):
                    ...
                    # This syncs real open positions count with MT5
                        ...
                        try:
                            ...
                            with self.stats_lock:
                                ...
                                # FIX RECONCILE: guardar snapshot del último portfolio_sync para el reconciler
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 0.88, "ask": 0.89, "symbol": "EURUSD", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 0.88, "ask": 0.89}, "metadata": {"symbol": "EURUSD"}, ...}
        """
            ...
            if genome_type in valid_types:
                ...
                if genome_type in ['genome', 'tick']:
                    ...
                    try:
                        ...
                        if bid > ask:  # Only reject truly inverted (bid==ask is valid zero-spread tick)
                            self.logger.warning(f"[Validator] Bridge: Inverted bid={bid} > ask={ask}")
                            return False
                        
                        # EURUSD range: 0.50 - 2.50
                        mid_price = (bid + ask) / 2.0
                        if mid_price < 0.50 or mid_price > 2.50:
                ...
                try:
                    ...
                    # Acceptable range for EURUSD (0.50 - 2.50)
                    mid_price = (bid + ask) / 2.0
                    if mid_price < 0.50 or mid_price > 2.50:
                        self.logger.warning(f"[Validator] Price out of range: {mid_price:.5f} (EU range: 0.50-2.50)")
                        return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-68: default=str serializa numpy/Enum/datetime
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            response = {
                ...
                'lot': decision.get('lot', 0.01),              # EA reads 'lot'
                'confidence': float(decision.get('confidence', 70)),  # EA rejects if < InpMinConfidence(63)
                'magic_number': decision.get('magic_number', 0),
                'sl': sl_price,
                'tp': tp_price,
...
class QuantumCore:
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            symbol = metadata.get('symbol', 'EURUSD')
            bid = float(tick_data.get('bid', 0))
            ask = float(tick_data.get('ask', 0))
            ...
            if symbol == 'EURUSD' and close_price > 0:
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            # ── Nova Dashboard bridge: save per-LLM votes for quantum_stats.json ──
            try:
                _llm_map = {'BAY':'BAYESIAN','TEC':'TECHNICAL','CHT':'CHART',
                            'RSK':'RISK','SUP':'SUPREME','SMI':'SMART_MONEY',
                            'OCS':'OCULUS','TMR':'CHRONOS','PRD':'PREDATOR',
                            'MSDA':'MSDA','GURU':'GURU','SNTL':'SENTINEL'}
                _llr = trinity_resp.get('llm_responses', {}) if trinity_resp else {}
                genome['llm_votes'] = {a: _llr.get(f, {}).get('decision', 'HOLD')
                                       for a, f in _llm_map.items()}
                genome['llm_confs'] = {a: int(_llr.get(f, {}).get('confidence', 0))
                                       for a, f in _llm_map.items()}
            except Exception:
                genome['llm_votes'] = {}
                genome['llm_confs'] = {}
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            if _poison_until > time.time():
                                ...
                                self.ipc.stats['last_action_time'] = time.time()  # FIX RECONCILE: registrar tiempo de última entrada
                        ...
                        if _poison_until_first > time.time():
                            ...
                            self.ipc.stats['last_action'] = decision['action']
                            self.ipc.stats['last_action_time'] = time.time()  # FIX RECONCILE: registrar tiempo de primera entrada
                            _did_increment = True  # FIX C-CHF-02
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                # FIX A-CHF-04 v2: POSITION RECONCILE every 60s — ahora funciona correctamente
                # Antes: last_action_time nunca se escribía → condición siempre False → nunca reseteaba
                # Ahora: se comparan 3 señales para detectar posiciones fantasma:
                #   1. last_action_time > 120s sin trade_closed → sospechoso
                #   2. open_positions > 0 pero trades_closed_received aumentó → las cerró MT5 sin notificar
                #   3. portfolio_sync con 0 posiciones pero QC dice > 0 → reset inmediato
                if time.time() - last_position_reconcile > 60:
                    with self.ipc.stats_lock:
                        ...
                        if last_sync == 0 and open_pos > 0:
                            self.logger.warning(
                                f"[RECONCILE] 🔄 portfolio_sync dice 0 posiciones pero QC tiene {open_pos} — RESETEANDO"
                            )
                            self.ipc.stats['open_positions'] = 0
                            self.ipc.stats['current_direction'] = None

                        # Señal 2: lleva más de 300s desde la última entrada sin cierre registrado
                        # y el bridge no ha mandado portfolio_sync reciente (bridge caído o desconectado)
                            ...
                            if time_since_sync > 120:  # bridge no ha hablado en 120s
                                self.logger.warning(
                                    f"[RECONCILE] ⚠️ open_positions={open_pos} desde hace "
                                    f"{(time.time()-last_act):.0f}s y bridge silencioso {time_since_sync:.0f}s — RESETEANDO"
                                )
                                self.ipc.stats['open_positions'] = 0
                                self.ipc.stats['current_direction'] = None

                        # Señal 3: stale clásico — open_positions > 0 por más de 600s sin any_action_time
                        elif open_pos > 0 and last_act == 0 and (time.time() - getattr(self, '_start_time', time.time())) > 600:
                            self.logger.warning(
                                f"[RECONCILE] 🕐 open_positions={open_pos} sin last_action_time registrado (startup stale) — RESETEANDO"
                            )
                            self.ipc.stats['open_positions'] = 0
                            self.ipc.stats['current_direction'] = None
        ...
        try:
            ...
            try:
                ...
                with self.ipc.stats_lock:
                    ...
                    # ── Mirror ALL data from terminal dashboard (nova_dashboard.data) ──
                    # nova_dashboard.update() was already called in process_with_full_consciousness
                    # so .data has fully-computed LLM votes/statuses, attack prob, patterns, etc.
                    ...
                    # Consensus % from ALL 12 LLM votes (from live terminal data)
                    ...
                    # Attack system (from computed terminal data, not raw genome)
                    ...
                    # Pattern data (properly formatted name list from terminal)
                    ...
                    # Performance — from terminal dashboard trade-close updates
                    _wr = float(_d.get('win_rate', 0.0) or 0.0)          # may be 0-1 or 0-100
                    if _wr > 1.0: _wr = _wr / 100.0                       # normalise to 0-1
                    ...
                    self.ipc.stats['winrate']   = round(_wr * 100, 1)    # percent
