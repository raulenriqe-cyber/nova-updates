...
║  Version: 2.0 PRODUCTION - EURUSD M15                                       ║
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 1.0500))
...
def run_tcp_server():
    ...
    # EURUSD M5 - Port 6562 (matches euquantum_core.py Config.LLM6_PORT)
    try:
        with open('euconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm6_port', 6562)
    except Exception as e:
        port = 6562  # FIXED: Was 6560, now matches euquantum_core Config
