...
║  Port: 6562 (TCP Server) | Pure Python | Low Latency                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import sys, os
...
_log_file = f'logs/llm7_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
_file_handler = logging.FileHandler(_log_file, encoding='utf-8')
_stream_handler = logging.StreamHandler()
_formatter = logging.Formatter("%(asctime)s | 🔍 LLM7_OCULUS | %(message)s", datefmt='%H:%M:%S')
...
class OCULUS:
    ...
    def __init__(self):
        self.port = 6563  # EURUSD LLM7 port (fixed: was 6561)
        self.history = deque(maxlen=100)  # Store last 100 validations
        self.data_quality_threshold = 70  # ADJUSTED: Lower for M15 swing (was 80)
        log.info("[OCULUS] ✨ Data Quality Validator initialized (Port 6562 EURUSD)")
        log.info("[OCULUS] 🔍 Scanning for data anomalies and discontinuities")
        log.info("[OCULUS] ⚡ Pure Python | <50ms latency | M15 optimized")
