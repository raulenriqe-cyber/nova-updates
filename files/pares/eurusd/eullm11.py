...
class NarrativeEngine:
    ...
    def generate_narrative(self, data: dict, macro: dict, cross: dict) -> dict:
        ...
        rsi = data.get('rsi', 50)
        adx = data.get('adx', 20)
        atr = data.get('atr', 0.0012)  # 🔧 Calibrated for EURUSD
        ...
        # DXY cross-asset for EURUSD
        dxy = cross.get('dxy_proxy', 104)
        if dxy > 0:
            # EURUSD IS the anti-DXY (57.6% weight). DXY down = EURUSD up.
            if dxy < 103: buy_signals += 1.5
            elif dxy > 105: sell_signals += 1.5
...
class PhaseDetector:
    ...
    def detect_phase(self, data: dict) -> dict:
        ...
        if adx < 20 and bb_width < 0.004 and volume_ratio > 0.8:  # 🔧 Calibrated for EURUSD
            phase = 'ACCUMULATION'
        elif adx > 25 and price > sma_50 and macd_hist > 0:
            phase = 'MARKUP'
        elif adx < 25 and price > sma_200 and bb_width > 0.006 and volume_ratio < 0.9:  # 🔧 Calibrated for EURUSD
            phase = 'DISTRIBUTION'
        elif adx > 25 and price < sma_50 and macd_hist < 0:
            phase = 'MARKDOWN'
        else:
...
class ScenarioProjector:
    ...
    def generate_scenarios(self, data: dict, narrative: dict, phase: dict) -> dict:
        ...
        price = data.get('price', data.get('bid', 0))
        atr = data.get('atr', data.get('atr_14', 0.0012))  # 🔧 Calibrated for EURUSD
...
class SmartTPSL:
    ...
    def calculate(self, data: dict, strategy: str, conviction: dict,
        ...
        break_even_pips = exec_quality.get('break_even_pips', 0.00020)  # 🔧 Calibrated for EURUSD
        ...
        rr_ratio = reward1 / max(risk, atr * 0.001)  # 🔧 pair-safe floor (was 0.01 → broke EURUSD)
