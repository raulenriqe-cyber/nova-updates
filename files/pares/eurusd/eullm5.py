...
║  Port: 6561 (TCP Server) - EURUSD M15                                        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import sys, os
...
# ============ CONFIGURACIÓN EURUSD M15 ============
HOST = '127.0.0.1'
PORT = 6561  # EURUSD LLM5 port (FIXED: was 6559, skips 6560)
MAX_CANDLES = 500
BUFFER_SIZE = 65536

os.makedirs('logs', exist_ok=True)
_log_file = f'logs/llm5_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
_file_handler = logging.FileHandler(_log_file, encoding='utf-8')
_stream_handler = logging.StreamHandler()
_formatter = logging.Formatter('%(asctime)s | 🎯 LLM5 | %(levelname)s | %(message)s', datefmt='%H:%M:%S')
_file_handler.setFormatter(_formatter)
_stream_handler.setFormatter(_formatter)
logging.basicConfig(level=logging.INFO, handlers=[_file_handler, _stream_handler])
logger = logging.getLogger(__name__)
logger.info(f"[INIT] Log file: {_log_file}")
# ============ LLM5 CHART ANALYZER ============
class LLM5ChartAnalyzer:
    ...
    def analyze(self, symbol='EURUSD', timeframe='M15') -> Dict[str, Any]:
        ...
        # ═══════════════════════════════════════════════════════════════
        # EURUSD M15: Movimientos más pequeños que el oro
        # Typical M15 move: 5-15 pips (vs gold 30-60 pips)
        # Ajustar umbrales de momentum para mayor sensibilidad
        # ═══════════════════════════════════════════════════════════════
        price_mom = closes[-1] - closes[-5] if len(closes) >= 5 else 0
        
        # EURUSD M15: 0.0005 = 5 pips de movimiento
        # Umbrales ajustados para mercado más lento
        MOMENTUM_THRESHOLD_LOW = 0.0003    # 3 pips - momentum mínimo
        MOMENTUM_THRESHOLD_HIGH = 0.0008   # 8 pips - momentum fuerte
        
        # RSI alto + precio subiendo = MOMENTUM ALCISTA → seguir comprando
        if rsi > 55 and price_mom > MOMENTUM_THRESHOLD_LOW:
            bullish_signals += 0.6
            patterns.insert(0, {'name': 'Momentum_Up', 'type': 'bullish', 'confidence': 0.75})
            logger.info(f"[LLM5] 🟢 MOMENTUM UP (RSI={rsi:.1f}, mom={price_mom*10000:.1f}pips)")
        # RSI bajo + precio bajando = MOMENTUM BAJISTA → seguir vendiendo
        elif rsi < 45 and price_mom < -MOMENTUM_THRESHOLD_LOW:
            bearish_signals += 0.6
            patterns.insert(0, {'name': 'Momentum_Down', 'type': 'bearish', 'confidence': 0.75})
            logger.info(f"[LLM5] 🔴 MOMENTUM DOWN (RSI={rsi:.1f}, mom={price_mom*10000:.1f}pips)")
        # Momentum fuerte confirmado (EURUSD: 8+ pips es fuerte en M15)
        elif rsi > 60 and price_mom > MOMENTUM_THRESHOLD_HIGH:
            bullish_signals += 0.8
            patterns.insert(0, {'name': 'Strong_Momentum_Up', 'type': 'bullish', 'confidence': 0.85})
        elif rsi < 40 and price_mom < -MOMENTUM_THRESHOLD_HIGH:
            bearish_signals += 0.8
    ...
    def _trend_patterns(self, closes):
        ...
        # ═══════════════════════════════════════════════════════════
        # TENDENCIA FUERTE (4+ velas consecutivas)
        # ═══════════════════════════════════════════════════════════
        
        # Uptrend fuerte (4 velas alcistas consecutivas)
        if len(closes) >= 5:
            up_count = sum(1 for i in range(-4, 0) if closes[i] > closes[i-1])
            if up_count >= 4:
                patterns.append({
                    'name': 'UPTREND_STRONG',
                    'type': 'bullish',
        ...
        # Downtrend fuerte (4 velas bajistas consecutivas)
        if len(closes) >= 5:
            down_count = sum(1 for i in range(-4, 0) if closes[i] < closes[i-1])
            if down_count >= 4:
                patterns.append({
...
class LLM5Client:
    ...
    @staticmethod
    def analyze(symbol='EURUSD', timeframe='M15', candles=None):
