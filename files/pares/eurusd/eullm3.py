...
║  Port: 6558 (TCP Server) - EURUSD M15                                        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
import sys, os
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'EURUSD').upper().strip()
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port = 6558):  # EURUSD LLM3 port (FIXED: was 6557)
        self.host = host
        self.port = port
...
def main():
    """Punto de entrada"""
    try:
        # Cargar puerto de euconfig.yaml
        global EU_CONFIG_LOADED
        if EU_CONFIG_LOADED:
            try:
                port = get_llm_port(3)  # LLM3 = 6557 from YAML
            except:
                port = 6558  # Fallback (FIXED)
        else:
            port = 6558  # EURUSD LLM3 default port (FIXED: was 6557)
