...
import socket, struct, json, logging, threading, time, pickle
import numpy as np
import requests
from collections import defaultdict, deque
from datetime import datetime

# OLLAMA AI CONFIGURATION - Uses shared Ollama (port 11434)
OLLAMA_HOST = "127.0.0.1"
OLLAMA_PORT = 11434
OLLAMA_MODEL = "llama3:8b"
OLLAMA_TIMEOUT = 8.0  # Fast for M5 analysis
...
class DynamicRiskCalculator:
    ...
    OPTIMIZADO PARA EURUSD M5:
    - ATR típico: 3-12 pips (0.0003 - 0.0012)
    - SL rango: 5-12 pips (tighter for M5)
    - TP rango: 8-20 pips (realistic for 5-7 min moves)
    - R:R objetivo: 1.5:1 a 2.0:1
    
    PREDICCIÓN: 5-7 minutos adelante (1-2 velas M5)
    """
    
    # ═══════════════════════════════════════════════════════════════
    # EURUSD M5 - Parámetros para movimientos rápidos direccionales
    # ═══════════════════════════════════════════════════════════════
    EURUSD_PIP_VALUE = 0.0001  # 1 pip = 0.0001 para EURUSD
    
    # Límites de SL en pips (M5 = SL más ajustados que M15)
    SL_MIN_PIPS = 25.0   # SL mínimo: 25 pips (broker STOPS_LEVEL safe)
    SL_MAX_PIPS = 12.0  # SL máximo: 12 pips
    
    # Límites de TP en pips (5-7 min realistic targets)
    TP_MIN_PIPS = 30.0   # TP mínimo: 30 pips (broker STOPS_LEVEL safe)
    TP_MAX_PIPS = 20.0  # TP máximo: 20 pips
    
    # Multiplicadores ATR por régimen (ajustados para EURUSD M5)
    REGIME_MULTIPLIERS = {
        'VOLATILE': {'sl': 1.0, 'tp': 1.8, 'rr_target': 1.80},  # Alta volatilidad: targets rápidos
        'TRENDING': {'sl': 0.8, 'tp': 2.2, 'rr_target': 2.75},  # Tendencia: dejar correr
        'RANGING':  {'sl': 0.6, 'tp': 1.2, 'rr_target': 2.00},  # Rango: scalp rápido
        'UNKNOWN':  {'sl': 0.8, 'tp': 1.5, 'rr_target': 1.87},  # Default conservador
    }
    
    def calculate_dynamic_targets(self, atr, current_price, regime='UNKNOWN', direction='BUY'):
        """Calcula TP y SL dinámicos OPTIMIZADOS para EURUSD M5"""
        
        # Fallback conservador si no hay ATR válido
        if not atr or atr <= 0:
            sl_pips = 7.0   # Default SL: 7 pips
            tp_pips = 12.0  # Default TP: 12 pips (R:R 1.7:1)
            sl_distance = sl_pips * self.EURUSD_PIP_VALUE
            tp_distance = tp_pips * self.EURUSD_PIP_VALUE
        else:
            # Obtener multiplicadores del régimen
            multipliers = self.REGIME_MULTIPLIERS.get(regime, self.REGIME_MULTIPLIERS['UNKNOWN'])
            
            # Calcular SL basado en ATR
            sl_distance = atr * multipliers['sl']
            sl_pips = sl_distance / self.EURUSD_PIP_VALUE
            
            # Aplicar límites inteligentes
            sl_pips = max(self.SL_MIN_PIPS, min(sl_pips, self.SL_MAX_PIPS))
            sl_distance = sl_pips * self.EURUSD_PIP_VALUE
            
            # Calcular TP basado en ATR y R:R objetivo
            tp_distance = atr * multipliers['tp']
            tp_pips = tp_distance / self.EURUSD_PIP_VALUE
            
            # Aplicar límites inteligentes al TP
            tp_pips = max(self.TP_MIN_PIPS, min(tp_pips, self.TP_MAX_PIPS))
            tp_distance = tp_pips * self.EURUSD_PIP_VALUE
            
            # Verificar R:R mínimo de 1.3:1
            rr_actual = tp_pips / (sl_pips + 0.001)
            if rr_actual < 1.3:
                tp_pips = sl_pips * 1.5  # Forzar mínimo 1.5:1
                tp_pips = min(tp_pips, self.TP_MAX_PIPS)
                tp_distance = tp_pips * self.EURUSD_PIP_VALUE
...
class LLM4RiskManager:
    ...
    def analyze(self, data):
        ...
        try:
            ...
            else:  # FALLBACK: has_prior = False, calculate confidence from market data
                # ═══════════════════════════════════════════════════════════════════════
                # 🐛 FIX CRÍTICO: Cuando LLMs no están listos, calcular direction desde
                # indicadores RSI/MACD en lugar de asumir 'BUY' por defecto
                # ═══════════════════════════════════════════════════════════════════════
                try:
                    ...
                    # Determine direction from RSI + MACD
                    bullish_signals = 0
                    bearish_signals = 0
                    ...
                    if fallback_rsi < 40:
                        bullish_signals += 1
                        prior_reasons.append(f"RSI({fallback_rsi:.0f})<40=BULL")
                    elif fallback_rsi > 60:
                        bearish_signals += 1
                        prior_reasons.append(f"RSI({fallback_rsi:.0f})>60=BEAR")
                    ...
                    if bullish_signals > bearish_signals:
                        prior_direction_hint = 'BUY'
                        prior_confidence_boost = min(15, bullish_signals * 5)
                    elif bearish_signals > bullish_signals:
                        prior_direction_hint = 'SELL'
                        prior_confidence_boost = min(15, bearish_signals * 5)
                    else:
                        # Neutral: no clear direction - use HOLD behavior
                        prior_direction_hint = 'HOLD'
                        prior_confidence_boost = 5
                        prior_reasons.append("RSI/MACD=NEUTRAL→HOLD")
                    
                    log.info(f"[LLM4 FALLBACK] NO LLMs ready! RSI={fallback_rsi:.1f} MACD={fallback_macd:.6f} HIST={fallback_macd_hist:.6f} → {prior_direction_hint} (Bull:{bullish_signals} Bear:{bearish_signals})")
                    
                    prices_np = np.array([float(p) for p in prices if p and p > 0]) if isinstance(prices, (list, tuple)) else np.array([price])
                    if len(prices_np) > 1:
                        ...
                        # Volatility-based confidence: higher volatility = higher confidence in risk management
                        prior_confidence_boost += int(price_range * 30)  # Add vol-based boost
                        prior_reasons.append(f"Vol-boost({int(price_range * 30)}%)")
                except Exception as e:
                    log.warning(f"[LLM4 FALLBACK ERROR] {e}")
                    prior_confidence_boost = 10  # Minimal boost if calculation fails
                    prior_direction_hint = 'HOLD'  # SAFE: Don't assume BUY on error!
                    prior_reasons = ["Fallback-error→HOLD"]
            ...
            if direction == 'HOLD':
                ...
                stop_loss = price - (atr_value * 1.0)  # 1 ATR below
                take_profit = price + (atr_value * 2.0)  # 2 ATR above (RR 1:2)
                # IMPROVE: ajustar TP si confianza es muy alta
                if confidence > 75:
                    take_profit = price + (atr_value * 2.5)  # 2.5 ATR para alta confianza
            else:  # SELL
                stop_loss = price + (atr_value * 1.0)  # 1 ATR above
                take_profit = price - (atr_value * 2.0)  # 2 ATR below (RR 1:2)
                if confidence > 75:
                    take_profit = price - (atr_value * 2.5)  # 2.5 ATR para alta confianza
            ...
            # - Si confianza 60-75%: requiere RR > 1.5
...
def run_tcp_server():
    ...
    # EURUSD M15 - Port +1000 offset
    try:
        with open('config.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm4_port', 6559)
    except:
        port = 6559  # EURUSD LLM4 RISK port (FIXED: was 6558)
