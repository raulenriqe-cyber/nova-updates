...
class LLMPerformanceTracker:
    ...
    def __init__(self, window_size=100):
        ...
        # ALL 10 LLMS - No one left behind!
        llm_names = ['BAYESIAN', 'TECHNICAL', 'CHART', 'RISK', 'SUPREME', 
                     'SMART_MONEY', 'OCULUS', 'CHRONOS', 'PREDATOR', 'NOVA', 'STRATEGIST', 'SENTINEL']
        ...
        # Basado en especialidad y experiencia observada
        self.base_weights = {
            # CORE VOTERS (Decisive power)
            'BAYESIAN': 1.4,      # Probabilistic reasoning - Master of uncertainty
            'TECHNICAL': 1.25,    # 🔧 FIX: 1.1→1.25 - Classic indicators need more voice
            'CHART': 1.3,         # Pattern recognition - Sees what others miss
            'RISK': 1.05,         # 🔧 FIX: 1.0→1.05 - Conservative but shouldn't be ignored
            'SUPREME': 1.5,       # Deep learning ensemble - Neural intelligence
            'SMART_MONEY': 1.6,   # Whale/sweep detector - Follows BIG money 🐋
            
            # ENHANCEMENT LAYER (Quality + Timing + Execution)
            'OCULUS': 1.2,        # Data quality validator - Trust guardian
            'CHRONOS': 1.45,      # 🔧 FIX: 1.4→1.45 - Timing is critical for M1 ⏰
            'PREDATOR': 1.3,      # Execution engine - Risk/Reward master
            'NOVA': 1.5,           # 🔧 FIX: 1.7→1.5 - Has separate audit, reduce double-counting 👑
...
def get_market_session() -> dict:
    """
    ⏰ Detecta la sesión de trading actual para USDCHF/Gold
    ...
    # OFF-HOURS (21:00-24:00 UTC) - Low volatility
        ...
        return {
            ...
            'description': '😴 Off Hours - Low volume, avoid trading'
        }
...
class TrinityConsensus:
    def __init__(self):
        ...
        self.llm_ports = {
            ...
            'BAYESIAN': 6555,      # LLM1 - Probabilistic reasoning
            'TECHNICAL': 6557,     # LLM2 - Technical indicators
            'CHART': 6558,         # LLM3 - Pattern recognition
            'RISK': 6559,          # LLM4 - Risk management
            'SUPREME': 6561,       # LLM5 - Deep learning ensemble
            'SMART_MONEY': 6562,   # LLM6 - Whale/sweep detection
            
            # INTELLIGENCE ENHANCEMENT LAYER
            'OCULUS': 6563,        # LLM7 - Data quality (NOW VOTES!)
            'CHRONOS': 6564,       # LLM8 - Timing optimization (NOW VOTES!)
            'PREDATOR': 6565,      # LLM9 - Execution optimization (NOW VOTES!)
            'NOVA': 6560,           # LLM10 - Supreme auditor (NOW VOTES!)
            'STRATEGIST': 6566,    # CHFUSD LLM11 - Strategist Guru
            'SENTINEL': 6567       # CHFUSD LLM12 - Quality Guardian
        }
        
        # Keep core_llm_ports for backward compatibility (but ALL vote now)
        self.core_llm_ports = {
            'BAYESIAN': 6555, 'TECHNICAL': 6557, 'CHART': 6558,
            'RISK': 6559, 'SUPREME': 6561, 'SMART_MONEY': 6562
        }
        
        self.enhancement_llm_ports = {
            'OCULUS': 6563, 'CHRONOS': 6564, 'PREDATOR': 6565, 'NOVA': 6560, 'STRATEGIST': 6566, 'SENTINEL': 6567
        }
    ...
    def _validate_ecosystem_data(self, genome: dict) -> dict:
        ...
        # 🔧 FIX: Velocity thresholds were TOO SENSITIVE for USDCHF M1
        # Old: ma_delta>0.5, rsi_delta>5, adx_delta>2 → fired constantly → penalty*0.7 killed ALL LLM confidences
        # This was a ROOT CAUSE of CHF never attacking (70% conf * 0.7 = 49% → below 60% min → forced HOLD)
        # New: Much higher thresholds + lighter penalty (0.85 instead of 0.7)
        if ma_delta > 2.0 or rsi_delta > 15 or adx_delta > 8:
            validation['velocity_alert'] = True
            validation['confidence_penalty'] *= 0.85  # 🔧 Was 0.7 → 0.85 (lighter penalty)
        ...
        if len(timeframe_analysis) > 1:
            ...
            if m1_signal:
                ...
                if disagreement_count >= 2:
                    ...
                    validation['confidence_penalty'] *= 0.75  # 🔧 Was 0.5 → 0.75 (TF disagreement shouldn't kill confidence)
                    validation['reason'] += f"TF_DISAGREE({disagreement_count}); "
                    log.warning(f"[ECOSYSTEM] ⚠️ Multi-timeframe disagreement detected")
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        log.info(f"[TRINITY] 🧙 Received {len(llm_responses)} / 10 LLM responses")
        ...
        if llm_decision == tf_decision and llm_decision in ['BUY', 'SELL']:
            ...
            # 🔧 RELAXED: Trade if LLM confidence >= 62% OR if TF has no data (0% confidence)
            # When TF analysis is unavailable, trust the LLM consensus
            if llm_confidence >= 72 or tf_confidence == 0:  # 🔧 FIX: raised from 60/62→72
                final_decision = llm_decision
                # If TF has no data, don't penalize as much
                ...
                # Safety: HOLD when there's conflict AND LLM confidence is low
                final_decision = 'HOLD'
        ...
        rr_ratio = llm_responses.get('PREDATOR', {}).get('rr_ratio', 0)            # Default: unknown
        
        # Ensure numeric types (safety against None/string)
        ...
        if quality_score < 40:
            confidence = max(25, confidence - 15)  # 🔧 FIX: reduced from -25
        ...
        if rr_ratio > 0:
            if rr_ratio < 0.4:
                # 🔧 FIX: R:R veto 0.6→0.4 (M1 scalping has TP=20p/SL=15p = R:R 1.33 natural)
                log.warning(f"[TRINITY] 🚫 R:R VETO: {rr_ratio:.2f} < 0.4 - Trade would have bad R:R")
                decision = 'HOLD'
                confidence = max(20, confidence * 0.3)
            elif rr_ratio < 0.8:
                # R:R marginal - slight penalty only
        ...
        nova_multiplier = nova_response.get('quality_multiplier', 0.85)  # 🔧 FIX: Default 0.7→0.85 (less aggressive)
        ...
        # ═══════════════════════════════════════════════════════════════════
        # OLD CODE: NOVA-MSDA DISABLED - Phase 3 NOVA already applies multiplier
        # Having both caused DOUBLE PENALTY: conf * 0.85 * 0.85 = 0.72 → killed 28% confidence
        # This was a ROOT CAUSE of CHF never attacking
        # ═══════════════════════════════════════════════════════════════════
        if 'nova_quality_multiplier' in genome:
            old_nova_multiplier = float(genome.get('nova_quality_multiplier', 1.0))
            log.debug(f"[TRINITY] 🔌 NOVA-MSDA multiplier {old_nova_multiplier:.2f}x SKIPPED (Phase 3 NOVA already applied)")
        ...
        return {
            ...
            'rr_ratio': rr_ratio,
            'position_multiplier': llm_responses.get('PREDATOR', {}).get('position_multiplier', 1.0),
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for USDCHF)
    ...
    def _aggregate_votes(self, llm_responses: dict) -> tuple:
        ...
        # 🚫 NON-VOTING GATE LLMs: These provide quality/timing/execution data
        # but should NEVER vote on direction. Their native decisions get converted
        # to HOLD in _query_llm, making them phantom HOLD voters.
        NON_VOTING_GATES = {'OCULUS', 'CHRONOS', 'NOVA_AUDIT', 'ABSTAIN'}  # [NOVA AUDIT F-15] ABSTAIN added
        
        for llm_name, llm_data in llm_responses.items():
            decision = llm_data.get('decision', 'HOLD')
            raw_confidence = llm_data.get('confidence', 50)
            
            # 🚫 SKIP NON-VOTING GATES BY NAME
            ...
            if len(self._llm_history[llm_name]) > 8:
                self._llm_history[llm_name] = self._llm_history[llm_name][-8:]
            
            _is_frozen = False
            _frozen_penalty = 1.0
            if len(self._llm_history[llm_name]) >= 5:
                _last_5 = self._llm_history[llm_name][-5:]
                _variance = max(_last_5) - min(_last_5)
                # NOTE: In low-volatility SQUEEZE markets, stable LLM outputs are CORRECT,
                # not broken. Only penalize if variance is truly zero (literally same value
                # every tick = likely stuck/buggy LLM) vs just stable in a quiet market.
                if _variance == 0.0:  # Exactly zero variation = truly FROZEN (buggy)
                    _is_frozen = True
                    _frozen_penalty = 0.6  # was 0.3 — less aggressive, still votes with penalty
                    log.warning(f"[BANK-GRADE] 🧊 FROZEN LLM: {llm_name} = {raw_confidence:.1f}% (var={_variance:.2f}) → weight x0.6 (penalized, still votes)")
                    # NOTE: No 'continue' — frozen LLMs still vote with penalty
                elif _variance < 2.0:  # Very low variation = semi-frozen
                    _frozen_penalty = 0.85  # was 0.5 — gentle penalty for stable market
                    log.info(f"[BANK-GRADE] 🥶 SEMI-FROZEN: {llm_name} = {raw_confidence:.1f}% (var={_variance:.2f}) → weight x0.85")
        ...
        hold_score = calculate_action_score(signals['HOLD'], posterior_hold) * 0.70  # 🔧 FIX: HOLD dampening relaxed (was 0.5)
        ...
        if buy_score > sell_score and buy_score > hold_score:
            ...
            # 🧠 SCORE DOMINANCE: When BUY crushes others, signal is clear
            score_ratio = buy_score / (max(sell_score, hold_score) + 1.0)  # 🔧 FIX: epsilon=1.0 not 1e-10
            if score_ratio >= 2.0:
                dominance_bonus = min(1.12, 1.0 + (score_ratio - 2.0) * 0.03)  # 🔧 FIX: capped at 1.12
                log.info(f"[Trinity] 🧠 BUY SCORE DOMINANCE: ratio={score_ratio:.1f}x → bonus={dominance_bonus:.2f}")
            else:
                dominance_bonus = 1.0
            
            # Smart divergence: only penalize if significant
            divergence_factor = max(divergence - 0.2, 0) * 0.35
            
            # 🧠 FIX: BLEND raw LLM confidence with Bayesian ensemble score
            # raw_conf averages individual LLMs (often 30-47%), killing valid signals
            blended_conf = raw_conf * 0.6 + buy_pct * 0.4  # 🔧 FIX: 60% raw + 40% Bayesian
            confidence = round(blended_conf * majority_bonus * dominance_bonus * (1 - divergence_factor))
            ...
            if vote_count >= 2:
                ...
                if high_conf_voters >= 2:
                    ...
                    if conviction_boost > 0.02:
                        log.info(f"[Trinity] 🔥 BUY CONVICTION: {high_conf_voters} strong voters → +{conviction_boost:.1%}")
            
            # 🔥 INTELLIGENT THRESHOLD: Dynamic min_confidence (was static 60 → death trap)
            ...
            # 🧠 FIX: BLEND raw + Bayesian
            blended_conf = raw_conf * 0.6 + sell_pct * 0.4  # 🔧 FIX: 60% raw + 40% Bayesian
            confidence = round(blended_conf * majority_bonus * dominance_bonus * (1 - divergence_factor))
            ...
            if vote_count >= 2:
                ...
                if high_conf_voters >= 2:
                    ...
                    confidence = round(confidence * (1.0 + conviction_boost))
                    if conviction_boost > 0.02:
                        log.info(f"[Trinity] 🔥 SELL CONVICTION: {high_conf_voters} strong voters → +{conviction_boost:.1%}")
            
            # 🔥 INTELLIGENT THRESHOLD: Dynamic (was static 60)
            if score_ratio >= 2.0 and vote_count >= 2:
                min_confidence = 38
            elif vote_count >= 4 and divergence <= 0.3:
                min_confidence = 42
        ...
        if len(signals[decision]) == num_llms and num_llms >= 5:  # 🔧 FIX: require min 5 voters
            confidence = min(95, confidence + 6)  # 🔧 FIX: +6% not +12%
            log.info(f"[BANK-GRADE] 🎯 PERFECT AGREEMENT! All {num_llms} LLMs voted {decision} (+12%)")
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        """
        🚀 QUERY LLM10 NOVA via TCP - Same protocol as all other LLMs
        No HTTP - pure TCP with 4-byte big-endian header + JSON
        """
        ...
        try:
            ...
            log.error(f"[NOVA CONNECTION] ❌ LLM10 not running on port 8605")
            return self._fallback_nova_response()
    ...
    def _validate_audit_context(self, context: dict) -> dict:
        ...
        if bid > 0 and ask > 0:
            if bid > ask:  # FIX: was >= (spread=0 is valid, eurusd often has 0 spread)
    ...
    def _fallback_nova_response(self) -> dict:
        """FALLBACK: Si LLM10 falla, ¿qué usar? (Hueco #23)"""
        
        return {
            'is_valid': True,
            'quality_multiplier': 0.90,  # 🔧 FIX: Was 0.7 - too aggressive, killed 30% confidence when LLM10 offline
            'audit_score': 75,
...
# ⭐ TCP SERVER - Trinity accepts genomes from quantum_core on port 7666
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                threading.Thread(target=self.dashboard.loop, daemon=True, name='DashDisplay').start()
        ...
        try:
            ...
            if self.dashboard:
                try:
                    ...
                    LLM_MAP = {
                        'BAYESIAN': 'LLM1', 'TECHNICAL': 'LLM2', 'CHART': 'LLM3',
                        'RISK': 'LLM4', 'SUPREME': 'LLM5', 'SMART_MONEY': 'LLM6',
                        'OCULUS': 'LLM7', 'CHRONOS': 'LLM8', 'PREDATOR': 'LLM9',
                        'NOVA': 'LLM10', 'STRATEGIST': 'LLM11', 'SENTINEL': 'LLM12'
                    }
                    ...
                    # Update LLM5 Supreme context (patterns, momentum, volatility)
                    supreme_data = llm_responses.get('SUPREME', {})
                    if supreme_data:
                        self.dashboard.update_llm5_context(supreme_data)
                except Exception as e:
