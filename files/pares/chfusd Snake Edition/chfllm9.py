...
║  Port: 8610 (TCP Server) | Pure Python + Optional Ollama | <100ms latency   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM9:
    ...
    def __init__(self):
        ...
        log.info(f"[LLM9] Port: 8610 | Hybrid: Deterministic + Ollama Context")
...
¿Contexto? 1-palabra: (Fuerte/Normal/Débil/Caótico)"""
    ...
    def _calculate_stop_loss(self, data):
        ...
        🔧 v2: Ajustado para broker STOPS_LEVEL de USDCHF (~30 pips mínimo)
        """
        try:
            price = float(data.get('price', 0))
            ...
            # 🔧 FIX v7: USDCHF 5-digit (0.88xxx) → 1 pip = 0.0001
            # Broker STOPS_LEVEL ~30 points = 3 pips → clamp 8-15 pips (0.0008 - 0.0015 price units)
            
            # 🚨 SANITY CHECK: Absolute max 25 pips for volatile spikes
            # "Islas Marianas" protection: prevent ultra-wide stops
            MAX_SL = 0.0025  # 25 pips absolute max
            MIN_SL = 0.0008  # 8 pips min
            
            sl_distance = max(MIN_SL, min(MAX_SL, sl_distance))
            
            # Final integrity check
            if sl_distance > 0.0030: # If somehow it's still > 30 pips
                 sl_distance = 0.0015 # Reset to 15 pips default
            ...
            # Ensure SL is not negative (sanity check)
            if stop_loss < 0:
                 stop_loss = price - 0.0015 # Fallback
            
            log.info(f"[LLM9 🎯 SL] {decision}: price={price:.5f} sl={stop_loss:.5f} dist={sl_distance:.5f} ({sl_distance/0.0001:.1f} pips) (ADX={adx:.0f})")
            return stop_loss
    ...
    def _calculate_take_profit(self, data):
        ...
        try:
            ...
            if adx > 60:
                rr_ratio += 0.3  # Tendencia muy fuerte = +0.3 R:R
            elif adx > 45:
                rr_ratio += 0.15  # Tendencia fuerte = +0.15 R:R
            
            # Calcular TP basado en SL distance
            # 🚨 SANITY CHECK: "Islas Marianas" protection for TP
            # Max TP 50 pips (0.0050) to prevent unrealistic targets
            ...
            if decision == 'BUY':
                ...
                # Cap TP distance
                tp_distance = min(tp_distance, 0.0050)
            ...
            log.info(f"[LLM9 🎯 TP] {decision}: price={price:.5f} tp={take_profit:.5f} R:R={rr_ratio:.1f}:1 (dist={tp_distance/0.0001:.1f} pips)")
            return take_profit
    ...
    def analyze(self, data):
        ...
        try:
            ...
            log.info(f"       • Risk: {abs(risk):.5f} ({abs(risk)/0.0001:.1f} pips) | Reward: {abs(reward):.5f} ({abs(reward)/0.0001:.1f} pips)")
            ...
            # R:R validation - 🔧 JPY-STYLE: Only penalize very poor R:R
            if rr_ratio < 1.2:
            ...
            # └─────────────────────────────────────────────────────────────────┘
            # 🔧 JPY-STYLE: Lowered thresholds (was 61%/1.5 - too restrictive)
            if consensus_confidence >= 50 and consensus_decision in ['BUY', 'SELL']:
                if quality_score >= 50 and rr_ratio >= 1.2:
                    decision = 'EXECUTE'
                    ...
                    decision = 'READY'
                    execution_status = 'READY'
                    reason = f"Conf OK, R:R acceptable - ready"
                    log.info(f"[LLM9] ⏳ READY (R:R={rr_ratio:.2f})")
                else:
                    decision = 'READY'
                    execution_status = 'READY'
