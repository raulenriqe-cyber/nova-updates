...
# Add parent directory to path for shared modules (XAU modules used by CHF)
...
# 🧠 SHARED BRAIN — Cross-pair intelligence (CHFUSD)
try:
...
class ConfluenceGate:
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        if trinity_llms:
            # 🧠 WEIGHTED VOTING SYSTEM - No todos los LLMs son iguales
            # Para SEK/CHF: Smart Money (LLM6) y Technical (LLM2) son CRÍTICOS
            llm_weights = {
                'LLM6': 2.0,  # NOVA Smart Money - El BOSS
                'LLM2': 1.5,  # Technical Analysis - La estructura
                'LLM9': 1.5,  # Predator Execution - El timing
                'LLM10': 1.2, # Nova Central - La visión global
                'LLM1': 1.0, 'LLM3': 1.0, 'LLM4': 1.0, 'LLM5': 1.0, 
                'LLM7': 1.0, 'LLM8': 1.0
            }
            ...
            for llm_name, llm_data in trinity_llms.items():
                if isinstance(llm_data, dict):
                    ...
                    if vote and conf > 0:
                        ...
                        # Apply weight adjustment based on confidence
                        if conf >= 80: weight *= 1.2  # High confidence boosts weight
                        elif conf < 40: weight *= 0.8 # Low confidence reduces weight
            ...
            if total_weight_active > 0 and raw_count >= 3:
                # Calcular consenso PONDERADO
                weighted_ratio = weighted_votes_for / total_weight_active
                
                # CONSENSO FUERTE PONDERADO: 60%+ del PESO (Smart Money > Randoms)
                if weighted_ratio >= 0.60:
                    base_boost = 1.25 + (weighted_ratio - 0.60) * 0.6  # Más agresivo
                    llm_boost = base_boost
                    evidence.append(f"🧠 WEIGHTED CONSENSUS: {raw_votes_for}/{raw_count} (Power: {weighted_ratio:.0%}) votan {direction} (+{int((llm_boost-1)*100)}%)")
                    base_prob = max(base_prob, 0.60)  # Elevar base mínima más agresivamente
                    
                    # 🌟 GOLDEN TICKET: Si LLM6 (Smart Money) está a favor con alta confianza
                    if 'LLM6' in trinity_llms and trinity_llms['LLM6'].get('decision') == direction:
                         if trinity_llms['LLM6'].get('confidence', 0) > 75:
                             llm_boost += 0.15
                             evidence.append("🌟 GOLDEN TICKET: LLM6 Smart Money apoya fuertemente (+15%)")

                # CONSENSO MODERADO
                elif weighted_ratio >= 0.45:
                    llm_boost = 1.10 + (weighted_ratio - 0.45) * 0.3
                    evidence.append(f"🧠 WEIGHTED SUPPORT: Power {weighted_ratio:.0%} a favor (+{int((llm_boost-1)*100)}%)")
                
                # CONTRADICCIÓN
                elif weighted_votes_against > weighted_votes_for:
                    llm_boost = 0.75 - (weighted_votes_against / total_weight_active) * 0.2
                    warnings.append(f"🚫 WEIGHTED VETO: Power {weighted_votes_against/total_weight_active:.0%} EN CONTRA (-{int((1-llm_boost)*100)}%)")
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # 🔧 JPY-STYLE: 85%+ (was 94%+ ORACLE - nearly unreachable)
            # Sin calc_entry_score gate (era barrera adicional innecesaria)
            # ═══════════════════════════════════════════════════════════════
            if trinity_confidence >= 85 and not sweep_trap_detected:
                llm_wisdom = breakdown.get('2_llm_wisdom', 0)
                if llm_wisdom >= 1.2:  # 🔧 Relaxed: 1.2 (was 1.60 ORACLE)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 VETO POR PROBABILIDAD MUY BAJA - ORACLE MODE
            # Si la probabilidad es <28%, NO atacar - MUST KNOW, not guess
            # ⭐ ORACLE: Raised from 26% to 28% (+7.7%)
            # ═══════════════════════════════════════════════════════════════
            if consciousness_prob < 0.38:  # 🔧 FIX: Raised from 28% to 38% - reject weak signals
                self._clear_pending(trinity_decision)
                return False, 'HOLD', trinity_confidence * 0.2, \
                       f"🧠 CONSCIOUSNESS VETO: Prob={consciousness_prob:.0%} < 38% | {warnings_str}"
            ...
            if len(closes) >= 3 and closes[-3] > 0:
                ...
                # Umbral: 0.04% = fuerte contra (USDCHF: ~3.6 pips)
                strong_against_pct = 0.04
            ...
            # 🔧 JPY-STYLE: Sin calc_entry_score gate (era la barrera principal)
            if consciousness_prob >= 0.70:
                self._clear_pending(trinity_decision)
            ...
            # 🔧 JPY-STYLE: prob>=0.55, sin calc gate
            if score >= self.min_perfect_score and consciousness_prob >= 0.55:
                self._clear_pending(trinity_decision)
            ...
            # 🔧 JPY-STYLE: prob>=0.50, sin calc gate
            if score >= self.min_good_score and consciousness_prob >= 0.50:
                self._clear_pending(trinity_decision)
            ...
            # 🔧 JPY-STYLE: prob>=0.48, sin calc gate
            if score >= self.min_wait_score and consciousness_prob >= 0.48:
                self._clear_pending(trinity_decision)
            ...
            # 🔧 JPY-STYLE: prob>=0.45, sin calc gate
            if score >= self.min_sweep_score and consciousness_prob >= 0.45:
            ...
            # 🎯 TIER 5: Trinity Force - Solo si Prob es decente (>=20%)
            # 🔧 RELAXED for CHF: conf>=50, prob>=0.20, score>=1.0
            if trinity_confidence >= 50 and score >= 1.0 and consciousness_prob >= 0.20:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.85, \
            ...
            # 🔧 RELAXED for CHF: conf>=55, prob>=0.20, score>=0.8
            if trinity_confidence >= 55 and score >= 0.8 and consciousness_prob >= 0.20:
                self._clear_pending(trinity_decision)
...
class Config:
    HOST = '127.0.0.1'
    # 🔧 FIX: Reverted to 8654/8656 — matches the COMPILED MQ5 bridge in MT5
    # SEK was moved to 8664/8666 to avoid conflict
    QUANTUM_PORT = 8654      # Injector - receive genomes from Quimera (CHF)
    EXECUTOR_PORT = 8656     # Executor - send commands to Quimera (CHF)
    ...
    MIN_CONFIDENCE = 61              # 🔧 +1%: 61%+ (was 60) - slightly stricter for quality
    MIN_TRINITY_CONFIDENCE = 54      # 🔧 +1%: 54%+ (was 53) - higher bar for Trinity
    MIN_ADX = 22                     # 🔧 JPY-STYLE: 22 (was 35 - blocked most setups)
...
class NOVADashboard:
    ...
    def render(self):
        ...
        lines.append(f"  {self.MAGENTA}{self.BOLD}10-LLM BRAIN{self.RESET} (9 Voters + MSDA Auditor)")
...
class AdaptiveTPSLEngine:
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
        ...
        """
        Calculate INTELLIGENT TP/SL for M1 USDCHF using ALL available data:
        ...
        """
        # Extract indicators
        atr = float(indicators.get('atr', 0.0005))
        rsi = float(indicators.get('rsi', 50))
        adx = float(indicators.get('adx', 25))
        # 🔧 FIX: BB defaults were +/-5 (Gold units). USDCHF should be +/- 0.0050
        bb_upper = float(indicators.get('bollinger_upper', current_price + 0.0050))
        bb_lower = float(indicators.get('bollinger_lower', current_price - 0.0050))
        ma_fast = float(indicators.get('ma_fast', current_price))
        ma_slow = float(indicators.get('ma_slow', current_price))
        
        # Ensure minimum ATR for USDCHF M1
        # 🔧 FIX: Was 0.30-2.0 (Gold units). USDCHF raw ATR is ~0.0003-0.0010
        atr = max(0.0003, min(0.0020, atr))  # Clamp ATR 0.0003-0.0020 for USDCHF M1 safety
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 USDCHF typical ATR: 0.3-1.5 (in price = 3-15 pips)
        ...
        if support_resistance:
            # 🔧 FIX: Was ±10 (Gold $). USDCHF: ±0.0050 (50 pips)
            nearest_support = support_resistance.get('support', current_price - 0.0050)
            nearest_resistance = support_resistance.get('resistance', current_price + 0.0050)
            ...
            if direction == 'BUY':
                ...
                if max_tp_distance > 0 and max_tp_distance < atr * base_tp_multiplier:
                    # 🔧 FIX: Was max(atr, 0.01). USDCHF: max(atr, 0.0001)
                    base_tp_multiplier = max_tp_distance / max(atr, 0.0001)
            else:  # SELL
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 USDCHF SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico USDCHF: 0.5-2.0 (en dólares)
        ...
        # ATR viene en dólares para USDCHF (ej: 1.5 = $1.50 de movimiento)
        # Clamp ATR a rangos razonables para M1
        atr_clamped = max(0.0003, min(0.0030, atr))  # Min $0.30, Max $3.00
        
        # ⭐ MULTIPLICADORES PARA M1 SCALPING
        # 1.1 / 0.8 = user-specified base for M1 scalping
        # base_tp/sl_multiplier = accumulated intelligent adjustments from Steps 2-6.5
        TP_MULTIPLIER = 1.1 * base_tp_multiplier  # Apply intelligent adjustments
        SL_MULTIPLIER = 0.8 * base_sl_multiplier  # Apply intelligent adjustments
        ...
        # 🔧 FIX: Reduced to realistic M1 scalping distances
        # ATR-based calculation is PRIMARY, minimums are just safety net
        # ═══════════════════════════════════════════════════════════════════════════
        MIN_TP = 0.0005   # 5 pips minimum TP (realistic M1 candle-based)
        MIN_SL = 0.0004   # 4 pips minimum SL (tight but broker-safe)
        SAFETY = 0.0002   # 2 pips safety buffer
        
        tp_distance_price = max(MIN_TP, tp_distance_price)
        sl_distance_price = max(MIN_SL, sl_distance_price)
        
        # ⚙️ HARD CAP for M1 scalping
        tp_distance_price = min(0.0025, tp_distance_price)   # Máximo 25 pips TP
        sl_distance_price = min(0.0015, sl_distance_price)   # Máximo 15 pips SL
        ...
        # Convert to pips for return (1 pip = $0.10 for USDCHF)
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.0005) if atr > 0 else 0
...
class QuantumConsciousnessEngine:
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        try:
            ...
            if consensus_score < 0.50 and trinity_decision in ['BUY', 'SELL']:
                ...
                if llm_consensus_decision == trinity_decision and llm_consensus_confidence >= 55:
                    ...
                    self.logger.warning(f"[CONSENSUS_VETO] 🚫 BLOCKING: Consensus {consensus_score*100:.0f}% < 50% (reason: {consensus_reason}) - No 10-LLM conviction backup")
                    return {
                        'action': 'HOLD',
            ...
            # Only block if ATR is extremely low (< 3 pips raw) which indicates a broken/frozen market
            # 🔧 FIX: Was 0.5 (Gold units!) - USDCHF raw ATR is ~0.0005, so 0.5 ALWAYS blocked!
            # All other pairs (EUR/GBP/SEK/JPY/AUD) correctly use 0.0003
            if atr_value < 0.0003:  # < 3 pips = dead/frozen market for USDCHF
                self.logger.warning(f"[CONSOLIDATION_VETO] 🚫 Blocking: ATR={atr_value:.6f} < 0.0003 (frozen market)")
...
class AdvancedIPC:
            ...
            if data:
                ...
                if request.get('type') == 'handshake':
                    ...
                    ping_thread = threading.Thread(
                        target=self._handle_quimera_connection,
                        args=(client, addr),
                        daemon=True
                    )
                    ping_thread.start()
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            # 1. Validate price is within realistic bounds (USDCHF: 0.5000-1.5000 range)
            # 2. Validate stops are within reasonable distance from entry (5-60 pips)
            ...
            self.logger.info(f"[EINSTEIN] 🎯 Starting calculation: {action} @ Entry={entry_price:.5f}")
            
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR USDCHF
            # USDCHF typical range: 0.5000-1.5000 (5-digit broker: 0.8xxxx)
            ...
            if abs(entry_price - original_entry) > 0.0010:  # > 10 pips change (USDCHF: 1 pip = 0.0001)
                self.logger.warning(f"[EINSTEIN] ⚠️ Entry price changed significantly: original={original_entry:.5f}, current={entry_price:.5f} (diff={abs(entry_price-original_entry):.5f})")
            
            # Use ATR-based approach (1.1x for TP, 0.8x for SL) - PROVEN WORKING
            atr_val = decision.get('atr', 0.0010)
            atr_original = atr_val
            
            # ⚠️ CRITICAL: Clamp ATR to M1 USDCHF realistic range
            # USDCHF M1 ATR típico: 0.0003-0.0030 (3-30 pips, 1 pip = 0.0001)
            ...
            # 🔧 SMART CANDLE-BASED TP/SL: Use recent candle body, not just ATR
            # This is MORE INTELLIGENT than static minimums
            if highs and lows and len(highs) >= 5 and len(lows) >= 5:
                ...
                # Clamp to realistic M1 USDCHF range
                ...
                tp_distance = blended * 1.8  # ~1.8x blended for TP (9-12 pips typically)
                sl_distance = blended * 1.2  # ~1.2x blended for SL (6-8 pips typically)
                self.logger.info(f"[EINSTEIN] 📊 Candle-blend: avg_body={avg_candle_range:.5f}, ATR={atr_val:.5f}, TP={tp_distance:.5f}({tp_distance/0.0001:.0f}p), SL={sl_distance:.5f}({sl_distance/0.0001:.0f}p)")
            else:
                # Fallback: pure ATR
            ...
            # ═══════════════════════════════════════════════════════════════════════════
            # MT5 STOPS_LEVEL REQUIREMENT: Min broker distance
            # 🔧 FIX: Was 20/15 pips — killed ALL intelligent calculation
            # Now: 5/4 pips — allows candle-based TP/SL to work as designed
            # ═══════════════════════════════════════════════════════════════════════════
            MIN_TP_DISTANCE = 0.0005  # 5 pips minimum TP
            MIN_SL_DISTANCE = 0.0004  # 4 pips minimum SL
            SAFETY_MARGIN = 0.0002    # 2 pips buffer
            
            # 🚨 HARD CAP: "Line NY" Protection - Prevent extreme distances
            MAX_TP_DISTANCE = 0.0040  # 40 pips MAX
            MAX_SL_DISTANCE = 0.0025  # 25 pips MAX
            ...
            self.logger.info(f"[EINSTEIN] ✅ CLAMPED: TP={tp_distance:.5f} ({tp_distance/0.0001:.0f}pips) SL={sl_distance:.5f} ({sl_distance/0.0001:.0f}pips)")
            ...
            tp_pips = actual_tp_distance / 0.0001  # Convert to pips
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                if current_price > 0:
                    spread = 0.10  # Half spread for USDCHF (typical spread = 0.20)
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                trinity_offline = time_since_trinity > 5  # No response in 5 seconds
