...
class TrinityConsensus:
    def __init__(self):
        ...
        self.enhancement_llm_ports = {
            'OCULUS': 8608, 'CHRONOS': 8609, 'PREDATOR': 8610, 'NOVA': 8605
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if llm_decision == tf_decision and llm_decision in ['BUY', 'SELL']:
            ...
            if llm_confidence >= 62 or tf_confidence == 0:
                final_decision = llm_decision
        ...
        if timing_multiplier >= 1.5:
            confidence = min(95, confidence + 14)
    ...
    def _aggregate_votes(self, llm_responses: dict) -> tuple:
        ...
        likelihood_hold = calculate_likelihood(signals['HOLD']) * 0.8  # HOLD penalty (🔧 was 0.6 → 0.8: less aggressive, allows waiting on mixed signals)
        ...
        hold_score = calculate_action_score(signals['HOLD'], posterior_hold) * 0.85  # HOLD dampening (🔧 was 0.7 → 0.85: better entry timing, wait for clear signals)
        ...
        if buy_score > sell_score and buy_score > hold_score:
            ...
            # 🎯 ORACLE: Majority bonus (3/6=+6%, 4/6=+18%, 5/6=+30%, 6/6=+42%)
            # Steeper reward for higher LLM agreement (ORACLE: 0.12 vs old 0.09)
            majority_bonus = 1.0 + (vote_count - 2.5) * 0.12
            confidence = int(raw_conf * majority_bonus * (1 - divergence * 0.40))
            
            # 🎯 ORACLE: Mínimo 60% para BUY (was 63% - too restrictive with enhancement modifiers AFTER)
            # quantum_core tiene su propio filtro calc_entry_score como red de seguridad
            if confidence < 60:
                decision = 'HOLD'
                confidence = 50
            ...
            # 🎯 ORACLE: Mínimo 60% para SELL (was 63%)
            if confidence < 60:
                decision = 'HOLD'
                confidence = 50
        ...
        if len(signals[decision]) == num_llms:
