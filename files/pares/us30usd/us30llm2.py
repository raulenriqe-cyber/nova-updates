...
║  Port: 8957 (TCP Server) - NEVER FAILS, ALWAYS RESPONDS                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM2TechnicalExpert:
    ...
    def __init__(self):
        ...
        self.memory = {'US30': deque(maxlen=300)}
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # Consolidation check - M1 US30: ATR very low = reduced confidence
            # CHANGE: Don't return early, just reduce confidence
            # US30 INDEX: ATR in points (5.0 = 5 points, 10.0 = 10 points)
            consolidation_penalty = 0
            if atr < 5.0:
                consolidation_penalty = 5  # Reduce confidence by 5 if extreme consolidation (<5 points)
            elif atr < 10.0:
                consolidation_penalty = 3  # Reduce by 3 if tight consolidation (<10 points)
            ...
            if bull_signals >= 2.2 and technical_score > 0.32:  # Adjusted for reduced scoring (was 2.5/0.30)
                ...
                # 🔧 FIX: Require 2/4 confirmations (was 3/4 — too restrictive)
                if confirmations >= 2:
                    decision = 'BUY'
                    ...
                    if confirmations == 4 and adx_strong:
                        ...
                        confidence = min(85.0, base_conf * 1.15)  # Strong
                        reason = f'✓✓✓ STRONG BULLISH: Bull={bull_signals:.1f} [{confirmations}/4 confirmed]'
                    else:  # confirmations == 2
                        confidence = min(75.0, base_conf * 1.05)  # Good
                        reason = f'✓✓ BULLISH: Bull={bull_signals:.1f} [{confirmations}/4 confirmed]'
                else:
                    decision = 'HOLD'
                    confidence = 50.0
            ...
            # ═══════════════════════════════════════════════════════════════
                ...
                if confirmations >= 2:
                    ...
                    if confirmations == 4 and adx_strong:
                        ...
                        confidence = min(75.0, base_conf * 1.05)  # Good
                        reason = f'✓✓ BEARISH: Bear={bear_signals:.1f} [{confirmations}/4 confirmed]'
                else:
                    decision = 'HOLD'
                    confidence = 50.0
...
def start_server():
    """Inicia servidor TCP en puerto 8957"""
    try:
        config_file = 'us30config.yaml'
        try:
        ...
        port = config.get('llm_settings', {}).get('llm2_port', 8957)
