...
class LLM6_SmartMoney:
    ...
    def _validate_genome(self, genome: Dict) -> bool:
        ...
        if not all(k in genome for k in required_keys):
            ...
            if 'current_price' in genome or 'price' in genome:
                price = genome.get('current_price', genome.get('price', 42000.0))
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (US30 75xx range)"""
        return {
            'trinity_port': 8966,
            'kraken_port': 8952,
            'executor_port': 8956,
...
def run_tcp_server():
    ...
    # Load port from us30config.yaml
    try:
        with open('us30config.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 8962)
    except Exception as e:
        port = 8962  # US30 LLM6 Smart Money Oracle
