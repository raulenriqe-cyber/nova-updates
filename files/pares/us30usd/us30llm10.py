...
║  Ports: 8960 (US30) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=8960, symbol='US30'):
        self.port = port  # US30 LLM10 SENTINEL
        self.symbol = symbol
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # US30 tends to fall on high VIX (flight to USD)
            else:
                score += 0
        ...
        try:
            ...
            if symbol == 'US30':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para US30
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for US30 - caution on long trades'
                        })
...
if __name__ == '__main__':
    ...
    # US30 LLM10 SENTINEL uses port 8960
