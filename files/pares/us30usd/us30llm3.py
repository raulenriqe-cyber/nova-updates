...
║  Port: 8958 (TCP Server) - MATHEMATICAL PRECISION ENHANCED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'US30').upper().strip()
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port=8958):  # FIXED: LLM3 is 8958
        self.host = host
        self.port = port
...
def main():
    ...
    try:
        # Cargar config
        port = 8958  # LLM3 is on 8958
        try:
            with open('us30config.yaml', 'r') as f:
                config = yaml.safe_load(f) or {}
            port = config.get('llm_settings', {}).get('llm3_port', 8958)
        except:
            pass
