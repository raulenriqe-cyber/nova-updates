...
class RealQLearningAgent:
    ...
    def __init__(self, learning_rate=0.15, discount_factor=0.95, epsilon_start=0.3, epsilon_decay=0.995, strategy_tag='default', pip_factor=1):
        """
        ...
        # PERSISTENCE
        self.strategy_tag = str(strategy_tag).lower()
    ...
    def learn_from_trade(self, entry_state, action, exit_price, entry_price, tp, sl, trade_duration):
        ...
        """
        # CALCULATE RAW P&L (US30: price diff IS in points, no conversion needed)
        pnl_pips = (exit_price - entry_price)  # US30: 1 point = 1.0
        risk_pips = (entry_price - sl) if action == 'BUY' else (sl - entry_price)
