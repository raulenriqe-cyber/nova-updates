...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 8952)
  - Audio announcements via sound.py (US30 only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='us30config.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 8952 (not 8956 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 8952)
            self.symbols = config.get('core_engine', {}).get('symbols', ['US30'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 8952  # Default to 8952, NOT 8956
            self.symbols = ['US30']
        
        # 🎯 Mathematical Validation: Order execution precision controls
        self.execution_precision = 2  # Decimal places for price precision (US30 = 2 decimals)
        self.order_validation_enabled = True  # Validate all orders before execution
        self.price_tolerance = 1.0  # Maximum price drift allowed (1 point for US30 index)
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            log.warning(f"[VALIDATION] TP is 0, setting to entry + 50 points")
            tp = entry + 50  # US30: 50 points fallback
            warning = "TP was 0, set to entry + 50pts"
            
        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 points")
            sl = entry - 30  # US30: 30 points fallback
            warning = "SL was 0, set to entry - 30pts"
    ...
    def _execute_order(self, order):
        ...
        try:
            ...
            # Validate symbol
            valid_symbols = {'US30', 'US30.cash', 'US30USD', 'DJI30',
                           'DJ30', 'WS30', 'USTEC'}
            ...
            if DIAGNOSTICS_ENABLED and TRACER:
                TRACER.log_llm_decision(
                    ...
                    reasoning=f"Executed:{order_id} Entry:{entry_price:.2f} TP:{tp_price:.2f} SL:{sl_price:.2f} Lot:{initial_lot:.2f}"
                )
                if DASHBOARD:
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('us30config.yaml')
        kraken.run()
