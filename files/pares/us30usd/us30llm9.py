...
║  Port: 8965 (TCP Server) | Pure Python + Optional Ollama | <100ms latency   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM9:
    ...
    def __init__(self):
        self.port = 8965  # US30 LLM9 PREDATOR
        ...
        log.info(f"[LLM9] Port: 8965 | Hybrid: Deterministic + Ollama Context")
...
¿Contexto? 1-palabra: (Fuerte/Normal/Débil/Caótico)"""
    ...
    def _calculate_stop_loss(self, data):
        ...
        try:
            ...
            # Clamp para broker requirements (M1 scalping US30 index)
            sl_distance = max(8.0, min(50.0, sl_distance))  # US30: 8-50 points
            ...
            log.info(f"[LLM9 🎯 SL] {decision}: price={price:.2f} sl={stop_loss:.2f} dist={sl_distance:.1f}pts")
            return stop_loss
    ...
    def _calculate_take_profit(self, data):
        ...
        try:
            ...
            log.info(f"[LLM9 🎯 TP] {decision}: price={price:.2f} tp={take_profit:.2f} R:R={rr_ratio}:1 (conf={confidence:.0f}%)")
            return take_profit
    ...
    def analyze(self, data):
        ...
        try:
            ...
            log.info(f"       • Risk: {abs(risk):.2f} pts | Reward: {abs(reward):.2f} pts")
            ...
            # └─────────────────────────────────────────────────────────────────┘
            if consensus_confidence >= 45 and consensus_decision in ['BUY', 'SELL']:
