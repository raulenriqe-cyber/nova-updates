...
"""
+------------------------------------------------------------------------------+
�     ?? GILGAMESH - QUANTUM CORE v18.00 US30 WALL STREET EDITION            �
...
1. BAYESIAN (LLM1) - An�lisis probabil�stico
2. TECHNICAL (LLM2) - An�lisis t�cnico puro
...
# ? INTELLIGENT CACHE ENGINE - Optional
try:
    from llm_cache_engine import SuperFastLLMBridge
except ImportError:
    SuperFastLLMBridge = None
...
try:
    ...
    WalkForwardBacktester = EnhancedGeneticAlgorithm = TradeLog = None

try:
    from regime_detector_hmm import MarketRegimeHMM, AdaptiveStrategyMgr, RegimeChangeAlerter
except ImportError:
    MarketRegimeHMM = AdaptiveStrategyMgr = RegimeChangeAlerter = None
...
# ✅ SUPREME CONSCIOUSNESS ENGINE (Meta-Cognitive Trading Intelligence)
try:
    from supreme_consciousness import get_supreme_consciousness, SupremeConsciousnessEngine
    SUPREME_CONSCIOUSNESS_AVAILABLE = True
except ImportError:
    SUPREME_CONSCIOUSNESS_AVAILABLE = False
    get_supreme_consciousness = None
    SupremeConsciousnessEngine = None
    print("[WARNING] Supreme Consciousness not available - running without meta-cognition")
...
# 🧠 NOVA SHARED BRAIN — Cross-pair intelligence (reads from all pairs)
try:
    import sys as _sys_brain_us30
    _sys_brain_us30.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    from nova_shared_brain import record_trade_outcome, update_regime_state, get_cross_pair_prior, get_market_regime, register_open_position, remove_open_position, check_portfolio_allow_trade
    SHARED_BRAIN_AVAILABLE = True
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for US30USD')
except ImportError as _sb_err:
    SHARED_BRAIN_AVAILABLE = False
    print(f'[WARNING] Shared Brain not available: {_sb_err}')

# ?? EXCALIBUR KNIGHT STRATEGY (News-Driven Breakout Elite)
try:
    from excalibur_knight_strategy import (
        TradingStrategy, ExcaliburKnightConfig, ExcaliburSignalAnalyzer,
        ExcaliburNewsManager, get_strategy_config
    )
    EXCALIBUR_AVAILABLE = True
except ImportError:
    EXCALIBUR_AVAILABLE = False
    TradingStrategy = None
    ExcaliburKnightConfig = None
    ExcaliburSignalAnalyzer = None
    ExcaliburNewsManager = None
    print("[WARNING] Excalibur Knight Strategy not available")
...
try:
    from us30llm6 import (  # 🔧 FIX CAT-96
        LLM6_SmartMoney,
        SmartMoneySignal,
        MarketIntention,
        SweepType
...
# "El sistema se observa a s� mismo desde afuera, como un maestro de ajedrez
...
# -------------------------------------------------------------------------------
#
# 1. INDICATOR SYMPHONY (0-1.5) - Armon�a de indicadores t�cnicos
# 2. LLM COLLECTIVE WISDOM (0-1.5) - Sabidur�a colectiva de los 6 LLMs
# 3. SMART MONEY FOOTPRINT (0-1.5) - Huellas del dinero institucional
# 4. MOMENTUM PHYSICS (0-1.0) - F�sica del momentum (velocidad + aceleraci�n)
# 5. VOLATILITY REGIME (0-0.75) - R�gimen de volatilidad �ptimo
# 6. DIVERGENCE DETECTION (0-0.75) - Divergencias ocultas RSI/MACD
# 7. FIBONACCI HARMONY (0-0.75) - Alineaci�n con niveles Fibonacci
# 8. SESSION TIMING (0-0.5) - Hora �ptima (London/NY overlap)
# 9. VOLUME INTELLIGENCE (0-0.5) - Volumen relativo vs promedio
# 10. SQUEEZE DETECTION (0-0.5) - Detecci�n de squeeze pre-explosi�n
...
#   Score 4.5-5.99 ? ? DECAY TIMER (espera m�x 25s, luego ataca)
#   Score 3.0-4.49 ? ?? SOLO si hay SWEEP detectado (smart money)
#   Score < 3.0 ? ?? RECHAZADO (demasiado d�bil)
#
# -------------------------------------------------------------------------------
...
class ConfluenceGate:
    ...
    Features:
    - 12 dimensiones de an�lisis para m�xima precisi�n
    - Decay timer inteligente que NO paraliza
    - Bonus por confluencia m�ltiple
    - Penalizaci�n por contradicciones
    - Memoria de se�ales pendientes
    """
    
    def __init__(self):
        self.pending_signals = {}
        self.decay_max_seconds = 120  # ?? SNIPER M1 - Match cooldown of 120s
        # ?? RELAXED THRESHOLDS FOR US30 - Allow more attacks
        self.min_perfect_score = 2.5   # PERFECT - attack on 2.5+ (was 4.0)
        self.min_good_score = 2.0      # GOOD - quality setups (was 3.0)
        self.min_wait_score = 1.0      # WAIT - minimum quality for attack (was 1.5)
        self.min_sweep_score = 2.0     # SWEEP - trust smart money signals (was 3.5)
        self.lock = Lock()
        ...
        # SETUPS DE ALTA PROBABILIDAD (>75% winrate hist�rico)
        ...
        # NOTE: LLM6 runs as TCP server on port 8962 (US30 89xx range)
        self.LLM6_PORT = 8962  # LLM6 NOVA Smart Money Oracle port (US30)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        """Query LLM6 NOVA Smart Money Oracle via TCP (port 8962)"""
        try:
            ...
            current_price = float(genome.get('tick_data', {}).get('last', genome.get('price', 42000.0)))
            if len(closes) < 3:
    ...
    # Este m�todo SABE en lugar de adivinar. Analiza TODO y calcula
    # la PROBABILIDAD REAL basada en evidencia matem�tica.
    # ---------------------------------------------------------------------------
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        Calcula la PROBABILIDAD REAL de �xito bas�ndose en:
        1. EVIDENCIA BAYESIANA - Cada indicador aporta o resta probabilidad
        2. PATRONES SECUENCIALES - Qu� pas� ANTES predice qu� pasar�
        3. MARKET STATE - En qu� FASE est� el mercado realmente
        4. ANTI-PATTERNS - Detecci�n de setups que SIEMPRE fallan
        ...
        # Esto es lo M�S importante - si los LLMs est�n de acuerdo
        # Guardamos los modificadores para aplicar despu�s
        # ---------------------------------------------------------------
        ...
        if trinity_llms:
            ...
            if llm_count >= 3:
                ...
                # CONSENSO FUERTE: 60%+ LLMs en la misma direcci�n (rebajado de 70% para 10 LLMs)
                if consensus_ratio >= 0.60 and avg_conf >= 55:
                    ...
                    base_prob = max(base_prob, 0.58)  # Elevar base m�nima
                ...
                # CONTRADICCI�N: M�s LLMs en contra que a favor
        ...
        atr = indicators.get('atr', 10.0)  # 10 points default for US30 index
        ...
        # en la direcci�n equivocada bajo NINGUNA circunstancia
        # ---------------------------------------------------------------
        
        # ?? RSI EXTREMO (>90 o <10) = VETO ABSOLUTO en direcci�n incorrecta
        ...
        # ?? RSI EXTREMO pero en LA DIRECCI�N CORRECTA = OPORTUNIDAD
        if direction == 'SELL' and rsi > 90:
            base_prob = 0.75  # Alto base para SELL cuando RSI > 90
            evidence.append(f"?? RSI={rsi:.0f} EXTREMO OVERBOUGHT - SELL �ptimo (+75% base)")
        
        if direction == 'BUY' and rsi < 10:
            base_prob = 0.75  # Alto base para BUY cuando RSI < 10
            evidence.append(f"?? RSI={rsi:.0f} EXTREMO OVERSOLD - BUY �ptimo (+75% base)")
        
        # ---------------------------------------------------------------
        # ?? SETUP RECOGNITION - Combinaciones PROBADAS que ganan
        # Estos son setups con >70% winrate hist�rico documentado
        # ---------------------------------------------------------------
        ...
        def pattern_matches(patterns_list, keywords):
            """Verifica si alg�n patr�n contiene alguna de las keywords"""
        ...
        if direction == 'BUY':
            ...
            # Probabilidad hist�rica: ~78%
            if is_oversold and (has_hammer or has_engulfing_bull):
                setup_detected = "OVERSOLD_REVERSAL"
                setup_probability = 0.78
                evidence.append(f"?? SETUP: {setup_detected} (RSI oversold + Patr�n reversi�n)")
            
            # SETUP 2: THREE WHITE SOLDIERS - Patr�n de 3 velas muy potente
            # Probabilidad hist�rica: ~82%
            ...
            # SETUP 4: SMART MONEY DEMAND - LLM6 detecta zona de demanda + patr�n alcista
            # Probabilidad hist�rica: ~80%
                ...
                evidence.append(f"?? SETUP: {setup_detected} (LLM6 Demanda + Patr�n)")
            
            # SETUP 5: MORNING STAR + Oversold - Patr�n de reversi�n cl�sico
            # Probabilidad hist�rica: ~77%
        ...
        # ---------------------------------------------------------------
            ...
            if is_overbought and (has_shooting_star or has_engulfing_bear):
                ...
                setup_probability = 0.78
                evidence.append(f"?? SETUP: {setup_detected} (RSI overbought + Patr�n reversi�n)")
            
            # SETUP 2: THREE BLACK CROWS - Patr�n de 3 velas muy potente
            # Probabilidad hist�rica: ~82%
            ...
            # SETUP 4: SMART MONEY SUPPLY - LLM6 detecta zona de oferta + patr�n bajista
            # Probabilidad hist�rica: ~80%
            elif is_supply_zone and (has_shooting_star or has_engulfing_bear or has_evening_star):
                setup_detected = "SMART_MONEY_SUPPLY"
                setup_probability = 0.80
                evidence.append(f"?? SETUP: {setup_detected} (LLM6 Supply + Patr�n)")
            
            # SETUP 5: EVENING STAR + Overbought - Patr�n de reversi�n cl�sico
            # Probabilidad hist�rica: ~77%
        ...
        if direction == 'BUY':
            if rsi < 25:
                ...
                likelihood_modifiers.append(1.20)  # +20% en zona de recuperaci�n
                evidence.append(f"RSI={rsi:.0f} zona recuperaci�n (+20%)")
            elif rsi > 75:
                likelihood_modifiers.append(0.60)  # -40% comprando en overbought
                warnings.append(f"?? RSI={rsi:.0f} OVERBOUGHT para BUY (-40%)")
            elif rsi > 65:
            ...
            if rsi > 75:
                ...
                # ?? TREND-AWARE: En tendencia bajista fuerte (ADX>50), RSI oversold
                # puede mantenerse bajo � reducir penalizaci�n
                if adx > 50 and ma5 and ma20 and ma5 < ma20:
                    likelihood_modifiers.append(0.82)  # Solo -18% en downtrend fuerte
                    warnings.append(f"?? RSI={rsi:.0f} OVERSOLD para SELL (downtrend ADX={adx:.0f}, -18%)")
                else:
                    likelihood_modifiers.append(0.60)  # -40% en mercado normal
                    warnings.append(f"?? RSI={rsi:.0f} OVERSOLD para SELL (-40%)")
            elif rsi < 35:
                likelihood_modifiers.append(0.85)
        ...
        if ma5 and ma20:
            if direction == 'BUY':
                if ma5 > ma20:
                    ...
                    warnings.append("?? BUY contra estructura bajista (-30%)")
            else:  # SELL
                if ma5 < ma20:
                    likelihood_modifiers.append(1.30)
                    ...
                    likelihood_modifiers.append(0.70)
                    warnings.append("?? SELL contra estructura alcista (-30%)")
        ...
        # ---------------------------------------------------------------
        # 2. AN�LISIS DE SECUENCIA - Lo que pas� ANTES predice lo que viene
        # ---------------------------------------------------------------
        
        if len(closes) >= 5:
            # Contar velas consecutivas en misma direcci�n
            consecutive_up = 0
            consecutive_down = 0
            ...
            # ANTI-PATTERN: Entrada tard�a despu�s de 4+ velas seguidas
            if direction == 'BUY' and consecutive_up >= 4:
                likelihood_modifiers.append(0.65)
                warnings.append(f"?? {consecutive_up} velas verdes seguidas - entrada tard�a (-35%)")
            elif direction == 'SELL' and consecutive_down >= 4:
                likelihood_modifiers.append(0.65)
                warnings.append(f"?? {consecutive_down} velas rojas seguidas - entrada tard�a (-35%)")
            
            # SETUP IDEAL: Reversi�n despu�s de 3 velas contra
            if direction == 'BUY' and consecutive_down >= 2:
                likelihood_modifiers.append(1.15)
                evidence.append(f"Reversi�n potencial tras {consecutive_down} velas rojas (+15%)")
            elif direction == 'SELL' and consecutive_up >= 2:
                likelihood_modifiers.append(1.15)
                evidence.append(f"Reversi�n potencial tras {consecutive_up} velas verdes (+15%)")
        ...
        # Cuando se rompe la �ltima estructura = oportunidad de entrada
        # ---------------------------------------------------------------
        
        if len(highs) >= 10 and len(lows) >= 10:
            # Identificar swing high y swing low
            swing_high = max(highs[-10:-3])  # No incluir las �ltimas 3 velas
        ...
        for pattern in patterns:
            ...
            if direction == 'BUY':
                if any(bp in p_name for bp in bullish_reversal):
                    likelihood_modifiers.append(1.25)
                    evidence.append(f"Patr�n {p_name} confirma BUY (+25%)")
                elif any(sp in p_name for sp in bearish_reversal):
                    likelihood_modifiers.append(0.70)
                    warnings.append(f"Patr�n {p_name} CONTRA BUY (-30%)")
            else:  # SELL
                if any(sp in p_name for sp in bearish_reversal):
                    likelihood_modifiers.append(1.25)
                    evidence.append(f"Patr�n {p_name} confirma SELL (+25%)")
                elif any(bp in p_name for bp in bullish_reversal):
                    likelihood_modifiers.append(0.70)
                    warnings.append(f"Patr�n {p_name} CONTRA SELL (-30%)")
        
        # ---------------------------------------------------------------
        # 5. SMART MONEY EVIDENCE (Sweeps, LLM6) - ULTRA STRICT VALIDATION
        # Cruzar TODAS las fuentes de smart money para confirmaci�n
        # ---------------------------------------------------------------
        ...
        if llm6_sweep and llm6_sweep != 'NONE':
            ...
            if direction == 'BUY' and llm6_sweep in buy_sweeps:
                ...
                # BONUS: Si tambi�n hay whale confidence alta
                if llm6_whale > 70:
                    smart_money_score += 1
            ...
            # ?? CONTRA SWEEP: Smart money contra nosotros - PELIGRO
            elif direction == 'BUY' and llm6_sweep in sell_sweeps:
                smart_money_score -= 3
                smart_money_reasons.append(f"??{llm6_sweep}_CONTRA")
            ...
            if len(closes) >= 10 and len(highs) >= 10 and len(lows) >= 10:
                ...
                if local_sweep_info and local_sweep_info.get('sweep_detected'):
                    ...
                    if (llm6_is_buy_sweep and local_is_buy_sweep) or (llm6_is_sell_sweep and local_is_sell_sweep):
                        ...
                        smart_money_score -= 2
                        smart_money_reasons.append("??CONFLICT")
        ...
        if volatility_regime == 'HIGH':
            if smart_money_score > 0:
                ...
                smart_money_reasons.append("??HighVol")
        ...
        if llm6_false > 70:
            ...
            warnings.append(f"?? False break {llm6_false:.0f}% (-35%)")
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if len(volumes) >= 20:
                ...
                # Precio sube fuerte pero volumen baja = distribuci�n
                if price_change_20 > 1.0 and vol_recent < vol_20_ago * 0.7:
                    divergence_signals.append('VOL_DISTRIBUTION')
                    divergence_strength += 1
                
                # Precio baja fuerte pero volumen baja = acumulaci�n
                if price_change_20 < -1.0 and vol_recent < vol_20_ago * 0.7:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('US30')
                    if news_conf > 60:
        ...
        if len(closes) >= 10 and len(highs) >= 10 and len(lows) >= 10:
            ...
            if direction == 'BUY':
                ...
                if recent_low < older_low and price_now > recent_low:
                    ...
                    if recovery_pct > 0.1:  # Recuper� algo
                        likelihood_modifiers.append(1.25)
            ...
            # Detectar divergencia bajista: precio hace nuevo HIGH pero momentum debilita
                ...
                if recent_high > older_high and price_now < recent_high:
                    ...
                    if rejection_pct > 0.1:  # Rechaz� algo
                        likelihood_modifiers.append(1.25)
        ...
        # ---------------------------------------------------------------
        # 9. BOLLINGER SQUEEZE DETECTION - Explosi�n inminente
        # Baja volatilidad = acumulaci�n de energ�a
        # ---------------------------------------------------------------
        ...
        if bb_upper and bb_lower and current_price:
            ...
            if bb_width < 0.3:  # Squeeze muy apretado
                # En squeeze, la direcci�n del breakout es crucial
                if direction == 'BUY' and current_price > (bb_upper + bb_lower) / 2:
                    ...
                    likelihood_modifiers.append(1.15)
                    evidence.append(f"?? SQUEEZE extremo - explosi�n inminente (+15%)")
        
        # ---------------------------------------------------------------
        # 10. SESSION TIMING - Horas de mejor probabilidad
        # London/NY overlap = m�xima liquidez y movimientos claros
        # ---------------------------------------------------------------
        
        # Using datetime imported at module level (line 147)
        now = datetime.utcnow()
        hour_utc = now.hour
        ...
        if 13 <= hour_utc <= 16:
            likelihood_modifiers.append(1.15)
            evidence.append("? LONDON/NY OVERLAP - M�xima liquidez (+15%)")
        # Asia session (0:00-7:00 UTC) = Menor liquidez para US30
        elif 0 <= hour_utc <= 6:
            likelihood_modifiers.append(0.90)
            warnings.append("? Sesi�n Asia - menor liquidez (-10%)")
        
        # ---------------------------------------------------------------
        # 11. MOMENTUM INMEDIATO - Las �ltimas 3 velas
        # �El precio se mueve en la direcci�n correcta?
        # ---------------------------------------------------------------
        
        if len(closes) >= 4:
            mom_3 = closes[-1] - closes[-4]  # Momentum de 3 velas
            mom_1 = closes[-1] - closes[-2]  # �ltima vela
            ...
            if direction == 'BUY':
                if mom_3 > 0 and mom_1 > 0:  # Momentum positivo y acelerando
                    ...
                    warnings.append(f"?? Momentum bajista fuerte (-25%)")
            elif direction == 'SELL':
                if mom_3 < 0 and mom_1 < 0:  # Momentum negativo y acelerando
                    ...
                    likelihood_modifiers.append(0.75)
                    warnings.append(f"?? Momentum alcista fuerte (-25%)")
        ...
        # ANTI-PATTERN 1: Entrada tard�a despu�s de movimiento exhaustivo
        if len(closes) >= 6:
            total_move = closes[-1] - closes[-6]  # Movimiento en 6 velas
            atr_normalized = total_move / max(0.01, atr)
            
            if direction == 'BUY' and atr_normalized > 4:  # Subi� 4+ ATRs
                likelihood_modifiers.append(0.40)
                warnings.append(f"?? ANTI-PATTERN: Entrada tard�a (subi� {atr_normalized:.1f} ATRs) (-60%)")
            elif direction == 'SELL' and atr_normalized < -4:  # Cay� 4+ ATRs
                likelihood_modifiers.append(0.40)
                warnings.append(f"?? ANTI-PATTERN: Entrada tard�a (cay� {abs(atr_normalized):.1f} ATRs) (-60%)")
        ...
        # ANTI-PATTERN 4: RSI extremo + direcci�n equivocada
        ...
        if len(volumes) >= 5:
            ...
            if vol_trend < 0 and abs(closes[-1] - closes[-5] if len(closes) >= 5 else 0) > 2 * atr:
                likelihood_modifiers.append(0.65)
                warnings.append("?? ANTI-PATTERN: Gran movimiento con volumen decreciente (-35%)")
        
        # ANTI-PATTERN 6: Doji en zona de indecisi�n (no hay convicci�n)
        if len(opens) >= 1 and len(closes) >= 1 and len(highs) >= 1 and len(lows) >= 1:
            ...
            if body / total_range < 0.1 and adx < 20:  # Doji + ADX bajo
                likelihood_modifiers.append(0.70)
                warnings.append("?? ANTI-PATTERN: Doji sin tendencia = indecisi�n (-30%)")
        
        # ANTI-PATTERN 7: Gap grande que puede revertir
        if len(opens) >= 2 and len(closes) >= 2:
            gap = abs(opens[-1] - closes[-2])
            if gap > 2 * atr:
                # Gaps tienden a cerrarse - cuidado al operar en la direcci�n del gap
                if direction == 'BUY' and opens[-1] > closes[-2]:
                    likelihood_modifiers.append(0.75)
                    warnings.append(f"?? ANTI-PATTERN: Gap alcista grande puede revertir (-25%)")
                elif direction == 'SELL' and opens[-1] < closes[-2]:
                    likelihood_modifiers.append(0.75)
                    warnings.append(f"?? ANTI-PATTERN: Gap bajista grande puede revertir (-25%)")
        
        # ANTI-PATTERN 8: M�ltiples rechazos de nivel (agotamiento)
        if len(highs) >= 10 and len(lows) >= 10:
            ...
            if recent_range < 1.5 * atr:  # Rango muy comprimido
                ...
                if abs(closes[-1] - closes[-5]) > atr:
                    likelihood_modifiers.append(0.70)
                    warnings.append("?? ANTI-PATTERN: Breakout de rango estrecho - puede ser falso (-30%)")
        
        # ---------------------------------------------------------------
        # 13. PRICE VELOCITY - Velocidad del movimiento
        # Movimientos parab�licos = agotamiento inminente
        # ---------------------------------------------------------------
        ...
        # ---------------------------------------------------------------
        # 14. CONSOLIDATION QUALITY - Calidad de la consolidaci�n previa
        # Buena consolidaci�n = mejor breakout
        # ---------------------------------------------------------------
        
        if len(closes) >= 15 and len(highs) >= 15 and len(lows) >= 15:
            # Analizar consolidaci�n antes del movimiento actual
        ...
        try:
            ...
            if trap_result.get('is_trap', False):
                ...
                # Si detectamos una trampa y la direcci�n que queremos es LA MISMA que la trampa
                # = Buena se�al, estamos del lado correcto
                if trap_dir == direction:
                    likelihood_modifiers.append(1.35)
                    evidence.append(f"?? {trap_type}: Trading CON la trampa (+35%)")
                # Si la direcci�n que queremos es CONTRARIA a la trampa = MUY MALO
                else:
        ...
        try:
            ...
            if exhaustion_result.get('is_exhausted', False):
                ...
                # Si la tendencia est� agotada y queremos entrar EN la direcci�n de reversal
                if exhaust_dir == direction:
                    likelihood_modifiers.append(1.25)
                    evidence.append(f"?? {exhaust_type}: Reversal probable (+25%)")
                # Si queremos entrar en la direcci�n de la tendencia agotada = MUY MALO
                elif exhaust_dir:
                    penalty = max(0.4, 0.7 - exhaust_prob * 0.3)
                    likelihood_modifiers.append(penalty)
                    signals_str = ', '.join(exhaust_signals[:2])
                    warnings.append(f"?? {exhaust_type}: Tendencia agot�ndose ({signals_str}) (-{int((1-penalty)*100)}%)")
        except Exception as e:
            pass  # Exhaustion detector optional
        
        # ---------------------------------------------------------------
        # ?? NEWS INTELLIGENCE BIAS - Sentimiento de noticias del mercado
        # Las noticias afectan la direcci�n pero NO son determinantes
        # ---------------------------------------------------------------
        
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('US30')
                
                if news_bias and news_confidence > 0:
                    # News a FAVOR de la direcci�n = boost moderado
                    ...
                    # News EN CONTRA de la direcci�n = penalizaci�n moderada
                    ...
                    # NEUTRAL = leve reducci�n de confianza
    ...
    def calculate_confluence_score(self, 
        ...
        del mercado para tomar la decisi�n �ptima.
        ...
        rsi = indicators.get('rsi', 50)
        adx = indicators.get('adx', 25)
        atr = indicators.get('atr', 10.0)  # Default 10 points for US30 index
        macd = indicators.get('macd', 0)
        macd_signal = indicators.get('macd_signal', 0)
        macd_histogram = macd - macd_signal
        
        # -------------------------------------------------------------------
        # 1. INDICATOR SYMPHONY (0-1.5) - �Los indicadores cantan en armon�a?
        # -------------------------------------------------------------------
        indicator_score = 0.0
        ...
        if trinity_decision == 'BUY':
            if 25 <= rsi <= 40:  # Zona de recuperaci�n �ptima
                indicator_score += 0.5
                ...
                indicator_score -= 0.3
                intelligence_notes.append("?? RSI alto para BUY - momentum agotado")
        elif trinity_decision == 'SELL':
            if 60 <= rsi <= 75:  # Zona de agotamiento �ptima
                indicator_score += 0.5
                indicators_aligned += 1
            elif rsi > 75:  # Extremo overbought - potente pero arriesgado
                indicator_score += 0.4
                intelligence_notes.append("RSI extremo overbought - alta probabilidad de ca�da")
            elif rsi < 35:
                ...
                if adx > 40:  # Tendencia fuerte - RSI bajo confirma momentum
                    ...
                    indicator_score -= 0.2
                    intelligence_notes.append("?? RSI bajo sin tendencia - rebote probable")
        ...
        # ADX Trend Strength (25-50 es �ptimo, >50 puede ser exhausto)
        if 25 <= adx <= 50:
            indicator_score += 0.4
            indicators_aligned += 1
        elif adx > 50:
        ...
        # -------------------------------------------------------------------
        # 2. LLM COLLECTIVE WISDOM (0-1.5) - Sabidur�a de los 5 cerebros
        # -------------------------------------------------------------------
        llm_score = 0.0
        ...
        # -------------------------------------------------------------------
        # 3. SMART MONEY FOOTPRINT (0-2.5) - Huellas institucionales CR�TICAS
        # -------------------------------------------------------------------
        ...
        if llm6_sweep and llm6_sweep != 'NONE':
            ...
            if (trinity_decision == 'BUY' and llm6_sweep in buy_sweep_types) or \
               (trinity_decision == 'SELL' and llm6_sweep in sell_sweep_types):
                ...
                smart_money_score -= 0.3
                intelligence_notes.append(f"?? LLM6 sweep {llm6_sweep} vs Trinity {trinity_decision}")
        ...
        if sweep_info and smart_money_score < 0.5:  # Only if LLM6 didn't give positive
            ...
            if sweep_detected and sweep_type != 'NONE':
                # Sweep alineado con decisi�n = ORO PURO
                ...
                if (trinity_decision == 'BUY' and sweep_type in buy_sweep_types) or \
                   (trinity_decision == 'SELL' and sweep_type in sell_sweep_types):
                    ...
                    # Sweep en direcci�n opuesta
                    smart_money_score -= 0.3
                    intelligence_notes.append(f"?? SWEEP CONTRA: {sweep_type} vs {trinity_decision}")
            ...
            if is_trap:
                if (trinity_decision == 'SELL' and trap_type == 'BULL_TRAP') or \
                   (trinity_decision == 'BUY' and trap_type == 'BEAR_TRAP'):
                    ...
                    intelligence_notes.append(f"?? {trap_type} detectada - {trinity_decision} puede ser trampa!")
            else:
        ...
        # -------------------------------------------------------------------
        # 4. MOMENTUM PHYSICS (0-1.0) - Velocidad + Aceleraci�n del precio
        # -------------------------------------------------------------------
        momentum_score = 0.0
        ...
        # ATR Goldilocks zone: 8-50 points for US30 index scalping
        if 8.0 <= atr <= 50.0:
            volatility_score += 0.5
            intelligence_notes.append(f"? ATR �ptimo para scalping US30: {atr:.1f} pts")
        elif 5.0 <= atr < 8.0:
            volatility_score += 0.3  # Baja vol pero tradeable
        elif 50.0 < atr <= 100.0:
            volatility_score += 0.3  # Alta vol pero tradeable
        elif atr > 100.0:
            volatility_score += 0.1  # Muy vol�til - arriesgado
            intelligence_notes.append(f"?? ATR muy alto ({atr:.1f} pts) - usar stops amplios")
        
        # Bonus: Bollinger squeeze (baja volatilidad = explosi�n inminente)
        if bb_upper and bb_lower:
            bb_width = (bb_upper - bb_lower) / current_price * 100 if current_price else 0
            if bb_width < 0.3:  # Squeeze muy apretado
                volatility_score += 0.25
                intelligence_notes.append("?? SQUEEZE detectado - explosi�n inminente")
        ...
        if len(price_history) >= 5:
            ...
            # Divergencia bajista: Precio sube pero RSI baja (o no sube tanto)
                ...
                divergence_score += 0.4
                intelligence_notes.append("?? Divergencia bajista potencial (precio sube, RSI debilit�ndose)")
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For US30 at ~42000, 0.5% = 210 which is reasonable for fib zone
        ...
        # -------------------------------------------------------------------
        # 8. SESSION TIMING (0-0.5) - Hora �ptima de trading
        # -------------------------------------------------------------------
        timing_score = 0.0
        ...
        if 13 <= hour_utc <= 16:
            timing_score += 0.5
            intelligence_notes.append("? LONDON/NY OVERLAP - M�xima liquidez")
        elif 8 <= hour_utc <= 12:
            timing_score += 0.3  # London solo
        elif 13 <= hour_utc <= 20:
            timing_score += 0.3  # NY solo
        elif 0 <= hour_utc <= 7:
            timing_score += 0.1  # Asia - baja liquidez para US30
        ...
        if volumes and len(volumes) >= 5:
            ...
            if avg_volume > 0:
                ...
                if volume_ratio >= 1.5:
                    volume_score += 0.5
                    intelligence_notes.append(f"?? Volumen {volume_ratio:.1f}x promedio - confirmaci�n fuerte")
                elif volume_ratio >= 1.2:
                    volume_score += 0.3
                elif volume_ratio >= 0.8:
                    volume_score += 0.2
                elif volume_ratio < 0.5:
                    intelligence_notes.append("?? Volumen bajo - se�al d�bil")
        ...
        # -------------------------------------------------------------------
        # 10. SQUEEZE DETECTION (0-0.5) - Pre-explosi�n de volatilidad
        # -------------------------------------------------------------------
        squeeze_score = 0.0
        ...
        if bb_upper and bb_lower and current_price:
            ...
            # ATR hist�rico vs actual
        # Consolidation check: check if ATR is too low (tight range)
            if atr < 5.0 and bb_width < 0.5:
                squeeze_score += 0.5
                intelligence_notes.append("?? SQUEEZE M�XIMO - Explosi�n de volatilidad inminente")
            elif atr < 8.0 and bb_width < 0.7:
                squeeze_score += 0.3
        ...
        potential_reward = atr * 2  # TP t�pico = 2x ATR
        potential_risk = atr * 1    # SL t�pico = 1x ATR
                ...
                for bp in bullish_patterns:
                    if bp in pattern_name_norm:
                        ...
                        if is_uptrend:
                            ...
                            intelligence_notes.append(f"??? BULLISH: {pattern_name.upper()} + UPTREND confirma BUY")
                        elif is_ranging:
                            bonus_multiplier = 0.7  # -30% if ranging (patterns less reliable)
                            intelligence_notes.append(f"??? {pattern_name.upper()} en ranging (reducido)")
                        else:
                            intelligence_notes.append(f"??? BULLISH: {pattern_name.upper()} confirma BUY")
                ...
                for sp in bearish_patterns:
                    if sp in pattern_name_norm:
                        ...
                        if is_downtrend:
                            ...
                            intelligence_notes.append(f"??? BEARISH: {pattern_name.upper()} + DOWNTREND confirma SELL")
                        elif is_ranging:
                            bonus_multiplier = 0.7  # -30% if ranging
                            intelligence_notes.append(f"??? {pattern_name.upper()} en ranging (reducido)")
                        else:
                            intelligence_notes.append(f"??? BEARISH: {pattern_name.upper()} confirma SELL")
        ...
        # -------------------------------------------------------------------
        # ?? 15. MARKET STRUCTURE INTELLIGENCE (0-2.0) - VER EL GR�FICO REAL
        # Analiza Higher Highs/Higher Lows, Lower Highs/Lower Lows
        # Esta es LA dimensi�n m�s importante - lo que un trader VE
        # -------------------------------------------------------------------
        structure_score = 0.0
        ...
        if len(closes) >= 15 and len(highs) >= 15 and len(lows) >= 15:
            ...
            for i in range(2, len(highs) - 2):
                # Swing High: punto m�s alto que 2 velas antes y 2 despu�s
                if highs[i] > highs[i-1] and highs[i] > highs[i-2] and \
                   highs[i] > highs[i+1] and highs[i] > highs[i+2]:
                    swing_highs.append((i, highs[i]))
                
                # Swing Low: punto m�s bajo que 2 velas antes y 2 despu�s
            ...
            # ------------------------------------------------------------
            # APLICAR BONUS/PENALTY SEG�N ALINEACI�N CON DECISI�N
            # ------------------------------------------------------------
            if trinity_decision == 'BUY':
                if structure_bias > 0:
        ...
        # -------------------------------------------------------------------
        # ?? MARKET CONTEXT COHERENCE - "PENSAR M�S" ANTES DE ATACAR
        # Eval�a si TODOS los sistemas apuntan en la misma direcci�n
        # -------------------------------------------------------------------
        coherence_score = 0.0
        signals_buy = 0
        signals_sell = 0
        ...
        if coherence_ratio >= 0.7:  # 70%+ se�ales alineadas = EXCELENTE
            coherence_score = 1.0
            intelligence_notes.append(f"?? COHERENCIA ALTA: {coherence_ratio*100:.0f}% se�ales alineadas")
        elif coherence_ratio >= 0.5:  # 50-70% = BUENA
            coherence_score = 0.5
            intelligence_notes.append(f"? Coherencia OK: {coherence_ratio*100:.0f}% se�ales alineadas")
        elif contradiction_ratio >= 0.5:  # 50%+ se�ales CONTRA = MALO
            coherence_score = -1.0
            intelligence_notes.append(f"?? CONTRADICCI�N: {contradiction_ratio*100:.0f}% se�ales en contra")
        else:
            coherence_score = 0.0
            intelligence_notes.append(f"?? Se�ales mixtas: BUY={signals_buy} SELL={signals_sell} HOLD={signals_hold}")
        ...
        # Las noticias pueden confirmar o contradecir la direcci�n
        # -------------------------------------------------------------------
        news_score = 0.0
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                ...
                if news_bias and news_confidence > 0:
                    # News CONFIRMA la direcci�n = bonus
                    ...
                    # News CONTRADICE la direcci�n = penalty
        ...
        # ML prediction CONFIRMA la direcci�n = bonus
        if ml_prediction in ['BUY', 'SELL']:
            ...
            # ML CONTRADICE la direcci�n = penalty
        ...
        # -------------------------------------------------------------------
        # BONUS/PENALTY: CONFLUENCIA M�LTIPLE
        # -------------------------------------------------------------------
        ...
        if (trinity_decision == 'BUY' and rsi > 75) or (trinity_decision == 'SELL' and rsi < 25):
            penalty = 0.3
            total_score -= penalty
            intelligence_notes.append(f"?? PENALTY: RSI contradice direcci�n (-{penalty})")
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # ?? DEBUG: Log score breakdown para entender qu� falta
            ...
            # ---------------------------------------------------------------
            # ?? CALCULATED ENTRY SCORE - Entrada m�s inteligente y calculada
            # Combina TODAS las se�ales en un score unificado ANTES de decidir
            # ---------------------------------------------------------------
            ...
            price_data = genome.get('price_data', {})
            closes = list(price_data.get('close', []))[-20:]  # Extended for range analysis
            highs = list(price_data.get('high', []))[-20:]    # For entry precision
            lows = list(price_data.get('low', []))[-20:]       # For entry precision
            if len(closes) >= 5:
            ...
            if spread > 5.0:  # > 5 points = wide spread for US30
                calc_entry_score -= 1.0
                calc_entry_reasons.append(f"Spread={spread:.1f}?")
            
            # ?? Factor 6: CROSS-VALIDATION de m�ltiples fuentes (0-4 puntos)
            # Este es el GOLD STANDARD - cuando TODAS las fuentes coinciden
            cross_validation_score = 0
            sources_aligned = []
            sources_conflicting = []
            
            # Source 1: Trinity consensus
            if trinity_confidence >= 65:
            ...
            if macd_aligned and abs(macd_hist) > 0.5:  # 0.5 pt threshold for US30 index MACD
            ...
            # ?? SETUP COMPLETION DETECTOR - El momento m�gico
            ...
            # Validaciones adicionales CR�TICAS antes de atacar
            # ---------------------------------------------------------------
            
            entry_precision_score = 0
            precision_reasons = []
            
            # 1?? CANDLE STRUCTURE: �ltima vela debe confirmar direcci�n
            if len(closes) >= 2 and len(price_data.get('open', [])) >= 2:
                last_close = closes[-1]
                last_open = price_data.get('open', [])[-1]
                last_body = last_close - last_open
                
                # BUY: �ltima vela debe ser verde (body positivo)
                if trinity_decision == 'BUY' and last_body > 0:
                    ...
                    precision_reasons.append("??CandleContra")
            
            # 2?? SPREAD CHECK: Spread bajo = mejor entry
            spread = genome.get('tick_data', {}).get('spread', 0)
            if spread <= 3.0:  # <= 3 points = tight spread for US30
                entry_precision_score += 1
                precision_reasons.append(f"?Spread{spread:.1f}pts")
            elif spread > 8.0:  # > 8 points = danger for US30
                entry_precision_score -= 1
                precision_reasons.append(f"??Spread{spread:.1f}pts")
            
            # 3?? ATR POSITION: No entrar en medio del rango, entrar cerca de extremos
            atr = indicators.get('atr', 10.0)  # Default 10 points for US30 index
            if len(highs) >= 10 and len(lows) >= 10:
            ...
            # 4?? VOLUME SPIKE: Volumen alto = institucionales entrando
            ...
            # 5?? PERFECT TIMING: Si SETUP_COMPLETE + precision score alto
            ...
            # Si detectamos se�ales de sweep, NO entrar (nos van a cazar)
            # ---------------------------------------------------------------
            ...
            if len(closes) >= 10:
                # Detectar wicks largas (se�al de rechazo/trampa)
                ...
                if len(last_5_opens) >= 5:
                    for i in range(len(last_5_highs)):
                        ...
                        if trinity_decision == 'BUY' and upper_wick > body * 2 and upper_wick > 0.50:
                        ...
                        if trinity_decision == 'SELL' and lower_wick > body * 2 and lower_wick > 0.50:
                            sweep_trap_detected = True
                            sweep_reason = f"?? Lower wick ${lower_wick:.2f} > body - rechazo de bajos"
                            break
                
                # Detectar swing failure (precio rompi� nivel y volvi�)
                recent_high = max(highs[-10:])
                recent_low = min(lows[-10:])
                current_price = closes[-1] if closes else 0
                
                # BUY despu�s de tocar m�ximo reciente = trampa de breakout falso
                if trinity_decision == 'BUY':
                    if any(h >= recent_high * 0.999 for h in highs[-3:]) and current_price < recent_high - 0.50:
                        sweep_trap_detected = True
                        sweep_reason = f"?? False breakout: Toc� high {recent_high:.2f}, cay� a {current_price:.2f}"
                
                # SELL despu�s de tocar m�nimo reciente = trampa de breakdown falso
                if trinity_decision == 'SELL':
                    if any(l <= recent_low * 1.001 for l in lows[-3:]) and current_price > recent_low + 0.50:
                        sweep_trap_detected = True
                        sweep_reason = f"?? False breakdown: Toc� low {recent_low:.2f}, subi� a {current_price:.2f}"
            
            # Si detectamos sweep/trampa, reducir calc_entry_score
            if sweep_trap_detected:
                calc_entry_score -= 2.0  # Penalizaci�n fuerte
                calc_entry_reasons.append(sweep_reason[:40])
                logger.warning(f"[SWEEP TRAP] ?? {sweep_reason} | Calc score reducido a {calc_entry_score:.1f}")
            
            # ---------------------------------------------------------------
            # ?? TRINITY FORCE BYPASS - HIGH confidence Trinity overrides gates
            # When Trinity (9 LLMs) agrees at 90%+, trust the consensus
            # INCREASED from 85% to 90% to prevent false entries
            # AHORA: Tambi�n requiere calc_entry_score m�nimo Y no sweep trap
            # ---------------------------------------------------------------
            if trinity_confidence >= 90 and calc_entry_score >= 4.0 and not sweep_trap_detected:
                llm_wisdom = breakdown.get('2_llm_wisdom', 0)
                if llm_wisdom >= 1.5:  # Increased from 1.0 to 1.5 - need stronger LLM agreement
            ...
            # Calcula la PROBABILIDAD REAL de �xito antes de cualquier decisi�n
            # No adivina - SABE bas�ndose en evidencia matem�tica
            ...
            if warnings:
                logger.warning(f"?? WARNINGS: {warnings_str}")
            
            # Guardar probabilidad en breakdown para an�lisis
            ...
            # ---------------------------------------------------------------
            # ?? VETO POR PROBABILIDAD MUY BAJA - ULTRA RELAXED FOR US30
            # Si la probabilidad es <12%, NO atacar bajo ning�n concepto
            # ? RELAXED: De 22% a 12% para permitir m�s ataques en US30
            # ---------------------------------------------------------------
            if consciousness_prob < 0.12:
                self._clear_pending(trinity_decision)
                return False, 'HOLD', trinity_confidence * 0.2, \
                       f"?? CONSCIOUSNESS VETO: Prob={consciousness_prob:.0%} < 12% | {warnings_str}"
            
            # ---------------------------------------------------------------
            # ?? ULTRA EXIGENT DIRECTION VALIDATION
            # Usa TODOS los datos disponibles para validar la direcci�n
            # Traps, Sweeps, Hammers, Cambios de tendencia, Smart Money
            # ---------------------------------------------------------------
            ...
            if not direction_validated:
                ...
                return False, 'HOLD', trinity_confidence * 0.3, \
                       f"?? DIRECCI�N INVALIDADA: {direction_reason} | Score={score:.1f}"
            else:
                notes.append(f"? Direcci�n validada: {direction_reason}")
            ...
            # v3.0: Entrar M�S R�PIDO con la tendencia, NO esperar reversiones
            # ---------------------------------------------------------------
            price_data = genome.get('price_data', {})
            ...
            if len(closes) >= 5:
                ...
                # US30 (~42000) vs EURUSD (~1.08) - percentage based is symbol-agnostic
                current_close = closes[-1]
                pct_threshold = 0.03  # 0.03% movement threshold (works for all index CFDs)
                ...
                # -----------------------------------------------------------
                # ?? NUEVA L�GICA: SEGUIR LA TENDENCIA, NO BLOQUEARLA
                # - BUY: Entrar cuando momentum es POSITIVO (no esperar ca�da)
                ...
                if trinity_decision == 'BUY':
                    ...
                    # ?? ACEPTABLE: Momentum plano pero RSI alto (overbought drop)
                    elif abs(momentum_3) < pct_threshold * 2 and rsi > 60:
                        timing_boost = 0.05
                        notes.append(f"? SELL en overbought (RSI={rsi:.0f})")
                    
                    # ?? SOLO BLOQUEAR: Subida MUY fuerte Y sin se�al de techo
                ...
                # -----------------------------------------------------------
                # ?? CHRONOS BOOST: Si timing_multiplier alto, confiar m�s
                # -----------------------------------------------------------
                ...
                if timing_boost > 0:
                    # Boost se aplicar� en el score total m�s adelante
            ...
            # ---------------------------------------------------------------
            # SMART OVERRIDE: Solo si momentum CONFIRMA la direcci�n
            # ---------------------------------------------------------------
            # SMART OVERRIDE v3: M�s inteligente, menos restrictivo
            # ---------------------------------------------------------------
            ...
            # Esto hace el c�digo compatible con US30, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            if len(closes) >= 3 and closes[-3] > 0:
                # Porcentaje de cambio en �ltimas 3 velas
                last_momentum_pct = ((closes[-1] - closes[-3]) / closes[-3]) * 100
                
                # Umbral: 0.08% = fuerte contra (percentage-based, works for all index CFDs)
            ...
            # ---------------------------------------------------------------
            # TIERS DE ATAQUE - SOLO SI MOMENTUM NO BLOQUE�
            ...
            # Initialize fallback-only variables (used for boost calculations)
            ...
            if self.llm6_enabled:
                try:
                    ...
                    if llm6_response:
                        ...
                        is_trap = llm6_response.get('is_trap', False)  # Indica si LLM6 detect� trampa
            ...
            # ??? INTELLIGENT FALLBACK: Advanced Smart Money Detection without LLM6
            # This is a sophisticated fallback that analyzes raw market data for institutional footprints
            if not llm6_response_received:
                # ---------------------------------------------------------------
                # ?? WHALE DETECTION - Multi-Factor Analysis
                # ---------------------------------------------------------------
                whale_signals = 0
                whale_confidence_factors = []
                
                # Extract key data from genome
                price_data = genome.get('price_data', {})
                indicators = genome.get('indicators', {})
                bar_data = genome.get('bar_data', {})
                tick_data = genome.get('tick_data', {})
                patterns = genome.get('patterns', [])
                
                # Current candle metrics
                high = price_data.get('high', bar_data.get('high', 0))
                low = price_data.get('low', bar_data.get('low', 0))
                open_price = price_data.get('open', bar_data.get('open', 0))
                close = price_data.get('close', bar_data.get('last_price', bar_data.get('close', 0)))
                volume = bar_data.get('tick_volume', tick_data.get('volume', 100))
                avg_volume = bar_data.get('avg_volume', volume)
                
                # -------------------------------------------------------------
                # -1. SPREAD ANALYSIS (Unusual spreads = manipulation)
                # -------------------------------------------------------------
                spread = price_data.get('spread', tick_data.get('spread', 0))
                avg_spread = price_data.get('avg_spread', spread if spread > 0 else 1.5)
                
                if spread > 0 and avg_spread > 0:
                    spread_ratio = spread / avg_spread
                    if spread_ratio > 2.5:
                        # Abnormally wide spread = low liquidity, potential trap
                        false_break_signals += 2
                        whale_confidence_factors.append(f"WIDE_SPREAD_{spread_ratio:.1f}x")
                    elif spread_ratio > 1.8:
                        false_break_signals += 1
                        whale_confidence_factors.append(f"ELEVATED_SPREAD_{spread_ratio:.1f}x")
                
                # -------------------------------------------------------------
                # 0. TIME-BASED INSTITUTIONAL ACTIVITY DETECTION
                # Banks and funds operate in specific windows (UTC times)
                # -------------------------------------------------------------
                from datetime import datetime
                current_hour = datetime.utcnow().hour
                institutional_window = False
                institutional_window_name = ""
                
                # London Open (07:00-09:00 UTC) - High institutional activity
                if 7 <= current_hour <= 9:
                    institutional_window = True
                    institutional_window_name = "LONDON_OPEN"
                    whale_signals += 1
                # NY Open (12:00-14:00 UTC) - Highest volatility
                elif 12 <= current_hour <= 14:
                    institutional_window = True
                    institutional_window_name = "NY_OPEN"
                    whale_signals += 2  # More weight - highest activity
                # London Close (15:00-17:00 UTC) - Position squaring
                elif 15 <= current_hour <= 17:
                    institutional_window = True
                    institutional_window_name = "LONDON_CLOSE"
                    whale_signals += 1
                # Asian session (23:00-02:00 UTC) - Low liquidity, easy manipulation
                elif current_hour >= 23 or current_hour <= 2:
                    institutional_window_name = "ASIAN_LOW_LIQ"
                    # Sweeps in low liquidity are common traps
                    false_break_signals += 1
                
                if institutional_window:
                    whale_confidence_factors.append(f"INST_WINDOW_{institutional_window_name}")
                
                # -------------------------------------------------------------
                # 1. VOLUME SPIKE DETECTION (Institutional footprint)
                # -------------------------------------------------------------
                volume_ratio = volume / avg_volume if avg_volume > 0 else 1.0
                if volume_ratio > 2.5:
                    whale_signals += 3
                    whale_confidence_factors.append(f"MASSIVE_VOLUME_{volume_ratio:.1f}x")
                elif volume_ratio > 1.8:
                    whale_signals += 2
                    whale_confidence_factors.append(f"HIGH_VOLUME_{volume_ratio:.1f}x")
                elif volume_ratio > 1.3:
                    whale_signals += 1
                    whale_confidence_factors.append(f"ELEVATED_VOLUME_{volume_ratio:.1f}x")
                
                # -------------------------------------------------------------
                # 2. WICK ANALYSIS (Sweep/Trap Detection)
                # Long wicks indicate stop hunts and liquidity sweeps
                # -------------------------------------------------------------
                candle_range = high - low if high > low else 0.1  # 0.1 pt fallback for US30 index
                body_size = abs(close - open_price)
                upper_wick = high - max(close, open_price)
                lower_wick = min(close, open_price) - low
                
                # Wick ratios (normalized)
                upper_wick_ratio = upper_wick / candle_range if candle_range > 0 else 0
                lower_wick_ratio = lower_wick / candle_range if candle_range > 0 else 0
                body_ratio = body_size / candle_range if candle_range > 0 else 0
                
                sweep_detected = False
                sweep_direction_fallback = None
                is_trap_fallback = False
                
                # Pin Bar / Hammer / Shooting Star detection
                if lower_wick_ratio > 0.60 and body_ratio < 0.25:
                    # Long lower wick = demand sweep, bullish
                    sweep_detected = True
                    sweep_direction_fallback = 'BUY'
                    whale_signals += 2
                    whale_confidence_factors.append("DEMAND_SWEEP_WICK")
                    if trinity_decision == 'SELL':
                        is_trap_fallback = True
                        whale_confidence_factors.append("??SELL_TRAP")
                        
                elif upper_wick_ratio > 0.60 and body_ratio < 0.25:
                    # Long upper wick = supply sweep, bearish
                    sweep_detected = True
                    sweep_direction_fallback = 'SELL'
                    whale_signals += 2
                    whale_confidence_factors.append("SUPPLY_SWEEP_WICK")
                    if trinity_decision == 'BUY':
                        is_trap_fallback = True
                        whale_confidence_factors.append("??BUY_TRAP")
                
                # -------------------------------------------------------------
                # 3. RSI DIVERGENCE (False Break Detection)
                ...
                false_break_signals = 0
                
                # Bearish divergence: price up, RSI down
                if close > open_price and rsi < rsi_prev and rsi > 60:
                    false_break_signals += 2
                    whale_confidence_factors.append("BEARISH_RSI_DIVERGENCE")
                    if trinity_decision == 'BUY':
                        is_trap_fallback = True
                
                # Bullish divergence: price down, RSI up
                if close < open_price and rsi > rsi_prev and rsi < 40:
                    false_break_signals += 2
                    whale_confidence_factors.append("BULLISH_RSI_DIVERGENCE")
                    if trinity_decision == 'SELL':
                        is_trap_fallback = True
                
                # -------------------------------------------------------------
                # 4. EXTREME RSI + REVERSAL PATTERN = HIGH PROBABILITY TRAP
                # -------------------------------------------------------------
                if rsi > 75 and close < open_price:  # Overbought + bearish candle
                    false_break_signals += 2
                    whale_confidence_factors.append("OVERBOUGHT_REVERSAL")
                    if trinity_decision == 'BUY':
                        is_trap_fallback = True
                        false_break_signals += 1
                        
                elif rsi < 25 and close > open_price:  # Oversold + bullish candle
                    false_break_signals += 2
                    whale_confidence_factors.append("OVERSOLD_REVERSAL")
                    if trinity_decision == 'SELL':
                        is_trap_fallback = True
                        false_break_signals += 1
                
                # -------------------------------------------------------------
                # 5. BOLLINGER BAND ANALYSIS (False Breakout Detection)
                # -------------------------------------------------------------
                bb_upper = indicators.get('bb_upper', indicators.get('bollinger_upper', 0))
                bb_lower = indicators.get('bb_lower', indicators.get('bollinger_lower', 0))
                bb_middle = indicators.get('bb_middle', indicators.get('bollinger_mid', 0))
                
                if bb_upper > 0 and bb_lower > 0:
                    # Price touched upper BB but closed inside = false breakout
                    if high > bb_upper and close < bb_upper and close > bb_middle:
                        false_break_signals += 2
                        whale_confidence_factors.append("BB_UPPER_REJECTION")
                        if trinity_decision == 'BUY':
                            is_trap_fallback = True
                    
                    # Price touched lower BB but closed inside = false breakdown
                    if low < bb_lower and close > bb_lower and close < bb_middle:
                        false_break_signals += 2
                        whale_confidence_factors.append("BB_LOWER_REJECTION")
                        if trinity_decision == 'SELL':
                            is_trap_fallback = True
                
                # -------------------------------------------------------------
                # 5b. ATR ANOMALY DETECTION (Unusual moves = institutional)
                # -------------------------------------------------------------
                atr = indicators.get('atr', indicators.get('atr_14', candle_range * 0.8))
                if atr > 0:
                    move_vs_atr = candle_range / atr if atr > 0 else 1.0
                    
                    # Move > 2x ATR = institutional spike, likely sweep
                    if move_vs_atr > 2.0:
                        whale_signals += 2
                        whale_confidence_factors.append(f"ATR_SPIKE_{move_vs_atr:.1f}x")
                        # If price reversed (long wick), high probability trap
                        if body_ratio < 0.3:  # Small body = reversal
                            false_break_signals += 2
                            whale_confidence_factors.append("ATR_SPIKE_REVERSAL")
                    elif move_vs_atr > 1.5:
                        whale_signals += 1
                        whale_confidence_factors.append(f"ATR_ELEVATED_{move_vs_atr:.1f}x")
                
                # -------------------------------------------------------------
                # 6. MOMENTUM ANALYSIS (MACD/Stochastic Conflicts)
                # -------------------------------------------------------------
                macd = indicators.get('macd', indicators.get('macd_line', 0))
                macd_signal = indicators.get('macd_signal', indicators.get('signal_line', 0))
                stoch_k = indicators.get('stoch_k', indicators.get('stochastic_k', 50))
                
                # MACD/Price conflict
                if trinity_decision == 'BUY' and macd < macd_signal and macd < 0:
                    false_break_signals += 1
                    whale_confidence_factors.append("MACD_BEARISH_CONFLICT")
                elif trinity_decision == 'SELL' and macd > macd_signal and macd > 0:
                    false_break_signals += 1
                    whale_confidence_factors.append("MACD_BULLISH_CONFLICT")
                
                # Stochastic extreme
                if stoch_k > 85 and trinity_decision == 'BUY':
                    false_break_signals += 1
                    whale_confidence_factors.append("STOCH_OVERBOUGHT")
                elif stoch_k < 15 and trinity_decision == 'SELL':
                    false_break_signals += 1
                    whale_confidence_factors.append("STOCH_OVERSOLD")
                
                # -------------------------------------------------------------
                # 6b. SUPPORT/RESISTANCE SWEEP DETECTION
                # Price breaking S/R and returning = liquidity sweep
                # -------------------------------------------------------------
                # Get S/R levels from genome if available
                sr_data = genome.get('support_resistance', genome.get('sr_levels', {}))
                support_levels = sr_data.get('support', sr_data.get('supports', []))
                resistance_levels = sr_data.get('resistance', sr_data.get('resistances', []))
                
                # Also check for levels in patterns
                pattern_sr = genome.get('detected_patterns', {})
                if not support_levels:
                    support_levels = pattern_sr.get('support_levels', [])
                if not resistance_levels:
                    resistance_levels = pattern_sr.get('resistance_levels', [])
                
                # Check for S/R sweep
                for sup in support_levels[:3]:  # Check nearest 3 supports
                    sup_price = sup if isinstance(sup, (int, float)) else sup.get('price', 0)
                    if sup_price > 0:
                        # Low broke below support but close above = demand sweep
                        if low < sup_price and close > sup_price:
                            sweep_detected = True
                            sweep_direction_fallback = 'BUY'
                            whale_signals += 2
                            whale_confidence_factors.append(f"SUPPORT_SWEEP_{sup_price:.2f}")
                            if trinity_decision == 'SELL':
                                is_trap_fallback = True
                                false_break_signals += 2
                            break
                
                for res in resistance_levels[:3]:  # Check nearest 3 resistances
                    res_price = res if isinstance(res, (int, float)) else res.get('price', 0)
                    if res_price > 0:
                        # High broke above resistance but close below = supply sweep
                        if high > res_price and close < res_price:
                            sweep_detected = True
                            sweep_direction_fallback = 'SELL'
                            whale_signals += 2
                            whale_confidence_factors.append(f"RESISTANCE_SWEEP_{res_price:.2f}")
                            if trinity_decision == 'BUY':
                                is_trap_fallback = True
                                false_break_signals += 2
                            break
                ...
                # -------------------------------------------------------------
                # 6c. MARKET STRUCTURE BREAK (MSB) ANALYSIS
                # -------------------------------------------------------------
                prev_high = bar_data.get('prev_high', price_data.get('prev_high', high))
                prev_low = bar_data.get('prev_low', price_data.get('prev_low', low))
                
                # Break of structure with reversal = sweep
                if high > prev_high and close < prev_high:
                    # Broke above previous high but closed back inside
                    whale_signals += 1
                    whale_confidence_factors.append("MSB_UP_FAILED")
                    if trinity_decision == 'BUY':
                        is_trap_fallback = True
                        false_break_signals += 1
                
                if low < prev_low and close > prev_low:
                    # Broke below previous low but closed back inside
                    whale_signals += 1
                    whale_confidence_factors.append("MSB_DOWN_FAILED")
                    if trinity_decision == 'SELL':
                        is_trap_fallback = True
                        false_break_signals += 1
                
                # -------------------------------------------------------------
                # 7. PATTERN ANALYSIS (Chart Patterns in genome)
                # -------------------------------------------------------------
                if isinstance(patterns, list):
                    pattern_names = [p.get('name', p) if isinstance(p, dict) else str(p) for p in patterns]
                    pattern_str = ' '.join(pattern_names).upper()
                    
                    # Bullish patterns
                    if any(p in pattern_str for p in ['HAMMER', 'MORNING_STAR', 'BULLISH_ENGULF', 'DRAGONFLY', 'DOUBLE_BOTTOM']):
                        whale_signals += 1
                        if trinity_decision == 'SELL':
                            is_trap_fallback = True
                            whale_confidence_factors.append("BULLISH_PATTERN_CONFLICT")
                    
                    # Bearish patterns
                    if any(p in pattern_str for p in ['SHOOTING_STAR', 'EVENING_STAR', 'BEARISH_ENGULF', 'GRAVESTONE', 'DOUBLE_TOP']):
                        whale_signals += 1
                        if trinity_decision == 'BUY':
                            is_trap_fallback = True
                            whale_confidence_factors.append("BEARISH_PATTERN_CONFLICT")
                
                # -------------------------------------------------------------
                # 8. CALCULATE FINAL FALLBACK VALUES (Weighted Composite)
                # -------------------------------------------------------------
                
                # Whale confidence: sophisticated weighted calculation
                # Base from Trinity confidence (40%) + institutional signals (60%)
                base_whale = trinity_confidence * 0.4
                
                # Institutional time window bonus
                time_bonus = 15 if institutional_window else 0
                
                # Volume-based whale activity
                volume_whale_score = min(30, (volume_ratio - 1.0) * 20) if volume_ratio > 1.0 else 0
                
                # Sweep detection adds significant whale confidence
                sweep_whale_score = 25 if sweep_detected else 0
                
                # Pattern-based institutional footprint
                pattern_whale_score = whale_signals * 5
                
                # Calculate composite whale confidence
                whale_conf = min(95, max(20, base_whale + time_bonus + volume_whale_score + 
                                         sweep_whale_score + pattern_whale_score))
                
                # False break probability: weighted trap detection
                # Base from divergence signals
                base_false_break = false_break_signals * 10
                
                # Trap confirmation adds significantly
                trap_bonus = 25 if is_trap_fallback else 0
                
                # Extreme RSI adds false break probability
                rsi_extreme_penalty = 0
                if rsi > 80 or rsi < 20:
                    rsi_extreme_penalty = 15
                elif rsi > 70 or rsi < 30:
                    rsi_extreme_penalty = 8
                
                # Low liquidity time (Asian session) increases false break risk
                low_liq_penalty = 10 if institutional_window_name == "ASIAN_LOW_LIQ" else 0
                
                # Calculate composite false break probability
                false_break_prob = min(90, max(5, base_false_break + trap_bonus + 
                                               rsi_extreme_penalty + low_liq_penalty))
                
                # -------------------------------------------------------------
                # 8b. DETERMINE MARKET INTENTION (Hierarchical Logic)
                # -------------------------------------------------------------
                
                # Priority 1: Clear sweep detected
                if sweep_detected:
                    if sweep_direction_fallback == 'BUY':
                        market_intention = 'ACCUMULATION'
                    else:
                        market_intention = 'DISTRIBUTION'
                # Priority 2: High trap/false break signals
                elif false_break_prob >= 50:
                    market_intention = 'TRAP'
                # Priority 3: Strong whale signals aligned
                elif whale_signals >= 4:
                    market_intention = 'ACCUMULATION' if trinity_decision == 'BUY' else 'DISTRIBUTION'
                # Priority 4: Moderate whale signals
                elif whale_signals >= 2 and volume_ratio > 1.3:
                    market_intention = 'ACCUMULATION' if close > open_price else 'DISTRIBUTION'
                # Default: Equilibrium
                else:
                    market_intention = 'EQUILIBRIUM'
                
                # -------------------------------------------------------------
                # 8c. DETERMINE SWEEP TYPE AND DIRECTION
                # -------------------------------------------------------------
                
                # Sweep type based on hierarchical analysis
                if sweep_detected:
                    if sweep_direction_fallback == 'BUY':
                        sweep_type = 'DEMAND_SWEEP' if lower_wick_ratio > 0.5 else 'SUPPORT_SWEEP'
                    else:
                        sweep_type = 'SUPPLY_SWEEP' if upper_wick_ratio > 0.5 else 'RESISTANCE_SWEEP'
                elif market_intention == 'TRAP':
                    sweep_type = 'BULL_TRAP' if trinity_decision == 'BUY' else 'BEAR_TRAP'
                elif volume_ratio > 2.0:
                    sweep_type = 'INSTITUTIONAL_ACCUMULATION' if close > open_price else 'INSTITUTIONAL_DISTRIBUTION'
                elif volume_ratio > 1.5:
                    sweep_type = 'ACCUMULATION' if close > open_price else 'DISTRIBUTION'
                elif institutional_window and whale_signals >= 2:
                    sweep_type = 'SMART_MONEY_ENTRY'
                else:
                    sweep_type = 'NEUTRAL'
                
                # Final sweep direction with confidence
                if sweep_detected:
                    sweep_direction = sweep_direction_fallback
                    is_trap = is_trap_fallback
                elif market_intention == 'TRAP':
                    # Trap detected - recommend opposite of Trinity
                    sweep_direction = 'SELL' if trinity_decision == 'BUY' else 'BUY'
                    is_trap = True
                elif whale_signals >= 3:
                    # Strong whale signals - trust the analysis
                    sweep_direction = 'BUY' if close > open_price else 'SELL'
                    is_trap = is_trap_fallback
                elif whale_signals >= 2:
                    sweep_direction = trinity_decision
                    is_trap = is_trap_fallback
                else:
                    sweep_direction = None
                    is_trap = is_trap_fallback
                ...
                # Log detailed fallback analysis with emoji indicators
                trap_emoji = "??" if is_trap else "?"
                inst_emoji = "??" if institutional_window else "?"
                factors_str = ', '.join(whale_confidence_factors[:6]) if whale_confidence_factors else 'none'
                logger.info(f"[LLM6 FALLBACK] {trap_emoji} Whale:{whale_conf:.0f}% FalseBreak:{false_break_prob:.0f}% "
                           f"Sweep:{sweep_type} Intent:{market_intention} Dir:{sweep_direction} "
                           f"{inst_emoji} Vol:{volume_ratio:.1f}x [{factors_str}]")
            
            # ---------------------------------------------------------------
            # ?? INTELLIGENT VETO SYSTEM - Multi-Layer Protection
            # ---------------------------------------------------------------
            
            # HARD VETO 1: Extreme false break probability (>75%)
            if false_break_prob > 75:
                logger.warning(f"?? VETO LEVEL 1: Extreme false break {false_break_prob:.0f}%")
                return False, 'HOLD', 0, \
                       f"?? LLM6 VETO: False break {false_break_prob:.0f}% | Whale:{whale_conf:.0f}%"
            
            # HARD VETO 2: Trap detected + high false break (>50%)
            if is_trap and false_break_prob > 50:
                logger.warning(f"?? VETO LEVEL 2: Trap confirmed + FalseBreak {false_break_prob:.0f}%")
                return False, 'HOLD', 0, \
                       f"?? TRAP VETO: {sweep_type} detected | FalseBreak:{false_break_prob:.0f}%"
            
            # HARD VETO 3: Market intention = TRAP + Trinity goes into it
            if market_intention == 'TRAP':
                logger.warning(f"?? VETO LEVEL 3: Market intention is TRAP")
                return False, 'HOLD', 0, \
                       f"?? MARKET TRAP: Trinity={trinity_decision} into {sweep_type}"
            
            # SOFT VETO: Moderate false break (50-75%) - reduce confidence but allow
            if false_break_prob > 50:
                score *= 0.7
                llm6_confidence_mod *= 0.6
                llm6_reason = f"?? Moderate false break risk ({false_break_prob:.0f}%)"
                logger.warning(f"?? SOFT VETO: FalseBreak={false_break_prob:.0f}% - reducing confidence")
            ...
            if trinity_decision == 'SELL' and market_intention == 'ACCUMULATION':
                ...
                llm6_reason = "??  LLM6: Selling into accumulation (confidence reduced)"
            elif trinity_decision == 'BUY' and market_intention == 'DISTRIBUTION':
                score *= 0.6  # Reduce score significantly
                llm6_confidence_mod *= 0.5
                llm6_reason = "??  LLM6: Buying into distribution (confidence reduced)"
            
            # BOOST/PENALIZE based on sweep and TRAP detection
            if sweep_type != 'NONE' and sweep_type != 'NEUTRAL' and sweep_direction:
                if (trinity_decision == 'BUY' and sweep_direction == 'BUY') or \
                   (trinity_decision == 'SELL' and sweep_direction == 'SELL'):
                    ...
                    # Extra boost if in institutional window
                    if institutional_window:
                        base_boost += 0.1
                        
                    # Extra boost if high volume confirms
                    if volume_ratio > 1.8:
                        base_boost += 0.1
                    
                    score *= base_boost
                    llm6_confidence_mod *= base_boost
                    trap_note = " (TRAP inverted)" if is_trap else ""
                    inst_note = " [INST_WINDOW]" if institutional_window else ""
                    llm6_reason = f"? Sweep confirms {trinity_decision}{trap_note}{inst_note} (+{int((base_boost-1)*100)}%)"
                else:
                    ...
                    if is_trap:
                        ...
                        score *= 0.35  # Heavy penalty (was 0.4)
                        llm6_confidence_mod *= 0.25  # Even heavier (was 0.3)
                        llm6_reason = f"?? TRAP! Trinity {trinity_decision} vs Smart Money {sweep_direction}"
                        ...
                        score *= 0.65
                        llm6_confidence_mod *= 0.55
                        llm6_reason = f"?? Sweep {sweep_direction} conflicts with {trinity_decision}"
            
            # BOOST if whale confidence is very high and aligned
            if whale_conf > 85:
                # Only boost if not conflicting
                if not is_trap and (not sweep_direction or sweep_direction == trinity_decision):
                    boost = 1.15 + (whale_conf - 85) / 100  # Up to 1.25x for 95%
                    score *= boost
                    llm6_confidence_mod *= boost
                    llm6_reason = f"?? Whale confidence {whale_conf:.0f}% (+{int((boost-1)*100)}%)"
            elif whale_conf > 70 and not is_trap:
                # Moderate whale confidence
                score *= 1.08
                llm6_confidence_mod *= 1.08
                if not llm6_reason:
                    llm6_reason = f"?? Whale confidence {whale_conf:.0f}% (+8%)"
            ...
            # ---------------------------------------------------------------
            # ?? SUPER ENTRADA INTELIGENTE - Detectar se�ales MUY fuertes
            # Bypass varios filtros cuando Multi-TF tiene SUPER se�al
            # ---------------------------------------------------------------
            ...
            if is_super_entry and alignment_score >= 60:
                logger.info(f"?? SUPER ENTRADA DETECTADA: {consensus_reason}")
                # Super entrada con alta alineaci�n = entrada inmediata
                if trinity_confidence >= 55:
                    self._clear_pending(trinity_decision)
            ...
            # ---------------------------------------------------------------
            # ?? TIERS BASADOS EN CONCIENCIA - RELAXED FOR US30 SCALPING
            # Combina Score + Consciousness Probability + Calc Entry Score
            # ? RELAXED: Thresholds bajados para permitir m�s ataques en US30
            # ---------------------------------------------------------------
            
            # ?? TIER 0: CONSCIOUSNESS GOLD - Probabilidad >50% (was 62%)
            # Requiere: calc_entry_score >= 2.0 (was 3.0)
            if consciousness_prob >= 0.50 and momentum_ok and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # Requiere: calc_entry_score >= 2.0 (was 3.0)
            if alignment_score >= 60 and trinity_confidence >= 50 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # Requiere: calc_entry_score >= 2.5 (was 4.0)
            if score >= self.min_perfect_score and momentum_ok and consciousness_prob >= 0.38 and calc_entry_score >= 2.5:
                self._clear_pending(trinity_decision)
            ...
            # Requiere: calc_entry_score >= 2.0 (was 3.5)
            if score >= self.min_good_score and momentum_ok and consciousness_prob >= 0.35 and calc_entry_score >= 2.0:
                self._clear_pending(trinity_decision)
            ...
            # ?? TIER 3: QUALITY - Score ok + Prob m�nima
            # Requiere: calc_entry_score >= 1.5 (was 3.0)
            if score >= self.min_wait_score and momentum_ok and consciousness_prob >= 0.30 and calc_entry_score >= 1.5:
                self._clear_pending(trinity_decision)
            ...
            # Requiere: calc_entry_score >= 1.5 (was 2.5)
            if score >= self.min_sweep_score and momentum_ok and consciousness_prob >= 0.28 and calc_entry_score >= 1.5:
            ...
            # ?? Block si hay patr�n fuerte en contra
            ...
            # ?? TIER 5: Trinity Force - Relajado para US30
            # Requiere: calc_entry_score >= 1.0 (was 2.0)
            if trinity_confidence >= 50 and momentum_ok and score >= 1.5 and consciousness_prob >= 0.25 and calc_entry_score >= 1.0:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.85, \
                       f"? TRINITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # ?? TIER 6: High Confidence Override - �ltimo recurso con confianza OK
            # Requiere: calc_entry_score >= 0.5 (was 1.5)
            if trinity_confidence >= 55 and score >= 1.0 and consciousness_prob >= 0.22 and calc_entry_score >= 0.5:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.75, \
                       f"?? OVERRIDE (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # ---------------------------------------------------------------
            # ?? TIER 7: FALLBACK - Si Trinity quiere atacar, dejarlo pasar con checks m�nimos
            # Para US30 scalping, Trinity sabe lo que hace
            # ---------------------------------------------------------------
            if trinity_confidence >= 45 and trinity_decision in ['BUY', 'SELL'] and consciousness_prob >= 0.15:
                self._clear_pending(trinity_decision)
                return True, trinity_decision, trinity_confidence * 0.70, \
                       f"?? FALLBACK (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # ---------------------------------------------------------------
            # ?? NO ATACAR - La Conciencia dice que no hay edge claro
            # ? Solo bloqueamos si realmente es muy malo
            # ---------------------------------------------------------------
            self._clear_pending(trinity_decision)
            
            # Log detallado de por qu� no ataca
            block_reasons = []
            if consciousness_prob < 0.15:
                block_reasons.append(f"Prob MUY baja ({consciousness_prob:.0%})")
            if score < 0.5:
                block_reasons.append(f"Score MUY bajo ({score:.1f})")
            if trinity_confidence < 40:
                block_reasons.append(f"Trinity baja ({trinity_confidence:.0f}%)")
            if not momentum_ok:
                block_reasons.append("Momentum CONTRA")
            if warnings:
                block_reasons.append(f"Warnings: {warnings[0][:30]}")
            
            block_str = " | ".join(block_reasons[:3]) if block_reasons else "Sin confluencia clara"
            logger.info(f"?? CONSCIOUSNESS HOLD: {trinity_decision} bloqueado ? {block_str}")
            
            reason = f"?? NO EDGE: Prob={consciousness_prob:.0%}, Score={score:.1f}, Calc={calc_entry_score:.1f} | {block_str[:50]}"
            return False, 'HOLD', 0, reason
    
    # -----------------------------------------------------------------------
    # ?? ULTRA EXIGENT DIRECTION VALIDATION
    # Usa TODOS los �ngulos disponibles para validar direcci�n
    # -----------------------------------------------------------------------
    def _validate_direction_ultra_exigent(
        ...
        """
        ?? ULTRA EXIGENT: Valida que la DIRECCI�N sea correcta usando TODOS los datos.
        
        Analiza:
        - Sweeps de liquidez (LLM6)
        - Trampas de ballenas (is_trap, BULL_TRAP, BEAR_TRAP)
        - Patrones de reversi�n (hammers, engulfing, shooting star)
        ...
        if sweep_detected and sweep_type != 'NONE':
            ...
            if direction == 'BUY' and sweep_type in buy_sweeps:
                ...
                validations_failed += 2  # Sweep contra = doble penalizaci�n
        ...
        for pattern in patterns:
            ...
            if direction == 'BUY':
                if any(bp in p_name for bp in bullish_reversal):
                    ...
                    confirmations.append(f"Patr�n {p_name} confirma BUY")
                elif any(bp in p_name for bp in bearish_reversal):
                    validations_failed += 1
                    failures.append(f"Patr�n {p_name} CONTRA BUY")
            else:  # SELL
                if any(bp in p_name for bp in bearish_reversal):
                    validations_passed += 1
                    confirmations.append(f"Patr�n {p_name} confirma SELL")
                elif any(bp in p_name for bp in bullish_reversal):
                    validations_failed += 1
                    failures.append(f"Patr�n {p_name} CONTRA SELL")
        ...
        if llm6_sweep_type:
            if direction == 'BUY' and llm6_sweep_type in ['DISTRIBUTION']:
                ...
                validations_passed += 1
                confirmations.append("LLM6: Acumulaci�n confirma BUY")
            elif direction == 'SELL' and llm6_sweep_type in ['DISTRIBUTION', 'SUPPLY']:
                validations_passed += 1
                confirmations.append("LLM6: Distribuci�n confirma SELL")
        ...
        # ---------------------------------------------------------------
        # 9. HARMONIC PATTERN PRZ CHECK (si est� en breakdown)
        # ---------------------------------------------------------------
        ...
        if harmonic_score != 0:
            if (direction == 'BUY' and harmonic_score > 0) or \
               (direction == 'SELL' and harmonic_score < 0):
                validations_passed += 1
                confirmations.append(f"Harmonic pattern confirma direcci�n")
            elif (direction == 'BUY' and harmonic_score < 0) or \
                 (direction == 'SELL' and harmonic_score > 0):
                validations_failed += 1
                failures.append("Harmonic pattern CONTRA direcci�n")
        
        # ---------------------------------------------------------------
        # 10. MARKET STRUCTURE (Higher Highs / Lower Lows) - CR�TICO
        # Analiza las �ltimas 10 velas para detectar estructura de mercado
        # ---------------------------------------------------------------
        price_data = genome.get('price_data', {})
        ...
        # ---------------------------------------------------------------
        adx = indicators.get('adx', 25)
        if adx < 15:  # ?? Lowered from 20 ? scalping v�lido con ADX 15-20
            validations_failed += 1
        ...
        # ---------------------------------------------------------------
        # ?? DECISI�N FINAL: ULTRA EXIGENTE - "PENSAR M�S"
        # Requiere MAYOR�A CLARA de validaciones para atacar
        # ---------------------------------------------------------------
        ...
        # ----------------------------------------------------------------
        # ?? MARKET STRUCTURE BONUS: Si la estructura confirma, ser m�s permisivo
        # ----------------------------------------------------------------
        ...
        # ---------------------------------------------------------------
        # REGLAS ULTRA EXIGENTES (PENSAR M�S):
        # ---------------------------------------------------------------
        
        # 1. VETO ABSOLUTO: Si hay 5+ fallas = NO ATACAR (relajado de 4)
        # ? Si LLMs est�n de acuerdo, dar m�s margen
        ...
        # 4. RATIO M�NIMO: Necesitamos 55%+ de confirmaciones (era 60%)
        if total_checks >= 4 and success_ratio < 0.55:
            return False, f"Ratio insuficiente: {success_ratio:.0%} < 55%"
        
        # 5. M�NIMO ABSOLUTO: Con <4 checks, necesitamos m�s confirmaciones que fallas
        ...
        # 8. M�NIMO PERMISIVO: Si hay >=2 m�s confirmaciones que fallas = ATACAR
        ...
        # 10. LOW DATA: Si muy pocos checks (<3), permitir si hay alguna confirmaci�n
        if total_checks < 3 and validations_passed > 0 and validations_failed == 0:
            return True, f"?? Low data pero sin fallas ({validations_passed} confirms)"
...
# ==================== ADX SMOOTHER (CALIBRACI�N) ====================
class ADXSmoother:
    """Suaviza ADX con MA para eliminar ruido de mercados vol�tiles"""
    ...
    def add(self, adx_raw):
        """Agrega ADX raw y retorna versi�n suavizada"""
        self.adx_history.append(adx_raw)
        return self.get_smoothed()
    
    def get_smoothed(self):
        """Calcula MA de los �ltimos N ADX valores"""
...
class NOVAMarketStateDetector:
    ...
    - Macro correlations (DXY, US30, VIX trending affecting this pair)
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Spread too wide (illiquidity) - US30 spread in points (POINT=1.0)
        elif spread > 10.0:  # For US30, > 10 points is abnormally wide
            self.alerts.append(f"?? WIDE_SPREAD: {spread:.1f} points (liquidity crisis)")
            liquidity_score -= 25
        
        # Missing tick volume
        tick_volume = tick_data.get('volume', 0)
        if tick_volume == 0:
            self.alerts.append("?? NO_TICK_VOLUME: volume=0 (data quality issue)")
            liquidity_score -= 15
        ...
        if tf_buys > 0 and tf_sells > 0:
            self.alerts.append(f"?? TIMEFRAME_CONFLICT: {tf_buys} BUY vs {tf_sells} SELL across TFs")
            divergence_score -= 20
        ...
        if (rsi > 75 or rsi < 25) and adx < 20:
            self.alerts.append(f"?? PRICE_VOLUME_DIVERGENCE: RSI extreme ({rsi:.0f}) but ADX weak ({adx:.0f})")
            divergence_score -= 20
        ...
        # We cannot access DXY/US30/VIX from local data
        ...
        if indicator_agreement < 2:
            self.alerts.append(f"?? WEAK_MACRO_CONTEXT: Only {indicator_agreement}/3 indicators aligned")
            macro_score -= 20
        ...
        if bar_volumes and len(bar_volumes) >= 5:
            ...
            if recent_vol > avg_vol * 2:
                self.alerts.append(f"?? VOLUME_SPIKE: {recent_vol:.0f} vs avg {avg_vol:.0f} (2x)")
                volume_score -= 15
            elif recent_vol < avg_vol * 0.3:
                self.alerts.append(f"?? VOLUME_DRAIN: {recent_vol:.0f} vs avg {avg_vol:.0f} (<30%)")
                volume_score -= 10
        ...
        if abs(adx) > 25 and abs(adx_delta) < 0.5:
            self.alerts.append(f"?? MOMENTUM_FADE: ADX strong ({adx:.0f}) but delta weak ({adx_delta:+.2f})")
            momentum_score -= 15
        
        if abs(rsi_delta) > 3:  # RSI moving fast = extreme
            self.alerts.append(f"?? RSI_EXTREME_VELOCITY: delta={rsi_delta:+.1f} (exhaustion risk)")
            momentum_score -= 10
        ...
        if session == 'UNKNOWN' or session == '':
            self.alerts.append("?? SESSION_UNKNOWN: Cannot determine market session (US/EU/ASIA)")
            session_score -= 10
        
        # SL history length = market experience
        if sl_history is None or len(sl_history) < 3:
            self.alerts.append(f"?? LIMITED_SL_HISTORY: Only {len(sl_history) if sl_history else 0} events")
            session_score -= 5
        ...
        if rsi > 85:
            self.alerts.append(f"?? RSI_OVERBOUGHT: {rsi:.0f} (strong but reversal risk)")
            saturation_score -= 10
        elif rsi < 15:
            self.alerts.append(f"?? RSI_OVERSOLD: {rsi:.0f} (strong but reversal risk)")
            saturation_score -= 10
        ...
        if abs(macd_hist) > 0.5:  # Very large histogram
            self.alerts.append(f"?? MACD_SATURATION: histogram={macd_hist:+.3f} (exhaustion)")
            saturation_score -= 10
...
class Config:
    """
    Configuration class supporting multiple trading strategies.
    
    Strategies:
    - SCALPER: Default M5 scalping (8-20 pip targets)
    - EXCALIBUR_KNIGHT: M1/M5 Elite Momentum Breakout (15-50 pip targets)
    """
    
    # -----------------------------------------------------------------------
    # ?? STRATEGY SELECTOR (loaded from us30config.yaml)
    # -----------------------------------------------------------------------
    ACTIVE_STRATEGY = "SCALPER"  # Default, will be overridden by yaml
    EXCALIBUR_CONFIG = None      # ExcaliburKnightConfig instance if active
    
    # -----------------------------------------------------------------------
    # NETWORK PORTS
    # -----------------------------------------------------------------------
    HOST = '127.0.0.1'
    QUANTUM_PORT = 8954      # Injector - receive genomes from Quimera (US30)
    EXECUTOR_PORT = 8956     # Executor - send commands to Quimera (US30)
    TRINITY_PORT = 8966      # Trinity server port (US30 89xx range)
    KRAKEN_PORT = 8952       # Kraken executor (US30)
    LLM1_PORT = 8955         # BAYESIAN
    LLM2_PORT = 8957         # TECHNICAL
    LLM3_PORT = 8958         # CHART
    LLM4_PORT = 8959         # RISK
    LLM5_PORT = 8961         # LLM5 SUPREME chart analyzer
    LLM6_PORT = 8962         # ? LLM6 NOVA Smart Money Oracle (US30)
    LLM7_PORT = 8963         # LLM7 OCULUS Data Quality
    LLM8_PORT = 8964         # LLM8 CHRONOS Timing
    LLM9_PORT = 8965         # LLM9 PREDATOR Execution
    LLM10_PORT = 8960        # LLM10 SENTINEL Guardian
    LLM11_PORT = 8967        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 8968        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 8970  # ?? Sound.py TTS alert engine (TP/SL audio alerts)
    
    # -----------------------------------------------------------------------
    # ?? SCALPER STRATEGY DEFAULTS (M1/M5 Index CFD Scalping)
    # -----------------------------------------------------------------------
    # Trading - SNIPER SCALPING M1 (Index CFD Scalper Style)
    # Strategy: Wait 120s between trades, HIGH quality setups only, quick profits
    # ?? US30: 1 point = 1.0 (not index pips). US30 M1 moves ~5-50 points per candle
    # Realistic M1 scalping: TP 10-30 points, SL 8-20 points
    TP_MIN_PIPS = 10      # 10 points MINIMUM - realistic M1 US30 scalping
    TP_MAX_PIPS = 50      # 50 points MAX - don't chase unrealistic targets
    SL_MIN_PIPS = 8       # 8 point stops - tight risk control for index
    SL_MAX_PIPS = 25      # Max 25 point stop - protect capital
    ATR_FLOOR = 5.0       # 5 point min ATR - lower floor for M1 US30
    ...
    MAX_CONCURRENT_TRADES = 3        # ?? REDUCED: 3 concurrent trades in SAME direction (was 4 - too many)
    MAX_DAILY_LOSS_PERCENT = 2.0     # Strict loss control - protect capital
    MAX_CONSECUTIVE_LOSSES = 3       # FIX A-US30-02: halt after 3 consecutive losses
    LOSS_HALT_DURATION_SECONDS = 300 # FIX A-US30-02: 5 min cool-down after loss streak
    MIN_CONFIDENCE = 57              # ?? +2%: 57%+ confidence for US30 scalping (was 55 - caused bidirectional attacks)
    MIN_TRINITY_CONFIDENCE = 12      # +2%: 12%+ required - Trinity/Kraken decide, Quimera executes (was 10)
    MIN_ADX = 22                     # +2%: Higher ADX filter to avoid choppy markets (was 20 - too permissive)
    ...
    MIN_RSI_FOR_BUY = 38             # BALANCED: (original 45, relajado 35 ? 38)
    MAX_RSI_FOR_BUY = 68             # BALANCED: (original 65, relajado 70 ? 68)
    MIN_RSI_FOR_SELL = 33            # BALANCED: (original 35, relajado 30 ? 33)
    MAX_RSI_FOR_SELL = 60            # BALANCED: (original 55, relajado 65 ? 60)
    ...
    SNIPER_COOLDOWN_SECONDS = 120   # Seconds between sniper attacks (PRODUCTION)
    ...
    # -----------------------------------------------------------------------
    # ?? EXCALIBUR KNIGHT STRATEGY OVERRIDES (M1 Entry ? M5/M15 Scalping)
    # -----------------------------------------------------------------------
    # These values are used when ACTIVE_STRATEGY = "EXCALIBUR_KNIGHT"
    EXCALIBUR_TP_MIN_PIPS = 15       # TP1 minimum (M5 target) - 15 points
    EXCALIBUR_TP_MAX_PIPS = 80       # TP2 maximum (M15 swing) - 80 points
    EXCALIBUR_TP_TYPICAL_PIPS = 35   # Typical post-breakout - 35 points
    EXCALIBUR_SL_PIPS = 15           # Tight SL for M1 entry - 15 points
    EXCALIBUR_MAX_POSITIONS = 3      # Selective but active
    EXCALIBUR_MIN_CONFIDENCE = 65    # Slightly lower for faster trades
    EXCALIBUR_MIN_TRINITY_CONFIDENCE = 60
    EXCALIBUR_MIN_ADX = 20
    EXCALIBUR_ATR_FLOOR = 5.0        # 5 points min ATR for M1 US30
    EXCALIBUR_ATR_MAX = 80.0         # 80 points max ATR (high volatility index)
    EXCALIBUR_COOLDOWN_SECONDS = 180  # 3 min cooldown (M1 scalping)
    EXCALIBUR_MIN_RSI_FOR_BUY = 60
    EXCALIBUR_MAX_RSI_FOR_BUY = 80
    EXCALIBUR_MIN_RSI_FOR_SELL = 20
    EXCALIBUR_MAX_RSI_FOR_SELL = 40
    EXCALIBUR_BREAKOUT_PIPS = 15     # Min breakout distance - 15 points for US30
    EXCALIBUR_VOLUME_MULTIPLIER = 1.3  # Volume > 1.3x required
    ...
    HEARTBEAT_TIMEOUT = 15.0  # Increased from 8s to 15s (more tolerant to network jitter)
    
    # LLM Timeouts
    OLLAMA_TIMEOUT = 90.0  # Increased from 45s to 90s (Ollama is slow)
    LLM_QUERY_TIMEOUT = 2.5  # Increased from 1.5s to 2.5s
    
    # Resilience
    MAX_MESSAGE_SIZE = 1_000_000
    CIRCUIT_BREAKER_THRESHOLD = 3  # Reduced from 5 to 3 (open faster, recover faster)
    CIRCUIT_BREAKER_TIMEOUT = 30  # Reduced from 60s to 30s (quicker recovery)
    
    @classmethod
    def load_strategy_from_yaml(cls):
        """Load active strategy from us30config.yaml"""
        try:
            import yaml
            ...
            if os.path.exists(config_path):
                ...
                strategy = config.get('active_strategy', 'SCALPER').upper()
                cls.ACTIVE_STRATEGY = strategy
                
                if strategy == 'EXCALIBUR_KNIGHT':
                    cls._apply_excalibur_config(config.get('excalibur_knight', {}))
                    print(f"[CONFIG] ?? EXCALIBUR KNIGHT strategy loaded - M1/M5 Elite Momentum Breakout")
                else:
                    print(f"[CONFIG] ?? SCALPER strategy active - M5 Fast Scalping")
                    
                return strategy
        except ImportError:
            print("[CONFIG] PyYAML not installed - using default SCALPER strategy")
        except Exception as e:
            print(f"[CONFIG] Error loading strategy: {e} - using default SCALPER")
        
        return 'SCALPER'
    
    @classmethod
    def _apply_excalibur_config(cls, excalibur_yaml: dict):
        """Apply Excalibur Knight configuration from yaml"""
        if not excalibur_yaml:
            return
            
        # Override main trading parameters
        cls.TP_MIN_PIPS = excalibur_yaml.get('tp_min_pips', cls.EXCALIBUR_TP_MIN_PIPS)
        cls.TP_MAX_PIPS = excalibur_yaml.get('tp_max_pips', cls.EXCALIBUR_TP_MAX_PIPS)
        cls.SL_MIN_PIPS = excalibur_yaml.get('sl_pips', cls.EXCALIBUR_SL_PIPS)
        cls.SL_MAX_PIPS = excalibur_yaml.get('sl_pips', cls.EXCALIBUR_SL_PIPS)
        cls.MAX_CONCURRENT_TRADES = excalibur_yaml.get('max_positions', cls.EXCALIBUR_MAX_POSITIONS)
        cls.MIN_CONFIDENCE = excalibur_yaml.get('min_confidence', cls.EXCALIBUR_MIN_CONFIDENCE)
        cls.MIN_TRINITY_CONFIDENCE = excalibur_yaml.get('min_llm_agreement', cls.EXCALIBUR_MIN_TRINITY_CONFIDENCE)
        cls.MIN_ADX = cls.EXCALIBUR_MIN_ADX
        cls.ATR_FLOOR = cls.EXCALIBUR_ATR_FLOOR
        cls.SNIPER_COOLDOWN_SECONDS = cls.EXCALIBUR_COOLDOWN_SECONDS
        
        # RSI bounds for Excalibur
        cls.MIN_RSI_FOR_BUY = cls.EXCALIBUR_MIN_RSI_FOR_BUY
        cls.MAX_RSI_FOR_BUY = cls.EXCALIBUR_MAX_RSI_FOR_BUY
        cls.MIN_RSI_FOR_SELL = cls.EXCALIBUR_MIN_RSI_FOR_SELL
        cls.MAX_RSI_FOR_SELL = cls.EXCALIBUR_MAX_RSI_FOR_SELL
        
        # Create Excalibur config object if module available
        if EXCALIBUR_AVAILABLE:
            cls.EXCALIBUR_CONFIG = ExcaliburKnightConfig()
            # Apply yaml overrides to config object
            for key, value in excalibur_yaml.items():
                if hasattr(cls.EXCALIBUR_CONFIG, key):
                    setattr(cls.EXCALIBUR_CONFIG, key, value)
    
    @classmethod
    def is_excalibur_active(cls) -> bool:
        """Check if Excalibur Knight strategy is active"""
        return cls.ACTIVE_STRATEGY == 'EXCALIBUR_KNIGHT'
    
    @classmethod
    def get_strategy_info(cls) -> dict:
        """Get current strategy information"""
        if cls.is_excalibur_active():
            return {
                'name': 'EXCALIBUR_KNIGHT',
                'description': 'Elite Momentum Sniper - M1/M5 Breakout',
                'timeframe': 'M1 ? M5/M15',
                'tp_range': f'{cls.EXCALIBUR_TP_MIN_PIPS}-{cls.EXCALIBUR_TP_MAX_PIPS} pips',
                'sl': f'{cls.EXCALIBUR_SL_PIPS} pips',
                'risk_reward': '1:2',
                'trades_per_week': '5-15',
                'style': 'Knight Precision'
            }
        else:
            return {
                'name': 'SCALPER',
                'description': 'Fast M5 Index Scalping',
                'timeframe': 'M5',
                'tp_range': f'{cls.TP_MIN_PIPS}-{cls.TP_MAX_PIPS} pips',
                'sl': f'{cls.SL_MIN_PIPS}-{cls.SL_MAX_PIPS} pips',
                'risk_reward': '1:1.5',
                'trades_per_week': 'Multiple daily',
                'style': 'Fast Sniper'
            }

# Load strategy on module import
Config.load_strategy_from_yaml()

# ==================== US30 GILGAMESH DASHBOARD ====================
class NOVADashboard:
    """
    US30 GILGAMESH - Wall Street Elite Momentum Dashboard
    by Polarice Labs 2026 | Index CFD Scalper Edition
    ...
    def __init__(self):
        ...
        self.data = {
            ...
            # ??? ECOSYSTEM VALIDATION (pre-flight checks before trading)
    ...
    def _format_price(self, price, decimals=2):
        """Format price with proper decimals - US30 index uses 2 decimal places (e.g. 42350.50)"""
        if price == 0:
            return "-----.--"
    ...
    def _format_change(self, change, pct=False):
        ...
        if pct:
            ...
            return f"{color}{sign}{change:.2f}{self.RESET}"  # 2 decimals for US30 index point moves
    
    def render(self):
        """
        ╔════════════════════════════════════════════════════════════════════════════╗
        ║  GILGAMESH - US30 WALL STREET SNIPER v18.00                                ║
        ║  by Polarice Labs © 2026 | Index CFD Scalper Precision                         ║
        ╚════════════════════════════════════════════════════════════════════════════╝
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'US30'))
        ...
        lines.append("")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}███╗   ██╗{self.RESET}{SOFT_WHITE}{self.BOLD} ██████╗ ██╗   ██╗ █████╗ {self.RESET}    {self.CYAN}{self.BOLD}⚔️ GILGAMESH - US30 Wall Street{self.RESET}  •  {self.GREEN}●{self.RESET} {now.strftime('%H:%M:%S')}  •  Up {self.CYAN}{uptime_str}{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}US30 Gilgamesh Index CFD by{self.RESET} {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        lines.append(f"  RSI  {rsi_color}{rsi:5.1f}{self.RESET} {rsi_bar} {rsi_color}{rsi_zone:>10}{self.RESET}    MACD {macd_color}{macd:>+9.2f}{self.RESET}  Sig {macd_s:>+9.2f}  Hist {macd_color}{macd_h:>+9.2f}{self.RESET}    Stoch %K {stoch_color}{stoch_k:5.1f}{self.RESET} {stoch_bar}  %D {stoch_d:5.1f}")
        lines.append(f"  ADX  {adx_color}{adx:5.1f}{self.RESET} {adx_bar} {adx_color}{adx_str:>10}{self.RESET}    ATR  {self.YELLOW}{atr:>9.2f}{self.RESET}  {trend_arrow} {self.YELLOW}{d['trend']:<8}{self.RESET}    {'🔥 SQUEEZE' if bb_squeeze else '          '} BB Width {bb_width:4.1f}%")
        lines.append("")
        
        lines.append(f"  {self.MAGENTA}{self.BOLD}BOLLINGER{self.RESET}  {self.RED}Upper {bb_u:>9.2f}{self.RESET}    {self.CYAN}Mid {bb_m:>9.2f}{self.RESET}    {self.GREEN}Lower {bb_l:>9.2f}{self.RESET}        {self.YELLOW}{self.BOLD}MAs{self.RESET}  Fast(5) {self.YELLOW}{ma_f:>9.2f}{self.RESET}  Slow(20) {self.CYAN}{ma_s:>9.2f}{self.RESET}  {ma_cross_color}{ma_cross}{self.RESET}")
        lines.append(f"  {self.CYAN}{self.BOLD}FIBONACCI{self.RESET}  23.6% {fib_236:>9.2f}    38.2% {fib_382:>9.2f}    50% {fib_500:>9.2f}    61.8% {fib_618:>9.2f}    {'⭐ Near ' + fib_near if fib_near != 'NONE' else '          '}")
        lines.append("")
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}⚔️ GILGAMESH{self.RESET} v18.00 US30 Wall Street {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
    ...
    def _send_llm_decision_to_dashboard(self, llm_name, decision, confidence, reason=""):
        """Send LLM decision message to trinity_dashboard_v3 TCP listener on port 8967 (US30)"""
        try:
            import socket
            ...
            # Serialize and send to localhost:8967 (US30 dashboard port)
...
class LLMParallelConsensus:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-100
        self.logger = logger
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-95
...
class AdaptiveTPSLEngine:
    """Dynamically adjust TP/SL based on volatility, ATR, and win rate"""
    ...
    def _calculate_m5_projection(self, closes, direction, atr):
        """
        ?? M5 PROJECTION: Calcula hacia d�nde apunta el precio en los pr�ximos 5 minutos.
        ...
        logger.debug(f"[M5 PROJECTION] slope={slope:.6f}, projected={projected_price:.2f}, "
                    f"distance={projection_distance:.1f}pts, alignment={alignment:.2f}")
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
                                     support_resistance=None, spread=1.0, closes=None):
        """
        Calculate INTELLIGENT TP/SL for M1 US30 using ALL available data:
        ...
        """
        # Extract indicators
        atr = float(indicators.get('atr', 10.0))  # Default 10 points for US30 index
        rsi = float(indicators.get('rsi', 50))
        adx = float(indicators.get('adx', 25))
        bb_upper = float(indicators.get('bollinger_upper', current_price + 20.0))
        bb_lower = float(indicators.get('bollinger_lower', current_price - 20.0))
        ma_fast = float(indicators.get('ma_fast', current_price))
        ma_slow = float(indicators.get('ma_slow', current_price))
        
        # Ensure minimum ATR for US30 M1
        # ?? FIX: M1 ATR typically 5-25 points for US30, clamp to realistic range
        atr = max(5.0, min(30.0, atr))  # Clamp ATR 5-30 points for M1 safety
        ...
        # ---------------------------------------------------------------
        # M1 US30 typical ATR: 3-15 points
        # TP should be 1.0-1.5x ATR, SL should be 0.6-0.8x ATR
        # ?? FIX: Reduced multipliers for M1 scalping
        base_tp_multiplier = 1.2  # Was 2.0 - too aggressive
        base_sl_multiplier = 0.7  # Was 1.0 - too wide
        ...
        # ---------------------------------------------------------------
        # STEP 7: M1 US30 SCALPING TP/SL - ATR BASED
        # ---------------------------------------------------------------
        # M1 ATR t�pico US30: 5-25 points
        # Multiplicadores: TP = ATR � 1.1, SL = ATR � 0.8
        # Esto da R:R de ~1.375:1 m�nimo
        # ---------------------------------------------------------------
        
        # ATR viene en puntos para US30 index (ej: 10.0 = 10 points)
        # Clamp ATR a rangos razonables para M1
        atr_clamped = max(5.0, min(40.0, atr))  # Min 5 points, Max 40 points
        
        # ? MULTIPLICADORES PARA M1 SCALPING - AHORA USA Steps 1-6 din�micos
        # base_tp_multiplier viene de Steps 1-6 (volatility, trend, BB, confidence, S/R)
        # Clamp a rangos seguros: TP entre 0.8 y 2.0, SL entre 0.5 y 1.5
        TP_MULTIPLIER = max(0.8, min(2.0, base_tp_multiplier))  # TP = ATR � dynamic multiplier
        SL_MULTIPLIER = max(0.5, min(1.5, base_sl_multiplier))  # SL = ATR � dynamic multiplier
        ...
        if m5_projected is not None and m5_alignment > 0.6:
            ...
            m5_tp_extension = abs(m5_distance) * m5_alignment * 0.5  # Use 50% of projected move
            m5_tp_extension = min(m5_tp_extension, 30.0)  # Cap at 30 points extra
            
            if m5_alignment > 0.8:
                # Very high alignment - use more of the projection
                logger.info(f"[M5 PROJECTION] ? STRONG alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension:.1f}pts")
                tp_distance_price += m5_tp_extension
            elif m5_alignment > 0.6:
                # Moderate alignment - use half
                logger.info(f"[M5 PROJECTION] ? Moderate alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension*0.5:.1f}pts")
            ...
            logger.warning(f"[M5 PROJECTION] ?? LOW alignment ({m5_alignment:.2f}) - Reducing TP by 15%")
            tp_distance_price *= 0.85
        
        # ---------------------------------------------------------------------------
        # MT5 STOPS_LEVEL ENFORCEMENT: Minimum distances to avoid "Invalid stops"
        # ?? FIX v5: US30 M1 SCALPING - reduced distances for tight entries
        # ---------------------------------------------------------------------------
        MIN_TP = 10.0     # 10 points minimum TP (M1 scalping)
        MIN_SL = 8.0      # 8 points minimum SL (M1 scalping)
        SAFETY = 3.0      # 3 points safety buffer for ASK/BID slippage
        
        tp_distance_price = max(MIN_TP, tp_distance_price)
        sl_distance_price = max(MIN_SL, sl_distance_price)
        
        # ?? HARD CAP para scalping M1 - US30 point distances
        tp_distance_price = min(40.0, tp_distance_price)  # M�ximo 40 points TP
        sl_distance_price = min(25.0, sl_distance_price)   # M�ximo 25 points SL
        
        # Asegurar ratio R:R m�nimo de 1.3:1
        min_rr = 1.3
        if tp_distance_price / max(sl_distance_price, 0.01) < min_rr:
            tp_distance_price = sl_distance_price * min_rr
        
        # Log para debugging
        logger.debug(f"[TP/SL M1] ATR={atr:.3f} ? TP=${tp_distance_price:.2f}, SL=${sl_distance_price:.2f}")
        ...
        if tp_dist_final < MIN_TP:
            logger.error(f"[TP/SL] ? TP distance too small: {tp_dist_final:.2f} < {MIN_TP:.2f}")
            tp_price = round(current_price + (MIN_TP + SAFETY if direction == 'BUY' else -(MIN_TP + SAFETY)), 2)
        
        if sl_dist_final < MIN_SL:
            logger.error(f"[TP/SL] ? SL distance too small: {sl_dist_final:.2f} < {MIN_SL:.2f}")
            sl_price = round(current_price - (MIN_SL + SAFETY if direction == 'BUY' else -(MIN_SL + SAFETY)), 2)
        
        # Convert to points for return (1 point = 1.0 for US30 index)
        tp_pips = int(tp_distance_price)
        sl_pips = int(sl_distance_price)
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        ...
        min_rr_ratio = 1.3
        if tp_distance / max(sl_distance, 0.01) < min_rr_ratio:
            tp_distance = sl_distance * min_rr_ratio
        
        # FIX v5: M�nimos REDUCIDOS para M1 US30 scalping
        # M1 requiere stops ajustados, 8-25 pips t�pico
        MIN_TP_PIPS = 10  # M�nimo 10 pips TP (M1 scalping)
        MIN_SL_PIPS = 8   # M�nimo 8 pips SL (M1 scalping)
        SAFETY_PIPS = 3   # 3 pips de seguridad extra
        
        tp_distance = max(MIN_TP_PIPS, min(40.0, tp_distance))   # 10-40 PIPS TP
        sl_distance = max(MIN_SL_PIPS, min(25.0, sl_distance))   # 8-25 PIPS SL
        
        # Point value conversion: US30 = 1.0 per point (index CFD)
        point_value = 1.0  # US30: 1 point = 1.0 (e.g., 35 points = 35.0)
        safety_margin = SAFETY_PIPS * point_value  # 3.0 extra
        
        if direction == 'BUY':
            tp = round(current_price + (tp_distance * point_value) + safety_margin, 2)
            sl = round(current_price - (sl_distance * point_value) - safety_margin, 2)
        else:
            tp = round(current_price - (tp_distance * point_value) - safety_margin, 2)
            sl = round(current_price + (sl_distance * point_value) + safety_margin, 2)
        ...
        if direction == 'BUY':
            if tp <= current_price or sl >= current_price or tp <= sl:
                ...
                tp = round(current_price + (MIN_TP_PIPS * point_value) + safety_margin, 2)
                sl = round(current_price - (MIN_SL_PIPS * point_value) - safety_margin, 2)
        else:  # SELL
            if tp >= current_price or sl <= current_price or tp >= sl:
                # Force valid values with safety
                tp = round(current_price - (MIN_TP_PIPS * point_value) - safety_margin, 2)
    ...
    def calculate_smart_plan(self, current_price, direction, genome, indicators, sweep_info=None):
        ...
        try:
            ...
            if direction == 'BUY':
                # TP1: Pr�ximo nivel de resistencia o Fib
                potential_tp1s = [
                    resistance,
                    fib_618 if current_price < fib_618 else fib_618 + atr,
                    current_price + atr * 2 * atr_mult
                ]
                tp1_raw = min([t for t in potential_tp1s if t > current_price + 10.0], default=current_price + 15.0)
                
                # TP2: Target agresivo (2x TP1 distance)
                tp1_distance = tp1_raw - current_price
                tp2_raw = current_price + tp1_distance * 1.8
                
                # SL: Debajo del soporte o �ltimo swing low
                potential_sls = [
                    support - atr * 0.3,
                    min(lows[-5:]) - atr * 0.2 if len(lows) >= 5 else support - 20.0,
                    current_price - atr * 1.5 * atr_mult
                ]
                sl_raw = max([s for s in potential_sls if s < current_price - 10.0], default=current_price - 10.0)
                ...
                # TP1: Pr�ximo nivel de soporte o Fib
                potential_tp1s = [
                    support,
                    fib_382 if current_price > fib_382 else fib_382 - atr,
                    current_price - atr * 2 * atr_mult
                ]
                tp1_raw = max([t for t in potential_tp1s if t < current_price - 10.0], default=current_price - 15.0)
                
                # TP2: Target agresivo
                tp1_distance = current_price - tp1_raw
                tp2_raw = current_price - tp1_distance * 1.8
                
                # SL: Encima de la resistencia o �ltimo swing high
                potential_sls = [
                    resistance + atr * 0.3,
                    max(highs[-5:]) + atr * 0.2 if len(highs) >= 5 else resistance + 20.0,
                    current_price + atr * 1.5 * atr_mult
                ]
                sl_raw = min([s for s in potential_sls if s > current_price + 10.0], default=current_price + 10.0)
            ...
            # --- 5. VALIDAR M�NIMOS (BROKER LIMITS) ---
            MIN_TP_PIPS = 10  # M1 scalping - 10 points min
            MIN_SL_PIPS = 8   # M1 scalping - 8 points min
            POINT = 1.0  # US30 index: 1 point = 1.0
            ...
            # Redondear (2 decimales para US30 index)
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 5.0) if atr > 0 else 0  # [NOVA AUDIT F-11] US30 ATR M1 ~5-50 pts → bins 1-10
...
class MarketMicrostructureAnalyzer:
    ...
    def analyze_tick(self, bid, ask, volume, direction):
        ...
        spread = ask - bid
        spread_pips = spread  # US30: spread already in points (ask - bid)
...
class DeepNeuralNetwork:
    ...
    def extract_advanced_features(self, prices, volumes=None):
        ...
        # Pattern features (6)
        if len(highs) >= 5:
            features['higher_highs'] = len([1 for i in range(1, 2) if highs[-i] > highs[-i-1]])
            features['lower_lows'] = len([1 for i in range(1, 2) if lows[-i] < lows[-i-1]])
        else:
    ...
    def analyze(self, closes, highs, lows, opens, volumes=None, 
        ...
        if recent_high > prev_high * 1.001:  # Broke above by >0.1%
            if recent_close < prev_high:  # But closed back below
                ...
                result['warning'] = f"???? BULL TRAP: Broke {prev_high:.2f} but closed at {recent_close:.2f}"
                return result
        ...
        if recent_low < prev_low * 0.999:  # Broke below by >0.1%
            if recent_close > prev_low:  # But closed back above
                ...
                if body_size > 0:
                    ...
                    result['trap_probability'] = 0.6
                result['warning'] = f"???? BEAR TRAP: Broke {prev_low:.2f} but closed at {recent_close:.2f}"
                return result
            ...
            if price_3_ago > price_2_ago and price_2_ago > price_1_ago:  # Was falling
                if price_now > price_1_ago * 1.002:  # Now recovering strongly
                    ...
                    if move_up > move_down * 0.5:  # Recovered more than 50%
                        ...
                        result['warning'] = "???? LIQUIDITY GRAB: Stopped out longs, now reversing UP"
                        return result
            ...
            if price_3_ago < price_2_ago and price_2_ago < price_1_ago:  # Was rising
                if price_now < price_1_ago * 0.998:  # Now declining strongly
                    ...
                    if move_down > move_up * 0.5:  # Declined more than 50%
                        ...
                        result['warning'] = "???? LIQUIDITY GRAB: Stopped out shorts, now reversing DOWN"
                        return result
        ...
        if len(closes) >= 6:
            ...
            if lookback_high > range_before * 1.002:  # There was a breakout
                if current < range_before:  # Now back inside range
                    ...
                    result['warning'] = "???? FAKEOUT: Breakout failed, back inside range"
                    return result
            ...
            if lookback_low < range_low * 0.998:  # There was a breakdown
                if current > range_low:  # Now back inside range
                    ...
                    result['trap_direction'] = 'BUY'
                    result['trap_probability'] = 0.65
                    result['warning'] = "???? FAKEOUT: Breakdown failed, back inside range"
                    return result
...
class AdvancedTrendAnalyzer:
    ...
    def identify_trend(self, prices):
        ...
        if price_range < 1.0:  # < 1 point = flat market for US30
            return 'RANGING'
...
class MarketCycleAnalyzer:
    ...
    def identify_cycle_phase(self, prices):
        ...
        if trend_strength < 1.0 and volume_trend > 1.2:
            return 'ACCUMULATION'  # Trending down or flat with high volume
        elif trend_strength > 5.0 and volume_trend > 1.1:
            return 'MARKUP'  # Trending up with high volume
        elif trend_strength > 3.0 and volume_trend < 0.9:
    ...
    def detect_sweep(self, prices, support_level, resistance_level, tolerance_pips=0.5, volumes=None):
        """
        SWEEP DETECTION PRO: Detecta si el precio toc� soporte/resistencia en �ltimas 10 VELAS
        
        Sweep = precio toc� S/R y REBOT� en direcci�n opuesta
        Esto valida que el nivel es REAL y hay REVERSI�N
        
        UPGRADED: Analiza 10 velas para contexto completo + m�ltiples validaciones
        - SUPPORT SWEEP: Precio baja, toca soporte, luego sube con confirmaci�n
        - RESISTANCE SWEEP: Precio sube, toca resistencia, luego baja con confirmaci�n
        ...
        # EXPANDED: Analyze �ltimas 10 velas para contexto completo
        if len(prices) < 10:
            return {'sweep_detected': False, 'type': 'NONE', 'entry_quality': 0}
        ...
        # FIX #4: Dynamic tolerance based on volatility (not fixed 0.5 points)
        # For M1 US30 index scalping: typical sweep tolerance is 5-15 points
        base_tolerance = float(tolerance_pips) if tolerance_pips else 5.0  # 5 points default
        atr_value = getattr(self, 'last_atr', 10.0)  # Get last ATR (10 points default)
        
        # Scale tolerance dynamically: higher ATR = higher tolerance needed
        dynamic_tolerance = max(5.0, min(20.0, atr_value * 1.5))  # 5-20 points typical
        ...
        for i in range(len(prices_clean) - 1):
            ...
            # ?? COHERENCIA TEMPORAL: Solo sweeps de �ltimas 5 velas son relevantes
            if candles_ago > 5:
        ...
        # No hay sweep en las �ltimas 10 velas
        return {'sweep_detected': False, 'type': 'NONE', 'entry_quality': 0}
    ...
    def detect_whale_trap(self, prices, volumes, lookback=5):
        ...
        3. Luego REVERSA r�pidamente (next candle reverses direction)
        
        = Esto indica que fueron WHALES generando movimiento falso
        
        IMPORTANTE: Un sweep es LEG�TIMO (rebota EN el nivel)
        Un whale trap es FALSO (breakout FUERA del nivel + reversi�n)
        """
        ...
        try:
            ...
            if vol_spike and breakout and reversal:
                ...
                return {
                    ...
                    'description': f'?? TRAMPA DE WHALES: Breakout falso con volumen {curr_volume:.0f} ({curr_volume/avg_volume:.1f}x promedio) + reversi�n r�pida',
                    'action': 'AVOID - No operar en esta direcci�n'
                }
            ...
            if conditions_met >= 2:
                ...
                if reversal: conditions_str.append('Reversi�n r�pida')
                return {
                    'is_trap': True, 
                    'confidence': confidence, 
                    'reason': f'{conditions_met}_of_3_conditions',
                    'description': f'?? POSIBLE TRAMPA: {" + ".join(conditions_str)}',
                    'action': 'CAUTION - Reducir tama�o de posici�n'
                }
...
class QuantumConsciousnessEngine:
    ...
    def __init__(self):
        ...
        self.attack_prep = {
            ...
            'whale_signals': deque(maxlen=20),  # Se�ales de whales detectadas
            ...
            'optimal_direction': None,  # Direcci�n �ptima basada en data
            'preparation_score': 0.0,  # Score de preparaci�n 0-100
        ...
        # ??? SUPREME CONSCIOUSNESS ENGINE - Meta-Cognitive Trading Intelligence
        if SUPREME_CONSCIOUSNESS_AVAILABLE:
            self.supreme_consciousness = get_supreme_consciousness()
            self.logger.info("[? SUPREME] Supreme Consciousness Engine ONLINE - Meta-cognitive intelligence active")
            self.logger.info("   ?? Meta-LLM Cognition: LLMs evaluate each other")
            self.logger.info("   ?? Quantum State Machine: Market states beyond bull/bear")
            self.logger.info("   ?? Divergence Oracle: Inter-LLM divergence as signal")
            self.logger.info("   ?? Wisdom Accumulator: Learning from trade outcomes")
            self.logger.info("   ? Institutional Time: Kill zones awareness")
            self.logger.info("   ??? Anti-Noise Filter: Signal purity detection")
        else:
            self.supreme_consciousness = None
            self.logger.warning("[?? SUPREME] Supreme Consciousness not available - using standard mode")
        ...
        try:
            ...
            if warmup_started:
                ...
                self.logger.warning("[?? FastLLM] Warmup thread already running or model already hot")
        except Exception as e:
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        symbol = genome.get('metadata', {}).get('symbol', 'US30')
        tick_data = genome.get('tick_data', {})
        ...
        if trinity_response:
            ...
            if _ecosystem_penalty < 1.0:
                self.logger.warning(f"[ECOSYSTEM] ?? Penalty applied: {_ecosystem_penalty:.2f} | Reason: {_ecosystem_validation.get('reason', 'N/A')}")
        ...
        spread = (ask - bid) if bid > 0 and ask > 0 else 0  # In points (US30: 1 point = 1.0)
        ...
        atr_val = indicators.get('atr', 10.0)  # Default 10 points for US30 index
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('US30')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        nova_dashboard.update(
            # Market data
            symbol=symbol,
            last_price=current_price,  # ?? FIX: was 'current_price' which is not a dashboard key, use 'last_price'
            ...
            # ? LLM7 OCULUS - Data Quality Validator (Port 8963)
            ...
            # ? LLM8 CHRONOS - Timing Optimizer (Port 8964)
            ...
            # ? LLM9 PREDATOR - Execution Engine (Port 8965)
            ...
            attack_ready=trinity_decision in ['BUY', 'SELL'] and trinity_confidence >= 65 and warmup_remaining <= 0,
            ...
            # ??? ECOSYSTEM VALIDATION - Pre-flight safety checks
        ...
        # INTELIGENCIA PRIMERO: 10 genomas (~10 segundos) permite acumular contexto m�nimo
        # Quimera envia 50 barras de historia, m�s 10 genomas = suficiente para decidir
        ...
        # ??? DURANTE COOLDOWN: Acumular inteligencia para el PR�XIMO ataque PERFECTO
        current_time = time.time()
        ...
        try:
            ...
            self.logger.debug(f"[Data] Received genome - Bid:{bid:.2f} Ask:{ask:.2f} Current:{current_price:.2f} BarClose:{bar_close:.2f} Bars:{len(closes)}")
            ...
            if volumes is not None:
                ...
                # Gap #6: Order Flow Analysis - FIXED: Use tick-based split instead of random
                bid_volume = volumes[-1] * 0.48  # Approximate bid volume (slight sell bias in index CFD)
                ask_volume = volumes[-1] * 0.52  # Approximate ask volume
            ...
            # SOLUCI�N: Reconstruir llm_responses dict completo con todos los 10 LLMs desde variables locales
            
            # STEP 1: Construir dict completo de 10 LLMs
            # Trinity proporciona: BAYESIAN, TECHNICAL, CHART, RISK, SUPREME (5)
            # Variables locales proporcionan: LLM6-LLM10 (5 m�s)
            all_10_llms = dict(trinity_llms) if trinity_llms else {}
            
            # Agregar LLM6-10 si existen
            if self.llm6_enabled:
                all_10_llms['NOVA'] = {
                    'decision': 'BUY' if self.last_llm6_whale_conf > 70 else 'SELL' if self.last_llm6_whale_conf < 30 else 'HOLD',
            ...
            # STEP 2: Calcular consensus from ALL 10 LLMs
            buy_votes = 0
            sell_votes = 0
            hold_votes = 0
            approve_votes = 0
            ...
            # Vote mapping para diferentes tipos de LLMs
            # ? FIXED: READY is timing (not directional), VETO is blocking (not SELL)
            buy_keywords = ['BUY', 'WHALE', 'EXECUTE']
            sell_keywords = ['SELL']
            hold_keywords = ['HOLD', 'WAIT', 'CAUTION', 'NEUTRAL', 'VETO', 'READY']
            ...
            # STEP 3: Calcular decisi�n de consenso basada en votos REALES de 10 LLMs
            ...
            bid = tick_data.get('bid', current_price - 2.5)
            ask = tick_data.get('ask', current_price + 2.5)
            ...
            # ---------------------------------------------------------------------------
            # ??? ECOSYSTEM PENALTY APPLICATION - Reduce lot size when risks detected
            # ---------------------------------------------------------------------------
            if _ecosystem_penalty < 1.0:
            ...
            # CRITICAL: Round to valid lot increments (0.01 for US30)
            ...
            # GA optimizes TP/SL dynamically based on accumulated learning
            tp_multiplier = 1.3 if self.ga_optimizer.generation > 0 else 1.1
            sl_multiplier = 0.70 if self.ga_optimizer.generation > 0 else 0.80
            
            # ============ COMPREHENSIVE CONSENSUS SCORING (OPTIMIZED) ============
            # ??? IMPROVED: Ahora incluye LLM7/8/9 en la decisi�n
            # ? FIXED: llm_parallel est� DESHABILITADO, redistribuir su peso a componentes activos
            weights = {
                'trinity': 0.30,           # Trinity consensus (core signal) - INCREASED from 0.25
                'llm_parallel': 0.0,       # ?? DISABLED - no usar peso muerto
            ...
            llm_score = 0  # ?? DISABLED - was llm_confidence / 100.0
            ...
            # ============ CRITICAL FIX: ALIGN WEAK SYSTEMS WITH TRINITY ============
            # If Trinity is very confident (>60%), other weak systems should ALIGN
            # This prevents 1 strong signal + 6 weak signals from blocking attacks
            if trinity_confidence > 60:
                # Trinity is highly confident - other systems should reinforce
                if trinity_score > 0.5:  # Trinity is BUY (>50%)
                    # Boost all other scores toward positive
                    ml_score = max(ml_score, 0.55)  # At least medium confidence
                    lstm_score = max(lstm_score, 0.55)  # At least slightly bullish
                    bayesian_score = max(bayesian_score, 0.55)  # At least slightly bullish
                    llm_score = max(llm_score, 0.50)  # Neutral at worst
                elif trinity_score < 0.5:  # Trinity is SELL (<50%, or we use opposite for SELL)
                    # Boost other scores toward negative
                    ml_score = min(ml_score, 0.45)  # At most medium-low
                    lstm_score = min(lstm_score, 0.45)  # At most slightly bearish
                    bayesian_score = min(bayesian_score, 0.45)  # At most slightly bearish
                    llm_score = min(llm_score, 0.50)  # Neutral at worst
                
                # Trinity-high weight scheme - mantiene LLM7/8/9 pero reduce otros
                # ? FIXED: llm_parallel=0.0 (was 0.12 ghost weight), redistributed to active components
                weights = {
                    'trinity': 0.40,           # Increase when highly confident
                    'llm_parallel': 0.0,       # ?? FIXED: Was 0.12 ghost (llm_score=0 always)
            ...
            # FIXED: anomaly_score is clipped [0,1] (np.clip(mean_z_score/3, 0, 1))
            # High score = high anomaly. Block when score > 0.85 (extreme market conditions)
            # Skip anomaly check if Trinity is highly confident (>65%) - Trinity knows best
            if is_anomaly and anomaly_score > 0.85 and trinity_confidence < 65:
                self.logger.warning(f"[ANOMALY_VETO] ?? Blocking: is_anomaly={is_anomaly}, score={anomaly_score:.2f} > 0.85, trinity_conf={trinity_confidence}% < 65%")
            ...
            if trend_type == 'RANGING' and trinity_decision in ['BUY', 'SELL']:
                ...
                if consensus_score < 0.55 and trinity_confidence < 60:
                    self.logger.warning(f"[TREND_VETO] ?? Blocking: RANGING market, consensus={consensus_score*100:.0f}% < 55%, trinity={trinity_confidence}% < 60%")
            ...
            # FIXED: Trinity needs >= 65% confidence + Consensus must NOT be HOLD
            ...
            # Block if consensus is very low (< 35%)
            consensus_reason = _consensus_data.get('reason', 'unknown') if '_consensus_data' in dir() else 'N/A'
            if consensus_score < 0.35 and trinity_decision in ['BUY', 'SELL']:
                self.logger.warning(f"[CONSENSUS_VETO] ?? BLOCKING: Consensus {consensus_score*100:.0f}% < 35% (reason: {consensus_reason}) - Too risky even with Trinity {trinity_confidence}%")
                return {
                    'action': 'HOLD',
            ...
            # ?? RELAXED THRESHOLD: Trinity must be >= 50% to execute trades (was 65% - too restrictive for US30)
            # US30 scalping requires faster entries - lower threshold allows more opportunities
            if trinity_confidence >= 50.0:
                self.logger.info(f"[CONFIDENCE_CHECK] ? Trinity {trinity_confidence}% >= 50% + Consensus {consensus_score*100:.0f}% - proceeding with {trinity_decision}")
                pass  # Continue to decision execution below
            else:
                # ?? BLOCK: Any confidence below 50% is rejected
                self.logger.warning(f"[CONFIDENCE_VETO] ?? Blocking {trinity_decision}: Trinity {trinity_confidence}% < 50% minimum (consensus {consensus_score*100:.0f}%)")
                return {
                    'action': 'HOLD',
            ...
            # For M1 index scalping, we can trade even in tight ranges - adjusted for US30
            atr_value = calculate_atr(highs, lows, closes, period=14) if len(highs) >= 14 else 5.0
            atr = atr_value  # CRITICAL: Update atr variable for decision_obj
            # Only block if ATR is extremely low (< 3 pips) which indicates a broken market
            if atr_value < 3.0:  # < 3 points = dead market
                self.logger.warning(f"[CONSOLIDATION_VETO] ?? Blocking: ATR={atr_value:.1f}points < 3 (broken market)")
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                # CRITICAL: Check if decision aligns with TREND
                trend_aligned = False
                if trinity_decision == 'BUY':
                    ...
                    if not trend_aligned:
                        self.logger.warning(f"[TREND CHECK] ? BUY against trend: MA5={ma5:.2f} MA20={ma20:.2f} RSI={rsi_val:.0f} ADX={adx_val:.0f}")
                        trinity_confidence *= 0.65  # Reduce confidence 35% if against trend
                elif trinity_decision == 'SELL':
                    ...
                    if not trend_aligned:
                        self.logger.warning(f"[TREND CHECK] ? SELL against trend: MA5={ma5:.2f} MA20={ma20:.2f} RSI={rsi_val:.0f} ADX={adx_val:.0f}")
                        trinity_confidence *= 0.65  # Reduce confidence 35%
                
                # WHALE TRAP: Reduce confidence if detected (don't block, just penalize)
                if is_trap and trap_confidence > 60:
                    trinity_confidence *= 0.70  # Reduce 30% if whale trap
                    self.logger.warning(f"[WHALE TRAP] Confidence reduced to {trinity_confidence:.0f}% due to trap")
                ...
                # ? SUPER ENTRADA: Agregar consensus data al genome para detecci�n
                genome['consensus'] = _consensus_data if _consensus_data else {}
                
                # --------------------------------------------------------------------
                # ?? EXCALIBUR KNIGHT STRATEGY VALIDATION
                # When EXCALIBUR_KNIGHT is active, validate H1 breakout setups
                # --------------------------------------------------------------------
                excalibur_valid = False
                excalibur_result = None
                if Config.is_excalibur_active() and hasattr(self, 'excalibur_analyzer') and self.excalibur_analyzer:
                    # Build Excalibur indicators
                    excalibur_indicators = {
                        'rsi': gate_indicators.get('rsi', 50),
                        'atr_pips': atr_value if atr_value else 0,  # US30: ATR already in points (no conversion needed)
                        'bb_upper': gate_indicators.get('bb_upper', 0),
                        'bb_lower': gate_indicators.get('bb_lower', 0),
                        'bb_middle': (gate_indicators.get('bb_upper', 0) + gate_indicators.get('bb_lower', 0)) / 2,
                        'price': closes[-1] if closes else 0,
                        'volume_ratio': genome.get('volume_ratio', 1.0)
                    }
                    ...
                    if NEWS_INTELLIGENCE_AVAILABLE:
                        try:
                            news_bias = get_trading_bias('US30')
                            if news_bias and news_bias.get('active_news_event'):
                                news_context = {
                                    'high_impact_active': True,
                                    'event_name': news_bias.get('event_name', 'Unknown'),
                                    'bias': news_bias.get('direction', 'NEUTRAL')
                                }
                        except:
                            pass
                    
                    # Validate Excalibur setup
                    excalibur_result = self.excalibur_analyzer.analyze_h1_breakout(
                        price_data={'close': closes},
                        indicators=excalibur_indicators,
                        news_context=news_context
                    )
                    
                    excalibur_valid = excalibur_result.get('excalibur_valid', False)
                    
                    if excalibur_valid:
                        # ?? EXCALIBUR APPROVED - Override with Excalibur parameters
                        trinity_decision = excalibur_result['signal']
                        trinity_confidence = excalibur_result['confidence']
                        
                        # Log Excalibur signal
                        self.logger.warning(f"[?? EXCALIBUR] ? KNIGHT APPROVED: {trinity_decision} @ {excalibur_result['entry_price']:.2f}")
                        self.logger.info(f"[?? EXCALIBUR] TP1={excalibur_result['tp1']:.2f} TP2={excalibur_result['tp2']:.2f} SL={excalibur_result['sl']:.2f}")
                        self.logger.info(f"[?? EXCALIBUR] R:R=1:{excalibur_result['risk_reward']:.1f} Confidence={trinity_confidence:.0f}%")
                        for reason in excalibur_result['reasons']:
                            self.logger.info(f"[?? EXCALIBUR] {reason}")
                        
                        # Add Excalibur data to genome for downstream processing
                        genome['excalibur_signal'] = excalibur_result
                        genome['excalibur_valid'] = True
                    else:
                        # Excalibur not valid but strategy is active - block non-Excalibur trades
                        if trinity_decision in ['BUY', 'SELL']:
                            self.logger.info(f"[?? EXCALIBUR] ? No valid breakout - waiting for knight's moment")
                            if excalibur_result.get('warnings'):
                                for warning in excalibur_result['warnings'][:3]:  # Limit to 3 warnings
                                    self.logger.debug(f"[?? EXCALIBUR] {warning}")
                            trinity_decision = 'HOLD'
                            trinity_confidence = 0
                ...
                # ??? SUPREME CONSCIOUSNESS LAYER - Meta-Cognitive Intelligence ???
                # This is the "brain of brains" - evaluates all signals through meta-cognition
                supreme_override = None
                supreme_veto = False
                
                if self.supreme_consciousness and gate_attack:
                    try:
                        # Build LLM responses dict for Supreme analysis
                        llm_responses_for_supreme = trinity_response.get('llm_responses', {}) if trinity_response else {}
                        
                        # Analyze through Supreme Consciousness
                        consciousness_snapshot = self.supreme_consciousness.analyze(
                            genome=genome,
                            llm_responses=llm_responses_for_supreme
                        )
                        
                        # Log Supreme Consciousness analysis
                        self.logger.info(f"[?? SUPREME] Q:{consciousness_snapshot.quantum_state.value[:6]} | "
                                        f"N:{consciousness_snapshot.noise_level:.0%} | "
                                        f"T:{consciousness_snapshot.time_quality:.0%} | "
                                        f"META:{consciousness_snapshot.meta_confidence:.0f}%")
                        
                        # VETO CONDITIONS from Supreme Consciousness
                        if consciousness_snapshot.decision == 'HOLD':
                            # Supreme Consciousness vetoes the trade
                            supreme_veto = True
                            gate_attack = False
                            gate_decision = 'HOLD'
                            gate_reason = f"??SUPREME VETO: {consciousness_snapshot.reasoning}"
                            self.logger.warning(f"[?? SUPREME] ? VETO: {consciousness_snapshot.reasoning}")
                        
                        # CONFIDENCE ADJUSTMENT from Supreme
                        elif consciousness_snapshot.meta_confidence < gate_confidence * 0.7:
                            # Supreme has much lower confidence - reduce gate confidence
                            old_conf = gate_confidence
                            gate_confidence = min(gate_confidence, consciousness_snapshot.meta_confidence * 1.1)
                            self.logger.info(f"[?? SUPREME] ?? Confidence adjusted: {old_conf:.0f}% ? {gate_confidence:.0f}%")
                        
                        # BOOST CONDITIONS from Supreme
                        elif consciousness_snapshot.quantum_state.value == 'RESONANT':
                            # Perfect market state - boost confidence
                            gate_confidence = min(95, gate_confidence * 1.15)
                            self.logger.info(f"[?? SUPREME] ?? RESONANT STATE: Confidence boosted to {gate_confidence:.0f}%")
                        
                    except Exception as supreme_error:
                        self.logger.debug(f"[SUPREME] Error: {supreme_error}")
            ...
            # ---------------------------------------------------------------
            # ?? DEBUG LOGGING: Entender por qu� se toma cada decisi�n
            # ---------------------------------------------------------------
            indicators = genome.get('indicators', {}).get('current', {})
            price_data = genome.get('price_data', {})
            ...
            # ?? FIX v4: Validate Trinity values against broker STOPS_LEVEL (index CFD)
            MIN_TP_DIST = 10.0  # 10 points minimum (M1 scalping)
            MIN_SL_DIST = 8.0   # 8 points minimum (M1 scalping)
            SAFETY = 5.0        # 5 points safety buffer
            ...
            if trinity_response and isinstance(trinity_response, dict) and not trinity_response.get('_is_fallback', False):
                ...
                if trinity_tp > 0 and trinity_sl > 0:
                    ...
                    if actual_tp_dist >= MIN_TP_DIST and actual_sl_dist >= MIN_SL_DIST:
                        tp_price = trinity_tp
                        sl_price = trinity_sl
                        tp_dist = trinity_tp_dist if trinity_tp_dist > 0 else int(actual_tp_dist)
                        sl_dist = trinity_sl_dist if trinity_sl_dist > 0 else int(actual_sl_dist)
                        self.logger.info(f"[TP/SL] ? Using Trinity: TP={tp_price:.2f} ({actual_tp_dist:.1f}pts), SL={sl_price:.2f} ({actual_sl_dist:.1f}pts)")
                    else:
                        # Trinity values too small - will recalculate locally
                        self.logger.warning(f"[TP/SL] ?? Trinity values TOO SMALL: TP_dist={actual_tp_dist:.2f}$ (min={MIN_TP_DIST}), SL_dist={actual_sl_dist:.2f}$ (min={MIN_SL_DIST}) - recalculating...")
                        tp_price = None
                        sl_price = None
            ...
            if tp_price is None or sl_price is None:
                ...
                # Ensure ATR is in indicators (default 10 points for US30 index)
                indicators['atr'] = atr if atr > 0 else 10.0
                
                # Get spread from tick data
                tick_data = genome.get('tick_data', {})
                spread = tick_data.get('spread', 10)  # Spread in points for US30 index
            ...
            decision_obj = {
                ...
                # ??? NEW: Ecosystem Validation Status
            ...
            try:
                ...
                nova_dashboard.update(
                    ...
                    market_state=getattr(market_analysis_result.market_state, 'value', market_analysis_result.market_state) if market_analysis_result else 'UNKNOWN',  # 🔧 FIX CAT-99
                    trend_strength=getattr(market_analysis_result.trend_strength, 'value', market_analysis_result.trend_strength) if market_analysis_result else 1,  # 🔧 FIX CAT-99
                    ...
                    attack_probability=self._calculate_attack_probability(trinity_llms, trinity_decision, trinity_confidence),
                    attack_ready=trinity_decision in ['BUY', 'SELL'] and trinity_confidence >= 65,
                    attack_direction=self._get_attack_direction(trinity_llms, trinity_decision),
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 8970
        ...
        try:
            ...
            # Send to sound.py server on port 8970 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 8970))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:8970")
        except socket.timeout:
    ...
    def _analyze_pattern_intelligence(self, patterns, price_data, indicators, decision):
        """
        ?? PATTERN INTELLIGENCE - Analiza patrones como un trader experto ver�a el chart
        
        No solo detecta patrones, INTERPRETA qu� significan en contexto:
        - �El patr�n est� en la ubicaci�n correcta? (soporte/resistencia)
        - �La vela confirma el patr�n? (volumen, tama�o)
        - �El contexto favorece el patr�n? (tendencia, momentum)
        - �Cu�l es la expectativa de movimiento?
        
        Returns: dict with intelligence analysis
        """
        analysis = {
            'pattern_quality': 0,  # 0-100 qu� tan bueno es el setup
            'pattern_location': 'UNKNOWN',  # WHERE: support, resistance, mid-range
        ...
        for p in patterns[:5]:
            ...
            if any(x in p_name for x in ['hammer', 'morning_star', 'bullish_engulf', 'piercing']):
                if range_position < 0.35:  # At support = PERFECT
                    ...
                    bullish_power += 5
                    analysis['visual_notes'].append(f"?? {p_name.upper()} at resistance = weak signal (wrong location)")
                else:
                    bullish_power += 15
            ...
            # BEARISH REVERSAL PATTERNS  
                ...
                if range_position > 0.65:  # At resistance = PERFECT
                    ...
                    bearish_power += 5
                    analysis['visual_notes'].append(f"?? {p_name.upper()} at support = weak signal (wrong location)")
                else:
                    bearish_power += 15
            ...
            # DOJI (Indecision - needs context)
                ...
                if range_position < 0.3 or range_position > 0.7:
                    analysis['visual_notes'].append(f"?? DOJI at extreme = Market deciding direction")
                else:
        ...
        if decision == 'BUY':
            if bullish_power > bearish_power + 10:
                ...
                analysis['visual_notes'].append(f"?? Patterns OPPOSE {decision} - bearish signals stronger!")
            else:
            ...
            if bearish_power > bullish_power + 10:
                ...
                analysis['pattern_alignment'] = 'OPPOSED'
                analysis['visual_notes'].append(f"?? Patterns OPPOSE {decision} - bullish signals stronger!")
            else:
                analysis['pattern_alignment'] = 'NEUTRAL'
    ...
    def _accumulate_attack_intelligence(self, trinity_llms, trinity_decision, trinity_confidence, 
        ...
        El cooldown NO es tiempo muerto - es tiempo de PREPARACI�N.
        Cada tick que pasa acumulamos:
        1. Votos de los 9 LLMs y sus tendencias
        2. Momentum de precio y direcci�n
        3. Se�ales de whales y smart money
        ...
        try:
            ...
            # --- 3. ACUMULAR SE�ALES DE WHALES (LLM6) ---
            ...
            # --- 5. CALCULAR DIRECCI�N �PTIMA (Consenso acumulado) ---
    ...
    def _get_prepared_attack_boost(self, trinity_decision, trinity_confidence):
        """
        ? Obtener boost de confianza basado en preparaci�n acumulada durante cooldown
        
        Si la preparaci�n confirma la direcci�n de Trinity, aumentamos confianza.
        ...
        try:
            ...
            # Si no hay suficiente preparaci�n, no hay boost
            if prep['preparation_score'] < 30:
                return 0.0
            
            # Direction alignment bonus
            if prep['optimal_direction'] == trinity_decision:
                boost += 0.10  # +10% si la preparaci�n confirma
                
                # Bonus extra por alta preparaci�n
                if prep['preparation_score'] >= 70:
                    boost += 0.05  # +5% m�s si preparaci�n es excelente
                ...
                # Preparaci�n contradice Trinity - reducir confianza
                boost -= 0.08  # -8%
                self.logger.warning(f"[AttackPrep] ?? Prep direction ({prep['optimal_direction']}) contradicts Trinity ({trinity_decision})")
...
class AdvancedIPC:
    ...
    def __init__(self):
        ...
        self.cache_ttl = 2.0  # FIXED: 2.0s reuse reduces blocking from every tick to every 2s (was 0.5s)
    ...
    def _play_sound_alert(self, alert_type, symbol='US30'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # ?? FIX: Use Config.SOUND_PORT instead of hardcoded 8970
            sound_port = getattr(Config, 'SOUND_PORT', 8970)
            ...
            self.logger.warning(f"[AUDIO] ?? sound.py timeout - may be busy")
        except Exception as e:
                ...
                if request.get('type') == 'handshake':
                    ...
                    with self.quimera_lock:
                        self.quimera_connections[('executor', addr)] = {
                            ...
                            'type': 'executor',
                            'last_ping': time.time(),
                            'symbol': request.get('symbol', 'US30')
                        }
                    ...
                    # CRITICAL: This enables ML learning from trade results!
                        ...
                        try:
                            ...
                            if 'TP' in reason.upper() or pnl > 0:
                                ...
                                self.logger.warning(f"[AUDIO] ?? No sound condition met: reason={reason}, pnl={pnl}")
                            ...
                            with self.stats_lock:
                                ...
                                if 'TP' in reason.upper():
                                    ...
                                    self.logger.info(f"[STATS] ??? SL Hit! Total SL: {self.stats['sl_hits']}")
                            ...
                            if SHARED_BRAIN_AVAILABLE:
                                try:
                                    ...
                                    pass
                            # FIX D5: RL immediate learn on trade close
                            try:
                                _reward_rl = float(__import__('numpy').tanh(pnl / 10.0))
                                _action_rl = action  # [NOVA REPAIR ARCHITECT - FIX CRÍTICO] C-01: use actual trade direction, not PnL sign
                                _state_rl = f"{symbol}_generic"
                                self.rl_engine.learn_immediate(_state_rl, _action_rl, _reward_rl)
                            ...
                            if self.trade_closed_callback is not None:
                                try:
                                    # [NOVA LEARNING ARCHITECT - FIX CRÍTICO] Match trade_id format
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # ?? DEBUG: Log raw genome keys BEFORE normalization
                    raw_tf = genome.get('timeframes', 'MISSING')
                    tf_info = f"len={len(raw_tf)}" if isinstance(raw_tf, list) else f"type={type(raw_tf).__name__}"
                    self.logger.info(f"[Injector] ?? RAW genome type={genome.get('type')} timeframes={tf_info} keys={sorted(list(genome.keys()))[:15]}")
                    if isinstance(raw_tf, list) and len(raw_tf) > 0:
                        self.logger.info(f"[Injector] ?? timeframes[0] sample keys: {sorted(list(raw_tf[0].keys()))[:15] if isinstance(raw_tf[0], dict) else raw_tf[0]}")
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "US30", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "US30"}, ...}
        """
    ...
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for US30 data"""
        try:
            genome_type = genome.get('type', '')
            
            # ? Accept both standard and Multi-TF genome types from us30quimera
            valid_types = [
                'market_genome', 'tick_genome', 'genome', 'tick', 'portfolio_sync',
                'market_genome_multi_tf', 'tick_genome_multi_tf'  # ? Multi-TF types
            ]
            
            if genome_type in valid_types:
                # Portfolio sync is always valid
                if genome_type == 'portfolio_sync':
                    return True
                
                # Try to get bid/ask from tick_data OR from root (MQL5 sends flat structure)
                tick_data = genome.get('tick_data', {})
                
                # Check tick_data first, then fall back to root
                bid = tick_data.get('bid') if 'bid' in tick_data else genome.get('bid')
                ask = tick_data.get('ask') if 'ask' in tick_data else genome.get('ask')
                
                # MUST have bid/ask
                if bid is None or ask is None:
                    self.logger.warning(f"[Validator] Missing bid/ask in genome")
                    return False
                ...
                try:
                    ...
                    # Minimal sanity: bid < ask (allow equal for index CFD)
                    if bid > ask:
                        self.logger.warning(f"[Validator] Inverted: bid={bid} > ask={ask}")
                        return False
                    
                    # Acceptable range for US30 (Dow Jones Index CFD)
                    # US30 trades around 30,000-50,000 points
                    # Accept range: 15000 - 60000 to be safe for historical/future variations
                    mid_price = (bid + ask) / 2.0
                    
                    # US30 index price range (15000 - 60000)
                    if 15000 <= mid_price <= 60000:
                        return True
                    
                    # Price out of realistic range for US30
                    self.logger.warning(f"[Validator] US30 price out of range: {mid_price:.2f} (expected 15000-60000)")
                    return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                sock.settimeout(2.5)  # FIXED: 2.5s max wait - fail fast if Trinity overloaded (was 5.0s)
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-94
        ...
        if offline_count > 20:
            self.logger.error(f"[Trinity] ?? OFFLINE FOR {offline_count} GENOMES - Check: python trinity.py")
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            # ---------------------------------------------------------------------------
            # ??? SAFETY LOCK REMOVED: The check is already done in process_genome()
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ? NO EXECUTOR CONNECTION - Quimera not connected to port 8956")
            ...
            # 1. Validate price is within realistic bounds (US30: 15000-60000 index range)
            # 2. Validate stops are within reasonable distance from entry
            ...
            # ? DEBUG: Log entry price y acci�n
            self.logger.info(f"[EINSTEIN] ?? Starting calculation: {action} @ Entry={entry_price:.2f}")
            
            # ?? STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR US30
            # ? US30 is Dow Jones Index CFD, typically 30000-50000, accept 15000-60000 range
            valid_index_range = 15000 <= entry_price <= 60000  # Normal US30 index pricing
            
            if not valid_index_range:
                self.logger.error(f"[EINSTEIN] ? Entry price {entry_price:.2f} out of realistic range for US30 (expected 15000-60000)")
                return False
            ...
            # ?? Check if entry_price changed significantly from original decision
            original_entry = decision.get('entry', entry_price)
            if abs(entry_price - original_entry) > 10.0:  # > 10 points change
                self.logger.warning(f"[EINSTEIN] ?? Entry price changed significantly: original={original_entry:.2f}, current={entry_price:.2f} (diff={abs(entry_price-original_entry):.1f}pts)")
            
            # Use ATR-based approach (1.1x for TP, 0.8x for SL) - PROVEN WORKING
            atr_val = decision.get('atr', 10.0)  # Default 10 points for US30 index
            atr_original = atr_val
            
            # ?? CRITICAL: Clamp ATR to M1 US30 realistic range
            # M1 ATR t�pico: 5-50 points
            # Si viene m�s alto, el mercado est� loco o hay error
            atr_val = max(5.0, min(80.0, atr_val))
            if atr_val != atr_original:
                self.logger.debug(f"[EINSTEIN] ATR clamped: {atr_original:.1f} ? {atr_val:.1f}")
            
            # ? SCALPING TP/SL: ATR � 1.1 (TP) and ATR � 0.8 (SL)
            tp_distance = atr_val * 1.1
            sl_distance = atr_val * 0.8
            
            # ---------------------------------------------------------------------------
            # MT5 STOPS_LEVEL REQUIREMENT: Broker requires minimum distance from ASK/BID
            # ?? FIX v4: US30 index - distances in points (1.0 = 1 point)
            # ---------------------------------------------------------------------------
            MIN_TP_DISTANCE = 10.0    # 10 points minimum TP (M1 scalping)
            MIN_SL_DISTANCE = 8.0     # 8 points minimum SL (M1 scalping)
            SAFETY_MARGIN = 5.0       # 5 points extra safety buffer for ASK/BID slippage
            
            tp_distance = max(MIN_TP_DISTANCE, tp_distance)
            sl_distance = max(MIN_SL_DISTANCE, sl_distance)
            
            # ?? HARD CAP: M�ximo absoluto para scalping M1 US30 index
            tp_distance = min(tp_distance, 60.0)  # Max 60 points TP
            sl_distance = min(sl_distance, 50.0)  # Max 50 points SL
            
            self.logger.info(f"[EINSTEIN] ? ATR-Based: TP={tp_distance:.0f}pts SL={sl_distance:.0f}pts")
            ...
            # ? CRITICAL FIX: Verificar distancias DESPU�S del redondeo
            # El redondeo puede causar que las distancias sean menores al m�nimo
            ...
            if actual_tp_distance < MIN_TP_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ?? TP distance ({actual_tp_distance:.1f}pts) < MIN ({MIN_TP_DISTANCE:.0f}pts), adjusting...")
                if action == 'BUY':
            ...
            if actual_sl_distance < MIN_SL_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ?? SL distance ({actual_sl_distance:.1f}pts) < MIN ({MIN_SL_DISTANCE:.0f}pts), adjusting...")
                if action == 'BUY':
            ...
            tp_pips = actual_tp_distance / 1.0  # Convert to points (US30 index)
            sl_pips = actual_sl_distance / 1.0
            ...
            if action == 'BUY':
                if not (tp_price > entry_price > sl_price):
                    self.logger.error(f"[EINSTEIN] ? BUY invalid: TP({tp_price:.2f}) > Entry({entry_price:.2f}) > SL({sl_price:.2f}) FAILED")
                    # Force recalculation with GUARANTEED SAFE values
                    tp_price = round(entry_price + MIN_TP_DISTANCE + SAFETY_MARGIN, 2)
                    sl_price = round(entry_price - MIN_SL_DISTANCE - SAFETY_MARGIN, 2)
                    self.logger.info(f"[EINSTEIN] ?? BUY corrected: TP={tp_price:.2f}, SL={sl_price:.2f}")
            elif action == 'SELL':
                if not (sl_price > entry_price > tp_price):
                    self.logger.error(f"[EINSTEIN] ? SELL invalid: SL({sl_price:.2f}) > Entry({entry_price:.2f}) > TP({tp_price:.2f}) FAILED")
                    # Force recalculation with GUARANTEED SAFE values
                    tp_price = round(entry_price - MIN_TP_DISTANCE - SAFETY_MARGIN, 2)
                    sl_price = round(entry_price + MIN_SL_DISTANCE + SAFETY_MARGIN, 2)
                    self.logger.info(f"[EINSTEIN] ?? SELL corrected: TP={tp_price:.2f}, SL={sl_price:.2f}")
            
            # Final verification log
            tp_distance_final = abs(tp_price - entry_price)
            sl_distance_final = abs(sl_price - entry_price)
            tp_pips_final = tp_distance_final / 1.0
            sl_pips_final = sl_distance_final / 1.0
            
            # ?? ASSERT: Distances meet minimum requirements
            if tp_distance_final < MIN_TP_DISTANCE or sl_distance_final < MIN_SL_DISTANCE:
                self.logger.error(f"[EINSTEIN] ?? CRITICAL: Final distances BELOW minimum! TP={tp_distance_final:.1f}pts (min={MIN_TP_DISTANCE:.0f}), SL={sl_distance_final:.1f}pts (min={MIN_SL_DISTANCE:.0f})")
                return False
            
            self.logger.info(f"[EINSTEIN] ? VALIDATED: {action} Entry={entry_price:.2f} TP={tp_price:.2f}({tp_pips_final:.0f}p, dist={tp_distance_final:.1f}pts) SL={sl_price:.2f}({sl_pips_final:.0f}p, dist={sl_distance_final:.1f}pts)")
            ...
            with self.quimera_lock:
                try:
                    ...
                    self.logger.info(f"[Quimera TX] ? Sent {action} @ {entry_price:.2f}: volume={response['volume']}, sl={sl_price:.2f}, tp={tp_price:.2f}")
...
# ==================== MAIN ENGINE ====================
class QuantumCore:
    """NOVA SNIPER SCALPING v18.00 - FRANCOTIRADOR EDITION"""
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        3. If gap > 1.0 point, append current tick to closes array
        ...
        if closes and current_price > 0:
            ...
            if gap > 1.0:  # More than 1.0 point difference (US30 index)
    ...
    def __init__(self):
        ...
        # [NOVA AUDIT F-13] CalibrationAdvisor — US30 was missing this
        try:
            if CALIBRATION_ADVISOR_AVAILABLE and CalibrationAdvisor:
                self.calibration_advisor = CalibrationAdvisor()
                self.logger.info("[📊 CALIBRATION] CalibrationAdvisor initialized for US30")
            else:
                self.calibration_advisor = None
        except Exception as _ca_e:
            self.calibration_advisor = None
            self.logger.warning(f"[CALIBRATION] CalibrationAdvisor init failed: {_ca_e}")

        # 🗡️ EXCALIBUR KNIGHT Strategy Integration
        self.excalibur_analyzer = None
        self.excalibur_news_manager = None
        if Config.is_excalibur_active() and EXCALIBUR_AVAILABLE:
            self.excalibur_analyzer = ExcaliburSignalAnalyzer(Config.EXCALIBUR_CONFIG)
            self.excalibur_news_manager = ExcaliburNewsManager(Config.EXCALIBUR_CONFIG)
            self.logger.info("[?? EXCALIBUR] Knight analyzer initialized - M1/M5 Elite Momentum mode")
        ...
        # ? SUPREME CONSCIOUSNESS Learning - Record trade outcomes for wisdom accumulation
        def _on_trade_closed_supreme(entry, exit, result_pips, symbol, reason):
            """Callback to teach Supreme Consciousness from trade results"""
            if self.engine.supreme_consciousness:
                try:
                    outcome = 'WIN' if result_pips > 0 else 'LOSS' if result_pips < 0 else 'BE'
                    self.engine.supreme_consciousness.record_trade_outcome(outcome, result_pips)
                    self.logger.info(f"[?? SUPREME] Wisdom recorded: {outcome} {result_pips:.1f}pips | "
                                    f"Patterns learned: {len(self.engine.supreme_consciousness.wisdom.pattern_win_rates)}")
                except Exception as e:
                    self.logger.debug(f"[SUPREME] Learning error: {e}")
        
        # Chain the callbacks
        original_ml_callback = self.ipc.trade_closed_callback
        def _chained_trade_closed_callback(entry, exit, result_pips, symbol, reason):
            """Chained callback for both ML and Supreme learning"""
            # Original ML learning
            if original_ml_callback:
                original_ml_callback(entry, exit, result_pips, symbol, reason)
            # Supreme Consciousness learning
            _on_trade_closed_supreme(entry, exit, result_pips, symbol, reason)
        
        self.ipc.trade_closed_callback = _chained_trade_closed_callback
        self.logger.info("[? SUPREME WISDOM] Supreme Consciousness learning connected")
        ...
        self.price_buffer = deque(maxlen=100)  # �ltimos 100 precios para momentum
        ...
        # FIX M-US30-01: NOVA singleton — create once, reuse every process_genome call
        self.nova_detector = NOVAMarketStateDetector(logger=logger)
        # FIX A-US30-02: consecutive-loss halt state
        self.consecutive_losses = 0
        self.loss_halt_until = 0.0
        # FIX A-US30-04: stale position reconcile timer
        self.last_position_reconcile = 0.0
        
        self._print_banner()
    
    def _print_banner(self):
        """Print professional banner with active strategy"""
        strategy_info = Config.get_strategy_info()
        is_excalibur = Config.is_excalibur_active()
        
        logger.info("")
        logger.info("=" * 110)
        if is_excalibur:
            logger.info("??  NOVA EXCALIBUR KNIGHT v18.00 - US30 ELITE MOMENTUM SNIPER")
            logger.info("???  \"En el fragor eterno de la batalla, aguardo el susurro del destino para asestar el golpe inmortal\"")
        else:
            logger.info("?? NOVA SNIPER SCALPING v18.00 - US30 FRANCOTIRADOR CONSCIOUSNESS (120s cooldown, 75%+ win rate)")
        logger.info("=" * 110)
        logger.info("")
        
        # Strategy info
        logger.info(f"ACTIVE STRATEGY: {strategy_info['name']}")
        logger.info(f"  � Description: {strategy_info['description']}")
        logger.info(f"  � Timeframe: {strategy_info['timeframe']}")
        logger.info(f"  � TP Range: {strategy_info['tp_range']}")
        logger.info(f"  � SL: {strategy_info['sl']}")
        logger.info(f"  � Risk/Reward: {strategy_info['risk_reward']}")
        logger.info(f"  � Trades/Week: {strategy_info['trades_per_week']}")
        logger.info(f"  � Style: {strategy_info['style']}")
        logger.info("")
        
        if is_excalibur:
            logger.info("?? EXCALIBUR KNIGHT FEATURES:")
            logger.info("  + M1 Entry with M5/M15 Target Confirmation")
            logger.info("  + Bollinger Band Breakout Analysis (>8 pips)")
            logger.info("  + Institutional Volume Confirmation (>1.3x)")
            logger.info("  + News Event Catalyst Detection")
            logger.info("  + Multi-Timeframe Trend Alignment")
            logger.info("  + Partial Close: 50% @ +15 pips")
            logger.info("  + Trailing Stop: +10 pips activation")
            logger.info("  + 30-minute Max Trade Duration")
            logger.info("")
    ...
    def start(self):
        ...
        # CRITICAL: Start ping thread to keep EA connection alive
        threading.Thread(target=self._send_pings_to_quimera, daemon=True).start()
        
        logger.info(f"[OK] SNIPER SCALPING v18.00 ONLINE - ?? Index CFD Scalper Mode ACTIVE")
        ...
        logger.info(f"[OK] Ping thread: Sending keepalive every 3s to EA")
        logger.info("")
    
    def _send_pings_to_quimera(self):
        """CRITICAL: Send PING every 3 seconds to keep EA connection alive
        
        The MT5 EA (US30quimera_bridge.mq5) expects to receive PING messages
        regularly. If no PING is received within InpBSF_HeartbeatTimeoutMs (15s),
        the EA enters BUNKER_MODE and disconnects.
        
        This thread sends {"type": "ping", "timestamp": X} to all executor connections.
        """
        logger.info("[PING] ?? Ping thread started - sending every 3 seconds to Quimera EA")
        ping_count = 0
        
        while self.running:
            try:
                time.sleep(3.0)  # Send ping every 3 seconds (EA timeout is 15s)
                
                with self.ipc.quimera_lock:
                    executor_connections = []
                    for key, conn_info in self.ipc.quimera_connections.items():
                        if isinstance(key, tuple) and key[0] == 'executor':
                            executor_connections.append((key, conn_info))
                    
                    if not executor_connections:
                    ...
                    msg_bytes = json.dumps(ping_msg).encode('utf-8')
                    header = struct.pack('<I', len(msg_bytes))
                    full_msg = header + msg_bytes
                    
                    for key, conn_info in executor_connections:
                        try:
                            ...
                            if sock:
                                ...
                                if ping_count % 10 == 0:  # Log every 10th ping
                                    logger.debug(f"[PING] ?? Sent ping #{ping_count} to {key}")
                        except Exception as e:
                            logger.warning(f"[PING] ?? Failed to ping {key}: {e}")
                            
            except Exception as e:
                logger.error(f"[PING] ? Error in ping loop: {e}")
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            # ?? FIX: Bridge format 'genome'/'tick' have bid/ask at ROOT level
            # Without this, genome passes process_genome gate but Transform returns
            # it unchanged ? no indicators ? no price_data.history ? 0/10 bars ? HOLD forever
            ...
            symbol = metadata.get('symbol', 'US30')
            bid = float(tick_data.get('bid', 0))
            ask = float(tick_data.get('ask', 0))
            ...
            # CRITICAL: Validate price is in reasonable range for US30 INDEX CFD
            # US30 (Dow Jones) typically trades 30000-50000, historical range 15000-60000
            # This is INDEX CFD, NOT forex - prices are around 42,000 not 1.XX
            if symbol == 'US30' and close_price > 0:
                if close_price < 15000 or close_price > 60000:
                    # Only reject EXTREME outliers (outside realistic index range)
                    ...
                    return None
                # For unusual but possible range, log warning but accept
                if close_price < 25000 or close_price > 50000:
                    self.logger.warning(f"[Transform] ?? Price {close_price:.2f} is unusual but accepting (volatile market)")
            ...
            if history and len(history) > 0:
                ...
                self.logger.warning(f"[Transform] ?? Insufficient history (buffer={len(self.price_buffer) if hasattr(self, 'price_buffer') else 0})")
                history = []
            ...
            if len(history) < 10:
                ...
                self.logger.info(f"[Transform] ?? Extended history with {remaining_bars} bars from current bar (insufficient history)")
            ...
            if hasattr(self, 'price_buffer') and len(self.price_buffer) >= 14:
                ...
                if len(history) >= 20:
                    ...
                    recent_ticks = list(self.price_buffer)[-50:]  # Last 50 ticks
                    bar_closes = [h.get('close', close_price) for h in history[-20:]]  # Last 20 bars
                ...
                self.logger.warning(f"[Transform] ?? INSUFFICIENT DATA: {len(history)} bars, using defaults")
            
            # -----------------------------------------------------------------------
            # ?? CRITICAL FIX: USE REAL MT5 INDICATORS FROM EA
            # The EA (us30quimera_bridge.mq5) sends REAL RSI, ATR, ADX, MAs, etc.
            # computed by MT5 from actual candle history via CopyBuffer().
            # These are in genome['timeframes'][0].
            # Previously we were IGNORING them and recalculating from synthetic bars
            # (price_buffer with identical prices) ? RSI=50, ATR=0, all flat ? never trades.
            ...
            self.logger.info(f"[Transform] ?? EA DEBUG: genome has 'timeframes'={type(timeframes_list).__name__} len={len(timeframes_list) if isinstance(timeframes_list, list) else 'N/A'}, 'indicators'={type(genome.get('indicators')).__name__}")
            if isinstance(timeframes_list, list) and len(timeframes_list) > 0:
                ea_tf = timeframes_list[0]  # M1 is first timeframe
                self.logger.info(f"[Transform] ?? EA DEBUG: timeframes[0] keys={list(ea_tf.keys()) if isinstance(ea_tf, dict) else ea_tf}")
            elif isinstance(genome.get('indicators'), dict):
                ea_ind = genome['indicators']
                self.logger.info(f"[Transform] ?? EA DEBUG: indicators keys={list(ea_ind.keys())[:10]}")
                # Support TWO formats:
                # 1. Bridge format: indicators = {rsi: 43, atr: 0.003, ...} (flat)
                # 2. Excalibur format: indicators = {current: {rsi: 43, atr_pips: 12, ...}, previous: {...}}
                if 'rsi' in ea_ind and ('atr' in ea_ind or 'atr_pips' in ea_ind):
                    ea_tf = ea_ind
                elif 'current' in ea_ind and isinstance(ea_ind.get('current'), dict):
                    # Excalibur Knight format: indicators.current has the real values
                    ea_current = ea_ind['current']
                    if 'rsi' in ea_current:
                        ea_tf = ea_current
                        self.logger.info(f"[Transform] ?? EA DEBUG: Using indicators.current (Excalibur format)")
            
            if ea_tf and isinstance(ea_tf, dict):
                _ea_rsi = ea_tf.get('rsi')
                # Support both 'atr' (bridge) and 'atr_pips' (Excalibur) field names
                _ea_atr = ea_tf.get('atr', ea_tf.get('atr_pips'))
                _ea_adx = ea_tf.get('adx')
                self.logger.info(f"[Transform] ?? EA VALUES: rsi={_ea_rsi} atr={_ea_atr} adx={_ea_adx}")
                ...
                if _ea_rsi is not None and _ea_atr is not None and _ea_adx is not None:
                    try:
                        ...
                        if _ea_atr >= 0:  # Accept ATR=0 for frozen markets - still use EA RSI/ADX
                            ea_rsi = _ea_rsi
                            ea_atr = _ea_atr
                            ea_adx = _ea_adx
                            ...
                            # Support both bridge ('boll_upper') and Excalibur ('bollinger_upper') field names
                            ea_boll_upper = float(ea_tf.get('boll_upper', ea_tf.get('bollinger_upper', 0))) if ea_tf.get('boll_upper') or ea_tf.get('bollinger_upper') else None
                            ea_boll_middle = float(ea_tf.get('boll_middle', ea_tf.get('bollinger_middle', 0))) if ea_tf.get('boll_middle') or ea_tf.get('bollinger_middle') else None
                            ea_boll_lower = float(ea_tf.get('boll_lower', ea_tf.get('bollinger_lower', 0))) if ea_tf.get('boll_lower') or ea_tf.get('bollinger_lower') else None
                            ea_stoch_k = float(ea_tf.get('stoch_main', 50))
                            ea_stoch_d = float(ea_tf.get('stoch_signal', 50))
                            # Support both 'adx_plus' (bridge) and 'di_plus' (Excalibur) field names
                            ea_adx_plus = float(ea_tf.get('adx_plus', ea_tf.get('di_plus', 0)))
                            ea_adx_minus = float(ea_tf.get('adx_minus', ea_tf.get('di_minus', 0)))
            ...
            if ea_rsi is not None and ea_atr is not None:
                ...
                self.logger.warning(f"[Transform] ?? No EA indicators, falling back to synthetic calculation")
            ...
            # ADX CALCULATION: Only if NOT using EA indicators (EA already provides ADX)
            if ea_rsi is None or ea_atr is None:
            ...
            # If EA real indicators are available, use them directly; otherwise calculate synthetically
            if ea_ma_fast is not None:
                ma_fast_calc = ea_ma_fast
                ma_slow_calc = ea_ma_slow if ea_ma_slow is not None else ea_ma_fast
                self.logger.info(f"[Transform] ? USING EA MA: fast={ma_fast_calc:.2f} slow={ma_slow_calc:.2f}")
            elif len(closes_array) >= 5:
                ma_fast_calc = float(np.mean(closes_array[-5:]))
                ma_slow_calc = float(np.mean(closes_array[-20:])) if len(closes_array) >= 20 else close_price
            elif len(self.price_buffer) >= 5:
                ma_fast_calc = float(np.mean(list(self.price_buffer)[-5:]))
                ma_slow_calc = float(np.mean(list(self.price_buffer)[-20:])) if len(self.price_buffer) >= 20 else close_price
            else:
                ma_fast_calc = close_price
                ma_slow_calc = close_price
            
            if ea_boll_upper is not None and ea_boll_lower is not None:
                bb_upper_calc = ea_boll_upper
                bb_middle_calc = ea_boll_middle if ea_boll_middle is not None else (ea_boll_upper + ea_boll_lower) / 2.0
                bb_lower_calc = ea_boll_lower
                self.logger.info(f"[Transform] ? USING EA BOLLINGER: U={bb_upper_calc:.2f} M={bb_middle_calc:.2f} L={bb_lower_calc:.2f}")
            elif len(closes_array) >= 20:
            ...
            if ea_stoch_k is not None:
                ...
                self.logger.info(f"[Transform] ? USING EA STOCHASTIC: K={stoch_k_calc:.1f} D={stoch_d_calc:.1f}")
            else:
                stoch_k_calc = 50.0
                stoch_d_calc = 50.0
                stoch_period = 14
            ...
            transformed['indicators'] = {
                'current': {
                    ...
                    # Aliases used by LLMs (short names)
                    'bb_upper': bb_upper_calc,
                    'bb_middle': bb_middle_calc,
                    'bb_lower': bb_lower_calc,
    ...
    def process_genome(self, genome):
        ...
        try:
            # ---------------------------------------------------------------------------
            # ??? MASTER SAFETY CHECK: Require full system initialization before trading
            ...
            if not hasattr(self, '_trading_enabled') or not self._trading_enabled:
                ...
                if trinity_responses == 0 and kraken_acks == 0:
                    ...
                    if not hasattr(self, '_safety_warning_logged'):
                        self.logger.warning(f"[MASTER SAFETY] ?? Trading DISABLED - trinity/kraken1 not detected")
            ...
            symbol = genome.get('metadata', {}).get('symbol', 'US30')
            # FIX: Also get symbol from root level (US30 EA sends flat structure)
            if symbol == 'US30' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')
            # FIX A-US30-02: consecutive-loss halt check
            if time.time() < self.loss_halt_until:
                self.logger.warning(f"[LOSS-HALT] Trading halted for {self.loss_halt_until - time.time():.0f}s more ({self.consecutive_losses} consecutive losses)")
                return None
            # FIX A-US30-03: daily loss circuit breaker
            _today_pnl = self.ipc.stats.get('today_pnl', 0.0)
            _balance = self.ipc.stats.get('balance', 10000.0) or 10000.0
            if _today_pnl / _balance * 100 <= -Config.MAX_DAILY_LOSS_PERCENT:
                self.logger.warning(f"[DAILY-LOSS] Daily loss limit reached ({_today_pnl/_balance*100:.2f}% <= -{Config.MAX_DAILY_LOSS_PERCENT}%), halting")
                return None
            
            # Handle Quimera format (market_genome, tick_genome, genome, tick) + Multi-TF variants
            # ? Multi-TF EA sends: tick_genome_multi_tf, market_genome_multi_tf
            valid_genome_types = [
                'market_genome', 'tick_genome', 'genome', 'tick',
                'market_genome_multi_tf', 'tick_genome_multi_tf'  # ? Multi-TF types
            ]
            if genome_type in valid_genome_types:
                ...
                # FIX: US30 EA sends bid/ask at root level, not in tick_data
                tick_data = genome.get('tick_data', {})
                bid = tick_data.get('bid') if 'bid' in tick_data else genome.get('bid', 0)
                ask = tick_data.get('ask') if 'ask' in tick_data else genome.get('ask', 0)
                if bid > 0 and ask > 0:
                    mid_price = (bid + ask) / 2.0
            ...
            # FIX M-US30-01: Use singleton self.nova_detector instead of per-call instantiation
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                self.logger.debug(f"[Trinity-Prep] ? Copied bid={genome['bid']:.2f} ask={genome['ask']:.2f} from tick_data")
            else:
                # Calculate bid/ask from current price with typical US30 spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 2.5  # Half spread for US30 (typical spread ~5 points)
                    genome['bid'] = current_price - spread
                    genome['ask'] = current_price + spread
                    self.logger.debug(f"[Trinity-Prep] ? Calculated bid={genome['bid']:.2f} ask={genome['ask']:.2f} from price={current_price:.2f}")
                else:
            ...
            self.logger.info(f"[Trinity-Send] ?? Sending to Trinity with bid={final_bid:.2f} ask={final_ask:.2f}")
            ...
            if not trinity_resp:
                ...
                self.logger.warning(f"[Process] ?? Trinity offline #{trinity_offline_count} - using FALLBACK for {symbol}")
                
                # If Trinity has been offline for a long time, show urgent warning
                if trinity_offline_count > 5:
                    self.logger.error(f"[Process] ?? Trinity appears to be OFFLINE")
                    self.logger.error(f"[Process] ?? Make sure to start: python trinity.py")
                    self.logger.error(f"[Process] ?? Also check: python kraken1.py")
            ...
            # FIX C-US30-01: ATR=0 protection — abort after 5 consecutive zero-ATR genomes
            if atr_val == 0.0:
                with self.ipc.stats_lock:
                    self._atr_zero_count = getattr(self, '_atr_zero_count', 0) + 1
                if self._atr_zero_count >= 5:
                    self.logger.warning(f"[ATR-ZERO] {self._atr_zero_count} consecutive ATR=0 readings — skipping genome")
                    return None
            else:
                self._atr_zero_count = 0
            ...
            # Cuando RSI es EXTREMO (>90 o <10), el sistema DEBE tomar una decisi�n clara
            # No importa lo que digan los LLMs - las matem�ticas son claras
            # ---------------------------------------------------------------------------
            ...
            if rsi_val > 90 and stoch_k_val > 95:
                if trinity_decision != 'SELL':
                    ...
                    trinity_conf = max(75, trinity_conf)  # M�nimo 75% confianza
            ...
            # ---------------------------------------------------------------------------
            # ??? MASTER SAFETY: Block trades if trading not enabled
            # ---------------------------------------------------------------------------
            ...
            _did_increment = False  # FIX C-US30-02: guard against double-rollback
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-US30-05: direction poison check before stacking
                            _poison_key = f'dir_poison_until_{decision["action"]}'
                            if time.time() < self.ipc.stats.get(_poison_key, 0):
                                logger.warning(f"[DIR-POISON] {decision['action']} poisoned for {self.ipc.stats[_poison_key]-time.time():.0f}s more — BLOCKED stacking")
                                decision_for_kraken = decision.copy()
                                decision_for_kraken['action'] = 'HOLD'
                                decision_for_kraken['reason'] = f'direction_poisoned_{decision["action"]}'
                            else:
                                ...
                                _did_increment = True  # FIX C-US30-02
                        ...
                        # FIX A-US30-05: direction poison check before first trade
                        _poison_key_ft = f'dir_poison_until_{decision["action"]}'
                        if time.time() < self.ipc.stats.get(_poison_key_ft, 0):
                            logger.warning(f"[DIR-POISON] {decision['action']} poisoned for {self.ipc.stats[_poison_key_ft]-time.time():.0f}s more — BLOCKED first trade")
                            decision_for_kraken = decision.copy()
                            decision_for_kraken['action'] = 'HOLD'
                            decision_for_kraken['reason'] = f'direction_poisoned_{decision["action"]}'
                        else:
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    # FIX A-US30-01: atomic timestamp — single time.time() call under lock
                    _now = time.time()
                    with self.ipc.stats_lock:
                    ...
                    try:
                        calibration_log = self._create_calibration_log(decision_for_kraken, genome)
                        self._save_calibration_log(calibration_log)
                    except Exception as cal_err:
                        self.logger.debug(f"[CALIBRATION] Log error: {cal_err}")
                    
                    # ?? ML FEEDBACK - Register trade for learning (CRITICAL!)
                    try:
                        # [NOVA LEARNING ARCHITECT - FIX CRÍTICO] Use rl_engine (not rl_agent)
                        state_key = self.engine.rl_engine.discretize_state(
                            price=decision_for_kraken['entry_price'],
                            volatility=genome.get('atr', 10.0),
                            momentum=genome.get('momentum', 0),
                            trend_strength=genome.get('trend', 0)
                        )
                        
                        # [NOVA LEARNING ARCHITECT - FIX CRÍTICO] Use action_price as trade_id
                    ...
                    # FIX C-US30-02: Rollback ONLY if we incremented (guard double-rollback)
                    if _did_increment:
                        with self.ipc.stats_lock:
    ...
    def run(self):
        ...
        start_time = time.time()
        self.last_position_reconcile = time.time()  # FIX A-US30-04: stale position reconcile timer - ULTRA FAST refresh for real-time visual updates (same as EURUSD)
        ...
        try:
            while self.running:
                ...
                # -----------------------------------------------------------
                # ??? DIAGNOSTIC: Check if Trinity is responding
                # -----------------------------------------------------------
                last_trinity_response = self.ipc.stats.get('last_trinity_response', 0)
                time_since_trinity = time.time() - last_trinity_response
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)  # 🔧 FIX CAT-98
                
                if trinity_offline and not trinity_offline_warned:
                    logger.error(f"[CRITICAL] ?? TRINITY OFFLINE for {time_since_trinity:.1f}s - No LLM responses")
                    logger.error(f"[CRITICAL] ?? Start: python trinity.py")
                ...
                # ?? QUEUE DRAIN: Only process LATEST genome when queue floods
                # EA sends ~4-14 genomes/sec but processing takes longer.
                # Skip stale genomes, count them, only PROCESS the freshest.
                # Dashboard price updates happen for ALL ticks (instant).
                if len(genomes) > 2:
                    ...
                    if skipped > 0:
                        logger.debug(f"[Queue] ?? Skipped {skipped} stale genomes, processing latest + {len(portfolio_syncs)} portfolio_sync")
                    ...
                    # Fast dashboard price update from ALL ticks (instant, no processing)
                    for g in genomes[:-1]:
                        if g.get('type') != 'portfolio_sync':
                            self._fast_dashboard_price_update(g)
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        # FIX M-US30-02: atomic write via temp file + os.replace()
                ...
                # FIX A-US30-04: reconcile stale open_positions every 60s
                # FIX RECONCILE v2 — 3 señales para detectar posiciones fantasma
                if time.time() - self.last_position_reconcile > 60:
                    with self.ipc.stats_lock:
                ...
                time.sleep(0.05)  # ⭐ 50ms sleep - balance entre CPU y responsiveness
    ...
    def _fast_dashboard_price_update(self, genome):
        """Ultra-fast dashboard price update from raw genome (no processing)
        
        Called for EVERY tick, even skipped ones, to keep dashboard price real-time.
        Only updates bid/ask/price/spread - indicators come from full processing.
        """
        try:
            # Get price from various genome formats
            tick_data = genome.get('tick_data', {})
            bid = float(tick_data.get('bid', 0)) if tick_data else float(genome.get('bid', 0))
            ask = float(tick_data.get('ask', 0)) if tick_data else float(genome.get('ask', 0))
            
            if bid <= 0 and ask <= 0:
                return  # No valid price
            
            last_price = float(tick_data.get('last', 0)) if tick_data else 0
            if last_price <= 0:
            ...
            spread = (ask - bid) if bid > 0 and ask > 0 else 0  # Points for US30 index
            ...
            # Update ONLY price fields (fast, no indicator processing)
            nova_dashboard.update(
                symbol='US30',
                bid=bid,
                ask=ask,
                ...
                quimera_connected=True
            )
        except Exception as e:
            pass  # Silent fail for fast updates
    ...
    def _update_dashboard_from_genome(self, genome):
        ...
        try:
            ...
            # Bridge sends spread as point difference for US30
            # US30: spread already in points (POINT=1.0), no conversion needed
            _raw_spread = float(tick_data.get('spread', (ask - bid) if bid and ask else 0.0))
            spread = _raw_spread  # US30: spread already in points (POINT=1.0)
    ...
    def _process_quantum_intelligence_layers(self, genome, trinity_response):
        ...
        try:
            ...
            self.reality_layers['surface'] = {
                ...
                'momentum_5m': self._calculate_momentum(genome, 2)
            }
    ...
    def _classify_market_state(self, genome, quantum_analysis):
        ...
        indicators = genome.get('indicators', {})
        momentum = self._calculate_momentum(genome, 2)
...
# ══════════════════════════════════════════════════════════════════════════════
# 📋 CERTIFICADO DE SALUD DEL CÓDIGO — us30quantum_core.py
# Par: US30USD (Dow Jones Industrial Average) | NOVA SNIPER SCALPING v18.00
# ══════════════════════════════════════════════════════════════════════════════
# Auditor    : GitHub Copilot (Claude Sonnet 4.6)
# Fecha      : 2026-03-01
# Campaña    : proyecto_quimera — QA sweep completo (Par 11/11, nuevo ingreso)
# Pares previos OK: XAU SEK JPY EUR GBP AUD CAD CHF CNH NZD
# ─────────────────────────────────────────────────────────────────────────────
#
# ┌─────────────────────────────────────────────────────────────────────────┐
# │  CONFIANZA GENERAL                                                      │
# │   ANTES de fixes  :  48 %   (par nuevo, sin hardening)                 │
# │   DESPUÉS de fixes:  91 %   (todos los fixes canónicos aplicados)      │
# └─────────────────────────────────────────────────────────────────────────┘
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# TABLA DE RIESGOS (antes de fixes)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
...
# │ BUG-ID         │ Descripción                                                   │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-C-01  │ NOVAMarketStateDetector instanciado en cada genome (M-US30-01)│
# │                │ → Alloca/destruye objeto en hot path, memoria fragmentada,    │
# │                │   logger no comparte estado; en picos de 14 genomes/s = fuga  │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-C-02  │ ATR=0 no abortaba (C-US30-01)                                │
# │                │ → US30 en pre-market/cierre puede enviar ATR=0.              │
# │                │   SL/TP calculados sobre 0 → stop a precio de entrada = cierre│
# │                │   inmediato en rojo                                           │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-C-03  │ Race condition doble-rollback (C-US30-02)                    │
# │                │ → Sin _did_increment, si Kraken falla en CLOSE decision,     │
# │                │   el rollback decrementa open_positions a -1 (negativo),     │
# │                │   corrompiendo el estado para siempre                         │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-C-04  │ Dos time.time() separados para timestamps (A-US30-01)        │
# │                │ → self.last_trade_time y engine.last_trade_time podían        │
# │                │   diferir hasta varios ms en threading → cooldown             │
# │                │   inconsistente entre ambos módulos                           │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-C-05  │ quantum_stats.json escritura no-atómica (M-US30-02)          │
# │                │ → El dashboard puede leer el archivo a mitad de write →      │
# │                │   JSON truncado → dashboard crash silencioso                  │
# └────────────────┴───────────────────────────────────────────────────────────────┘
#
# SEVERIDAD ALTA (Pérdidas potenciales en escenarios de stress)
# ┌────────────────┬───────────────────────────────────────────────────────────────┐
# │ BUG-US30-A-01  │ Sin consecutive-loss halt (A-US30-02)                        │
# │                │ → Racha de 5+ pérdidas consecutivas no detenía trading.      │
# │                │   US30 puede moverse 200+ puntos en noticias (NFP, FOMC);    │
# │                │   sin halt, el sistema puede abrir 3 posiciones extra en     │
# │                │   el mismo movimiento adverso                                 │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-A-02  │ Sin daily-loss circuit breaker (A-US30-03)                   │
# │                │ → MAX_DAILY_LOSS_PERCENT=2.0 definido pero nunca chequeado.  │
# │                │   En black-swan (e.g. circuit breaker del mercado), el        │
# │                │   sistema podía perder 5-10× el límite diario                 │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-A-03  │ Sin direction poison check (A-US30-05)                       │
# │                │ → Un par de loss rápido podía poisonear una dirección, pero  │
# │                │   el quantum_core la ignoraba y seguía apilando en esa dir.  │
# │                │   US30 es muy sensible a este patrón en reversiones de sesión │
# ├────────────────┼───────────────────────────────────────────────────────────────┤
# │ BUG-US30-A-04  │ Sin stale position reconciler (A-US30-04)                    │
# │                │ → Estado open_positions bloqueado en >0 indefinidamente si   │
# │                │   MT5 cierra una posición sin notificación (reconexión, etc.) │
# └────────────────┴───────────────────────────────────────────────────────────────┘
#
# SEVERIDAD MEDIA (Calidad / robustez)
# ┌────────────────┬───────────────────────────────────────────────────────────────┐
# │ BUG-US30-M-01  │ _build_closes_with_live_tick gap de 1.0 punto — umbral       │
# │                │ correcto para US30 (índice en puntos, no pips). ✅ OK          │
# │ BUG-US30-M-02  │ MAX_CONCURRENT_TRADES=3 vs comentario "was 4" — valor        │
# │                │ reducido confirma decisión deliberada. ✅ Intencional          │
# │ BUG-US30-M-03  │ spread fallback = 2.5 puntos en NOVA bid/ask calc —         │
# │                │ valor razonable para US30 (spread típico 1-3 pts). ✅ OK       │
# │ BUG-US30-M-04  │ Queue drain lógico en run() (procesar solo el último genome) │
# │                │ — patrón correcto para EA de alta frecuencia. ✅ OK           │
# └────────────────┴───────────────────────────────────────────────────────────────┘
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# FIXES CANÓNICOS APLICADOS (11 total)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
#  Config     │ MAX_CONSECUTIVE_LOSSES = 3, LOSS_HALT_DURATION_SECONDS = 300
#             │ insertados después de MAX_DAILY_LOSS_PERCENT = 2.0 en class Config
#
#  M-US30-01  │ NOVAMarketStateDetector singleton — self.nova_detector en __init__;
#             │ per-call instantiation reemplazada con self.nova_detector.analyze()
#
#  A-US30-02  │ Consecutive-loss halt: if time.time() < self.loss_halt_until → return None
#             │ Vars self.consecutive_losses / self.loss_halt_until añadidas a __init__
#
#  A-US30-03  │ Daily-loss circuit breaker: today_pnl/balance*100 <= -MAX_DAILY_LOSS_PERCENT
#             │ → return None al inicio de process_genome (post genome_type extract)
#
#  C-US30-01  │ ATR=0 protection: self._atr_zero_count (bajo stats_lock) aborta tras 5
#             │ lecturas consecutivas de ATR=0 (crítico en pre-market US30 09:30 ET)
#
#  C-US30-02  │ _did_increment = False antes del position lock; True solo en APPROVE.
#             │ Rollback gateado con if _did_increment: (evita doble-decremento)
#
#  A-US30-05  │ dir_poison_until_{BUY|SELL} chequeado en AMBAS ramas APPROVE:
#             │ stacking (open_positions >0) y first-trade (open_positions ==0)
#
#  A-US30-01  │ Timestamp atómico: _now = time.time() + stats_lock block sincroniza
#             │ self.last_trade_time y self.engine.last_trade_time con el mismo valor
#
#  A-US30-04  │ Reconciler de posiciones en run(): cada 60s, si open_positions > 0
#             │ y last_trade_time > 600s → reset positions/direction
#             │ self.last_position_reconcile añadido a __init__ y run() init
#
#  M-US30-02  │ Escritura atómica: quantum_stats.json.tmp + os.replace() reemplaza
#             │ el with open('quantum_stats.json','w') directo
#
# CLEANUPS:
#  • Eliminadas 2 líneas duplicadas del bloque first-trade (logger.info + decision=decision)
#    que quedaron como remainder tras el reemplazo de posición
#
# NOTAS ESPECÍFICAS US30:
#  • gap > 1.0 en _build_closes_with_live_tick es correcto (US30 es índice en puntos)
#  • spread fallback = 2.5 pts en NOVA bid/ask es correcto (spread típico 1-3 pts)
#  • MAX_CONCURRENT_TRADES=3 (deliberadamente reducido desde 4) — preservado
#  • Queue drain del run() (solo procesar el último genome en flood) — preservado
#  • daily_trade_count NO existe en este par → no se aplica C-US30-03
#  • last_attack_direction tracking ya existía en el bloque Kraken-success → preservado
#
# SYNTAX CHECK : python -m py_compile us30quantum_core.py → SYNTAX OK ✅
#
# ┌─────────────────────────────────────┐
