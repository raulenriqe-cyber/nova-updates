"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - US30 Dow Jones Market Sentiment                  ║
...
║  Optimizado para US30 - Dow Jones Industrial Average                         ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-US30")
...
# Símbolos a monitorear - US30 FOCUSED
SYMBOLS = ["US30", "SPX500", "NAS100"]

# Fuentes confiables para US indices
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "cnbc", "marketwatch",
    "investing.com", "tradingview", "wsj.com", "barrons.com",
    "finance.yahoo.com", "seekingalpha.com"
]

# Keywords por símbolo - US30 OPTIMIZED
KEYWORDS = {
    "US30": [
        "US30", "Dow Jones", "DJIA", "Dow 30", "Wall Street",
        "dow jones news", "dow forecast", "US stock market",
        "blue chip stocks", "fed decision", "federal reserve",
        "fomc", "powell fed", "nonfarm payrolls", "NFP",
        "US CPI", "US inflation", "US GDP", "US employment",
        "US retail sales", "wall street rally", "wall street crash",
        "dow rally", "dow crash", "US interest rate",
        "fed rate decision", "bond yields", "VIX",
        "S&P 500", "nasdaq", "new york session",
        "earnings season", "corporate earnings"
    ],
    "SPX500": [
        "S&P 500", "SPX", "SP500", "S&P forecast",
        "US equities", "US stock market"
    ],
    "NAS100": [
        "NASDAQ", "NAS100", "tech stocks", "tech rally",
        "semiconductor", "nasdaq forecast"
    ],
    "EURUSD": [
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        symbol = news_item.get('symbol', 'US30')
...
class NewsStorage:
    ...
    def _get_today_file(self) -> Path:
        ...
        return self.news_dir / f"news_us30_{today}.json"
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        us30_score = abs(sentiment.get('US30', {}).get('score', 0))
        us30_total = sentiment.get('US30', {}).get('total', 0)
        us30_conf = min(1.0, us30_score * (1 + min(us30_total, 10) * 0.1))
        
        return {
            'US30': sentiment.get('US30', {}).get('sentiment', 'N'),
            'SPX500': sentiment.get('SPX500', {}).get('sentiment', 'N'),
            'NAS100': sentiment.get('NAS100', {}).get('sentiment', 'N'),
            'summary': ' | '.join(summaries),
            'confidence': us30_conf,
            'confidence_us30': us30_conf,
            'raw': sentiment
        }
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para US30.
    ...
    def fetch_and_evaluate(self) -> Dict:
        ...
        """
        logger.info("=" * 60)
        logger.info("📰 US30 NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        ...
        if symbol == 'US30':
            conf = sentiment.get('confidence_us30', sentiment.get('confidence', 0.5))
        else:
...
def main():
    ...
    print("=" * 70)
    print("📰 NEWS INTELLIGENCE ENGINE - US30")
    ...
    print("=" * 70)
    print(f"   US30:   {sentiment.get('US30', 'N')}")
    print(f"   SPX500: {sentiment.get('SPX500', 'N')}")
    print(f"   NAS100: {sentiment.get('NAS100', 'N')}")
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('US30')
