...
class TrinityConsensus:
    def __init__(self):
        log.info('[TRINITY] BANK-GRADE CONSENSUS v3.0 Initialized - US30')
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD) - US30 PORTS (89xx)
        self.core_llm_ports = {
            'BAYESIAN': 8955,     # US30 LLM1
            'TECHNICAL': 8957,    # US30 LLM2
            'CHART': 8958,        # US30 LLM3
            'RISK': 8959,         # US30 LLM4
            'SUPREME': 8961,      # US30 LLM5
            'SMART_MONEY': 8962   # LLM6 - Smart Money Oracle (US30)
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution) - US30 PORTS
        self.enhancement_llm_ports = {
            'OCULUS': 8963,    # US30 LLM7 - Data Quality Validator
            'CHRONOS': 8964,   # US30 LLM8 - Timing Optimizer  
            'PREDATOR': 8965,  # US30 LLM9 - Execution Engine
            'NOVA': 8960,   # US30 LLM10 - Guardian Protector
            'STRATEGIST': 8967,   # US30USD LLM11 - Strategist Guru
            'SENTINEL': 8968      # US30USD LLM12 - Quality Guardian
        }
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 8963, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 8964, genome)
        if chronos_response:
        ...
        predator_response = self._query_enhancement_llm('PREDATOR', 8965, enhanced_genome)
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] ?? Make sure all 6 LLMs are running on ports 8955, 8957, 8958, 8959, 8961, 8962")
        ...
        if llm_decision == tf_decision and llm_decision in ['BUY', 'SELL']:
            ...
            # ?? CONFLICT: LLMs want to trade, Timeframes don't agree
            # Only trade if LLM confidence is very high (75%+) AND timeframe not opposite
            if llm_confidence >= 75 and tf_decision != ('SELL' if llm_decision == 'BUY' else 'BUY'):
                final_decision = llm_decision
                final_confidence = int(llm_confidence * 0.8)  # Reduce confidence due to TF uncertainty
                log.warning(f"[TRINITY] ?? TRADING WITH CAUTION: LLM strong ({llm_confidence}%) but TF uncertain ({consensus_result['reason']})")
            else:
                # Safety: HOLD when there's conflict
                final_decision = 'HOLD'
                final_confidence = max(llm_confidence, tf_confidence)
                log.warning(f"[TRINITY] ??? CONFLICT HOLD: LLM {llm_decision}/{llm_confidence}% vs TF {tf_decision}/{tf_confidence}% - waiting for clarity")
        else:
        ...
        # ? RELAXED FOR US30: Less aggressive penalties, more boosts
        # -------------------------------------------------------------------
        ...
        # ?? RELAXED FOR US30: Bajar thresholds para permitir más trades
        # US30 scalping tiene R:R más bajo naturalmente
        if rr_ratio > 0:
            if rr_ratio < 0.35:
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # For US30 index: detect tick changes of at least 1 point
                # This catches micro-movements that were previously ignored
                if len(closes) > 0:
                    price_diff = abs(closes[-1] - price)
                    if price_diff > 1.0:  # More than 1 point difference (US30)
    ...
    def _aggregate_votes(self, llm_responses: dict) -> tuple:
        ...
        # Decision based on highest score - 🔧 FIXED FOR US30
        if buy_score > sell_score and buy_score > hold_score:
            ...
            # 🔥 INTELLIGENT THRESHOLD: Dynamic min_confidence (was static 40)
            ...
            if confidence < min_confidence:
                decision = 'HOLD'
                confidence = 45
                log.info(f"[Trinity] ⚠️ BUY→HOLD: conf={confidence} < min={min_confidence}")
            ...
            # 🔥 INTELLIGENT THRESHOLD: Dynamic (was static 40)
            if score_ratio >= 2.0 and vote_count >= 2:
                min_confidence = 38
            elif vote_count >= 4 and divergence <= 0.3:
                min_confidence = 42
            ...
            if confidence < min_confidence:
                decision = 'HOLD'
                confidence = 45
                log.info(f"[Trinity] ⚠️ SELL→HOLD: conf={confidence} < min={min_confidence}")
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            try:
                ...
                log.error(f"[NOVA CONNECTION] ? No se pudo conectar a puerto 8965")
                return self._fallback_nova_response()
    ...
    def _validate_audit_context(self, context: dict) -> dict:
        ...
        # FIX: bid == ask means spread=0, which is normal for some brokers/pairs
        # Only flag as corrupted if bid > ask (inverted spread = truly corrupted)
        if bid > 0 and ask > 0:
            if bid > ask:
                errors.append(f"Price corrupted: bid {bid} > ask {ask}")
            elif bid == ask:
                log.info(f"[NOVA VALIDATE] Spread=0 (bid==ask={bid}), normal for some brokers - NOT blocking")
        else:
...
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                self.dashboard = QuantumDashboard(port=8967)  # US30 dashboard port
    ...
    def run(self, port=8966):
        """Start TCP server on specified port (US30 Trinity on 8966)"""
...
def run_server(port=8966):
    """Run Trinity consensus server with optional dashboard - US30"""
...
if __name__ == '__main__':
    try:
        log.info('[TRINITY] -----------------------------------------')
        log.info('[TRINITY] TRINITY CONSENSUS ENGINE v1.0 - US30')
        log.info('[TRINITY] Consensus Orchestrator - 10 LLM Integration')
        log.info('[TRINITY] -----------------------------------------')
        run_server(port=8966)  # US30 Trinity port
