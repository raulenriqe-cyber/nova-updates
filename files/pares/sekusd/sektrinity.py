...
# Add parent directory to path for trinity_dashboard_v3 import
...
class LLMPerformanceTracker:
    ...
    def __init__(self, window_size=100):
        ...
        # FIX v7: Aligned with quantum_core LLM_WEIGHTS for USDSEK exotic
        # SMART_MONEY downgraded: SM doesn't understand exotic pairs well
        # TECHNICAL/SUPREME/CHRONOS boosted: SEK trends strongly when it moves
        self.base_weights = {
            'BAYESIAN': 1.0,    # Standard probabilistic
            'TECHNICAL': 1.4,   # ⭐ Key for SEK trends (matches quantum_core)
            'CHART': 1.2,       # Pattern recognition
            'RISK': 0.9,        # Conservative
            'SUPREME': 1.3,     # Deep learning ensemble
            'SMART_MONEY': 0.3,  # ⚠️ FIX: Was 1.4 (highest!) but SM doesn't understand exotics
...
class TrinityConsensus:
    def __init__(self):
        log.info('[TRINITY] BANK-GRADE CONSENSUS v3.0 Initialized - USDSEK')
        ...
        # PHASE 1: Core voting LLMs (decide BUY/SELL/HOLD) - USDSEK PORTS (75xx)
        self.core_llm_ports = {
            'BAYESIAN': 8655,     # USDSEK LLM1
            'TECHNICAL': 8657,    # USDSEK LLM2
            'CHART': 8658,        # USDSEK LLM3
            'RISK': 8659,         # USDSEK LLM4
            'SUPREME': 8661,      # USDSEK LLM5
            'SMART_MONEY': 8662   # LLM6 - Smart Money Oracle (USDSEK)
        }
        
        # PHASE 2: Intelligence enhancement LLMs (quality, timing, execution) - USDSEK PORTS
        self.enhancement_llm_ports = {
            'OCULUS': 8663,    # USDSEK LLM7 - Data Quality Validator
            'CHRONOS': 8664,   # USDSEK LLM8 - Timing Optimizer  
            'PREDATOR': 8665,  # USDSEK LLM9 - Execution Engine
            'NOVA': 8660,   # USDSEK LLM10 - Guardian Protector
            'STRATEGIST': 8667,   # SEKUSD LLM11 - Strategist Guru
            'SENTINEL': 8668      # SEKUSD LLM12 - Quality Guardian
        }
    ...
    def _validate_ecosystem_data(self, genome: dict) -> dict:
        ...
        # 🔧 FIX: Velocity thresholds were TOO SENSITIVE for USDSEK M1
        # Old: ma_delta>0.5, rsi_delta>5, adx_delta>2 → fired constantly → penalty*0.7 killed ALL LLM confidences
        # This was a ROOT CAUSE of SEK never attacking (70% conf * 0.7 = 49% → below 60% min → forced HOLD)
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        if 'nova_quality_score' in genome:
            ...
            oculus_response = self._query_enhancement_llm('OCULUS', 8663, genome)
        ...
        chronos_response = self._query_enhancement_llm('CHRONOS', 8664, genome)
        if chronos_response:
        ...
        predator_response = self._query_enhancement_llm('PREDATOR', 8665, enhanced_genome)
        rr_ratio = 0.0  # 🔧 FIX CAT-106: initialize before conditional to prevent UnboundLocalError
        ...
        if core_responses:
            ...
            log.error(f"[TRINITY] ?? Make sure all 6 LLMs are running on ports 8655, 8657, 8658, 8659, 8661, 8662")
        ...
        if llm_decision == tf_decision and llm_decision in ['BUY', 'SELL']:
            ...
            # 🔧 PHASE 10 FIX: Trade if LLM confidence >= 62% OR if TF has no data (0% confidence)
        ...
        # This was a ROOT CAUSE of SEK never attacking
        # ═══════════════════════════════════════════════════════════════════
        ...
        return {
            ...
            'tp': float(predator_response.get('take_profit', 0) or 0) if predator_response else 0,
            'sl': float(predator_response.get('stop_loss', 0) or 0) if predator_response else 0,
            'entry_price': float(predator_response.get('entry_price', 0) or 0) if predator_response else 0,
    ...
    def _query_llm(self, llm_name: str, port: int, genome: dict, responses: dict, lock: threading.Lock):
        try:
            ...
            if price > 0:
                ...
                # Changed from > 0.01 to > 0.001 (0.1 pip for USDSEK)
    ...
    def _query_llm10_nova(self, genome: dict, trinity_decision: dict) -> dict:
        ...
        try:
            ...
            # 3️ CONECTAR CON LLM10 VIA TCP (LLM10 runs a TCP server, NOT HTTP)
            start_time = time.time()
            
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(2.0)  # 2 segundos max para LLM10
                sock.connect((self.host, 8660))
                
                # Send: 4-byte header + JSON (same protocol as other LLMs)
                ...
                # 4️⃣ VALIDACIÓN POST-RECEPCIÓN (Hueco #10)
                post_flight = self._validate_nova_response(nova_response)
                if not post_flight['valid']:
                    log.warning(f"[NOVA POST-FLIGHT] ⚠️ {post_flight['reason']}")
                    return self._fallback_nova_response()
                ...
                log.warning(f"[NOVA TIMEOUT] LLM10 no respondió en 2s")
                return self._fallback_nova_response()
            
            except ConnectionRefusedError:
                log.error(f"[NOVA CONNECTION] 🚫 No se pudo conectar a puerto 8660")
                return self._fallback_nova_response()
...
class TrinityServer:
    ...
    def __init__(self, dashboard_enabled=True):
        ...
        if dashboard_enabled:
            try:
                ...
                self.dashboard = QuantumDashboard(port=7567)  # USDSEK dashboard port
    ...
    def run(self, port=8666):
        """Start TCP server on specified port (USDSEK Trinity on 8666)"""
...
def run_server(port=8666):
    """Run Trinity consensus server with optional dashboard - USDSEK"""
...
if __name__ == '__main__':
    try:
        log.info('[TRINITY] -----------------------------------------')
        log.info('[TRINITY] TRINITY CONSENSUS ENGINE v1.0 - USDSEK')
        log.info('[TRINITY] Consensus Orchestrator - 10 LLM Integration')
        log.info('[TRINITY] -----------------------------------------')
        run_server(port=8666)  # USDSEK Trinity port
