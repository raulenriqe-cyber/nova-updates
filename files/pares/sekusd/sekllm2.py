...
║  Port: 8657 (TCP Server) - NEVER FAILS, ALWAYS RESPONDS                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class MonteCarloSimulator:
    ...
    def simulate_price_paths(current_price, atr, periods=10, simulations=500, recent_prices=None):
        ...
        # Derive drift from recent returns (momentum)
        drift = 0
        if recent_prices is not None and len(recent_prices) >= 2:
            returns_hist = [(recent_prices[i] - recent_prices[i-1]) / recent_prices[i-1]
                           for i in range(1, len(recent_prices)) if recent_prices[i-1] > 0]
            if returns_hist:
                drift = float(np.mean(returns_hist))
...
class LLM2TechnicalExpert:
    ...
    def __init__(self):
        ...
        self.memory = {'USDSEK': deque(maxlen=300)}
    ...
    def analyze(self, genome):
        ...
        try:
            ...
            # ============================================================
            mc_sim = MonteCarloSimulator.simulate_price_paths(price, atr, periods=10, simulations=500, recent_prices=prices_list)
            ...
            # Consolidation check - M1 USDSEK: ATR very low = reduced confidence
            ...
            # ═════════════════════════════════════════════════════════════
            # 🚫 REJECT LOW-QUALITY SETUPS - PHASE 10: Relaxed thresholds
            # ═════════════════════════════════════════════════════════════
            else:
                # PHASE 10: If there are ANY signals, produce a weak directional vote
...
def start_server():
    """Inicia servidor TCP en puerto 8657"""
    try:
        config_file = 'sekconfig.yaml'
        try:
        ...
        port = config.get('llm_settings', {}).get('llm2_port', 8657)
