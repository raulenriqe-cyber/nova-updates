...
class InstitutionalTimeEngine:
    ...
    def __init__(self):
        ...
        self.kill_zones = {
            ...
            'LONDON_OPEN': (7, 9),       # London session start (máxima liquidez GBP)
            'LONDON_CLOSE': (15, 17),    # London close
            'NY_OPEN': (12, 14),         # New York open (overlap with London)
    ...
    def get_time_quality(self, utc_hour: int = None, utc_minute: int = None, 
        ...
        for zone_name, (start_h, end_h) in self.kill_zones.items():
            if start_h <= utc_hour < end_h:
                # London Open es el MEJOR para GBP
                if zone_name == 'LONDON_OPEN':
                    quality = 1.0
                    reason = f"🎯 {zone_name} - MAXIMUM institutional activity"
                elif zone_name == 'NY_OPEN':
                    quality = 0.95
                    reason = f"🎯 {zone_name} - High liquidity overlap"
                else:
...
class QuantumStateMachine:
    ...
    def analyze_market_state(self, genome: Dict, llm_votes: List[LLMVote],
        ...
        # Extraer datos - PHASE 10 FIX: Also check root-level arrays (transform puts data there)
        price_data = genome.get('price_data', {})
        closes = list(price_data.get('close', []))[-30:]
        if not closes:
            closes = list(genome.get('closes', []))[-30:]
        volumes = list(price_data.get('volume', []))[-30:]
        if not volumes:
            volumes = list(genome.get('volumes', []))[-30:]
...
class AntiNoiseFilter:
    ...
    def calculate_noise_level(self, genome: Dict, llm_votes: List[LLMVote]) -> float:
        ...
        """
        # PHASE 10 FIX: Also check root-level arrays (transform puts closes/highs/lows there)
        price_data = genome.get('price_data', {})
        closes = list(price_data.get('close', []))[-20:]
        if not closes:
            closes = list(genome.get('closes', []))[-20:]
        highs = list(price_data.get('high', []))[-20:]
        if not highs:
            highs = list(genome.get('highs', []))[-20:]
        lows = list(price_data.get('low', []))[-20:]
        if not lows:
            lows = list(genome.get('lows', []))[-20:]
...
class SupremeConsciousnessEngine:
    ...
    def _detect_institutional_activity(self, genome: Dict) -> InstitutionalActivity:
        ...
        if recent_vol > avg_vol * 2 and abs(price_change) > 0.003:
            return InstitutionalActivity.ATTACKING
        
        # Volumen bajo pero precio sube = acumulación silenciosa
        if recent_vol < avg_vol * 0.7 and price_change > 0.001:
...
if __name__ == "__main__":
    ...
    test_genome = {
        'price_data': {
            'close': [1.2500 + i * 0.0001 for i in range(30)],
            'high': [1.2505 + i * 0.0001 for i in range(30)],
            'low': [1.2495 + i * 0.0001 for i in range(30)],
