...
║  Port: 8665 (TCP Server) | Pure Python + Optional Ollama | <100ms latency   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class LLM9:
    ...
    def __init__(self):
        self.port = 8665  # USDSEK LLM9 PREDATOR
        ...
        log.info(f"[LLM9] Port: 8665 | Hybrid: Deterministic + Ollama Context")
        ...
        try:
            ...
            if consensus_confidence >= 65 and consensus_decision in ['BUY', 'SELL']:
                if quality_score >= 50 and rr_ratio >= 1.5:
                    ...
                    log.info(f"[LLM9] ✅ {decision} APPROVED")
                else:
                    decision = 'HOLD'
                    execution_status = 'READY'
                    reason = f"Conf OK but quality or R:R weak - ready if needed"
                    log.info(f"[LLM9] ⏳ HOLD (conditions sub-optimal)")
            else:
                decision = 'HOLD'
