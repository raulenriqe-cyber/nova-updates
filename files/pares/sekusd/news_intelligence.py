"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NEWS INTELLIGENCE ENGINE - USDSEK Market Sentiment                         ║
...
║  Optimizado para USDSEK - US Dollar / Swedish Krona                          ║
║  DIRECTION MAP: B=Bullish SEK → BEARISH USDSEK (price DOWN)                 ║
║                 M=Bearish SEK → BULLISH USDSEK (price UP)                    ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
logger = logging.getLogger("NewsIntelligence-SEK")
...
# Símbolos a monitorear - USDSEK FOCUSED
SYMBOLS = ["USDSEK", "EURSEK", "XAUUSD"]

# Fuentes confiables para forex y Nordic markets
TRUSTED_SOURCES = [
    "reuters", "bloomberg", "forexlive", "fxstreet", "dailyfx",
    "investing.com", "tradingview", "marketwatch", "cnbc",
    "financialjuice", "forexfactory", "ft.com",
    "riksbank.se", "di.se", "svd.se", "dn.se",
    "scb.se", "nordea.com", "seb.se", "handelsbanken.se",
    "swedbank.se", "economibygget.se"
]

# Keywords por símbolo - USDSEK OPTIMIZED
KEYWORDS = {
    "USDSEK": [
        "USDSEK", "dollar krona", "USD/SEK", "swedish krona news",
        "riksbank decision", "riksbank rate", "krona forecast",
        "swedish krona", "fed krona", "krona strength",
        "krona trading", "krona weakness", "sweden economy",
        "sweden inflation", "sweden employment", "sweden gdp",
        "sweden retail sales", "swedish economy", "sweden interest rate",
        "riksbank rate decision", "thedeen riksbank", "sweden monetary policy",
        "swedish bond yields", "omx stockholm", "nordic economy",
        "krona rally", "krona crash", "scandinavian forex",
        "sweden housing", "sweden export", "nordic currencies"
    ],
    "EURSEK": [
        "EURSEK", "euro krona", "EUR/SEK", "euro sweden",
        "ECB riksbank", "european central bank sweden",
        "euro nordic", "scandinavian currency", "nordic forex",
        "euro scandinavian", "nordic central banks"
    ],
    "XAUUSD": [
        "gold price", "XAUUSD", "gold forecast", "gold trading",
        "gold safe haven", "gold fed", "gold dollar",
        "gold rally", "gold crash", "precious metals"
    ]
}
...
class OllamaNewsEvaluator:
    ...
    def evaluate_news(self, news_item: Dict) -> Dict:
        ...
        For USDSEK: B = bullish SEK (USDSEK DOWN), M = bearish SEK (USDSEK UP)
        """
        title = news_item.get('title', '')
        symbol = news_item.get('symbol', 'USDSEK')
...
B = Bullish (price likely to go UP)
M = Bearish (price likely to go DOWN)  
...
class NewsStorage:
    ...
    def _get_today_file(self) -> Path:
        ...
        return self.news_dir / f"news_sek_{today}.json"
    ...
    def get_sentiment_for_llm(self) -> Dict:
        ...
        sek_score = abs(sentiment.get('USDSEK', {}).get('score', 0))
        sek_total = sentiment.get('USDSEK', {}).get('total', 0)
        sek_conf = min(1.0, sek_score * (1 + min(sek_total, 10) * 0.1))
        
        return {
            'USDSEK': sentiment.get('USDSEK', {}).get('sentiment', 'N'),
            'EURSEK': sentiment.get('EURSEK', {}).get('sentiment', 'N'),
            'XAUUSD': sentiment.get('XAUUSD', {}).get('sentiment', 'N'),
            'summary': ' | '.join(summaries),
            'confidence': sek_conf,
            'confidence_usdsek': sek_conf,
            'raw': sentiment
        }
...
class NewsIntelligenceEngine:
    """
    Motor principal de inteligencia de noticias para USDSEK.
    Orquesta: Fetch → Evaluate → Store → Provide to LLM
    
    CRITICAL DIRECTION MAP FOR USDSEK (reverse pair):
    - B = Bullish SEK news → SEK strengthens → USDSEK price goes DOWN → BEARISH trade
    - M = Bearish SEK news → SEK weakens → USDSEK price goes UP → BULLISH trade
    """
    ...
    def fetch_and_evaluate(self) -> Dict:
        ...
        """
        logger.info("=" * 60)
        logger.info("📰 USDSEK NEWS INTELLIGENCE - Starting fetch cycle...")
        logger.info("=" * 60)
    ...
    def get_trading_bias(self, symbol: str) -> Tuple[str, float]:
        """
        Retorna el sesgo de trading para USDSEK.
        
        ⚠️ CRITICAL DIRECTION MAP (USDSEK is a REVERSE pair):
        ═══════════════════════════════════════════════════════════
        SEK News     | SEK Effect      | USDSEK Price | Trade Direction
        ─────────────┼─────────────────┼──────────────┼────────────────
        B (Bullish)  | SEK strengthens | Goes DOWN ↓  | BEARISH (SELL)
        M (Bearish)  | SEK weakens     | Goes UP ↑    | BULLISH (BUY)
        N (Neutral)  | No change       | Flat →       | NEUTRAL
        ═══════════════════════════════════════════════════════════
        
        Composite score: 60% USDSEK direct + 25% gold + 15% EURSEK correlation
        Gold UP typically = USD weakness = SEK strength = USDSEK DOWN
        """
        sentiment = self.storage.get_sentiment_for_llm()
        
        if symbol == 'USDSEK':
            sek_sent = sentiment.get('USDSEK', 'N')
            sek_conf = sentiment.get('confidence_usdsek', sentiment.get('confidence', 0.5))
            
            # Gold correlation: Gold UP = USD weak = SEK strong = USDSEK DOWN
            gold_sent = sentiment.get('XAUUSD', 'N')
            gold_score = 0
            if gold_sent == 'B':
                gold_score = -0.3  # Gold up → bearish USDSEK
            elif gold_sent == 'M':
                gold_score = 0.3   # Gold down → bullish USDSEK
            
            # EURSEK correlation: EURSEK UP = SEK weak = USDSEK UP (same direction)
            eursek_sent = sentiment.get('EURSEK', 'N')
            eursek_score = 0
            if eursek_sent == 'B':
                eursek_score = 0.15  # EURSEK up (SEK weak) → bullish USDSEK
            elif eursek_sent == 'M':
                eursek_score = -0.15  # EURSEK down (SEK strong) → bearish USDSEK
            
            # Composite: USDSEK sentiment B/M is already from USDSEK perspective
            # B = USDSEK bullish (up), M = USDSEK bearish (down)
            sek_score = 0
            if sek_sent == 'B':
                sek_score = 0.5  # News bullish for USDSEK pair directly
            elif sek_sent == 'M':
                sek_score = -0.5  # News bearish for USDSEK pair directly
            
            # Composite: 60% USDSEK direct + 25% gold + 15% EURSEK
            composite = sek_score * 0.60 + gold_score * 0.25 + eursek_score * 0.15
            
            if composite > 0.1:
                return ('BULLISH', max(0.4, min(0.9, abs(composite) + sek_conf * 0.3)))
            elif composite < -0.1:
                return ('BEARISH', max(0.4, min(0.9, abs(composite) + sek_conf * 0.3)))
            else:
                return ('NEUTRAL', 0.2)
        else:
...
def main():
    ...
    print("=" * 70)
    print("📰 NEWS INTELLIGENCE ENGINE - USDSEK")
    ...
    print("=" * 70)
    print(f"   USDSEK: {sentiment.get('USDSEK', 'N')}")
    print(f"   EURSEK: {sentiment.get('EURSEK', 'N')}")
...
def get_news_sentiment() -> Dict:
    ...
    Usage:
        ...
        if sentiment['USDSEK'] == 'B':
            # Bias bullish para USDSEK
    """
    global _news_engine
...
def get_news_score():
    """Wrapper compatible con quantum_core imports"""
    sentiment = get_news_sentiment()
    return sentiment
...
def get_trading_bias(symbol: str) -> Tuple[str, float]:
    ...
    Usage:
        ...
        bias, confidence = get_trading_bias('USDSEK')
        # bias = 'BULLISH', 'BEARISH', or 'NEUTRAL'
        
    USDSEK Direction:
        B news (Bullish SEK) → BEARISH for USDSEK (price goes DOWN)
        M news (Bearish SEK) → BULLISH for USDSEK (price goes UP)
    """
    global _news_engine
