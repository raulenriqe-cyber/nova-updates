...
class RealQLearningAgent:
    ...
    def __init__(self, learning_rate=0.15, discount_factor=0.95, epsilon_start=0.3, epsilon_decay=0.995, strategy_tag='default'):
        """
        ...
        Args:
            ...
            strategy_tag: Active strategy name for per-strategy pkl isolation (Mejora #1)
        """
    ...
    def get_action(self, state, use_exploration=True):
        ...
        with self.lock:
            ...
            if all(v == 0 for v in q_values.values()):
                logger.debug(f"[RL] Untrained state {state}, defaulting to HOLD")
                return 'HOLD'
    ...
    def learn_from_trade(self, entry_state, action, exit_price, entry_price, tp, sl, trade_duration):
        ...
        risk_pips = (entry_price - sl) * 10000 if action == 'BUY' else (sl - entry_price) * 10000
    ...
    def _load_q_table(self):
        """Load Q-table from disk. Migrates legacy .q_table.pkl on first run (Mejora #1)."""
        try:
            ...
            if os.path.exists(self.q_table_file):
                with open(self.q_table_file, 'rb') as f:
                    ...
                    logger.info(f"[RL] Q-table loaded ({len(self.q_table)} states)")
        except Exception as e:
