...
║  Ports: 8660 (USDSEK) | 6560 (EURUSD) | Pure Python | <100ms latency        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
class NOVA:
    ...
    def __init__(self, port=8660, symbol='USDSEK'):
        self.port = port  # USDSEK LLM10 SENTINEL
        self.symbol = symbol
        ...
        try:
            ...
            if vix_monitored:
                ...
                if vix_level > 25:  # High fear = risk-off sentiment
                    decision = genome.get('decision', 'HOLD')
                    if decision == 'SELL':
                        score += 1  # USDSEK tends to fall on high VIX (flight to USD)
            else:
                score += 0
    ...
    def _audit_correlation_risks(self, symbol: str, genome: dict) -> list:
        ...
        alerts = []
        try:
            # Para USDSEK: correlación con DXY
            if symbol == 'USDSEK':
                ...
                if dxy_data:
                    ...
                    if dxy_direction == 'UP':
                        # USD subiendo = bearish para USDSEK
                        alerts.append({
                            'type': 'CORRELATION_RISK',
                            'severity': 'MEDIUM',
                            'issue': 'DXY_RISING',
                            'message': 'USD strengthening is bearish for USDSEK - caution on long trades'
                        })
...
if __name__ == '__main__':
    ...
    # USDSEK LLM10 SENTINEL uses port 8660
