...
║  Port: 8658 (TCP Server) - MATHEMATICAL PRECISION ENHANCED                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
_log_file = f'logs/llm3_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
_file_handler = logging.FileHandler(_log_file, encoding='utf-8')
_stream_handler = logging.StreamHandler()
_formatter = logging.Formatter('%(asctime)s | LLM3 v17.03 | %(levelname)s | %(message)s', datefmt='%H:%M:%S')
...
class PatternGuardian:
    ...
    def analyze_request(self, request):
        ...
        """
        try:
            symbol = request.get('symbol', 'USDSEK').upper().strip()
            ...
            # PHASE 10: Lowered thresholds for exotic pair
            # BUY requires: context_score >= +1.2 (was 2.0)
            # SELL requires: context_score <= -1.2 (was -2.0)
            # HOLD if: -1.2 < context_score < +1.2 (unclear/ranging)
            
            if context_score >= 1.2:
                # Bullish context - allow BUY if RSI not extreme
                if rsi < 78 and adx > 15:  # PHASE 10: ADX 22→15, RSI 75→78
                    decision = 'BUY'
            ...
            elif context_score <= -1.2:
                # Bearish context - allow SELL if RSI not extreme
                if rsi > 22 and adx > 15:  # PHASE 10: ADX 22→15, RSI 25→22
                    decision = 'SELL'
            ...
            # RSI > 60 + momentum > 0.002 + context > 0.8 + ADX > 25 = BUY (momentum override)
            elif rsi > 60 and price_mom_5 > 0.002 and context_score >= 0.8 and adx > 25:
                decision = 'BUY'
                confidence = min(88.0, 60.0 + (rsi - 50) * 0.4)
                pattern = 'MOMENTUM_UP'
                ...
                log.info(f"[LLM3] MOMENTUM UP (RSI={rsi:.1f}, mom={price_mom_5:.4f}, ctx={context_score:.1f}, ADX={adx:.0f}) -> BUY")
            elif context_score >= 0.8:  # PHASE 10: was 1.5
                decision = 'BUY'
                ...
                # PHASE 10: Produce weak directional votes instead of always HOLD
                if context_score > 0.3:
                    decision = 'BUY'
                    confidence = 35.0 + context_score * 15.0
                    confidence = min(50.0, confidence)
                    pattern = 'WEAK_BULLISH'
                    reason = f"Weak bullish context (score={context_score:.1f})"
                elif context_score < -0.3:
                    decision = 'SELL'
                    confidence = 35.0 + abs(context_score) * 15.0
                    confidence = min(50.0, confidence)
                    pattern = 'WEAK_BEARISH'
                    reason = f"Weak bearish context (score={context_score:.1f})"
                else:
                    decision = 'HOLD'
...
class LLM3Server:
    ...
    def __init__(self, host='127.0.0.1', port=8658):  # FIXED: LLM3 is 8658
        self.host = host
        self.port = port
...
def main():
    ...
    try:
        # Cargar config
        port = 8658  # LLM3 is on 8658
        try:
            with open('sekconfig.yaml', 'r') as f:
                config = yaml.safe_load(f) or {}
            port = config.get('llm_settings', {}).get('llm3_port', 8658)
        except:
            pass
