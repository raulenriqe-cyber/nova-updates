...
class LLM1Experto:
    ...
    def analyze(self, data):
        ...
        try:
            ...
            prices = data.get('prices', data.get('bar_data', {}).get('closes', [price]))
            ...
            opens = data.get('opens', data.get('bar_data', {}).get('opens', prices))
            closes = data.get('closes', data.get('prices', data.get('bar_data', {}).get('closes', [])))
            highs = data.get('highs', data.get('bar_data', {}).get('highs', prices))
            lows = data.get('lows', data.get('bar_data', {}).get('lows', prices))
            ...
            elif rsi > 60 and price_momentum > 0.001:
                # RSI muy fuerte + buen momentum = BUY fuerte (0.001 scaled for USDSEK ~10.50)
                rsi_signal = 1
                rsi_extrema_confidence = 0.6
            elif rsi < 40 and price_momentum < -0.001:
                # RSI muy débil + momentum negativo = SELL fuerte (scaled for USDSEK)
                rsi_signal = -1
                rsi_extrema_confidence = 0.6
            else:
            ...
            if len(prices) >= 3:
                ...
                if macd_histogram > 0 and price_momentum_3 > 0:
                    ...
                    macd_signal = -1
                    macd_momentum_boost = -0.3
                elif macd_histogram > 0 and price_momentum_3 < -0.0007:
                    # MACD positivo pero precio cayendo = NO COMPRAR (divergencia, evitar) (scaled for USDSEK)
                    macd_signal = 0
                    macd_momentum_boost = 0
                elif macd_histogram < 0 and price_momentum_3 > 0.0007:
                    # MACD negativo pero precio subiendo = NO VENDER (divergencia, evitar) (scaled for USDSEK)
                    macd_signal = 0
                    macd_momentum_boost = 0
            ...
            # VALIDATION: Señal clara requerida - PHASE 10 RELAXED FOR EXOTIC
            # net_signal range: -5.0 to +5.0 | Threshold: 0.10 (2%) - was 0.2 (4%)
            has_signals = net_signal > 0.10 or net_signal < -0.10  # PHASE 10: 2% threshold (exotic needs lower)
            has_momentum = price_momentum > 0.0005 or price_momentum < -0.0005  # FIX: scaled for USDSEK (was 0.15 from XAUUSD)
            
            valid_setup = has_signals or has_momentum  # REQUIRE ONE (more responsive)
            
            # 🎯 PRECISION ENTRY: ADX filter - PHASE 10 RELAXED for exotic
            adx_trending = adx > 18  # PHASE 10: was 25 (exotic pairs have lower ADX)
            adx_strong = adx > 28    # PHASE 10: was 35
            
            if net_signal > 0.10 and valid_setup and rsi > 35:  # PHASE 10: RSI floor 35 (was 40)
                decision = 'BUY'
                confidence = min(95, max(55, confidence))
                # BOOST: ADX fuerte + momentum confirmado = entrada de alta calidad
                if adx_strong and price_momentum > 0.001 and rsi > 50:
                    confidence = min(94, int(confidence * 1.20))  # Strong boost (scaled for USDSEK)
                elif adx_trending and price_momentum > 0.0007:
                    confidence = min(90, int(confidence * 1.10))  # Moderate boost (scaled for USDSEK)
                elif not adx_trending:  # ADX < 18 = ranging
                    confidence = max(45, int(confidence * 0.85))  # PHASE 10: Less penalty (was 0.75)
                    
            elif net_signal < -0.10 and valid_setup and rsi < 65:  # PHASE 10: RSI ceiling 65 (was 60)
                decision = 'SELL'
                confidence = min(95, max(55, confidence))
            ...
            # SAFE FALLBACK: On error, default to HOLD with 0% confidence
            ...
            # PHASE 10: Lowered from 2→1 for exotic pairs (MTF data often sparse)
            if trinity_decision != 'HOLD' and mtf_validation.get('confirms', 0) < 1:
            ...
            # 🧠 CRITICAL: ADX check - PHASE 10: Lowered from 22→15 for exotic
            if trinity_decision != 'HOLD' and adx < 15:
...
{Y}|{R}                                                              {Y}|{R}
{Y}+{'-'*62}+{R}
{Y}|{R}  {W}USDSEK Scalping Edition{R}      {D}by Polarice Labs © 2026{R}       {Y}|{R}
...
def run_tcp_server():
    ...
    # Cargar puerto desde sekconfig.yaml
    try:
        with open('sekconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('llm_settings', {}).get('llm1_port', 8655)
    except:
        port = 8655  # Fallback
