...
file_handler = logging.FileHandler(f'logs/llm6_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8')
...
logger.info("[INIT] Log file: logs/llm6_<timestamped>")
...
class LLM6_SmartMoney:
    ...
    def _make_decision(self, signal: SmartMoneySignal) -> SmartMoneySignal:
        ...
        if signal.votes:
            ...
            if total_weight > 0.3:  # PHASE 10: was 0.5 (lowered for exotic pair)
            ...
            elif total_weight < -0.3:  # PHASE 10: was -0.5 (lowered for exotic pair)
    ...
    def get_port_config(self) -> Dict:
        """Retorna configuración de puertos para integración (USDSEK 75xx range)"""
        return {
            'trinity_port': 8666,
            'kraken_port': 8652,
            'executor_port': 8656,
...
def run_tcp_server():
    ...
    # Load port from sekconfig.yaml
    try:
        with open('sekconfig.yaml', 'r') as f:
            config = yaml.safe_load(f) or {}
        port = config.get('sockets', {}).get('llm6_port', 8662)
    except Exception as e:
        port = 8662  # USDSEK LLM6 Smart Money Oracle
