...
class LLM5ChartAnalyzer:
    ...
    def analyze(self, symbol='USDSEK', timeframe='M1') -> Dict[str, Any]:
        ...
        # 🔍 STEP 6: Decisión final - PHASE 10: Lower signal difference threshold
        signal_diff = abs(bullish_signals - bearish_signals)
        
        if signal_diff >= 0.15:  # PHASE 10: was 0.3 (exotic pairs generate less signal diff)
        ...
        # PHASE 10: Raised trend_strength threshold from 0.4→0.6 (less blocking)
        if trend_direction == 'DOWNTREND' and trend_strength > 0.6:
            if decision == 'BUY':
                logger.warning(f"[LLM5] ⚠️ BLOCKED BUY in strong DOWNTREND! Forcing HOLD")
                decision = 'HOLD'
                pattern_confidence = 35.0
        elif trend_direction == 'UPTREND' and trend_strength > 0.6:
            if decision == 'SELL':
        ...
        adx_trending = adx > 20  # PHASE 10: was 25
        ...
        if decision in ('BUY', 'SELL'):
            if adx_strong:
                ...
                # Ranging market: PHASE 10: Less penalty (was 0.82)
                pattern_confidence = pattern_confidence * 0.90
                logger.debug(f"[LLM5] ADX={adx:.1f} RANGING → conf reduced 0.90x")
...
class LLM5Client:
    ...
    @staticmethod
    def analyze(symbol='USDSEK', timeframe='M1', candles=None):
