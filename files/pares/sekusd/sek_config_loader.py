...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║                 🇸🇪 SEKUSD CONFIG LOADER - CENTRALIZED CONFIGURATION           ║
║                                                                              ║
║  Loads USDSEK ports and parameters from sekconfig.yaml                       ║
║  Single source of truth for all SEK system configuration                     ║
║                                                                              ║
...
class sekconfig:
    """Centralized configuration manager for USDSEK system"""
    ...
    def load(cls) -> Dict[str, Any]:
        """Load configuration from sekconfig.yaml"""
        if cls._loaded and cls._config:
            return cls._config
        
        if not os.path.exists(CONFIG_PATH):
            raise FileNotFoundError(f"sekconfig.yaml not found at {CONFIG_PATH}")
    ...
    def get(cls, key_path: str, default: Any = None) -> Any:
        ...
        Example: get('ports.trinity') -> 8666
        """
        config = cls.load()
    ...
    def get_port(cls, service: str) -> int:
        ...
        if service.lower() == 'trinity':
            return cls.get('ports.trinity', 8666)
        elif service.lower() == 'kraken':
            return cls.get('ports.kraken', 8652)
        elif service.lower() == 'ollama':
            return 11434  # Default ollama port (shared)
        elif service.lower() == 'seksound':
            return cls.get('ports.sound', 8670)
        elif service.lower() == 'injector':
            return cls.get('ports.quantum', 8654)
        elif service.lower() == 'executor':
            return cls.get('ports.executor', 8656)
        else:
            # LLM service (llm1-llm10)
            llm_num = service.lower().replace('llm', '')
            if llm_num.isdigit():
                port = cls.get(f'ports.llm{llm_num}')
                if port is not None:
                    return port
...
def get_trinity_port() -> int:
    """Get Trinity port for USDSEK"""
    return sekconfig.get_port('trinity')

def get_kraken_port() -> int:
    """Get Kraken port for USDSEK"""
    return sekconfig.get_port('kraken')

def get_llm_port(llm_num: int) -> int:
    """Get port for specific LLM (1-10)"""
    return sekconfig.get_port(f'llm{llm_num}')

def get_ollama_port() -> int:
    """Get Ollama port (shared)"""
    return sekconfig.get_port('ollama')

def get_sound_port() -> int:
    """Get Sound alert port"""
    return sekconfig.get_port('seksound')

def get_injector_port() -> int:
    """Get Injector port (MT5 -> Python)"""
    return sekconfig.get_port('injector')

def get_executor_port() -> int:
    """Get Executor port (Python -> MT5)"""
    return sekconfig.get_port('executor')
...
def get_all_llm_ports() -> Dict[str, int]:
    ...
    for i in range(1, 11):
        try:
            ports[f'llm{i}'] = sekconfig.get_port(f'llm{i}')
        except:
            pass
    return ports
...
def get_symbol() -> str:
    """Get market symbol"""
    return sekconfig.get('symbol', 'USDSEK')

def get_timeframe() -> str:
    """Get primary timeframe"""
    return sekconfig.get('primary_timeframe', 'M5')

def get_sl_range() -> tuple:
    """Get SL min/max pips"""
    return (
        sekconfig.get('scalper.sl_min_pips', 10),
        sekconfig.get('scalper.sl_max_pips', 25)
    )

def get_tp_range() -> tuple:
    """Get TP min/max pips"""
    return (
        sekconfig.get('scalper.tp_min_pips', 15),
        sekconfig.get('scalper.tp_max_pips', 40)
    )

def get_cooldown_seconds() -> int:
    """Get cooldown between trades (SEK needs longer cooldown)"""
    return 300  # 5 minutes for exotic pair

def get_min_confidence() -> int:
    """Get minimum confidence threshold"""
    return sekconfig.get('scalper.min_confidence', 60)

def get_max_positions() -> int:
    """Get max positions"""
    return sekconfig.get('scalper.max_concurrent_trades', 3)

def get_pip_value() -> float:
    """Get pip value for USDSEK (pip = 0.001)"""
    return sekconfig.get('trading.pip_value', 0.001)

def get_volatility_thresholds() -> dict:
    """Get ATR thresholds for volatility regimes"""
    return {
        'low': sekconfig.get('indicators.atr.low_volatility', 0.010),
        'normal': sekconfig.get('indicators.atr.normal_volatility', 0.025),
        'high': sekconfig.get('indicators.atr.high_volatility', 0.050),
        'extreme': sekconfig.get('indicators.atr.extreme_volatility', 0.100)
    }
...
if __name__ == '__main__':
    print("═" * 80)
    print("🇸🇪 USDSEK CONFIG LOADER - TEST")
    print("═" * 80)
