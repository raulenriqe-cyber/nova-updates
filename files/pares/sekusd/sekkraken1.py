...
ENHANCEMENTS:
  ...
  - Price sanity checks
  - Enhanced logging
  - Dynamic port from config (default 8652)
  - Audio announcements via sound.py (USDSEK only)
"""
import socket
import struct
import json
...
class KrakenExecutor:
    ...
    def __init__(self, config_path='sekconfig.yaml'):
        ...
        try:
            ...
            # BUGFIX #62: Read port from config, default to 8652 (not 8656 which is quantum_core executor)
            self.port = config.get('kraken_executor_port', 8652)
            self.symbols = config.get('core_engine', {}).get('symbols', ['USDSEK'])
        except Exception as e:
            log.warning(f"Config load failed: {e}, using defaults")
            self.port = 8652  # Default to 8652, NOT 8656
            self.symbols = ['USDSEK']
    ...
    def _validate_tp_sl(self, action, entry, tp, sl):
        ...
        if tp == 0:
            ...
            tp = entry + 0.0500  # [NOVA AUDIT F-12] 50-pip fallback USDSEK (pip=0.001)
            warning = "TP was 0, set to entry + 50pips"

        if sl == 0:
            log.warning(f"[VALIDATION] SL is 0, setting to entry - 30 pips")
            sl = entry - 0.0300  # [NOVA AUDIT F-12] 30-pip fallback USDSEK
    ...
    def _execute_order(self, order):
        ...
        try:
            symbol = order.get('symbol', 'USDSEK')
            ...
            # Validate symbol
            valid_symbols = {'USDSEK', 'GOLD', 'XAU', 'EURUSD', 'EUR', 'USDSEK', 'GBP',
                           'USDJPY', 'JPY', 'USDSEK', 'GBP', 'ETHUSD', 'ETH'}
...
if __name__ == "__main__":
    try:
        kraken = KrakenExecutor('sekconfig.yaml')
        kraken.run()
