...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║     ⚔️ NOVA EXCALIBUR KNIGHT - QUANTUM CORE v18.00 USDSEK ELITE EDITION      ║
...
try:
    ...
    print('[OK SHARED BRAIN] Cross-pair intelligence loaded for SEKUSD')
...
try:
    ...
    try:
        from sekllm5 import LLM5Client  # SEK-specific LLM5
    except ImportError:
...
try:
    from sekllm6 import (  # 🔧 FIX CAT-104
        LLM6_SmartMoney,
        SmartMoneySignal,
        MarketIntention,
        SweepType
...
class ConfluenceGate:
    ...
    def __init__(self):
        ...
        self.decay_max_seconds = 180  # 🎯 SNIPER M1 - Match cooldown of 180s (FIX v7)
        # 🎯 PHASE 10 RELAXED THRESHOLDS - Allow more attacks for exotic
        self.min_perfect_score = 2.0   # PERFECT - attack on 2.0+ (was 2.5)
        self.min_good_score = 1.5      # GOOD - quality setups (was 2.0)
        self.min_wait_score = 0.8      # WAIT - minimum quality for attack (was 1.0)
        self.min_sweep_score = 1.5     # SWEEP - trust smart money signals (was 2.0)
        self.lock = Lock()
        ...
        # NOTE: LLM6 runs as TCP server on port 8662 (USDSEK 75xx range)
        self.llm6_port = 8662  # LLM6 NOVA Smart Money Oracle port (USDSEK)
    ...
    def _query_llm6_remote(self, genome: dict) -> dict:
        """Query LLM6 NOVA Smart Money Oracle via TCP (port 8662)"""
        try:
            ...
            request = {
                ...
                'symbol': genome.get('symbol', 'USDSEK'),
            }
    ...
    def _calculate_consciousness_probability(self, direction: str, indicators: dict, 
        ...
        # PHASE 10: Added tier at 40% for exotic pairs
        ...
        atr = indicators.get('atr', 0.010)  # 10 pips default for USDSEK exotic (1 pip = 0.001)
        ...
        if len(closes) >= 20 and len(highs) >= 20 and len(lows) >= 20:
            ...
            if NEWS_INTELLIGENCE_AVAILABLE:
                try:
                    news_bias, news_conf = get_trading_bias('USDSEK')
                    if news_conf > 60:
        ...
        # Asia session (0:00-7:00 UTC) = Menor liquidez para USDSEK
        elif 0 <= hour_utc <= 6:
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                news_bias, news_confidence = get_trading_bias('USDSEK')
    ...
    def calculate_confluence_score(self, 
        ...
        rsi = indicators.get('rsi', 50)
        adx = indicators.get('adx', 25)
        atr = indicators.get('atr', 0.010)  # Default 10 pips for USDSEK exotic (1 pip = 0.001)
        macd = indicators.get('macd', 0)
        macd_signal = indicators.get('macd_signal', 0)
        ...
        if high_prices and low_prices and current_price:
            ...
            if fib_range > 0:
                ...
                # For USDSEK at ~1.25, 0.5% = 0.00625 which is reasonable for fib zone
        ...
        if 13 <= hour_utc <= 16:
            ...
            timing_score += 0.1  # Asia - baja liquidez para GBP
    ...
    def evaluate(self, 
        ...
        with self.lock:
            ...
            # 🔧 PHASE 10: USDSEK has wider spreads (40-80 points normal)
            # Threshold 500 points = 50 pips (was 30 points = too tight for USDSEK)
            spread = genome.get('tick_data', {}).get('spread', 0)
            if spread > 500:  # 50 pips = wide for USDSEK
                calc_entry_score -= 1.0
            ...
            if macd_aligned and abs(macd_hist) > 0.001:
            ...
            # AHORA: También requiere calc_entry_score mínimo Y no sweep trap
            # ═══════════════════════════════════════════════════════════════
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 VETO POR PROBABILIDAD MUY BAJA - ULTRA RELAXED FOR GBP
            # Si la probabilidad es <12%, NO atacar bajo ningún concepto
            # ⭐ RELAXED: De 22% a 12% para permitir más ataques en GBP
            # ═══════════════════════════════════════════════════════════════
            ...
            if len(closes) >= 5:
                ...
                # USDSEK (~1.25) vs EURUSD (~1.08) - percentage based is symbol-agnostic
            ...
            # Esto hace el código compatible con USDSEK, EURUSD, etc.
            momentum_ok = True
            momentum_bonus = 0.0
            if len(closes) >= 3 and closes[-3] > 0:
                ...
                # Umbral: 0.12% = fuerte contra (USDSEK exotic: wider threshold needed)
                # 🔧 FIX: Was 0.08% which blocked most SEK entries (exotic pairs move differently)
                # PHASE 10: Widened from 0.12% to 0.15% for exotic pair
                strong_against_pct = 0.15
                
                if trinity_decision == 'BUY':
                    if last_momentum_pct < -strong_against_pct:
                        # Momentum fuerte CONTRA - pero solo bloquear si RSI alto
                        # PHASE 10: Widened RSI gate 45→40 (was blocking too many entries)
                    ...
                    if last_momentum_pct > strong_against_pct:
                        ...
                        # PHASE 10: Widened RSI gate 55→60
            ...
            if not llm6_response_received:
                ...
                # ─────────────────────────────────────────────────────────────
                candle_range = high - low if high > low else 0.001  # Min 1 pip for USDSEK
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🧠 TIERS BASADOS EN CONCIENCIA - RELAXED FOR GBP SCALPING
            # Combina Score + Consciousness Probability + Calc Entry Score
            # ⭐ FIX: Tiers were absurdly relaxed (prob 22%, calc 1.0, trinity 40%)
            # Raised to reasonable levels to prevent garbage entries like -$5 SEK trades
            # ═══════════════════════════════════════════════════════════════
            
            # 🎯 TIER 0: CONSCIOUSNESS GOLD - FIX: prob 42→48%, calc 1.5→3.0
            if consciousness_prob >= 0.48 and momentum_ok and calc_entry_score >= 3.0:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 0.5: Multi-TF ALIGNMENT - FIX: align 50→55, conf 40→50, calc 1.5→2.5
            if alignment_score >= 55 and trinity_confidence >= 50 and calc_entry_score >= 2.5:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 1: ULTRA-PREMIUM - FIX: prob 30→38%, calc 2.0→4.0
            if score >= self.min_perfect_score and momentum_ok and consciousness_prob >= 0.38 and calc_entry_score >= 4.0:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 2: ELITE - FIX: prob 28→36%, calc 1.5→3.5
            if score >= self.min_good_score and momentum_ok and consciousness_prob >= 0.36 and calc_entry_score >= 3.5:
                self._clear_pending(trinity_decision)
            ...
            # 🎯 TIER 3: QUALITY - FIX: prob 22→32%, calc 1.0→3.0
            if score >= self.min_wait_score and momentum_ok and consciousness_prob >= 0.32 and calc_entry_score >= 3.0:
                self._clear_pending(trinity_decision)
                adjusted_conf = trinity_confidence * 0.9
                return True, trinity_decision, adjusted_conf, \
                       f"⭐⭐⭐ QUALITY (Score={score:.1f}, Prob={consciousness_prob:.0%}, Calc={calc_entry_score:.1f})"
            
            # 🎯 TIER 4: SMART MONEY - FIX: prob 22→30%, calc 1.0→2.5
            ...
            # 🎯 TIER 5: Trinity Force - FIX: was trinity>=40, prob>=0.15 — insanely low
            ...
            # 🎯 TIER 6: High Confidence Override - FIX: was trinity>=45, prob>=0.15
            if trinity_confidence >= 60 and score >= 1.5 and consciousness_prob >= 0.26 and calc_entry_score >= 1.5:
                self._clear_pending(trinity_decision)
            ...
            # ═══════════════════════════════════════════════════════════════
            # 🚫 REMOVED TIER 7 FALLBACK — was trinity>=35, consciousness>=10%
            # This tier let ANY garbage trade through and was the root cause
            # of SEK entering -$5 trades immediately
            # ═══════════════════════════════════════════════════════════════
    ...
    def _validate_direction_ultra_exigent(
        ...
        # ═══════════════════════════════════════════════════════════════
        adx = indicators.get('adx', 25)
        # 🔧 FIX: ADX<20 was too strict for M1 scalping (ADX often 12-19 during valid moves)
        if adx < 12:
            validations_failed += 1
        ...
        # 1. VETO ABSOLUTO: Si hay 6+ fallas = NO ATACAR (relaxed from 5)
        # ⭐ 🔧 FIX: Raised from 5 to 6 to allow more attacks when LLMs agree
        ...
        # 4. RATIO MÍNIMO: Necesitamos 50%+ de confirmaciones (🔧 was 55%)
        if total_checks >= 4 and success_ratio < 0.50:
        ...
        if success_ratio >= 0.50 and validations_passed > validations_failed:
...
class NOVAMarketStateDetector:
    ...
    def analyze(self, genome: dict) -> dict:
        ...
        # Spread too wide (illiquidity) - USDSEK spread in price units (0.001 = 1 pip)
        elif spread > 0.020:  # For USDSEK, > 20 pips is abnormally wide (exotic pair)
            self.alerts.append(f"⚠️ WIDE_SPREAD: {spread/0.001:.1f} pips (liquidity crisis)")
            liquidity_score -= 25
...
class Config:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ⚔️ STRATEGY SELECTOR (loaded from sekconfig.yaml)
    # ═══════════════════════════════════════════════════════════════════════
    ACTIVE_STRATEGY = "SCALPER"  # Default, will be overridden by yaml
    EXCALIBUR_CONFIG = None      # ExcaliburKnightConfig instance if active
    
    # ═══════════════════════════════════════════════════════════════════════
    # NETWORK PORTS - 86xx range for USDSEK
    # ═══════════════════════════════════════════════════════════════════════
    HOST = '127.0.0.1'
    QUANTUM_PORT = 8654      # Injector - receive genomes from Quimera (matches yaml)
    EXECUTOR_PORT = 8656     # Executor - send commands to Quimera (matches yaml)
    TRINITY_PORT = 8666      # Trinity server port (USDSEK)
    KRAKEN_PORT = 8652       # Kraken executor (USDSEK)
    LLM1_PORT = 8655         # BAYESIAN
    LLM2_PORT = 8657         # TECHNICAL
    LLM3_PORT = 8658         # CHART
    LLM4_PORT = 8659         # RISK
    LLM5_PORT = 8661         # LLM5 SUPREME chart analyzer
    LLM6_PORT = 8662         # ⭐ LLM6 NOVA Smart Money Oracle (USDSEK)
    LLM7_PORT = 8663         # LLM7 OCULUS Data Quality
    LLM8_PORT = 8664         # LLM8 CHRONOS Timing
    LLM9_PORT = 8665         # LLM9 PREDATOR Execution
    LLM10_PORT = 8660        # LLM10 SENTINEL Guardian
    LLM11_PORT = 8667        # LLM11 STRATEGIST Guru Consciousness
    LLM12_PORT = 8668        # LLM12 SENTINEL Quality Guardian
    SOUND_PORT = 8670  # 🔊 Sound.py TTS alert engine (TP/SL audio alerts)
    
    # ═══════════════════════════════════════════════════════════════════════
    # 🎯 SCALPER STRATEGY DEFAULTS (M1/M5 Forex Scalping)
    # ═══════════════════════════════════════════════════════════════════════
    # USDSEK: 1 pip = 0.001 (3 decimal pairs, NOT 0.0001 like majors!)
    # Price ~10.50, typical M1 move: 5-30 pips (0.005-0.030)
    # ATR M1: 0.015-0.080, spread ~6 pips (0.006) in price units
    PIP_VALUE = 0.001          # ⭐ CRITICAL: USDSEK 1 pip = 0.001 (internal calc units)
    TP_MIN_PIPS = 20           # FIX v7: 20 pips minimum (0.020) - was 15
    TP_MAX_PIPS = 80           # FIX v7: 80 pips max (0.080) - wider for exotic trends
    SL_MIN_PIPS = 15           # FIX v7: 15 pip stops (0.015) - wider for exotic noise
    SL_MAX_PIPS = 50           # FIX v7: Max 50 pip stop (0.050) - protect capital
    ATR_FLOOR = 0.015          # 15 pip min ATR (correct for USDSEK M1)
    CONSOLIDATION_THRESHOLD = 8.0    # Higher for exotic: needs more movement
    
    # Risk - SNIPER PRECISION FOR EXOTIC PAIR
    MAX_CONCURRENT_TRADES = 3        # 3 concurrent trades in SAME direction
    MAX_DAILY_LOSS_PERCENT = 1.5     # Strict loss control (matches yaml)
    MAX_CONSECUTIVE_LOSSES = 3       # Halt after N consecutive losses (backport GBP A-GBP-02)
    LOSS_HALT_DURATION_SECONDS = 300 # Pause 5 min after consecutive-loss trigger
    MIN_CONFIDENCE = 60              # FIX v7: 60%+ (52 was too low - entering garbage trades)
    MIN_TRINITY_CONFIDENCE = 50      # FIX: was 20 — too low, letting 36% trades through. 50% is minimum reasonable.
    MIN_ADX = 20                     # ADX filter (matches yaml)
    
    # ═══════════════════════════════════════════════════════════════
    # 🧠 PAIR-SPECIFIC LLM WEIGHTS - USDSEK (Nordic Exotic pair)
    # SEK responds to: Riksbank, EUR flows, Nordic economics, carry trade
    # TECHNICAL matters most (SEK trends strongly when it moves)
    # SMART_MONEY low (SM doesn't understand exotic pairs well)
    # ═══════════════════════════════════════════════════════════════
    LLM_WEIGHTS = {
        'BAYESIAN': 1.0,       # Standard probabilistic
        'TECHNICAL': 1.4,      # ⭐ Technical analysis key for SEK trends
        'CHART': 1.2,          # Chart patterns relevant (SEK trends)
        'RISK': 0.7,           # Risk sentiment less direct for SEK
        'SUPREME': 1.3,        # Pattern analysis - good for trending pairs
        'SMART_MONEY': 0.2,    # ⚠️ CRITICAL: SM doesn't understand exotics - near-zero
        'OCULUS': 0.8,         # Quality validator
        'CHRONOS': 1.3,        # ⭐ Timing: EU session critical (Nordic market hours)
        'PREDATOR': 1.0,       # Execution - neutral
        'NOVA': 0.5,           # Fallback - reduced
    }
    HOLD_VOTE_WEIGHT = 0.3     # HOLD = abstention, reduced weight
    MIN_LLM_AGREEMENT = 2     # Need 2+ directional LLMs (was 3 - blocks exotic pairs)
    ...
    # SEK exotic: wider RSI bands (more noise in exotic pairs)
    MIN_RSI_FOR_BUY = 35             # Aligned with yaml
    MAX_RSI_FOR_BUY = 68
    MIN_RSI_FOR_SELL = 32            # Aligned with yaml
    MAX_RSI_FOR_SELL = 65            # Aligned with yaml
    ...
    # SNIPER COOLDOWN - Key setting for exotic pair (wider = more selective)
    SNIPER_COOLDOWN_SECONDS = 180   # FIX v7: 180s (user requested, exotic needs patience)
    ...
    EXCALIBUR_ATR_FLOOR = 0.015      # 15 pips min ATR for USDSEK M1 (1 pip = 0.001)
    EXCALIBUR_ATR_MAX = 0.050        # 50 pips max ATR
    ...
    def load_strategy_from_yaml(cls):
        """Load active strategy from sekconfig.yaml"""
        try:
            import yaml
            config_path = os.path.join(os.path.dirname(__file__), 'sekconfig.yaml')
            if os.path.exists(config_path):
                ...
                if strategy == 'EXCALIBUR_KNIGHT':
                    ...
                    cls._apply_scalper_config(config.get('scalper', {}))
                    print(f"[CONFIG] 🎯 SCALPER strategy loaded from YAML - M5 Fast Scalping")
                else:
                    print(f"[CONFIG] 🎯 {strategy} strategy active - using defaults")
    ...
    @classmethod
    def _apply_scalper_config(cls, scalper_yaml: dict):
        """Apply SCALPER configuration from YAML - FIX v7: was DEAD CONFIG before"""
        if not scalper_yaml:
            print("[CONFIG] ⚠️ No scalper config in YAML - using hardcoded defaults")
            return
        
        # Override trading parameters from YAML
        cls.TP_MIN_PIPS = scalper_yaml.get('tp_min_pips', cls.TP_MIN_PIPS)
        cls.TP_MAX_PIPS = scalper_yaml.get('tp_max_pips', cls.TP_MAX_PIPS)
        cls.SL_MIN_PIPS = scalper_yaml.get('sl_min_pips', cls.SL_MIN_PIPS)
        cls.SL_MAX_PIPS = scalper_yaml.get('sl_max_pips', cls.SL_MAX_PIPS)
        cls.ATR_FLOOR = scalper_yaml.get('atr_floor', cls.ATR_FLOOR)
        cls.CONSOLIDATION_THRESHOLD = scalper_yaml.get('consolidation_threshold', cls.CONSOLIDATION_THRESHOLD)
        
        # Risk parameters
        cls.MAX_CONCURRENT_TRADES = scalper_yaml.get('max_concurrent_trades', cls.MAX_CONCURRENT_TRADES)
        cls.MAX_DAILY_LOSS_PERCENT = scalper_yaml.get('max_daily_loss_percent', cls.MAX_DAILY_LOSS_PERCENT)
        cls.MIN_CONFIDENCE = scalper_yaml.get('min_confidence', cls.MIN_CONFIDENCE)
        cls.MIN_TRINITY_CONFIDENCE = scalper_yaml.get('min_trinity_confidence', cls.MIN_TRINITY_CONFIDENCE)
        cls.MIN_ADX = scalper_yaml.get('min_adx', cls.MIN_ADX)
        
        # RSI bounds
        cls.MIN_RSI_FOR_BUY = scalper_yaml.get('min_rsi_for_buy', cls.MIN_RSI_FOR_BUY)
        cls.MAX_RSI_FOR_BUY = scalper_yaml.get('max_rsi_for_buy', cls.MAX_RSI_FOR_BUY)
        cls.MIN_RSI_FOR_SELL = scalper_yaml.get('min_rsi_for_sell', cls.MIN_RSI_FOR_SELL)
        cls.MAX_RSI_FOR_SELL = scalper_yaml.get('max_rsi_for_sell', cls.MAX_RSI_FOR_SELL)
        
        # Cooldown
        cls.SNIPER_COOLDOWN_SECONDS = scalper_yaml.get('sniper_cooldown_seconds', cls.SNIPER_COOLDOWN_SECONDS)
        
        print(f"[CONFIG] 🎯 SCALPER config loaded: TP={cls.TP_MIN_PIPS}-{cls.TP_MAX_PIPS}p, "
              f"SL={cls.SL_MIN_PIPS}-{cls.SL_MAX_PIPS}p, cooldown={cls.SNIPER_COOLDOWN_SECONDS}s, "
              f"max_trades={cls.MAX_CONCURRENT_TRADES}, min_conf={cls.MIN_CONFIDENCE}%")
    ...
    def get_strategy_info(cls) -> dict:
        ...
        if cls.is_excalibur_active():
            ...
            return {
                'name': 'SCALPER',
                'description': 'Fast M5 Forex Scalping',
                'timeframe': 'M5',
...
class NOVADashboard:
    """
    NOVA EXCALIBUR KNIGHT - USDSEK Elite Momentum Dashboard
    ...
    def _format_price(self, price, decimals=3):
        """Format price with proper decimals for USDSEK (3-decimal exotic pair)"""
        if price == 0:
            return "-.---"
    ...
    def render(self):
        ...
        # Sanitize strings
        d['symbol'] = safe_str(d.get('symbol', 'USDSEK'))
        ...
        lines.append("")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}███╗   ██╗{self.RESET}{SOFT_WHITE}{self.BOLD} ██████╗ ██╗   ██╗ █████╗ {self.RESET}    {self.CYAN}{self.BOLD}⚔️ NOVA Excalibur Knight{self.RESET}  •  {self.GREEN}●{self.RESET} {now.strftime('%H:%M:%S')}  •  Up {self.CYAN}{uptime_str}{self.RESET}")
        lines.append(f"  {SOFT_YELLOW}{self.BOLD}████╗  ██║{self.RESET}{SOFT_WHITE}{self.BOLD}██╔═══██╗██║   ██║██╔══██╗{self.RESET}    {SOFT_WHITE}USDSEK Elite Momentum by{self.RESET} {polarice_anim}{ORANGE}{self.BOLD}Polarice Labs{self.RESET}{polarice_anim} © 2026")
        ...
        lines.append(f"  {SOFT_YELLOW_F}{self.BOLD}⚔️ NOVA Excalibur Knight{self.RESET} v18.00 USDSEK {ORANGE}{footer_anim} Polarice Labs {footer_anim}{self.RESET} {self.GRAY}© 2026{self.RESET}")
        lines.append("")
    ...
    def _send_llm_decision_to_dashboard(self, llm_name, decision, confidence, reason=""):
        """Send LLM decision message to trinity_dashboard_v3 TCP listener on port 7567 (USDSEK)"""
        try:
            import socket
            ...
            # Serialize and send to localhost:7567 (USDSEK dashboard port)
...
class LLMParallelConsensus:
    ...
    def __init__(self):
        ...
        self.executor = ThreadPoolExecutor(max_workers=5)  # 🔧 FIX CAT-109
        self.logger = logger
    ...
    def _query_single_llm(self, llm_name, port, genome, timeout):
        ...
        try:
            ...
            msg = json.dumps(genome, default=str).encode('utf-8')  # 🔧 FIX CAT-103
...
class AdaptiveTPSLEngine:
    ...
    def _calculate_m5_projection(self, closes, direction, atr):
        ...
        logger.debug(f"[M5 PROJECTION] slope={slope:.6f}, projected={projected_price:.5f}, "
                    f"distance={projection_distance/0.001:.1f}pips, alignment={alignment:.2f}")
    ...
    def calculate_intelligent_tp_sl(self, current_price, direction, indicators, llm_confidence=50, 
                                     support_resistance=None, spread=0.005, closes=None):
        """
        Calculate INTELLIGENT TP/SL for M1 USDSEK using ALL available data.
        USDSEK: 1 pip = 0.001, price ~10.50, typical M1 ATR 0.015-0.050
        """
        # Extract indicators
        atr = float(indicators.get('atr', 0.020))  # Default 20 pips for USDSEK (0.020)
        rsi = float(indicators.get('rsi', 50))
        adx = float(indicators.get('adx', 25))
        bb_upper = float(indicators.get('bollinger_upper', current_price + 0.040))
        bb_lower = float(indicators.get('bollinger_lower', current_price - 0.040))
        ma_fast = float(indicators.get('ma_fast', current_price))
        ma_slow = float(indicators.get('ma_slow', current_price))
        
        # Ensure minimum ATR for USDSEK M1
        # 🔧 FIX v7: USDSEK M1 ATR typically 0.010-0.060 (10-60 pips, 1 pip = 0.001 internal)
        atr = max(0.015, min(0.080, atr))  # Clamp ATR 15-80 pips for M1 safety
        ...
        # ═══════════════════════════════════════════════════════════════
        # M1 USDSEK typical ATR: 0.005-0.025 (= 5-25 pips, 1 pip = 0.001)
        ...
        # ═══════════════════════════════════════════════════════════════
        # STEP 7: M1 USDSEK SCALPING TP/SL - ATR BASED
        # ═══════════════════════════════════════════════════════════════
        # M1 ATR típico USDSEK: 0.010-0.050 (10-50 pips, 1 pip = 0.001)
        # Multiplicadores: TP = ATR × 1.2, SL = ATR × 0.7
        # Esto da R:R de ~1.7:1 mínimo
        # ═══════════════════════════════════════════════════════════════
        
        # ATR viene en precio para USDSEK exotic (ej: 0.020 = 20 pips)
        # 🔧 FIX v7: Allow wider range for exotic pair ATR
        atr_clamped = max(0.015, min(0.080, atr))  # Min 15 pips, Max 80 pips (exotic)
        ...
        if m5_projected is not None and m5_alignment > 0.6:
            ...
            m5_tp_extension = min(m5_tp_extension, 0.030)  # Cap at 30 pips extra (0.030 for USDSEK)
            
            if m5_alignment > 0.8:
                # Very high alignment - use more of the projection
                logger.info(f"[M5 PROJECTION] ✅ STRONG alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension/0.001:.1f}pips")
                tp_distance_price += m5_tp_extension
            elif m5_alignment > 0.6:
                # Moderate alignment - use half
                logger.info(f"[M5 PROJECTION] ↗ Moderate alignment ({m5_alignment:.2f}) - Extending TP by {m5_tp_extension*0.5/0.001:.1f}pips")
        ...
        # 🔧 FIX v7: USDSEK EXOTIC - spread ~6 pips (0.006 in price), TP must exceed it
        # ATR-based TP/SL adjusted with better minimums
        # ═══════════════════════════════════════════════════════════════════════════
        MIN_TP = 0.020   # 20 pips minimum TP (USDSEK exotic, > 3x spread)
        MIN_SL = 0.015   # 15 pips minimum SL (USDSEK exotic)
        SAFETY = 0.005   # 5 pips safety buffer for ASK/BID slippage
        
        # 🔧 FIX v7: SPREAD-AWARE TP - ensure TP exceeds actual spread
        # spread comes in PRICE units from caller (0.006 = 6 pips typical)
        actual_spread = max(spread, 0.004)  # Floor at 4 pips (USDSEK minimum)
        min_tp_above_spread = actual_spread * 2.5  # TP must be at least 2.5x spread
        effective_min_tp = max(MIN_TP, min_tp_above_spread)
        
        tp_distance_price = max(effective_min_tp, tp_distance_price)
        sl_distance_price = max(MIN_SL, sl_distance_price)
        
        logger.info(f"[TP/SL SPREAD-AWARE] spread={actual_spread:.5f}({actual_spread/0.001:.1f}p), min_tp_for_profit={effective_min_tp:.4f}({effective_min_tp/0.001:.0f}p)")
        
        # ⚠️ HARD CAP para scalping M1 - USDSEK exotic pip distances
        tp_distance_price = min(0.100, tp_distance_price)  # Máximo 100 pips TP (exotic)
        sl_distance_price = min(0.060, sl_distance_price)   # Máximo 60 pips SL (exotic)
        ...
        # Log para debugging
        logger.debug(f"[TP/SL M1] ATR={atr:.4f} → TP={tp_distance_price:.4f} ({tp_distance_price/0.001:.0f}p), SL={sl_distance_price:.4f} ({sl_distance_price/0.001:.0f}p)")
        ...
        # Convert to pips for return (1 pip = 0.001 for USDSEK exotic)
        tp_pips = int(tp_distance_price / 0.001)
        sl_pips = int(sl_distance_price / 0.001)
    ...
    def calculate_adaptive_tp_sl(self, current_price, atr, direction, win_rate, volatility_regime):
        ...
        # FIX v5: Mínimos REDUCIDOS para M1 USDSEK scalping
        # M1 requiere stops ajustados, 8-25 pips típico
        MIN_TP_PIPS = 25  # Mínimo 25 pips TP (calibrado 2026-03-10, R:R 2:1 con SL ~19p)
        MIN_SL_PIPS = 8   # Mínimo 8 pips SL (M1 scalping)
        SAFETY_PIPS = 3   # 3 pips de seguridad extra
        
        tp_distance = max(MIN_TP_PIPS, min(65.0, tp_distance))   # 25-65 PIPS TP
        sl_distance = max(MIN_SL_PIPS, min(25.0, sl_distance))   # 8-25 PIPS SL
        
        # Point value conversion: USDSEK = 0.001 per pip (3-decimal exotic pair)
        point_value = 0.001  # USDSEK: 1 pip = 0.001 (e.g., 35 pips = 0.035)
        safety_margin = SAFETY_PIPS * point_value  # 5 pips × 0.001 = 0.005
    ...
    def calculate_smart_plan(self, current_price, direction, genome, indicators, sweep_info=None):
        ...
        try:
            ...
            atr = indicators.get('atr', 0.010)  # Default 10 pips for forex
            ...
            if direction == 'BUY':
                ...
                tp1_raw = min([t for t in potential_tp1s if t > current_price + 0.010], default=current_price + 0.015)
                ...
                potential_sls = [
                    support - atr * 0.3,
                    min(lows[-5:]) - atr * 0.2 if len(lows) >= 5 else support - 0.020,
                    current_price - atr * 1.5 * atr_mult
                ]
                sl_raw = max([s for s in potential_sls if s < current_price - 0.010], default=current_price - 0.010)
                ...
                tp1_raw = max([t for t in potential_tp1s if t < current_price - 0.010], default=current_price - 0.015)
                ...
                potential_sls = [
                    resistance + atr * 0.3,
                    max(highs[-5:]) + atr * 0.2 if len(highs) >= 5 else resistance + 0.020,
                    current_price + atr * 1.5 * atr_mult
                ]
                sl_raw = min([s for s in potential_sls if s > current_price + 0.010], default=current_price + 0.010)
            ...
            MIN_TP_PIPS = 15  # USDSEK exotic: wider minimum
            MIN_SL_PIPS = 10  # USDSEK exotic: wider minimum 
            POINT = 0.001  # USDSEK exotic: 1 pip = 0.001
...
class ReinforcementLearningQNetwork:
    ...
    def discretize_state(self, price, atr, volatility, trend):
        ...
        atr_bin = int(atr / 0.001) if atr > 0 else 0  # [NOVA AUDIT F-11] USDSEK pip=0.001, ATR M1 ~5-15 pips
...
class MarketMicrostructureAnalyzer:
    ...
    def analyze_tick(self, bid, ask, volume, direction):
        ...
        spread = ask - bid
        spread_pips = spread / 0.001  # For USDSEK (1 pip = 0.001)
...
class MarketCycleAnalyzer:
    ...
    def identify_cycle_phase(self, prices):
        ...
        # Phase identification (thresholds scaled for USDSEK, 1 pip = 0.001)
    ...
    def detect_sweep(self, prices, support_level, resistance_level, tolerance_pips=0.5, volumes=None):
        ...
        base_tolerance = float(tolerance_pips) if tolerance_pips else 0.005  # 5 pips default
        atr_value = getattr(self, 'last_atr', 0.010)  # Get last ATR (10 pips default)
        
        # Scale tolerance dynamically: higher ATR = higher tolerance needed
        dynamic_tolerance = max(0.005, min(0.020, atr_value * 1.5))  # 5-20 pips typical
...
class QuantumConsciousnessEngine:
    ...
    def __init__(self):
        ...
        if SUPREME_CONSCIOUSNESS_AVAILABLE:
            ...
            self.logger.info("   🛡️ Anti-Noise Filter: Signal purity detection")
        else:
            self.supreme_consciousness = None
            self.logger.warning("[⚠️ SUPREME] Supreme Consciousness not available - using standard mode")
    ...
    def process_with_full_consciousness(self, trinity_decision, trinity_confidence, genome, trinity_response=None, trading_stats=None):
        ...
        if SHARED_BRAIN_AVAILABLE and self.genome_counter % 30 == 0:
            try:
                ...
                update_regime_state(
                    symbol=genome.get('metadata', {}).get('symbol', 'USDSEK'),
        ...
        spread = (ask - bid) * 1000 if bid > 0 and ask > 0 else 0  # In pips (1 pip = 0.001)
        ...
        atr_val = indicators.get('atr', 0.010)  # Default 10 pips for USDSEK
        ...
        if NEWS_INTELLIGENCE_AVAILABLE:
            try:
                _news_bias, _news_confidence = get_trading_bias('USDSEK')
                if not _news_bias:
                    _news_bias = 'NEUTRAL'
        ...
        # 🔧 FIX: Use only VOTING LLMs count (5-6 core), not all 10 (LLM7/8/9/NOVA don't vote BUY/SELL)
        # Old: num_llms=len(trinity_llms)=10 → consensus always deflated (4/10=0.28 instead of 4/6=0.67)
        voting_llms = sum(1 for name in ['BAYESIAN', 'TECHNICAL', 'CHART', 'RISK', 'SUPREME', 'SMART_MONEY'] 
                         if name in trinity_llms and trinity_llms[name].get('decision', 'HOLD') in ['BUY', 'SELL', 'HOLD'])
        num_llms = max(voting_llms, 6)  # At least 6 voting LLMs (SEK has 6 core)
        ...
        nova_dashboard.update(
            ...
            # ✨ LLM7 OCULUS - Data Quality Validator (Port 8662)
            ...
            # ✨ LLM8 CHRONOS - Timing Optimizer (Port 8663)
            ...
            # ✨ LLM9 PREDATOR - Execution Engine (Port 8664)
        ...
        try:
            ...
            bid = tick_data.get('bid', current_price - 0.005)
            ask = tick_data.get('ask', current_price + 0.005)
            ...
            # CRITICAL: Round to valid lot increments (0.01 for USDSEK)
            ...
            if consensus_score < 0.20 and trinity_decision in ['BUY', 'SELL']:
                # Check if 10-LLM conviction can substitute for missing TF consensus
                if llm_consensus_decision == trinity_decision and llm_consensus_confidence >= 55:
                    ...
                    self.logger.warning(f"[CONSENSUS_VETO] 🚫 BLOCKING: Consensus {consensus_score*100:.0f}% < 20% (reason: {consensus_reason}) - No 10-LLM conviction backup")
                    return {
                        'action': 'HOLD',
            ...
            # � FIX: Was HARDCODED 50% which BLOCKED all USDSEK trades (trinity stays 34-47%)
            # Now uses Config.MIN_TRINITY_CONFIDENCE (35%) which is still strict but reachable
            min_trinity_gate = Config.MIN_TRINITY_CONFIDENCE
            if trinity_confidence >= min_trinity_gate:
                self.logger.info(f"[CONFIDENCE_CHECK] ✅ Trinity {trinity_confidence}% >= {min_trinity_gate}% + Consensus {consensus_score*100:.0f}% - proceeding with {trinity_decision}")
                pass  # Continue to decision execution below
            else:
                # 🚫 BLOCK: Any confidence below Config threshold is rejected
                self.logger.warning(f"[CONFIDENCE_VETO] 🚫 Blocking {trinity_decision}: Trinity {trinity_confidence}% < {min_trinity_gate}% minimum (consensus {consensus_score*100:.0f}%)")
                return {
                    'action': 'HOLD',
                    'reason': 'insufficient_trinity_confidence',
                    'trinity_confidence': trinity_confidence,
                    'threshold_required': min_trinity_gate,
                    'consensus_score': consensus_score
                }
            
            # ============ CONSOLIDATION TRAP DETECTION (RELAXED FOR M1) ============
            # For M1 forex scalping, we can trade even in tight ranges - adjusted for USDSEK
            atr_value = calculate_atr(highs, lows, closes, period=14) if len(highs) >= 14 else 0.005
            ...
            if atr_value < 0.003:  # < 3 pips (0.003) = dead market for USDSEK
                self.logger.warning(f"[CONSOLIDATION_VETO] 🚫 Blocking: ATR={atr_value/0.001:.1f}pips < 3 (broken market)")
            ...
            if trinity_decision in ['BUY', 'SELL']:
                ...
                if trinity_decision == 'BUY':
                    ...
                    if not trend_aligned:
                        ...
                        trinity_confidence *= 0.80  # 🔧 FIX: Was 0.65 (35%!) → 0.80 (20%) — cascading penalties killed all entries
                elif trinity_decision == 'SELL':
                    ...
                    if not trend_aligned:
                        ...
                        trinity_confidence *= 0.80  # 🔧 FIX: Was 0.65 → 0.80 — prevent cascading penalty death
                
                # WHALE TRAP: Reduce confidence if detected (don't block, just penalize)
                if is_trap and trap_confidence > 60:
                    trinity_confidence *= 0.85  # 🔧 FIX: Was 0.70 (30%!) → 0.85 (15%) — prevent cascading kills
                ...
                if Config.is_excalibur_active() and hasattr(self, 'excalibur_analyzer') and self.excalibur_analyzer:
                    ...
                    excalibur_indicators = {
                        ...
                        'atr_pips': atr_value / 0.001 if atr_value else 0,  # Convert to pips (USDSEK: 1 pip = 0.001)
                    ...
                    if NEWS_INTELLIGENCE_AVAILABLE:
                        try:
                            news_bias = get_trading_bias('USDSEK')
                ...
                if self.supreme_consciousness and gate_attack:
                    try:
                        ...
                        # CONFIDENCE ADJUSTMENT from Supreme
                            ...
                            self.logger.info(f"[🧠 SUPREME] ⚠️ Confidence adjusted: {old_conf:.0f}% → {gate_confidence:.0f}%")
            ...
            # 🔧 FIX v7: USDSEK exotic - wider mins for better R:R vs spread
            MIN_TP_DIST = 0.020   # 20 pips minimum (> 3x the ~6-pip spread)
            MIN_SL_DIST = 0.015   # 15 pips minimum (USDSEK exotic)
            SAFETY = 0.005        # 5 pips safety buffer
            ...
            if trinity_response and isinstance(trinity_response, dict) and not trinity_response.get('_is_fallback', False):
                ...
                # 🔧 FIX: VALIDATE Trinity values before using
                if trinity_tp > 0 and trinity_sl > 0:
                    ...
                    if actual_tp_dist >= MIN_TP_DIST and actual_sl_dist >= MIN_SL_DIST:
                        tp_price = trinity_tp
                        sl_price = trinity_sl
                        tp_dist = trinity_tp_dist if trinity_tp_dist > 0 else int(actual_tp_dist / 0.001)
                        sl_dist = trinity_sl_dist if trinity_sl_dist > 0 else int(actual_sl_dist / 0.001)
                        self.logger.info(f"[TP/SL] ✓ Using Trinity: TP={tp_price:.5f} ({actual_tp_dist/0.001:.1f}pips), SL={sl_price:.5f} ({actual_sl_dist/0.001:.1f}pips)")
                    else:
                        # Trinity values too small - will recalculate locally
                        self.logger.warning(f"[TP/SL] ⚠️ Trinity values TOO SMALL: TP_dist={actual_tp_dist/0.001:.1f}p (min={MIN_TP_DIST/0.001:.0f}p), SL_dist={actual_sl_dist/0.001:.1f}p (min={MIN_SL_DIST/0.001:.0f}p) - recalculating...")
                        tp_price = None
                        sl_price = None
            ...
            if tp_price is None or sl_price is None:
                ...
                # Get spread from tick data - FIX v7: convert points to PRICE units
                # EA sends: (ask-bid)/point = points (e.g., 602)
                # USDSEK 5-digit: point=0.00001, so spread_price = 602 * 0.00001 = 0.00602
                tick_data = genome.get('tick_data', {})
                spread = tick_data.get('spread', 10) * 0.00001  # Convert points to price units
            ...
            try:
                ...
                # 🔧 FIX: Use only VOTING LLMs count, not all 10 (LLM7/8/9/NOVA don't vote BUY/SELL)
                voting_llms = sum(1 for name in ['BAYESIAN', 'TECHNICAL', 'CHART', 'RISK', 'SUPREME', 'SMART_MONEY'] 
                                 if name in trinity_llms and trinity_llms[name].get('decision', 'HOLD') in ['BUY', 'SELL', 'HOLD'])
                num_llms = max(voting_llms, 6)
                if num_llms > 0 and buy_votes > 0:
                    consensus_score = min(1.0, (buy_votes / num_llms) * (total_conf / (buy_votes * 100.0)))
                else:
                    consensus_score = trinity_confidence / 100.0 if trinity_confidence else 0.0
                ...
                nova_dashboard.update(
                    ...
                    market_state=getattr(market_analysis_result.market_state, 'value', market_analysis_result.market_state) if market_analysis_result else 'UNKNOWN',  # 🔧 FIX CAT-108
                    trend_strength=getattr(market_analysis_result.trend_strength, 'value', market_analysis_result.trend_strength) if market_analysis_result else 1,  # 🔧 FIX CAT-108
    ...
    def play_trade_sound(self, decision, symbol):
        """Play audio alert via sound.py server on port 8670
        ...
        try:
            ...
            # Send to sound.py server on port 8670 with 4-byte length header (little endian)
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            sock.connect(('127.0.0.1', 8670))
            ...
            self.logger.warning(f"[AUDIO] sound.py not running on 127.0.0.1:8670")
        except socket.timeout:
    ...
    def _accumulate_attack_intelligence(self, trinity_llms, trinity_decision, trinity_confidence, 
        ...
        try:
            ...
            # FIX v7: Increased dominance threshold from 1.2x to 1.5x to prevent direction flipping
            if len(self.attack_prep['llm_votes_history']) >= 3:
                ...
                if total_buy_weight > total_sell_weight * 1.5:
                    self.attack_prep['optimal_direction'] = 'BUY'
                elif total_sell_weight > total_buy_weight * 1.5:
                    self.attack_prep['optimal_direction'] = 'SELL'
                else:
                    # No clear dominance - keep existing direction or use Trinity
                    # FIX v7: Don't flip to live Trinity on weak signals (causes whipsaw)
                    if self.attack_prep.get('optimal_direction') in ['BUY', 'SELL']:
                        pass  # Keep existing stable direction
                    else:
...
class AdvancedIPC:
    ...
    def _play_sound_alert(self, alert_type, symbol='USDSEK'):
        """Play TP/SL audio alert via sound.py server"""
        try:
            # 🔧 FIX: Use Config.SOUND_PORT instead of hardcoded 8670
            sound_port = getattr(Config, 'SOUND_PORT', 8670)
    ...
    def _handle_executor_connection(self, client, addr):
        ...
        try:
            ...
            while len(len_header) < 4:
                chunk = client.recv(4 - len(len_header))
                if not chunk:
                    self.logger.warning(f"[Executor] Invalid header from {addr} (connection closed)")
                    return
                len_header += chunk
            if len(len_header) < 4:
                ...
                if request.get('type') == 'handshake':
                    ...
                    with self.quimera_lock:
                        self.quimera_connections[('executor', addr)] = {
                            ...
                            'type': 'executor',
                            'last_ping': time.time(),
                            'symbol': request.get('symbol', 'USDSEK')
                        }
    ...
    def _handle_genome(self, client, addr):
        ...
        try:
            client.settimeout(120.0)  # 120 sec timeout for persistent connections (match CNH/EURUSD)
            ...
            while True:
                try:
                    ...
                    # Respond to handshakes on injector port (MUST send ACK like CNH/EURUSD)
    ...
    def _normalize_genome(self, genome):
        ...
        MQL5 sends: {"type": "genome", "bid": 97000, "ask": 97001, "symbol": "USDSEK", ...}
        Python expects: {"type": "genome", "tick_data": {"bid": 97000, "ask": 97001}, "metadata": {"symbol": "USDSEK"}, ...}
        """
    ...
    def _validate_tick(self, genome):
        """Validate genome structure - ULTRA LENIENT for USDSEK data"""
        try:
            genome_type = genome.get('type', '')
            
            # ⭐ Accept both standard and Multi-TF genome types from gbpquimera
            valid_types = [
            ...
            if genome_type in valid_types:
                ...
                try:
                    ...
                    # Acceptable range for USDSEK
                    # 🔧 FIX: USDSEK trades ~10.5-11.0 SEK per USD!
                    # Was 0.5-3.0 (copied from EURUSD) — rejected ALL valid data!
                    # Accept range: 5.0 - 15.0 to be safe for variations
                    mid_price = (bid + ask) / 2.0
                    
                    # USDSEK forex price range (5.0 - 15.0)
                    if 5.0 <= mid_price <= 15.0:
                        return True
                    
                    # Price out of realistic range for USDSEK
                    self.logger.warning(f"[Validator] USDSEK price out of range: {mid_price:.5f} (expected 5.0-15.0)")
                    return False
    ...
    def send_to_trinity(self, genome):
        ...
        for attempt in range(1):  # FAST MODE: Single attempt to fail quickly
            ...
            try:
                ...
                msg = json.dumps(genome, default=str)  # 🔧 FIX CAT-102
    ...
    def send_decision_to_quimera(self, decision, tick_id):
        ...
        try:
            ...
            if not executor_conn:
                self.logger.error(f"[Quimera TX] ❌ NO EXECUTOR CONNECTION - Quimera not connected to port {Config.EXECUTOR_PORT}")
            ...
            # 1. Validate price is within realistic bounds (USDSEK: 5.0-15.0 range)
            ...
            # 🔬 STEP 1: VALIDATE ENTRY PRICE IS REALISTIC FOR USDSEK
            # 🔧 FIX: USDSEK trades ~10.5 SEK/USD! Was 0.5-3.0 (EURUSD range)
            valid_forex_range = 5.0 <= entry_price <= 15.0  # USDSEK pricing
            
            if not valid_forex_range:
                self.logger.error(f"[EINSTEIN] ❌ Entry price {entry_price:.5f} out of realistic range for USDSEK (expected 5.0-15.0)")
                return False
            ...
            if abs(entry_price - original_entry) > 0.010:  # > 10 pips change (USDSEK: 1 pip = 0.001)
                self.logger.warning(f"[EINSTEIN] ⚠️ Entry price changed significantly: original={original_entry:.5f}, current={entry_price:.5f} (diff={abs(entry_price-original_entry)/0.001:.1f}pips)")
            
            # Use ATR-based approach (1.5x for TP, 1.0x for SL)
            atr_val = decision.get('atr', 0.020)  # Default 20 pips for USDSEK exotic
            atr_original = atr_val
            
            # ⚠️ CRITICAL: Clamp ATR to M1 USDSEK realistic range
            # USDSEK M1 ATR typical: 0.005-0.060 (5-60 pips, 1 pip = 0.001)
            atr_val = max(0.005, min(0.060, atr_val))
            if atr_val != atr_original:
                self.logger.debug(f"[EINSTEIN] ATR clamped: {atr_original:.5f} → {atr_val:.5f}")
            
            # ⭐ SCALPING TP/SL: ATR × 1.5 (TP) and ATR × 1.0 (SL)
            tp_distance = atr_val * 1.5
            sl_distance = atr_val * 1.0
            
            # ═══════════════════════════════════════════════════════════════════════════
            # MT5 STOPS_LEVEL REQUIREMENT: Broker requires minimum distance from ASK/BID
            # 🔧 FIX v7: USDSEK exotic - wider minimums for better R:R
            # ═══════════════════════════════════════════════════════════════════════════
            MIN_TP_DISTANCE = 0.020   # 20 pips minimum TP (USDSEK exotic)
            MIN_SL_DISTANCE = 0.015   # 15 pips minimum SL (USDSEK exotic)
            SAFETY_MARGIN = 0.005     # 5 pips extra safety buffer for ASK/BID slippage
            
            tp_distance = max(MIN_TP_DISTANCE, tp_distance)
            sl_distance = max(MIN_SL_DISTANCE, sl_distance)
            
            # ⚠️ HARD CAP: Maximum for USDSEK M1 scalping
            tp_distance = min(tp_distance, 0.100)  # Max 100 pips TP
            sl_distance = min(sl_distance, 0.060)  # Max 60 pips SL
            
            self.logger.info(f"[EINSTEIN] ✅ ATR-Based: TP={tp_distance/0.001:.0f}pips SL={sl_distance/0.001:.0f}pips")
            ...
            if actual_tp_distance < MIN_TP_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ⚠️ TP distance ({actual_tp_distance/0.001:.1f}pips) < MIN ({MIN_TP_DISTANCE/0.001:.0f}pips), adjusting...")
                if action == 'BUY':
            ...
            if actual_sl_distance < MIN_SL_DISTANCE:
                self.logger.warning(f"[EINSTEIN] ⚠️ SL distance ({actual_sl_distance/0.001:.1f}pips) < MIN ({MIN_SL_DISTANCE/0.001:.0f}pips), adjusting...")
                if action == 'BUY':
            ...
            tp_pips = actual_tp_distance / 0.001  # Convert to pips (USDSEK: 1 pip = 0.001)
            sl_pips = actual_sl_distance / 0.001
            ...
            tp_pips_final = tp_distance_final / 0.001
            sl_pips_final = sl_distance_final / 0.001
            
            # 🚨 ASSERT: Distances meet minimum requirements
            if tp_distance_final < MIN_TP_DISTANCE or sl_distance_final < MIN_SL_DISTANCE:
                self.logger.error(f"[EINSTEIN] 🚨 CRITICAL: Final distances BELOW minimum! TP={tp_distance_final/0.001:.1f}pips (min={MIN_TP_DISTANCE/0.001:.0f}), SL={sl_distance_final/0.001:.1f}pips (min={MIN_SL_DISTANCE/0.001:.0f})")
                return False
            
            self.logger.info(f"[EINSTEIN] ✅ VALIDATED: {action} Entry={entry_price:.5f} TP={tp_price:.5f}({tp_pips_final:.0f}p) SL={sl_price:.5f}({sl_pips_final:.0f}p)")
...
class QuantumCore:
    ...
    def _build_closes_with_live_tick(self, history, current_price):
        ...
        if closes and current_price > 0:
            ...
            if gap > 0.01:  # More than 0.01 (1 pip for USDSEK) difference
    ...
    def __init__(self):
        ...
        # FIX M-SEK-01: NOVAMarketStateDetector created ONCE here instead of on every
        # process_genome() call (~1/sec). analyze() receives the genome each time so
        # sharing a single instance across threads is safe (stateless between calls).
    ...
    def _print_banner(self):
        ...
        logger.info("=" * 110)
        if is_excalibur:
            logger.info("⚔️  NOVA EXCALIBUR KNIGHT v18.00 - USDSEK ELITE MOMENTUM SNIPER")
            logger.info("🛡️  \"En el fragor eterno de la batalla, aguardo el susurro del destino para asestar el golpe inmortal\"")
        else:
            logger.info("🎯 NOVA SNIPER SCALPING v18.00 - USDSEK FRANCOTIRADOR CONSCIOUSNESS (120s cooldown, 75%+ win rate)")
        logger.info("=" * 110)
        logger.info("")
        ...
        if is_excalibur:
            logger.info("⚔️ EXCALIBUR KNIGHT FEATURES:")
    ...
    def _send_pings_to_quimera(self):
        ...
        The MT5 EA (GBPquimera_bridge.mq5) expects to receive PING messages
        ...
        while self.running:
            try:
                time.sleep(2.0)  # Send ping every 2s (EA bunker timeout is 60s)
    ...
    def _transform_quimera_to_trinity(self, genome):
        ...
        try:
            ...
            valid_types = [
                ...
                'genome', 'tick'  # ⭐ Bridge format from sekquimera_bridge.mq5
            ]
            ...
            symbol = genome.get('symbol') or metadata.get('symbol', 'USDSEK')
            ...
            # CRITICAL: Validate price is in reasonable range for USDSEK FOREX
            # USDSEK trades ~10.5 SEK/USD, historical range 5.0-15.0
            if close_price > 0:
                if close_price < 5.0 or close_price > 15.0:
                ...
                if close_price < 8.0 or close_price > 12.0:
            ...
            if history and len(history) > 0:
                ...
                self.logger.debug(f"[Transform] Built synthetic history from {len(prices)} prices: {len(history)} bars with OHLC variation")
            else:
            ...
            if len(history) < 10:
                ...
                self.logger.debug(f"[Transform] ℹ️ Extended history with {remaining_bars} bars from current bar (insufficient history)")
            ...
            if hasattr(self, 'price_buffer') and len(self.price_buffer) >= 14:
                ...
                if len(history) >= 20:
                    ...
                    self.logger.debug(f"[Transform] 🚀 STARTUP MODE: Using {len(recent_ticks)} live ticks (bars insufficient)")
            elif len(history) >= 14:
            ...
            # ═══════════════════════════════════════════════════════════════
            # PHASE 10 FIX: EXTRACT EA REAL INDICATORS (FROM MT5 CopyBuffer)
            # The EA sends real RSI/ATR/ADX/Stoch in the timeframes array
            # Priority: Use real EA indicators over synthetic calculation
            # Without this, synthetic RSI oscillates 0↔100 constantly,
            # triggering EMERGENCY OVERRIDE that flips direction randomly
            # ═══════════════════════════════════════════════════════════════
            ea_rsi = None
            ea_atr = None
            ea_adx = None
            ...
            if ea_rsi is not None and ea_atr is not None:
                ...
                self.logger.warning(f"[Transform] ⚠️ No EA indicators, falling back to synthetic calculation")
                ...
                # ============================================================
                # CORRECT ADX CALCULATION (Average Directional Index) - SYNTHETIC ONLY
                ...
                # ═══ PHASE 10: SAFETY GUARD FOR SYNTHETIC INDICATORS ═══
                # Synthetic RSI from tick data produces 0/100 noise - reset to neutral
                ATR_FLOOR_USDSEK = 0.003  # ~3 pips minimum for USDSEK
                atr_at_floor = False
                if atr_calculated < ATR_FLOOR_USDSEK:
                    self.logger.warning(f"[Transform] ⚠️ ATR floor: {atr_calculated:.6f} → {ATR_FLOOR_USDSEK} ({ATR_FLOOR_USDSEK*10000:.0f} pips minimum)")
                    atr_calculated = ATR_FLOOR_USDSEK
                    atr_at_floor = True
            ...
            # Bollinger Bands - Use EA values if available
            ...
            if ea_stoch_k is not None:
                ...
                self.logger.debug(f"[Transform] Using EA Stochastic: K={stoch_k_calc:.1f} D={stoch_d_calc:.1f}")
            else:
            ...
            # 🔧 PHASE 10 FIX: Also populate price_data dict for downstream consumers
            # SupremeConsciousness, QuantumStateMachine etc. read from price_data.close
            # 🔧 FIX: Add 'history' key - process_with_full_consciousness reads price_data.history
            # Without this key, DATA_CHECK always returns "Insufficient bars: 0/10" → permanent HOLD
            transformed['price_data'] = {
                'close': closes_list,
                'open': genome.get('opens', [open_price] * n_closes),
                'high': genome.get('highs', [high] * n_closes),
                'low': genome.get('lows', [low] * n_closes),
                'volume': genome.get('volumes', [volume] * n_closes),
                'history': history[-20:]  # CRITICAL: List of bar dicts for process_with_full_consciousness
            }
    ...
    def process_genome(self, genome):
        ...
        try:
            ...
            if symbol == 'USDSEK' and genome.get('symbol'):
                symbol = genome.get('symbol')
            genome_type = genome.get('type', '')
            ...
            if genome_type in valid_genome_types:
                ...
                logger.debug(f"[Process] Processing {genome_type} for {symbol}")
                ...
                if sl_history:
                    ...
                    logger.debug(f"[ECOSYSTEM→Trinity] SL_History: {len(sl_history)} events, latest: {latest_sl.get('reason', 'N/A')}")
                
                if velocity_data:
                    ma_delta = velocity_data.get('ma_fast_delta', 0)
                    rsi_delta = velocity_data.get('rsi_delta', 0)
                    adx_delta = velocity_data.get('adx_delta', 0)
                    logger.debug(f"[ECOSYSTEM→Trinity] Velocity: MA={ma_delta:+.2f} RSI={rsi_delta:+.1f} ADX={adx_delta:+.1f}")
                
                if timeframe_analysis:
                    tf_votes = {tf.get('timeframe'): tf.get('signal', '?') for tf in timeframe_analysis if 'timeframe' in tf}
                    logger.debug(f"[ECOSYSTEM→Trinity] Timeframe Votes: {tf_votes}")
            else:
            ...
            # ═══════════════════════════════════════════════════════════════════════════════
            # 🛡️ TRADING SAFEGUARDS — Prevent catastrophic losses
            # ═══════════════════════════════════════════════════════════════════════════════

            # GUARD 1: ATR=0 DATA CORRUPTION — MT5 sent garbage genome, refuse to trade
            # FIX C-SEK-01: counter moved to stats dict and protected under stats_lock.
            # Without lock, 16 concurrent threads could all read 0 and all write 1 so
            # the counter never reaches 3 and the guard is silently bypassed.
            _ind = genome.get('indicators', {}).get('current', {})
            _atr_val = float(_ind.get('atr', 0) or 0)
            if _atr_val == 0.0:
                ...
                if _atr_zero_count_val >= 3:
                    logger.debug(f"[GUARD] \U0001f6d1 ATR=0 x{_atr_zero_count_val} consecutive \u2014 corrupted genome, skipping")
                    return None
            else:
                with self.ipc.stats_lock:
                    self.ipc.stats['_atr_zero_count'] = 0  # FIX C-SEK-01: reset under lock

            # GUARD 3: CONSECUTIVE LOSS HARD HALT — 5+ losses in a row = trading pause
            # FIX A-SEK-02: Guard was absent in sekquantum_core.py — mirrors quantum_core.py
            ...
            if _consec >= Config.MAX_CONSECUTIVE_LOSSES:
                ...
                if time.time() < _halt_until:
                    ...
                    _resume = _dt_guard.fromtimestamp(_halt_until).strftime('%H:%M:%S')
                    logger.warning(f"[GUARD] \U0001f6a8 LOSS HALT active: {_consec} consecutive losses \u2014 resumes at {_resume} UTC")
                    return None
                elif _halt_until > 0:
                    with self.ipc.stats_lock:
                        self.ipc.stats['consecutive_losses'] = 0
                    self._loss_halt_until = 0
                    logger.warning(f"[GUARD] \u2705 LOSS HALT expired \u2014 trading resumed")
                else:
                    self._loss_halt_until = time.time() + Config.LOSS_HALT_DURATION_SECONDS
                    logger.warning(f"[GUARD] \U0001f6a8 Loss halt TRIGGERED: {_consec} losses \u2014 pausing {Config.LOSS_HALT_DURATION_SECONDS}s")
                    return None

            # GUARD 4: DAILY LOSS LIMIT — Hard stop when daily drawdown exceeds threshold
            # FIX A-SEK-03: Guard was absent in sekquantum_core.py — mirrors quantum_core.py
            try:
                _today_pnl = self.ipc.stats.get('today_pnl', 0.0)
                _account_bal = self.ipc.stats.get('account_balance', 10000.0) or 10000.0
                _loss_pct = abs(min(0.0, _today_pnl)) / _account_bal * 100
                if _loss_pct >= Config.MAX_DAILY_LOSS_PERCENT:
                    logger.warning(f"[GUARD] \U0001f4b8 DAILY LOSS LIMIT HIT: -{_loss_pct:.1f}% (limit={Config.MAX_DAILY_LOSS_PERCENT}%) \u2014 no new trades today")
                    return None
            except Exception:
                pass  # Non-blocking: guard failure must not crash processing

            # ═══════════════════════════════════════════════════════════════════════════════
            # 🚀 NOVA-MSDA: Market State Detection Agent - Audit missing information
            # ═══════════════════════════════════════════════════════════════════════════════
            # FIX M-SEK-01: Use singleton created in __init__ instead of new object per call.
            ...
            logger.debug(f"[NOVA-MSDA] {symbol}: Quality={quality_score:.0f}% → Multiplier={quality_mult:.2f}x ({len(alerts)} alerts)")
            if alerts:
            ...
            if bid_from_tick > 0 and ask_from_tick > 0:
                ...
                # Calculate bid/ask from current price with typical USDSEK spread (0.20 pips)
                current_price = (
                    tick_data.get('last', 0) or 
                    tick_data.get('close', 0) or 
                    tick_data.get('price', 0) or 
                ...
                if current_price > 0:
                    spread = 0.005  # Half spread for USDSEK (typical spread ~10 pips = 0.010)
            ...
            self.logger.debug(f"[Trinity-Send] 🚀 Sending to Trinity with bid={final_bid:.5f} ask={final_ask:.5f}")
            ...
            # 🔧 PHASE 10 FIX: Guard against ATR=0 (synthetic broken indicators)
            # Synthetic RSI from tick data produces 0/100 noise - don't trust it
            # ═══════════════════════════════════════════════════════════════════════════
            stoch_k_val = float(indicators.get('stoch_k', 50))
            stoch_d_val = float(indicators.get('stoch_d', 50))
            atr_val_check = float(indicators.get('atr', 0))
            ...
            # 🔧 PHASE 10: Guard against ATR=0 (synthetic broken data)
            if rsi_val > 90 and stoch_k_val > 95 and atr_val_check > 0.001:
                if trinity_decision != 'SELL':
            ...
            # 🔧 PHASE 10: Guard against RSI=0 + ATR=0 (no data, NOT real oversold)
            elif rsi_val < 10 and stoch_k_val < 5 and atr_val_check > 0.001:
                if trinity_decision != 'BUY':
            ...
            # 🔧 PHASE 10: Also require ATR > 0.001 for reliable data
            elif rsi_val > 85 and adx_val > 40 and ma_fast_val < ma_slow_val and atr_val_check > 0.001:
                if trinity_decision == 'BUY':
            ...
            # 🔧 PHASE 10: ATR guard against synthetic noise
            elif rsi_val > 80 and trinity_decision == 'BUY' and atr_val_check > 0.001:
            ...
            # 🔧 PHASE 10: ATR guard against synthetic noise
            elif rsi_val < 20 and trinity_decision == 'SELL' and atr_val_check > 0.001:
            ...
            # FIX C-SEK-02: Track whether THIS invocation incremented open_positions.
            # Needed for safe rollback on Kraken failure — avoids decrementing a counter
            # incremented by a different concurrent thread that succeeded.
                ...
                with self.ipc.stats_lock:
                    ...
                    if decision['action'] == 'CLOSE':
                        ...
                        if decision['action'] != current_direction:
                            ...
                            # FIX A-SEK-05: direction_poison blocks repeated SL losses in one direction
                            ...
                            if _remaining_poison > 0:
                                logger.warning(f"[DIR_POISON] \U0001f6ab {decision['action']} direction blocked for {_remaining_poison}s more (2+ SL hits \u2014 protecting capital)")
                                decision_for_kraken = decision.copy()
                                decision_for_kraken['action'] = 'HOLD'
                                ...
                                _did_increment = True  # FIX C-SEK-02: mark that we incremented
                        ...
                        # ✅ No positions open - First trade — check direction poison first
                        # FIX A-SEK-05: direction_poison blocks repeated SL losses in one direction
                        _poison_expiry = self.ipc.stats.get(f'dir_poison_until_{decision["action"]}', 0)
                        _remaining_poison = int(_poison_expiry - time.time())
                        if _remaining_poison > 0:
            ...
            if decision_for_kraken and decision_for_kraken['action'] != 'HOLD':
                ...
                if kraken_success:
                    # FIX A-SEK-01: Single timestamp, both writes atomic under stats_lock.
                    # Two separate time.time() calls without lock produce inconsistent
                    # timestamps when 16 concurrent threads execute this block in parallel.
                    _now = time.time()
                    with self.ipc.stats_lock:
                        self.last_trade_time = _now
                        self.engine.last_trade_time = _now  # also update engine cooldown
                    ...
                    try:
                        ...
                        state_key = self.engine.rl_engine.discretize_state(
                            ...
                            volatility=genome.get('atr', 0.010),
                    ...
                    # FIX C-SEK-02: Rollback ONLY if THIS invocation incremented open_positions.
    ...
    def run(self):
        ...
        last_position_reconcile = time.time()  # FIX A-SEK-04
        start_time = time.time()
        
        # Start live dashboard - ULTRA FAST refresh for real-time visual updates (same as EURUSD)
        nova_dashboard.start_live(interval=0.1)  # ⭐ 100ms para movimiento dinámico y visual
        ...
        try:
            while self.running:
                ...
                trinity_offline = (last_trinity_response > 0) and (time_since_trinity > 5)  # 🔧 FIX CAT-107
                ...
                if time.time() - last_stats_write > 1:
                    try:
                        ...
                        # FIX M-SEK-02: Atomic write via temp file + os.replace().
                        # Direct open('quantum_stats.json', 'w') truncates the file before
                        # writing; a crash/disk-full mid-write leaves corrupt JSON that crashes
                        # the external dashboard reader. os.replace() is atomic on Windows + POSIX.
                ...
                # FIX A-SEK-04: POSITION STALENESS RECONCILIATION (every 60s)
                # FIX RECONCILE v2 (every 60s) — 3 señales para detectar posiciones fantasma
                # Antes: last_trade_time nunca actualizado → stale_seconds=0 → nunca reseteaba
                if time.time() - last_position_reconcile > 60:
                    ...
                    with self.ipc.stats_lock:
                        ...
                        # Señal 2: >300s desde última entrada + bridge silencioso >120s
                        ...
                        # Señal 3: startup stale — sin last_action_time >600s
    ...
    def _update_dashboard_from_genome(self, genome):
        ...
        try:
            # Extract metadata
            metadata = genome.get('metadata', {})
            symbol = metadata.get('symbol', 'USDSEK')
            ...
            # ── Nova Dashboard bridge: copy market snapshot into ipc.stats ──
            # quantum_stats.json (written every 1s) will include live indicator data
            # so the external nova_dashboard.py process can read and display it.
            try:
                _bb_width = round((bb_upper - bb_lower) / bb_middle * 100, 4) if bb_middle > 0 else round(bb_upper - bb_lower, 6)
                with self.ipc.stats_lock:
                    self.ipc.stats.update({
                        'genome_counter': self.engine.genome_counter,
                    ...
                    # Mirror LLM votes from terminal dashboard
                    ...
                    if isinstance(_pats_raw, list):
                        _pnames = []
                        for _p in _pats_raw[:5]:
                            if isinstance(_p, dict):
                                _pnames.append(str(_p.get('type', _p.get('name', ''))).upper())
                            elif isinstance(_p, str):
                                _pnames.append(_p.upper())
                        self.ipc.stats['patterns_text'] = ', '.join(_pnames)[:60] if _pnames else ''
                    else:
                        self.ipc.stats['patterns_text'] = str(_pats_raw)[:60]
                    # ML
                    self.ipc.stats['ml_prediction'] = str(_d.get('ml_prediction', 'NEUTRAL') or 'NEUTRAL')
                    self.ipc.stats['ml_confidence'] = round(float(_d.get('ml_confidence', 0.0) or 0.0), 3)
                    # LLM7/8/9
                    ...
                    if _wr > 1.0: _wr = _wr / 100.0
                    _tt = int(_d.get('total_trades', 0) or 0)
                    _wins   = int(round(_wr * _tt)) if _tt else 0
                    _losses = max(0, _tt - _wins)
                    self.ipc.stats['wins']      = _wins
                    self.ipc.stats['losses']    = _losses
                    self.ipc.stats['winrate']   = round(_wr * 100, 1)
                    self.ipc.stats['pnl_today'] = round(float(
                        _d.get('today_pnl', self.ipc.stats.get('today_pnl', 0.0)) or 0.0), 2)
...
def _acquire_sek_singleton():
    """Prevent multiple instances using PID file"""
    import os as _os2, atexit as _atexit2
    _pid_file = _os2.path.join(_os2.path.dirname(__file__), 'sekquantum_core.pid')
    if _os2.path.exists(_pid_file):
        try:
            ...
            if _ps2.pid_exists(_old_pid) and _ps2.Process(_old_pid).is_running():
                print(f'[SINGLETON] SEK instance already running (PID={_old_pid}). Exiting.')
                return False
        except Exception:
            pass
